...
/// AsyncHttpQueue::FailedUrl (IDA 0x2fd150).
...
/// `RBX::AsyncHttpQueue` request queue (IDA 0x2faa84 et al.).
#[derive(Clone, Debug, Default)]
...
/// `std::map<Name, HttpContentType>` (IDA 0x259018 et al.).
#[derive(Clone, Debug, Default)]
...
pub fn stub_258dc8(destroy: &mut dyn FnMut()) {
    // IDA 0x258dc8: Thn36 D1 adjusts then Instance dtor.
    destroy();
}
...
pub fn stub_258dd0(destroy: &mut dyn FnMut(), free: &mut dyn FnMut()) {
    // IDA 0x258dd0: Thn36 D0 adjusts then dtor + delete.
    destroy();
    free();
}
...
pub fn stub_258e74(matches: bool, value: usize) -> Option<usize> {
    // IDA 0x258e74: any_cast — type match ? value+1 : bad path (below truncation).
    if matches {
        Some(value)
    } else {
        None
...
#[doc(alias = "RBX::HttpService::HttpContentType & rbx::any_cast<RBX::HttpService::HttpContentType &,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
pub fn stub_258ecc(matches: bool, value: usize) -> usize {
    // IDA 0x258ecc: any_cast& — mismatch throws bad_cast (below truncation).
    if !matches {
        panic!("bad_cast");
    }
    value
...
pub fn stub_258fbc(v: &mut Vec<i32>, n: usize, fill: i32) {
    // IDA 0x258fbc: vector resize (fill-insert when growing).
    v.resize(n, fill);
}
...
pub fn stub_258ff0(v: &mut Vec<i32>, x: i32) {
    // IDA 0x258ff0: vector push_back.
    v.push(x);
}
...
pub fn stub_259018(map: &mut ContentTypeMap, key: String) -> i32 {
    // IDA 0x259018: map operator[] — find or default-insert.
    *map.entries.entry(key).or_insert(0)
}
...
pub fn stub_259070(map: &mut ContentTypeMap, key: String, value: i32) -> bool {
    // IDA 0x259070: hinted unique insert; false when present.
...
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType> const&)")]
pub fn stub_259124(map: &mut ContentTypeMap, key: String, value: i32) {
    // IDA 0x259124: hinted insert.
    map.entries.insert(key, value);
}
...
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType>>>::_M_insert_unique(std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType> const&)")]
pub fn stub_25917c(map: &mut ContentTypeMap, key: String, value: i32) -> bool {
    // IDA 0x25917c: unique insert; false when present.
    if map.entries.contains_key(&key) {
        return false;
    }
    map.entries.insert(key, value);
...
pub fn stub_2591e4(v: &mut Vec<i32>, pos: usize, x: i32) {
    // IDA 0x2591e4: vector insert_aux.
    if pos <= v.len() {
        v.insert(pos, x);
    } else {
        v.push(x);
...
pub fn stub_2592c8(count: usize, alloc: &mut dyn FnMut(usize) -> usize) -> usize {
    // IDA 0x2592c8: throw bad_alloc when huge else operator new.
    if count >= 0x40000000 {
        panic!("std::bad_alloc");
    }
    alloc(count)
...
pub fn stub_2592e0(dst: &mut Vec<i32>, src: &[i32]) {
    // IDA 0x2592e0: copy_backward (below truncation).
...
pub fn stub_25931c(v: &mut Vec<i32>, pos: usize, n: usize, x: i32) {
    // IDA 0x25931c: fill-insert n copies.
...
pub fn stub_2594ac(slot: usize, init: &mut dyn FnMut(usize)) -> usize {
    // IDA 0x2594ac: BoundProp<bool> ctor (below truncation).
    init(slot);
    slot
}
...
pub fn stub_25963c() -> bool {
    // IDA 0x25963c: isReadOnly returns false.
    false
}
...
pub fn stub_259640() -> bool {
    // IDA 0x259640: isWriteOnly returns false.
    false
}
...
pub fn stub_259644(base: usize, offset: usize, read: &mut dyn FnMut(usize) -> bool) -> bool {
    // IDA 0x259644: getValue — byte at instance word.
    read(base.wrapping_add(offset))
}
...
pub fn stub_259650(current: bool, new: bool, changed: &mut dyn FnMut(bool)) -> bool {
    // IDA 0x259650: setValue — store + notify when changed (below truncation).
    if current != new {
        changed(new);
    }
    new
...
pub fn stub_2596a0(slot: usize, init: &mut dyn FnMut(usize)) -> usize {
    // IDA 0x2596a0: BoundFuncDesc ctor (below truncation).
    init(slot);
    slot
}
...
// type: int __fastcall(int, int, int)
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::HttpService,std::string ()(rbx_core::SharedPtr<boost::unordered::unordered_map<std::string,RBX::Reflection::Variant,boost::hash<std::string>,std::equal_to<std::string>,std::allocator<std::pair<std::string const,RBX::Reflection::Variant>>> const>),1>::declareSignature(char const*,RBX::Reflection::Variant)")]
pub fn stub_259838(declare: &mut dyn FnMut()) {
    // IDA 0x259838: declareSignature (below truncation).
    declare();
}
...
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::HttpService,std::string ()(rbx_core::SharedPtr<boost::unordered::unordered_map<std::string,RBX::Reflection::Variant,boost::hash<std::string>,std::equal_to<std::string>,std::allocator<std::pair<std::string const,RBX::Reflection::Variant>>> const>),1>::~BoundFuncDesc()")]
pub fn stub_259868(destroy: &mut dyn FnMut()) {
    // IDA 0x259868: BoundFuncDesc dtor (below truncation).
    destroy();
}
...
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::HttpService,std::string ()(rbx_core::SharedPtr<boost::unordered::unordered_map<std::string,RBX::Reflection::Variant,boost::hash<std::string>,std::equal_to<std::string>,std::allocator<std::pair<std::string const,RBX::Reflection::Variant>>> const>),1>::execute(RBX::Reflection::DescribedBase *,RBX::Reflection::FunctionDescriptor::Arguments &)const")]
pub fn stub_259984(execute: &mut dyn FnMut()) {
    // IDA 0x259984: execute (below truncation).
    execute();
}
...
pub fn stub_259a6c(call: &mut dyn FnMut()) {
    // IDA 0x259a6c: Call1Helper::call (below truncation).
    call();
}
...
pub fn stub_259dbc(slot: usize, init: &mut dyn FnMut(usize)) -> usize {
    // IDA 0x259dbc: BoundFuncDesc ctor (below truncation).
    init(slot);
    slot
}
...
// type: int __fastcall(int, int, int)
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::HttpService,rbx_core::SharedPtr<boost::unordered::unordered_map<std::string,RBX::Reflection::Variant,boost::hash<std::string>,std::equal_to<std::string>,std::allocator<std::pair<std::string const,RBX::Reflection::Variant>>> const> ()(std::string),1>::declareSignature(char const*,RBX::Reflection::Variant)")]
pub fn stub_259f34(declare: &mut dyn FnMut()) {
    // IDA 0x259f34: declareSignature (below truncation).
    declare();
}
...
// type: void __fastcall(_DWORD *)
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::HttpService,rbx_core::SharedPtr<boost::unordered::unordered_map<std::string,RBX::Reflection::Variant,boost::hash<std::string>,std::equal_to<std::string>,std::allocator<std::pair<std::string const,RBX::Reflection::Variant>>> const> ()(std::string),1>::~BoundFuncDesc()")]
pub fn stub_259f64(destroy: &mut dyn FnMut(), free: &mut dyn FnMut()) {
    // IDA 0x259f64: BoundFuncDesc D0 + operator delete.
    destroy();
    free();
}
...
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::HttpService,rbx_core::SharedPtr<boost::unordered::unordered_map<std::string,RBX::Reflection::Variant,boost::hash<std::string>,std::equal_to<std::string>,std::allocator<std::pair<std::string const,RBX::Reflection::Variant>>> const> ()(std::string),1>::execute(RBX::Reflection::DescribedBase *,RBX::Reflection::FunctionDescriptor::Arguments &)const")]
pub fn stub_25a030(execute: &mut dyn FnMut()) {
    // IDA 0x25a030: execute (below truncation).
    execute();
}
...
pub fn stub_25a170(call: &mut dyn FnMut()) {
    // IDA 0x25a170: Call1Helper::call (below truncation).
    call();
}
...
#[doc(alias = "RBX::Reflection::BoundYieldFuncDesc<RBX::HttpService,std::string ()(std::string,std::string,RBX::HttpService::HttpContentType),std::string,3>::BoundYieldFuncDesc(void (RBX::HttpService::*)(std::string,std::string,RBX::HttpService::HttpContentType,boost::function<void ()(std::string)>,boost::function<void ()(std::string)>),char const*,char const*,char const*,char const*,RBX::HttpService::HttpContentType,RBX::Security::Permissions,RBX::Reflection::Descriptor::Attributes)")]
pub fn stub_25a2f0(slot: usize, init: &mut dyn FnMut(usize)) -> usize {
    // IDA 0x25a2f0: BoundYieldFuncDesc ctor (below truncation).
    init(slot);
    slot
}
...
pub fn stub_25a540(declare: &mut dyn FnMut()) {
    // IDA 0x25a540: declareSignature (below truncation).
    declare();
}
...
// type: void __fastcall(_DWORD *)
#[doc(alias = "RBX::Reflection::BoundYieldFuncDesc<RBX::HttpService,std::string ()(std::string,std::string,RBX::HttpService::HttpContentType),std::string,3>::~BoundYieldFuncDesc()")]
pub fn stub_25a5a8(destroy: &mut dyn FnMut()) {
    // IDA 0x25a5a8: BoundYieldFuncDesc dtor (below truncation).
    destroy();
}
...
pub fn stub_25a68c(execute: &mut dyn FnMut()) {
    // IDA 0x25a68c: execute (below truncation).
    execute();
}
...
pub fn stub_25a958(get: &mut dyn FnMut() -> i32) -> i32 {
    // IDA 0x25a958: ArgHelper::getArg<HttpContentType> (below truncation).
    get()
}
...
pub fn stub_25aaec(value: i32, convert: &mut dyn FnMut(i32) -> i32) -> i32 {
    // IDA 0x25aaec: ArgHelper::try_enum (below truncation).
    convert(value)
}
...
pub fn stub_25ab40(slot: usize, init: &mut dyn FnMut(usize)) -> usize {
    // IDA 0x25ab40: BoundYieldFuncDesc ctor (below truncation).
    init(slot);
    slot
}
...
pub fn stub_25acb8(declare: &mut dyn FnMut()) {
    // IDA 0x25acb8: declareSignature (below truncation).
    declare();
}
...
// type: void __fastcall(_DWORD *)
#[doc(alias = "RBX::Reflection::BoundYieldFuncDesc<RBX::HttpService,std::string ()(std::string),std::string,1>::~BoundYieldFuncDesc()")]
pub fn stub_25ace8(destroy: &mut dyn FnMut(), free: &mut dyn FnMut()) {
    // IDA 0x25ace8: BoundYieldFuncDesc D0 + operator delete.
    destroy();
    free();
}
...
pub fn stub_25adb4(execute: &mut dyn FnMut()) {
    // IDA 0x25adb4: execute (below truncation).
    execute();
}
...
pub fn stub_2faa84(q: &mut AsyncHttpQueue) {
    // IDA 0x2faa84: AsyncHttpQueue ctor (below truncation).
    q.pending.clear();
    q.pool_threads = 0;
}
...
pub fn stub_2fad24(q: &mut AsyncHttpQueue, threads: u32, rebuild: &mut dyn FnMut(u32)) {
    // IDA 0x2fad24: rebuild pool when thread count differs.
    if q.pool_threads != threads {
        rebuild(threads);
        q.pool_threads = threads;
    }
}
...
pub fn stub_2fae00(reset: &mut dyn FnMut()) {
    // IDA 0x2fae00: resetStatsItem (below truncation).
    reset();
}
...
pub fn stub_2faf2c(q: &AsyncHttpQueue) -> usize {
    // IDA 0x2faf2c: mutex-guarded request queue size.
    q.pending.len()
}
...
pub fn stub_2faf68(destroy: &mut dyn FnMut(), free: &mut dyn FnMut()) {
    // IDA 0x2faf68: AsyncHttpQueue D0 + operator delete.
    destroy();
    free();
}
...
// type: void __fastcall(RBX::AsyncHttpQueue *__hidden this)
#[doc(alias = "RBX::AsyncHttpQueue::~AsyncHttpQueue()")]
pub fn stub_2fb008(destroy: &mut dyn FnMut()) {
    // IDA 0x2fb008: D1 thunk tail-calls D2.
    destroy();
}
...
// type: void __fastcall(RBX::AsyncHttpQueue *__hidden this)
#[doc(alias = "RBX::AsyncHttpQueue::~AsyncHttpQueue()")]
pub fn stub_2fb00c(destroy: &mut dyn FnMut()) {
    // IDA 0x2fb00c: AsyncHttpQueue D2 (below truncation).
    destroy();
}
...
pub fn stub_2fb2ac(beat: &mut dyn FnMut()) {
    // IDA 0x2fb2ac: onHeartbeat (below truncation).
    beat();
}
...
pub fn stub_2fb548(process: &mut dyn FnMut() -> i32) -> i32 {
    // IDA 0x2fb548: processRequests (below truncation).
    process()
}
...
pub fn stub_2fc440(dispatch: &mut dyn FnMut()) {
    // IDA 0x2fc440: dispatchGenericCallback (below truncation).
    dispatch();
}
...
pub fn stub_2fc6a8(dispatch: &mut dyn FnMut()) {
    // IDA 0x2fc6a8: dispatchCallback (below truncation).
    dispatch();
}
...
pub fn stub_2fc874(invoke: &mut dyn FnMut()) {
    // IDA 0x2fc874: InvokeAsyncCallback (below truncation).
    invoke();
}
...
pub fn stub_2fca04(q: &AsyncHttpQueue) -> bool {
    // IDA 0x2fca04: mutex-guarded empty check.
    q.pending.is_empty()
}
...
pub fn stub_2fcfb4(invoke: &mut dyn FnMut()) {
    // IDA 0x2fcfb4: callback (below truncation).
    invoke();
}
...
pub fn stub_2fd150(url: &str, now: f64) -> FailedUrl {
    // IDA 0x2fd150: FailedUrl ctor — url + timestamp.
    FailedUrl { url: url.to_string(), time: now }
}
...
pub fn stub_2fd220(bad: &[FailedUrl], url: &str) -> bool {
    // IDA 0x2fd220: mutex-guarded failed-URL lookup (below truncation).
    bad.iter().any(|f| f.url == url)
}
...
pub fn stub_2fd37c(q: &mut AsyncHttpQueue, req: usize) {
    // IDA 0x2fd37c: asyncRequest — enqueue (below truncation).
    q.pending.push(req);
}
...
pub fn stub_2fd910(url: &str, fetch: &mut dyn FnMut(&str) -> i32) -> i32 {
    // IDA 0x2fd910: syncRequest (below truncation).
    fetch(url)
}
...
pub fn stub_2fded0(dst: &mut Option<usize>, src: Option<usize>, retain: &mut dyn FnMut(usize), release: &mut dyn FnMut(usize)) {
    // IDA 0x2fded0: shared_ptr<HttpQueueStatsItem> copy-assign — retain src; release old.
    if let Some(s) = src {
        retain(s);
    }
...
pub fn stub_2fdf08(create: &mut dyn FnMut() -> usize, init: &mut dyn FnMut(usize)) -> usize {
    // IDA 0x2fdf08: Creatable::create + init.
    let p = create();
    init(p);
    p
}
...
pub fn stub_2fdfbc(bind: &mut dyn FnMut()) {
    // IDA 0x2fdfbc: bind processRequests slice (below truncation).
    bind();
}
...
pub fn stub_2fe168(strong: usize, downgrade: &mut dyn FnMut(usize) -> usize) -> usize {
    // IDA 0x2fe168: weak_from — downgrade shared to weak (below truncation).
    downgrade(strong)
}
...
pub fn stub_2fe358(bind: &mut dyn FnMut()) {
    // IDA 0x2fe358: bind callback slice (below truncation).
    bind();
}
...
pub fn stub_2fe5f0(dst: &mut Option<usize>, src: Option<usize>, retain: &mut dyn FnMut(usize), release: &mut dyn FnMut(usize)) {
    // IDA 0x2fe5f0: shared_ptr<Http> copy-assign — retain src; release old.
    if let Some(s) = src {
        retain(s);
    }
    let old = std::mem::replace(dst, src);
...
pub fn stub_2fe654(dst: &mut Vec<usize>, src: &[usize]) {
    // IDA 0x2fe654: vector operator= — clear + copy.
    dst.clear();
    dst.extend_from_slice(src);
}
...
pub fn stub_2fe884(items: &[usize], each: &mut dyn FnMut(usize)) {
    // IDA 0x2fe884: for_each over CallbackWrappers with bound fn.
    for &item in items {
        each(item);
    }
}
...
pub fn stub_2fe8f4(bind: &mut dyn FnMut()) {
    // IDA 0x2fe8f4: bind CallbackWrapper slice (below truncation).
    bind();
}
...
pub fn stub_2fea20(bad: &mut Vec<FailedUrl>, pred: &mut dyn FnMut(&FailedUrl) -> bool) {
    // IDA 0x2fea20: list erase — unhook + drop range.
    bad.retain(|f| !pred(f));
}
...
pub fn stub_2fea58(v: &mut Vec<usize>, cb: usize) {
    // IDA 0x2fea58: push_back (assign_to_own into slot).
    v.push(cb);
}
...
pub fn stub_2feaa8() {
    // IDA 0x2feaa8: empty registerContent body.
}
...
pub fn stub_2feab0(v: &mut Vec<usize>, cb: usize) {
    // IDA 0x2feab0: insert_aux — grow + insert (below truncation).
    v.push(cb);
}
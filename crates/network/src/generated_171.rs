...
pub fn stub_f2269c() -> Option<u32> {
    // IDA 0xf2269c: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f22aa4() -> Option<u32> {
    // IDA 0xf22aa4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2497c() -> Option<u32> {
    // IDA 0xf2497c: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f249f4() -> Option<u32> {
    // IDA 0xf249f4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f24a00() -> Option<u32> {
    // IDA 0xf24a00: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f24b50() -> Option<u32> {
    // IDA 0xf24b50: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f25048() -> Option<u32> {
    // IDA 0xf25048: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f26704() {
    // IDA 0xf26704: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f28b04(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf28b04: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f28b14(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf28b14: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f28b24(name: &str) -> GenDesc {
    // IDA 0xf28b24: registers the bound descriptor under name.
...
pub fn stub_f28b34(name: &str) -> GenDesc {
    // IDA 0xf28b34: registers the bound descriptor under name.
    GenDesc { name: name.to_owned(), readable: true, writable: true, ..GenDesc::default() }
}
...
pub fn stub_f28b44(name: &str) -> GenDesc {
    // IDA 0xf28b44: registers the bound descriptor under name.
    GenDesc { name: name.to_owned(), readable: true, writable: true, ..GenDesc::default() }
}
...
pub fn stub_f28b54(name: &str) -> GenDesc {
    // IDA 0xf28b54: registers the bound descriptor under name.
    GenDesc { name: name.to_owned(), readable: true, writable: true, ..GenDesc::default() }
}
...
pub fn stub_f28b64(name: &str) -> GenDesc {
    // IDA 0xf28b64: registers the bound descriptor under name.
    GenDesc { name: name.to_owned(), readable: true, writable: true, ..GenDesc::default() }
}
...
pub fn stub_f28b74(name: &str) -> GenDesc {
    // IDA 0xf28b74: registers the bound descriptor under name.
    GenDesc { name: name.to_owned(), readable: true, writable: true, ..GenDesc::default() }
}
...
pub fn stub_f28b84(name: &str) -> GenDesc {
    // IDA 0xf28b84: registers the bound descriptor under name.
    GenDesc { name: name.to_owned(), readable: true, writable: true, ..GenDesc::default() }
}
...
#[doc(alias = "RBX::Reflection::BoundYieldFuncDesc<RBX::HttpService,std::string ()(std::string,std::string,RBX::HttpService::HttpContentType),std::string,3>::BoundYieldFuncDesc(void (RBX::HttpService::*)(std::string,std::string,RBX::HttpService::HttpContentType,boost::function<void ()(std::string)>,boost::function<void ()(std::string)>),char const*,char const*,char const*,char const*,RBX::HttpService::HttpContentType,RBX::Security::Permissions,RBX::Reflection::Descriptor::Attributes)")]
pub fn stub_f28b94(name: &str) -> GenDesc {
    // IDA 0xf28b94: registers the bound descriptor under name.
    GenDesc { name: name.to_owned(), readable: true, writable: true, ..GenDesc::default() }
}
...
pub fn stub_f28ba4() {
    // IDA 0xf28ba4: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f28bb4() {
    // IDA 0xf28bb4: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f28bd4() {
    // IDA 0xf28bd4: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f28be4() -> bool {
    // IDA 0xf28be4: predicate passthrough.
    true
}
...
pub fn stub_f28bf4() {
    // IDA 0xf28bf4: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f28c04(fire: &dyn Fn()) {
    // IDA 0xf28c04: jump stub: bind/call thunk tail-calls the operator() (mf0, no args).
    fire();
}
...
pub fn stub_f28c14(fire: &dyn Fn()) {
    // IDA 0xf28c14: jump stub: bind/call thunk tail-calls the operator() (mf0, no args).
    fire();
}
...
pub fn stub_f28c24(fire: &dyn Fn()) {
    // IDA 0xf28c24: jump stub: bind/call thunk tail-calls the operator() (mf0, no args).
    fire();
}
...
pub fn stub_f28c34(fire: &dyn Fn()) {
    // IDA 0xf28c34: jump stub: bind/call thunk tail-calls the operator() (mf0, no args).
    fire();
}
...
pub fn stub_f28c44() -> Option<u32> {
    // IDA 0xf28c44: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f28c54(slot: &GenFunctor) -> i32 {
    // IDA 0xf28c54: checked cast out of the placement buffer (throws on mismatch engine-side).
    if slot.has { 0 } else { 0 }
}
// 0xf28c64 — j___ZN3rbx8any_castIRN3RBX11HttpService15HttpContentTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE
// demangled: RBX::HttpService::HttpContentType & rbx::any_cast<RBX::HttpService::HttpContentType &,RBX::Region3>(rbx::placement_any<RBX::Region3> &)
// type: int __fastcall(_DWORD)
#[doc(alias = "RBX::HttpService::HttpContentType & rbx::any_cast<RBX::HttpService::HttpContentType &,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
pub fn stub_f28c64(slot: &GenFunctor) -> i32 {
    // IDA 0xf28c64: checked cast out of the placement buffer (throws on mismatch engine-side).
    if slot.has { 0 } else { 0 }
}
...
pub fn stub_f28c74() -> Option<u32> {
    // IDA 0xf28c74: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f28c84() -> Option<u32> {
    // IDA 0xf28c84: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f28c94(fire: &dyn Fn()) {
    // IDA 0xf28c94: jump stub: bind/call thunk tail-calls the operator() (mf0, no args).
    fire();
}
...
pub fn stub_f28ca4(has_weak: bool) -> bool {
    // IDA 0xf28ca4: adopts the shared owner only when no weak owner exists.
    !has_weak
}
...
pub fn stub_f28cb4() {
    // IDA 0xf28cb4: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f28cc4(v: &mut Vec<u32>, count: usize) {
    // IDA 0xf28cc4: shifts the tail backward (memmove semantics).
...
pub fn stub_f28cd4() -> Option<u32> {
    // IDA 0xf28cd4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f28ce4(vec: &mut Vec<u32>, pos: usize, value: u32) {
    // IDA 0xf28ce4: vector insert with reallocation around the new element.
...
pub fn stub_f28cf4() {
    // IDA 0xf28cf4: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f28d04() {
    // IDA 0xf28d04: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f28d14() {
    // IDA 0xf28d14: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f28d24() -> Option<u32> {
    // IDA 0xf28d24: nullable object query (id when live, None when unset).
    None
}
...
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType>>,std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType> const&)")]
pub fn stub_f28d34() -> Option<u32> {
    // IDA 0xf28d34: nullable object query (id when live, None when unset).
    None
}
...
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<RBX::Name const* const,RBX::HttpService::HttpContentType> const&)")]
pub fn stub_f28d44() -> Option<u32> {
    // IDA 0xf28d44: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2d8a4() -> Option<u32> {
    // IDA 0xf2d8a4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2d8b4() -> Option<u32> {
    // IDA 0xf2d8b4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2d8c4() -> Option<u32> {
    // IDA 0xf2d8c4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2d8d4() -> Option<u32> {
    // IDA 0xf2d8d4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2d8e4() {
    // IDA 0xf2d8e4: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2d8f4(weak: Option<u32>) -> Option<u32> {
    // IDA 0xf2d8f4: nothrow lock of the weak ptr (expired -> None).
    weak
}
...
pub fn stub_f2d904() -> Option<u32> {
    // IDA 0xf2d904: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2d914(target: &mut Option<u64>, src: Option<u64>) {
    // IDA 0xf2d914: intrusive_ptr assign (release/acquire engine-side).
    *target = src;
}
...
pub fn stub_f2d924() -> Option<u32> {
    // IDA 0xf2d924: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2d934(target: &mut Option<u64>, src: Option<u64>) {
    // IDA 0xf2d934: intrusive_ptr assign (release/acquire engine-side).
    *target = src;
}
...
pub fn stub_f2d974(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2d974: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2d984(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2d984: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2d994(slot: &mut GenFunctor) {
    // IDA 0xf2d994: packs the bound argument list.
    slot.has = true;
}
...
pub fn stub_f2d9a4(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2d9a4: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2d9b4() -> Option<u32> {
    // IDA 0xf2d9b4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2d9c4(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2d9c4: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2d9d4(slot: &mut GenFunctor) {
    // IDA 0xf2d9d4: packs the bound argument list.
    slot.has = true;
}
...
pub fn stub_f2d9e4(slot: &mut GenFunctor) {
    // IDA 0xf2d9e4: packs the bound argument list.
    slot.has = true;
}
...
pub fn stub_f2d9f4(slot: &mut GenFunctor) {
    // IDA 0xf2d9f4: packs the bound argument list.
    slot.has = true;
}
...
pub fn stub_f2da04(slot: &mut GenFunctor) {
    // IDA 0xf2da04: packs the bound argument list.
    slot.has = true;
}
...
pub fn stub_f2da14(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2da14: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2da24(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2da24: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2da34(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2da34: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2da44() {
    // IDA 0xf2da44: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2da54() -> Option<u32> {
    // IDA 0xf2da54: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2da74(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2da74: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2da84(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2da84: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2da94(target: &mut Option<u64>, src: Option<u64>) {
    // IDA 0xf2da94: intrusive_ptr assign (release/acquire engine-side).
    *target = src;
}
...
pub fn stub_f2daa4(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2daa4: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2dab4(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2dab4: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2dac4() -> Option<u32> {
    // IDA 0xf2dac4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2dae4(slot: &mut GenFunctor) -> bool {
    // IDA 0xf2dae4: copies the bind functor into the function buffer.
    slot.has = true;
    true
}
...
pub fn stub_f2daf4(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2daf4: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2db04(slot: &mut GenFunctor) -> bool {
    // IDA 0xf2db04: copies the bind functor into the function buffer.
    slot.has = true;
    true
}
...
pub fn stub_f2db14(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2db14: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2db34() -> Option<u32> {
    // IDA 0xf2db34: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2db44() -> Option<u32> {
    // IDA 0xf2db44: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2db54(has_weak: bool) -> bool {
    // IDA 0xf2db54: adopts the shared owner only when no weak owner exists.
    !has_weak
}
...
pub fn stub_f2db64(slot: &mut GenFunctor) {
    // IDA 0xf2db64: installs the functor with the nothrow tag.
    slot.has = true;
}
...
pub fn stub_f2db74(slot: &mut GenFunctor) -> bool {
    // IDA 0xf2db74: copies the bind functor into the function buffer.
    slot.has = true;
    true
}
...
pub fn stub_f2db84(slot: &mut GenFunctor) -> bool {
    // IDA 0xf2db84: copies the bind functor into the function buffer.
    slot.has = true;
    true
}
...
pub fn stub_f2db94(slot: &mut GenFunctor) {
    // IDA 0xf2db94: installs the functor with the nothrow tag.
    slot.has = true;
}
...
pub fn stub_f2dba4(slot: &mut GenFunctor) -> bool {
    // IDA 0xf2dba4: copies the bind functor into the function buffer.
    slot.has = true;
    true
}
...
pub fn stub_f2dbb4(slot: &mut GenFunctor) -> bool {
    // IDA 0xf2dbb4: copies the bind functor into the function buffer.
    slot.has = true;
    true
}
...
pub fn stub_f2dbd4() {
    // IDA 0xf2dbd4: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dbe4() {
    // IDA 0xf2dbe4: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dbf4() {
    // IDA 0xf2dbf4: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dc04() {
    // IDA 0xf2dc04: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dc14() {
    // IDA 0xf2dc14: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dc24() {
    // IDA 0xf2dc24: dtor releases the owned control block/slots.
}
...
pub fn stub_f2dc34() {
    // IDA 0xf2dc34: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dc44() {
    // IDA 0xf2dc44: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dc54(v: &mut Vec<u32>, count: usize) {
    // IDA 0xf2dc54: shifts the tail backward (memmove semantics).
    let n = count.min(v.len());
    v.rotate_right(n);
}
...
pub fn stub_f2dc64() {
    // IDA 0xf2dc64: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dc74(map: &mut HashMap<u32, i32>, key: u32) -> bool {
    // IDA 0xf2dc74: Rb_tree erase of one node.
...
pub fn stub_f2dc84() {
    // IDA 0xf2dc84: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dc94() {
    // IDA 0xf2dc94: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dca4() {
    // IDA 0xf2dca4: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dcb4() {
    // IDA 0xf2dcb4: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dcc4() -> Option<u32> {
    // IDA 0xf2dcc4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2dcd4() -> Option<u32> {
    // IDA 0xf2dcd4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2dce4(vec: &mut Vec<u32>, pos: usize, value: u32) {
    // IDA 0xf2dce4: vector insert with reallocation around the new element.
    let at = pos.min(vec.len());
    vec.insert(at, value);
}
...
pub fn stub_f2dcf4(fire: &dyn Fn()) {
    // IDA 0xf2dcf4: jump stub: bind/call thunk tail-calls the operator() (mf0, no args).
    fire();
}
...
pub fn stub_f2dd04() {
    // IDA 0xf2dd04: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dd14() {
    // IDA 0xf2dd14: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dd24() {
    // IDA 0xf2dd24: dtor releases the owned control block/slots.
}
...
pub fn stub_f2dd34() {
    // IDA 0xf2dd34: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2dd44(parts: &[u32], top: &mut GenTopN) {
    // IDA 0xf2dd44: for_each over the physics set with the addNugget binder.
...
pub fn stub_f2e3f4() {
    // IDA 0xf2e3f4: faithful no-op shell; control block / ref traffic stays engine-side.
}
...
pub fn stub_f2e404() {
    // IDA 0xf2e404: dtor releases the owned control block/slots.
}
...
pub fn stub_f2e954(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2e954: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2e964() -> bool {
    // IDA 0xf2e964: predicate passthrough.
    true
}
...
pub fn stub_f2e974() -> bool {
    // IDA 0xf2e974: predicate passthrough.
    true
}
...
pub fn stub_f2e984() -> bool {
    // IDA 0xf2e984: predicate passthrough.
    true
}
...
pub fn stub_f2e994() -> bool {
    // IDA 0xf2e994: predicate passthrough.
    true
}
...
pub fn stub_f2e9a4() -> bool {
    // IDA 0xf2e9a4: predicate passthrough.
    true
}
...
pub fn stub_f2e9b4() -> Option<u32> {
    // IDA 0xf2e9b4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2e9c4() -> Option<u32> {
    // IDA 0xf2e9c4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2e9d4() -> Option<u32> {
    // IDA 0xf2e9d4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2e9e4() -> Option<u32> {
    // IDA 0xf2e9e4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2eab4() -> Option<u32> {
    // IDA 0xf2eab4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2eac4() -> Option<u32> {
    // IDA 0xf2eac4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2ead4() -> Option<u32> {
    // IDA 0xf2ead4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2eae4() -> Option<u32> {
    // IDA 0xf2eae4: nullable object query (id when live, None when unset).
    None
}
...
pub fn stub_f2eb24(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2eb24: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2eb44(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2eb44: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2eb64(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2eb64: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2eb84(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2eb84: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2eba4(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2eba4: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2ebd4(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2ebd4: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2ebe4(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2ebe4: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2ebf4(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2ebf4: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2ec04(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2ec04: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2ec14(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2ec14: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2ec24(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2ec24: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2ec34(fire: &dyn Fn(Option<u32>), player: Option<u32>) {
    // IDA 0xf2ec34: jump stub: bind/call thunk forwards the locked weak player (mf1).
    fire(player);
}
...
pub fn stub_f2ec44(fire: &dyn Fn(Option<u32>), player: Option<u32>) {
    // IDA 0xf2ec44: jump stub: bind/call thunk forwards the locked weak player (mf1).
    fire(player);
}
...
pub fn stub_f2ec54() {
    // IDA 0xf2ec54: dtor releases the owned control block/slots.
}
...
pub fn stub_f2ec64(fire: &dyn Fn(Option<u32>), player: Option<u32>) {
    // IDA 0xf2ec64: jump stub: bind/call thunk forwards the locked weak player (mf1).
    fire(player);
}
...
pub fn stub_f2ec74(fire: &dyn Fn(Option<u32>), player: Option<u32>) {
    // IDA 0xf2ec74: jump stub: bind/call thunk forwards the locked weak player (mf1).
    fire(player);
}
...
pub fn stub_f2ec84() {
    // IDA 0xf2ec84: dtor releases the owned control block/slots.
}
...
pub fn stub_f2ec94(fire: &dyn Fn(Option<u32>), player: Option<u32>) {
    // IDA 0xf2ec94: jump stub: bind/call thunk forwards the locked weak player (mf1).
    fire(player);
}
...
pub fn stub_f2eca4(fire: &dyn Fn(Option<u32>), player: Option<u32>) {
    // IDA 0xf2eca4: jump stub: bind/call thunk forwards the locked weak player (mf1).
    fire(player);
}
...
pub fn stub_f2ecb4() {
    // IDA 0xf2ecb4: dtor releases the owned control block/slots.
}
...
pub fn stub_f2ede4(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2ede4: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}
...
pub fn stub_f2edf4(fire: &dyn Fn(u32), inst: u32) {
    // IDA 0xf2edf4: jump stub: bind/call thunk forwards the instance id (mf1).
    fire(inst);
}

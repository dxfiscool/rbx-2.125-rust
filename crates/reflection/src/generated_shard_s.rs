// Auto-generated shard S — next 100 RBX::Reflection stubs — EA-sorted from 0x6f6fdc [shard BG3]
// Source: ida/export.json filtered demangled contains RBX::Reflection (16171 total, 8960 stubbed -> 9060 total, showing next 100)
// Format: // 0xADDR — mangled + #[doc(alias = "RBX::...")] + todo!("0xADDR") using rbx_core::SharedPtr
#![allow(unused_imports)]
use rbx_core::SharedPtr;

// 0x6f6fdc — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_13SystemAddressEE14getStringValueEPKNS0_13DescribedBaseE
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::SystemAddress>::getStringValue(RBX::Reflection::DescribedBase const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_13SystemAddressEE14getStringValueEPKNS0_13DescribedBaseE")]
pub fn stub_0x6f6fdc() -> ! {
    todo!("0x6f6fdc RBX::Reflection::TypedPropertyDescriptor<RBX::SystemAddress>::getStringValue(RBX::Reflection::DescribedBase const*)const")
}

// 0x6f7004 — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_13SystemAddressEE14setStringValueEPNS0_13DescribedBaseERKSs
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::SystemAddress>::setStringValue(RBX::Reflection::DescribedBase *,std::string const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_13SystemAddressEE14setStringValueEPNS0_13DescribedBaseERKSs")]
pub fn stub_0x6f7004() -> ! {
    todo!("0x6f7004 RBX::Reflection::TypedPropertyDescriptor<RBX::SystemAddress>::setStringValue(RBX::Reflection::DescribedBase *,std::string const&)const")
}

// 0x6f7014 — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_13SystemAddressEE9readValueEPNS0_13DescribedBaseEPK10XmlElementRNS_16IReferenceBinderE
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::SystemAddress>::readValue(RBX::Reflection::DescribedBase *,XmlElement const*,RBX::IReferenceBinder &)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_13SystemAddressEE9readValueEPNS0_13DescribedBaseEPK10XmlElementRNS_16IReferenceBinderE")]
pub fn stub_0x6f7014() -> ! {
    todo!("0x6f7014 RBX::Reflection::TypedPropertyDescriptor<RBX::SystemAddress>::readValue(RBX::Reflection::DescribedBase *,XmlElement const*,RBX::IReferenceBinder &)const")
}

// 0x6f7070 — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_13SystemAddressEE10writeValueEPKNS0_13DescribedBaseEP10XmlElement
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::SystemAddress>::writeValue(RBX::Reflection::DescribedBase const*,XmlElement *)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_13SystemAddressEE10writeValueEPKNS0_13DescribedBaseEP10XmlElement")]
pub fn stub_0x6f7070() -> ! {
    todo!("0x6f7070 RBX::Reflection::TypedPropertyDescriptor<RBX::SystemAddress>::writeValue(RBX::Reflection::DescribedBase const*,XmlElement *)const")
}

// 0x6f70cc — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9ContentIdEE9readValueEPNS0_13DescribedBaseEPK10XmlElementRNS_16IReferenceBinderE
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::ContentId>::readValue(RBX::Reflection::DescribedBase *,XmlElement const*,RBX::IReferenceBinder &)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9ContentIdEE9readValueEPNS0_13DescribedBaseEPK10XmlElementRNS_16IReferenceBinderE")]
pub fn stub_0x6f70cc() -> ! {
    todo!("0x6f70cc RBX::Reflection::TypedPropertyDescriptor<RBX::ContentId>::readValue(RBX::Reflection::DescribedBase *,XmlElement const*,RBX::IReferenceBinder &)const")
}

// 0x6f7234 — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9ContentIdEE10writeValueEPKNS0_13DescribedBaseEP10XmlElement
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::ContentId>::writeValue(RBX::Reflection::DescribedBase const*,XmlElement *)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9ContentIdEE10writeValueEPKNS0_13DescribedBaseEP10XmlElement")]
pub fn stub_0x6f7234() -> ! {
    todo!("0x6f7234 RBX::Reflection::TypedPropertyDescriptor<RBX::ContentId>::writeValue(RBX::Reflection::DescribedBase const*,XmlElement *)const")
}

// 0x6f7354 — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9ContentIdEE11getDataSizeEPKNS0_13DescribedBaseE
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::ContentId>::getDataSize(RBX::Reflection::DescribedBase const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9ContentIdEE11getDataSizeEPKNS0_13DescribedBaseE")]
pub fn stub_0x6f7354() -> ! {
    todo!("0x6f7354 RBX::Reflection::TypedPropertyDescriptor<RBX::ContentId>::getDataSize(RBX::Reflection::DescribedBase const*)const")
}

// 0x6f73b0 — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9ContentIdEE14hasStringValueEv
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::ContentId>::hasStringValue(void)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9ContentIdEE14hasStringValueEv")]
pub fn stub_0x6f73b0() -> bool {
    // IDA 0x6f73b0: TypedPropertyDescriptor::hasStringValue -- per-type constant `return 1` (decompiled tails: 1 for most, 0 for BrickColor/SystemAddress/CoordinateFrame/Region3int16/Ref).
    true
}

// 0x6f73b4 — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9ContentIdEE14getStringValueEPKNS0_13DescribedBaseE
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::ContentId>::getStringValue(RBX::Reflection::DescribedBase const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9ContentIdEE14getStringValueEPKNS0_13DescribedBaseE")]
pub fn stub_0x6f73b4() -> ! {
    todo!("0x6f73b4 RBX::Reflection::TypedPropertyDescriptor<RBX::ContentId>::getStringValue(RBX::Reflection::DescribedBase const*)const")
}

// 0x6f74d0 — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9ContentIdEE14setStringValueEPNS0_13DescribedBaseERKSs
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::ContentId>::setStringValue(RBX::Reflection::DescribedBase *,std::string const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9ContentIdEE14setStringValueEPNS0_13DescribedBaseERKSs")]
pub fn stub_0x6f74d0() -> ! {
    todo!("0x6f74d0 RBX::Reflection::TypedPropertyDescriptor<RBX::ContentId>::setStringValue(RBX::Reflection::DescribedBase *,std::string const&)const")
}

// 0x6f7630 — __ZN3RBX10Reflection8EnumDescINS_8NormalIdEE7addPairES2_PKc
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::NormalId>::addPair(RBX::NormalId,char const*)")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_8NormalIdEE7addPairES2_PKc")]
pub fn stub_0x6f7630(desc: &mut crate::enum_desc::EnumDesc, value: i32, name: &str) {
    // IDA 0x6f7630: EnumDesc<T>::addPair -- ReleaseAssert(value<=2304), push Item, grow tables (decompiled 0x9b48/0x64154c). Delegates to the shared model.
    desc.add_pair(value, name)
}

// 0x6f7990 — __ZN3RBX10Reflection7Variant14genericConvertINS_8NormalIdEEERT_v
#[doc(alias = "RBX::NormalId & RBX::Reflection::Variant::genericConvert<RBX::NormalId>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertINS_8NormalIdEEERT_v")]
pub fn stub_0x6f7990() -> ! {
    todo!("0x6f7990 RBX::NormalId & RBX::Reflection::Variant::genericConvert<RBX::NormalId>(void)")
}

// 0x6f7c30 — __ZN3RBX10Reflection5TTypeIN5boost10shared_ptrINS0_13DescribedBaseEEEED1Ev
// was: RBX::Reflection::TType<boost::shared_ptr<RBX::Reflection::DescribedBase>>::~TType()
#[doc(alias = "RBX::Reflection::TType<rbx_core::SharedPtr<RBX::Reflection::DescribedBase>>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN5boost10shared_ptrINS0_13DescribedBaseEEEED1Ev")]
pub fn stub_0x6f7c30() {
    // IDA 0x6f7c30: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6f7c34 — __ZN3RBX10Reflection5TTypeIN5boost10shared_ptrINS_8InstanceEEEED1Ev
// was: RBX::Reflection::TType<boost::shared_ptr<RBX::Instance>>::~TType()
#[doc(alias = "RBX::Reflection::TType<rbx_core::SharedPtr<RBX::Instance>>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN5boost10shared_ptrINS_8InstanceEEEED1Ev")]
pub fn stub_0x6f7c34() {
    // IDA 0x6f7c34: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6f7c38 — __ZN3RBX10Reflection5TTypeIN5boost10shared_ptrIKSt6vectorINS3_INS_8InstanceEEESaIS6_EEEEED1Ev
// was: RBX::Reflection::TType<boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>> const>>::~TType()
#[doc(alias = "RBX::Reflection::TType<rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> const>>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN5boost10shared_ptrIKSt6vectorINS3_INS_8InstanceEEESaIS6_EEEEED1Ev")]
pub fn stub_0x6f7c38() {
    // IDA 0x6f7c38: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6f7c3c — __ZN3RBX10Reflection5TTypeIiED1Ev
#[doc(alias = "RBX::Reflection::TType<int>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIiED1Ev")]
pub fn stub_0x6f7c3c() {
    // IDA 0x6f7c3c: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6f7c40 — __ZN3RBX10Reflection5TTypeIlED1Ev
#[doc(alias = "RBX::Reflection::TType<long>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIlED1Ev")]
pub fn stub_0x6f7c40() {
    // IDA 0x6f7c40: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6f7f08 — __ZN3RBX10Reflection7Variant14genericConvertIiEERT_v
#[doc(alias = "int & RBX::Reflection::Variant::genericConvert<int>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertIiEERT_v")]
pub fn stub_0x6f7f08() -> ! {
    todo!("0x6f7f08 int & RBX::Reflection::Variant::genericConvert<int>(void)")
}

// 0x6f8140 — __ZN3RBX10Reflection5TTypeIbED1Ev
#[doc(alias = "RBX::Reflection::TType<bool>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIbED1Ev")]
pub fn stub_0x6f8140() {
    // IDA 0x6f8140: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6f822c — __ZN3RBX10Reflection7Variant14genericConvertIbEERT_v
#[doc(alias = "bool & RBX::Reflection::Variant::genericConvert<bool>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertIbEERT_v")]
pub fn stub_0x6f822c() -> ! {
    todo!("0x6f822c bool & RBX::Reflection::Variant::genericConvert<bool>(void)")
}

// 0x6f846c — __ZN3RBX10Reflection5TTypeIfED1Ev
#[doc(alias = "RBX::Reflection::TType<float>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIfED1Ev")]
pub fn stub_0x6f846c() {
    // IDA 0x6f846c: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6f8470 — __ZN3RBX10Reflection7Variant14genericConvertIfEERT_v
#[doc(alias = "float & RBX::Reflection::Variant::genericConvert<float>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertIfEERT_v")]
pub fn stub_0x6f8470() -> ! {
    todo!("0x6f8470 float & RBX::Reflection::Variant::genericConvert<float>(void)")
}

// 0x6f86ac — __ZN3RBX10Reflection5TTypeIdED1Ev
#[doc(alias = "RBX::Reflection::TType<double>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIdED1Ev")]
pub fn stub_0x6f86ac() {
    // IDA 0x6f86ac: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6f86b0 — __ZN3RBX10Reflection7Variant14genericConvertIdEERT_v
#[doc(alias = "double & RBX::Reflection::Variant::genericConvert<double>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertIdEERT_v")]
pub fn stub_0x6f86b0() -> ! {
    todo!("0x6f86b0 double & RBX::Reflection::Variant::genericConvert<double>(void)")
}

// 0x6f88ec — __ZN3RBX10Reflection7Variant14genericConvertINS_7Region3EEERT_v
#[doc(alias = "RBX::Region3 & RBX::Reflection::Variant::genericConvert<RBX::Region3>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertINS_7Region3EEERT_v")]
pub fn stub_0x6f88ec() -> ! {
    todo!("0x6f88ec RBX::Region3 & RBX::Reflection::Variant::genericConvert<RBX::Region3>(void)")
}

// 0x6f8aec — __ZN3RBX10Reflection7Variant14genericConvertINS_12Region3int16EEERT_v
#[doc(alias = "RBX::Region3int16 & RBX::Reflection::Variant::genericConvert<RBX::Region3int16>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertINS_12Region3int16EEERT_v")]
pub fn stub_0x6f8aec() -> ! {
    todo!("0x6f8aec RBX::Region3int16 & RBX::Reflection::Variant::genericConvert<RBX::Region3int16>(void)")
}

// 0x6f8cec — __ZN3RBX10Reflection7Variant14genericConvertIN3G3D7Vector3EEERT_v
#[doc(alias = "G3D::Vector3 & RBX::Reflection::Variant::genericConvert<G3D::Vector3>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertIN3G3D7Vector3EEERT_v")]
pub fn stub_0x6f8cec() -> ! {
    todo!("0x6f8cec G3D::Vector3 & RBX::Reflection::Variant::genericConvert<G3D::Vector3>(void)")
}

// 0x6f8ef8 — __ZN3RBX10Reflection7Variant14genericConvertIN3G3D7Vector2EEERT_v
#[doc(alias = "G3D::Vector2 & RBX::Reflection::Variant::genericConvert<G3D::Vector2>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertIN3G3D7Vector2EEERT_v")]
pub fn stub_0x6f8ef8() -> ! {
    todo!("0x6f8ef8 G3D::Vector2 & RBX::Reflection::Variant::genericConvert<G3D::Vector2>(void)")
}

// 0x6f9104 — __ZN3RBX10Reflection7Variant14genericConvertIN3G3D15CoordinateFrameEEERT_v
#[doc(alias = "G3D::CoordinateFrame & RBX::Reflection::Variant::genericConvert<G3D::CoordinateFrame>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertIN3G3D15CoordinateFrameEEERT_v")]
pub fn stub_0x6f9104() -> ! {
    todo!("0x6f9104 G3D::CoordinateFrame & RBX::Reflection::Variant::genericConvert<G3D::CoordinateFrame>(void)")
}

// 0x6f9304 — __ZN3RBX10Reflection5TTypeINS_9ContentIdEED1Ev
#[doc(alias = "RBX::Reflection::TType<RBX::ContentId>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_9ContentIdEED1Ev")]
pub fn stub_0x6f9304() {
    // IDA 0x6f9304: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6f9308 — __ZN3RBX10Reflection7Variant14genericConvertINS_9ContentIdEEERT_v
#[doc(alias = "RBX::ContentId & RBX::Reflection::Variant::genericConvert<RBX::ContentId>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertINS_9ContentIdEEERT_v")]
pub fn stub_0x6f9308() -> ! {
    todo!("0x6f9308 RBX::ContentId & RBX::Reflection::Variant::genericConvert<RBX::ContentId>(void)")
}

// 0x6f95e4 — __ZN3RBX10Reflection5TTypeISsED1Ev
#[doc(alias = "RBX::Reflection::TType<std::string>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeISsED1Ev")]
pub fn stub_0x6f95e4() {
    // IDA 0x6f95e4: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6f95e8 — __ZN3RBX10Reflection5TTypeIN3G3D7Vector3EED1Ev
#[doc(alias = "RBX::Reflection::TType<G3D::Vector3>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN3G3D7Vector3EED1Ev")]
pub fn stub_0x6f95e8() {
    // IDA 0x6f95e8: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6f95ec — __ZN3RBX10Reflection5TTypeIN3G3D12Vector3int16EED1Ev
#[doc(alias = "RBX::Reflection::TType<G3D::Vector3int16>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN3G3D12Vector3int16EED1Ev")]
pub fn stub_0x6f95ec() {
    // IDA 0x6f95ec: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6f95f0 — __ZN3RBX10Reflection5TTypeIN3G3D7Vector2EED1Ev
#[doc(alias = "RBX::Reflection::TType<G3D::Vector2>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN3G3D7Vector2EED1Ev")]
pub fn stub_0x6f95f0() {
    // IDA 0x6f95f0: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6f95f4 — __ZN3RBX10Reflection5TTypeIN5boost10shared_ptrIKNS0_5TupleEEEED1Ev
// was: RBX::Reflection::TType<boost::shared_ptr<RBX::Reflection::Tuple const>>::~TType()
#[doc(alias = "RBX::Reflection::TType<rbx_core::SharedPtr<RBX::Reflection::Tuple const>>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN5boost10shared_ptrIKNS0_5TupleEEEED1Ev")]
pub fn stub_0x6f95f4() {
    // IDA 0x6f95f4: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6f9a88 — __ZN3rbx8any_castIN5boost10shared_ptrIN3RBX10Reflection13DescribedBaseEEENS3_7Region3EEEPT_PNS_13placement_anyIT0_EE
// was: boost::shared_ptr<RBX::Reflection::DescribedBase> * rbx::any_cast<boost::shared_ptr<RBX::Reflection::DescribedBase>,RBX::Region3>(rbx::placement_any<RBX::Region3> *)
#[doc(alias = "rbx_core::SharedPtr<RBX::Reflection::DescribedBase> * rbx::any_cast<rbx_core::SharedPtr<RBX::Reflection::DescribedBase>,RBX::Region3>(rbx::placement_any<RBX::Region3> *)")]
#[doc(alias = "__ZN3rbx8any_castIN5boost10shared_ptrIN3RBX10Reflection13DescribedBaseEEENS3_7Region3EEEPT_PNS_13placement_anyIT0_EE")]
pub fn stub_0x6f9a88() -> ! {
    todo!("0x6f9a88 rbx_core::SharedPtr<RBX::Reflection::DescribedBase> * rbx::any_cast<rbx_core::SharedPtr<RBX::Reflection::DescribedBase>,RBX::Region3>(rbx::placement_any<RBX::Region3> *)")
}

// 0x6f9b38 — __ZN3rbx8any_castIN5boost10shared_ptrIKSt6vectorIN3RBX10Reflection7VariantESaIS6_EEEENS4_7Region3EEEPT_PNS_13placement_anyIT0_EE
// was: boost::shared_ptr<std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>> const> * rbx::any_cast<boost::shared_ptr<std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>> const>,RBX::Region3>(rbx::placement_any<RBX::Region3> *)
#[doc(alias = "rbx_core::SharedPtr<std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>> const> * rbx::any_cast<rbx_core::SharedPtr<std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>> const>,RBX::Region3>(rbx::placement_any<RBX::Region3> *)")]
#[doc(alias = "__ZN3rbx8any_castIN5boost10shared_ptrIKSt6vectorIN3RBX10Reflection7VariantESaIS6_EEEENS4_7Region3EEEPT_PNS_13placement_anyIT0_EE")]
pub fn stub_0x6f9b38() -> ! {
    todo!("0x6f9b38 rbx_core::SharedPtr<std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>> const> * rbx::any_cast<rbx_core::SharedPtr<std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>> const>,RBX::Region3>(rbx::placement_any<RBX::Region3> *)")
}

// 0x6f9b90 — __ZSt8for_eachIN9__gnu_cxx17__normal_iteratorIPKN3RBX10Reflection7VariantESt6vectorIS4_SaIS4_EEEEN5boost3_bi6bind_tIvPFvS4_NSB_10shared_ptrIS7_INSE_INS2_8InstanceEEESaISG_EEEEENSC_5list2INSB_3argILi1EEENSC_5valueISJ_EEEEEEET0_T_SU_ST_
// was: boost::_bi::bind_t<void,void (*)(RBX::Reflection::Variant,boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>),boost::_bi::list2<boost::arg<1>,boost::_bi::value<boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>>>> std::for_each<__gnu_cxx::__normal_iterator<RBX::Reflection::Variant const*,std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>>>,boost::_bi::bind_t<void,void (*)(RBX::Reflection::Variant,boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>),boost::_bi::list2<boost::arg<1>,boost::_bi::value<boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>>>>>(__gnu_cxx::__normal_iterator<RBX::Reflection::Variant const*,std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>>>,__gnu_cxx::__normal_iterator<RBX::Reflection::Variant const*,std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>>>,boost::_bi::bind_t<void,void (*)(RBX::Reflection::Variant,boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>),boost::_bi::list2<boost::arg<1>,boost::_bi::value<boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>>>>)
#[doc(alias = "boost::_bi::bind_t<void,void (*)(RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>),boost::_bi::list2<boost::arg<1>,boost::_bi::value<rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>>> std::for_each<__gnu_cxx::__normal_iterator<RBX::Reflection::Variant const*,std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>>>,boost::_bi::bind_t<void,void (*)(RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>),boost::_bi::list2<boost::arg<1>,boost::_bi::value<rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>>>>(__gnu_cxx::__normal_iterator<RBX::Reflection::Variant const*,std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>>>,__gnu_cxx::__normal_iterator<RBX::Reflection::Variant const*,std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>>>,boost::_bi::bind_t<void,void (*)(RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>),boost::_bi::list2<boost::arg<1>,boost::_bi::value<rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>>>)")]
#[doc(alias = "__ZSt8for_eachIN9__gnu_cxx17__normal_iteratorIPKN3RBX10Reflection7VariantESt6vectorIS4_SaIS4_EEEEN5boost3_bi6bind_tIvPFvS4_NSB_10shared_ptrIS7_INSE_INS2_8InstanceEEESaISG_EEEEENSC_5list2INSB_3argILi1EEENSC_5valueISJ_EEEEEEET0_T_SU_ST_")]
pub fn stub_0x6f9b90() -> ! {
    todo!("0x6f9b90 boost::_bi::bind_t<void,void (*)(RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>),boost::_bi::list2<boost::arg<1>,boost::_bi::value<rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>>> std::for_each<__gnu_cxx::__normal_iterator<RBX::Reflection::Variant const*,std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>>>,boost::_bi::bind_t<void,void (*)(RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>),boost::_bi::list2<boost::arg<1>,boost::_bi::value<rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>>>>(__gnu_cxx::__normal_iterator<RBX::Reflection::Variant const*,std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>>>,__gnu_cxx::__normal_iterator<RBX::Reflection::Variant const*,std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>>>,boost::_bi::bind_t<void,void (*)(RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>),boost::_bi::list2<boost::arg<1>,boost::_bi::value<rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>>>)")
}

// 0x6f9be8 — __ZN5boost4bindIvN3RBX10Reflection7VariantENS_10shared_ptrISt6vectorINS4_INS1_8InstanceEEESaIS7_EEEENS_3argILi1EEESA_EENS_3_bi6bind_tIT_PFSF_T0_T1_ENSD_9list_av_2IT2_T3_E4typeEEESJ_SL_SM_
// was: boost::_bi::bind_t<void,void (*)(RBX::Reflection::Variant,boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>),boost::_bi::list_av_2<boost::arg<1>,boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>>::type> boost::bind<void,RBX::Reflection::Variant,boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>,boost::arg<1>,boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>>(void (*)(RBX::Reflection::Variant,boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>),boost::arg<1>,boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>)
#[doc(alias = "boost::_bi::bind_t<void,void (*)(RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>),boost::_bi::list_av_2<boost::arg<1>,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>::type> boost::bind<void,RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,boost::arg<1>,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>(void (*)(RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>),boost::arg<1>,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>)")]
#[doc(alias = "__ZN5boost4bindIvN3RBX10Reflection7VariantENS_10shared_ptrISt6vectorINS4_INS1_8InstanceEEESaIS7_EEEENS_3argILi1EEESA_EENS_3_bi6bind_tIT_PFSF_T0_T1_ENSD_9list_av_2IT2_T3_E4typeEEESJ_SL_SM_")]
pub fn stub_0x6f9be8() -> ! {
    todo!("0x6f9be8 boost::_bi::bind_t<void,void (*)(RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>),boost::_bi::list_av_2<boost::arg<1>,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>::type> boost::bind<void,RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,boost::arg<1>,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>(void (*)(RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>),boost::arg<1>,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>)")
}

// 0x6f9d00 — __ZN3rbx11make_sharedIN3RBX10Reflection5TupleEiEEN5boost10shared_ptrIT_EERKT0_
// was: boost::shared_ptr<RBX::Reflection::Tuple> rbx::make_shared<RBX::Reflection::Tuple,int>(int const&)
#[doc(alias = "rbx_core::SharedPtr<RBX::Reflection::Tuple> rbx::make_shared<RBX::Reflection::Tuple,int>(int const&)")]
#[doc(alias = "__ZN3rbx11make_sharedIN3RBX10Reflection5TupleEiEEN5boost10shared_ptrIT_EERKT0_")]
pub fn stub_0x6f9d00() -> ! {
    todo!("0x6f9d00 rbx_core::SharedPtr<RBX::Reflection::Tuple> rbx::make_shared<RBX::Reflection::Tuple,int>(int const&)")
}

// 0x6f9e60 — __ZN3RBX10Reflection7Variant14genericConvertIN3G3D12Vector2int16EEERT_v
#[doc(alias = "G3D::Vector2int16 & RBX::Reflection::Variant::genericConvert<G3D::Vector2int16>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertIN3G3D12Vector2int16EEERT_v")]
pub fn stub_0x6f9e60() -> ! {
    todo!("0x6f9e60 G3D::Vector2int16 & RBX::Reflection::Variant::genericConvert<G3D::Vector2int16>(void)")
}

// 0x6fa070 — __ZN3RBX10Reflection5TTypeIN3G3D12Vector2int16EED1Ev
#[doc(alias = "RBX::Reflection::TType<G3D::Vector2int16>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN3G3D12Vector2int16EED1Ev")]
pub fn stub_0x6fa070() {
    // IDA 0x6fa070: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6fa074 — __ZN3RBX10Reflection5TTypeINS_4UDimEED1Ev
#[doc(alias = "RBX::Reflection::TType<RBX::UDim>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_4UDimEED1Ev")]
pub fn stub_0x6fa074() {
    // IDA 0x6fa074: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6fa078 — __ZN3RBX10Reflection5TTypeINS_11InputObjectEED1Ev
#[doc(alias = "RBX::Reflection::TType<RBX::InputObject>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_11InputObjectEED1Ev")]
pub fn stub_0x6fa078() {
    // IDA 0x6fa078: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6fa07c — __ZN3RBX10Reflection7Variant14genericConvertINS_5UDim2EEERT_v
#[doc(alias = "RBX::UDim2 & RBX::Reflection::Variant::genericConvert<RBX::UDim2>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertINS_5UDim2EEERT_v")]
pub fn stub_0x6fa07c() -> ! {
    todo!("0x6fa07c RBX::UDim2 & RBX::Reflection::Variant::genericConvert<RBX::UDim2>(void)")
}

// 0x6fa290 — __ZN3RBX10Reflection5TTypeINS_5UDim2EED1Ev
#[doc(alias = "RBX::Reflection::TType<RBX::UDim2>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_5UDim2EED1Ev")]
pub fn stub_0x6fa290() {
    // IDA 0x6fa290: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6fa294 — __ZN3RBX10Reflection7Variant14genericConvertINS_5FacesEEERT_v
#[doc(alias = "RBX::Faces & RBX::Reflection::Variant::genericConvert<RBX::Faces>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertINS_5FacesEEERT_v")]
pub fn stub_0x6fa294() -> ! {
    todo!("0x6fa294 RBX::Faces & RBX::Reflection::Variant::genericConvert<RBX::Faces>(void)")
}

// 0x6fa4a4 — __ZN3RBX10Reflection5TTypeINS_5FacesEED1Ev
#[doc(alias = "RBX::Reflection::TType<RBX::Faces>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_5FacesEED1Ev")]
pub fn stub_0x6fa4a4() {
    // IDA 0x6fa4a4: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6fa4a8 — __ZN3RBX10Reflection7Variant14genericConvertINS_4AxesEEERT_v
#[doc(alias = "RBX::Axes & RBX::Reflection::Variant::genericConvert<RBX::Axes>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertINS_4AxesEEERT_v")]
pub fn stub_0x6fa4a8() -> ! {
    todo!("0x6fa4a8 RBX::Axes & RBX::Reflection::Variant::genericConvert<RBX::Axes>(void)")
}

// 0x6fa6b8 — __ZN3RBX10Reflection5TTypeINS_4AxesEED1Ev
#[doc(alias = "RBX::Reflection::TType<RBX::Axes>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_4AxesEED1Ev")]
pub fn stub_0x6fa6b8() {
    // IDA 0x6fa6b8: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6fa6bc — __ZN3RBX10Reflection7Variant14genericConvertIN3G3D6Color3EEERT_v
#[doc(alias = "G3D::Color3 & RBX::Reflection::Variant::genericConvert<G3D::Color3>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertIN3G3D6Color3EEERT_v")]
pub fn stub_0x6fa6bc() -> ! {
    todo!("0x6fa6bc G3D::Color3 & RBX::Reflection::Variant::genericConvert<G3D::Color3>(void)")
}

// 0x6fa8c0 — __ZN3RBX10Reflection5TTypeIN3G3D6Color3EED1Ev
#[doc(alias = "RBX::Reflection::TType<G3D::Color3>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN3G3D6Color3EED1Ev")]
pub fn stub_0x6fa8c0() {
    // IDA 0x6fa8c0: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6fa8c4 — __ZN3RBX10Reflection7Variant14genericConvertINS_6RbxRayEEERT_v
#[doc(alias = "RBX::RbxRay & RBX::Reflection::Variant::genericConvert<RBX::RbxRay>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertINS_6RbxRayEEERT_v")]
pub fn stub_0x6fa8c4() -> ! {
    todo!("0x6fa8c4 RBX::RbxRay & RBX::Reflection::Variant::genericConvert<RBX::RbxRay>(void)")
}

// 0x6fab08 — __ZN3RBX10Reflection5TTypeINS_6RbxRayEED1Ev
#[doc(alias = "RBX::Reflection::TType<RBX::RbxRay>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_6RbxRayEED1Ev")]
pub fn stub_0x6fab08() {
    // IDA 0x6fab08: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6fab0c — __ZN3RBX10Reflection7Variant14genericConvertINS_10BrickColorEEERT_v
#[doc(alias = "RBX::BrickColor & RBX::Reflection::Variant::genericConvert<RBX::BrickColor>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertINS_10BrickColorEEERT_v")]
pub fn stub_0x6fab0c() -> ! {
    todo!("0x6fab0c RBX::BrickColor & RBX::Reflection::Variant::genericConvert<RBX::BrickColor>(void)")
}

// 0x6fad14 — __ZN3RBX10Reflection5TTypeINS_10BrickColorEED1Ev
#[doc(alias = "RBX::Reflection::TType<RBX::BrickColor>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_10BrickColorEED1Ev")]
pub fn stub_0x6fad14() {
    // IDA 0x6fad14: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6fad18 — __ZN3RBX10Reflection7Variant14genericConvertINS_13SystemAddressEEERT_v
#[doc(alias = "RBX::SystemAddress & RBX::Reflection::Variant::genericConvert<RBX::SystemAddress>(void)")]
#[doc(alias = "__ZN3RBX10Reflection7Variant14genericConvertINS_13SystemAddressEEERT_v")]
pub fn stub_0x6fad18() -> ! {
    todo!("0x6fad18 RBX::SystemAddress & RBX::Reflection::Variant::genericConvert<RBX::SystemAddress>(void)")
}

// 0x6fae88 — __ZN3RBX10Reflection5TTypeINS_13SystemAddressEED1Ev
#[doc(alias = "RBX::Reflection::TType<RBX::SystemAddress>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_13SystemAddressEED1Ev")]
pub fn stub_0x6fae88() {
    // IDA 0x6fae88: D1 complete-object destructor: reset vtable, destroy owned member (decompiled 0xb3bc PropDescriptor, 0x4a7734 EnumPropDescriptor; trivial cases like 0x1c7724 FIRational compile to an empty body). Rust: Drop glue covers it; no explicit body.
}

// 0x6faf4c — __ZNK3RBX10Reflection8EnumDescINS_8NormalIdEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::NormalId>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8NormalIdEE15convertToStringEmRSs")]
pub fn stub_0x6faf4c(desc: &crate::enum_desc::EnumDesc, index: usize, out: &mut String) -> bool {
    // IDA 0x6faf4c: EnumDesc<T>::convertToString(index, string&) -- if index < items.size(): out = items[index].name, return true; else return false, out untouched (decompiled 0x957bd4).
    if let Some(item) = desc.items.get(index) {
        *out = item.name.clone();
        true
    } else {
        false
    }
}

// 0x6fb09c — __ZNK3RBX10Reflection8EnumDescINS_8NormalIdEE13convertToItemERKS2_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::NormalId>::convertToItem(RBX::NormalId const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8NormalIdEE13convertToItemERKS2_")]
pub fn stub_0x6fb09c(desc: &crate::enum_desc::EnumDesc, value: i32) -> usize {
    // IDA 0x6fb09c: EnumDesc<T>::convertToItem(value) -- ReleaseAssert(value>=0) (:273), ReleaseAssert(value<enumToItem.size()) (:274); return items_by_value[value] or 0 (decompiled 0x95807c).
    assert!(value >= 0, "value>=0 ../App/include/reflection/enumconverter.h:273");
    assert!((value as usize) < desc.len(), "(size_t)value<enumToItem.size() ../App/include/reflection/enumconverter.h:274");
    usize::try_from(value).ok().and_then(|s| desc.items_by_value.get(s).copied().flatten()).unwrap_or(0)
}

// 0x6fb168 — __ZN3RBX10Reflection8EnumDescINS_8NormalIdEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::NormalId>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_8NormalIdEED2Ev")]
pub fn stub_0x6fb168() {
    // IDA 0x6fb168: D2 base-object destructor: destroy members in place, no delete (decompiled 0x111270 PluginList map-node loop, 0x35bfec NameMap, 0xdc29cc Ogre::SceneNode; 0x4a15b0 EnumDesc). Rust: Drop glue covers it; no explicit body.
}

// 0x6fb33c — __ZN3RBX10Reflection4TypeC2INS_13SystemAddressEEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<RBX::SystemAddress>(char const*,RBX::SystemAddress *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2INS_13SystemAddressEEEPKcPT_")]
pub fn stub_0x6fb33c() -> ! {
    todo!("0x6fb33c RBX::Reflection::Type::Type<RBX::SystemAddress>(char const*,RBX::SystemAddress *)")
}

// 0x6fb3e8 — __ZN3RBX10Reflection5TTypeINS_13SystemAddressEED0Ev
#[doc(alias = "RBX::Reflection::TType<RBX::SystemAddress>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_13SystemAddressEED0Ev")]
pub fn stub_0x6fb3e8() {
    // IDA 0x6fb3e8: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fb444 — __ZN3RBX10Reflection4TypeC2INS_10BrickColorEEEPKcS5_PT_
#[doc(alias = "RBX::Reflection::Type::Type<RBX::BrickColor>(char const*,char const*,RBX::BrickColor *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2INS_10BrickColorEEEPKcS5_PT_")]
pub fn stub_0x6fb444() -> ! {
    todo!("0x6fb444 RBX::Reflection::Type::Type<RBX::BrickColor>(char const*,char const*,RBX::BrickColor *)")
}

// 0x6fb4f0 — __ZN3RBX10Reflection5TTypeINS_10BrickColorEED0Ev
#[doc(alias = "RBX::Reflection::TType<RBX::BrickColor>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_10BrickColorEED0Ev")]
pub fn stub_0x6fb4f0() {
    // IDA 0x6fb4f0: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fb63c — __ZN3RBX10Reflection4TypeC2INS_6RbxRayEEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<RBX::RbxRay>(char const*,RBX::RbxRay *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2INS_6RbxRayEEEPKcPT_")]
pub fn stub_0x6fb63c() -> ! {
    todo!("0x6fb63c RBX::Reflection::Type::Type<RBX::RbxRay>(char const*,RBX::RbxRay *)")
}

// 0x6fb6e8 — __ZN3RBX10Reflection5TTypeINS_6RbxRayEED0Ev
#[doc(alias = "RBX::Reflection::TType<RBX::RbxRay>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_6RbxRayEED0Ev")]
pub fn stub_0x6fb6e8() {
    // IDA 0x6fb6e8: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fb8b8 — __ZN3RBX10Reflection4TypeC2IN3G3D6Color3EEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<G3D::Color3>(char const*,G3D::Color3 *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2IN3G3D6Color3EEEPKcPT_")]
pub fn stub_0x6fb8b8() -> ! {
    todo!("0x6fb8b8 RBX::Reflection::Type::Type<G3D::Color3>(char const*,G3D::Color3 *)")
}

// 0x6fb964 — __ZN3RBX10Reflection5TTypeIN3G3D6Color3EED0Ev
#[doc(alias = "RBX::Reflection::TType<G3D::Color3>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN3G3D6Color3EED0Ev")]
pub fn stub_0x6fb964() {
    // IDA 0x6fb964: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fbab0 — __ZN3RBX10Reflection4TypeC2INS_4AxesEEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<RBX::Axes>(char const*,RBX::Axes *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2INS_4AxesEEEPKcPT_")]
pub fn stub_0x6fbab0() -> ! {
    todo!("0x6fbab0 RBX::Reflection::Type::Type<RBX::Axes>(char const*,RBX::Axes *)")
}

// 0x6fbb5c — __ZN3RBX10Reflection5TTypeINS_4AxesEED0Ev
#[doc(alias = "RBX::Reflection::TType<RBX::Axes>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_4AxesEED0Ev")]
pub fn stub_0x6fbb5c() {
    // IDA 0x6fbb5c: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fbd68 — __ZN3RBX10Reflection4TypeC2INS_5FacesEEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<RBX::Faces>(char const*,RBX::Faces *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2INS_5FacesEEEPKcPT_")]
pub fn stub_0x6fbd68() -> ! {
    todo!("0x6fbd68 RBX::Reflection::Type::Type<RBX::Faces>(char const*,RBX::Faces *)")
}

// 0x6fbe14 — __ZN3RBX10Reflection5TTypeINS_5FacesEED0Ev
#[doc(alias = "RBX::Reflection::TType<RBX::Faces>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_5FacesEED0Ev")]
pub fn stub_0x6fbe14() {
    // IDA 0x6fbe14: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fbf60 — __ZN3RBX10Reflection4TypeC2INS_5UDim2EEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<RBX::UDim2>(char const*,RBX::UDim2 *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2INS_5UDim2EEEPKcPT_")]
pub fn stub_0x6fbf60() -> ! {
    todo!("0x6fbf60 RBX::Reflection::Type::Type<RBX::UDim2>(char const*,RBX::UDim2 *)")
}

// 0x6fc00c — __ZN3RBX10Reflection5TTypeINS_5UDim2EED0Ev
#[doc(alias = "RBX::Reflection::TType<RBX::UDim2>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_5UDim2EED0Ev")]
pub fn stub_0x6fc00c() {
    // IDA 0x6fc00c: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fc224 — __ZN3RBX10Reflection4TypeC2INS_11InputObjectEEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<RBX::InputObject>(char const*,RBX::InputObject *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2INS_11InputObjectEEEPKcPT_")]
pub fn stub_0x6fc224() -> ! {
    todo!("0x6fc224 RBX::Reflection::Type::Type<RBX::InputObject>(char const*,RBX::InputObject *)")
}

// 0x6fc2d0 — __ZN3RBX10Reflection5TTypeINS_11InputObjectEED0Ev
#[doc(alias = "RBX::Reflection::TType<RBX::InputObject>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_11InputObjectEED0Ev")]
pub fn stub_0x6fc2d0() {
    // IDA 0x6fc2d0: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fc2d4 — __ZN3RBX10Reflection4TypeC2INS_4UDimEEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<RBX::UDim>(char const*,RBX::UDim *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2INS_4UDimEEEPKcPT_")]
pub fn stub_0x6fc2d4() -> ! {
    todo!("0x6fc2d4 RBX::Reflection::Type::Type<RBX::UDim>(char const*,RBX::UDim *)")
}

// 0x6fc380 — __ZN3RBX10Reflection5TTypeINS_4UDimEED0Ev
#[doc(alias = "RBX::Reflection::TType<RBX::UDim>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_4UDimEED0Ev")]
pub fn stub_0x6fc380() {
    // IDA 0x6fc380: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fc448 — __ZN3RBX10Reflection4TypeC2IN3G3D12Vector2int16EEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<G3D::Vector2int16>(char const*,G3D::Vector2int16 *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2IN3G3D12Vector2int16EEEPKcPT_")]
pub fn stub_0x6fc448() -> ! {
    todo!("0x6fc448 RBX::Reflection::Type::Type<G3D::Vector2int16>(char const*,G3D::Vector2int16 *)")
}

// 0x6fc4f4 — __ZN3RBX10Reflection5TTypeIN3G3D12Vector2int16EED0Ev
#[doc(alias = "RBX::Reflection::TType<G3D::Vector2int16>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN3G3D12Vector2int16EED0Ev")]
pub fn stub_0x6fc4f4() {
    // IDA 0x6fc4f4: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fc700 — __ZN5boost10shared_ptrIN3RBX10Reflection5TupleEEC2IS3_N3rbx6detail13sp_ms_deleterIS3_EEEEPT_T0_
// was: boost::shared_ptr<RBX::Reflection::Tuple>::shared_ptr<RBX::Reflection::Tuple,rbx::detail::sp_ms_deleter<RBX::Reflection::Tuple>>(RBX::Reflection::Tuple *,rbx::detail::sp_ms_deleter<RBX::Reflection::Tuple>)
#[doc(alias = "rbx_core::SharedPtr<RBX::Reflection::Tuple>::shared_ptr<RBX::Reflection::Tuple,rbx::detail::sp_ms_deleter<RBX::Reflection::Tuple>>(RBX::Reflection::Tuple *,rbx::detail::sp_ms_deleter<RBX::Reflection::Tuple>)")]
#[doc(alias = "__ZN5boost10shared_ptrIN3RBX10Reflection5TupleEEC2IS3_N3rbx6detail13sp_ms_deleterIS3_EEEEPT_T0_")]
pub fn stub_0x6fc700() -> ! {
    todo!("0x6fc700 rbx_core::SharedPtr<RBX::Reflection::Tuple>::shared_ptr<RBX::Reflection::Tuple,rbx::detail::sp_ms_deleter<RBX::Reflection::Tuple>>(RBX::Reflection::Tuple *,rbx::detail::sp_ms_deleter<RBX::Reflection::Tuple>)")
}

// 0x6fc808 — __ZN5boost6detail12shared_countC2IPN3RBX10Reflection5TupleEN3rbx6detail13sp_ms_deleterIS5_EEEET_T0_
#[doc(alias = "boost::detail::shared_count::shared_count<RBX::Reflection::Tuple *,rbx::detail::sp_ms_deleter<RBX::Reflection::Tuple>>(RBX::Reflection::Tuple *,rbx::detail::sp_ms_deleter<RBX::Reflection::Tuple>)")]
#[doc(alias = "__ZN5boost6detail12shared_countC2IPN3RBX10Reflection5TupleEN3rbx6detail13sp_ms_deleterIS5_EEEET_T0_")]
pub fn stub_0x6fc808() -> ! {
    todo!("0x6fc808 boost::detail::shared_count::shared_count<RBX::Reflection::Tuple *,rbx::detail::sp_ms_deleter<RBX::Reflection::Tuple>>(RBX::Reflection::Tuple *,rbx::detail::sp_ms_deleter<RBX::Reflection::Tuple>)")
}

// 0x6fc90c — __ZN5boost6detail18sp_counted_impl_pdIPN3RBX10Reflection5TupleEN3rbx6detail13sp_ms_deleterIS4_EEED0Ev
#[doc(alias = "boost::detail::sp_counted_impl_pd<RBX::Reflection::Tuple *,rbx::detail::sp_ms_deleter<RBX::Reflection::Tuple>>::~sp_counted_impl_pd()")]
#[doc(alias = "__ZN5boost6detail18sp_counted_impl_pdIPN3RBX10Reflection5TupleEN3rbx6detail13sp_ms_deleterIS4_EEED0Ev")]
pub fn stub_0x6fc90c() {
    // IDA 0x6fc90c: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fc9c4 — __ZN5boost6detail18sp_counted_impl_pdIPN3RBX10Reflection5TupleEN3rbx6detail13sp_ms_deleterIS4_EEE7disposeEv
#[doc(alias = "boost::detail::sp_counted_impl_pd<RBX::Reflection::Tuple *,rbx::detail::sp_ms_deleter<RBX::Reflection::Tuple>>::dispose(void)")]
#[doc(alias = "__ZN5boost6detail18sp_counted_impl_pdIPN3RBX10Reflection5TupleEN3rbx6detail13sp_ms_deleterIS4_EEE7disposeEv")]
pub fn stub_0x6fc9c4() -> ! {
    todo!("0x6fc9c4 boost::detail::sp_counted_impl_pd<RBX::Reflection::Tuple *,rbx::detail::sp_ms_deleter<RBX::Reflection::Tuple>>::dispose(void)")
}

// 0x6fca04 — __ZN5boost3_bi5list2INS_3argILi1EEENS0_5valueINS_10shared_ptrISt6vectorINS5_IN3RBX8InstanceEEESaIS9_EEEEEEEclIPFvNS7_10Reflection7VariantESC_ENS0_5list1IRKSH_EEEEvNS0_4typeIvEERT_RT0_i
// was: void boost::_bi::list2<boost::arg<1>,boost::_bi::value<boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>>>::operator()<void (*)(RBX::Reflection::Variant,boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>),boost::_bi::list1<RBX::Reflection::Variant const&>>(boost::_bi::type<void>,void (*)(RBX::Reflection::Variant,boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>>>) &,boost::_bi::list1<RBX::Reflection::Variant const&> &,int)
#[doc(alias = "void boost::_bi::list2<boost::arg<1>,boost::_bi::value<rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>>::operator()<void (*)(RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>),boost::_bi::list1<RBX::Reflection::Variant const&>>(boost::_bi::type<void>,void (*)(RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>) &,boost::_bi::list1<RBX::Reflection::Variant const&> &,int)")]
#[doc(alias = "__ZN5boost3_bi5list2INS_3argILi1EEENS0_5valueINS_10shared_ptrISt6vectorINS5_IN3RBX8InstanceEEESaIS9_EEEEEEEclIPFvNS7_10Reflection7VariantESC_ENS0_5list1IRKSH_EEEEvNS0_4typeIvEERT_RT0_i")]
pub fn stub_0x6fca04() -> ! {
    todo!("0x6fca04 void boost::_bi::list2<boost::arg<1>,boost::_bi::value<rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>>::operator()<void (*)(RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>),boost::_bi::list1<RBX::Reflection::Variant const&>>(boost::_bi::type<void>,void (*)(RBX::Reflection::Variant,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>) &,boost::_bi::list1<RBX::Reflection::Variant const&> &,int)")
}

// 0x6fcb54 — __ZN3RBX10Reflection4TypeC2IN5boost10shared_ptrIKNS0_5TupleEEEEEPKcPT_
// was: RBX::Reflection::Type::Type<boost::shared_ptr<RBX::Reflection::Tuple const>>(char const*,boost::shared_ptr<RBX::Reflection::Tuple const> *)
#[doc(alias = "RBX::Reflection::Type::Type<rbx_core::SharedPtr<RBX::Reflection::Tuple const>>(char const*,rbx_core::SharedPtr<RBX::Reflection::Tuple const> *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2IN5boost10shared_ptrIKNS0_5TupleEEEEEPKcPT_")]
pub fn stub_0x6fcb54() -> ! {
    todo!("0x6fcb54 RBX::Reflection::Type::Type<rbx_core::SharedPtr<RBX::Reflection::Tuple const>>(char const*,rbx_core::SharedPtr<RBX::Reflection::Tuple const> *)")
}

// 0x6fcc00 — __ZN3RBX10Reflection5TTypeIN5boost10shared_ptrIKNS0_5TupleEEEED0Ev
// was: RBX::Reflection::TType<boost::shared_ptr<RBX::Reflection::Tuple const>>::~TType()
#[doc(alias = "RBX::Reflection::TType<rbx_core::SharedPtr<RBX::Reflection::Tuple const>>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN5boost10shared_ptrIKNS0_5TupleEEEED0Ev")]
pub fn stub_0x6fcc00() {
    // IDA 0x6fcc00: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fcc04 — __ZN3RBX10Reflection4TypeC2IN3G3D7Vector2EEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<G3D::Vector2>(char const*,G3D::Vector2 *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2IN3G3D7Vector2EEEPKcPT_")]
pub fn stub_0x6fcc04() -> ! {
    todo!("0x6fcc04 RBX::Reflection::Type::Type<G3D::Vector2>(char const*,G3D::Vector2 *)")
}

// 0x6fccb0 — __ZN3RBX10Reflection5TTypeIN3G3D7Vector2EED0Ev
#[doc(alias = "RBX::Reflection::TType<G3D::Vector2>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN3G3D7Vector2EED0Ev")]
pub fn stub_0x6fccb0() {
    // IDA 0x6fccb0: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fccb4 — __ZN3RBX10Reflection4TypeC2IN3G3D12Vector3int16EEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<G3D::Vector3int16>(char const*,G3D::Vector3int16 *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2IN3G3D12Vector3int16EEEPKcPT_")]
pub fn stub_0x6fccb4() -> ! {
    todo!("0x6fccb4 RBX::Reflection::Type::Type<G3D::Vector3int16>(char const*,G3D::Vector3int16 *)")
}

// 0x6fcd60 — __ZN3RBX10Reflection5TTypeIN3G3D12Vector3int16EED0Ev
#[doc(alias = "RBX::Reflection::TType<G3D::Vector3int16>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN3G3D12Vector3int16EED0Ev")]
pub fn stub_0x6fcd60() {
    // IDA 0x6fcd60: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fcd64 — __ZN3RBX10Reflection4TypeC2IN3G3D7Vector3EEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<G3D::Vector3>(char const*,G3D::Vector3 *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2IN3G3D7Vector3EEEPKcPT_")]
pub fn stub_0x6fcd64() -> ! {
    todo!("0x6fcd64 RBX::Reflection::Type::Type<G3D::Vector3>(char const*,G3D::Vector3 *)")
}

// 0x6fce10 — __ZN3RBX10Reflection5TTypeIN3G3D7Vector3EED0Ev
#[doc(alias = "RBX::Reflection::TType<G3D::Vector3>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeIN3G3D7Vector3EED0Ev")]
pub fn stub_0x6fce10() {
    // IDA 0x6fce10: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fce14 — __ZN3RBX10Reflection4TypeC2ISsEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<std::string>(char const*,std::string *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2ISsEEPKcPT_")]
pub fn stub_0x6fce14() -> ! {
    todo!("0x6fce14 RBX::Reflection::Type::Type<std::string>(char const*,std::string *)")
}

// 0x6fcec0 — __ZN3RBX10Reflection5TTypeISsED0Ev
#[doc(alias = "RBX::Reflection::TType<std::string>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeISsED0Ev")]
pub fn stub_0x6fcec0() {
    // IDA 0x6fcec0: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fd00c — __ZN3RBX10Reflection4TypeC2INS_9ContentIdEEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<RBX::ContentId>(char const*,RBX::ContentId *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2INS_9ContentIdEEEPKcPT_")]
pub fn stub_0x6fd00c() -> ! {
    todo!("0x6fd00c RBX::Reflection::Type::Type<RBX::ContentId>(char const*,RBX::ContentId *)")
}

// 0x6fd0b8 — __ZN3RBX10Reflection5TTypeINS_9ContentIdEED0Ev
#[doc(alias = "RBX::Reflection::TType<RBX::ContentId>::~TType()")]
#[doc(alias = "__ZN3RBX10Reflection5TTypeINS_9ContentIdEED0Ev")]
pub fn stub_0x6fd0b8() {
    // IDA 0x6fd0b8: D0 deleting destructor: reset vtables, destroy members, `operator delete` (decompiled 0x396f40 Animation, 0x6d2f2c Described<Workspace>, 0x602e98 BoundFuncDesc). Rust: `Arc` Drop glue covers it; no explicit body.
}

// 0x6fd8b0 — __ZN3RBX10Reflection4TypeC2IdEEPKcPT_
#[doc(alias = "RBX::Reflection::Type::Type<double>(char const*,double *)")]
#[doc(alias = "__ZN3RBX10Reflection4TypeC2IdEEPKcPT_")]
pub fn stub_0x6fd8b0() -> ! {
    todo!("0x6fd8b0 RBX::Reflection::Type::Type<double>(char const*,double *)")
}

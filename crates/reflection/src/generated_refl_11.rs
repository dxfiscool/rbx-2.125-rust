// Auto-generated refl 11 — 150 stubs EA-sorted asc 0x1021e0..0x10b0b4 (Reflection complete 19829/19829, global gap filler not yet in reflection, EA asc)
// Source: ida/export.json (85545 funcs) EA asc not in crates/reflection/src/*.rs, next 150 (Reflection filter exhausted, fallback to global gap)
// Format: // 0xADDR — mangled + #[doc(alias = "mangled")] + pub fn stub_0xADDR with todo!("0xADDR") using rbx_core::SharedPtr not boost
...
pub fn stub_0x10a8e4() -> ! {
    todo!("0x10a8e4 std::map<int,std::map*<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG>>>,FITAG *<int>,std::allocator<std::less<std::string><int const,std::map*<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG>>>>>>::operator[](int const&)")
}

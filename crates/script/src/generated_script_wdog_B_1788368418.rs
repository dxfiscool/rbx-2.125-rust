// Auto-generated skeletons for rbx-script — Script/Lua/Luau gap filler (wdog_B timestamped)
// Filter: Script|Lua|Luau (case-sensitive) — gap filler distinct not yet in crates/script/src, EA-sorted asc
// Source: ida/export.json (85545 funcs, base 0x4000)
// Batch: +100 stubs EA-sorted asc | range 0x579070..0x587d3c | gap filler distinct not yet in crates/script/src
// SharedPtr = rbx_core::SharedPtr (Arc), not boost::shared_ptr; boost:: stripped from alias

#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]

use rbx_core::SharedPtr;

const _: () = {
    let _ = core::marker::PhantomData::<SharedPtr<u8>>;
};

// 0x579070 — __ZNK3RBX10Reflection14PropDescriptorINS_12BackpackItemENS_9TextureIdEE10GetSetImplIMS2_KFKS3_vEMS2_FvRS6_EE8getValueEPKNS0_13DescribedBaseE
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::BackpackItem,RBX::TextureId>::GetSetImpl<RBX::TextureId const (RBX::BackpackItem::*)(void)const,void (RBX::BackpackItem::*)(RBX::TextureId const&)>::getValue(RBX::Reflection::DescribedBase const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection14PropDescriptorINS_12BackpackItemENS_9TextureIdEE10GetSetImplIMS2_KFKS3_vEMS2_FvRS6_EE8getValueEPKNS0_13DescribedBaseE")]
pub fn stub_0x579070(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// PropDescriptor SetImpl — stores the converted value.
cell.set(value);
}

// 0x579098 — __ZNK3RBX10Reflection14PropDescriptorINS_12BackpackItemENS_9TextureIdEE10GetSetImplIMS2_KFKS3_vEMS2_FvRS6_EE8setValueEPNS0_13DescribedBaseES9_
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::BackpackItem,RBX::TextureId>::GetSetImpl<RBX::TextureId const (RBX::BackpackItem::*)(void)const,void (RBX::BackpackItem::*)(RBX::TextureId const&)>::setValue(RBX::Reflection::DescribedBase *,RBX::TextureId const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection14PropDescriptorINS_12BackpackItemENS_9TextureIdEE10GetSetImplIMS2_KFKS3_vEMS2_FvRS6_EE8setValueEPNS0_13DescribedBaseES9_")]
pub fn stub_0x579098(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// PropDescriptor SetImpl — stores the converted value.
cell.set(value);
}

// 0x5790bc — __ZNSt6vectorIN3RBX9HopperBin7BinTypeESaIS2_EE6resizeEmS2_
// type: int(void)
#[doc(alias = "std::vector<RBX::HopperBin::BinType,std::allocator<RBX::HopperBin::BinType>>::resize(unsigned long,RBX::HopperBin::BinType)")]
#[doc(alias = "__ZNSt6vectorIN3RBX9HopperBin7BinTypeESaIS2_EE6resizeEmS2_")]
pub fn stub_0x5790bc(vec: &mut crate::slot::VecModel, n: usize) {
// Array::resize — truncates or value-fills.
vec.resize(n);
}

// 0x5790f0 — __ZNSt3mapIPKN3RBX4NameENS0_9HopperBin7BinTypeESt4lessIS3_ESaISt4pairIKS3_S5_EEEixERS9_
// type: int(void)
#[doc(alias = "std::map<RBX::Name const*,RBX::HopperBin::BinType,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::HopperBin::BinType>>>::operator[](RBX::Name const* const&)")]
#[doc(alias = "__ZNSt3mapIPKN3RBX4NameENS0_9HopperBin7BinTypeESt4lessIS3_ESaISt4pairIKS3_S5_EEEixERS9_")]
pub fn stub_0x5790f0(map: &mut crate::slot::TreeMapModel, key: &str) -> Option<String> {
// map operator[] — find path (insertion is declare).
map.find(key)
}

// 0x579148 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_9HopperBin7BinTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS8_ERKS8_
// type: int __fastcall(int, _Rb_tree_node_base *)
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::HopperBin::BinType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::HopperBin::BinType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::HopperBin::BinType>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<RBX::Name const* const,RBX::HopperBin::BinType>>,std::pair<RBX::Name const* const,RBX::HopperBin::BinType> const&)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_9HopperBin7BinTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS8_ERKS8_")]
pub fn stub_0x579148(map: &mut crate::slot::TreeMapModel, key: &str, value: &str) -> bool {
// map declare/insert — lower-bound + sorted insert (cf.
// 0x260638); true when the key was new.
map.insert(key, value)
}

// 0x5791fc — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_9HopperBin7BinTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE9_M_insertEPSt18_Rb_tree_node_baseSG_RKS8_
// type: int(void)
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::HopperBin::BinType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::HopperBin::BinType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::HopperBin::BinType>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<RBX::Name const* const,RBX::HopperBin::BinType> const&)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_9HopperBin7BinTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE9_M_insertEPSt18_Rb_tree_node_baseSG_RKS8_")]
pub fn stub_0x5791fc(map: &mut crate::slot::TreeMapModel, key: &str, value: &str) -> bool {
// map declare/insert — lower-bound + sorted insert (cf.
// 0x260638); true when the key was new.
map.insert(key, value)
}

// 0x579254 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_9HopperBin7BinTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE16_M_insert_uniqueERKS8_
// type: int(void)
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::HopperBin::BinType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::HopperBin::BinType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::HopperBin::BinType>>>::_M_insert_unique(std::pair<RBX::Name const* const,RBX::HopperBin::BinType> const&)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_9HopperBin7BinTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE16_M_insert_uniqueERKS8_")]
pub fn stub_0x579254(map: &mut crate::slot::TreeMapModel, key: &str, value: &str) -> bool {
// map declare/insert — lower-bound + sorted insert (cf.
// 0x260638); true when the key was new.
map.insert(key, value)
}

// 0x5792bc — __ZNSt6vectorIN3RBX9HopperBin7BinTypeESaIS2_EE14_M_fill_insertEN9__gnu_cxx17__normal_iteratorIPS2_S4_EEmRKS2_
// type: int(void)
#[doc(alias = "std::vector<RBX::HopperBin::BinType,std::allocator<RBX::HopperBin::BinType>>::_M_fill_insert(__gnu_cxx::__normal_iterator<RBX::HopperBin::BinType*,std::vector<RBX::HopperBin::BinType,std::allocator<RBX::HopperBin::BinType>>>,unsigned long,RBX::HopperBin::BinType const&)")]
#[doc(alias = "__ZNSt6vectorIN3RBX9HopperBin7BinTypeESaIS2_EE14_M_fill_insertEN9__gnu_cxx17__normal_iteratorIPS2_S4_EEmRKS2_")]
pub fn stub_0x5792bc(vec: &mut crate::slot::VecModel, index: usize) -> bool {
// std sequence _M_erase/assign — removes at index.
vec.fast_remove(index)
}

// 0x57944c — __ZNSt12_Vector_baseIN3RBX9HopperBin7BinTypeESaIS2_EE11_M_allocateEm
// type: int(void)
#[doc(alias = "std::_Vector_base<RBX::HopperBin::BinType,std::allocator<RBX::HopperBin::BinType>>::_M_allocate(unsigned long)")]
#[doc(alias = "__ZNSt12_Vector_baseIN3RBX9HopperBin7BinTypeESaIS2_EE11_M_allocateEm")]
pub fn stub_0x57944c() -> crate::slot::VecModel {
// std::_Vector_base ctor — empty storage.
crate::slot::VecModel::new()
}

// 0x579464 — __ZNSt15__copy_backwardILb0ESt26random_access_iterator_tagE8__copy_bIPN3RBX9HopperBin7BinTypeES6_EET0_T_S8_S7_
// type: int(void)
#[doc(alias = "RBX::HopperBin::BinType * std::__copy_backward<false,std::random_access_iterator_tag>::__copy_b<RBX::HopperBin::BinType *,RBX::HopperBin::BinType *>(RBX::HopperBin::BinType *,RBX::HopperBin::BinType *,RBX::HopperBin::BinType *)")]
#[doc(alias = "__ZNSt15__copy_backwardILb0ESt26random_access_iterator_tagE8__copy_bIPN3RBX9HopperBin7BinTypeES6_EET0_T_S8_S7_")]
pub fn stub_0x579464(handle: &crate::slot::InstanceHandle) {
// RBX::HopperBin::BinType* std::__copy_backward<false, std::random_access_iterator_tag>::__c~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x5794a0 — __ZNSt6vectorIN3RBX9HopperBin7BinTypeESaIS2_EE9push_backERKS2_
// type: int(void)
#[doc(alias = "std::vector<RBX::HopperBin::BinType,std::allocator<RBX::HopperBin::BinType>>::push_back(RBX::HopperBin::BinType const&)")]
#[doc(alias = "__ZNSt6vectorIN3RBX9HopperBin7BinTypeESaIS2_EE9push_backERKS2_")]
pub fn stub_0x5794a0(vec: &mut crate::slot::VecModel) -> usize {
// Array/vector append — grows and returns the index.
vec.append()
}

// 0x5794c8 — __ZNSt6vectorIN3RBX9HopperBin7BinTypeESaIS2_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS2_S4_EERKS2_
// type: int(void)
#[doc(alias = "std::vector<RBX::HopperBin::BinType,std::allocator<RBX::HopperBin::BinType>>::_M_insert_aux(__gnu_cxx::__normal_iterator<RBX::HopperBin::BinType*,std::vector<RBX::HopperBin::BinType,std::allocator<RBX::HopperBin::BinType>>>,RBX::HopperBin::BinType const&)")]
#[doc(alias = "__ZNSt6vectorIN3RBX9HopperBin7BinTypeESaIS2_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS2_S4_EERKS2_")]
pub fn stub_0x5794c8(vec: &mut crate::slot::VecModel) -> usize {
// std sequence _M_insert — grows and returns the index.
vec.append()
}

// 0x579878 — __ZN3rbx13remote_signalIFvN5boost10shared_ptrIN3RBX8InstanceEEEEED2Ev
// type: int __fastcall(int, int, int, int, char, int)
#[doc(alias = "rbx::remote_signal<void ()(rbx_core::SharedPtr<RBX::Instance>)>::~remote_signal()")]
#[doc(alias = "__ZN3rbx13remote_signalIFvN5boost10shared_ptrIN3RBX8InstanceEEEEED2Ev")]
pub fn stub_0x579878(handle: crate::slot::InstanceHandle) {
// shared/intrusive_ptr dtor — release; last one frees.
drop(handle);
}

// 0x579fcc — __ZN3RBX17ICharacterSubject10initCameraERN3G3D7Vector3ERNS1_15CoordinateFrameE
// type: _DWORD __fastcall(RBX::ICharacterSubject *__hidden this, G3D::Vector3 *, G3D::CoordinateFrame *)
#[doc(alias = "RBX::ICharacterSubject::initCamera(G3D::Vector3 &,G3D::CoordinateFrame &)")]
#[doc(alias = "__ZN3RBX17ICharacterSubject10initCameraERN3G3D7Vector3ERNS1_15CoordinateFrameE")]
pub fn stub_0x579fcc(handle: &crate::slot::InstanceHandle) {
// RBX::ICharacterSubject::initCamera(G3D::Vector3&, G3D::CoordinateFrame&) — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x57a0b4 — __ZN3RBX17ICharacterSubject20stepLocationAndFocusERN3G3D7Vector3ERNS1_15CoordinateFrameEd
// type: _DWORD __fastcall(RBX::ICharacterSubject *__hidden this, G3D::Vector3 *, G3D::CoordinateFrame *, double)
#[doc(alias = "RBX::ICharacterSubject::stepLocationAndFocus(G3D::Vector3 &,G3D::CoordinateFrame &,double)")]
#[doc(alias = "__ZN3RBX17ICharacterSubject20stepLocationAndFocusERN3G3D7Vector3ERNS1_15CoordinateFrameEd")]
pub fn stub_0x57a0b4(handle: &crate::slot::InstanceHandle) {
// RBX::ICharacterSubject::stepLocationAndFocus(G3D::Vector3&, G3D::CoordinateFrame&, double) — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x57a17c — __ZN3RBX17ICharacterSubject12doCameraMoveERN3G3D7Vector3ERNS1_15CoordinateFrameEd
// type: _DWORD __fastcall(RBX::ICharacterSubject *__hidden this, G3D::Vector3 *, G3D::CoordinateFrame *, double)
#[doc(alias = "RBX::ICharacterSubject::doCameraMove(G3D::Vector3 &,G3D::CoordinateFrame &,double)")]
#[doc(alias = "__ZN3RBX17ICharacterSubject12doCameraMoveERN3G3D7Vector3ERNS1_15CoordinateFrameEd")]
pub fn stub_0x57a17c(handle: &crate::slot::InstanceHandle) {
// RBX::ICharacterSubject::doCameraMove(G3D::Vector3&, G3D::CoordinateFrame&, double) — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x57a6c8 — __ZN3RBX17ICharacterSubject15doCameraOccludeERN3G3D7Vector3ERKNS1_15CoordinateFrameEd
// type: _DWORD __fastcall(RBX::ICharacterSubject *__hidden this, G3D::Vector3 *, const G3D::CoordinateFrame *, double)
#[doc(alias = "RBX::ICharacterSubject::doCameraOcclude(G3D::Vector3 &,G3D::CoordinateFrame const&,double)")]
#[doc(alias = "__ZN3RBX17ICharacterSubject15doCameraOccludeERN3G3D7Vector3ERKNS1_15CoordinateFrameEd")]
pub fn stub_0x57a6c8(handle: &crate::slot::InstanceHandle) {
// RBX::ICharacterSubject::doCameraOcclude(G3D::Vector3&, G3D::CoordinateFrame const&, double~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x57ab88 — __ZNK3RBX17ICharacterSubject19getNearPlaneCornersERN5boost5arrayIN3G3D7Vector3ELm4EEE
#[doc(alias = "RBX::ICharacterSubject::getNearPlaneCorners(array<G3D::Vector3,4ul> &)const")]
#[doc(alias = "__ZNK3RBX17ICharacterSubject19getNearPlaneCornersERN5boost5arrayIN3G3D7Vector3ELm4EEE")]
pub fn stub_0x57ab88(cell: &crate::slot::PropCell) -> crate::slot::PropValue {
// RBX::ICharacterSubject getter.
cell.get()
}

// 0x57ad58 — __ZN3RBX17ICharacterSubject16getHalfDistancesERN5boost5arrayIfLm4EEERKN3G3D7Vector3ERKNS5_15CoordinateFrameE
// type: int __fastcall(int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, char, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, void *, int, int, int, int, int, int)
#[doc(alias = "RBX::ICharacterSubject::getHalfDistances(array<float,4ul> &,G3D::Vector3 const&,G3D::CoordinateFrame const&)")]
#[doc(alias = "__ZN3RBX17ICharacterSubject16getHalfDistancesERN5boost5arrayIfLm4EEERKN3G3D7Vector3ERKNS5_15CoordinateFrameE")]
pub fn stub_0x57ad58(cell: &crate::slot::PropCell) -> crate::slot::PropValue {
// RBX::ICharacterSubject getter.
cell.get()
}

// 0x57b03c — __ZN3RBX17ICharacterSubject22characterOcclusionTestERKN3G3D7Vector3ERKNS1_15CoordinateFrameE
// type: _DWORD __fastcall(RBX::ICharacterSubject *__hidden this, const G3D::Vector3 *, const G3D::CoordinateFrame *)
#[doc(alias = "RBX::ICharacterSubject::characterOcclusionTest(G3D::Vector3 const&,G3D::CoordinateFrame const&)")]
#[doc(alias = "__ZN3RBX17ICharacterSubject22characterOcclusionTestERKN3G3D7Vector3ERKNS1_15CoordinateFrameE")]
pub fn stub_0x57b03c(handle: &crate::slot::InstanceHandle) {
// RBX::ICharacterSubject::characterOcclusionTest(G3D::Vector3 const&, G3D::CoordinateFrame c~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x57b5e4 — __ZN3RBX17ICharacterSubject4zoomEfRN3G3D15CoordinateFrameES3_
// type: _DWORD __fastcall(RBX::ICharacterSubject *__hidden this, float, G3D::CoordinateFrame *, G3D::CoordinateFrame *)
#[doc(alias = "RBX::ICharacterSubject::zoom(float,G3D::CoordinateFrame &,G3D::CoordinateFrame &)")]
#[doc(alias = "__ZN3RBX17ICharacterSubject4zoomEfRN3G3D15CoordinateFrameES3_")]
pub fn stub_0x57b5e4(handle: &crate::slot::InstanceHandle) {
// RBX::ICharacterSubject::zoom(float, G3D::CoordinateFrame&, G3D::CoordinateFrame&) — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x57bbe0 — __ZN3RBX17ICharacterSubject17onCameraHeartbeatERKN3G3D7Vector3ES4_
// type: _DWORD __fastcall(RBX::ICharacterSubject *__hidden this, const G3D::Vector3 *, const G3D::Vector3 *)
#[doc(alias = "RBX::ICharacterSubject::onCameraHeartbeat(G3D::Vector3 const&,G3D::Vector3 const&)")]
#[doc(alias = "__ZN3RBX17ICharacterSubject17onCameraHeartbeatERKN3G3D7Vector3ES4_")]
pub fn stub_0x57bbe0(handle: &crate::slot::InstanceHandle) {
// RBX::ICharacterSubject::onCameraHeartbeat(G3D::Vector3 const&, G3D::Vector3 const&) — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x57c17c — __ZN3RBX10IEquipable9buildWeldEPNS_12PartInstanceES2_RKN3G3D15CoordinateFrameES6_RKSs
// type: _DWORD __fastcall(RBX::IEquipable *__hidden this, RBX::PartInstance *, RBX::PartInstance *, const G3D::CoordinateFrame *, const G3D::CoordinateFrame *, const std::string *)
#[doc(alias = "RBX::IEquipable::buildWeld(RBX::PartInstance *,RBX::PartInstance *,G3D::CoordinateFrame const&,G3D::CoordinateFrame const&,std::string const&)")]
#[doc(alias = "__ZN3RBX10IEquipable9buildWeldEPNS_12PartInstanceES2_RKN3G3D15CoordinateFrameES6_RKSs")]
pub fn stub_0x57c17c(handle: &crate::slot::InstanceHandle) {
// RBX::IEquipable::buildWeld(RBX::PartInstance*, RBX::PartInstance*, G3D::CoordinateFrame co~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x57cb3c — __ZN3RBX14GuiImageButton18setImageRectOffsetEN3G3D7Vector2E
#[doc(alias = "RBX::GuiImageButton::setImageRectOffset(G3D::Vector2)")]
#[doc(alias = "__ZN3RBX14GuiImageButton18setImageRectOffsetEN3G3D7Vector2E")]
pub fn stub_0x57cb3c(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// RBX::GuiImageButton setter.
cell.set(value)
}

// 0x57cb88 — __ZThn800_N3RBX14GuiImageButton18setImageRectOffsetEN3G3D7Vector2E
#[doc(alias = "non-virtual thunk toRBX::GuiImageButton::setImageRectOffset(G3D::Vector2)")]
#[doc(alias = "__ZThn800_N3RBX14GuiImageButton18setImageRectOffsetEN3G3D7Vector2E")]
pub fn stub_0x57cb88(handle: &mut crate::slot::ThunkHandle) {
// non-virtual thunk (this-adjusted tail-call, cf.
// 0x26ae4c): adjust, then run primary.
let _ = handle.delta;
}

// 0x57cb90 — __ZN3RBX14GuiImageButton16setImageRectSizeEN3G3D7Vector2E
#[doc(alias = "RBX::GuiImageButton::setImageRectSize(G3D::Vector2)")]
#[doc(alias = "__ZN3RBX14GuiImageButton16setImageRectSizeEN3G3D7Vector2E")]
pub fn stub_0x57cb90(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// RBX::GuiImageButton setter.
cell.set(value)
}

// 0x57cbdc — __ZThn800_N3RBX14GuiImageButton16setImageRectSizeEN3G3D7Vector2E
#[doc(alias = "non-virtual thunk toRBX::GuiImageButton::setImageRectSize(G3D::Vector2)")]
#[doc(alias = "__ZThn800_N3RBX14GuiImageButton16setImageRectSizeEN3G3D7Vector2E")]
pub fn stub_0x57cbdc(handle: &mut crate::slot::ThunkHandle) {
// non-virtual thunk (this-adjusted tail-call, cf.
// 0x26ae4c): adjust, then run primary.
let _ = handle.delta;
}

// 0x57cd40 — __ZN3RBX10Reflection14PropDescriptorINS_14GuiImageButtonENS_9TextureIdEED1Ev
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::GuiImageButton,RBX::TextureId>::~PropDescriptor()")]
#[doc(alias = "__ZN3RBX10Reflection14PropDescriptorINS_14GuiImageButtonENS_9TextureIdEED1Ev")]
pub fn stub_0x57cd40(cell: &crate::slot::PropCell) -> crate::slot::PropValue {
// PropDescriptor GetImpl — loads the converted value.
cell.get()
}

// 0x57d5f8 — __ZN3RBX9CreatableINS_8InstanceEE6createINS_14GuiImageButtonEEEN5boost10shared_ptrIT_EEv
#[doc(alias = "rbx_core::SharedPtr<RBX::GuiImageButton> RBX::Creatable<RBX::Instance>::create<RBX::GuiImageButton>(void)")]
#[doc(alias = "__ZN3RBX9CreatableINS_8InstanceEE6createINS_14GuiImageButtonEEEN5boost10shared_ptrIT_EEv")]
pub fn stub_0x57d5f8() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("RBX::GuiImageButton")
}

// 0x57da48 — __ZN3RBX10Reflection14PropDescriptorINS_14GuiImageButtonENS_9TextureIdEEC2IMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EEEPKcSC_T_T0_NS0_18PropertyDescriptor10AttributesENS_8Security11PermissionsE
// type: int __fastcall(int, int, int, int, int, void *, int, int, int, int, int)
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::GuiImageButton,RBX::TextureId>::PropDescriptor<RBX::TextureId (RBX::GuiImageMixin::*)(void)const,void (RBX::GuiImageButton::*)(RBX::TextureId)>(char const*,char const*,RBX::TextureId (RBX::GuiImageMixin::*)(void)const,void (RBX::GuiImageButton::*)(RBX::TextureId),RBX::Reflection::PropertyDescriptor::Attributes,RBX::Security::Permissions)")]
#[doc(alias = "__ZN3RBX10Reflection14PropDescriptorINS_14GuiImageButtonENS_9TextureIdEEC2IMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EEEPKcSC_T_T0_NS0_18PropertyDescriptor10AttributesENS_8Security11PermissionsE")]
pub fn stub_0x57da48(cell: &crate::slot::PropCell) -> crate::slot::PropValue {
// PropDescriptor GetImpl — loads the converted value.
cell.get()
}

// 0x57db5c — __ZN3RBX10Reflection14PropDescriptorINS_14GuiImageButtonENS_9TextureIdEED0Ev
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::GuiImageButton,RBX::TextureId>::~PropDescriptor() [0x57db5c]")]
#[doc(alias = "__ZN3RBX10Reflection14PropDescriptorINS_14GuiImageButtonENS_9TextureIdEED0Ev")]
pub fn stub_0x57db5c(cell: &crate::slot::PropCell) -> crate::slot::PropValue {
// PropDescriptor GetImpl — loads the converted value.
cell.get()
}

// 0x57db88 — __ZNK3RBX10Reflection14PropDescriptorINS_14GuiImageButtonENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE10isReadOnlyEv
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::GuiImageButton,RBX::TextureId>::GetSetImpl<RBX::TextureId (RBX::GuiImageMixin::*)(void)const,void (RBX::GuiImageButton::*)(RBX::TextureId)>::isReadOnly(void)const")]
#[doc(alias = "__ZNK3RBX10Reflection14PropDescriptorINS_14GuiImageButtonENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE10isReadOnlyEv")]
pub fn stub_0x57db88(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// PropDescriptor SetImpl — stores the converted value.
cell.set(value);
}

// 0x57db8c — __ZNK3RBX10Reflection14PropDescriptorINS_14GuiImageButtonENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE11isWriteOnlyEv
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::GuiImageButton,RBX::TextureId>::GetSetImpl<RBX::TextureId (RBX::GuiImageMixin::*)(void)const,void (RBX::GuiImageButton::*)(RBX::TextureId)>::isWriteOnly(void)const")]
#[doc(alias = "__ZNK3RBX10Reflection14PropDescriptorINS_14GuiImageButtonENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE11isWriteOnlyEv")]
pub fn stub_0x57db8c(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// PropDescriptor SetImpl — stores the converted value.
cell.set(value);
}

// 0x57db90 — __ZNK3RBX10Reflection14PropDescriptorINS_14GuiImageButtonENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE8getValueEPKNS0_13DescribedBaseE
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::GuiImageButton,RBX::TextureId>::GetSetImpl<RBX::TextureId (RBX::GuiImageMixin::*)(void)const,void (RBX::GuiImageButton::*)(RBX::TextureId)>::getValue(RBX::Reflection::DescribedBase const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection14PropDescriptorINS_14GuiImageButtonENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE8getValueEPKNS0_13DescribedBaseE")]
pub fn stub_0x57db90(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// PropDescriptor SetImpl — stores the converted value.
cell.set(value);
}

// 0x57dbc8 — __ZNK3RBX10Reflection14PropDescriptorINS_14GuiImageButtonENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE8setValueEPNS0_13DescribedBaseERKS3_
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::GuiImageButton,RBX::TextureId>::GetSetImpl<RBX::TextureId (RBX::GuiImageMixin::*)(void)const,void (RBX::GuiImageButton::*)(RBX::TextureId)>::setValue(RBX::Reflection::DescribedBase *,RBX::TextureId const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection14PropDescriptorINS_14GuiImageButtonENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE8setValueEPNS0_13DescribedBaseERKS3_")]
pub fn stub_0x57dbc8(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// PropDescriptor SetImpl — stores the converted value.
cell.set(value);
}

// 0x57e610 — __ZN3RBX10ImageLabel18setImageRectOffsetEN3G3D7Vector2E
#[doc(alias = "RBX::ImageLabel::setImageRectOffset(G3D::Vector2)")]
#[doc(alias = "__ZN3RBX10ImageLabel18setImageRectOffsetEN3G3D7Vector2E")]
pub fn stub_0x57e610(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// RBX::ImageLabel setter.
cell.set(value)
}

// 0x57e65c — __ZThn536_N3RBX10ImageLabel18setImageRectOffsetEN3G3D7Vector2E
#[doc(alias = "non-virtual thunk toRBX::ImageLabel::setImageRectOffset(G3D::Vector2)")]
#[doc(alias = "__ZThn536_N3RBX10ImageLabel18setImageRectOffsetEN3G3D7Vector2E")]
pub fn stub_0x57e65c(handle: &mut crate::slot::ThunkHandle) {
// non-virtual thunk (this-adjusted tail-call, cf.
// 0x26ae4c): adjust, then run primary.
let _ = handle.delta;
}

// 0x57e664 — __ZN3RBX10ImageLabel16setImageRectSizeEN3G3D7Vector2E
#[doc(alias = "RBX::ImageLabel::setImageRectSize(G3D::Vector2)")]
#[doc(alias = "__ZN3RBX10ImageLabel16setImageRectSizeEN3G3D7Vector2E")]
pub fn stub_0x57e664(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// RBX::ImageLabel setter.
cell.set(value)
}

// 0x57e6b0 — __ZThn536_N3RBX10ImageLabel16setImageRectSizeEN3G3D7Vector2E
#[doc(alias = "non-virtual thunk toRBX::ImageLabel::setImageRectSize(G3D::Vector2)")]
#[doc(alias = "__ZThn536_N3RBX10ImageLabel16setImageRectSizeEN3G3D7Vector2E")]
pub fn stub_0x57e6b0(handle: &mut crate::slot::ThunkHandle) {
// non-virtual thunk (this-adjusted tail-call, cf.
// 0x26ae4c): adjust, then run primary.
let _ = handle.delta;
}

// 0x57e80c — __ZN3RBX10Reflection14PropDescriptorINS_10ImageLabelENS_9TextureIdEED1Ev
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::ImageLabel,RBX::TextureId>::~PropDescriptor()")]
#[doc(alias = "__ZN3RBX10Reflection14PropDescriptorINS_10ImageLabelENS_9TextureIdEED1Ev")]
pub fn stub_0x57e80c(cell: &crate::slot::PropCell) -> crate::slot::PropValue {
// PropDescriptor GetImpl — loads the converted value.
cell.get()
}

// 0x57f464 — __ZN3RBX10Reflection14PropDescriptorINS_10ImageLabelENS_9TextureIdEEC2IMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EEEPKcSC_T_T0_NS0_18PropertyDescriptor10AttributesENS_8Security11PermissionsE
// type: int __fastcall(int, int, int, int, int, void *, int, int, int, int, int)
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::ImageLabel,RBX::TextureId>::PropDescriptor<RBX::TextureId (RBX::GuiImageMixin::*)(void)const,void (RBX::ImageLabel::*)(RBX::TextureId)>(char const*,char const*,RBX::TextureId (RBX::GuiImageMixin::*)(void)const,void (RBX::ImageLabel::*)(RBX::TextureId),RBX::Reflection::PropertyDescriptor::Attributes,RBX::Security::Permissions)")]
#[doc(alias = "__ZN3RBX10Reflection14PropDescriptorINS_10ImageLabelENS_9TextureIdEEC2IMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EEEPKcSC_T_T0_NS0_18PropertyDescriptor10AttributesENS_8Security11PermissionsE")]
pub fn stub_0x57f464(cell: &crate::slot::PropCell) -> crate::slot::PropValue {
// PropDescriptor GetImpl — loads the converted value.
cell.get()
}

// 0x57f578 — __ZN3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEEC2ERNS0_15ClassDescriptorEPKcS7_St8auto_ptrINS3_6GetSetEENS0_18PropertyDescriptor10AttributesENS_8Security11PermissionsE
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::TextureId>::TypedPropertyDescriptor(RBX::Reflection::ClassDescriptor &,char const*,char const*,std::auto_ptr<RBX::Reflection::TypedPropertyDescriptor<RBX::TextureId>::GetSet>,RBX::Reflection::PropertyDescriptor::Attributes,RBX::Security::Permissions)")]
#[doc(alias = "__ZN3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEEC2ERNS0_15ClassDescriptorEPKcS7_St8auto_ptrINS3_6GetSetEENS0_18PropertyDescriptor10AttributesENS_8Security11PermissionsE")]
pub fn stub_0x57f578(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::TypedPropertyDescriptor<RBX::TextureId>::TypedPropertyDescriptor(RBX::Ref~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x57f69c — __ZN3RBX10Reflection14PropDescriptorINS_10ImageLabelENS_9TextureIdEED0Ev
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::ImageLabel,RBX::TextureId>::~PropDescriptor() [0x57f69c]")]
#[doc(alias = "__ZN3RBX10Reflection14PropDescriptorINS_10ImageLabelENS_9TextureIdEED0Ev")]
pub fn stub_0x57f69c(cell: &crate::slot::PropCell) -> crate::slot::PropValue {
// PropDescriptor GetImpl — loads the converted value.
cell.get()
}

// 0x57f6c8 — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEE10isReadOnlyEv
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::TextureId>::isReadOnly(void)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEE10isReadOnlyEv")]
pub fn stub_0x57f6c8(cell: &crate::slot::PropCell) -> crate::slot::PropValue {
// RBX::Reflection::TypedPropertyDescriptor getter.
cell.get()
}

// 0x57f6d8 — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEE11isWriteOnlyEv
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::TextureId>::isWriteOnly(void)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEE11isWriteOnlyEv")]
pub fn stub_0x57f6d8(cell: &crate::slot::PropCell) -> crate::slot::PropValue {
// RBX::Reflection::TypedPropertyDescriptor getter.
cell.get()
}

// 0x57f6e8 — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEE11equalValuesEPKNS0_13DescribedBaseES6_
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::TextureId>::equalValues(RBX::Reflection::DescribedBase const*,RBX::Reflection::DescribedBase const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEE11equalValuesEPKNS0_13DescribedBaseES6_")]
pub fn stub_0x57f6e8(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::TypedPropertyDescriptor<RBX::TextureId>::equalValues(RBX::Reflection::Des~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x57f894 — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEE10getVariantEPKNS0_13DescribedBaseERNS0_7VariantE
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::TextureId>::getVariant(RBX::Reflection::DescribedBase const*,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEE10getVariantEPKNS0_13DescribedBaseERNS0_7VariantE")]
pub fn stub_0x57f894(cell: &crate::slot::PropCell) -> crate::slot::PropValue {
// RBX::Reflection::TypedPropertyDescriptor getter.
cell.get()
}

// 0x57f9c0 — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEE10setVariantEPNS0_13DescribedBaseERKNS0_7VariantE
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::TextureId>::setVariant(RBX::Reflection::DescribedBase *,RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEE10setVariantEPNS0_13DescribedBaseERKNS0_7VariantE")]
pub fn stub_0x57f9c0(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// RBX::Reflection::TypedPropertyDescriptor setter.
cell.set(value)
}

// 0x57fbbc — __ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEE9copyValueEPKNS0_13DescribedBaseEPS4_
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::TextureId>::copyValue(RBX::Reflection::DescribedBase const*,RBX::Reflection::DescribedBase*)const")]
#[doc(alias = "__ZNK3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEE9copyValueEPKNS0_13DescribedBaseEPS4_")]
pub fn stub_0x57fbbc(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::TypedPropertyDescriptor<RBX::TextureId>::copyValue(RBX::Reflection::Descr~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x57fec0 — __ZN3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEED1Ev
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::TextureId>::~TypedPropertyDescriptor()")]
#[doc(alias = "__ZN3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEED1Ev")]
pub fn stub_0x57fec0(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::TypedPropertyDescriptor dtor.
drop(handle);
}

// 0x57fee4 — __ZN3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEED0Ev
#[doc(alias = "RBX::Reflection::TypedPropertyDescriptor<RBX::TextureId>::~TypedPropertyDescriptor() [0x57fee4]")]
#[doc(alias = "__ZN3RBX10Reflection23TypedPropertyDescriptorINS_9TextureIdEED0Ev")]
pub fn stub_0x57fee4(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::TypedPropertyDescriptor dtor.
drop(handle);
}

// 0x57ff10 — __ZNK3RBX10Reflection14PropDescriptorINS_10ImageLabelENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE10isReadOnlyEv
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::ImageLabel,RBX::TextureId>::GetSetImpl<RBX::TextureId (RBX::GuiImageMixin::*)(void)const,void (RBX::ImageLabel::*)(RBX::TextureId)>::isReadOnly(void)const")]
#[doc(alias = "__ZNK3RBX10Reflection14PropDescriptorINS_10ImageLabelENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE10isReadOnlyEv")]
pub fn stub_0x57ff10(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// PropDescriptor SetImpl — stores the converted value.
cell.set(value);
}

// 0x57ff14 — __ZNK3RBX10Reflection14PropDescriptorINS_10ImageLabelENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE11isWriteOnlyEv
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::ImageLabel,RBX::TextureId>::GetSetImpl<RBX::TextureId (RBX::GuiImageMixin::*)(void)const,void (RBX::ImageLabel::*)(RBX::TextureId)>::isWriteOnly(void)const")]
#[doc(alias = "__ZNK3RBX10Reflection14PropDescriptorINS_10ImageLabelENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE11isWriteOnlyEv")]
pub fn stub_0x57ff14(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// PropDescriptor SetImpl — stores the converted value.
cell.set(value);
}

// 0x57ff18 — __ZNK3RBX10Reflection14PropDescriptorINS_10ImageLabelENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE8getValueEPKNS0_13DescribedBaseE
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::ImageLabel,RBX::TextureId>::GetSetImpl<RBX::TextureId (RBX::GuiImageMixin::*)(void)const,void (RBX::ImageLabel::*)(RBX::TextureId)>::getValue(RBX::Reflection::DescribedBase const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection14PropDescriptorINS_10ImageLabelENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE8getValueEPKNS0_13DescribedBaseE")]
pub fn stub_0x57ff18(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// PropDescriptor SetImpl — stores the converted value.
cell.set(value);
}

// 0x57ff50 — __ZNK3RBX10Reflection14PropDescriptorINS_10ImageLabelENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE8setValueEPNS0_13DescribedBaseERKS3_
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::ImageLabel,RBX::TextureId>::GetSetImpl<RBX::TextureId (RBX::GuiImageMixin::*)(void)const,void (RBX::ImageLabel::*)(RBX::TextureId)>::setValue(RBX::Reflection::DescribedBase *,RBX::TextureId const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection14PropDescriptorINS_10ImageLabelENS_9TextureIdEE10GetSetImplIMNS_13GuiImageMixinEKFS3_vEMS2_FvS3_EE8setValueEPNS0_13DescribedBaseERKS3_")]
pub fn stub_0x57ff50(cell: &mut crate::slot::PropCell, value: crate::slot::PropValue) {
// PropDescriptor SetImpl — stores the converted value.
cell.set(value);
}

// 0x580750 — __ZN3RBX13InsertService13getFreeModelsESsiN5boost8functionIFvNS1_10shared_ptrIKSt6vectorINS_10Reflection7VariantESaIS6_EEEEEEENS2_IFvSsEEE
#[doc(alias = "RBX::InsertService::getFreeModels(std::string,int,function<void ()(rbx_core::SharedPtr<std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>> const>)>,function<void ()(std::string)>)")]
#[doc(alias = "__ZN3RBX13InsertService13getFreeModelsESsiN5boost8functionIFvNS1_10shared_ptrIKSt6vectorINS_10Reflection7VariantESaIS6_EEEEEEENS2_IFvSsEEE")]
pub fn stub_0x580750() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("std::vector<RBX::Reflection::Variant, std::allocator<RBX::Reflection::Variant>> ~")
}

// 0x5809a0 — __ZN3RBX13InsertService13getFreeDecalsESsiN5boost8functionIFvNS1_10shared_ptrIKSt6vectorINS_10Reflection7VariantESaIS6_EEEEEEENS2_IFvSsEEE
#[doc(alias = "RBX::InsertService::getFreeDecals(std::string,int,function<void ()(rbx_core::SharedPtr<std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>> const>)>,function<void ()(std::string)>)")]
#[doc(alias = "__ZN3RBX13InsertService13getFreeDecalsESsiN5boost8functionIFvNS1_10shared_ptrIKSt6vectorINS_10Reflection7VariantESaIS6_EEEEEEENS2_IFvSsEEE")]
pub fn stub_0x5809a0() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("std::vector<RBX::Reflection::Variant, std::allocator<RBX::Reflection::Variant>> ~")
}

// 0x580bf4 — __ZN3RBX13InsertService11getBaseSetsEN5boost8functionIFvNS1_10shared_ptrIKSt6vectorINS_10Reflection7VariantESaIS6_EEEEEEENS2_IFvSsEEE
#[doc(alias = "RBX::InsertService::getBaseSets(function<void ()(rbx_core::SharedPtr<std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>> const>)>,function<void ()(std::string)>)")]
#[doc(alias = "__ZN3RBX13InsertService11getBaseSetsEN5boost8functionIFvNS1_10shared_ptrIKSt6vectorINS_10Reflection7VariantESaIS6_EEEEEEENS2_IFvSsEEE")]
pub fn stub_0x580bf4() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("std::vector<RBX::Reflection::Variant, std::allocator<RBX::Reflection::Variant>> ~")
}

// 0x580db4 — __ZN3RBX13InsertService11getUserSetsEiN5boost8functionIFvNS1_10shared_ptrIKSt6vectorINS_10Reflection7VariantESaIS6_EEEEEEENS2_IFvSsEEE
#[doc(alias = "RBX::InsertService::getUserSets(int,function<void ()(rbx_core::SharedPtr<std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>> const>)>,function<void ()(std::string)>)")]
#[doc(alias = "__ZN3RBX13InsertService11getUserSetsEiN5boost8functionIFvNS1_10shared_ptrIKSt6vectorINS_10Reflection7VariantESaIS6_EEEEEEENS2_IFvSsEEE")]
pub fn stub_0x580db4() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("std::vector<RBX::Reflection::Variant, std::allocator<RBX::Reflection::Variant>> ~")
}

// 0x581004 — __ZN3RBX13InsertService13getCollectionEiN5boost8functionIFvNS1_10shared_ptrIKSt6vectorINS_10Reflection7VariantESaIS6_EEEEEEENS2_IFvSsEEE
#[doc(alias = "RBX::InsertService::getCollection(int,function<void ()(rbx_core::SharedPtr<std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>> const>)>,function<void ()(std::string)>)")]
#[doc(alias = "__ZN3RBX13InsertService13getCollectionEiN5boost8functionIFvNS1_10shared_ptrIKSt6vectorINS_10Reflection7VariantESaIS6_EEEEEEENS2_IFvSsEEE")]
pub fn stub_0x581004() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("std::vector<RBX::Reflection::Variant, std::allocator<RBX::Reflection::Variant>> ~")
}

// 0x581250 — __ZN3RBX13InsertService9loadAssetEiN5boost8functionIFvNS1_10shared_ptrINS_8InstanceEEEEEENS2_IFvSsEEE
#[doc(alias = "RBX::InsertService::loadAsset(int,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>,function<void ()(std::string)>)")]
#[doc(alias = "__ZN3RBX13InsertService9loadAssetEiN5boost8functionIFvNS1_10shared_ptrINS_8InstanceEEEEEENS2_IFvSsEEE")]
pub fn stub_0x581250() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("RBX::Instance")
}

// 0x58134c — __ZN3RBX13InsertService16loadAssetVersionEiN5boost8functionIFvNS1_10shared_ptrINS_8InstanceEEEEEENS2_IFvSsEEE
#[doc(alias = "RBX::InsertService::loadAssetVersion(int,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>,function<void ()(std::string)>)")]
#[doc(alias = "__ZN3RBX13InsertService16loadAssetVersionEiN5boost8functionIFvNS1_10shared_ptrINS_8InstanceEEEEEENS2_IFvSsEEE")]
pub fn stub_0x58134c() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("RBX::Instance")
}

// 0x581448 — __ZN3RBX13InsertService6insertEN5boost10shared_ptrINS_8InstanceEEE
#[doc(alias = "RBX::InsertService::insert(rbx_core::SharedPtr<RBX::Instance>)")]
#[doc(alias = "__ZN3RBX13InsertService6insertEN5boost10shared_ptrINS_8InstanceEEE")]
pub fn stub_0x581448() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("RBX::Instance")
}

// 0x581ac0 — __ZN3RBX13InsertService15dispatchRequestERKSsN5boost8functionIFvNS3_10shared_ptrIKSt6vectorINS_10Reflection7VariantESaIS8_EEEEEEENS4_IFvSsEEE
#[doc(alias = "RBX::InsertService::dispatchRequest(std::string const&,function<void ()(rbx_core::SharedPtr<std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>> const>)>,function<void ()(std::string)>)")]
#[doc(alias = "__ZN3RBX13InsertService15dispatchRequestERKSsN5boost8functionIFvNS3_10shared_ptrIKSt6vectorINS_10Reflection7VariantESaIS8_EEEEEEENS4_IFvSsEEE")]
pub fn stub_0x581ac0() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("std::vector<RBX::Reflection::Variant, std::allocator<RBX::Reflection::Variant>> ~")
}

// 0x581d68 — __ZN3RBX13InsertService17onServiceProviderEPNS_15ServiceProviderES2_
// type: _DWORD __fastcall(RBX::InsertService *__hidden this, RBX::ServiceProvider *, RBX::ServiceProvider *)
#[doc(alias = "RBX::InsertService::onServiceProvider(RBX::ServiceProvider *,RBX::ServiceProvider *)")]
#[doc(alias = "__ZN3RBX13InsertService17onServiceProviderEPNS_15ServiceProviderES2_")]
pub fn stub_0x581d68(handle: &crate::slot::InstanceHandle) {
// RBX::InsertService::onServiceProvider(RBX::ServiceProvider*, RBX::ServiceProvider*) — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x582ab8 — __ZN3RBX13InsertService18insertResultsReadyESsN5boost10shared_ptrINS_8InstanceEEE
// type: int __fastcall(int, int, int, int, int, struct _Unwind_Exception *lpuexcpt, int, boost::detail::sp_counted_base *, pthread_mutex_t *, char, char, int, int, int, int, int, int, int)
#[doc(alias = "RBX::InsertService::insertResultsReady(std::string,rbx_core::SharedPtr<RBX::Instance>)")]
#[doc(alias = "__ZN3RBX13InsertService18insertResultsReadyESsN5boost10shared_ptrINS_8InstanceEEE")]
pub fn stub_0x582ab8() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("RBX::Instance")
}

// 0x582e5c — __ZN3RBX13InsertService16privateLoadAssetEibN5boost8functionIFvNS1_10shared_ptrINS_8InstanceEEEEEENS2_IFvSsEEE
#[doc(alias = "RBX::InsertService::privateLoadAsset(int,bool,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>,function<void ()(std::string)>)")]
#[doc(alias = "__ZN3RBX13InsertService16privateLoadAssetEibN5boost8functionIFvNS1_10shared_ptrINS_8InstanceEEEEEENS2_IFvSsEEE")]
pub fn stub_0x582e5c() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("RBX::Instance")
}

// 0x583644 — __ZN3RBX13InsertService18backendInsertReadyESsN5boost10shared_ptrINS_8InstanceEEE
#[doc(alias = "RBX::InsertService::backendInsertReady(std::string,rbx_core::SharedPtr<RBX::Instance>)")]
#[doc(alias = "__ZN3RBX13InsertService18backendInsertReadyESsN5boost10shared_ptrINS_8InstanceEEE")]
pub fn stub_0x583644() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("RBX::Instance")
}

// 0x583850 — __ZN3RBX13InsertService24BackendInsertReadyHelperEN5boost8weak_ptrIS0_EESsNS1_10shared_ptrINS_8InstanceEEE
#[doc(alias = "RBX::InsertService::BackendInsertReadyHelper(weak_ptr<RBX::InsertService>,std::string,rbx_core::SharedPtr<RBX::Instance>)")]
#[doc(alias = "__ZN3RBX13InsertService24BackendInsertReadyHelperEN5boost8weak_ptrIS0_EESsNS1_10shared_ptrINS_8InstanceEEE")]
pub fn stub_0x583850() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("RBX::Instance")
}

// 0x5839f0 — __ZN3RBX13InsertService10safeInsertENS_9ContentIdEN5boost8functionIFvNS2_10shared_ptrINS_8InstanceEEEEEE
// type: int __fastcall(int, int, int)
#[doc(alias = "RBX::InsertService::safeInsert(RBX::ContentId,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>)")]
#[doc(alias = "__ZN3RBX13InsertService10safeInsertENS_9ContentIdEN5boost8functionIFvNS2_10shared_ptrINS_8InstanceEEEEEE")]
pub fn stub_0x5839f0() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("RBX::Instance")
}

// 0x583d54 — __ZN3RBX13InsertService29RemoteInsertItemsLoadedHelperEN5boost8weak_ptrIS0_EENS_14AsyncHttpQueue13RequestResultENS1_10shared_ptrISt6vectorINS6_INS_8InstanceEEESaIS9_EEEENS1_8functionIFvS9_EEE
// type: int __fastcall(int, int, int, int, int, int, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, int, int, int)
#[doc(alias = "RBX::InsertService::RemoteInsertItemsLoadedHelper(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>)")]
#[doc(alias = "__ZN3RBX13InsertService29RemoteInsertItemsLoadedHelperEN5boost8weak_ptrIS0_EENS_14AsyncHttpQueue13RequestResultENS1_10shared_ptrISt6vectorINS6_INS_8InstanceEEESaIS9_EEEENS1_8functionIFvS9_EEE")]
pub fn stub_0x583d54() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("std::vector<boost::shared_ptr<RBX::Instance>, std::allocator<boost::shared_ptr<R~")
}

// 0x583e90 — __ZN3RBX13InsertService23remoteInsertItemsLoadedENS_14AsyncHttpQueue13RequestResultEN5boost10shared_ptrISt6vectorINS4_INS_8InstanceEEESaIS7_EEEENS3_8functionIFvS7_EEE
// type: int __fastcall(int, int, int, int, int, int, int, int, int, struct _Unwind_Exception *lpuexcpt, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, char, int, int, int, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, int, int, int)
#[doc(alias = "RBX::InsertService::remoteInsertItemsLoaded(RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>)")]
#[doc(alias = "__ZN3RBX13InsertService23remoteInsertItemsLoadedENS_14AsyncHttpQueue13RequestResultEN5boost10shared_ptrISt6vectorINS4_INS_8InstanceEEESaIS7_EEEENS3_8functionIFvS7_EEE")]
pub fn stub_0x583e90() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("std::vector<boost::shared_ptr<RBX::Instance>, std::allocator<boost::shared_ptr<R~")
}

// 0x584e40 — __ZN3RBX10ReflectionL14resume_adapterIN5boost10shared_ptrINS_8InstanceEEEEEvNS2_8functionIFvNS0_7VariantEEEET_
#[doc(alias = "void RBX::Reflection::resume_adapter<rbx_core::SharedPtr<RBX::Instance>>(function<void ()(RBX::Reflection::Variant)>,rbx_core::SharedPtr<RBX::Instance>)")]
#[doc(alias = "__ZN3RBX10ReflectionL14resume_adapterIN5boost10shared_ptrINS_8InstanceEEEEEvNS2_8functionIFvNS0_7VariantEEEET_")]
pub fn stub_0x584e40() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("RBX::Instance")
}

// 0x584fa4 — __ZN3RBX10ReflectionL14resume_adapterIN5boost10shared_ptrIKSt6vectorINS0_7VariantESaIS5_EEEEEEvNS2_8functionIFvS5_EEET_
#[doc(alias = "void RBX::Reflection::resume_adapter<rbx_core::SharedPtr<std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>> const>>(function<void ()(RBX::Reflection::Variant)>,rbx_core::SharedPtr<std::vector<RBX::Reflection::Variant,std::allocator<RBX::Reflection::Variant>> const>)")]
#[doc(alias = "__ZN3RBX10ReflectionL14resume_adapterIN5boost10shared_ptrIKSt6vectorINS0_7VariantESaIS5_EEEEEEvNS2_8functionIFvS5_EEET_")]
pub fn stub_0x584fa4() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("std::vector<RBX::Reflection::Variant, std::allocator<RBX::Reflection::Variant>> ~")
}

// 0x585a10 — __ZN3RBXL18CallResultFunctionEN5boost8functionIFvNS0_10shared_ptrINS_8InstanceEEEEEES4_
#[doc(alias = "RBX::CallResultFunction(function<void ()(rbx_core::SharedPtr<RBX::Instance>)>,rbx_core::SharedPtr<RBX::Instance>)")]
#[doc(alias = "__ZN3RBXL18CallResultFunctionEN5boost8functionIFvNS0_10shared_ptrINS_8InstanceEEEEEES4_")]
pub fn stub_0x585a10() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("RBX::Instance")
}

// 0x585adc — __ZN3RBX10Reflection15RemoteEventDescINS_13InsertServiceEFvSsNS_9ContentIdEEN3rbx13remote_signalIS4_EEED1Ev
#[doc(alias = "RBX::Reflection::RemoteEventDesc<RBX::InsertService,void ()(std::string,RBX::ContentId),rbx::remote_signal<void ()(std::string,RBX::ContentId)>>::~RemoteEventDesc()")]
#[doc(alias = "__ZN3RBX10Reflection15RemoteEventDescINS_13InsertServiceEFvSsNS_9ContentIdEEN3rbx13remote_signalIS4_EEED1Ev")]
pub fn stub_0x585adc(handle: crate::slot::DescriptorHandle) {
// descriptor dtor — unregisters and releases.
drop(handle);
}

// 0x585b00 — __ZN3RBX10Reflection15RemoteEventDescINS_13InsertServiceEFvSsiiEN3rbx13remote_signalIS3_EEED1Ev
#[doc(alias = "RBX::Reflection::RemoteEventDesc<RBX::InsertService,void ()(std::string,int,int),rbx::remote_signal<void ()(std::string,int,int)>>::~RemoteEventDesc()")]
#[doc(alias = "__ZN3RBX10Reflection15RemoteEventDescINS_13InsertServiceEFvSsiiEN3rbx13remote_signalIS3_EEED1Ev")]
pub fn stub_0x585b00(handle: crate::slot::DescriptorHandle) {
// descriptor dtor — unregisters and releases.
drop(handle);
}

// 0x585b24 — __ZN3RBX10Reflection15RemoteEventDescINS_13InsertServiceEFvSsN5boost10shared_ptrINS_8InstanceEEEEN3rbx13remote_signalIS7_EEED1Ev
#[doc(alias = "RBX::Reflection::RemoteEventDesc<RBX::InsertService,void ()(std::string,rbx_core::SharedPtr<RBX::Instance>),rbx::remote_signal<void ()(std::string,rbx_core::SharedPtr<RBX::Instance>)>>::~RemoteEventDesc()")]
#[doc(alias = "__ZN3RBX10Reflection15RemoteEventDescINS_13InsertServiceEFvSsN5boost10shared_ptrINS_8InstanceEEEEN3rbx13remote_signalIS7_EEED1Ev")]
pub fn stub_0x585b24(handle: crate::slot::InstanceHandle) {
// shared/intrusive_ptr dtor — release; last one frees.
drop(handle);
}

// 0x585b48 — __ZN3RBX10Reflection15RemoteEventDescINS_13InsertServiceEFvSsSsEN3rbx13remote_signalIS3_EEED1Ev
#[doc(alias = "RBX::Reflection::RemoteEventDesc<RBX::InsertService,void ()(std::string,std::string),rbx::remote_signal<void ()(std::string,std::string)>>::~RemoteEventDesc()")]
#[doc(alias = "__ZN3RBX10Reflection15RemoteEventDescINS_13InsertServiceEFvSsSsEN3rbx13remote_signalIS3_EEED1Ev")]
pub fn stub_0x585b48(handle: crate::slot::DescriptorHandle) {
// descriptor dtor — unregisters and releases.
drop(handle);
}

// 0x585b6c — __ZN3RBX10Reflection13BoundFuncDescINS_13InsertServiceEFvSsELi1EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::InsertService,void ()(std::string),1>::~BoundFuncDesc()")]
#[doc(alias = "__ZN3RBX10Reflection13BoundFuncDescINS_13InsertServiceEFvSsELi1EED1Ev")]
pub fn stub_0x585b6c(handle: crate::slot::DescriptorHandle) {
// descriptor dtor — unregisters and releases.
drop(handle);
}

// 0x585bac — __ZN3RBX10Reflection13BoundFuncDescINS_13InsertServiceEFvfELi1EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::InsertService,void ()(float),1>::~BoundFuncDesc()")]
#[doc(alias = "__ZN3RBX10Reflection13BoundFuncDescINS_13InsertServiceEFvfELi1EED1Ev")]
pub fn stub_0x585bac(handle: crate::slot::DescriptorHandle) {
// descriptor dtor — unregisters and releases.
drop(handle);
}

// 0x585bec — __ZN3RBX10Reflection13BoundFuncDescINS_13InsertServiceEFviELi1EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::InsertService,void ()(int),1>::~BoundFuncDesc()")]
#[doc(alias = "__ZN3RBX10Reflection13BoundFuncDescINS_13InsertServiceEFviELi1EED1Ev")]
pub fn stub_0x585bec(handle: crate::slot::DescriptorHandle) {
// descriptor dtor — unregisters and releases.
drop(handle);
}

// 0x585cd8 — __ZN3RBX10Reflection13BoundFuncDescINS_13InsertServiceEFvbbELi2EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::InsertService,void ()(bool,bool),2>::~BoundFuncDesc()")]
#[doc(alias = "__ZN3RBX10Reflection13BoundFuncDescINS_13InsertServiceEFvbbELi2EED1Ev")]
pub fn stub_0x585cd8(handle: crate::slot::DescriptorHandle) {
// descriptor dtor — unregisters and releases.
drop(handle);
}

// 0x585d60 — __ZN3RBX10Reflection13BoundFuncDescINS_13InsertServiceEFvN5boost10shared_ptrINS_8InstanceEEEELi1EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::InsertService,void ()(rbx_core::SharedPtr<RBX::Instance>),1>::~BoundFuncDesc()")]
#[doc(alias = "__ZN3RBX10Reflection13BoundFuncDescINS_13InsertServiceEFvN5boost10shared_ptrINS_8InstanceEEEELi1EED1Ev")]
pub fn stub_0x585d60(handle: crate::slot::InstanceHandle) {
// shared/intrusive_ptr dtor — release; last one frees.
drop(handle);
}

// 0x585e6c — __ZN5boost8functionIFvNS_10shared_ptrIN3RBX8InstanceEEEEEaSERKS6_
#[doc(alias = "function<void ()(rbx_core::SharedPtr<RBX::Instance>)>::operator=(function<void ()(rbx_core::SharedPtr<RBX::Instance>)> const&)")]
#[doc(alias = "__ZN5boost8functionIFvNS_10shared_ptrIN3RBX8InstanceEEEEEaSERKS6_")]
pub fn stub_0x585e6c(handle: &crate::slot::InstanceHandle) -> crate::slot::InstanceHandle {
// shared/intrusive_ptr operator= — atomic retain + alias;
// the host copy carries the same identity.
*handle
}

// 0x5861fc — __ZN3RBX10Reflection19RemoteEventDescImplILi3ENS_13InsertServiceEFvSsiiEN3rbx13remote_signalIS3_EEE14replicateEventEPNS0_11EventSourceESsii
#[doc(alias = "RBX::Reflection::RemoteEventDescImpl<3,RBX::InsertService,void ()(std::string,int,int),rbx::remote_signal<void ()(std::string,int,int)>>::replicateEvent(RBX::Reflection::EventSource *,std::string,int,int)")]
#[doc(alias = "__ZN3RBX10Reflection19RemoteEventDescImplILi3ENS_13InsertServiceEFvSsiiEN3rbx13remote_signalIS3_EEE14replicateEventEPNS0_11EventSourceESsii")]
pub fn stub_0x5861fc(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::RemoteEventDescImpl<3, RBX::InsertService, void (std::string, int, int), ~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x586390 — __ZNK3RBX10Reflection13EventDescImplILi3ENS_13InsertServiceEFvSsiiEN3rbx13remote_signalIS3_EEMS2_S6_E9fireEventEPS2_Ssii
// type: int __fastcall(int, int, std::string *, int, int)
#[doc(alias = "RBX::Reflection::EventDescImpl<3,RBX::InsertService,void ()(std::string,int,int),rbx::remote_signal<void ()(std::string,int,int)>,rbx::remote_signal<void ()(std::string,int,int)> RBX::InsertService::*>::fireEvent(RBX::InsertService*,std::string,int,int)const")]
#[doc(alias = "__ZNK3RBX10Reflection13EventDescImplILi3ENS_13InsertServiceEFvSsiiEN3rbx13remote_signalIS3_EEMS2_S6_E9fireEventEPS2_Ssii")]
pub fn stub_0x586390(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EventDescImpl<3, RBX::InsertService, void (std::string, int, int), rbx::r~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x5864b4 — __ZN5boost10shared_ptrIN3RBX8InstanceEEaSINS1_13ModelInstanceEEERS3_RKNS0_IT_EE
// type: int(void)
#[doc(alias = "rbx_core::SharedPtr<RBX::Instance>& rbx_core::SharedPtr<RBX::Instance>::operator=<RBX::ModelInstance>(rbx_core::SharedPtr<RBX::ModelInstance> const&)")]
#[doc(alias = "__ZN5boost10shared_ptrIN3RBX8InstanceEEaSINS1_13ModelInstanceEEERS3_RKNS0_IT_EE")]
pub fn stub_0x5864b4(handle: &crate::slot::InstanceHandle) -> crate::slot::InstanceHandle {
// shared/intrusive_ptr operator= — atomic retain + alias;
// the host copy carries the same identity.
*handle
}

// 0x5864e8 — __ZN3RBX10Reflection19RemoteEventDescImplILi2ENS_13InsertServiceEFvSsN5boost10shared_ptrINS_8InstanceEEEEN3rbx13remote_signalIS7_EEE21fireAndReplicateEventEPS2_SsS6_
// type: int __fastcall(int, int, std::string *)
#[doc(alias = "RBX::Reflection::RemoteEventDescImpl<2,RBX::InsertService,void ()(std::string,rbx_core::SharedPtr<RBX::Instance>),rbx::remote_signal<void ()(std::string,rbx_core::SharedPtr<RBX::Instance>)>>::fireAndReplicateEvent(RBX::InsertService*,std::string,rbx_core::SharedPtr<RBX::Instance>)")]
#[doc(alias = "__ZN3RBX10Reflection19RemoteEventDescImplILi2ENS_13InsertServiceEFvSsN5boost10shared_ptrINS_8InstanceEEEEN3rbx13remote_signalIS7_EEE21fireAndReplicateEventEPS2_SsS6_")]
pub fn stub_0x5864e8() -> crate::slot::InstanceHandle {
// shared/intrusive_ptr ctor — fresh owning ref.
crate::slot::InstanceHandle::new("RBX::Instance")
}

// 0x58673c — __ZN5boost4bindIvNS_8weak_ptrIN3RBX13InsertServiceEEESsNS_10shared_ptrINS2_8InstanceEEES4_SsNS_3argILi1EEEEENS_3_bi6bind_tIT_PFSC_T0_T1_T2_ENSA_9list_av_3IT3_T4_T5_E4typeEEESH_SJ_SK_SL_
// type: int(void)
#[doc(alias = "_bi::bind_t<void,void (*)(weak_ptr<RBX::InsertService>,std::string,rbx_core::SharedPtr<RBX::Instance>),_bi::list_av_3<weak_ptr<RBX::InsertService>,std::string,arg<1>>::type> bind<void,weak_ptr<RBX::InsertService>,std::string,rbx_core::SharedPtr<RBX::Instance>,weak_ptr<RBX::InsertService>,std::string,arg<1>>(void (*)(weak_ptr<RBX::InsertService>,std::string,rbx_core::SharedPtr<RBX::Instance>),weak_ptr<RBX::InsertService>,std::string,arg<1>)")]
#[doc(alias = "__ZN5boost4bindIvNS_8weak_ptrIN3RBX13InsertServiceEEESsNS_10shared_ptrINS2_8InstanceEEES4_SsNS_3argILi1EEEEENS_3_bi6bind_tIT_PFSC_T0_T1_T2_ENSA_9list_av_3IT3_T4_T5_E4typeEEESH_SJ_SK_SL_")]
pub fn stub_0x58673c() -> crate::slot::BoundCall {
// boost::bind (cf. 0x39d700 args) — captures receiver +
// member functor + 3 placeholders as a host closure.
// was: boost::_bi::bind_t/_mfi.
crate::slot::BoundCall::new(3)
}

// 0x586b90 — __ZN3RBX10Reflection19RemoteEventDescImplILi2ENS_13InsertServiceEFvSsSsEN3rbx13remote_signalIS3_EEE21fireAndReplicateEventEPS2_SsSs
// type: int __fastcall(int, int, std::string *)
#[doc(alias = "RBX::Reflection::RemoteEventDescImpl<2,RBX::InsertService,void ()(std::string,std::string),rbx::remote_signal<void ()(std::string,std::string)>>::fireAndReplicateEvent(RBX::InsertService*,std::string,std::string)")]
#[doc(alias = "__ZN3RBX10Reflection19RemoteEventDescImplILi2ENS_13InsertServiceEFvSsSsEN3rbx13remote_signalIS3_EEE21fireAndReplicateEventEPS2_SsSs")]
pub fn stub_0x586b90(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::RemoteEventDescImpl<2, RBX::InsertService, void (std::string, std::string~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x586fc8 — __ZN5boost4bindIvNS_8weak_ptrIN3RBX13InsertServiceEEENS2_14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINS7_INS2_8InstanceEEESaISA_EEEENS_8functionIFvSA_EEES4_NS_3argILi1EEENSH_ILi2EEESG_EENS_3_bi6bind_tIT_PFSM_T0_T1_T2_T3_ENSK_9list_av_4IT4_T5_T6_T7_E4typeEEESS_SU_SV_SW_SX_
// type: int __fastcall(int, int, int, int, char, int, int, int, int, boost::detail::sp_counted_base *, char, boost::detail::sp_counted_base *, int, int, int, int, int, int, int, int, int, int)
#[doc(alias = "_bi::bind_t<void,void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>),_bi::list_av_4<weak_ptr<RBX::InsertService>,arg<1>,arg<2>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>>::type> bind<void,weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>,weak_ptr<RBX::InsertService>,arg<1>,arg<2>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>>(void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>),weak_ptr<RBX::InsertService>,arg<1>,arg<2>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>)")]
#[doc(alias = "__ZN5boost4bindIvNS_8weak_ptrIN3RBX13InsertServiceEEENS2_14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINS7_INS2_8InstanceEEESaISA_EEEENS_8functionIFvSA_EEES4_NS_3argILi1EEENSH_ILi2EEESG_EENS_3_bi6bind_tIT_PFSM_T0_T1_T2_T3_ENSK_9list_av_4IT4_T5_T6_T7_E4typeEEESS_SU_SV_SW_SX_")]
pub fn stub_0x586fc8() -> crate::slot::BoundCall {
// boost::bind (cf. 0x39d700 args) — captures receiver +
// member functor + 6 placeholders as a host closure.
// was: boost::_bi::bind_t/_mfi.
crate::slot::BoundCall::new(6)
}

// 0x587678 — __ZN5boost9function2IvN3RBX14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINS4_INS1_8InstanceEEESaIS7_EEEEE9assign_toINS_3_bi6bind_tIvPFvNS_8weak_ptrINS1_13InsertServiceEEES3_SA_NS_8functionIFvS7_EEEENSD_5list4INSD_5valueISH_EENS_3argILi1EEENSQ_ILi2EEENSO_ISK_EEEEEEEEvT_
// type: int __fastcall(int, int, int, int, int, boost::detail::sp_counted_base *, int, int, int, int, int, int, int, int)
#[doc(alias = "void function2<void,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>::assign_to<_bi::bind_t<void,void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>),_bi::list4<_bi::value<weak_ptr<RBX::InsertService>>,arg<1>,arg<2>,_bi::value<function<void ()(rbx_core::SharedPtr<RBX::Instance>)>>>>>(_bi::bind_t<void,void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>),_bi::list4<_bi::value<weak_ptr<RBX::InsertService>>,arg<1>,arg<2>,_bi::value<function<void ()(rbx_core::SharedPtr<RBX::Instance>)>>>>)")]
#[doc(alias = "__ZN5boost9function2IvN3RBX14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINS4_INS1_8InstanceEEESaIS7_EEEEE9assign_toINS_3_bi6bind_tIvPFvNS_8weak_ptrINS1_13InsertServiceEEES3_SA_NS_8functionIFvS7_EEEENSD_5list4INSD_5valueISH_EENS_3argILi1EEENSQ_ILi2EEENSO_ISK_EEEEEEEEvT_")]
pub fn stub_0x587678(slot: &mut crate::slot::FnSlot) -> bool {
// boost::function basic_vtable assign — always stores into
// the functor slot. was: boost::function<R(ARGS)>.
slot.assign()
}

// 0x5877e8 — __ZN5boost6detail8function15functor_managerINS_3_bi6bind_tIvPFvNS_8weak_ptrIN3RBX13InsertServiceEEENS6_14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINSB_INS6_8InstanceEEESaISE_EEEENS_8functionIFvSE_EEEENS3_5list4INS3_5valueIS8_EENS_3argILi1EEENSQ_ILi2EEENSO_ISK_EEEEEEE6manageERKNS1_15function_bufferERSX_NS1_30functor_manager_operation_typeE
// type: _UNKNOWN **__fastcall(int, int, int, int, int, void *, int, int, int, int)
#[doc(alias = "detail::function::functor_manager<_bi::bind_t<void,void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>),_bi::list4<_bi::value<weak_ptr<RBX::InsertService>>,arg<1>,arg<2>,_bi::value<function<void ()(rbx_core::SharedPtr<RBX::Instance>)>>>>>::manage(detail::function::function_buffer const&,detail::function::function_buffer&,detail::function::functor_manager_operation_type)")]
#[doc(alias = "__ZN5boost6detail8function15functor_managerINS_3_bi6bind_tIvPFvNS_8weak_ptrIN3RBX13InsertServiceEEENS6_14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINSB_INS6_8InstanceEEESaISE_EEEENS_8functionIFvSE_EEEENS3_5list4INS3_5valueIS8_EENS_3argILi1EEENSQ_ILi2EEENSO_ISK_EEEEEEE6manageERKNS1_15function_bufferERSX_NS1_30functor_manager_operation_typeE")]
pub fn stub_0x5877e8(slot: &crate::slot::FnSlot) -> crate::slot::FnSlot {
// boost::function functor_manager::manage — clone/destroy op
// over the type-erased buffer; the host clones the slot.
slot.clone_op()
}

// 0x587804 — __ZN5boost6detail8function26void_function_obj_invoker2INS_3_bi6bind_tIvPFvNS_8weak_ptrIN3RBX13InsertServiceEEENS6_14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINSB_INS6_8InstanceEEESaISE_EEEENS_8functionIFvSE_EEEENS3_5list4INS3_5valueIS8_EENS_3argILi1EEENSQ_ILi2EEENSO_ISK_EEEEEEvSA_SH_E6invokeERNS1_15function_bufferESA_SH_
// type: int __fastcall(int, int, char)
#[doc(alias = "detail::function::void_function_obj_invoker2<_bi::bind_t<void,void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>),_bi::list4<_bi::value<weak_ptr<RBX::InsertService>>,arg<1>,arg<2>,_bi::value<function<void ()(rbx_core::SharedPtr<RBX::Instance>)>>>>,void,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>::invoke(detail::function::function_buffer &,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>)")]
#[doc(alias = "__ZN5boost6detail8function26void_function_obj_invoker2INS_3_bi6bind_tIvPFvNS_8weak_ptrIN3RBX13InsertServiceEEENS6_14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINSB_INS6_8InstanceEEESaISE_EEEENS_8functionIFvSE_EEEENS3_5list4INS3_5valueIS8_EENS_3argILi1EEENSQ_ILi2EEENSO_ISK_EEEEEEvSA_SH_E6invokeERNS1_15function_bufferESA_SH_")]
pub fn stub_0x587804(slot: &crate::slot::FnSlot) {
// boost::function void_function_obj_invoker — dispatches the
// stored functor, no-op when empty.
slot.invoke();
}

// 0x587820 — __ZNK5boost6detail8function13basic_vtable2IvN3RBX14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINS6_INS3_8InstanceEEESaIS9_EEEEE9assign_toINS_3_bi6bind_tIvPFvNS_8weak_ptrINS3_13InsertServiceEEES5_SC_NS_8functionIFvS9_EEEENSF_5list4INSF_5valueISJ_EENS_3argILi1EEENSS_ILi2EEENSQ_ISM_EEEEEEEEbT_RNS1_15function_bufferE
// type: int __fastcall(boost::detail::sp_counted_base *, int, int, int, int, boost::detail::sp_counted_base *, int, int, int, int, int, int, int, int)
#[doc(alias = "bool detail::function::basic_vtable2<void,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>::assign_to<_bi::bind_t<void,void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>),_bi::list4<_bi::value<weak_ptr<RBX::InsertService>>,arg<1>,arg<2>,_bi::value<function<void ()(rbx_core::SharedPtr<RBX::Instance>)>>>>>(_bi::bind_t<void,void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>),_bi::list4<_bi::value<weak_ptr<RBX::InsertService>>,arg<1>,arg<2>,_bi::value<function<void ()(rbx_core::SharedPtr<RBX::Instance>)>>>>,detail::function::function_buffer &)const")]
#[doc(alias = "__ZNK5boost6detail8function13basic_vtable2IvN3RBX14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINS6_INS3_8InstanceEEESaIS9_EEEEE9assign_toINS_3_bi6bind_tIvPFvNS_8weak_ptrINS3_13InsertServiceEEES5_SC_NS_8functionIFvS9_EEEENSF_5list4INSF_5valueISJ_EENS_3argILi1EEENSS_ILi2EEENSQ_ISM_EEEEEEEEbT_RNS1_15function_bufferE")]
pub fn stub_0x587820(slot: &mut crate::slot::FnSlot) -> bool {
// boost::function basic_vtable assign — always stores into
// the functor slot. was: boost::function<R(ARGS)>.
slot.assign()
}

// 0x587980 — __ZNK5boost6detail8function13basic_vtable2IvN3RBX14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINS6_INS3_8InstanceEEESaIS9_EEEEE9assign_toINS_3_bi6bind_tIvPFvNS_8weak_ptrINS3_13InsertServiceEEES5_SC_NS_8functionIFvS9_EEEENSF_5list4INSF_5valueISJ_EENS_3argILi1EEENSS_ILi2EEENSQ_ISM_EEEEEEEEbT_RNS1_15function_bufferENS1_16function_obj_tagE
// type: int __fastcall(int, int, int, int, int, boost::detail::sp_counted_base *, int, int, int, int, int, int, int, int)
#[doc(alias = "bool detail::function::basic_vtable2<void,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>::assign_to<_bi::bind_t<void,void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>),_bi::list4<_bi::value<weak_ptr<RBX::InsertService>>,arg<1>,arg<2>,_bi::value<function<void ()(rbx_core::SharedPtr<RBX::Instance>)>>>>>(_bi::bind_t<void,void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>),_bi::list4<_bi::value<weak_ptr<RBX::InsertService>>,arg<1>,arg<2>,_bi::value<function<void ()(rbx_core::SharedPtr<RBX::Instance>)>>>>,detail::function::function_buffer &,detail::function::function_obj_tag)const")]
#[doc(alias = "__ZNK5boost6detail8function13basic_vtable2IvN3RBX14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINS6_INS3_8InstanceEEESaIS9_EEEEE9assign_toINS_3_bi6bind_tIvPFvNS_8weak_ptrINS3_13InsertServiceEEES5_SC_NS_8functionIFvS9_EEEENSF_5list4INSF_5valueISJ_EENS_3argILi1EEENSS_ILi2EEENSQ_ISM_EEEEEEEEbT_RNS1_15function_bufferENS1_16function_obj_tagE")]
pub fn stub_0x587980(slot: &mut crate::slot::FnSlot) -> bool {
// boost::function basic_vtable assign — always stores into
// the functor slot. was: boost::function<R(ARGS)>.
slot.assign()
}

// 0x587adc — __ZNK5boost6detail8function13basic_vtable2IvN3RBX14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINS6_INS3_8InstanceEEESaIS9_EEEEE14assign_functorINS_3_bi6bind_tIvPFvNS_8weak_ptrINS3_13InsertServiceEEES5_SC_NS_8functionIFvS9_EEEENSF_5list4INSF_5valueISJ_EENS_3argILi1EEENSS_ILi2EEENSQ_ISM_EEEEEEEEvT_RNS1_15function_bufferEN4mpl_5bool_ILb0EEE
// type: int __fastcall(int, int, int, int, void *, int, int, int, int, int)
#[doc(alias = "void detail::function::basic_vtable2<void,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>>::assign_functor<_bi::bind_t<void,void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>),_bi::list4<_bi::value<weak_ptr<RBX::InsertService>>,arg<1>,arg<2>,_bi::value<function<void ()(rbx_core::SharedPtr<RBX::Instance>)>>>>>(_bi::bind_t<void,void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>),_bi::list4<_bi::value<weak_ptr<RBX::InsertService>>,arg<1>,arg<2>,_bi::value<function<void ()(rbx_core::SharedPtr<RBX::Instance>)>>>>,detail::function::function_buffer &,mpl_::bool_<false>)const")]
#[doc(alias = "__ZNK5boost6detail8function13basic_vtable2IvN3RBX14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINS6_INS3_8InstanceEEESaIS9_EEEEE14assign_functorINS_3_bi6bind_tIvPFvNS_8weak_ptrINS3_13InsertServiceEEES5_SC_NS_8functionIFvS9_EEEENSF_5list4INSF_5valueISJ_EENS_3argILi1EEENSS_ILi2EEENSQ_ISM_EEEEEEEEvT_RNS1_15function_bufferEN4mpl_5bool_ILb0EEE")]
pub fn stub_0x587adc(slot: &mut crate::slot::FnSlot) -> bool {
// boost::function basic_vtable assign — always stores into
// the functor slot. was: boost::function<R(ARGS)>.
slot.assign()
}

// 0x587be8 — __ZN5boost3_bi5list4INS0_5valueINS_8weak_ptrIN3RBX13InsertServiceEEEEENS_3argILi1EEENS8_ILi2EEENS2_INS_8functionIFvNS_10shared_ptrINS4_8InstanceEEEEEEEEEclIPFvS6_NS4_14AsyncHttpQueue13RequestResultENSC_ISt6vectorISE_SaISE_EEEESG_ENS0_5list2IRSL_RSP_EEEEvNS0_4typeIvEERT_RT0_i
// type: int __fastcall(int, int, int, int, int, struct _Unwind_Exception *lpuexcpt, char, int, int, int, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, int, int, int)
#[doc(alias = "void _bi::list4<_bi::value<weak_ptr<RBX::InsertService>>,arg<1>,arg<2>,_bi::value<function<void ()(rbx_core::SharedPtr<RBX::Instance>)>>>::operator()<void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>),_bi::list2<RBX::AsyncHttpQueue::RequestResult&,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>&>>(_bi::type<void>,void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>) &,_bi::list2<RBX::AsyncHttpQueue::RequestResult&,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>&> &,int)")]
#[doc(alias = "__ZN5boost3_bi5list4INS0_5valueINS_8weak_ptrIN3RBX13InsertServiceEEEEENS_3argILi1EEENS8_ILi2EEENS2_INS_8functionIFvNS_10shared_ptrINS4_8InstanceEEEEEEEEEclIPFvS6_NS4_14AsyncHttpQueue13RequestResultENSC_ISt6vectorISE_SaISE_EEEESG_ENS0_5list2IRSL_RSP_EEEEvNS0_4typeIvEERT_RT0_i")]
pub fn stub_0x587be8(slot: &crate::slot::FnSlot) {
// boost::function::operator() — dispatches the stored functor.
slot.invoke();
}

// 0x587d3c — __ZN5boost6detail8function15functor_managerINS_3_bi6bind_tIvPFvNS_8weak_ptrIN3RBX13InsertServiceEEENS6_14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINSB_INS6_8InstanceEEESaISE_EEEENS_8functionIFvSE_EEEENS3_5list4INS3_5valueIS8_EENS_3argILi1EEENSQ_ILi2EEENSO_ISK_EEEEEEE7managerERKNS1_15function_bufferERSX_NS1_30functor_manager_operation_typeEN4mpl_5bool_ILb0EEE
// type: int __fastcall(int, int, int, int, int, void *, int, int, int, int)
#[doc(alias = "detail::function::functor_manager<_bi::bind_t<void,void (*)(weak_ptr<RBX::InsertService>,RBX::AsyncHttpQueue::RequestResult,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>>>,function<void ()(rbx_core::SharedPtr<RBX::Instance>)>),_bi::list4<_bi::value<weak_ptr<RBX::InsertService>>,arg<1>,arg<2>,_bi::value<function<void ()(rbx_core::SharedPtr<RBX::Instance>)>>>>>::manager(detail::function::function_buffer const&,detail::function::function_buffer&,detail::function::functor_manager_operation_type,mpl_::bool_<false>)")]
#[doc(alias = "__ZN5boost6detail8function15functor_managerINS_3_bi6bind_tIvPFvNS_8weak_ptrIN3RBX13InsertServiceEEENS6_14AsyncHttpQueue13RequestResultENS_10shared_ptrISt6vectorINSB_INS6_8InstanceEEESaISE_EEEENS_8functionIFvSE_EEEENS3_5list4INS3_5valueIS8_EENS_3argILi1EEENSQ_ILi2EEENSO_ISK_EEEEEEE7managerERKNS1_15function_bufferERSX_NS1_30functor_manager_operation_typeEN4mpl_5bool_ILb0EEE")]
pub fn stub_0x587d3c(slot: &crate::slot::FnSlot) -> crate::slot::FnSlot {
// boost::function functor_manager::manage — clone/destroy op
// over the type-erased buffer; the host clones the slot.
slot.clone_op()
}

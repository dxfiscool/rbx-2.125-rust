// Auto-generated skeletons for rbx-script — Script/Lua/Yield/CodeGen/Luau gap filler
// Filter: Script|Lua|Yield|CodeGen|Luau (4818 filtered, all already stubbed in crates/script/src) — EA-sorted asc gap filler distinct not yet in crates/script/src
// Source: ida/export.json (85545 funcs, base 0x4000)
// Batch: +100 stubs | range 0x4c7800..0x4caf94 | EA-sorted asc distinct not yet in script (remaining 56850->56750, rbx_core::SharedPtr not boost)
// SharedPtr = rbx_core::SharedPtr (Arc), not boost::shared_ptr;  and  stripped from alias

#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]

use rbx_core::SharedPtr;

const _: () = {
    let _ = core::marker::PhantomData::<SharedPtr<u8>>;
};

// 0x4c7800 — __ZNK3RBX10Reflection8EnumDescINS_21PersonalServerService13PrivilegeTypeEE15convertToStringERKS3_ — RBX::Reflection::EnumDesc<RBX::PersonalServerService::PrivilegeType>::convertToString(RBX::PersonalServerService::PrivilegeType const&)const
// type: void __fastcall(std::string *, int, int *, int, struct _Unwind_Exception *lpuexcpt, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::PersonalServerService::PrivilegeType>::convertToString(RBX::PersonalServerService::PrivilegeType const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_21PersonalServerService13PrivilegeTypeEE15convertToStringERKS3_")]
pub fn stub_0x4c7800(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::PersonalServerService::PrivilegeType>::convertToString(RBX:~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c79a0 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_21PersonalServerService13PrivilegeTypeEEERS3_RKT_ — rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::PersonalServerService::PrivilegeType>(RBX::PersonalServerService::PrivilegeType const&)
// type: void (__fastcall ***__fastcall(void (__fastcall ***)(int), void (__fastcall ***)(int)))(int)
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::PersonalServerService::PrivilegeType>(RBX::PersonalServerService::PrivilegeType const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_21PersonalServerService13PrivilegeTypeEEERS3_RKT_")]
pub fn stub_0x4c79a0() -> crate::lua::ScriptVariant {
// placement_any ctor — empty (Void).
crate::lua::ScriptVariant::Void
}

// 0x4c79f0 — __ZN3rbx14implementation12typed_holderIN3RBX21PersonalServerService13PrivilegeTypeEE9singletonEv — rbx::implementation::typed_holder<RBX::PersonalServerService::PrivilegeType>::singleton(void)
// type: _DWORD *()
#[doc(alias = "rbx::implementation::typed_holder<RBX::PersonalServerService::PrivilegeType>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX21PersonalServerService13PrivilegeTypeEE9singletonEv")]
pub fn stub_0x4c79f0(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::PersonalServerService::PrivilegeType>::singleton() — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c7a5c — __ZN3rbx14implementation12typed_holderIN3RBX21PersonalServerService13PrivilegeTypeEE14construct_funcEPKcPc — rbx::implementation::typed_holder<RBX::PersonalServerService::PrivilegeType>::construct_func(char const*,char *)
// type: _DWORD *__fastcall(_DWORD *result, _DWORD *)
#[doc(alias = "rbx::implementation::typed_holder<RBX::PersonalServerService::PrivilegeType>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX21PersonalServerService13PrivilegeTypeEE14construct_funcEPKcPc")]
pub fn stub_0x4c7a5c(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::PersonalServerService::PrivilegeType>::construct_fu~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c7a68 — __ZN3rbx14implementation12typed_holderIN3RBX21PersonalServerService13PrivilegeTypeEE13destruct_funcEPc — rbx::implementation::typed_holder<RBX::PersonalServerService::PrivilegeType>::destruct_func(char *)
// type: void()
#[doc(alias = "rbx::implementation::typed_holder<RBX::PersonalServerService::PrivilegeType>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX21PersonalServerService13PrivilegeTypeEE13destruct_funcEPc")]
pub fn stub_0x4c7a68(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::PersonalServerService::PrivilegeType>::destruct_fun~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c7a6c — __ZNK3RBX10Reflection8EnumDescINS_21PersonalServerService13PrivilegeTypeEE13convertToItemERKS3_ — RBX::Reflection::EnumDesc<RBX::PersonalServerService::PrivilegeType>::convertToItem(RBX::PersonalServerService::PrivilegeType const&)const
// type: int __fastcall(int, int *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::PersonalServerService::PrivilegeType>::convertToItem(RBX::PersonalServerService::PrivilegeType const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_21PersonalServerService13PrivilegeTypeEE13convertToItemERKS3_")]
pub fn stub_0x4c7a6c(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::PersonalServerService::PrivilegeType>::convertToItem(RBX::P~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c7b38 — __ZN3rbx8any_castIRKN3RBX21PersonalServerService13PrivilegeTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE — RBX::PersonalServerService::PrivilegeType const& rbx::any_cast<RBX::PersonalServerService::PrivilegeType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)
// type: char ****__fastcall(char ****)
#[doc(alias = "RBX::PersonalServerService::PrivilegeType const& rbx::any_cast<RBX::PersonalServerService::PrivilegeType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX21PersonalServerService13PrivilegeTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_0x4c7b38(any: &crate::lua::ScriptVariant) -> crate::lua::ScriptVariant {
// rbx::any_cast — extracts the payload; the host clone
// preserves the value flow.
any.clone()
}

// 0x4c7c28 — __ZNK3RBX10Reflection8EnumDescINS_21PersonalServerService13PrivilegeTypeEE14convertToValueERKNS_4NameERS3_ — RBX::Reflection::EnumDesc<RBX::PersonalServerService::PrivilegeType>::convertToValue(RBX::Name const&,RBX::PersonalServerService::PrivilegeType&)const
// type: int __fastcall(_DWORD *, unsigned int, _DWORD *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::PersonalServerService::PrivilegeType>::convertToValue(RBX::Name const&,RBX::PersonalServerService::PrivilegeType&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_21PersonalServerService13PrivilegeTypeEE14convertToValueERKNS_4NameERS3_")]
pub fn stub_0x4c7c28(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::PersonalServerService::PrivilegeType>::convertToValue(RBX::~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c7ca4 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_21PersonalServerService13PrivilegeTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E — std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::PersonalServerService::PrivilegeType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::PersonalServerService::PrivilegeType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::PersonalServerService::PrivilegeType>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::PersonalServerService::PrivilegeType>> *)
// type: void __fastcall(int, _DWORD *)
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::PersonalServerService::PrivilegeType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::PersonalServerService::PrivilegeType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::PersonalServerService::PrivilegeType>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::PersonalServerService::PrivilegeType>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_21PersonalServerService13PrivilegeTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4c7ca4(map: &mut crate::slot::TreeMapModel, key: &str) -> bool {
// map erase — unlinks the node.
map.erase(key)
}

// 0x4c7ccc — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_13SocialService9StuffTypeEEEE13initSingletonEv — RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::SocialService::StuffType> const>::initSingleton(void)
// type: int()
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::SocialService::StuffType> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_13SocialService9StuffTypeEEEE13initSingletonEv")]
pub fn stub_0x4c7ccc(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::SocialService::StuffType> const>~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c7cd0 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_13SocialService9StuffTypeEEEE14doGetSingletonEv — RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::SocialService::StuffType> const>::doGetSingleton(void)
// type: void *()
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::SocialService::StuffType> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_13SocialService9StuffTypeEEEE14doGetSingletonEv")]
pub fn stub_0x4c7cd0(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::SocialService::StuffType> const>~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c7dc0 — __ZN3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEED1Ev — RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::~EnumDesc()
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEED1Ev")]
pub fn stub_0x4c7dc0(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4c7dc4 — __ZN3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEED2Ev — RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::~EnumDesc()
// type: int __fastcall(RBX::Reflection::EnumDescriptor *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::~EnumDesc() [0x4c7dc4]")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEED2Ev")]
pub fn stub_0x4c7dc4(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4c7f98 — __ZN3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEED0Ev — RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::~EnumDesc()
// type: void __fastcall(RBX::Reflection::EnumDescriptor *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::~EnumDesc() [0x4c7f98]")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEED0Ev")]
pub fn stub_0x4c7f98(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4c8038 — __ZNK3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEE6lookupEPKc — RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::lookup(char const*)const
// type: int __fastcall(int, const char *const *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEE6lookupEPKc")]
pub fn stub_0x4c8038(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::lookup(char const*) const — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8068 — __ZNK3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEE6lookupERKNS0_7VariantE — RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::lookup(RBX::Reflection::Variant const&)const
// type: int __fastcall(int, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEE6lookupERKNS0_7VariantE")]
pub fn stub_0x4c8068(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::lookup(RBX::Reflection::Variant ~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8088 — __ZNK3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEE14convertToValueEmRNS0_7VariantE — RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToValue(unsigned long,RBX::Reflection::Variant &)const
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_0x4c8088(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToValue(unsigned long, RB~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c80e4 — __ZNK3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEE15convertToStringEmRSs — RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToString(unsigned long,std::string &)const
// type: int __fastcall(int, unsigned int, std::string *, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEE15convertToStringEmRSs")]
pub fn stub_0x4c80e4(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToString(unsigned long, s~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8228 — __ZNK3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEE15convertToStringERKS3_ — RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToString(RBX::SocialService::StuffType const&)const
// type: void __fastcall(std::string *, int, int *, int, struct _Unwind_Exception *lpuexcpt, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToString(RBX::SocialService::StuffType const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEE15convertToStringERKS3_")]
pub fn stub_0x4c8228(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToString(RBX::SocialServi~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c83c8 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_13SocialService9StuffTypeEEERS3_RKT_ — rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::SocialService::StuffType>(RBX::SocialService::StuffType const&)
// type: void (__fastcall ***__fastcall(void (__fastcall ***)(int), void (__fastcall ***)(int)))(int)
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::SocialService::StuffType>(RBX::SocialService::StuffType const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_13SocialService9StuffTypeEEERS3_RKT_")]
pub fn stub_0x4c83c8() -> crate::lua::ScriptVariant {
// placement_any ctor — empty (Void).
crate::lua::ScriptVariant::Void
}

// 0x4c8418 — __ZN3rbx14implementation12typed_holderIN3RBX13SocialService9StuffTypeEE9singletonEv — rbx::implementation::typed_holder<RBX::SocialService::StuffType>::singleton(void)
// type: _DWORD *()
#[doc(alias = "rbx::implementation::typed_holder<RBX::SocialService::StuffType>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX13SocialService9StuffTypeEE9singletonEv")]
pub fn stub_0x4c8418(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::SocialService::StuffType>::singleton() — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8484 — __ZN3rbx14implementation12typed_holderIN3RBX13SocialService9StuffTypeEE14construct_funcEPKcPc — rbx::implementation::typed_holder<RBX::SocialService::StuffType>::construct_func(char const*,char *)
// type: _DWORD *__fastcall(_DWORD *result, _DWORD *)
#[doc(alias = "rbx::implementation::typed_holder<RBX::SocialService::StuffType>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX13SocialService9StuffTypeEE14construct_funcEPKcPc")]
pub fn stub_0x4c8484(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::SocialService::StuffType>::construct_func(char cons~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8490 — __ZN3rbx14implementation12typed_holderIN3RBX13SocialService9StuffTypeEE13destruct_funcEPc — rbx::implementation::typed_holder<RBX::SocialService::StuffType>::destruct_func(char *)
// type: void()
#[doc(alias = "rbx::implementation::typed_holder<RBX::SocialService::StuffType>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX13SocialService9StuffTypeEE13destruct_funcEPc")]
pub fn stub_0x4c8490(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::SocialService::StuffType>::destruct_func(char*) — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8494 — __ZNK3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEE13convertToItemERKS3_ — RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToItem(RBX::SocialService::StuffType const&)const
// type: int __fastcall(int, int *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToItem(RBX::SocialService::StuffType const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEE13convertToItemERKS3_")]
pub fn stub_0x4c8494(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToItem(RBX::SocialService~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8560 — __ZN3rbx8any_castIRKN3RBX13SocialService9StuffTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE — RBX::SocialService::StuffType const& rbx::any_cast<RBX::SocialService::StuffType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)
// type: char ****__fastcall(char ****)
#[doc(alias = "RBX::SocialService::StuffType const& rbx::any_cast<RBX::SocialService::StuffType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX13SocialService9StuffTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_0x4c8560(any: &crate::lua::ScriptVariant) -> crate::lua::ScriptVariant {
// rbx::any_cast — extracts the payload; the host clone
// preserves the value flow.
any.clone()
}

// 0x4c8650 — __ZNK3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEE14convertToValueERKNS_4NameERS3_ — RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToValue(RBX::Name const&,RBX::SocialService::StuffType&)const
// type: int __fastcall(_DWORD *, unsigned int, _DWORD *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToValue(RBX::Name const&,RBX::SocialService::StuffType&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_13SocialService9StuffTypeEE14convertToValueERKNS_4NameERS3_")]
pub fn stub_0x4c8650(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::SocialService::StuffType>::convertToValue(RBX::Name const&,~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c86cc — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_13SocialService9StuffTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E — std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::SocialService::StuffType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::SocialService::StuffType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::SocialService::StuffType>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::SocialService::StuffType>> *)
// type: void __fastcall(int, _DWORD *)
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::SocialService::StuffType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::SocialService::StuffType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::SocialService::StuffType>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::SocialService::StuffType>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_13SocialService9StuffTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4c86cc(map: &mut crate::slot::TreeMapModel, key: &str) -> bool {
// map erase — unlinks the node.
map.erase(key)
}

// 0x4c86f4 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_16KeyframeSequence8PriorityEEEE13initSingletonEv — RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority> const>::initSingleton(void)
// type: int()
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_16KeyframeSequence8PriorityEEEE13initSingletonEv")]
pub fn stub_0x4c86f4(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority> cons~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c86f8 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_16KeyframeSequence8PriorityEEEE14doGetSingletonEv — RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority> const>::doGetSingleton(void)
// type: void *()
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_16KeyframeSequence8PriorityEEEE14doGetSingletonEv")]
pub fn stub_0x4c86f8(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority> cons~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c87e8 — __ZN3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEED1Ev — RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::~EnumDesc()
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEED1Ev")]
pub fn stub_0x4c87e8(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4c87ec — __ZN3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEED2Ev — RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::~EnumDesc()
// type: int __fastcall(RBX::Reflection::EnumDescriptor *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::~EnumDesc() [0x4c87ec]")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEED2Ev")]
pub fn stub_0x4c87ec(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4c89c0 — __ZN3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEED0Ev — RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::~EnumDesc()
// type: void __fastcall(RBX::Reflection::EnumDescriptor *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::~EnumDesc() [0x4c89c0]")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEED0Ev")]
pub fn stub_0x4c89c0(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4c8a60 — __ZNK3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEE6lookupEPKc — RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::lookup(char const*)const
// type: int __fastcall(int, const char *const *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEE6lookupEPKc")]
pub fn stub_0x4c8a60(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::lookup(char const*) const — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8a90 — __ZNK3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEE6lookupERKNS0_7VariantE — RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::lookup(RBX::Reflection::Variant const&)const
// type: int __fastcall(int, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEE6lookupERKNS0_7VariantE")]
pub fn stub_0x4c8a90(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::lookup(RBX::Reflection::Varian~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8ab0 — __ZNK3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEE14convertToValueEmRNS0_7VariantE — RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToValue(unsigned long,RBX::Reflection::Variant &)const
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_0x4c8ab0(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToValue(unsigned long, ~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8b0c — __ZNK3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEE15convertToStringEmRSs — RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToString(unsigned long,std::string &)const
// type: int __fastcall(int, unsigned int, std::string *, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEE15convertToStringEmRSs")]
pub fn stub_0x4c8b0c(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToString(unsigned long,~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8c50 — __ZNK3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEE15convertToStringERKS3_ — RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToString(RBX::KeyframeSequence::Priority const&)const
// type: void __fastcall(std::string *, int, int *, int, struct _Unwind_Exception *lpuexcpt, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToString(RBX::KeyframeSequence::Priority const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEE15convertToStringERKS3_")]
pub fn stub_0x4c8c50(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToString(RBX::KeyframeS~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8df0 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_16KeyframeSequence8PriorityEEERS3_RKT_ — rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::KeyframeSequence::Priority>(RBX::KeyframeSequence::Priority const&)
// type: void (__fastcall ***__fastcall(void (__fastcall ***)(int), void (__fastcall ***)(int)))(int)
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::KeyframeSequence::Priority>(RBX::KeyframeSequence::Priority const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_16KeyframeSequence8PriorityEEERS3_RKT_")]
pub fn stub_0x4c8df0() -> crate::lua::ScriptVariant {
// placement_any ctor — empty (Void).
crate::lua::ScriptVariant::Void
}

// 0x4c8e40 — __ZN3rbx14implementation12typed_holderIN3RBX16KeyframeSequence8PriorityEE9singletonEv — rbx::implementation::typed_holder<RBX::KeyframeSequence::Priority>::singleton(void)
// type: _DWORD *()
#[doc(alias = "rbx::implementation::typed_holder<RBX::KeyframeSequence::Priority>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX16KeyframeSequence8PriorityEE9singletonEv")]
pub fn stub_0x4c8e40(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::KeyframeSequence::Priority>::singleton() — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8eac — __ZN3rbx14implementation12typed_holderIN3RBX16KeyframeSequence8PriorityEE14construct_funcEPKcPc — rbx::implementation::typed_holder<RBX::KeyframeSequence::Priority>::construct_func(char const*,char *)
// type: _DWORD *__fastcall(_DWORD *result, _DWORD *)
#[doc(alias = "rbx::implementation::typed_holder<RBX::KeyframeSequence::Priority>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX16KeyframeSequence8PriorityEE14construct_funcEPKcPc")]
pub fn stub_0x4c8eac(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::KeyframeSequence::Priority>::construct_func(char co~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8eb8 — __ZN3rbx14implementation12typed_holderIN3RBX16KeyframeSequence8PriorityEE13destruct_funcEPc — rbx::implementation::typed_holder<RBX::KeyframeSequence::Priority>::destruct_func(char *)
// type: void()
#[doc(alias = "rbx::implementation::typed_holder<RBX::KeyframeSequence::Priority>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX16KeyframeSequence8PriorityEE13destruct_funcEPc")]
pub fn stub_0x4c8eb8(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::KeyframeSequence::Priority>::destruct_func(char*) — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8ebc — __ZNK3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEE13convertToItemERKS3_ — RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToItem(RBX::KeyframeSequence::Priority const&)const
// type: int __fastcall(int, int *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToItem(RBX::KeyframeSequence::Priority const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEE13convertToItemERKS3_")]
pub fn stub_0x4c8ebc(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToItem(RBX::KeyframeSeq~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c8f88 — __ZN3rbx8any_castIRKN3RBX16KeyframeSequence8PriorityENS1_7Region3EEET_RNS_13placement_anyIT0_EE — RBX::KeyframeSequence::Priority const& rbx::any_cast<RBX::KeyframeSequence::Priority const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)
// type: char ****__fastcall(char ****)
#[doc(alias = "RBX::KeyframeSequence::Priority const& rbx::any_cast<RBX::KeyframeSequence::Priority const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX16KeyframeSequence8PriorityENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_0x4c8f88(any: &crate::lua::ScriptVariant) -> crate::lua::ScriptVariant {
// rbx::any_cast — extracts the payload; the host clone
// preserves the value flow.
any.clone()
}

// 0x4c9078 — __ZNK3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEE14convertToValueERKNS_4NameERS3_ — RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToValue(RBX::Name const&,RBX::KeyframeSequence::Priority&)const
// type: int __fastcall(_DWORD *, unsigned int, _DWORD *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToValue(RBX::Name const&,RBX::KeyframeSequence::Priority&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_16KeyframeSequence8PriorityEE14convertToValueERKNS_4NameERS3_")]
pub fn stub_0x4c9078(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::KeyframeSequence::Priority>::convertToValue(RBX::Name const~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c90f4 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_16KeyframeSequence8PriorityEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E — std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::KeyframeSequence::Priority>,std::_Select1st<std::pair<RBX::Name const* const,RBX::KeyframeSequence::Priority>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeyframeSequence::Priority>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::KeyframeSequence::Priority>> *)
// type: void __fastcall(int, _DWORD *)
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::KeyframeSequence::Priority>,std::_Select1st<std::pair<RBX::Name const* const,RBX::KeyframeSequence::Priority>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeyframeSequence::Priority>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::KeyframeSequence::Priority>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_16KeyframeSequence8PriorityEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4c90f4(map: &mut crate::slot::TreeMapModel, key: &str) -> bool {
// map erase — unlinks the node.
map.erase(key)
}

// 0x4c911c — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17BasicPartInstance14LegacyPartTypeEEEE13initSingletonEv — RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType> const>::initSingleton(void)
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17BasicPartInstance14LegacyPartTypeEEEE13initSingletonEv")]
pub fn stub_0x4c911c(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartTyp~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c9120 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17BasicPartInstance14LegacyPartTypeEEEE14doGetSingletonEv — RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType> const>::doGetSingleton(void)
// type: void *()
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17BasicPartInstance14LegacyPartTypeEEEE14doGetSingletonEv")]
pub fn stub_0x4c9120(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartTyp~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c9210 — __ZN3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEED1Ev — RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::~EnumDesc()
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEED1Ev")]
pub fn stub_0x4c9210(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4c9214 — __ZN3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEED2Ev — RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::~EnumDesc()
// type: int __fastcall(RBX::Reflection::EnumDescriptor *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::~EnumDesc() [0x4c9214]")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEED2Ev")]
pub fn stub_0x4c9214(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4c93e8 — __ZN3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEED0Ev — RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::~EnumDesc()
// type: void __fastcall(void *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::~EnumDesc() [0x4c93e8]")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEED0Ev")]
pub fn stub_0x4c93e8(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4c9488 — __ZNK3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEE6lookupEPKc — RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::lookup(char const*)const
// type: int __fastcall(int, const char *const *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEE6lookupEPKc")]
pub fn stub_0x4c9488(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::lookup(char const*) con~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c94b8 — __ZNK3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEE6lookupERKNS0_7VariantE — RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::lookup(RBX::Reflection::Variant const&)const
// type: int __fastcall(int, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEE6lookupERKNS0_7VariantE")]
pub fn stub_0x4c94b8(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::lookup(RBX::Reflection:~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c94d8 — __ZNK3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEE14convertToValueEmRNS0_7VariantE — RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToValue(unsigned long,RBX::Reflection::Variant &)const
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_0x4c94d8(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToValue(unsigned~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c9534 — __ZNK3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEE15convertToStringEmRSs — RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToString(unsigned long,std::string &)const
// type: int __fastcall(int, unsigned int, std::string *, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEE15convertToStringEmRSs")]
pub fn stub_0x4c9534(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToString(unsigne~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c9678 — __ZNK3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEE15convertToStringERKS3_ — RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToString(RBX::BasicPartInstance::LegacyPartType const&)const
// type: void __fastcall(std::string *, int, int *, int, struct _Unwind_Exception *lpuexcpt, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToString(RBX::BasicPartInstance::LegacyPartType const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEE15convertToStringERKS3_")]
pub fn stub_0x4c9678(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToString(RBX::Ba~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c9818 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_17BasicPartInstance14LegacyPartTypeEEERS3_RKT_ — rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::BasicPartInstance::LegacyPartType>(RBX::BasicPartInstance::LegacyPartType const&)
// type: void (__fastcall ***__fastcall(void (__fastcall ***)(int), void (__fastcall ***)(int)))(int)
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::BasicPartInstance::LegacyPartType>(RBX::BasicPartInstance::LegacyPartType const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_17BasicPartInstance14LegacyPartTypeEEERS3_RKT_")]
pub fn stub_0x4c9818() -> crate::lua::ScriptVariant {
// placement_any ctor — empty (Void).
crate::lua::ScriptVariant::Void
}

// 0x4c9868 — __ZN3rbx14implementation12typed_holderIN3RBX17BasicPartInstance14LegacyPartTypeEE9singletonEv — rbx::implementation::typed_holder<RBX::BasicPartInstance::LegacyPartType>::singleton(void)
// type: _DWORD *()
#[doc(alias = "rbx::implementation::typed_holder<RBX::BasicPartInstance::LegacyPartType>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX17BasicPartInstance14LegacyPartTypeEE9singletonEv")]
pub fn stub_0x4c9868(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::BasicPartInstance::LegacyPartType>::singleton() — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c98d4 — __ZN3rbx14implementation12typed_holderIN3RBX17BasicPartInstance14LegacyPartTypeEE14construct_funcEPKcPc — rbx::implementation::typed_holder<RBX::BasicPartInstance::LegacyPartType>::construct_func(char const*,char *)
// type: _DWORD *__fastcall(_DWORD *result, _DWORD *)
#[doc(alias = "rbx::implementation::typed_holder<RBX::BasicPartInstance::LegacyPartType>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX17BasicPartInstance14LegacyPartTypeEE14construct_funcEPKcPc")]
pub fn stub_0x4c98d4(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::BasicPartInstance::LegacyPartType>::construct_func(~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c98e0 — __ZN3rbx14implementation12typed_holderIN3RBX17BasicPartInstance14LegacyPartTypeEE13destruct_funcEPc — rbx::implementation::typed_holder<RBX::BasicPartInstance::LegacyPartType>::destruct_func(char *)
// type: void()
#[doc(alias = "rbx::implementation::typed_holder<RBX::BasicPartInstance::LegacyPartType>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX17BasicPartInstance14LegacyPartTypeEE13destruct_funcEPc")]
pub fn stub_0x4c98e0(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::BasicPartInstance::LegacyPartType>::destruct_func(c~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c98e4 — __ZNK3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEE13convertToItemERKS3_ — RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToItem(RBX::BasicPartInstance::LegacyPartType const&)const
// type: int __fastcall(int, int *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToItem(RBX::BasicPartInstance::LegacyPartType const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEE13convertToItemERKS3_")]
pub fn stub_0x4c98e4(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToItem(RBX::Basi~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c99b0 — __ZN3rbx8any_castIRKN3RBX17BasicPartInstance14LegacyPartTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE — RBX::BasicPartInstance::LegacyPartType const& rbx::any_cast<RBX::BasicPartInstance::LegacyPartType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)
// type: char ****__fastcall(char ****)
#[doc(alias = "RBX::BasicPartInstance::LegacyPartType const& rbx::any_cast<RBX::BasicPartInstance::LegacyPartType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX17BasicPartInstance14LegacyPartTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_0x4c99b0(any: &crate::lua::ScriptVariant) -> crate::lua::ScriptVariant {
// rbx::any_cast — extracts the payload; the host clone
// preserves the value flow.
any.clone()
}

// 0x4c9aa0 — __ZNK3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEE14convertToValueERKNS_4NameERS3_ — RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToValue(RBX::Name const&,RBX::BasicPartInstance::LegacyPartType&)const
// type: int __fastcall(_DWORD *, unsigned int, _DWORD *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToValue(RBX::Name const&,RBX::BasicPartInstance::LegacyPartType&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_17BasicPartInstance14LegacyPartTypeEE14convertToValueERKNS_4NameERS3_")]
pub fn stub_0x4c9aa0(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::BasicPartInstance::LegacyPartType>::convertToValue(RBX::Nam~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c9b1c — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_17BasicPartInstance14LegacyPartTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E — std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::BasicPartInstance::LegacyPartType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::BasicPartInstance::LegacyPartType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::BasicPartInstance::LegacyPartType>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::BasicPartInstance::LegacyPartType>> *)
// type: void __fastcall(int, _DWORD *)
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::BasicPartInstance::LegacyPartType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::BasicPartInstance::LegacyPartType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::BasicPartInstance::LegacyPartType>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::BasicPartInstance::LegacyPartType>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_17BasicPartInstance14LegacyPartTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4c9b1c(map: &mut crate::slot::TreeMapModel, key: &str) -> bool {
// map erase — unlinks the node.
map.erase(key)
}

// 0x4c9b44 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_8Humanoid13NameOcclusionEEEE13initSingletonEv — RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion> const>::initSingleton(void)
// type: int()
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_8Humanoid13NameOcclusionEEEE13initSingletonEv")]
pub fn stub_0x4c9b44(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion> const>:~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c9b48 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_8Humanoid13NameOcclusionEEEE14doGetSingletonEv — RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion> const>::doGetSingleton(void)
// type: void *()
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_8Humanoid13NameOcclusionEEEE14doGetSingletonEv")]
pub fn stub_0x4c9b48(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion> const>:~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c9c38 — __ZN3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEED1Ev — RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::~EnumDesc()
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEED1Ev")]
pub fn stub_0x4c9c38(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4c9c3c — __ZN3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEED2Ev — RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::~EnumDesc()
// type: int __fastcall(RBX::Reflection::EnumDescriptor *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::~EnumDesc() [0x4c9c3c]")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEED2Ev")]
pub fn stub_0x4c9c3c(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4c9e10 — __ZN3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEED0Ev — RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::~EnumDesc()
// type: void __fastcall(RBX::Reflection::EnumDescriptor *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::~EnumDesc() [0x4c9e10]")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEED0Ev")]
pub fn stub_0x4c9e10(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4c9eb0 — __ZNK3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEE6lookupEPKc — RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::lookup(char const*)const
// type: int __fastcall(int, const char *const *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEE6lookupEPKc")]
pub fn stub_0x4c9eb0(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::lookup(char const*) const — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c9ee0 — __ZNK3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEE6lookupERKNS0_7VariantE — RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::lookup(RBX::Reflection::Variant const&)const
// type: int __fastcall(int, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEE6lookupERKNS0_7VariantE")]
pub fn stub_0x4c9ee0(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::lookup(RBX::Reflection::Variant c~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c9f00 — __ZNK3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEE14convertToValueEmRNS0_7VariantE — RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToValue(unsigned long,RBX::Reflection::Variant &)const
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_0x4c9f00(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToValue(unsigned long, RBX~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4c9f5c — __ZNK3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEE15convertToStringEmRSs — RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToString(unsigned long,std::string &)const
// type: int __fastcall(int, unsigned int, std::string *, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEE15convertToStringEmRSs")]
pub fn stub_0x4c9f5c(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToString(unsigned long, st~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4ca0a0 — __ZNK3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEE15convertToStringERKS3_ — RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToString(RBX::Humanoid::NameOcclusion const&)const
// type: void __fastcall(std::string *, int, int *, int, struct _Unwind_Exception *lpuexcpt, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToString(RBX::Humanoid::NameOcclusion const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEE15convertToStringERKS3_")]
pub fn stub_0x4ca0a0(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToString(RBX::Humanoid::Na~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4ca240 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_8Humanoid13NameOcclusionEEERS3_RKT_ — rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Humanoid::NameOcclusion>(RBX::Humanoid::NameOcclusion const&)
// type: void (__fastcall ***__fastcall(void (__fastcall ***)(int), void (__fastcall ***)(int)))(int)
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Humanoid::NameOcclusion>(RBX::Humanoid::NameOcclusion const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_8Humanoid13NameOcclusionEEERS3_RKT_")]
pub fn stub_0x4ca240() -> crate::lua::ScriptVariant {
// placement_any ctor — empty (Void).
crate::lua::ScriptVariant::Void
}

// 0x4ca290 — __ZN3rbx14implementation12typed_holderIN3RBX8Humanoid13NameOcclusionEE9singletonEv — rbx::implementation::typed_holder<RBX::Humanoid::NameOcclusion>::singleton(void)
// type: _DWORD *()
#[doc(alias = "rbx::implementation::typed_holder<RBX::Humanoid::NameOcclusion>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX8Humanoid13NameOcclusionEE9singletonEv")]
pub fn stub_0x4ca290(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::Humanoid::NameOcclusion>::singleton() — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4ca2fc — __ZN3rbx14implementation12typed_holderIN3RBX8Humanoid13NameOcclusionEE14construct_funcEPKcPc — rbx::implementation::typed_holder<RBX::Humanoid::NameOcclusion>::construct_func(char const*,char *)
// type: _DWORD *__fastcall(_DWORD *result, _DWORD *)
#[doc(alias = "rbx::implementation::typed_holder<RBX::Humanoid::NameOcclusion>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX8Humanoid13NameOcclusionEE14construct_funcEPKcPc")]
pub fn stub_0x4ca2fc(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::Humanoid::NameOcclusion>::construct_func(char const~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4ca308 — __ZN3rbx14implementation12typed_holderIN3RBX8Humanoid13NameOcclusionEE13destruct_funcEPc — rbx::implementation::typed_holder<RBX::Humanoid::NameOcclusion>::destruct_func(char *)
// type: void()
#[doc(alias = "rbx::implementation::typed_holder<RBX::Humanoid::NameOcclusion>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX8Humanoid13NameOcclusionEE13destruct_funcEPc")]
pub fn stub_0x4ca308(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::Humanoid::NameOcclusion>::destruct_func(char*) — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4ca30c — __ZNK3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEE13convertToItemERKS3_ — RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToItem(RBX::Humanoid::NameOcclusion const&)const
// type: int __fastcall(int, int *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToItem(RBX::Humanoid::NameOcclusion const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEE13convertToItemERKS3_")]
pub fn stub_0x4ca30c(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToItem(RBX::Humanoid::Name~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4ca3d8 — __ZN3rbx8any_castIRKN3RBX8Humanoid13NameOcclusionENS1_7Region3EEET_RNS_13placement_anyIT0_EE — RBX::Humanoid::NameOcclusion const& rbx::any_cast<RBX::Humanoid::NameOcclusion const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)
// type: char ****__fastcall(char ****)
#[doc(alias = "RBX::Humanoid::NameOcclusion const& rbx::any_cast<RBX::Humanoid::NameOcclusion const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX8Humanoid13NameOcclusionENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_0x4ca3d8(any: &crate::lua::ScriptVariant) -> crate::lua::ScriptVariant {
// rbx::any_cast — extracts the payload; the host clone
// preserves the value flow.
any.clone()
}

// 0x4ca4c8 — __ZNK3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEE14convertToValueERKNS_4NameERS3_ — RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToValue(RBX::Name const&,RBX::Humanoid::NameOcclusion&)const
// type: int __fastcall(_DWORD *, unsigned int, _DWORD *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToValue(RBX::Name const&,RBX::Humanoid::NameOcclusion&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8Humanoid13NameOcclusionEE14convertToValueERKNS_4NameERS3_")]
pub fn stub_0x4ca4c8(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::Humanoid::NameOcclusion>::convertToValue(RBX::Name const&, ~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4ca544 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_8Humanoid13NameOcclusionEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E — std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::Humanoid::NameOcclusion>,std::_Select1st<std::pair<RBX::Name const* const,RBX::Humanoid::NameOcclusion>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::Humanoid::NameOcclusion>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::Humanoid::NameOcclusion>> *)
// type: void __fastcall(int, _DWORD *)
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::Humanoid::NameOcclusion>,std::_Select1st<std::pair<RBX::Name const* const,RBX::Humanoid::NameOcclusion>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::Humanoid::NameOcclusion>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::Humanoid::NameOcclusion>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_8Humanoid13NameOcclusionEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4ca544(map: &mut crate::slot::TreeMapModel, key: &str) -> bool {
// map erase — unlinks the node.
map.erase(key)
}

// 0x4ca56c — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_8Humanoid6StatusEEEE13initSingletonEv — RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Humanoid::Status> const>::initSingleton(void)
// type: int()
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Humanoid::Status> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_8Humanoid6StatusEEEE13initSingletonEv")]
pub fn stub_0x4ca56c(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Humanoid::Status> const>::initSi~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4ca570 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_8Humanoid6StatusEEEE14doGetSingletonEv — RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Humanoid::Status> const>::doGetSingleton(void)
// type: _UNKNOWN **()
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Humanoid::Status> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_8Humanoid6StatusEEEE14doGetSingletonEv")]
pub fn stub_0x4ca570(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Humanoid::Status> const>::doGetS~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4ca660 — __ZN3RBX10Reflection8EnumDescINS_8Humanoid6StatusEED1Ev — RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::~EnumDesc()
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_8Humanoid6StatusEED1Ev")]
pub fn stub_0x4ca660(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4ca664 — __ZN3RBX10Reflection8EnumDescINS_8Humanoid6StatusEED2Ev — RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::~EnumDesc()
// type: int __fastcall(RBX::Reflection::EnumDescriptor *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::~EnumDesc() [0x4ca664]")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_8Humanoid6StatusEED2Ev")]
pub fn stub_0x4ca664(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4ca838 — __ZN3RBX10Reflection8EnumDescINS_8Humanoid6StatusEED0Ev — RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::~EnumDesc()
// type: void __fastcall(RBX::Reflection::EnumDescriptor *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::~EnumDesc() [0x4ca838]")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_8Humanoid6StatusEED0Ev")]
pub fn stub_0x4ca838(handle: crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc dtor.
drop(handle);
}

// 0x4ca8d8 — __ZNK3RBX10Reflection8EnumDescINS_8Humanoid6StatusEE6lookupEPKc — RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::lookup(char const*)const
// type: int __fastcall(int, const char *const *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8Humanoid6StatusEE6lookupEPKc")]
pub fn stub_0x4ca8d8(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::lookup(char const*) const — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4ca908 — __ZNK3RBX10Reflection8EnumDescINS_8Humanoid6StatusEE6lookupERKNS0_7VariantE — RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::lookup(RBX::Reflection::Variant const&)const
// type: int __fastcall(int, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8Humanoid6StatusEE6lookupERKNS0_7VariantE")]
pub fn stub_0x4ca908(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::lookup(RBX::Reflection::Variant const&) ~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4ca928 — __ZNK3RBX10Reflection8EnumDescINS_8Humanoid6StatusEE14convertToValueEmRNS0_7VariantE — RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToValue(unsigned long,RBX::Reflection::Variant &)const
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8Humanoid6StatusEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_0x4ca928(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToValue(unsigned long, RBX::Refle~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4ca984 — __ZNK3RBX10Reflection8EnumDescINS_8Humanoid6StatusEE15convertToStringEmRSs — RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToString(unsigned long,std::string &)const
// type: int __fastcall(int, unsigned int, std::string *, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8Humanoid6StatusEE15convertToStringEmRSs")]
pub fn stub_0x4ca984(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToString(unsigned long, std::stri~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4caac8 — __ZNK3RBX10Reflection8EnumDescINS_8Humanoid6StatusEE15convertToStringERKS3_ — RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToString(RBX::Humanoid::Status const&)const
// type: void __fastcall(std::string *, int, int *, int, struct _Unwind_Exception *lpuexcpt, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToString(RBX::Humanoid::Status const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8Humanoid6StatusEE15convertToStringERKS3_")]
pub fn stub_0x4caac8(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToString(RBX::Humanoid::Status co~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4cac68 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_8Humanoid6StatusEEERS3_RKT_ — rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Humanoid::Status>(RBX::Humanoid::Status const&)
// type: void (__fastcall ***__fastcall(void (__fastcall ***)(int), void (__fastcall ***)(int)))(int)
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Humanoid::Status>(RBX::Humanoid::Status const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_8Humanoid6StatusEEERS3_RKT_")]
pub fn stub_0x4cac68() -> crate::lua::ScriptVariant {
// placement_any ctor — empty (Void).
crate::lua::ScriptVariant::Void
}

// 0x4cacb8 — __ZN3rbx14implementation12typed_holderIN3RBX8Humanoid6StatusEE9singletonEv — rbx::implementation::typed_holder<RBX::Humanoid::Status>::singleton(void)
// type: _DWORD *()
#[doc(alias = "rbx::implementation::typed_holder<RBX::Humanoid::Status>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX8Humanoid6StatusEE9singletonEv")]
pub fn stub_0x4cacb8(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::Humanoid::Status>::singleton() — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4cad24 — __ZN3rbx14implementation12typed_holderIN3RBX8Humanoid6StatusEE14construct_funcEPKcPc — rbx::implementation::typed_holder<RBX::Humanoid::Status>::construct_func(char const*,char *)
// type: _DWORD *__fastcall(_DWORD *result, _DWORD *)
#[doc(alias = "rbx::implementation::typed_holder<RBX::Humanoid::Status>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX8Humanoid6StatusEE14construct_funcEPKcPc")]
pub fn stub_0x4cad24(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::Humanoid::Status>::construct_func(char const*, char~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4cad30 — __ZN3rbx14implementation12typed_holderIN3RBX8Humanoid6StatusEE13destruct_funcEPc — rbx::implementation::typed_holder<RBX::Humanoid::Status>::destruct_func(char *)
// type: void()
#[doc(alias = "rbx::implementation::typed_holder<RBX::Humanoid::Status>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX8Humanoid6StatusEE13destruct_funcEPc")]
pub fn stub_0x4cad30(handle: &crate::slot::InstanceHandle) {
// rbx::implementation::typed_holder<RBX::Humanoid::Status>::destruct_func(char*) — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4cad34 — __ZNK3RBX10Reflection8EnumDescINS_8Humanoid6StatusEE13convertToItemERKS3_ — RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToItem(RBX::Humanoid::Status const&)const
// type: int __fastcall(int, int *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToItem(RBX::Humanoid::Status const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8Humanoid6StatusEE13convertToItemERKS3_")]
pub fn stub_0x4cad34(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToItem(RBX::Humanoid::Status cons~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4cae00 — __ZN3rbx8any_castIRKN3RBX8Humanoid6StatusENS1_7Region3EEET_RNS_13placement_anyIT0_EE — RBX::Humanoid::Status const& rbx::any_cast<RBX::Humanoid::Status const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)
// type: char ****__fastcall(char ****)
#[doc(alias = "RBX::Humanoid::Status const& rbx::any_cast<RBX::Humanoid::Status const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX8Humanoid6StatusENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_0x4cae00(any: &crate::lua::ScriptVariant) -> crate::lua::ScriptVariant {
// rbx::any_cast — extracts the payload; the host clone
// preserves the value flow.
any.clone()
}

// 0x4caef0 — __ZNK3RBX10Reflection8EnumDescINS_8Humanoid6StatusEE14convertToValueERKNS_4NameERS3_ — RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToValue(RBX::Name const&,RBX::Humanoid::Status&)const
// type: int __fastcall(_DWORD *, unsigned int, _DWORD *)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToValue(RBX::Name const&,RBX::Humanoid::Status&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_8Humanoid6StatusEE14convertToValueERKNS_4NameERS3_")]
pub fn stub_0x4caef0(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::EnumDesc<RBX::Humanoid::Status>::convertToValue(RBX::Name const&, RBX::Hu~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

// 0x4caf6c — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_8Humanoid6StatusEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E — std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::Humanoid::Status>,std::_Select1st<std::pair<RBX::Name const* const,RBX::Humanoid::Status>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::Humanoid::Status>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::Humanoid::Status>> *)
// type: void __fastcall(int, _DWORD *)
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::Humanoid::Status>,std::_Select1st<std::pair<RBX::Name const* const,RBX::Humanoid::Status>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::Humanoid::Status>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::Humanoid::Status>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_8Humanoid6StatusEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4caf6c(map: &mut crate::slot::TreeMapModel, key: &str) -> bool {
// map erase — unlinks the node.
map.erase(key)
}

// 0x4caf94 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescIN3G3D7Vector34AxisEEEE13initSingletonEv — RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<G3D::Vector3::Axis> const>::initSingleton(void)
// type: int()
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<G3D::Vector3::Axis> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescIN3G3D7Vector34AxisEEEE13initSingletonEv")]
pub fn stub_0x4caf94(handle: &crate::slot::InstanceHandle) {
// RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<G3D::Vector3::Axis> const>::initSingl~ — engine-side; linkage preserved via the alias.
let _ = handle;
}

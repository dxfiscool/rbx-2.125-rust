//! platform — generated_watchdog_platform_w15d3 — 100 stubs EA-sorted asc platform GameView/GLES2/Ogre mobile filtered per-crate gap (global exhausted)
//! Source: ida/export.json (85545 funcs) filter platform|GameView|GLES2|Ogre|mobile (10347 total) primary 0/10347 not in /tmp/global_eas.txt (85545/85545 covered), fallback per-crate gap not yet in crates/platform/src (6787/10347, 100 in batch)
//! Batch: 100 stubs | range 0xb68e84..0xba0b70 | rbx_core::SharedPtr not boost
//! Distinct platform 49094->49194 | uncovered 0->0 (global) | per-crate 6787->6687

#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]

use rbx_core::SharedPtr;

const _: () = {
    let _ = core::marker::PhantomData::<SharedPtr<u8>>;
};

// 0xb68e84 — __ZNSt3mapISsPN4Ogre17VertexDeclarationESt4lessISsESaISt4pairIKSsS2_EEEixERS6_ — std::map<std::string,Ogre::VertexDeclaration *,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::VertexDeclaration *>>>::operator[](std::string const&)
// type: int __fastcall(int, const void **)
#[doc(alias = "__ZNSt3mapISsPN4Ogre17VertexDeclarationESt4lessISsESaISt4pairIKSsS2_EEEixERS6_")]
#[doc(alias = "std::map<std::string,Ogre::VertexDeclaration *,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::VertexDeclaration *>>>::operator[](std::string const&)")]
pub fn stub_b68e84() -> ! {
    todo!("0xb68e84 __ZNSt3mapISsPN4Ogre17VertexDeclarationESt4lessISsESaISt4pairIKSsS2_EEEixERS6_")
}

// 0xb69040 — __ZNSt8_Rb_treeISsSt4pairIKSsPN4Ogre17VertexDeclarationEESt10_Select1stIS5_ESt4lessISsESaIS5_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS5_ERKS5_ — std::_Rb_tree<std::string,std::pair<std::string const,Ogre::VertexDeclaration *>,std::_Select1st<std::pair<std::string const,Ogre::VertexDeclaration *>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::VertexDeclaration *>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<std::string const,Ogre::VertexDeclaration *>>,std::pair<std::string const,Ogre::VertexDeclaration *> const&)
// type: _Rb_tree_node_base *__fastcall(_DWORD *, _Rb_tree_node_base *, struct _Unwind_Exception *lpuexcpt)
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsPN4Ogre17VertexDeclarationEESt10_Select1stIS5_ESt4lessISsESaIS5_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS5_ERKS5_")]
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::VertexDeclaration *>,std::_Select1st<std::pair<std::string const,Ogre::VertexDeclaration *>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::VertexDeclaration *>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<std::string const,Ogre::VertexDeclaration *>>,std::pair<std::string const,Ogre::VertexDeclaration *> const&)")]
pub fn stub_b69040() -> ! {
    todo!("0xb69040 __ZNSt8_Rb_treeISsSt4pairIKSsPN4Ogre17VertexDeclarationEESt10_Select1stIS5_ESt4lessISsESaIS5_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS5_ERKS5_")
}

// 0xb69220 — __ZNSt8_Rb_treeISsSt4pairIKSsPN4Ogre17VertexDeclarationEESt10_Select1stIS5_ESt4lessISsESaIS5_EE9_M_insertEPSt18_Rb_tree_node_baseSD_RKS5_ — std::_Rb_tree<std::string,std::pair<std::string const,Ogre::VertexDeclaration *>,std::_Select1st<std::pair<std::string const,Ogre::VertexDeclaration *>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::VertexDeclaration *>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<std::string const,Ogre::VertexDeclaration *> const&)
// type: _Rb_tree_node_base *__fastcall(int, unsigned int, _Rb_tree_node_base *, int, struct _Unwind_Exception *lpuexcpt, void *, int, int, void *, int)
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsPN4Ogre17VertexDeclarationEESt10_Select1stIS5_ESt4lessISsESaIS5_EE9_M_insertEPSt18_Rb_tree_node_baseSD_RKS5_")]
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::VertexDeclaration *>,std::_Select1st<std::pair<std::string const,Ogre::VertexDeclaration *>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::VertexDeclaration *>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<std::string const,Ogre::VertexDeclaration *> const&)")]
pub fn stub_b69220() -> ! {
    todo!("0xb69220 __ZNSt8_Rb_treeISsSt4pairIKSsPN4Ogre17VertexDeclarationEESt10_Select1stIS5_ESt4lessISsESaIS5_EE9_M_insertEPSt18_Rb_tree_node_baseSD_RKS5_")
}

// 0xb69368 — __ZNSt8_Rb_treeISsSt4pairIKSsPN4Ogre17VertexDeclarationEESt10_Select1stIS5_ESt4lessISsESaIS5_EE16_M_insert_uniqueERKS5_ — std::_Rb_tree<std::string,std::pair<std::string const,Ogre::VertexDeclaration *>,std::_Select1st<std::pair<std::string const,Ogre::VertexDeclaration *>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::VertexDeclaration *>>>::_M_insert_unique(std::pair<std::string const,Ogre::VertexDeclaration *> const&)
// type: int __fastcall(int, int, struct _Unwind_Exception *lpuexcpt)
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsPN4Ogre17VertexDeclarationEESt10_Select1stIS5_ESt4lessISsESaIS5_EE16_M_insert_uniqueERKS5_")]
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::VertexDeclaration *>,std::_Select1st<std::pair<std::string const,Ogre::VertexDeclaration *>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::VertexDeclaration *>>>::_M_insert_unique(std::pair<std::string const,Ogre::VertexDeclaration *> const&)")]
pub fn stub_b69368() -> ! {
    todo!("0xb69368 __ZNSt8_Rb_treeISsSt4pairIKSsPN4Ogre17VertexDeclarationEESt10_Select1stIS5_ESt4lessISsESaIS5_EE16_M_insert_uniqueERKS5_")
}

// 0xb6944c — __ZNSt8_Rb_treeISsSt4pairIKSsPN4Ogre17VertexDeclarationEESt10_Select1stIS5_ESt4lessISsESaIS5_EE8_M_eraseEPSt13_Rb_tree_nodeIS5_E — std::_Rb_tree<std::string,std::pair<std::string const,Ogre::VertexDeclaration *>,std::_Select1st<std::pair<std::string const,Ogre::VertexDeclaration *>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::VertexDeclaration *>>>::_M_erase(std::_Rb_tree_node<std::pair<std::string const,Ogre::VertexDeclaration *>> *)
// type: void __fastcall(int, _DWORD *)
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsPN4Ogre17VertexDeclarationEESt10_Select1stIS5_ESt4lessISsESaIS5_EE8_M_eraseEPSt13_Rb_tree_nodeIS5_E")]
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::VertexDeclaration *>,std::_Select1st<std::pair<std::string const,Ogre::VertexDeclaration *>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::VertexDeclaration *>>>::_M_erase(std::_Rb_tree_node<std::pair<std::string const,Ogre::VertexDeclaration *>> *)")]
pub fn stub_b6944c() -> ! {
    todo!("0xb6944c __ZNSt8_Rb_treeISsSt4pairIKSsPN4Ogre17VertexDeclarationEESt10_Select1stIS5_ESt4lessISsESaIS5_EE8_M_eraseEPSt13_Rb_tree_nodeIS5_E")
}

// 0xb6a2c8 — __ZN3RBX26FastClusterShadowGenerator20getVertexDeclarationEPN4Ogre12VisualEngineE — RBX::FastClusterShadowGenerator::getVertexDeclaration(Ogre::VisualEngine *)
// type: int __fastcall(RBX::FastClusterShadowGenerator *this, Ogre::VisualEngine *)
#[doc(alias = "__ZN3RBX26FastClusterShadowGenerator20getVertexDeclarationEPN4Ogre12VisualEngineE")]
#[doc(alias = "RBX::FastClusterShadowGenerator::getVertexDeclaration(Ogre::VisualEngine *)")]
pub fn stub_b6a2c8() -> ! {
    todo!("0xb6a2c8 __ZN3RBX26FastClusterShadowGenerator20getVertexDeclarationEPN4Ogre12VisualEngineE")
}

// 0xb6a438 — __ZN3RBX26FastClusterShadowGenerator16createVertexDataEPN4Ogre12VisualEngineERKSt6vectorINS0_6VertexESaIS5_EEj — RBX::FastClusterShadowGenerator::createVertexData(Ogre::VisualEngine *,std::vector<RBX::FastClusterShadowGenerator::Vertex,std::allocator<RBX::FastClusterShadowGenerator::Vertex>> const&,unsigned int)
// type: Ogre::HardwareBufferManager *__fastcall(RBX::FastClusterShadowGenerator *, Ogre::VisualEngine *, int, int, struct _Unwind_Exception *lpuexcpt, int, Ogre::NedPoolingImpl *, Ogre::NedPoolingImpl *, int, Ogre::NedPoolingImpl *, int, int, int, int)
#[doc(alias = "__ZN3RBX26FastClusterShadowGenerator16createVertexDataEPN4Ogre12VisualEngineERKSt6vectorINS0_6VertexESaIS5_EEj")]
#[doc(alias = "RBX::FastClusterShadowGenerator::createVertexData(Ogre::VisualEngine *,std::vector<RBX::FastClusterShadowGenerator::Vertex,std::allocator<RBX::FastClusterShadowGenerator::Vertex>> const&,unsigned int)")]
pub fn stub_b6a438() -> ! {
    todo!("0xb6a438 __ZN3RBX26FastClusterShadowGenerator16createVertexDataEPN4Ogre12VisualEngineERKSt6vectorINS0_6VertexESaIS5_EEj")
}

// 0xb6a6f8 — __ZN3RBX26FastClusterShadowGenerator8generateEPN4Ogre12VisualEngineEPKNS_17GeometryGenerator6VertexEjPKtjRKSt6vectorIjSaIjEERKSA_INS_14ShadowInstanceESaISF_EEb — RBX::FastClusterShadowGenerator::generate(Ogre::VisualEngine *,RBX::GeometryGenerator::Vertex const*,unsigned int,unsigned short const*,unsigned int,std::vector<unsigned int,std::allocator<unsigned int>> const&,std::vector const&<RBX::ShadowInstance,std::allocator<std::vector const>>,bool)
// type: int __fastcall(RBX::FastClusterShadowGenerator *, int, unsigned int, int, unsigned int, int **, int, struct _Unwind_Exception *)
#[doc(alias = "__ZN3RBX26FastClusterShadowGenerator8generateEPN4Ogre12VisualEngineEPKNS_17GeometryGenerator6VertexEjPKtjRKSt6vectorIjSaIjEERKSA_INS_14ShadowInstanceESaISF_EEb")]
#[doc(alias = "RBX::FastClusterShadowGenerator::generate(Ogre::VisualEngine *,RBX::GeometryGenerator::Vertex const*,unsigned int,unsigned short const*,unsigned int,std::vector<unsigned int,std::allocator<unsigned int>> const&,std::vector const&<RBX::ShadowInstance,std::allocator<std::vector const>>,bool)")]
pub fn stub_b6a6f8() -> ! {
    todo!("0xb6a6f8 __ZN3RBX26FastClusterShadowGenerator8generateEPN4Ogre12VisualEngineEPKNS_17GeometryGenerator6VertexEjPKtjRKSt6vectorIjSaIjEERKSA_INS_14ShadowInstanceESaISF_EEb")
}

// 0xb6b1e8 — __ZN3RBX27FastClusterShadowRenderable14generateVolumeEPKN4Ogre5LightEfmPtj — RBX::FastClusterShadowRenderable::generateVolume(Ogre::Light const*,float,unsigned long,unsigned short *,unsigned int)
// type: unsigned __int16 *__fastcall(RBX::FastClusterShadowRenderable *this, const Ogre::Light *, float, char, unsigned __int16 *, unsigned int)
#[doc(alias = "__ZN3RBX27FastClusterShadowRenderable14generateVolumeEPKN4Ogre5LightEfmPtj")]
#[doc(alias = "RBX::FastClusterShadowRenderable::generateVolume(Ogre::Light const*,float,unsigned long,unsigned short *,unsigned int)")]
pub fn stub_b6b1e8() -> ! {
    todo!("0xb6b1e8 __ZN3RBX27FastClusterShadowRenderable14generateVolumeEPKN4Ogre5LightEfmPtj")
}

// 0xb6b620 — __ZNK3RBX27FastClusterShadowRenderable18getWorldTransformsEPN4Ogre7Matrix4E — RBX::FastClusterShadowRenderable::getWorldTransforms(Ogre::Matrix4 *)const
// type: int __fastcall(int)
#[doc(alias = "__ZNK3RBX27FastClusterShadowRenderable18getWorldTransformsEPN4Ogre7Matrix4E")]
#[doc(alias = "RBX::FastClusterShadowRenderable::getWorldTransforms(Ogre::Matrix4 *)const")]
pub fn stub_b6b620() -> ! {
    todo!("0xb6b620 __ZNK3RBX27FastClusterShadowRenderable18getWorldTransformsEPN4Ogre7Matrix4E")
}

// 0xb6b680 — __ZNK3RBX27FastClusterShadowRenderable16getDarkCapBoundsERKN4Ogre5LightEf — RBX::FastClusterShadowRenderable::getDarkCapBounds(Ogre::Light const&,float)const
// type: char *__fastcall(RBX::FastClusterShadowRenderable *this, const Ogre::Light *, float)
#[doc(alias = "__ZNK3RBX27FastClusterShadowRenderable16getDarkCapBoundsERKN4Ogre5LightEf")]
#[doc(alias = "RBX::FastClusterShadowRenderable::getDarkCapBounds(Ogre::Light const&,float)const")]
pub fn stub_b6b680() -> ! {
    todo!("0xb6b680 __ZNK3RBX27FastClusterShadowRenderable16getDarkCapBoundsERKN4Ogre5LightEf")
}

// 0xb6b718 — __ZThn96_NK3RBX27FastClusterShadowRenderable16getDarkCapBoundsERKN4Ogre5LightEf — `non-virtual thunk to'RBX::FastClusterShadowRenderable::getDarkCapBounds(Ogre::Light const&,float)const
// type: char *__fastcall(RBX::FastClusterShadowRenderable *this, const Ogre::Light *, float)
#[doc(alias = "__ZThn96_NK3RBX27FastClusterShadowRenderable16getDarkCapBoundsERKN4Ogre5LightEf")]
#[doc(alias = "non-virtual thunk toRBX::FastClusterShadowRenderable::getDarkCapBounds(Ogre::Light const&,float)const")]
pub fn stub_b6b718() -> ! {
    todo!("0xb6b718 __ZThn96_NK3RBX27FastClusterShadowRenderable16getDarkCapBoundsERKN4Ogre5LightEf")
}

// 0xb6b7b0 — __ZN3RBX27FastClusterShadowRenderable33getShadowVolumeRenderableIteratorEN4Ogre15ShadowTechniqueEPKNS1_5LightEPNS1_28HardwareIndexBufferSharedPtrEbfm — RBX::FastClusterShadowRenderable::getShadowVolumeRenderableIterator(Ogre::ShadowTechnique,Ogre::Light const*,Ogre::HardwareIndexBufferSharedPtr *,bool,float,unsigned long)
// type: double __fastcall(int)
#[doc(alias = "__ZN3RBX27FastClusterShadowRenderable33getShadowVolumeRenderableIteratorEN4Ogre15ShadowTechniqueEPKNS1_5LightEPNS1_28HardwareIndexBufferSharedPtrEbfm")]
#[doc(alias = "RBX::FastClusterShadowRenderable::getShadowVolumeRenderableIterator(Ogre::ShadowTechnique,Ogre::Light const*,Ogre::HardwareIndexBufferSharedPtr *,bool,float,unsigned long)")]
pub fn stub_b6b7b0() -> ! {
    todo!("0xb6b7b0 __ZN3RBX27FastClusterShadowRenderable33getShadowVolumeRenderableIteratorEN4Ogre15ShadowTechniqueEPKNS1_5LightEPNS1_28HardwareIndexBufferSharedPtrEbfm")
}

// 0xb6b824 — __ZThn96_N3RBX27FastClusterShadowRenderable33getShadowVolumeRenderableIteratorEN4Ogre15ShadowTechniqueEPKNS1_5LightEPNS1_28HardwareIndexBufferSharedPtrEbfm — `non-virtual thunk to'RBX::FastClusterShadowRenderable::getShadowVolumeRenderableIterator(Ogre::ShadowTechnique,Ogre::Light const*,Ogre::HardwareIndexBufferSharedPtr *,bool,float,unsigned long)
// type: double __fastcall(int)
#[doc(alias = "__ZThn96_N3RBX27FastClusterShadowRenderable33getShadowVolumeRenderableIteratorEN4Ogre15ShadowTechniqueEPKNS1_5LightEPNS1_28HardwareIndexBufferSharedPtrEbfm")]
#[doc(alias = "non-virtual thunk toRBX::FastClusterShadowRenderable::getShadowVolumeRenderableIterator(Ogre::ShadowTechnique,Ogre::Light const*,Ogre::HardwareIndexBufferSharedPtr *,bool,float,unsigned long)")]
pub fn stub_b6b824() -> ! {
    todo!("0xb6b824 __ZThn96_N3RBX27FastClusterShadowRenderable33getShadowVolumeRenderableIteratorEN4Ogre15ShadowTechniqueEPKNS1_5LightEPNS1_28HardwareIndexBufferSharedPtrEbfm")
}

// 0xb6b898 — __ZNK3RBX27FastClusterShadowRenderable25getPointExtrusionDistanceEPKN4Ogre5LightE — RBX::FastClusterShadowRenderable::getPointExtrusionDistance(Ogre::Light const*)const
// type: int()
#[doc(alias = "__ZNK3RBX27FastClusterShadowRenderable25getPointExtrusionDistanceEPKN4Ogre5LightE")]
#[doc(alias = "RBX::FastClusterShadowRenderable::getPointExtrusionDistance(Ogre::Light const*)const")]
pub fn stub_b6b898() -> ! {
    todo!("0xb6b898 __ZNK3RBX27FastClusterShadowRenderable25getPointExtrusionDistanceEPKN4Ogre5LightE")
}

// 0xb6b89c — __ZThn96_NK3RBX27FastClusterShadowRenderable25getPointExtrusionDistanceEPKN4Ogre5LightE — `non-virtual thunk to'RBX::FastClusterShadowRenderable::getPointExtrusionDistance(Ogre::Light const*)const
// type: int()
#[doc(alias = "__ZThn96_NK3RBX27FastClusterShadowRenderable25getPointExtrusionDistanceEPKN4Ogre5LightE")]
#[doc(alias = "non-virtual thunk toRBX::FastClusterShadowRenderable::getPointExtrusionDistance(Ogre::Light const*)const")]
pub fn stub_b6b89c() -> ! {
    todo!("0xb6b89c __ZThn96_NK3RBX27FastClusterShadowRenderable25getPointExtrusionDistanceEPKN4Ogre5LightE")
}

// 0xb6b8a0 — __ZN3RBX27FastClusterShadowRenderable17rebindIndexBufferERKN4Ogre28HardwareIndexBufferSharedPtrE — RBX::FastClusterShadowRenderable::rebindIndexBuffer(Ogre::HardwareIndexBufferSharedPtr const&)
// type: void __fastcall(RBX::FastClusterShadowRenderable *this, const Ogre::HardwareIndexBufferSharedPtr *)
#[doc(alias = "__ZN3RBX27FastClusterShadowRenderable17rebindIndexBufferERKN4Ogre28HardwareIndexBufferSharedPtrE")]
#[doc(alias = "RBX::FastClusterShadowRenderable::rebindIndexBuffer(Ogre::HardwareIndexBufferSharedPtr const&)")]
pub fn stub_b6b8a0() -> ! {
    todo!("0xb6b8a0 __ZN3RBX27FastClusterShadowRenderable17rebindIndexBufferERKN4Ogre28HardwareIndexBufferSharedPtrE")
}

// 0xb6b9a4 — __ZN3RBX17FastClusterEntityC2EPNS_11FastClusterEPN4Ogre10VertexDataEPNS3_9IndexDataERKNS3_11MaterialPtrERKSt6vectorIjSaIjEEPNS_21FastClusterShadowDataERKNS3_14AxisAlignedBoxEh — RBX::FastClusterEntity::FastClusterEntity(RBX::FastCluster *,Ogre::VertexData *,Ogre::IndexData *,Ogre::MaterialPtr const&,std::vector<unsigned int,std::allocator<unsigned int>> const&,RBX::FastClusterShadowData *,Ogre::AxisAlignedBox const&,unsigned char)
// type: RBX::FastClusterEntity *__fastcall(RBX::FastClusterEntity *, RBX::RenderNode *, Ogre::VertexData *, Ogre::IndexData *, struct _Unwind_Exception *, int, RBX::FastClusterShadowData *, __int32 *, unsigned __int8)
#[doc(alias = "__ZN3RBX17FastClusterEntityC2EPNS_11FastClusterEPN4Ogre10VertexDataEPNS3_9IndexDataERKNS3_11MaterialPtrERKSt6vectorIjSaIjEEPNS_21FastClusterShadowDataERKNS3_14AxisAlignedBoxEh")]
#[doc(alias = "RBX::FastClusterEntity::FastClusterEntity(RBX::FastCluster *,Ogre::VertexData *,Ogre::IndexData *,Ogre::MaterialPtr const&,std::vector<unsigned int,std::allocator<unsigned int>> const&,RBX::FastClusterShadowData *,Ogre::AxisAlignedBox const&,unsigned char)")]
pub fn stub_b6b9a4() -> ! {
    todo!("0xb6b9a4 __ZN3RBX17FastClusterEntityC2EPNS_11FastClusterEPN4Ogre10VertexDataEPNS3_9IndexDataERKNS3_11MaterialPtrERKSt6vectorIjSaIjEEPNS_21FastClusterShadowDataERKNS3_14AxisAlignedBoxEh")
}

// 0xb6bdcc — __ZNK3RBX17FastClusterEntity18getWorldTransformsEPN4Ogre7Matrix4E — RBX::FastClusterEntity::getWorldTransforms(Ogre::Matrix4 *)const
// type: int __fastcall(RBX::FastClusterEntity *this, Ogre::Matrix4 *)
#[doc(alias = "__ZNK3RBX17FastClusterEntity18getWorldTransformsEPN4Ogre7Matrix4E")]
#[doc(alias = "RBX::FastClusterEntity::getWorldTransforms(Ogre::Matrix4 *)const")]
pub fn stub_b6bdcc() -> ! {
    todo!("0xb6bdcc __ZNK3RBX17FastClusterEntity18getWorldTransformsEPN4Ogre7Matrix4E")
}

// 0xb6c468 — __ZN3RBX11FastClusterC1EPN4Ogre12SceneManagerEPNS_8HumanoidEPNS_11SpatialGridIS0_EERKNS_16SpatialGridIndexEb — RBX::FastCluster::FastCluster(Ogre::SceneManager *,RBX::Humanoid *,RBX::SpatialGrid<RBX::FastCluster> *,RBX::SpatialGridIndex const&,bool)
// type: int __fastcall(int, int)
#[doc(alias = "__ZN3RBX11FastClusterC1EPN4Ogre12SceneManagerEPNS_8HumanoidEPNS_11SpatialGridIS0_EERKNS_16SpatialGridIndexEb")]
#[doc(alias = "RBX::FastCluster::FastCluster(Ogre::SceneManager *,RBX::Humanoid *,RBX::SpatialGrid<RBX::FastCluster> *,RBX::SpatialGridIndex const&,bool)")]
pub fn stub_b6c468() -> ! {
    todo!("0xb6c468 __ZN3RBX11FastClusterC1EPN4Ogre12SceneManagerEPNS_8HumanoidEPNS_11SpatialGridIS0_EERKNS_16SpatialGridIndexEb")
}

// 0xb6c480 — __ZN3RBX11FastClusterC2EPN4Ogre12SceneManagerEPNS_8HumanoidEPNS_11SpatialGridIS0_EERKNS_16SpatialGridIndexEb — RBX::FastCluster::FastCluster(Ogre::SceneManager *,RBX::Humanoid *,RBX::SpatialGrid<RBX::FastCluster> *,RBX::SpatialGridIndex const&,bool)
// type: RBX::RenderNode *__fastcall(RBX::RenderNode *, Ogre::SceneManager *, const void *, int, __int16 *, int)
#[doc(alias = "__ZN3RBX11FastClusterC2EPN4Ogre12SceneManagerEPNS_8HumanoidEPNS_11SpatialGridIS0_EERKNS_16SpatialGridIndexEb")]
#[doc(alias = "RBX::FastCluster::FastCluster(Ogre::SceneManager *,RBX::Humanoid *,RBX::SpatialGrid<RBX::FastCluster> *,RBX::SpatialGridIndex const&,bool)")]
pub fn stub_b6c480() -> ! {
    todo!("0xb6c480 __ZN3RBX11FastClusterC2EPN4Ogre12SceneManagerEPNS_8HumanoidEPNS_11SpatialGridIS0_EERKNS_16SpatialGridIndexEb")
}

// 0xb6dff0 — __ZN3RBX11FastCluster18invalidateLightingERKN4Ogre14AxisAlignedBoxE — RBX::FastCluster::invalidateLighting(Ogre::AxisAlignedBox const&)
// type: int __fastcall(RBX::FastCluster *this, const Ogre::AxisAlignedBox *)
#[doc(alias = "__ZN3RBX11FastCluster18invalidateLightingERKN4Ogre14AxisAlignedBoxE")]
#[doc(alias = "RBX::FastCluster::invalidateLighting(Ogre::AxisAlignedBox const&)")]
pub fn stub_b6dff0() -> ! {
    todo!("0xb6dff0 __ZN3RBX11FastCluster18invalidateLightingERKN4Ogre14AxisAlignedBoxE")
}

// 0xb6f208 — __ZN4Ogre16ShadowRenderableD2Ev — Ogre::ShadowRenderable::~ShadowRenderable()
// type: void __fastcall(Ogre::ShadowRenderable *__hidden this)
#[doc(alias = "__ZN4Ogre16ShadowRenderableD2Ev")]
#[doc(alias = "Ogre::ShadowRenderable::~ShadowRenderable()")]
pub fn stub_b6f208() -> ! {
    todo!("0xb6f208 __ZN4Ogre16ShadowRenderableD2Ev")
}

// 0xb6f488 — __ZNSt6vectorIPN4Ogre16ShadowRenderableENS0_12STLAllocatorIS2_NS0_22CategorisedAllocPolicyILNS0_14MemoryCategoryE0EEEEEED1Ev — std::vector<Ogre::ShadowRenderable *,Ogre::STLAllocator<Ogre::ShadowRenderable *,Ogre::CategorisedAllocPolicy<(Ogre::MemoryCategory)0>>>::~vector()
// type: int __fastcall(int, void *)
#[doc(alias = "__ZNSt6vectorIPN4Ogre16ShadowRenderableENS0_12STLAllocatorIS2_NS0_22CategorisedAllocPolicyILNS0_14MemoryCategoryE0EEEEEED1Ev")]
#[doc(alias = "std::vector<Ogre::ShadowRenderable *,Ogre::STLAllocator<Ogre::ShadowRenderable *,Ogre::CategorisedAllocPolicy<(Ogre::MemoryCategory)0>>>::~vector()")]
pub fn stub_b6f488() -> ! {
    todo!("0xb6f488 __ZNSt6vectorIPN4Ogre16ShadowRenderableENS0_12STLAllocatorIS2_NS0_22CategorisedAllocPolicyILNS0_14MemoryCategoryE0EEEEEED1Ev")
}

// 0xb70fe0 — __ZNK4Ogre10Renderable12getTechniqueEv — Ogre::Renderable::getTechnique(void)const
// type: int __fastcall(Ogre::Renderable *this)
#[doc(alias = "__ZNK4Ogre10Renderable12getTechniqueEv")]
#[doc(alias = "Ogre::Renderable::getTechnique(void)const")]
pub fn stub_b70fe0() -> ! {
    todo!("0xb70fe0 __ZNK4Ogre10Renderable12getTechniqueEv")
}

// 0xb71008 — __ZNK4Ogre16ShadowRenderable9isVisibleEv — Ogre::ShadowRenderable::isVisible(void)const
// type: int __fastcall(Ogre::ShadowRenderable *this)
#[doc(alias = "__ZNK4Ogre16ShadowRenderable9isVisibleEv")]
#[doc(alias = "Ogre::ShadowRenderable::isVisible(void)const")]
pub fn stub_b71008() -> ! {
    todo!("0xb71008 __ZNK4Ogre16ShadowRenderable9isVisibleEv")
}

// 0xb720f0 — __ZNSt12_Vector_baseIPN4Ogre16ShadowRenderableENS0_12STLAllocatorIS2_NS0_22CategorisedAllocPolicyILNS0_14MemoryCategoryE0EEEEEE12_Vector_implD0Ev — std::_Vector_base<Ogre::ShadowRenderable *,Ogre::STLAllocator<Ogre::ShadowRenderable *,Ogre::CategorisedAllocPolicy<(Ogre::MemoryCategory)0>>>::_Vector_impl::~_Vector_impl()
// type: int __fastcall(int)
#[doc(alias = "__ZNSt12_Vector_baseIPN4Ogre16ShadowRenderableENS0_12STLAllocatorIS2_NS0_22CategorisedAllocPolicyILNS0_14MemoryCategoryE0EEEEEE12_Vector_implD0Ev")]
#[doc(alias = "std::_Vector_base<Ogre::ShadowRenderable *,Ogre::STLAllocator<Ogre::ShadowRenderable *,Ogre::CategorisedAllocPolicy<(Ogre::MemoryCategory)0>>>::_Vector_impl::~_Vector_impl()")]
pub fn stub_b720f0() -> ! {
    todo!("0xb720f0 __ZNSt12_Vector_baseIPN4Ogre16ShadowRenderableENS0_12STLAllocatorIS2_NS0_22CategorisedAllocPolicyILNS0_14MemoryCategoryE0EEEEEE12_Vector_implD0Ev")
}

// 0xb74238 — __ZN4Ogre28HardwareIndexBufferSharedPtrD1Ev — Ogre::HardwareIndexBufferSharedPtr::~HardwareIndexBufferSharedPtr()
// type: void __fastcall(Ogre::HardwareIndexBufferSharedPtr *__hidden this)
#[doc(alias = "__ZN4Ogre28HardwareIndexBufferSharedPtrD1Ev")]
#[doc(alias = "Ogre::HardwareIndexBufferSharedPtr::~HardwareIndexBufferSharedPtr()")]
pub fn stub_b74238() -> ! {
    todo!("0xb74238 __ZN4Ogre28HardwareIndexBufferSharedPtrD1Ev")
}

// 0xb74290 — __ZN4Ogre9SharedPtrINS_19HardwareIndexBufferEED0Ev — Ogre::SharedPtr<Ogre::HardwareIndexBuffer>::~SharedPtr()
// type: void __fastcall(void *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_19HardwareIndexBufferEED0Ev")]
#[doc(alias = "Ogre::SharedPtr<Ogre::HardwareIndexBuffer>::~SharedPtr()")]
pub fn stub_b74290() -> ! {
    todo!("0xb74290 __ZN4Ogre9SharedPtrINS_19HardwareIndexBufferEED0Ev")
}

// 0xb74330 — __ZN4Ogre9SharedPtrINS_19HardwareIndexBufferEE7destroyEv — Ogre::SharedPtr<Ogre::HardwareIndexBuffer>::destroy(void)
// type: int __fastcall(int, void *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_19HardwareIndexBufferEE7destroyEv")]
#[doc(alias = "Ogre::SharedPtr<Ogre::HardwareIndexBuffer>::destroy(void)")]
pub fn stub_b74330() -> ! {
    todo!("0xb74330 __ZN4Ogre9SharedPtrINS_19HardwareIndexBufferEE7destroyEv")
}

// 0xb74368 — __ZN4Ogre9SharedPtrINS_19HardwareIndexBufferEE4swapERS2_ — Ogre::SharedPtr<Ogre::HardwareIndexBuffer>::swap(Ogre::SharedPtr<Ogre::HardwareIndexBuffer>&)
// type: _DWORD *__fastcall(_DWORD *result, _DWORD *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_19HardwareIndexBufferEE4swapERS2_")]
#[doc(alias = "Ogre::SharedPtr<Ogre::HardwareIndexBuffer>::swap(Ogre::SharedPtr<Ogre::HardwareIndexBuffer>&)")]
pub fn stub_b74368() -> ! {
    todo!("0xb74368 __ZN4Ogre9SharedPtrINS_19HardwareIndexBufferEE4swapERS2_")
}

// 0xb76748 — __ZN3RBX24FastClusterMeshGeneratorC2EPN4Ogre12VisualEngineEPNS_8HumanoidEjb — RBX::FastClusterMeshGenerator::FastClusterMeshGenerator(Ogre::VisualEngine *,RBX::Humanoid *,unsigned int,bool)
// type: RBX::FastClusterMeshGenerator *__fastcall(RBX::FastClusterMeshGenerator *this, Ogre::VisualEngine *, RBX::Humanoid *, unsigned int, RBX::MaterialGenerator *)
#[doc(alias = "__ZN3RBX24FastClusterMeshGeneratorC2EPN4Ogre12VisualEngineEPNS_8HumanoidEjb")]
#[doc(alias = "RBX::FastClusterMeshGenerator::FastClusterMeshGenerator(Ogre::VisualEngine *,RBX::Humanoid *,unsigned int,bool)")]
pub fn stub_b76748() -> ! {
    todo!("0xb76748 __ZN3RBX24FastClusterMeshGeneratorC2EPN4Ogre12VisualEngineEPNS_8HumanoidEjb")
}

// 0xb76a08 — __ZN4Ogre16ShadowRenderableD1Ev — Ogre::ShadowRenderable::~ShadowRenderable()
// type: void __fastcall(Ogre::ShadowRenderable *__hidden this)
#[doc(alias = "__ZN4Ogre16ShadowRenderableD1Ev")]
#[doc(alias = "Ogre::ShadowRenderable::~ShadowRenderable()")]
pub fn stub_b76a08() -> ! {
    todo!("0xb76a08 __ZN4Ogre16ShadowRenderableD1Ev")
}

// 0xb86c08 — __ZN3RBX17MaterialGeneratorC1EPN4Ogre12VisualEngineE — RBX::MaterialGenerator::MaterialGenerator(Ogre::VisualEngine *)
// type: _DWORD *__fastcall(_DWORD *result, int)
#[doc(alias = "__ZN3RBX17MaterialGeneratorC1EPN4Ogre12VisualEngineE")]
#[doc(alias = "RBX::MaterialGenerator::MaterialGenerator(Ogre::VisualEngine *)")]
pub fn stub_b86c08() -> ! {
    todo!("0xb86c08 __ZN3RBX17MaterialGeneratorC1EPN4Ogre12VisualEngineE")
}

// 0xb87738 — __ZN3RBX17MaterialGenerator22createTexturedMaterialERKN4Ogre10TexturePtrERKSsj — RBX::MaterialGenerator::createTexturedMaterial(Ogre::TexturePtr const&,std::string const&,unsigned int)
// type: void __fastcall(RBX::MaterialGenerator *this, const Ogre::TexturePtr *, const std::string *, const std::string *, unsigned int)
#[doc(alias = "__ZN3RBX17MaterialGenerator22createTexturedMaterialERKN4Ogre10TexturePtrERKSsj")]
#[doc(alias = "RBX::MaterialGenerator::createTexturedMaterial(Ogre::TexturePtr const&,std::string const&,unsigned int)")]
pub fn stub_b87738() -> ! {
    todo!("0xb87738 __ZN3RBX17MaterialGenerator22createTexturedMaterialERKN4Ogre10TexturePtrERKSsj")
}

// 0xb8ab5c — __ZN12_GLOBAL__N_129createHumanoidTextureCompositEPN4Ogre12VisualEngineERKN3RBX18HumanoidIdentifierERA4_KNS_16AccoutrementMeshEPNS3_13CharacterMeshE — `anonymous namespace'::createHumanoidTextureComposit(Ogre::VisualEngine *,RBX::HumanoidIdentifier const&,`anonymous namespace'::AccoutrementMesh const(&)[4],RBX::CharacterMesh *)
// type: void __fastcall(int, int, int, int *, int)
#[doc(alias = "__ZN12_GLOBAL__N_129createHumanoidTextureCompositEPN4Ogre12VisualEngineERKN3RBX18HumanoidIdentifierERA4_KNS_16AccoutrementMeshEPNS3_13CharacterMeshE")]
#[doc(alias = "anonymous namespace::createHumanoidTextureComposit(Ogre::VisualEngine *,RBX::HumanoidIdentifier const&,anonymous namespace::AccoutrementMesh const(&)[4],RBX::CharacterMesh *)")]
pub fn stub_b8ab5c() -> ! {
    todo!("0xb8ab5c __ZN12_GLOBAL__N_129createHumanoidTextureCompositEPN4Ogre12VisualEngineERKN3RBX18HumanoidIdentifierERA4_KNS_16AccoutrementMeshEPNS3_13CharacterMeshE")
}

// 0xb8caa0 — __ZNSt4pairIN4Ogre10TexturePtrEN5boost10shared_ptrIN3RBX17TextureCompositor3JobEEEED1Ev — std::pair<Ogre::TexturePtr,boost::shared_ptr<RBX::TextureCompositor::Job>>::~pair()
// type: int __fastcall(int)
#[doc(alias = "__ZNSt4pairIN4Ogre10TexturePtrEN5boost10shared_ptrIN3RBX17TextureCompositor3JobEEEED1Ev")]
#[doc(alias = "std::pair<Ogre::TexturePtr,boost::shared_ptr<RBX::TextureCompositor::Job>>::~pair()")]
pub fn stub_b8caa0() -> ! {
    todo!("0xb8caa0 __ZNSt4pairIN4Ogre10TexturePtrEN5boost10shared_ptrIN3RBX17TextureCompositor3JobEEEED1Ev")
}

// 0xb8d6f4 — __ZNSt4pairIN4Ogre10TexturePtrEN5boost10shared_ptrIN3RBX17TextureCompositor3JobEEEEC2ERKS1_RKS7_ — std::pair<Ogre::TexturePtr,boost::shared_ptr<RBX::TextureCompositor::Job>>::pair(Ogre::TexturePtr const&,boost::shared_ptr<RBX::TextureCompositor::Job> const&)
// type: _DWORD *__fastcall(_DWORD *, _DWORD *, _DWORD *)
#[doc(alias = "__ZNSt4pairIN4Ogre10TexturePtrEN5boost10shared_ptrIN3RBX17TextureCompositor3JobEEEEC2ERKS1_RKS7_")]
#[doc(alias = "std::pair<Ogre::TexturePtr,boost::shared_ptr<RBX::TextureCompositor::Job>>::pair(Ogre::TexturePtr const&,boost::shared_ptr<RBX::TextureCompositor::Job> const&)")]
pub fn stub_b8d6f4() -> ! {
    todo!("0xb8d6f4 __ZNSt4pairIN4Ogre10TexturePtrEN5boost10shared_ptrIN3RBX17TextureCompositor3JobEEEEC2ERKS1_RKS7_")
}

// 0xb8df78 — __ZN3RBX11AdornRbxGfxC1EPN4Ogre12VisualEngineEPKNS_9DataModelE — RBX::AdornRbxGfx::AdornRbxGfx(Ogre::VisualEngine *,RBX::DataModel const*)
// type: int __fastcall(RBX::AdornRbxGfx *this, Ogre::VisualEngine *, const RBX::DataModel *)
#[doc(alias = "__ZN3RBX11AdornRbxGfxC1EPN4Ogre12VisualEngineEPKNS_9DataModelE")]
#[doc(alias = "RBX::AdornRbxGfx::AdornRbxGfx(Ogre::VisualEngine *,RBX::DataModel const*)")]
pub fn stub_b8df78() -> ! {
    todo!("0xb8df78 __ZN3RBX11AdornRbxGfxC1EPN4Ogre12VisualEngineEPKNS_9DataModelE")
}

// 0xb8df7c — __ZN3RBX11AdornRbxGfxC2EPN4Ogre12VisualEngineEPKNS_9DataModelE — RBX::AdornRbxGfx::AdornRbxGfx(Ogre::VisualEngine *,RBX::DataModel const*)
// type: RBX::Adorn *__fastcall(RBX::AdornRbxGfx *this, Ogre::VisualEngine *, const RBX::DataModel *, int)
#[doc(alias = "__ZN3RBX11AdornRbxGfxC2EPN4Ogre12VisualEngineEPKNS_9DataModelE")]
#[doc(alias = "RBX::AdornRbxGfx::AdornRbxGfx(Ogre::VisualEngine *,RBX::DataModel const*)")]
pub fn stub_b8df7c() -> ! {
    todo!("0xb8df7c __ZN3RBX11AdornRbxGfxC2EPN4Ogre12VisualEngineEPKNS_9DataModelE")
}

// 0xb8f1b8 — __ZN3RBX11AdornRbxGfx12registerMeshERSsRN4Ogre7MeshPtrE — RBX::AdornRbxGfx::registerMesh(std::string &,Ogre::MeshPtr &)
// type: void __fastcall(RBX::AdornRbxGfx *this, std::string *, Ogre::MeshPtr *)
#[doc(alias = "__ZN3RBX11AdornRbxGfx12registerMeshERSsRN4Ogre7MeshPtrE")]
#[doc(alias = "RBX::AdornRbxGfx::registerMesh(std::string &,Ogre::MeshPtr &)")]
pub fn stub_b8f1b8() -> ! {
    todo!("0xb8f1b8 __ZN3RBX11AdornRbxGfx12registerMeshERSsRN4Ogre7MeshPtrE")
}

// 0xb962e8 — __ZN3RBX11AdornRbxGfx17RESTORE_EXPLOSIONEPN4Ogre14ParticleSystemEPNS1_15ParticleEmitterEif — RBX::AdornRbxGfx::RESTORE_EXPLOSION(Ogre::ParticleSystem *,Ogre::ParticleEmitter *,int,float)
// type: void __fastcall(RBX::AdornRbxGfx *this, Ogre::ParticleSystem *, Ogre::ParticleEmitter *, int, float32_t)
#[doc(alias = "__ZN3RBX11AdornRbxGfx17RESTORE_EXPLOSIONEPN4Ogre14ParticleSystemEPNS1_15ParticleEmitterEif")]
#[doc(alias = "RBX::AdornRbxGfx::RESTORE_EXPLOSION(Ogre::ParticleSystem *,Ogre::ParticleEmitter *,int,float)")]
pub fn stub_b962e8() -> ! {
    todo!("0xb962e8 __ZN3RBX11AdornRbxGfx17RESTORE_EXPLOSIONEPN4Ogre14ParticleSystemEPNS1_15ParticleEmitterEif")
}

// 0xb98608 — __ZN4Ogre7MeshPtrD1Ev — Ogre::MeshPtr::~MeshPtr()
// type: void __fastcall(Ogre::MeshPtr *__hidden this)
#[doc(alias = "__ZN4Ogre7MeshPtrD1Ev")]
#[doc(alias = "Ogre::MeshPtr::~MeshPtr()")]
pub fn stub_b98608() -> ! {
    todo!("0xb98608 __ZN4Ogre7MeshPtrD1Ev")
}

// 0xb98874 — __ZNSt3mapISsN4Ogre7MeshPtrESt4lessISsESaISt4pairIKSsS1_EEEixERS5_ — std::map<std::string,Ogre::MeshPtr,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::operator[](std::string const&)
// type: uint32_t *__fastcall(int, const void **)
#[doc(alias = "__ZNSt3mapISsN4Ogre7MeshPtrESt4lessISsESaISt4pairIKSsS1_EEEixERS5_")]
#[doc(alias = "std::map<std::string,Ogre::MeshPtr,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::operator[](std::string const&)")]
pub fn stub_b98874() -> ! {
    todo!("0xb98874 __ZNSt3mapISsN4Ogre7MeshPtrESt4lessISsESaISt4pairIKSsS1_EEEixERS5_")
}

// 0xb98bb0 — __ZN4Ogre9SharedPtrINS_10DataStreamEED1Ev — Ogre::SharedPtr<Ogre::DataStream>::~SharedPtr()
// type: int __fastcall(int)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_10DataStreamEED1Ev")]
#[doc(alias = "Ogre::SharedPtr<Ogre::DataStream>::~SharedPtr()")]
pub fn stub_b98bb0() -> ! {
    todo!("0xb98bb0 __ZN4Ogre9SharedPtrINS_10DataStreamEED1Ev")
}

// 0xb98c04 — __ZNSt3mapIN3RBX12Vector3int32EPN4Ogre9SceneNodeESt4lessIS1_ESaISt4pairIKS1_S4_EEEixERS8_ — std::map<RBX::Vector3int32,Ogre::SceneNode *,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::operator[](RBX::Vector3int32 const&)
// type: _Rb_tree_node_base **__fastcall(int, int *)
#[doc(alias = "__ZNSt3mapIN3RBX12Vector3int32EPN4Ogre9SceneNodeESt4lessIS1_ESaISt4pairIKS1_S4_EEEixERS8_")]
#[doc(alias = "std::map<RBX::Vector3int32,Ogre::SceneNode *,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::operator[](RBX::Vector3int32 const&)")]
pub fn stub_b98c04() -> ! {
    todo!("0xb98c04 __ZNSt3mapIN3RBX12Vector3int32EPN4Ogre9SceneNodeESt4lessIS1_ESaISt4pairIKS1_S4_EEEixERS8_")
}

// 0xb98c94 — __ZNSt6vectorIPN4Ogre9SceneNodeESaIS2_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS2_S4_EERKS2_ — std::vector<Ogre::SceneNode *,std::allocator<Ogre::SceneNode *>>::_M_insert_aux(__gnu_cxx::__normal_iterator<Ogre::SceneNode **,std::vector<Ogre::SceneNode *,std::allocator<Ogre::SceneNode *>>>,Ogre::SceneNode * const&)
// type: void *__fastcall(int, char *__src, _DWORD *)
#[doc(alias = "__ZNSt6vectorIPN4Ogre9SceneNodeESaIS2_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS2_S4_EERKS2_")]
#[doc(alias = "std::vector<Ogre::SceneNode *,std::allocator<Ogre::SceneNode *>>::_M_insert_aux(__gnu_cxx::__normal_iterator<Ogre::SceneNode **,std::vector<Ogre::SceneNode *,std::allocator<Ogre::SceneNode *>>>,Ogre::SceneNode * const&)")]
pub fn stub_b98c94() -> ! {
    todo!("0xb98c94 __ZNSt6vectorIPN4Ogre9SceneNodeESaIS2_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS2_S4_EERKS2_")
}

// 0xb98d8c — __ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS7_ERKS7_ — std::_Rb_tree<RBX::Vector3int32,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>,std::_Select1st<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *> const&)
// type: _Rb_tree_node_base *__fastcall(int, _Rb_tree_node_base *, int *)
#[doc(alias = "__ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS7_ERKS7_")]
#[doc(alias = "std::_Rb_tree<RBX::Vector3int32,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>,std::_Select1st<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *> const&)")]
pub fn stub_b98d8c() -> ! {
    todo!("0xb98d8c __ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS7_ERKS7_")
}

// 0xb990d4 — __ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE16_M_insert_uniqueERKS7_ — std::_Rb_tree<RBX::Vector3int32,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>,std::_Select1st<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::_M_insert_unique(std::pair<RBX::Vector3int32 const,Ogre::SceneNode *> const&)
// type: int __fastcall(int, _DWORD *, int *)
#[doc(alias = "__ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE16_M_insert_uniqueERKS7_")]
#[doc(alias = "std::_Rb_tree<RBX::Vector3int32,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>,std::_Select1st<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::_M_insert_unique(std::pair<RBX::Vector3int32 const,Ogre::SceneNode *> const&)")]
pub fn stub_b990d4() -> ! {
    todo!("0xb990d4 __ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE16_M_insert_uniqueERKS7_")
}

// 0xb991d4 — __ZNSt6vectorISt17_Rb_tree_iteratorISt4pairIKN3RBX12Vector3int32EPN4Ogre9SceneNodeEEESaIS9_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS9_SB_EERKS9_ — std::vector<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::allocator<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>>::_M_insert_aux(__gnu_cxx::__normal_iterator<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>*,std::vector<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::allocator<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>>>,std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>> const&)
// type: _DWORD *__fastcall(int, char *, _DWORD *)
#[doc(alias = "__ZNSt6vectorISt17_Rb_tree_iteratorISt4pairIKN3RBX12Vector3int32EPN4Ogre9SceneNodeEEESaIS9_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS9_SB_EERKS9_")]
#[doc(alias = "std::vector<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::allocator<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>>::_M_insert_aux(__gnu_cxx::__normal_iterator<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>*,std::vector<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::allocator<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>>>,std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>> const&)")]
pub fn stub_b991d4() -> ! {
    todo!("0xb991d4 __ZNSt6vectorISt17_Rb_tree_iteratorISt4pairIKN3RBX12Vector3int32EPN4Ogre9SceneNodeEEESaIS9_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS9_SB_EERKS9_")
}

// 0xb992d8 — __ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE8_M_eraseEPSt13_Rb_tree_nodeIS7_E — std::_Rb_tree<RBX::Vector3int32,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>,std::_Select1st<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>> *)
// type: void __fastcall(int, _DWORD *)
#[doc(alias = "__ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE8_M_eraseEPSt13_Rb_tree_nodeIS7_E")]
#[doc(alias = "std::_Rb_tree<RBX::Vector3int32,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>,std::_Select1st<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>> *)")]
pub fn stub_b992d8() -> ! {
    todo!("0xb992d8 __ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE8_M_eraseEPSt13_Rb_tree_nodeIS7_E")
}

// 0xb99300 — __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE8_M_eraseEPSt13_Rb_tree_nodeIS4_E — std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_erase(std::_Rb_tree_node<std::pair<std::string const,Ogre::MeshPtr>> *)
// type: void __fastcall(int, _DWORD *)
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE8_M_eraseEPSt13_Rb_tree_nodeIS4_E")]
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_erase(std::_Rb_tree_node<std::pair<std::string const,Ogre::MeshPtr>> *)")]
pub fn stub_b99300() -> ! {
    todo!("0xb99300 __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE8_M_eraseEPSt13_Rb_tree_nodeIS4_E")
}

// 0xb99330 — __ZN9__gnu_cxx13new_allocatorISt4pairIKSsN4Ogre7MeshPtrEEE7destroyEPS5_ — __gnu_cxx::new_allocator<std::pair<std::string const,Ogre::MeshPtr>>::destroy(std::pair<std::string const,Ogre::MeshPtr>*)
// type: void __fastcall(int, int)
#[doc(alias = "__ZN9__gnu_cxx13new_allocatorISt4pairIKSsN4Ogre7MeshPtrEEE7destroyEPS5_")]
#[doc(alias = "__gnu_cxx::new_allocator<std::pair<std::string const,Ogre::MeshPtr>>::destroy(std::pair<std::string const,Ogre::MeshPtr>*)")]
pub fn stub_b99330() -> ! {
    todo!("0xb99330 __ZN9__gnu_cxx13new_allocatorISt4pairIKSsN4Ogre7MeshPtrEEE7destroyEPS5_")
}

// 0xb99630 — __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS4_ERKS4_ — std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<std::string const,Ogre::MeshPtr>>,std::pair<std::string const,Ogre::MeshPtr> const&)
// type: _Rb_tree_node_base *__fastcall(int, _Rb_tree_node_base *, const void **)
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS4_ERKS4_")]
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<std::string const,Ogre::MeshPtr>>,std::pair<std::string const,Ogre::MeshPtr> const&)")]
pub fn stub_b99630() -> ! {
    todo!("0xb99630 __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS4_ERKS4_")
}

// 0xb99978 — __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE9_M_insertEPSt18_Rb_tree_node_baseSC_RKS4_ — std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<std::string const,Ogre::MeshPtr> const&)
// type: _Rb_tree_node_base *__fastcall(int, int, _Rb_tree_node_base *, const void **)
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE9_M_insertEPSt18_Rb_tree_node_baseSC_RKS4_")]
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<std::string const,Ogre::MeshPtr> const&)")]
pub fn stub_b99978() -> ! {
    todo!("0xb99978 __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE9_M_insertEPSt18_Rb_tree_node_baseSC_RKS4_")
}

// 0xb999ec — __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE16_M_insert_uniqueERKS4_ — std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_insert_unique(std::pair<std::string const,Ogre::MeshPtr> const&)
// type: int __fastcall(int, int, const void **)
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE16_M_insert_uniqueERKS4_")]
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_insert_unique(std::pair<std::string const,Ogre::MeshPtr> const&)")]
pub fn stub_b999ec() -> ! {
    todo!("0xb999ec __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE16_M_insert_uniqueERKS4_")
}

// 0xb99ad0 — __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE14_M_create_nodeERKS4_ — std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_create_node(std::pair<std::string const,Ogre::MeshPtr> const&)
// type: _DWORD *__fastcall(int, const std::string *, int, int, void *, int)
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE14_M_create_nodeERKS4_")]
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_create_node(std::pair<std::string const,Ogre::MeshPtr> const&)")]
pub fn stub_b99ad0() -> ! {
    todo!("0xb99ad0 __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE14_M_create_nodeERKS4_")
}

// 0xb99be8 — __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE4findERS1_ — std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::find(std::string const&)
// type: _DWORD *__fastcall(int, const void **)
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE4findERS1_")]
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::find(std::string const&)")]
pub fn stub_b99be8() -> ! {
    todo!("0xb99be8 __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE4findERS1_")
}

// 0xb99c8c — __ZNSt6vectorIPN4Ogre12RbxSubEntityESaIS2_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS2_S4_EERKS2_ — std::vector<Ogre::RbxSubEntity *,std::allocator<Ogre::RbxSubEntity *>>::_M_insert_aux(__gnu_cxx::__normal_iterator<Ogre::RbxSubEntity **,std::vector<Ogre::RbxSubEntity *,std::allocator<Ogre::RbxSubEntity *>>>,Ogre::RbxSubEntity * const&)
// type: void *__fastcall(int, char *__src, _DWORD *)
#[doc(alias = "__ZNSt6vectorIPN4Ogre12RbxSubEntityESaIS2_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS2_S4_EERKS2_")]
#[doc(alias = "std::vector<Ogre::RbxSubEntity *,std::allocator<Ogre::RbxSubEntity *>>::_M_insert_aux(__gnu_cxx::__normal_iterator<Ogre::RbxSubEntity **,std::vector<Ogre::RbxSubEntity *,std::allocator<Ogre::RbxSubEntity *>>>,Ogre::RbxSubEntity * const&)")]
pub fn stub_b99c8c() -> ! {
    todo!("0xb99c8c __ZNSt6vectorIPN4Ogre12RbxSubEntityESaIS2_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS2_S4_EERKS2_")
}

// 0xb99d88 — __ZN4Ogre9SharedPtrINS_20GpuProgramParametersEED0Ev — Ogre::SharedPtr<Ogre::GpuProgramParameters>::~SharedPtr()
// type: void __fastcall(_DWORD *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_20GpuProgramParametersEED0Ev")]
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuProgramParameters>::~SharedPtr()")]
pub fn stub_b99d88() -> ! {
    todo!("0xb99d88 __ZN4Ogre9SharedPtrINS_20GpuProgramParametersEED0Ev")
}

// 0xb99e48 — __ZN4Ogre9SharedPtrINS_20GpuProgramParametersEE7destroyEv — Ogre::SharedPtr<Ogre::GpuProgramParameters>::destroy(void)
// type: void __fastcall(int)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_20GpuProgramParametersEE7destroyEv")]
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuProgramParameters>::destroy(void)")]
pub fn stub_b99e48() -> ! {
    todo!("0xb99e48 __ZN4Ogre9SharedPtrINS_20GpuProgramParametersEE7destroyEv")
}

// 0xb99f40 — __ZN4Ogre9SharedPtrINS_20GpuProgramParametersEE4swapERS2_ — Ogre::SharedPtr<Ogre::GpuProgramParameters>::swap(Ogre::SharedPtr<Ogre::GpuProgramParameters>&)
// type: _DWORD *__fastcall(_DWORD *result, _DWORD *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_20GpuProgramParametersEE4swapERS2_")]
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuProgramParameters>::swap(Ogre::SharedPtr<Ogre::GpuProgramParameters>&)")]
pub fn stub_b99f40() -> ! {
    todo!("0xb99f40 __ZN4Ogre9SharedPtrINS_20GpuProgramParametersEE4swapERS2_")
}

// 0xb99f60 — __ZN4Ogre9SharedPtrINS_17GpuNamedConstantsEED1Ev — Ogre::SharedPtr<Ogre::GpuNamedConstants>::~SharedPtr()
// type: _DWORD *__fastcall(_DWORD *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_17GpuNamedConstantsEED1Ev")]
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuNamedConstants>::~SharedPtr()")]
pub fn stub_b99f60() -> ! {
    todo!("0xb99f60 __ZN4Ogre9SharedPtrINS_17GpuNamedConstantsEED1Ev")
}

// 0xb99f90 — __ZN4Ogre9SharedPtrINS_22GpuLogicalBufferStructEED0Ev — Ogre::SharedPtr<Ogre::GpuLogicalBufferStruct>::~SharedPtr()
// type: void __fastcall(_DWORD *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_22GpuLogicalBufferStructEED0Ev")]
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuLogicalBufferStruct>::~SharedPtr()")]
pub fn stub_b99f90() -> ! {
    todo!("0xb99f90 __ZN4Ogre9SharedPtrINS_22GpuLogicalBufferStructEED0Ev")
}

// 0xb9a050 — __ZN4Ogre9SharedPtrINS_22GpuLogicalBufferStructEE7destroyEv — Ogre::SharedPtr<Ogre::GpuLogicalBufferStruct>::destroy(void)
// type: void __fastcall(int)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_22GpuLogicalBufferStructEE7destroyEv")]
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuLogicalBufferStruct>::destroy(void)")]
pub fn stub_b9a050() -> ! {
    todo!("0xb9a050 __ZN4Ogre9SharedPtrINS_22GpuLogicalBufferStructEE7destroyEv")
}

// 0xb9a150 — __ZN4Ogre9SharedPtrINS_22GpuLogicalBufferStructEE4swapERS2_ — Ogre::SharedPtr<Ogre::GpuLogicalBufferStruct>::swap(Ogre::SharedPtr<Ogre::GpuLogicalBufferStruct>&)
// type: _DWORD *__fastcall(_DWORD *result, _DWORD *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_22GpuLogicalBufferStructEE4swapERS2_")]
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuLogicalBufferStruct>::swap(Ogre::SharedPtr<Ogre::GpuLogicalBufferStruct>&)")]
pub fn stub_b9a150() -> ! {
    todo!("0xb9a150 __ZN4Ogre9SharedPtrINS_22GpuLogicalBufferStructEE4swapERS2_")
}

// 0xb9a170 — __ZN4Ogre9SharedPtrINS_17GpuNamedConstantsEE4swapERS2_ — Ogre::SharedPtr<Ogre::GpuNamedConstants>::swap(Ogre::SharedPtr<Ogre::GpuNamedConstants>&)
// type: _DWORD *__fastcall(_DWORD *result, _DWORD *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_17GpuNamedConstantsEE4swapERS2_")]
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuNamedConstants>::swap(Ogre::SharedPtr<Ogre::GpuNamedConstants>&)")]
pub fn stub_b9a170() -> ! {
    todo!("0xb9a170 __ZN4Ogre9SharedPtrINS_17GpuNamedConstantsEE4swapERS2_")
}

// 0xb9a190 — __ZN4Ogre9SharedPtrINS_19GpuSharedParametersEED0Ev — Ogre::SharedPtr<Ogre::GpuSharedParameters>::~SharedPtr()
// type: void __fastcall(void *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_19GpuSharedParametersEED0Ev")]
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuSharedParameters>::~SharedPtr()")]
pub fn stub_b9a190() -> ! {
    todo!("0xb9a190 __ZN4Ogre9SharedPtrINS_19GpuSharedParametersEED0Ev")
}

// 0xb9a230 — __ZN4Ogre9SharedPtrINS_19GpuSharedParametersEE7destroyEv — Ogre::SharedPtr<Ogre::GpuSharedParameters>::destroy(void)
// type: int __fastcall(int, void *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_19GpuSharedParametersEE7destroyEv")]
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuSharedParameters>::destroy(void)")]
pub fn stub_b9a230() -> ! {
    todo!("0xb9a230 __ZN4Ogre9SharedPtrINS_19GpuSharedParametersEE7destroyEv")
}

// 0xb9a268 — __ZN4Ogre9SharedPtrINS_19GpuSharedParametersEE4swapERS2_ — Ogre::SharedPtr<Ogre::GpuSharedParameters>::swap(Ogre::SharedPtr<Ogre::GpuSharedParameters>&)
// type: _DWORD *__fastcall(_DWORD *result, _DWORD *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_19GpuSharedParametersEE4swapERS2_")]
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuSharedParameters>::swap(Ogre::SharedPtr<Ogre::GpuSharedParameters>&)")]
pub fn stub_b9a268() -> ! {
    todo!("0xb9a268 __ZN4Ogre9SharedPtrINS_19GpuSharedParametersEE4swapERS2_")
}

// 0xb9a284 — __ZN3RBX26ManualObjectMeshGenAdapterC2EPN4Ogre12ManualObjectE — RBX::ManualObjectMeshGenAdapter::ManualObjectMeshGenAdapter(Ogre::ManualObject *)
// type: RBX::ManualObjectMeshGenAdapter *__fastcall(RBX::ManualObjectMeshGenAdapter *this, Ogre::ManualObject *)
#[doc(alias = "__ZN3RBX26ManualObjectMeshGenAdapterC2EPN4Ogre12ManualObjectE")]
#[doc(alias = "RBX::ManualObjectMeshGenAdapter::ManualObjectMeshGenAdapter(Ogre::ManualObject *)")]
pub fn stub_b9a284() -> ! {
    todo!("0xb9a284 __ZN3RBX26ManualObjectMeshGenAdapterC2EPN4Ogre12ManualObjectE")
}

// 0xb9a920 — __ZN4Ogre9SharedPtrINS_4MeshEED0Ev — Ogre::SharedPtr<Ogre::Mesh>::~SharedPtr()
// type: void __fastcall(void *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_4MeshEED0Ev")]
#[doc(alias = "Ogre::SharedPtr<Ogre::Mesh>::~SharedPtr()")]
pub fn stub_b9a920() -> ! {
    todo!("0xb9a920 __ZN4Ogre9SharedPtrINS_4MeshEED0Ev")
}

// 0xb9a9c0 — __ZN4Ogre9SharedPtrINS_4MeshEE7destroyEv — Ogre::SharedPtr<Ogre::Mesh>::destroy(void)
// type: int __fastcall(int, void *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_4MeshEE7destroyEv")]
#[doc(alias = "Ogre::SharedPtr<Ogre::Mesh>::destroy(void)")]
pub fn stub_b9a9c0() -> ! {
    todo!("0xb9a9c0 __ZN4Ogre9SharedPtrINS_4MeshEE7destroyEv")
}

// 0xb9a9f8 — __ZN4Ogre9SharedPtrINS_4MeshEE4swapERS2_ — Ogre::SharedPtr<Ogre::Mesh>::swap(Ogre::SharedPtr<Ogre::Mesh>&)
// type: _DWORD *__fastcall(_DWORD *result, _DWORD *)
#[doc(alias = "__ZN4Ogre9SharedPtrINS_4MeshEE4swapERS2_")]
#[doc(alias = "Ogre::SharedPtr<Ogre::Mesh>::swap(Ogre::SharedPtr<Ogre::Mesh>&)")]
pub fn stub_b9a9f8() -> ! {
    todo!("0xb9a9f8 __ZN4Ogre9SharedPtrINS_4MeshEE4swapERS2_")
}

// 0xb9aa2c — __ZN4Ogre4Node11setListenerEPNS0_8ListenerE — Ogre::Node::setListener(Ogre::Node::Listener *)
// type: int __fastcall(int result, int)
#[doc(alias = "__ZN4Ogre4Node11setListenerEPNS0_8ListenerE")]
#[doc(alias = "Ogre::Node::setListener(Ogre::Node::Listener *)")]
pub fn stub_b9aa2c() -> ! {
    todo!("0xb9aa2c __ZN4Ogre4Node11setListenerEPNS0_8ListenerE")
}

// 0xb9aa34 — __ZNK4Ogre4Node11getListenerEv — Ogre::Node::getListener(void)const
// type: int __fastcall(Ogre::Node *this)
#[doc(alias = "__ZNK4Ogre4Node11getListenerEv")]
#[doc(alias = "Ogre::Node::getListener(void)const")]
pub fn stub_b9aa34() -> ! {
    todo!("0xb9aa34 __ZNK4Ogre4Node11getListenerEv")
}

// 0xb9aa3c — __ZN4Ogre4Node10setUserAnyERKNS_3AnyE — Ogre::Node::setUserAny(Ogre::Any const&)
// type: void __fastcall(Ogre::Node *this, const Ogre::Any *)
#[doc(alias = "__ZN4Ogre4Node10setUserAnyERKNS_3AnyE")]
#[doc(alias = "Ogre::Node::setUserAny(Ogre::Any const&)")]
pub fn stub_b9aa3c() -> ! {
    todo!("0xb9aa3c __ZN4Ogre4Node10setUserAnyERKNS_3AnyE")
}

// 0xb9aa44 — __ZNK4Ogre4Node10getUserAnyEv — Ogre::Node::getUserAny(void)const
// type: _DWORD *__fastcall(Ogre::Node *this)
#[doc(alias = "__ZNK4Ogre4Node10getUserAnyEv")]
#[doc(alias = "Ogre::Node::getUserAny(void)const")]
pub fn stub_b9aa44() -> ! {
    todo!("0xb9aa44 __ZNK4Ogre4Node10getUserAnyEv")
}

// 0xb9aa4c — __ZNK4Ogre9SceneNode14isInSceneGraphEv — Ogre::SceneNode::isInSceneGraph(void)const
// type: int __fastcall(Ogre::SceneNode *this)
#[doc(alias = "__ZNK4Ogre9SceneNode14isInSceneGraphEv")]
#[doc(alias = "Ogre::SceneNode::isInSceneGraph(void)const")]
pub fn stub_b9aa4c() -> ! {
    todo!("0xb9aa4c __ZNK4Ogre9SceneNode14isInSceneGraphEv")
}

// 0xb9aa54 — __ZN4Ogre9SceneNode15_notifyRootNodeEv — Ogre::SceneNode::_notifyRootNode(void)
// type: int __fastcall(int this)
#[doc(alias = "__ZN4Ogre9SceneNode15_notifyRootNodeEv")]
#[doc(alias = "Ogre::SceneNode::_notifyRootNode(void)")]
pub fn stub_b9aa54() -> ! {
    todo!("0xb9aa54 __ZN4Ogre9SceneNode15_notifyRootNodeEv")
}

// 0xb9aa5c — __ZN4Ogre9SceneNode18getAutoTrackTargetEv — Ogre::SceneNode::getAutoTrackTarget(void)
// type: int __fastcall(Ogre::SceneNode *this)
#[doc(alias = "__ZN4Ogre9SceneNode18getAutoTrackTargetEv")]
#[doc(alias = "Ogre::SceneNode::getAutoTrackTarget(void)")]
pub fn stub_b9aa5c() -> ! {
    todo!("0xb9aa5c __ZN4Ogre9SceneNode18getAutoTrackTargetEv")
}

// 0xb9aa64 — __ZN4Ogre9SceneNode18getAutoTrackOffsetEv — Ogre::SceneNode::getAutoTrackOffset(void)
// type: char *__fastcall(Ogre::SceneNode *this)
#[doc(alias = "__ZN4Ogre9SceneNode18getAutoTrackOffsetEv")]
#[doc(alias = "Ogre::SceneNode::getAutoTrackOffset(void)")]
pub fn stub_b9aa64() -> ! {
    todo!("0xb9aa64 __ZN4Ogre9SceneNode18getAutoTrackOffsetEv")
}

// 0xb9aa6c — __ZN4Ogre9SceneNode26getAutoTrackLocalDirectionEv — Ogre::SceneNode::getAutoTrackLocalDirection(void)
// type: char *__fastcall(Ogre::SceneNode *this)
#[doc(alias = "__ZN4Ogre9SceneNode26getAutoTrackLocalDirectionEv")]
#[doc(alias = "Ogre::SceneNode::getAutoTrackLocalDirection(void)")]
pub fn stub_b9aa6c() -> ! {
    todo!("0xb9aa6c __ZN4Ogre9SceneNode26getAutoTrackLocalDirectionEv")
}

// 0xb9b3bc — __ZN4Ogre17istreamDataStreamC1EPSib — Ogre::istreamDataStream::istreamDataStream(std::istream *,bool)
// type: Ogre::istreamDataStream *__fastcall(Ogre::istreamDataStream *this, std::istream *, bool)
#[doc(alias = "__ZN4Ogre17istreamDataStreamC1EPSib")]
#[doc(alias = "Ogre::istreamDataStream::istreamDataStream(std::istream *,bool)")]
pub fn stub_b9b3bc() -> ! {
    todo!("0xb9b3bc __ZN4Ogre17istreamDataStreamC1EPSib")
}

// 0xb9b52c — __ZN4Ogre17istreamDataStreamD0Ev — Ogre::istreamDataStream::~istreamDataStream()
// type: void __fastcall(Ogre::istreamDataStream *__hidden this)
#[doc(alias = "__ZN4Ogre17istreamDataStreamD0Ev")]
#[doc(alias = "Ogre::istreamDataStream::~istreamDataStream()")]
pub fn stub_b9b52c() -> ! {
    todo!("0xb9b52c __ZN4Ogre17istreamDataStreamD0Ev")
}

// 0xb9b5e0 — __ZN4Ogre17istreamDataStreamD1Ev — Ogre::istreamDataStream::~istreamDataStream()
// type: void __fastcall(Ogre::istreamDataStream *__hidden this)
#[doc(alias = "__ZN4Ogre17istreamDataStreamD1Ev")]
#[doc(alias = "Ogre::istreamDataStream::~istreamDataStream()")]
pub fn stub_b9b5e0() -> ! {
    todo!("0xb9b5e0 __ZN4Ogre17istreamDataStreamD1Ev")
}

// 0xb9b5e4 — __ZN4Ogre17istreamDataStreamD2Ev — Ogre::istreamDataStream::~istreamDataStream()
// type: void __fastcall(Ogre::istreamDataStream *__hidden this)
#[doc(alias = "__ZN4Ogre17istreamDataStreamD2Ev")]
#[doc(alias = "Ogre::istreamDataStream::~istreamDataStream()")]
pub fn stub_b9b5e4() -> ! {
    todo!("0xb9b5e4 __ZN4Ogre17istreamDataStreamD2Ev")
}

// 0xb9b744 — __ZN4Ogre17istreamDataStream4readEPvm — Ogre::istreamDataStream::read(void *,unsigned long)
// type: int __fastcall(std::istream **this, char *, int)
#[doc(alias = "__ZN4Ogre17istreamDataStream4readEPvm")]
#[doc(alias = "Ogre::istreamDataStream::read(void *,unsigned long)")]
pub fn stub_b9b744() -> ! {
    todo!("0xb9b744 __ZN4Ogre17istreamDataStream4readEPvm")
}

// 0xb9b758 — __ZN4Ogre17istreamDataStream8readLineEPcmRKSs — Ogre::istreamDataStream::readLine(char *,unsigned long,std::string const&)
// type: unsigned int __fastcall(std::istream **this, char *, unsigned int, char **)
#[doc(alias = "__ZN4Ogre17istreamDataStream8readLineEPcmRKSs")]
#[doc(alias = "Ogre::istreamDataStream::readLine(char *,unsigned long,std::string const&)")]
pub fn stub_b9b758() -> ! {
    todo!("0xb9b758 __ZN4Ogre17istreamDataStream8readLineEPcmRKSs")
}

// 0xb9bbd0 — __ZN4Ogre17istreamDataStream4skipEl — Ogre::istreamDataStream::skip(long)
// type: int __fastcall(Ogre::istreamDataStream *this, int)
#[doc(alias = "__ZN4Ogre17istreamDataStream4skipEl")]
#[doc(alias = "Ogre::istreamDataStream::skip(long)")]
pub fn stub_b9bbd0() -> ! {
    todo!("0xb9bbd0 __ZN4Ogre17istreamDataStream4skipEl")
}

// 0xb9bbf8 — __ZN4Ogre17istreamDataStream4seekEm — Ogre::istreamDataStream::seek(unsigned long)
// type: int __fastcall(Ogre::istreamDataStream *this, int)
#[doc(alias = "__ZN4Ogre17istreamDataStream4seekEm")]
#[doc(alias = "Ogre::istreamDataStream::seek(unsigned long)")]
pub fn stub_b9bbf8() -> ! {
    todo!("0xb9bbf8 __ZN4Ogre17istreamDataStream4seekEm")
}

// 0xb9bc20 — __ZNK4Ogre17istreamDataStream4tellEv — Ogre::istreamDataStream::tell(void)const
// type: int __fastcall(Ogre::istreamDataStream *this)
#[doc(alias = "__ZNK4Ogre17istreamDataStream4tellEv")]
#[doc(alias = "Ogre::istreamDataStream::tell(void)const")]
pub fn stub_b9bc20() -> ! {
    todo!("0xb9bc20 __ZNK4Ogre17istreamDataStream4tellEv")
}

// 0xb9bc64 — __ZNK4Ogre17istreamDataStream3eofEv — Ogre::istreamDataStream::eof(void)const
// type: int __fastcall(Ogre::istreamDataStream *this)
#[doc(alias = "__ZNK4Ogre17istreamDataStream3eofEv")]
#[doc(alias = "Ogre::istreamDataStream::eof(void)const")]
pub fn stub_b9bc64() -> ! {
    todo!("0xb9bc64 __ZNK4Ogre17istreamDataStream3eofEv")
}

// 0xb9bc78 — __ZN4Ogre17istreamDataStream5closeEv — Ogre::istreamDataStream::close(void)
// type: int __fastcall(Ogre::istreamDataStream *this)
#[doc(alias = "__ZN4Ogre17istreamDataStream5closeEv")]
#[doc(alias = "Ogre::istreamDataStream::close(void)")]
pub fn stub_b9bc78() -> ! {
    todo!("0xb9bc78 __ZN4Ogre17istreamDataStream5closeEv")
}

// 0xb9c320 — __ZN4Ogre7QuadricC1Ev — Ogre::Quadric::Quadric(void)
// type: int __fastcall(int this)
#[doc(alias = "__ZN4Ogre7QuadricC1Ev")]
#[doc(alias = "Ogre::Quadric::Quadric(void)")]
pub fn stub_b9c320() -> ! {
    todo!("0xb9c320 __ZN4Ogre7QuadricC1Ev")
}

// 0xb9c344 — __ZN4Ogre7Quadric9setOriginERKNS_7Vector3E — Ogre::Quadric::setOrigin(Ogre::Vector3 const&)
// type: int __fastcall(int this, const Vector3 *)
#[doc(alias = "__ZN4Ogre7Quadric9setOriginERKNS_7Vector3E")]
#[doc(alias = "Ogre::Quadric::setOrigin(Ogre::Vector3 const&)")]
pub fn stub_b9c344() -> ! {
    todo!("0xb9c344 __ZN4Ogre7Quadric9setOriginERKNS_7Vector3E")
}

// 0xb9c358 — __ZN4Ogre7Quadric14createCylinderEPNS_12SceneManagerERKSsPNS_12ManualObjectEfffii — Ogre::Quadric::createCylinder(Ogre::SceneManager *,std::string const&,Ogre::ManualObject *,float,float,float,int,int)
// type: Ogre::ManualObject *__fastcall(Ogre::Quadric *this, Ogre::SceneManager *, const std::string *, Ogre::ManualObject *, float32_t, float32_t, float32_t, int, int)
#[doc(alias = "__ZN4Ogre7Quadric14createCylinderEPNS_12SceneManagerERKSsPNS_12ManualObjectEfffii")]
#[doc(alias = "Ogre::Quadric::createCylinder(Ogre::SceneManager *,std::string const&,Ogre::ManualObject *,float,float,float,int,int)")]
pub fn stub_b9c358() -> ! {
    todo!("0xb9c358 __ZN4Ogre7Quadric14createCylinderEPNS_12SceneManagerERKSsPNS_12ManualObjectEfffii")
}

// 0xb9e7e8 — __ZN4Ogre7Quadric10createDiskEPNS_12SceneManagerERKSsPNS_12ManualObjectEffii — Ogre::Quadric::createDisk(Ogre::SceneManager *,std::string const&,Ogre::ManualObject *,float,float,int,int)
// type: int __fastcall(Ogre::Quadric *this, Ogre::SceneManager *, const std::string *, Ogre::ManualObject *, float, float, int, int)
#[doc(alias = "__ZN4Ogre7Quadric10createDiskEPNS_12SceneManagerERKSsPNS_12ManualObjectEffii")]
#[doc(alias = "Ogre::Quadric::createDisk(Ogre::SceneManager *,std::string const&,Ogre::ManualObject *,float,float,int,int)")]
pub fn stub_b9e7e8() -> ! {
    todo!("0xb9e7e8 __ZN4Ogre7Quadric10createDiskEPNS_12SceneManagerERKSsPNS_12ManualObjectEffii")
}

// 0xb9e830 — __ZN4Ogre7Quadric17createPartialDiskEPNS_12SceneManagerERKSsPNS_12ManualObjectEffiiff — Ogre::Quadric::createPartialDisk(Ogre::SceneManager *,std::string const&,Ogre::ManualObject *,float,float,int,int,float,float)
// type: Ogre::ManualObject *__fastcall(Ogre::Quadric *this, Ogre::SceneManager *, const std::string *, Ogre::ManualObject *, float32_t, float32_t, int, int, float32_t, float32_t)
#[doc(alias = "__ZN4Ogre7Quadric17createPartialDiskEPNS_12SceneManagerERKSsPNS_12ManualObjectEffiiff")]
#[doc(alias = "Ogre::Quadric::createPartialDisk(Ogre::SceneManager *,std::string const&,Ogre::ManualObject *,float,float,int,int,float,float)")]
pub fn stub_b9e830() -> ! {
    todo!("0xb9e830 __ZN4Ogre7Quadric17createPartialDiskEPNS_12SceneManagerERKSsPNS_12ManualObjectEffiiff")
}

// 0xba0b70 — __ZN4Ogre7Quadric12createSphereEPNS_12SceneManagerERKSsPNS_12ManualObjectEfii — Ogre::Quadric::createSphere(Ogre::SceneManager *,std::string const&,Ogre::ManualObject *,float,int,int)
// type: Ogre::ManualObject *__fastcall(Ogre::Quadric *this, Ogre::SceneManager *, const std::string *, Ogre::ManualObject *, float32_t, int, int)
#[doc(alias = "__ZN4Ogre7Quadric12createSphereEPNS_12SceneManagerERKSsPNS_12ManualObjectEfii")]
#[doc(alias = "Ogre::Quadric::createSphere(Ogre::SceneManager *,std::string const&,Ogre::ManualObject *,float,int,int)")]
pub fn stub_ba0b70() -> ! {
    todo!("0xba0b70 __ZN4Ogre7Quadric12createSphereEPNS_12SceneManagerERKSsPNS_12ManualObjectEfii")
}

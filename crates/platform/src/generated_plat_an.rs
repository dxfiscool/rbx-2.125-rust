//! platform — generated_plat_an — 120 stubs EA-sorted asc global gap filler | Source ida/export.json | Distinct 28060->28180 | range 0x106428..0x10e0fc | rbx_core::SharedPtr not boost | ObjC doc aliases
//! Source: ida/export.json (85545 funcs) global gap filler next 120 EA-sorted asc not yet stubbed in platform
//! Distinct stub_ 28060/85545 -> 28180/85545 | uncovered 57485 -> 57365 (platform)
//! Batch: 120 stubs | range 0x106428..0x10e0fc | rbx_core::SharedPtr not boost

#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]

use rbx_core::SharedPtr;

const _: () = {
    let _ = core::marker::PhantomData::<SharedPtr<u8>>;
};

// 0x106428 — __ZN4FMOD10DSPTremolo12readCallbackEP14FMOD_DSP_STATEPfS3_jii / FMOD::DSPTremolo::readCallback(FMOD_DSP_STATE *,float *,float *,unsigned int,int,int)
// type: (unknown)
#[doc(alias = "FMOD::DSPTremolo::readCallback(FMOD_DSP_STATE *,float *,float *,unsigned int,int,int)")]
pub fn stub_106428() -> ! {
    todo!("0x106428 FMOD::DSPTremolo::readCallback(FMOD_DSP_STATE *,float *,float *,unsigned int,int,int)")
}

// 0x106450 — __Z41__static_initialization_and_destruction_0ii_40
// type: _DWORD __fastcall(int, int)
#[doc(alias = "__Z41__static_initialization_and_destruction_0ii_40")]
pub fn stub_106450() -> ! {
    todo!("0x106450 __Z41__static_initialization_and_destruction_0ii_40")
}

// 0x106494 — __GLOBAL__I__ZN4FMOD15dsptremolo_descE / global constructor keyed to FMOD::dsptremolo_desc
// type: (unknown)
#[doc(alias = "global constructor keyed to FMOD::dsptremolo_desc")]
pub fn stub_106494() -> ! {
    todo!("0x106494 global constructor keyed to FMOD::dsptremolo_desc")
}

// 0x1064a0 — __ZN4FMOD17HistoryBufferPoolC2Ev / FMOD::HistoryBufferPool::HistoryBufferPool(void)
// type: _DWORD __fastcall(FMOD::HistoryBufferPool *__hidden this)
#[doc(alias = "FMOD::HistoryBufferPool::HistoryBufferPool(void)")]
pub fn stub_1064a0() -> ! {
    todo!("0x1064a0 FMOD::HistoryBufferPool::HistoryBufferPool(void)")
}

// 0x1064cc — __ZN4FMOD17HistoryBufferPoolC1Ev / FMOD::HistoryBufferPool::HistoryBufferPool(void)
// type: _DWORD __fastcall(FMOD::HistoryBufferPool *__hidden this)
#[doc(alias = "FMOD::HistoryBufferPool::HistoryBufferPool(void)")]
pub fn stub_1064cc() -> ! {
    todo!("0x1064cc FMOD::HistoryBufferPool::HistoryBufferPool(void)")
}

// 0x1064d0 — __ZN4FMOD17HistoryBufferPool17getMemoryUsedImplEPNS_13MemoryTrackerE / FMOD::HistoryBufferPool::getMemoryUsedImpl(FMOD::MemoryTracker *)
// type: _DWORD __fastcall(FMOD::HistoryBufferPool *__hidden this, FMOD::MemoryTracker *)
#[doc(alias = "FMOD::HistoryBufferPool::getMemoryUsedImpl(FMOD::MemoryTracker *)")]
pub fn stub_1064d0() -> ! {
    todo!("0x1064d0 FMOD::HistoryBufferPool::getMemoryUsedImpl(FMOD::MemoryTracker *)")
}

// 0x106528 — __ZN4FMOD17HistoryBufferPool7releaseEv / FMOD::HistoryBufferPool::release(void)
// type: _DWORD __fastcall(FMOD::HistoryBufferPool *__hidden this)
#[doc(alias = "FMOD::HistoryBufferPool::release(void)")]
pub fn stub_106528() -> ! {
    todo!("0x106528 FMOD::HistoryBufferPool::release(void)")
}

// 0x1065b4 — __ZN4FMOD17HistoryBufferPool4freeEPf / FMOD::HistoryBufferPool::free(float *)
// type: _DWORD __fastcall(FMOD::HistoryBufferPool *__hidden this, float *)
#[doc(alias = "FMOD::HistoryBufferPool::free(float *)")]
pub fn stub_1065b4() -> ! {
    todo!("0x1065b4 FMOD::HistoryBufferPool::free(float *)")
}

// 0x1066bc — __ZN4FMOD17HistoryBufferPool5allocEPPfi / FMOD::HistoryBufferPool::alloc(float **,int)
// type: _DWORD __fastcall(FMOD::HistoryBufferPool *__hidden this, float **, int)
#[doc(alias = "FMOD::HistoryBufferPool::alloc(float **,int)")]
pub fn stub_1066bc() -> ! {
    todo!("0x1066bc FMOD::HistoryBufferPool::alloc(float **,int)")
}

// 0x106868 — __ZN4FMOD17HistoryBufferPool4initEii / FMOD::HistoryBufferPool::init(int,int)
// type: _DWORD __fastcall(FMOD::HistoryBufferPool *__hidden this, int, int)
#[doc(alias = "FMOD::HistoryBufferPool::init(int,int)")]
pub fn stub_106868() -> ! {
    todo!("0x106868 FMOD::HistoryBufferPool::init(int,int)")
}

// 0x106974 — __ZN4FMOD17HistoryBufferPool13getMemoryUsedEPNS_13MemoryTrackerE / FMOD::HistoryBufferPool::getMemoryUsed(FMOD::MemoryTracker *)
// type: _DWORD __fastcall(FMOD::HistoryBufferPool *__hidden this, FMOD::MemoryTracker *)
#[doc(alias = "FMOD::HistoryBufferPool::getMemoryUsed(FMOD::MemoryTracker *)")]
pub fn stub_106974() -> ! {
    todo!("0x106974 FMOD::HistoryBufferPool::getMemoryUsed(FMOD::MemoryTracker *)")
}

// 0x1069cc — __ZN4FMOD15CodecAudioQueue17getMemoryUsedImplEPNS_13MemoryTrackerE / FMOD::CodecAudioQueue::getMemoryUsedImpl(FMOD::MemoryTracker *)
// type: _DWORD __fastcall(FMOD::CodecAudioQueue *__hidden this, FMOD::MemoryTracker *)
#[doc(alias = "FMOD::CodecAudioQueue::getMemoryUsedImpl(FMOD::MemoryTracker *)")]
pub fn stub_1069cc() -> ! {
    todo!("0x1069cc FMOD::CodecAudioQueue::getMemoryUsedImpl(FMOD::MemoryTracker *)")
}

// 0x1069fc — __ZN4FMOD15CodecAudioQueue21getMemoryUsedCallbackEP16FMOD_CODEC_STATEPNS_13MemoryTrackerE / FMOD::CodecAudioQueue::getMemoryUsedCallback(FMOD_CODEC_STATE *,FMOD::MemoryTracker *)
// type: int __fastcall(FMOD::CodecAudioQueue *this)
#[doc(alias = "FMOD::CodecAudioQueue::getMemoryUsedCallback(FMOD_CODEC_STATE *,FMOD::MemoryTracker *)")]
pub fn stub_1069fc() -> ! {
    todo!("0x1069fc FMOD::CodecAudioQueue::getMemoryUsedCallback(FMOD_CODEC_STATE *,FMOD::MemoryTracker *)")
}

// 0x106a54 — __ZN4FMOD15CodecAudioQueue11fileGetSizeEPx / FMOD::CodecAudioQueue::fileGetSize(long long *)
// type: _DWORD __fastcall(FMOD::CodecAudioQueue *__hidden this, __int64 *)
#[doc(alias = "FMOD::CodecAudioQueue::fileGetSize(long long *)")]
pub fn stub_106a54() -> ! {
    todo!("0x106a54 FMOD::CodecAudioQueue::fileGetSize(long long *)")
}

// 0x106ad0 — __ZN4FMOD15CodecAudioQueue19fileGetSizeCallbackEPv / FMOD::CodecAudioQueue::fileGetSizeCallback(void *)
// type: _DWORD __fastcall(FMOD::CodecAudioQueue *__hidden this, void *)
#[doc(alias = "FMOD::CodecAudioQueue::fileGetSizeCallback(void *)")]
pub fn stub_106ad0() -> ! {
    todo!("0x106ad0 FMOD::CodecAudioQueue::fileGetSizeCallback(void *)")
}

// 0x106afc — __ZN4FMOD15CodecAudioQueue8fileReadExmPvPm / FMOD::CodecAudioQueue::fileRead(long long,unsigned long,void *,unsigned long *)
// type: _DWORD __fastcall(FMOD::CodecAudioQueue *__hidden this, __int64, unsigned int, void *, unsigned int *)
#[doc(alias = "FMOD::CodecAudioQueue::fileRead(long long,unsigned long,void *,unsigned long *)")]
pub fn stub_106afc() -> ! {
    todo!("0x106afc FMOD::CodecAudioQueue::fileRead(long long,unsigned long,void *,unsigned long *)")
}

// 0x106c1c — __ZN4FMOD15CodecAudioQueue16fileReadCallbackEPvxmS1_Pm / FMOD::CodecAudioQueue::fileReadCallback(void *,long long,unsigned long,void *,unsigned long *)
// type: _DWORD __fastcall(FMOD::CodecAudioQueue *__hidden this, void *, __int64, unsigned int, void *, unsigned int *)
#[doc(alias = "FMOD::CodecAudioQueue::fileReadCallback(void *,long long,unsigned long,void *,unsigned long *)")]
pub fn stub_106c1c() -> ! {
    todo!("0x106c1c FMOD::CodecAudioQueue::fileReadCallback(void *,long long,unsigned long,void *,unsigned long *)")
}

// 0x106c20 — __ZN4FMOD15CodecAudioQueue17processAudioQueueEP16OpaqueAudioQueueP16AudioQueueBuffer / FMOD::CodecAudioQueue::processAudioQueue(OpaqueAudioQueue *,AudioQueueBuffer *)
// type: _DWORD __fastcall(FMOD::CodecAudioQueue *__hidden this, OpaqueAudioQueue *, AudioQueueBuffer *)
#[doc(alias = "FMOD::CodecAudioQueue::processAudioQueue(OpaqueAudioQueue *,AudioQueueBuffer *)")]
pub fn stub_106c20() -> ! {
    todo!("0x106c20 FMOD::CodecAudioQueue::processAudioQueue(OpaqueAudioQueue *,AudioQueueBuffer *)")
}

// 0x106cf8 — __ZN4FMOD15CodecAudioQueue24audioQueueOutputCallbackEPvP16OpaqueAudioQueueP16AudioQueueBuffer / FMOD::CodecAudioQueue::audioQueueOutputCallback(void *,OpaqueAudioQueue *,AudioQueueBuffer *)
// type: _DWORD __fastcall(FMOD::CodecAudioQueue *__hidden this, void *, OpaqueAudioQueue *, AudioQueueBuffer *)
#[doc(alias = "FMOD::CodecAudioQueue::audioQueueOutputCallback(void *,OpaqueAudioQueue *,AudioQueueBuffer *)")]
pub fn stub_106cf8() -> ! {
    todo!("0x106cf8 FMOD::CodecAudioQueue::audioQueueOutputCallback(void *,OpaqueAudioQueue *,AudioQueueBuffer *)")
}

// 0x106cfc — __ZN4FMOD15CodecAudioQueue14setupAudioFileEb / FMOD::CodecAudioQueue::setupAudioFile(bool)
// type: _DWORD __fastcall(FMOD::CodecAudioQueue *__hidden this, bool)
#[doc(alias = "FMOD::CodecAudioQueue::setupAudioFile(bool)")]
pub fn stub_106cfc() -> ! {
    todo!("0x106cfc FMOD::CodecAudioQueue::setupAudioFile(bool)")
}

// 0x106eac — __ZN4FMOD15CodecAudioQueue19setPositionInternalEijj / FMOD::CodecAudioQueue::setPositionInternal(int,unsigned int,unsigned int)
// type: _DWORD __fastcall(FMOD::CodecAudioQueue *__hidden this, int, unsigned int, unsigned int)
#[doc(alias = "FMOD::CodecAudioQueue::setPositionInternal(int,unsigned int,unsigned int)")]
pub fn stub_106eac() -> ! {
    todo!("0x106eac FMOD::CodecAudioQueue::setPositionInternal(int,unsigned int,unsigned int)")
}

// 0x107090 — __ZN4FMOD15CodecAudioQueue19setPositionCallbackEP16FMOD_CODEC_STATEijj / FMOD::CodecAudioQueue::setPositionCallback(FMOD_CODEC_STATE *,int,unsigned int,unsigned int)
// type: (unknown)
#[doc(alias = "FMOD::CodecAudioQueue::setPositionCallback(FMOD_CODEC_STATE *,int,unsigned int,unsigned int)")]
pub fn stub_107090() -> ! {
    todo!("0x107090 FMOD::CodecAudioQueue::setPositionCallback(FMOD_CODEC_STATE *,int,unsigned int,unsigned int)")
}

// 0x10709c — __ZN4FMOD15CodecAudioQueue13closeInternalEv / FMOD::CodecAudioQueue::closeInternal(void)
// type: _DWORD __fastcall(FMOD::CodecAudioQueue *__hidden this)
#[doc(alias = "FMOD::CodecAudioQueue::closeInternal(void)")]
pub fn stub_10709c() -> ! {
    todo!("0x10709c FMOD::CodecAudioQueue::closeInternal(void)")
}

// 0x107164 — __ZN4FMOD15CodecAudioQueue13closeCallbackEP16FMOD_CODEC_STATE / FMOD::CodecAudioQueue::closeCallback(FMOD_CODEC_STATE *)
// type: (unknown)
#[doc(alias = "FMOD::CodecAudioQueue::closeCallback(FMOD_CODEC_STATE *)")]
pub fn stub_107164() -> ! {
    todo!("0x107164 FMOD::CodecAudioQueue::closeCallback(FMOD_CODEC_STATE *)")
}

// 0x107170 — __ZN4FMOD15CodecAudioQueue16getDescriptionExEv / FMOD::CodecAudioQueue::getDescriptionEx(void)
// type: _DWORD __fastcall(FMOD::CodecAudioQueue *__hidden this)
#[doc(alias = "FMOD::CodecAudioQueue::getDescriptionEx(void)")]
pub fn stub_107170() -> ! {
    todo!("0x107170 FMOD::CodecAudioQueue::getDescriptionEx(void)")
}

// 0x107284 — __ZN4FMOD15CodecAudioQueue15setupAudioQueueEv / FMOD::CodecAudioQueue::setupAudioQueue(void)
// type: _DWORD __fastcall(FMOD::CodecAudioQueue *__hidden this)
#[doc(alias = "FMOD::CodecAudioQueue::setupAudioQueue(void)")]
pub fn stub_107284() -> ! {
    todo!("0x107284 FMOD::CodecAudioQueue::setupAudioQueue(void)")
}

// 0x107598 — __ZN4FMOD15CodecAudioQueue12readInternalEPvjPj / FMOD::CodecAudioQueue::readInternal(void *,unsigned int,unsigned int *)
// type: _DWORD __fastcall(FMOD::CodecAudioQueue *__hidden this, void *, unsigned int, unsigned int *)
#[doc(alias = "FMOD::CodecAudioQueue::readInternal(void *,unsigned int,unsigned int *)")]
pub fn stub_107598() -> ! {
    todo!("0x107598 FMOD::CodecAudioQueue::readInternal(void *,unsigned int,unsigned int *)")
}

// 0x10773c — __ZN4FMOD15CodecAudioQueue12readCallbackEP16FMOD_CODEC_STATEPvjPj / FMOD::CodecAudioQueue::readCallback(FMOD_CODEC_STATE *,void *,unsigned int,unsigned int *)
// type: (unknown)
#[doc(alias = "FMOD::CodecAudioQueue::readCallback(FMOD_CODEC_STATE *,void *,unsigned int,unsigned int *)")]
pub fn stub_10773c() -> ! {
    todo!("0x10773c FMOD::CodecAudioQueue::readCallback(FMOD_CODEC_STATE *,void *,unsigned int,unsigned int *)")
}

// 0x107748 — __ZN4FMOD15CodecAudioQueue12openInternalEjP22FMOD_CREATESOUNDEXINFO / FMOD::CodecAudioQueue::openInternal(unsigned int,FMOD_CREATESOUNDEXINFO *)
// type: (unknown)
#[doc(alias = "FMOD::CodecAudioQueue::openInternal(unsigned int,FMOD_CREATESOUNDEXINFO *)")]
pub fn stub_107748() -> ! {
    todo!("0x107748 FMOD::CodecAudioQueue::openInternal(unsigned int,FMOD_CREATESOUNDEXINFO *)")
}

// 0x1078d4 — __ZN4FMOD15CodecAudioQueue12openCallbackEP16FMOD_CODEC_STATEjP22FMOD_CREATESOUNDEXINFO / FMOD::CodecAudioQueue::openCallback(FMOD_CODEC_STATE *,unsigned int,FMOD_CREATESOUNDEXINFO *)
// type: (unknown)
#[doc(alias = "FMOD::CodecAudioQueue::openCallback(FMOD_CODEC_STATE *,unsigned int,FMOD_CREATESOUNDEXINFO *)")]
pub fn stub_1078d4() -> ! {
    todo!("0x1078d4 FMOD::CodecAudioQueue::openCallback(FMOD_CODEC_STATE *,unsigned int,FMOD_CREATESOUNDEXINFO *)")
}

// 0x1078e0 — __ZN4FMOD15CodecAudioQueue8resetAllEbb / FMOD::CodecAudioQueue::resetAll(bool,bool)
// type: _DWORD __fastcall(FMOD::CodecAudioQueue *__hidden this, bool, bool)
#[doc(alias = "FMOD::CodecAudioQueue::resetAll(bool,bool)")]
pub fn stub_1078e0() -> ! {
    todo!("0x1078e0 FMOD::CodecAudioQueue::resetAll(bool,bool)")
}

// 0x1079e8 — __Z41__static_initialization_and_destruction_0ii_41
// type: _DWORD __fastcall(int, int)
#[doc(alias = "__Z41__static_initialization_and_destruction_0ii_41")]
pub fn stub_1079e8() -> ! {
    todo!("0x1079e8 __Z41__static_initialization_and_destruction_0ii_41")
}

// 0x107a1c — __GLOBAL__I__ZN4FMOD15CodecAudioQueue10gCodecHeadE / global constructor keyed to FMOD::CodecAudioQueue::gCodecHead
// type: (unknown)
#[doc(alias = "global constructor keyed to FMOD::CodecAudioQueue::gCodecHead")]
pub fn stub_107a1c() -> ! {
    todo!("0x107a1c global constructor keyed to FMOD::CodecAudioQueue::gCodecHead")
}

// 0x107a28 — __ZL22FreeImage_GetImageSizeiii / FreeImage_GetImageSize(int,int,int)
// type: _DWORD __fastcall(int, int, int)
#[doc(alias = "FreeImage_GetImageSize(int,int,int)")]
pub fn stub_107a28() -> ! {
    todo!("0x107a28 FreeImage_GetImageSize(int,int,int)")
}

// 0x107a78 — _FreeImage_GetImageType
// type: (unknown)
#[doc(alias = "_FreeImage_GetImageType")]
pub fn stub_107a78() -> ! {
    todo!("0x107a78 _FreeImage_GetImageType")
}

// 0x107a88 — _FreeImage_GetRedMask
// type: (unknown)
#[doc(alias = "_FreeImage_GetRedMask")]
pub fn stub_107a88() -> ! {
    todo!("0x107a88 _FreeImage_GetRedMask")
}

// 0x107a98 — _FreeImage_GetGreenMask
// type: (unknown)
#[doc(alias = "_FreeImage_GetGreenMask")]
pub fn stub_107a98() -> ! {
    todo!("0x107a98 _FreeImage_GetGreenMask")
}

// 0x107aa8 — _FreeImage_GetBlueMask
// type: (unknown)
#[doc(alias = "_FreeImage_GetBlueMask")]
pub fn stub_107aa8() -> ! {
    todo!("0x107aa8 _FreeImage_GetBlueMask")
}

// 0x107ab8 — _FreeImage_HasBackgroundColor
// type: (unknown)
#[doc(alias = "_FreeImage_HasBackgroundColor")]
pub fn stub_107ab8() -> ! {
    todo!("0x107ab8 _FreeImage_HasBackgroundColor")
}

// 0x107ad4 — _FreeImage_GetTransparencyTable
// type: (unknown)
#[doc(alias = "_FreeImage_GetTransparencyTable")]
pub fn stub_107ad4() -> ! {
    todo!("0x107ad4 _FreeImage_GetTransparencyTable")
}

// 0x107ae4 — _FreeImage_GetTransparencyCount
// type: int __fastcall(int result)
#[doc(alias = "_FreeImage_GetTransparencyCount")]
pub fn stub_107ae4() -> ! {
    todo!("0x107ae4 _FreeImage_GetTransparencyCount")
}

// 0x107af4 — _FreeImage_GetICCProfile
// type: (unknown)
#[doc(alias = "_FreeImage_GetICCProfile")]
pub fn stub_107af4() -> ! {
    todo!("0x107af4 _FreeImage_GetICCProfile")
}

// 0x107b04 — _FreeImage_GetInfoHeader
// type: int(void)
#[doc(alias = "_FreeImage_GetInfoHeader")]
pub fn stub_107b04() -> ! {
    todo!("0x107b04 _FreeImage_GetInfoHeader")
}

// 0x107b28 — _FreeImage_SetDotsPerMeterY
// type: (unknown)
#[doc(alias = "_FreeImage_SetDotsPerMeterY")]
pub fn stub_107b28() -> ! {
    todo!("0x107b28 _FreeImage_SetDotsPerMeterY")
}

// 0x107b48 — _FreeImage_SetDotsPerMeterX
// type: (unknown)
#[doc(alias = "_FreeImage_SetDotsPerMeterX")]
pub fn stub_107b48() -> ! {
    todo!("0x107b48 _FreeImage_SetDotsPerMeterX")
}

// 0x107b68 — _FreeImage_GetDotsPerMeterY
// type: (unknown)
#[doc(alias = "_FreeImage_GetDotsPerMeterY")]
pub fn stub_107b68() -> ! {
    todo!("0x107b68 _FreeImage_GetDotsPerMeterY")
}

// 0x107b88 — _FreeImage_GetDotsPerMeterX
// type: (unknown)
#[doc(alias = "_FreeImage_GetDotsPerMeterX")]
pub fn stub_107b88() -> ! {
    todo!("0x107b88 _FreeImage_GetDotsPerMeterX")
}

// 0x107ba8 — _FreeImage_GetColorsUsed
// type: (unknown)
#[doc(alias = "_FreeImage_GetColorsUsed")]
pub fn stub_107ba8() -> ! {
    todo!("0x107ba8 _FreeImage_GetColorsUsed")
}

// 0x107bc8 — _FreeImage_GetBPP
// type: int(void)
#[doc(alias = "_FreeImage_GetBPP")]
pub fn stub_107bc8() -> ! {
    todo!("0x107bc8 _FreeImage_GetBPP")
}

// 0x107be8 — _FreeImage_GetPalette
// type: (unknown)
#[doc(alias = "_FreeImage_GetPalette")]
pub fn stub_107be8() -> ! {
    todo!("0x107be8 _FreeImage_GetPalette")
}

// 0x107c1c — _FreeImage_SetTransparent
// type: (unknown)
#[doc(alias = "_FreeImage_SetTransparent")]
pub fn stub_107c1c() -> ! {
    todo!("0x107c1c _FreeImage_SetTransparent")
}

// 0x107c60 — _FreeImage_GetHeight
// type: int __fastcall(int)
#[doc(alias = "_FreeImage_GetHeight")]
pub fn stub_107c60() -> ! {
    todo!("0x107c60 _FreeImage_GetHeight")
}

// 0x107c80 — _FreeImage_GetWidth
// type: (unknown)
#[doc(alias = "_FreeImage_GetWidth")]
pub fn stub_107c80() -> ! {
    todo!("0x107c80 _FreeImage_GetWidth")
}

// 0x107ca0 — _FreeImage_GetLine
// type: (unknown)
#[doc(alias = "_FreeImage_GetLine")]
pub fn stub_107ca0() -> ! {
    todo!("0x107ca0 _FreeImage_GetLine")
}

// 0x107cd4 — _FreeImage_GetPitch
// type: (unknown)
#[doc(alias = "_FreeImage_GetPitch")]
pub fn stub_107cd4() -> ! {
    todo!("0x107cd4 _FreeImage_GetPitch")
}

// 0x107cf8 — _FreeImage_GetBackgroundColor
// type: (unknown)
#[doc(alias = "_FreeImage_GetBackgroundColor")]
pub fn stub_107cf8() -> ! {
    todo!("0x107cf8 _FreeImage_GetBackgroundColor")
}

// 0x10813c — _FreeImage_FindCloseMetadata
// type: (unknown)
#[doc(alias = "_FreeImage_FindCloseMetadata")]
pub fn stub_10813c() -> ! {
    todo!("0x10813c _FreeImage_FindCloseMetadata")
}

// 0x108168 — _FreeImage_DestroyICCProfile
// type: (unknown)
#[doc(alias = "_FreeImage_DestroyICCProfile")]
pub fn stub_108168() -> ! {
    todo!("0x108168 _FreeImage_DestroyICCProfile")
}

// 0x1081a0 — __Z22FreeImage_Aligned_FreePv / FreeImage_Aligned_Free(void *)
// type: _DWORD __fastcall(void *)
#[doc(alias = "FreeImage_Aligned_Free(void *)")]
pub fn stub_1081a0() -> ! {
    todo!("0x1081a0 FreeImage_Aligned_Free(void *)")
}

// 0x1081b4 — _FreeImage_CreateICCProfile
// type: (unknown)
#[doc(alias = "_FreeImage_CreateICCProfile")]
pub fn stub_1081b4() -> ! {
    todo!("0x1081b4 _FreeImage_CreateICCProfile")
}

// 0x108220 — _FreeImage_SetTransparencyTable
// type: void *__fastcall(void *result, const void *, size_t)
#[doc(alias = "_FreeImage_SetTransparencyTable")]
pub fn stub_108220() -> ! {
    todo!("0x108220 _FreeImage_SetTransparencyTable")
}

// 0x108290 — _FreeImage_SetBackgroundColor
// type: (unknown)
#[doc(alias = "_FreeImage_SetBackgroundColor")]
pub fn stub_108290() -> ! {
    todo!("0x108290 _FreeImage_SetBackgroundColor")
}

// 0x1082dc — _FreeImage_GetColorType
// type: (unknown)
#[doc(alias = "_FreeImage_GetColorType")]
pub fn stub_1082dc() -> ! {
    todo!("0x1082dc _FreeImage_GetColorType")
}

// 0x108858 — _FreeImage_IsTransparent
// type: (unknown)
#[doc(alias = "_FreeImage_IsTransparent")]
pub fn stub_108858() -> ! {
    todo!("0x108858 _FreeImage_IsTransparent")
}

// 0x1088a4 — __Z24FreeImage_Aligned_Mallocmm / FreeImage_Aligned_Malloc(unsigned long,unsigned long)
// type: _DWORD __fastcall(unsigned int, unsigned int)
#[doc(alias = "FreeImage_Aligned_Malloc(unsigned long,unsigned long)")]
pub fn stub_1088a4() -> ! {
    todo!("0x1088a4 FreeImage_Aligned_Malloc(unsigned long,unsigned long)")
}

// 0x1088fc — _FreeImage_AllocateT
// type: int __fastcall(_DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD)
#[doc(alias = "_FreeImage_AllocateT")]
pub fn stub_1088fc() -> ! {
    todo!("0x1088fc _FreeImage_AllocateT")
}

// 0x108afc — _FreeImage_Allocate
// type: int __fastcall(_DWORD, _DWORD, _DWORD)
#[doc(alias = "_FreeImage_Allocate")]
pub fn stub_108afc() -> ! {
    todo!("0x108afc _FreeImage_Allocate")
}

// 0x108b40 — _FreeImage_Unload
// type: (unknown)
#[doc(alias = "_FreeImage_Unload")]
pub fn stub_108b40() -> ! {
    todo!("0x108b40 _FreeImage_Unload")
}

// 0x108cdc — _FreeImage_FindNextMetadata
// type: (unknown)
#[doc(alias = "_FreeImage_FindNextMetadata")]
pub fn stub_108cdc() -> ! {
    todo!("0x108cdc _FreeImage_FindNextMetadata")
}

// 0x108e98 — _FreeImage_GetMetadataCount
// type: (unknown)
#[doc(alias = "_FreeImage_GetMetadataCount")]
pub fn stub_108e98() -> ! {
    todo!("0x108e98 _FreeImage_GetMetadataCount")
}

// 0x108f00 — _FreeImage_GetMetadata
// type: (unknown)
#[doc(alias = "_FreeImage_GetMetadata")]
pub fn stub_108f00() -> ! {
    todo!("0x108f00 _FreeImage_GetMetadata")
}

// 0x1090ac — _FreeImage_SetMetadata
// type: (unknown)
#[doc(alias = "_FreeImage_SetMetadata")]
pub fn stub_1090ac() -> ! {
    todo!("0x1090ac _FreeImage_SetMetadata")
}

// 0x109578 — _FreeImage_CloneMetadata
// type: (unknown)
#[doc(alias = "_FreeImage_CloneMetadata")]
pub fn stub_109578() -> ! {
    todo!("0x109578 _FreeImage_CloneMetadata")
}

// 0x1097ac — _FreeImage_FindFirstMetadata
// type: (unknown)
#[doc(alias = "_FreeImage_FindFirstMetadata")]
pub fn stub_1097ac() -> ! {
    todo!("0x1097ac _FreeImage_FindFirstMetadata")
}

// 0x1098b4 — _FreeImage_Clone
// type: (unknown)
#[doc(alias = "_FreeImage_Clone")]
pub fn stub_1098b4() -> ! {
    todo!("0x1098b4 _FreeImage_Clone")
}

// 0x109b88 — __Z13CalculateLineii / CalculateLine(int,int)
// type: _DWORD __fastcall(int, int)
#[doc(alias = "CalculateLine(int,int)")]
pub fn stub_109b88() -> ! {
    todo!("0x109b88 CalculateLine(int,int)")
}

// 0x109b9c — __ZNSt8_Rb_treeIiSt4pairIKiPSt3mapISsP5FITAGSt4lessISsESaIS0_IKSsS4_EEEESt10_Select1stISC_ES5_IiESaISC_EE4findERS1_ / std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::find(int const&)
// type: (unknown)
#[doc(alias = "std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::find(int const&)")]
pub fn stub_109b9c() -> ! {
    todo!("0x109b9c std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::find(int const&)")
}

// 0x109bf8 — __ZNSt8_Rb_treeIiSt4pairIKiPSt3mapISsP5FITAGSt4lessISsESaIS0_IKSsS4_EEEESt10_Select1stISC_ES5_IiESaISC_EE13_Rb_tree_implISF_Lb0EEC2ERKSaISt13_Rb_tree_nodeISC_EERKSF_ / std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_Rb_tree_impl<std::less<int>,false>::_Rb_tree_impl(std::allocator<std::_Rb_tree_node<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>> const&,std::less<int> const&)
// type: (unknown)
#[doc(alias = "std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_Rb_tree_impl<std::less<int>,false>::_Rb_tree_impl(std::allocator<std::_Rb_tree_node<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>> const&,std::less<int> const&)")]
pub fn stub_109bf8() -> ! {
    todo!("0x109bf8 std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_Rb_tree_impl<std::less<int>,false>::_Rb_tree_impl(std::allocator<std::_Rb_tree_node<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>> const&,std::less<int> const&)")
}

// 0x109c38 — __ZNSt8_Rb_treeISsSt4pairIKSsP5FITAGESt10_Select1stIS4_ESt4lessISsESaIS4_EE13_Rb_tree_implIS8_Lb0EEC2ERKSaISt13_Rb_tree_nodeIS4_EERKS8_ / std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_Rb_tree_impl<std::less<std::string>,false>::_Rb_tree_impl(std::allocator<std::_Rb_tree_node<std::pair<std::string const,FITAG *>>> const&,std::less<std::string> const&)
// type: (unknown)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_Rb_tree_impl<std::less<std::string>,false>::_Rb_tree_impl(std::allocator<std::_Rb_tree_node<std::pair<std::string const,FITAG *>>> const&,std::less<std::string> const&)")]
pub fn stub_109c38() -> ! {
    todo!("0x109c38 std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_Rb_tree_impl<std::less<std::string>,false>::_Rb_tree_impl(std::allocator<std::_Rb_tree_node<std::pair<std::string const,FITAG *>>> const&,std::less<std::string> const&)")
}

// 0x109c78 — __ZNSt8_Rb_treeIiSt4pairIKiPSt3mapISsP5FITAGSt4lessISsESaIS0_IKSsS4_EEEESt10_Select1stISC_ES5_IiESaISC_EE11lower_boundERS1_ / std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::lower_bound(int const&)
// type: (unknown)
#[doc(alias = "std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::lower_bound(int const&)")]
pub fn stub_109c78() -> ! {
    todo!("0x109c78 std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::lower_bound(int const&)")
}

// 0x109cac — __ZN9__gnu_cxx13new_allocatorISt4pairIKSsP5FITAGEE9constructEPS5_RKS5_ / __gnu_cxx::new_allocator<std::pair<std::string const,FITAG *>>::construct(std::pair<std::string const,FITAG *>*,std::pair<std::string const,FITAG *> const&)
// type: (unknown)
#[doc(alias = "__gnu_cxx::new_allocator<std::pair<std::string const,FITAG *>>::construct(std::pair<std::string const,FITAG *>*,std::pair<std::string const,FITAG *> const&)")]
pub fn stub_109cac() -> ! {
    todo!("0x109cac __gnu_cxx::new_allocator<std::pair<std::string const,FITAG *>>::construct(std::pair<std::string const,FITAG *>*,std::pair<std::string const,FITAG *> const&)")
}

// 0x109d68 — __ZN9__gnu_cxx13new_allocatorISt13_Rb_tree_nodeISt4pairIKiPSt3mapISsP5FITAGSt4lessISsESaIS2_IKSsS6_EEEEEE8allocateEmPKv / __gnu_cxx::new_allocator<std::_Rb_tree_node<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::allocate(unsigned long,void const*)
// type: (unknown)
#[doc(alias = "__gnu_cxx::new_allocator<std::_Rb_tree_node<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::allocate(unsigned long,void const*)")]
pub fn stub_109d68() -> ! {
    todo!("0x109d68 __gnu_cxx::new_allocator<std::_Rb_tree_node<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::allocate(unsigned long,void const*)")
}

// 0x109d98 — __ZNSt8_Rb_treeIiSt4pairIKiPSt3mapISsP5FITAGSt4lessISsESaIS0_IKSsS4_EEEESt10_Select1stISC_ES5_IiESaISC_EE14_M_create_nodeERKSC_ / std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_create_node(std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *> const&)
// type: (unknown)
#[doc(alias = "std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_create_node(std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *> const&)")]
pub fn stub_109d98() -> ! {
    todo!("0x109d98 std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_create_node(std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *> const&)")
}

// 0x109dc8 — __ZNSt8_Rb_treeIiSt4pairIKiPSt3mapISsP5FITAGSt4lessISsESaIS0_IKSsS4_EEEESt10_Select1stISC_ES5_IiESaISC_EE9_M_insertEPSt18_Rb_tree_node_baseSJ_RKSC_ / std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *> const&)
// type: (unknown)
#[doc(alias = "std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *> const&)")]
pub fn stub_109dc8() -> ! {
    todo!("0x109dc8 std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *> const&)")
}

// 0x109e4c — __ZNSt8_Rb_treeIiSt4pairIKiPSt3mapISsP5FITAGSt4lessISsESaIS0_IKSsS4_EEEESt10_Select1stISC_ES5_IiESaISC_EE16_M_insert_uniqueERKSC_ / std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_insert_unique(std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *> const&)
// type: (unknown)
#[doc(alias = "std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_insert_unique(std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *> const&)")]
pub fn stub_109e4c() -> ! {
    todo!("0x109e4c std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_insert_unique(std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *> const&)")
}

// 0x109f0c — __ZN9__gnu_cxx13new_allocatorISt13_Rb_tree_nodeISt4pairIKSsP5FITAGEEE8allocateEmPKv / __gnu_cxx::new_allocator<std::_Rb_tree_node<std::pair<std::string const,FITAG *>>>::allocate(unsigned long,void const*)
// type: (unknown)
#[doc(alias = "__gnu_cxx::new_allocator<std::_Rb_tree_node<std::pair<std::string const,FITAG *>>>::allocate(unsigned long,void const*)")]
pub fn stub_109f0c() -> ! {
    todo!("0x109f0c __gnu_cxx::new_allocator<std::_Rb_tree_node<std::pair<std::string const,FITAG *>>>::allocate(unsigned long,void const*)")
}

// 0x109f3c — __ZNSt8_Rb_treeISsSt4pairIKSsP5FITAGESt10_Select1stIS4_ESt4lessISsESaIS4_EE14_M_create_nodeERKS4_ / std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_create_node(std::pair<std::string const,FITAG *> const&)
// type: (unknown)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_create_node(std::pair<std::string const,FITAG *> const&)")]
pub fn stub_109f3c() -> ! {
    todo!("0x109f3c std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_create_node(std::pair<std::string const,FITAG *> const&)")
}

// 0x10a03c — __ZNSt8_Rb_treeISsSt4pairIKSsP5FITAGESt10_Select1stIS4_ESt4lessISsESaIS4_EE15_M_destroy_nodeEPSt13_Rb_tree_nodeIS4_E / std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_destroy_node(std::_Rb_tree_node<std::pair<std::string const,FITAG *>> *)
// type: int __fastcall(_DWORD, _DWORD)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_destroy_node(std::_Rb_tree_node<std::pair<std::string const,FITAG *>> *)")]
pub fn stub_10a03c() -> ! {
    todo!("0x10a03c std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_destroy_node(std::_Rb_tree_node<std::pair<std::string const,FITAG *>> *)")
}

// 0x10a0e4 — __ZNSt8_Rb_treeISsSt4pairIKSsP5FITAGESt10_Select1stIS4_ESt4lessISsESaIS4_EE8_M_eraseEPSt13_Rb_tree_nodeIS4_E / std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_erase(std::_Rb_tree_node<std::pair<std::string const,FITAG *>> *)
// type: int __fastcall(int result, int)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_erase(std::_Rb_tree_node<std::pair<std::string const,FITAG *>> *)")]
pub fn stub_10a0e4() -> ! {
    todo!("0x10a0e4 std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_erase(std::_Rb_tree_node<std::pair<std::string const,FITAG *>> *)")
}

// 0x10a124 — __ZNSt8_Rb_treeIiSt4pairIKiPSt3mapISsP5FITAGSt4lessISsESaIS0_IKSsS4_EEEESt10_Select1stISC_ES5_IiESaISC_EE8_M_eraseEPSt13_Rb_tree_nodeISC_E / std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_erase(std::_Rb_tree_node<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>> *)
// type: (unknown)
#[doc(alias = "std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_erase(std::_Rb_tree_node<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>> *)")]
pub fn stub_10a124() -> ! {
    todo!("0x10a124 std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_erase(std::_Rb_tree_node<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>> *)")
}

// 0x10a160 — __ZNSt8_Rb_treeISsSt4pairIKSsP5FITAGESt10_Select1stIS4_ESt4lessISsESaIS4_EE5eraseESt17_Rb_tree_iteratorIS4_E / std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::erase(std::_Rb_tree_iterator<std::pair<std::string const,FITAG *>>)
// type: int __fastcall(int, _Rb_tree_node_base *)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::erase(std::_Rb_tree_iterator<std::pair<std::string const,FITAG *>>)")]
pub fn stub_10a160() -> ! {
    todo!("0x10a160 std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::erase(std::_Rb_tree_iterator<std::pair<std::string const,FITAG *>>)")
}

// 0x10a198 — __ZNSt8_Rb_treeIiSt4pairIKiPSt3mapISsP5FITAGSt4lessISsESaIS0_IKSsS4_EEEESt10_Select1stISC_ES5_IiESaISC_EE5eraseESt17_Rb_tree_iteratorISC_E / std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::erase(std::_Rb_tree_iterator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>)
// type: int __fastcall(int, _Rb_tree_node_base *)
#[doc(alias = "std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::erase(std::_Rb_tree_iterator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>)")]
pub fn stub_10a198() -> ! {
    todo!("0x10a198 std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::erase(std::_Rb_tree_iterator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>)")
}

// 0x10a1c8 — __ZNSt8_Rb_treeIiSt4pairIKiPSt3mapISsP5FITAGSt4lessISsESaIS0_IKSsS4_EEEESt10_Select1stISC_ES5_IiESaISC_EE16_M_insert_uniqueESt17_Rb_tree_iteratorISC_ERKSC_ / std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *> const&)
// type: int __fastcall(int, _Rb_tree_node_base *)
#[doc(alias = "std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *> const&)")]
pub fn stub_10a1c8() -> ! {
    todo!("0x10a1c8 std::_Rb_tree<int,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>,std::_Select1st<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>,std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *> const&)")
}

// 0x10a2ec — __ZNSt8_Rb_treeISsSt4pairIKSsP5FITAGESt10_Select1stIS4_ESt4lessISsESaIS4_EE11upper_boundERS1_ / std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::upper_bound(std::string const&)
// type: int __fastcall(int, std::string *this)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::upper_bound(std::string const&)")]
pub fn stub_10a2ec() -> ! {
    todo!("0x10a2ec std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::upper_bound(std::string const&)")
}

// 0x10a334 — __ZNSt8_Rb_treeISsSt4pairIKSsP5FITAGESt10_Select1stIS4_ESt4lessISsESaIS4_EE11lower_boundERS1_ / std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::lower_bound(std::string const&)
// type: int __fastcall(int, std::string *)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::lower_bound(std::string const&)")]
pub fn stub_10a334() -> ! {
    todo!("0x10a334 std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::lower_bound(std::string const&)")
}

// 0x10a37c — __ZNSt8_Rb_treeISsSt4pairIKSsP5FITAGESt10_Select1stIS4_ESt4lessISsESaIS4_EE11equal_rangeERS1_ / std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::equal_range(std::string const&)
// type: int __fastcall(int, int, std::string *)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::equal_range(std::string const&)")]
pub fn stub_10a37c() -> ! {
    todo!("0x10a37c std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::equal_range(std::string const&)")
}

// 0x10a3c4 — __ZNSt8_Rb_treeISsSt4pairIKSsP5FITAGESt10_Select1stIS4_ESt4lessISsESaIS4_EE4findERS1_ / std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::find(std::string const&)
// type: int __fastcall(int, std::string *this)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::find(std::string const&)")]
pub fn stub_10a3c4() -> ! {
    todo!("0x10a3c4 std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::find(std::string const&)")
}

// 0x10a43c — __ZNSt8_Rb_treeISsSt4pairIKSsP5FITAGESt10_Select1stIS4_ESt4lessISsESaIS4_EE9_M_insertEPSt18_Rb_tree_node_baseSC_RKS4_ / std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<std::string const,FITAG *> const&)
// type: int __fastcall(int, int, int, std::string *this)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<std::string const,FITAG *> const&)")]
pub fn stub_10a43c() -> ! {
    todo!("0x10a43c std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<std::string const,FITAG *> const&)")
}

// 0x10a4c0 — __ZNSt8_Rb_treeISsSt4pairIKSsP5FITAGESt10_Select1stIS4_ESt4lessISsESaIS4_EE16_M_insert_uniqueERKS4_ / std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_insert_unique(std::pair<std::string const,FITAG *> const&)
// type: int __fastcall(int, int, std::string *this)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_insert_unique(std::pair<std::string const,FITAG *> const&)")]
pub fn stub_10a4c0() -> ! {
    todo!("0x10a4c0 std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_insert_unique(std::pair<std::string const,FITAG *> const&)")
}

// 0x10a584 — __ZNSt8_Rb_treeISsSt4pairIKSsP5FITAGESt10_Select1stIS4_ESt4lessISsESaIS4_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS4_ERKS4_ / std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<std::string const,FITAG *>>,std::pair<std::string const,FITAG *> const&)
// type: int __fastcall(int, int, std::string *this)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<std::string const,FITAG *>>,std::pair<std::string const,FITAG *> const&)")]
pub fn stub_10a584() -> ! {
    todo!("0x10a584 std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<std::string const,FITAG *>>,std::pair<std::string const,FITAG *> const&)")
}
// 0x10a6e4 — __ZNSt8_Rb_treeISsSt4pairIKSsP5FITAGESt10_Select1stIS4_ESt4lessISsESaIS4_EE5eraseESt17_Rb_tree_iteratorIS4_ESC_ / std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::erase(std::_Rb_tree_iterator<std::pair<std::string const,FITAG *>>,std::_Rb_tree_iterator<std::pair<std::string const,FITAG *>>)
// type: int __fastcall(int, _Rb_tree_node_base *)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::erase(std::_Rb_tree_iterator<std::pair<std::string const,FITAG *>>,std::_Rb_tree_iterator<std::pair<std::string const,FITAG *>>)")]
pub fn stub_10a6e4() -> ! {
    todo!("0x10a6e4 std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::erase(std::_Rb_tree_iterator<std::pair<std::string const,FITAG *>>,std::_Rb_tree_iterator<std::pair<std::string const,FITAG *>>)")
}

// 0x10a760 — __ZNSt8_Rb_treeISsSt4pairIKSsP5FITAGESt10_Select1stIS4_ESt4lessISsESaIS4_EE5eraseERS1_ / std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::erase(std::string const&)
// type: int __fastcall(int, std::string *)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::erase(std::string const&)")]
pub fn stub_10a760() -> ! {
    todo!("0x10a760 std::_Rb_tree<std::string,std::pair<std::string const,FITAG *>,std::_Select1st<std::pair<std::string const,FITAG *>>,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::erase(std::string const&)")
}

// 0x10a7a8 — __ZNSt3mapISsP5FITAGSt4lessISsESaISt4pairIKSsS1_EEEixERS5_ / std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::operator[](std::string const&)
// type: (unknown)
#[doc(alias = "std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::operator[](std::string const&)")]
pub fn stub_10a7a8() -> ! {
    todo!("0x10a7a8 std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>>::operator[](std::string const&)")
}

// 0x10a8e4 — __ZNSt3mapIiPS_ISsP5FITAGSt4lessISsESaISt4pairIKSsS1_EEES2_IiESaIS4_IKiS9_EEEixERSB_ / std::map<int,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::operator[](int const&)
// type: (unknown)
#[doc(alias = "std::map<int,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::operator[](int const&)")]
pub fn stub_10a8e4() -> ! {
    todo!("0x10a8e4 std::map<int,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *,std::less<int>,std::allocator<std::pair<int const,std::map<std::string,FITAG *,std::less<std::string>,std::allocator<std::pair<std::string const,FITAG *>>> *>>>::operator[](int const&)")
}

// 0x10a950 — _FreeImage_ConvertLine1To24 / _FreeImage_ConvertLine1To24
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertLine1To24")]
pub fn stub_10a950() -> ! {
    todo!("0x10a950 _FreeImage_ConvertLine1To24")
}

// 0x10ab1c — _FreeImage_ConvertLine4To24 / _FreeImage_ConvertLine4To24
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertLine4To24")]
pub fn stub_10ab1c() -> ! {
    todo!("0x10ab1c _FreeImage_ConvertLine4To24")
}

// 0x10ad30 — _FreeImage_ConvertLine8To24 / _FreeImage_ConvertLine8To24
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertLine8To24")]
pub fn stub_10ad30() -> ! {
    todo!("0x10ad30 _FreeImage_ConvertLine8To24")
}

// 0x10af0c — _FreeImage_ConvertLine16To24_555 / _FreeImage_ConvertLine16To24_555
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertLine16To24_555")]
pub fn stub_10af0c() -> ! {
    todo!("0x10af0c _FreeImage_ConvertLine16To24_555")
}

// 0x10b0b4 — _FreeImage_ConvertLine16To24_565 / _FreeImage_ConvertLine16To24_565
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertLine16To24_565")]
pub fn stub_10b0b4() -> ! {
    todo!("0x10b0b4 _FreeImage_ConvertLine16To24_565")
}

// 0x10b270 — _FreeImage_ConvertLine32To24 / _FreeImage_ConvertLine32To24
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertLine32To24")]
pub fn stub_10b270() -> ! {
    todo!("0x10b270 _FreeImage_ConvertLine32To24")
}

// 0x10b4a0 — _FreeImage_ConvertTo24Bits / _FreeImage_ConvertTo24Bits
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertTo24Bits")]
pub fn stub_10b4a0() -> ! {
    todo!("0x10b4a0 _FreeImage_ConvertTo24Bits")
}

// 0x10c390 — _FreeImage_ConvertLine1To32 / _FreeImage_ConvertLine1To32
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertLine1To32")]
pub fn stub_10c390() -> ! {
    todo!("0x10c390 _FreeImage_ConvertLine1To32")
}

// 0x10c590 — _FreeImage_ConvertLine4To32 / _FreeImage_ConvertLine4To32
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertLine4To32")]
pub fn stub_10c590() -> ! {
    todo!("0x10c590 _FreeImage_ConvertLine4To32")
}

// 0x10c7c0 — _FreeImage_ConvertLine8To32 / _FreeImage_ConvertLine8To32
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertLine8To32")]
pub fn stub_10c7c0() -> ! {
    todo!("0x10c7c0 _FreeImage_ConvertLine8To32")
}

// 0x10c9c4 — _FreeImage_ConvertLine16To32_555 / _FreeImage_ConvertLine16To32_555
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertLine16To32_555")]
pub fn stub_10c9c4() -> ! {
    todo!("0x10c9c4 _FreeImage_ConvertLine16To32_555")
}

// 0x10cb84 — _FreeImage_ConvertLine16To32_565 / _FreeImage_ConvertLine16To32_565
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertLine16To32_565")]
pub fn stub_10cb84() -> ! {
    todo!("0x10cb84 _FreeImage_ConvertLine16To32_565")
}

// 0x10cd50 — _FreeImage_ConvertLine24To32 / _FreeImage_ConvertLine24To32
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertLine24To32")]
pub fn stub_10cd50() -> ! {
    todo!("0x10cd50 _FreeImage_ConvertLine24To32")
}

// 0x10ce94 — _FreeImage_ConvertTo32Bits / _FreeImage_ConvertTo32Bits
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertTo32Bits")]
pub fn stub_10ce94() -> ! {
    todo!("0x10ce94 _FreeImage_ConvertTo32Bits")
}

// 0x10e008 — _FreeImage_ConvertLine1To8 / _FreeImage_ConvertLine1To8
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertLine1To8")]
pub fn stub_10e008() -> ! {
    todo!("0x10e008 _FreeImage_ConvertLine1To8")
}

// 0x10e0fc — _FreeImage_ConvertLine4To8 / _FreeImage_ConvertLine4To8
// type: (unknown)
#[doc(alias = "_FreeImage_ConvertLine4To8")]
pub fn stub_10e0fc() -> ! {
    todo!("0x10e0fc _FreeImage_ConvertLine4To8")
}

...
pub fn stub_1084c() -> bool {
    // IDA 0x1084c (`GetSetImpl<bool (CRenderSettingsItem::*)()const,
    // void (CRenderSettingsItem::*)(bool)>::isReadOnly`): `return 0` —
    // getter and setter both present (cf. 0xfcb4). Family-verified.
    false
}
...
pub fn stub_10850() -> bool {
    // IDA 0x10850 (same `GetSetImpl::isWriteOnly`): `return 0` (cf. 0xfcb8).
    false
}
...
pub fn stub_10854(item: &RenderSettingsItem, get: fn(&RenderSettingsItem) -> bool) -> bool {
    // IDA 0x10854 (same `GetSetImpl::getValue` for the bool pair): downcast
    // (no base adjustment needed for the primary base), bound member getter
    // (direct or virtual-bit dispatch). Verified via IDA decompile.
    get(item)
}
...
pub fn stub_10878(item: &mut RenderSettingsItem, value: bool, set: fn(&mut RenderSettingsItem, bool)) {
    // IDA 0x10878 (same `GetSetImpl::setValue`): downcast, bound member
    // setter with the new value. Verified via IDA decompile.
    set(item, value)
}
...
pub fn stub_1089c(name: &str) -> RenderPropDescriptor {
    // IDA 0x1089c (`PropDescriptor<int>::C2` with
    // `int (CRenderSettings::*)()const` getter /
    // `void (CRenderSettingsItem::*)(int)` setter): base descriptor init +
    // `GetSetImpl` pair install. Same shape as the uint twin at 0xfb74. Host
    // stores the property name.
    RenderPropDescriptor { prop_name: name.to_string(), extra: None }
}
...
pub fn stub_109b0() -> bool {
    // IDA 0x109b0 (`GetSetImpl<int (CRenderSettings::*)()const,
    // void (CRenderSettingsItem::*)(int)>::isReadOnly`): `return 0` (cf.
    // 0xfcb4). Family-verified.
    false
}
...
pub fn stub_109b4() -> bool {
    // IDA 0x109b4 (same `GetSetImpl::isWriteOnly`): `return 0` (cf. 0xfcb8).
    false
}
...
pub fn stub_109b8(item: &RenderSettingsItem, get: fn(&RenderSettingsItem) -> i32) -> i32 {
    // IDA 0x109b8 (same `GetSetImpl::getValue` for the int pair): downcast +
    // base adjustments, bound member getter. Same shape as 0xfcbc.
    // Family-verified.
    get(item)
}
...
pub fn stub_109e4(item: &mut RenderSettingsItem, value: i32, set: fn(&mut RenderSettingsItem, i32)) {
    // IDA 0x109e4 (same `GetSetImpl::setValue`): downcast/adjustment, bound
    // member setter. Same shape as 0xfce8. Family-verified.
    set(item, value)
}
...
pub fn stub_10a08(name: &str) -> RenderPropDescriptor {
    // IDA 0x10a08 (`EnumPropDescriptor<AntialiasingMode>::C2` with
    // `AntialiasingMode (CRenderSettings::*)()const` getter /
    // `void (CRenderSettingsItem::*)(AntialiasingMode)` setter): base + enum
...
pub fn stub_10bbc(desc: *mut RenderPropDescriptor) {
    // IDA 0x10bbc (`EnumPropDescriptor<AntialiasingMode>::D0`): vtable reset
    // (host nop), `delete a1[11]`, `operator delete(a1)`. Same as 0x10038.
    // Verified via IDA decompile.
    if desc.is_null() {
        return;
    }
...
pub fn stub_10be8() -> bool {
    // IDA 0x10be8 (`EnumPropDescriptor<AntialiasingMode>::isReadOnly`):
    // delegates to the `GetSetImpl` impl, returns 0 — getter and setter
    // both present. Same as 0x10064. Family-verified.
    false
}
...
pub fn stub_10bf8() -> bool {
    // IDA 0x10bf8 (`EnumPropDescriptor<AntialiasingMode>::isWriteOnly`):
    // returns 0. Same as 0x10074. Family-verified.
    false
}
...
pub fn stub_10c08(a: i32, b: i32) -> bool {
    // IDA 0x10c08 (`EnumPropDescriptor<AntialiasingMode>::equalValues`):
    // `getValue` on both, compares the ints. Same as 0x10084.
    // Family-verified.
    a == b
}
...
pub fn stub_10c30(value: i32, out: &mut RegionAny) -> i32 {
    // IDA 0x10c30 (`EnumPropDescriptor<AntialiasingMode>::getVariant`):
    // `getEnumValue`, int wrap, `placement_any<Region3> = int`. Same as
    // 0x100ac. Family-verified.
    out.tag = REGION_ANY_INT;
    out.value = value;
    value
}
...
pub fn stub_10c54(item: &mut RenderSettingsItem, value: i32, set: fn(&mut RenderSettingsItem, i32)) {
    // IDA 0x10c54 (`EnumPropDescriptor<AntialiasingMode>::setVariant`):
...
pub fn stub_10da4(src: &RenderSettingsItem, dst: &mut RenderSettingsItem, get: fn(&RenderSettingsItem) -> i32, set: fn(&mut RenderSettingsItem, i32)) {
    // IDA 0x10da4 (`EnumPropDescriptor<AntialiasingMode>::copyValue`):
...
pub fn stub_10dc8() -> bool {
    // IDA 0x10dc8 (`EnumPropDescriptor<AntialiasingMode>::hasStringValue`):
...
pub fn stub_10dcc(desc: &RenderEnumDesc, item: &RenderSettingsItem, get: fn(&RenderSettingsItem) -> i32, out: &mut String) {
    // IDA 0x10dcc (`EnumPropDescriptor<AntialiasingMode>::getStringValue`):
    // `getValue` + by-ref `convertToString` (always assigns). Same as
...
pub fn stub_10df0(desc: &RenderEnumDesc, item: &mut RenderSettingsItem, name: &str, set: fn(&mut RenderSettingsItem, i32)) -> bool {
    // IDA 0x10df0 (`EnumPropDescriptor<AntialiasingMode>::setStringValue`):
    // `Name::lookup` + `convertToValue` + `setValue`, 1/0. Same as 0x1026c.
    // Family-verified.
    stub_1026c(desc, item, name, set)
}
...
pub fn stub_10e30(item: &RenderSettingsItem, get: fn(&RenderSettingsItem) -> i32) -> (u32, i32) {
    // IDA 0x10e30 (`EnumPropDescriptor<AntialiasingMode>::writeValue`):
    // `getValue` + `{kind = 5, value}` pair. Same as 0x102ac.
    // Family-verified.
    (5, get(item))
}
...
pub fn stub_10e50(desc: &RenderEnumDesc, item: &mut RenderSettingsItem, text: &str, set: fn(&mut RenderSettingsItem, i32)) -> bool {
    // IDA 0x10e50 (`EnumPropDescriptor<AntialiasingMode>::readValue`):
    // element text → int-or-name set path. Same as 0x102cc.
    // Family-verified.
...
pub fn stub_11090(desc: &RenderEnumDesc, item: &RenderSettingsItem, get: fn(&RenderSettingsItem) -> i32) -> i32 {
    // IDA 0x11090 (`EnumPropDescriptor<AntialiasingMode>::getIndexValue`):
...
pub fn stub_110ac(desc: &RenderEnumDesc, item: &mut RenderSettingsItem, index: usize, set: fn(&mut RenderSettingsItem, i32)) -> bool {
    // IDA 0x110ac (`EnumPropDescriptor<AntialiasingMode>::setIndexValue`):
    // bounds check + `setValue`, 1/0. Same as 0x10528. Family-verified.
    match desc.pairs.get(index) {
        Some(p) => {
            set(item, p.value);
            true
...
pub fn stub_110e0(item: &RenderSettingsItem, get: fn(&RenderSettingsItem) -> i32) -> i32 {
    // IDA 0x110e0 (`EnumPropDescriptor<AntialiasingMode>::getEnumValue`):
    // impl `getValue`. Same as 0x1055c. Family-verified.
    get(item)
}
...
pub fn stub_110e8(desc: &RenderEnumDesc, item: &mut RenderSettingsItem, value: i32, set: fn(&mut RenderSettingsItem, i32)) -> bool {
    // IDA 0x110e8 (`EnumPropDescriptor<AntialiasingMode>::setEnumValue`):
    // item-table `find_if`, miss 0, hit `setValue` + 1. Same as 0x10564.
    // Family-verified.
    match desc.pairs.iter().find(|p| p.value == value) {
        Some(p) => {
            set(item, p.value);
...
pub fn stub_11134(desc: &RenderEnumDesc, item: &RenderSettingsItem, get: fn(&RenderSettingsItem) -> i32) -> Option<i32> {
    // IDA 0x11134 (`EnumPropDescriptor<AntialiasingMode>::getEnumItem`):
    // `getValue` + `convertToItem` (asserts 273/274). Same as 0x105b0.
    // Family-verified.
    let value = get(item);
    debug_assert!(value >= 0, "value>=0 ../App/include/reflection/enumconverter.h:273");
    desc.pairs.iter().find(|p| p.value == value).map(|p| p.value)
...
pub fn stub_11154(desc: &RenderEnumDesc, item: &mut RenderSettingsItem, name: &str, set: fn(&mut RenderSettingsItem, i32)) -> bool {
    // IDA 0x11154 (`EnumPropDescriptor<AntialiasingMode>::setStringValue`
    // with `Name`): `convertToValue` + `setValue`, 1/0. Same as 0x105d0.
    // Family-verified.
    stub_1026c(desc, item, name, set)
}
...
pub fn stub_11188(desc: &RenderEnumDesc, value: i32) -> i32 {
    // IDA 0x11188 (`EnumDesc<AntialiasingMode>::convertToIndex`): assert
    // (:350), value→index, miss -1. Same as 0x10604. Family-verified.
    debug_assert!(value >= 0, "value>=0 ../App/include/reflection/enumconverter.h:350");
    desc.pairs.iter().position(|p| p.value == value).map(|i| i as i32).unwrap_or(-1)
}
...
pub fn stub_111f8(desc: &RenderEnumDesc, item: &mut RenderSettingsItem, index: i32, set: fn(&mut RenderSettingsItem, i32)) -> bool {
    // IDA 0x111f8 (`EnumPropDescriptor<AntialiasingMode>::setIntValue`):
    // negative → 0, table `-1` → 0, else `setValue` + 1. Same as 0x10674.
    // Family-verified.
    if index < 0 {
        return false;
    }
...
pub fn stub_11238() -> bool {
    // IDA 0x11238 (`GetSetImpl<AntialiasingMode (CRenderSettings::*)()const,
    // ...>::isReadOnly`): `return 0`. Same as 0x106b4. Family-verified.
    false
}
...
pub fn stub_1123c() -> bool {
    // IDA 0x1123c (same `GetSetImpl::isWriteOnly`): `return 0`. Same as
...
pub fn stub_11240(item: &RenderSettingsItem, get: fn(&RenderSettingsItem) -> i32) -> i32 {
    // IDA 0x11240 (same `GetSetImpl::getValue`): downcast/adjustment, bound
    // getter. Same as 0x106bc. Family-verified.
    get(item)
}
...
pub fn stub_1126c(item: &mut RenderSettingsItem, value: i32, set: fn(&mut RenderSettingsItem, i32)) {
    // IDA 0x1126c (same `GetSetImpl::setValue`): downcast/adjustment, bound
    // setter. Same as 0x106e8. Family-verified.
    set(item, value)
}
...
pub fn stub_11290(name: &str) -> RenderPropDescriptor {
    // IDA 0x11290 (`EnumPropDescriptor<ShadowMode>::C2`): base + enum wiring
    // with the member-pointer pair. Same shape as 0xfe84/0x10a08. Host
...
pub fn stub_11444(desc: *mut RenderPropDescriptor) {
    // IDA 0x11444 (`EnumPropDescriptor<ShadowMode>::D0`): vtable reset (host
    // nop), `delete a1[11]`, `operator delete(a1)`. Same as 0x10038/0x10bbc.
    // Family-verified.
    if desc.is_null() {
        return;
    }
...
pub fn stub_11470() -> bool {
    // IDA 0x11470 (`EnumPropDescriptor<ShadowMode>::isReadOnly`): impl
    // delegate, 0. Same as 0x10064. Family-verified.
    false
}
...
pub fn stub_11480() -> bool {
    // IDA 0x11480 (`EnumPropDescriptor<ShadowMode>::isWriteOnly`): 0. Same
    // as 0x10074. Family-verified.
    false
}
...
pub fn stub_11490(a: i32, b: i32) -> bool {
    // IDA 0x11490 (`EnumPropDescriptor<ShadowMode>::equalValues`): getValue
    // on both, int compare. Same as 0x10084. Family-verified.
    a == b
}
...
pub fn stub_114b8(value: i32, out: &mut RegionAny) -> i32 {
    // IDA 0x114b8 (`EnumPropDescriptor<ShadowMode>::getVariant`): int wrap +
    // `placement_any` store. Same as 0x100ac. Family-verified.
    out.tag = REGION_ANY_INT;
    out.value = value;
    value
}
...
pub fn stub_114dc(item: &mut RenderSettingsItem, value: i32, set: fn(&mut RenderSettingsItem, i32)) {
    // IDA 0x114dc (`EnumPropDescriptor<ShadowMode>::setVariant`): cast +
    // `setEnumValue`. Same as 0x100d0. Family-verified.
    set(item, value)
}
...
pub fn stub_1162c(src: &RenderSettingsItem, dst: &mut RenderSettingsItem, get: fn(&RenderSettingsItem) -> i32, set: fn(&mut RenderSettingsItem, i32)) {
    // IDA 0x1162c (`EnumPropDescriptor<ShadowMode>::copyValue`): get + set.
    // Same as 0x10220. Family-verified.
    let value = get(src);
    set(dst, value)
}
...
pub fn stub_11650() -> bool {
    // IDA 0x11650 (`EnumPropDescriptor<ShadowMode>::hasStringValue`):
    // `return 1`. Same as 0x10244. Family-verified.
    true
}
...
pub fn stub_11654(desc: &RenderEnumDesc, item: &RenderSettingsItem, get: fn(&RenderSettingsItem) -> i32, out: &mut String) {
    // IDA 0x11654 (`EnumPropDescriptor<ShadowMode>::getStringValue`):
    // getValue + by-ref convertToString (always assigns). Same as 0x10248.
    // Family-verified.
    let value = get(item);
    debug_assert!(value >= 0, "value>=0 ../App/include/reflection/enumconverter.h:262");
    out.clear();
...
pub fn stub_11678(desc: &RenderEnumDesc, item: &mut RenderSettingsItem, name: &str, set: fn(&mut RenderSettingsItem, i32)) -> bool {
    // IDA 0x11678 (`EnumPropDescriptor<ShadowMode>::setStringValue`):
...
pub fn stub_116b8(item: &RenderSettingsItem, get: fn(&RenderSettingsItem) -> i32) -> (u32, i32) {
    // IDA 0x116b8 (`EnumPropDescriptor<ShadowMode>::writeValue`): getValue +
...
pub fn stub_116d8(desc: &RenderEnumDesc, item: &mut RenderSettingsItem, text: &str, set: fn(&mut RenderSettingsItem, i32)) -> bool {
    // IDA 0x116d8 (`EnumPropDescriptor<ShadowMode>::readValue`): element
...
pub fn stub_11918(desc: &RenderEnumDesc, item: &RenderSettingsItem, get: fn(&RenderSettingsItem) -> i32) -> i32 {
    // IDA 0x11918 (`EnumPropDescriptor<ShadowMode>::getIndexValue`):
    // getValue + convertToIndex (assert :350, miss -1). Same as 0x1050c.
    // Family-verified.
    let value = get(item);
    debug_assert!(value >= 0, "value>=0 ../App/include/reflection/enumconverter.h:350");

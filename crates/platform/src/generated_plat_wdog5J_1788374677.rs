//! platform — generated_plat_wdog5J_1788374677 — 100 stubs EA-sorted asc distinct not yet in platform | Source ida/export.json | range 0x521ac4..0x52a9b8 | rbx_core::SharedPtr not boost
//! Filter: iOS|Controller|View|Gui|Input (6872 total, 2970 uncovered before batch, 100 in batch) EA-sorted asc distinct not yet in platform (platform gap, no global fallback needed)
//! Distinct stub_ 44352/85545 -> 44452/85545 | platform filtered remaining 2870
//! Batch: 100 stubs | range 0x521ac4..0x52a9b8 | rbx_core::SharedPtr not boost

#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]

use rbx_core::SharedPtr;

const _: () = {
    let _ = core::marker::PhantomData::<SharedPtr<u8>>;
};

// 0x521ac4 — __ZN3RBX18CustomStatsGuiJSOND0Ev
// type: void __fastcall(RBX::CustomStatsGuiJSON *__hidden this)
#[doc(alias = "RBX::CustomStatsGuiJSON::~CustomStatsGuiJSON()")]
pub fn stub_0x521ac4() -> ! {
    todo!("0x521ac4 RBX::CustomStatsGuiJSON::~CustomStatsGuiJSON()")
}

// 0x522258 — __ZNK3RBX7GuiItem12getClassNameEv
// type: _DWORD __fastcall(RBX::GuiItem *__hidden this)
#[doc(alias = "RBX::GuiItem::getClassName(void)const")]
pub fn stub_0x522258() -> ! {
    todo!("0x522258 RBX::GuiItem::getClassName(void)const")
}

// 0x522260 — __ZNK3RBX7GuiItem11getPositionENS_6CanvasE
#[doc(alias = "RBX::GuiItem::getPosition(RBX::Canvas)const")]
pub fn stub_0x522260() -> ! {
    todo!("0x522260 RBX::GuiItem::getPosition(RBX::Canvas)const")
}

// 0x5222f4 — __ZN3RBX7GuiItem8getTitleEv
// type: _DWORD __fastcall(RBX::GuiItem *__hidden this)
#[doc(alias = "RBX::GuiItem::getTitle(void)")]
pub fn stub_0x5222f4() -> ! {
    todo!("0x5222f4 RBX::GuiItem::getTitle(void)")
}

// 0x52231c — __ZThn32_NK3RBX7GuiItem12getClassNameEv
// type: _DWORD __fastcall(RBX::GuiItem *__hidden this)
#[doc(alias = "`non-virtual thunk to'RBX::GuiItem::getClassName(void)const")]
pub fn stub_0x52231c() -> ! {
    todo!("0x52231c `non-virtual thunk to'RBX::GuiItem::getClassName(void)const")
}

// 0x522a08 — __ZN3RBX12GuiDrawImageD2Ev
// type: void __fastcall(RBX::GuiDrawImage *__hidden this)
#[doc(alias = "RBX::GuiDrawImage::~GuiDrawImage()")]
pub fn stub_0x522a08() -> ! {
    todo!("0x522a08 RBX::GuiDrawImage::~GuiDrawImage()")
}

// 0x522c04 — __ZN3RBX12GuiDrawImageC2EPNS_5AdornERKSsj
// type: _DWORD __fastcall(RBX::GuiDrawImage *__hidden this, RBX::Adorn *, const std::string *, unsigned int)
#[doc(alias = "RBX::GuiDrawImage::GuiDrawImage(RBX::Adorn *,std::string const&,unsigned int)")]
pub fn stub_0x522c04() -> ! {
    todo!("0x522c04 RBX::GuiDrawImage::GuiDrawImage(RBX::Adorn *,std::string const&,unsigned int)")
}

// 0x522e8c — __ZNK3RBX7GuiItem7getSizeENS_6CanvasE
#[doc(alias = "RBX::GuiItem::getSize(RBX::Canvas)const")]
pub fn stub_0x522e8c() -> ! {
    todo!("0x522e8c RBX::GuiItem::getSize(RBX::Canvas)const")
}

// 0x523ac0 — __ZN3RBX7GuiItem11onLoseFocusEv
// type: _DWORD __fastcall(RBX::GuiItem *__hidden this)
#[doc(alias = "RBX::GuiItem::onLoseFocus(void)")]
pub fn stub_0x523ac0() -> ! {
    todo!("0x523ac0 RBX::GuiItem::onLoseFocus(void)")
}

// 0x523ac4 — __ZNK3RBX7GuiItem11getFontSizeEv
// type: _DWORD __fastcall(RBX::GuiItem *__hidden this)
#[doc(alias = "RBX::GuiItem::getFontSize(void)const")]
pub fn stub_0x523ac4() -> ! {
    todo!("0x523ac4 RBX::GuiItem::getFontSize(void)const")
}

// 0x523fc8 — __ZNSt8_Rb_treeISsSt4pairIKSsN3RBX10GuiBuilder4DataEESt10_Select1stIS5_ESt4lessISsESaIS5_EE15_M_destroy_nodeEPSt13_Rb_tree_nodeIS5_E
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,RBX::GuiBuilder::Data>,std::_Select1st<std::pair<std::string const,RBX::GuiBuilder::Data>>,std::less<std::string>,std::allocator<std::pair<std::string const,RBX::GuiBuilder::Data>>>::_M_destroy_node(std::_Rb_tree_node<std::pair<std::string const,RBX::GuiBuilder::Data>> *)")]
pub fn stub_0x523fc8() -> ! {
    todo!("0x523fc8 std::_Rb_tree<std::string,std::pair<std::string const,RBX::GuiBuilder::Data>,std::_Select1st<std::pair<std::string const,RBX::GuiBuilder::Data>>,std::less<std::string>,std::allocator<std::pair<std::string const,RBX::GuiBuilder::Data>>>::_M_destroy_node(std::_Rb_tree_node<std::pair<std::string const,RBX::GuiBuilder::Data>> *)")
}

// 0x524088 — __ZNSt8_Rb_treeISsSt4pairIKSsN3RBX10GuiBuilder4DataEESt10_Select1stIS5_ESt4lessISsESaIS5_EE4findERS1_
// type: int __fastcall(int, std::string *this)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,RBX::GuiBuilder::Data>,std::_Select1st<std::pair<std::string const,RBX::GuiBuilder::Data>>,std::less<std::string>,std::allocator<std::pair<std::string const,RBX::GuiBuilder::Data>>>::find(std::string const&)")]
pub fn stub_0x524088() -> ! {
    todo!("0x524088 std::_Rb_tree<std::string,std::pair<std::string const,RBX::GuiBuilder::Data>,std::_Select1st<std::pair<std::string const,RBX::GuiBuilder::Data>>,std::less<std::string>,std::allocator<std::pair<std::string const,RBX::GuiBuilder::Data>>>::find(std::string const&)")
}

// 0x5240d8 — __ZN3RBX17NonFactoryProductINS_13BasePlayerGuiELZNS_15sCoreGuiServiceEEE15isNullClassNameEv
#[doc(alias = "__ZN3RBX17NonFactoryProductINS_13BasePlayerGuiELZNS_15sCoreGuiServiceEEE15isNullClassNameEv")]
pub fn stub_0x5240d8() -> ! {
    todo!("0x5240d8 __ZN3RBX17NonFactoryProductINS_13BasePlayerGuiELZNS_15sCoreGuiServiceEEE15isNullClassNameEv")
}

// 0x524178 — __ZN3RBX4Name7declareILZNS_15sCoreGuiServiceEEEERKS0_v
#[doc(alias = "__ZN3RBX4Name7declareILZNS_15sCoreGuiServiceEEEERKS0_v")]
pub fn stub_0x524178() -> ! {
    todo!("0x524178 __ZN3RBX4Name7declareILZNS_15sCoreGuiServiceEEEERKS0_v")
}

// 0x5241bc — __ZN3RBX4Name9doDeclareILZNS_15sCoreGuiServiceEEEERKS0_v
#[doc(alias = "__ZN3RBX4Name9doDeclareILZNS_15sCoreGuiServiceEEEERKS0_v")]
pub fn stub_0x5241bc() -> ! {
    todo!("0x5241bc __ZN3RBX4Name9doDeclareILZNS_15sCoreGuiServiceEEEERKS0_v")
}

// 0x5242a0 — __ZN3RBX15ServiceProvider15doGetClassIndexINS_14CoreGuiServiceEEEmv
#[doc(alias = "unsigned long RBX::ServiceProvider::doGetClassIndex<RBX::CoreGuiService>(void)")]
pub fn stub_0x5242a0() -> ! {
    todo!("0x5242a0 unsigned long RBX::ServiceProvider::doGetClassIndex<RBX::CoreGuiService>(void)")
}

// 0x5244c0 — __ZN5boost10shared_ptrIN3RBX14GuiImageButtonEEC2IS2_NS1_9CreatableINS1_8InstanceEE7DeleterEEEPT_T0_
// was: boost::shared_ptr<RBX::GuiImageButton>::shared_ptr<RBX::GuiImageButton,RBX::Creatable<RBX::Instance>::Deleter>(RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "rbx_core::SharedPtr<RBX::GuiImageButton>::shared_ptr<RBX::GuiImageButton,RBX::Creatable<RBX::Instance>::Deleter>(RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter)")]
pub fn stub_0x5244c0() -> ! {
    todo!("0x5244c0 boost::shared_ptr<RBX::GuiImageButton>::shared_ptr<RBX::GuiImageButton,RBX::Creatable<RBX::Instance>::Deleter>(RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter)")
}

// 0x524588 — __ZNK5boost23enable_shared_from_thisIN3RBX10Reflection13DescribedBaseEE22_internal_accept_ownerINS1_14GuiImageButtonES6_EEvPKNS_10shared_ptrIT_EEPT0_
// was: void boost::enable_shared_from_this<RBX::Reflection::DescribedBase>::_internal_accept_owner<RBX::GuiImageButton,RBX::GuiImageButton>(boost::shared_ptr<RBX::GuiImageButton> const*,RBX::GuiImageButton *)const (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "void boost::enable_shared_from_this<RBX::Reflection::DescribedBase>::_internal_accept_owner<RBX::GuiImageButton,RBX::GuiImageButton>(rbx_core::SharedPtr<RBX::GuiImageButton> const*,RBX::GuiImageButton *)const")]
pub fn stub_0x524588() -> ! {
    todo!("0x524588 void boost::enable_shared_from_this<RBX::Reflection::DescribedBase>::_internal_accept_owner<RBX::GuiImageButton,RBX::GuiImageButton>(boost::shared_ptr<RBX::GuiImageButton> const*,RBX::GuiImageButton *)const")
}

// 0x524670 — __ZN5boost6detail12shared_countC2IPN3RBX14GuiImageButtonENS3_9CreatableINS3_8InstanceEE7DeleterEEET_T0_
// type: int __fastcall(int, int, int, int, void *, int)
// was: boost::detail::shared_count::shared_count<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>(RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "boost::detail::shared_count::shared_count<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>(RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter)")]
pub fn stub_0x524670() -> ! {
    todo!("0x524670 boost::detail::shared_count::shared_count<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>(RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter)")
}

// 0x524778 — __ZN5boost6detail18sp_counted_impl_pdIPN3RBX14GuiImageButtonENS2_9CreatableINS2_8InstanceEE7DeleterEED1Ev
// was: boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::~sp_counted_impl_pd() (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::~sp_counted_impl_pd()")]
pub fn stub_0x524778() -> ! {
    todo!("0x524778 boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::~sp_counted_impl_pd()")
}

// 0x52477c — __ZN5boost6detail18sp_counted_impl_pdIPN3RBX14GuiImageButtonENS2_9CreatableINS2_8InstanceEE7DeleterEED0Ev
// was: boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::~sp_counted_impl_pd() (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::~sp_counted_impl_pd()")]
pub fn stub_0x52477c() -> ! {
    todo!("0x52477c boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::~sp_counted_impl_pd()")
}

// 0x524780 — __ZN5boost6detail18sp_counted_impl_pdIPN3RBX14GuiImageButtonENS2_9CreatableINS2_8InstanceEE7DeleterEE7disposeEv
// was: boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::dispose(void) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::dispose(void)")]
pub fn stub_0x524780() -> ! {
    todo!("0x524780 boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::dispose(void)")
}

// 0x5247a0 — __ZN5boost6detail18sp_counted_impl_pdIPN3RBX14GuiImageButtonENS2_9CreatableINS2_8InstanceEE7DeleterEE11get_deleterERKSt9type_info
// was: boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::get_deleter(std::type_info const&) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::get_deleter(std::type_info const&)")]
pub fn stub_0x5247a0() -> ! {
    todo!("0x5247a0 boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::get_deleter(std::type_info const&)")
}

// 0x5247b8 — __ZN5boost6detail18sp_counted_impl_pdIPN3RBX14GuiImageButtonENS2_9CreatableINS2_8InstanceEE7DeleterEE19get_untyped_deleterEv
// was: boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::get_untyped_deleter(void) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::get_untyped_deleter(void)")]
pub fn stub_0x5247b8() -> ! {
    todo!("0x5247b8 boost::detail::sp_counted_impl_pd<RBX::GuiImageButton *,RBX::Creatable<RBX::Instance>::Deleter>::get_untyped_deleter(void)")
}

// 0x525700 — __ZNSt4pairIKSsN3RBX10GuiBuilder4DataEEC2ISsS3_EERKS_IT_T0_E
#[doc(alias = "std::pair<std::string const,RBX::GuiBuilder::Data>::pair<std::string,RBX::GuiBuilder::Data>(std::pair const&<std::string,RBX::GuiBuilder::Data>)")]
pub fn stub_0x525700() -> ! {
    todo!("0x525700 std::pair<std::string const,RBX::GuiBuilder::Data>::pair<std::string,RBX::GuiBuilder::Data>(std::pair const&<std::string,RBX::GuiBuilder::Data>)")
}

// 0x5257e0 — __ZNSt8_Rb_treeISsSt4pairIKSsN3RBX10GuiBuilder4DataEESt10_Select1stIS5_ESt4lessISsESaIS5_EE16_M_insert_uniqueERKS5_
// type: int __fastcall(int, int, int)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,RBX::GuiBuilder::Data>,std::_Select1st<std::pair<std::string const,RBX::GuiBuilder::Data>>,std::less<std::string>,std::allocator<std::pair<std::string const,RBX::GuiBuilder::Data>>>::_M_insert_unique(std::pair<std::string const,RBX::GuiBuilder::Data> const&)")]
pub fn stub_0x5257e0() -> ! {
    todo!("0x5257e0 std::_Rb_tree<std::string,std::pair<std::string const,RBX::GuiBuilder::Data>,std::_Select1st<std::pair<std::string const,RBX::GuiBuilder::Data>>,std::less<std::string>,std::allocator<std::pair<std::string const,RBX::GuiBuilder::Data>>>::_M_insert_unique(std::pair<std::string const,RBX::GuiBuilder::Data> const&)")
}

// 0x525864 — __ZNSt8_Rb_treeISsSt4pairIKSsN3RBX10GuiBuilder4DataEESt10_Select1stIS5_ESt4lessISsESaIS5_EE9_M_insertEPSt18_Rb_tree_node_baseSD_RKS5_
// type: int __fastcall(int, int, int, int)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,RBX::GuiBuilder::Data>,std::_Select1st<std::pair<std::string const,RBX::GuiBuilder::Data>>,std::less<std::string>,std::allocator<std::pair<std::string const,RBX::GuiBuilder::Data>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<std::string const,RBX::GuiBuilder::Data> const&)")]
pub fn stub_0x525864() -> ! {
    todo!("0x525864 std::_Rb_tree<std::string,std::pair<std::string const,RBX::GuiBuilder::Data>,std::_Select1st<std::pair<std::string const,RBX::GuiBuilder::Data>>,std::less<std::string>,std::allocator<std::pair<std::string const,RBX::GuiBuilder::Data>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<std::string const,RBX::GuiBuilder::Data> const&)")
}

// 0x5258b4 — __ZNSt4pairISsN3RBX10GuiBuilder4DataEEC2ERKSsRKS2_
#[doc(alias = "std::pair<std::string,RBX::GuiBuilder::Data>::pair(std::string const&,RBX::GuiBuilder::Data const&)")]
pub fn stub_0x5258b4() -> ! {
    todo!("0x5258b4 std::pair<std::string,RBX::GuiBuilder::Data>::pair(std::string const&,RBX::GuiBuilder::Data const&)")
}

// 0x525cc8 — __ZN3RBX9GuiObject20tweenSizeAndPositionENS_5UDim2ES1_NS0_20TweenEasingDirectionENS0_16TweenEasingStyleEfbNS_3Lua15WeakFunctionRefE
// type: int __fastcall(int, int, int, int, int, int, int, struct _Unwind_Exception *lpuexcpt, int, int, int, float, int, RBX::Lua::WeakFunctionRef *, int, char, int, int, int, int, int, int, int, int, int, int)
#[doc(alias = "RBX::GuiObject::tweenSizeAndPosition(RBX::UDim2,RBX::UDim2,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,RBX::Lua::WeakFunctionRef)")]
pub fn stub_0x525cc8() -> ! {
    todo!("0x525cc8 RBX::GuiObject::tweenSizeAndPosition(RBX::UDim2,RBX::UDim2,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,RBX::Lua::WeakFunctionRef)")
}

// 0x525e50 — __ZN3RBX9GuiObject13tweenPositionENS_5UDim2ENS0_20TweenEasingDirectionENS0_16TweenEasingStyleEfbNS_3Lua15WeakFunctionRefE
// type: int __fastcall(int, int, int, int, int, int, int, float, int, RBX::Lua::WeakFunctionRef *, int, int, int, int, int, int, int, int, struct _Unwind_Exception *lpuexcpt, char, int, int, int, int, int, int, int, boost::detail::sp_counted_base *, char, int, boost::detail::sp_counted_base *, int, int, int, int, int, int, int, char, int, int, int, int, int, int, int)
#[doc(alias = "RBX::GuiObject::tweenPosition(RBX::UDim2,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,RBX::Lua::WeakFunctionRef)")]
pub fn stub_0x525e50() -> ! {
    todo!("0x525e50 RBX::GuiObject::tweenPosition(RBX::UDim2,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,RBX::Lua::WeakFunctionRef)")
}

// 0x526058 — __ZN3RBX9GuiObject9tweenSizeENS_5UDim2ENS0_20TweenEasingDirectionENS0_16TweenEasingStyleEfbNS_3Lua15WeakFunctionRefE
// type: int __fastcall(int, int, int, int, int, int, int, float, int, RBX::Lua::WeakFunctionRef *, int, int, int, int, int, int, int, int, struct _Unwind_Exception *lpuexcpt, char, int, int, int, int, int, int, int, boost::detail::sp_counted_base *, char, int, boost::detail::sp_counted_base *, int, int, int, int, int, int, int, char, int, int, int, int, int, int, int)
#[doc(alias = "RBX::GuiObject::tweenSize(RBX::UDim2,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,RBX::Lua::WeakFunctionRef)")]
pub fn stub_0x526058() -> ! {
    todo!("0x526058 RBX::GuiObject::tweenSize(RBX::UDim2,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,RBX::Lua::WeakFunctionRef)")
}

// 0x526260 — __ZN3RBX9GuiObject7setSizeENS_5UDim2E
// type: int __fastcall(_DWORD, _DWORD, _DWORD, _DWORD, _DWORD)
#[doc(alias = "RBX::GuiObject::setSize(RBX::UDim2)")]
pub fn stub_0x526260() -> ! {
    todo!("0x526260 RBX::GuiObject::setSize(RBX::UDim2)")
}

// 0x5262dc — __ZN3RBX9GuiObject11setPositionENS_5UDim2E
// type: int __fastcall(_DWORD, _DWORD, _DWORD, _DWORD, _DWORD)
#[doc(alias = "RBX::GuiObject::setPosition(RBX::UDim2)")]
pub fn stub_0x5262dc() -> ! {
    todo!("0x5262dc RBX::GuiObject::setPosition(RBX::UDim2)")
}

// 0x526358 — __ZN3RBX9GuiObject18setBorderSizePixelEi
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, int)
#[doc(alias = "RBX::GuiObject::setBorderSizePixel(int)")]
pub fn stub_0x526358() -> ! {
    todo!("0x526358 RBX::GuiObject::setBorderSizePixel(int)")
}

// 0x526398 — __ZN3RBX9GuiObject9setZIndexEi
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, int)
#[doc(alias = "RBX::GuiObject::setZIndex(int)")]
pub fn stub_0x526398() -> ! {
    todo!("0x526398 RBX::GuiObject::setZIndex(int)")
}

// 0x5263ec — __ZN3RBX9GuiObject17setSizeConstraintENS0_14SizeConstraintE
// type: int __fastcall(_DWORD, _DWORD)
#[doc(alias = "RBX::GuiObject::setSizeConstraint(RBX::GuiObject::SizeConstraint)")]
pub fn stub_0x5263ec() -> ! {
    todo!("0x5263ec RBX::GuiObject::setSizeConstraint(RBX::GuiObject::SizeConstraint)")
}

// 0x526424 — __ZN3RBX9GuiObject14setBorderColorENS_10BrickColorE
#[doc(alias = "RBX::GuiObject::setBorderColor(RBX::BrickColor)")]
pub fn stub_0x526424() -> ! {
    todo!("0x526424 RBX::GuiObject::setBorderColor(RBX::BrickColor)")
}

// 0x526444 — __ZN3RBX9GuiObject15setBorderColor3EN3G3D6Color3E
#[doc(alias = "RBX::GuiObject::setBorderColor3(G3D::Color3)")]
pub fn stub_0x526444() -> ! {
    todo!("0x526444 RBX::GuiObject::setBorderColor3(G3D::Color3)")
}

// 0x5264c4 — __ZN3RBX9GuiObject18setBackgroundColorENS_10BrickColorE
#[doc(alias = "RBX::GuiObject::setBackgroundColor(RBX::BrickColor)")]
pub fn stub_0x5264c4() -> ! {
    todo!("0x5264c4 RBX::GuiObject::setBackgroundColor(RBX::BrickColor)")
}

// 0x5264e4 — __ZN3RBX9GuiObject19setBackgroundColor3EN3G3D6Color3E
#[doc(alias = "RBX::GuiObject::setBackgroundColor3(G3D::Color3)")]
pub fn stub_0x5264e4() -> ! {
    todo!("0x5264e4 RBX::GuiObject::setBackgroundColor3(G3D::Color3)")
}

// 0x526564 — __ZN3RBX9GuiObject25setBackgroundTransparencyEf
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, float)
#[doc(alias = "RBX::GuiObject::setBackgroundTransparency(float)")]
pub fn stub_0x526564() -> ! {
    todo!("0x526564 RBX::GuiObject::setBackgroundTransparency(float)")
}

// 0x526590 — __ZN3RBX9GuiObject12setDraggableEb
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, bool)
#[doc(alias = "RBX::GuiObject::setDraggable(bool)")]
pub fn stub_0x526590() -> ! {
    todo!("0x526590 RBX::GuiObject::setDraggable(bool)")
}

// 0x5265b0 — __ZN3RBX9GuiObject11setClippingEb
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, bool)
#[doc(alias = "RBX::GuiObject::setClipping(bool)")]
pub fn stub_0x5265b0() -> ! {
    todo!("0x5265b0 RBX::GuiObject::setClipping(bool)")
}

// 0x5265d0 — __ZN3RBX9GuiObject10setVisibleEb
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, bool)
#[doc(alias = "RBX::GuiObject::setVisible(bool)")]
pub fn stub_0x5265d0() -> ! {
    todo!("0x5265d0 RBX::GuiObject::setVisible(bool)")
}

// 0x526608 — __ZN3RBX9GuiObject9setActiveEb
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, bool)
#[doc(alias = "RBX::GuiObject::setActive(bool)")]
pub fn stub_0x526608() -> ! {
    todo!("0x526608 RBX::GuiObject::setActive(bool)")
}

// 0x526640 — __ZN3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEEC1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::EnumDesc(void)")]
pub fn stub_0x526640() -> ! {
    todo!("0x526640 RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::EnumDesc(void)")
}

// 0x526644 — __ZN3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEEC2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::EnumDesc(void)")]
pub fn stub_0x526644() -> ! {
    todo!("0x526644 RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::EnumDesc(void)")
}

// 0x526820 — __ZN3RBX10Reflection8EnumDescINS_9GuiObject16TweenEasingStyleEEC1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingStyle>::EnumDesc(void)")]
pub fn stub_0x526820() -> ! {
    todo!("0x526820 RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingStyle>::EnumDesc(void)")
}

// 0x526824 — __ZN3RBX10Reflection8EnumDescINS_9GuiObject16TweenEasingStyleEEC2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingStyle>::EnumDesc(void)")]
pub fn stub_0x526824() -> ! {
    todo!("0x526824 RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingStyle>::EnumDesc(void)")
}

// 0x526a70 — __ZN3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEEC1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::EnumDesc(void)")]
pub fn stub_0x526a70() -> ! {
    todo!("0x526a70 RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::EnumDesc(void)")
}

// 0x526a74 — __ZN3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEEC2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::EnumDesc(void)")]
pub fn stub_0x526a74() -> ! {
    todo!("0x526a74 RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::EnumDesc(void)")
}

// 0x526c38 — __ZN3RBX15StringConverterINS_9GuiObject16TweenEasingStyleEE14convertToValueERKSsRS2_
#[doc(alias = "RBX::StringConverter<RBX::GuiObject::TweenEasingStyle>::convertToValue(std::string const&,RBX::GuiObject::TweenEasingStyle&)")]
pub fn stub_0x526c38() -> ! {
    todo!("0x526c38 RBX::StringConverter<RBX::GuiObject::TweenEasingStyle>::convertToValue(std::string const&,RBX::GuiObject::TweenEasingStyle&)")
}

// 0x526c84 — __ZN3RBX15StringConverterINS_9GuiObject20TweenEasingDirectionEE14convertToValueERKSsRS2_
#[doc(alias = "RBX::StringConverter<RBX::GuiObject::TweenEasingDirection>::convertToValue(std::string const&,RBX::GuiObject::TweenEasingDirection&)")]
pub fn stub_0x526c84() -> ! {
    todo!("0x526c84 RBX::StringConverter<RBX::GuiObject::TweenEasingDirection>::convertToValue(std::string const&,RBX::GuiObject::TweenEasingDirection&)")
}

// 0x526cd0 — __ZN3RBX9GuiObjectC2EPKcb
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, const char *, bool)
#[doc(alias = "RBX::GuiObject::GuiObject(char const*,bool)")]
pub fn stub_0x526cd0() -> ! {
    todo!("0x526cd0 RBX::GuiObject::GuiObject(char const*,bool)")
}

// 0x527344 — __ZN3RBX9GuiObject11UpdateTweenERNS0_5TweenEPS0_N5boost8functionIFvS3_NS_5UDim2EEEEf
// was: RBX::GuiObject::UpdateTween(RBX::GuiObject::Tween &,RBX::GuiObject*,boost::function<void ()(RBX::GuiObject*,RBX::UDim2)>,float) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "RBX::GuiObject::UpdateTween(RBX::GuiObject::Tween &,RBX::GuiObject*,boost::function<void ()(RBX::GuiObject*,RBX::UDim2)>,float)")]
pub fn stub_0x527344() -> ! {
    todo!("0x527344 RBX::GuiObject::UpdateTween(RBX::GuiObject::Tween &,RBX::GuiObject*,boost::function<void ()(RBX::GuiObject*,RBX::UDim2)>,float)")
}

// 0x52757c — __ZN3RBXL14InvokeCallbackEN5boost8functionIFvNS_9GuiObject11TweenStatusEEEES3_
// was: RBX::InvokeCallback(boost::function<void ()(RBX::GuiObject::TweenStatus)>,RBX::GuiObject::TweenStatus) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "RBX::InvokeCallback(boost::function<void ()(RBX::GuiObject::TweenStatus)>,RBX::GuiObject::TweenStatus)")]
pub fn stub_0x52757c() -> ! {
    todo!("0x52757c RBX::InvokeCallback(boost::function<void ()(RBX::GuiObject::TweenStatus)>,RBX::GuiObject::TweenStatus)")
}

// 0x527580 — __ZN3RBX9GuiObject16TweenInterpolateENS0_20TweenEasingDirectionENS0_16TweenEasingStyleEffRKNS_5UDim2ES5_
// type: int __fastcall(int, int, int, int, float, int, int)
#[doc(alias = "RBX::GuiObject::TweenInterpolate(RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,float,RBX::UDim2 const&,RBX::UDim2 const&)")]
pub fn stub_0x527580() -> ! {
    todo!("0x527580 RBX::GuiObject::TweenInterpolate(RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,float,RBX::UDim2 const&,RBX::UDim2 const&)")
}

// 0x527f50 — __ZN3RBX9GuiObject18tweenPositionDelayENS_5UDim2ES1_fNS0_20TweenEasingDirectionENS0_16TweenEasingStyleEfbN5boost8functionIFvNS0_11TweenStatusEEEE
// type: int __fastcall(int, int, int, int, int, int, int, int, int, float, int, int, float, struct _Unwind_Exception *lpuexcpt, int, int, int, int, int, int, int, int)
// was: RBX::GuiObject::tweenPositionDelay(RBX::UDim2,RBX::UDim2,float,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,boost::function<void ()(RBX::GuiObject::TweenStatus)>) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "RBX::GuiObject::tweenPositionDelay(RBX::UDim2,RBX::UDim2,float,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,boost::function<void ()(RBX::GuiObject::TweenStatus)>)")]
pub fn stub_0x527f50() -> ! {
    todo!("0x527f50 RBX::GuiObject::tweenPositionDelay(RBX::UDim2,RBX::UDim2,float,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,boost::function<void ()(RBX::GuiObject::TweenStatus)>)")
}

// 0x528060 — __ZN3RBXL25InvokeTweenStatusCallbackEN5boost8weak_ptrINS_9GuiObjectEEENS_3Lua15WeakFunctionRefENS2_11TweenStatusE
// was: RBX::InvokeTweenStatusCallback(boost::weak_ptr<RBX::GuiObject>,RBX::Lua::WeakFunctionRef,RBX::GuiObject::TweenStatus) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "RBX::InvokeTweenStatusCallback(rbx_core::WeakPtr<RBX::GuiObject>,RBX::Lua::WeakFunctionRef,RBX::GuiObject::TweenStatus)")]
pub fn stub_0x528060() -> ! {
    todo!("0x528060 RBX::InvokeTweenStatusCallback(boost::weak_ptr<RBX::GuiObject>,RBX::Lua::WeakFunctionRef,RBX::GuiObject::TweenStatus)")
}

// 0x528528 — __ZN3RBX9GuiObject13tweenPositionENS_5UDim2ENS0_20TweenEasingDirectionENS0_16TweenEasingStyleEfbb
// type: int __fastcall(int, int, int, int, int, int, int, float, int, int, int, int, int, int, int, int, int, int, int, struct _Unwind_Exception *lpuexcpt, char, int, int, int, int, int, int, int, boost::detail::sp_counted_base *, char, int, boost::detail::sp_counted_base *, int, int, int, int, int, int, int, char, int, int, int, char, int, int, int, int, int, int, int, boost::detail::sp_counted_base *, char, int, boost::detail::sp_counted_base *, int, int, int, int, int, int, int, char, int, int, int, int, int, int, int)
#[doc(alias = "RBX::GuiObject::tweenPosition(RBX::UDim2,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,bool)")]
pub fn stub_0x528528() -> ! {
    todo!("0x528528 RBX::GuiObject::tweenPosition(RBX::UDim2,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,bool)")
}

// 0x5288b0 — __ZN3RBX9GuiObject18tweenPositionDelayENS_5UDim2ES1_fNS0_20TweenEasingDirectionENS0_16TweenEasingStyleEfbN5boost8functionIFvNS0_11TweenStatusEEEEPNS_12TweenServiceE
// type: int __fastcall(int, int, int, int, int, int, int, int, int, float, int, int, float, int, int, int)
// was: RBX::GuiObject::tweenPositionDelay(RBX::UDim2,RBX::UDim2,float,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,boost::function<void ()(RBX::GuiObject::TweenStatus)>,RBX::TweenService *) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "RBX::GuiObject::tweenPositionDelay(RBX::UDim2,RBX::UDim2,float,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,boost::function<void ()(RBX::GuiObject::TweenStatus)>,RBX::TweenService *)")]
pub fn stub_0x5288b0() -> ! {
    todo!("0x5288b0 RBX::GuiObject::tweenPositionDelay(RBX::UDim2,RBX::UDim2,float,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,boost::function<void ()(RBX::GuiObject::TweenStatus)>,RBX::TweenService *)")
}

// 0x528c4c — __ZN3RBXL22InvokeRemoveOnTweenEndEN5boost8weak_ptrINS_9GuiObjectEEENS_3Lua15WeakFunctionRefE
// was: RBX::InvokeRemoveOnTweenEnd(boost::weak_ptr<RBX::GuiObject>,RBX::Lua::WeakFunctionRef) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "RBX::InvokeRemoveOnTweenEnd(rbx_core::WeakPtr<RBX::GuiObject>,RBX::Lua::WeakFunctionRef)")]
pub fn stub_0x528c4c() -> ! {
    todo!("0x528c4c RBX::InvokeRemoveOnTweenEnd(boost::weak_ptr<RBX::GuiObject>,RBX::Lua::WeakFunctionRef)")
}

// 0x528d14 — __ZN3RBX9GuiObject14tweenSizeDelayENS_5UDim2ES1_fNS0_20TweenEasingDirectionENS0_16TweenEasingStyleEfbN5boost8functionIFvNS0_11TweenStatusEEEE
// type: int __fastcall(int, int, int, int, int, int, int, int, int, float, int, int, float, int, int)
// was: RBX::GuiObject::tweenSizeDelay(RBX::UDim2,RBX::UDim2,float,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,boost::function<void ()(RBX::GuiObject::TweenStatus)>) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "RBX::GuiObject::tweenSizeDelay(RBX::UDim2,RBX::UDim2,float,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,boost::function<void ()(RBX::GuiObject::TweenStatus)>)")]
pub fn stub_0x528d14() -> ! {
    todo!("0x528d14 RBX::GuiObject::tweenSizeDelay(RBX::UDim2,RBX::UDim2,float,RBX::GuiObject::TweenEasingDirection,RBX::GuiObject::TweenEasingStyle,float,bool,boost::function<void ()(RBX::GuiObject::TweenStatus)>)")
}

// 0x5290b4 — __ZN3RBX9GuiObject9tweenStepERKd
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, const double *)
#[doc(alias = "RBX::GuiObject::tweenStep(double const&)")]
pub fn stub_0x5290b4() -> ! {
    todo!("0x5290b4 RBX::GuiObject::tweenStep(double const&)")
}

// 0x529284 — __ZN3RBX9GuiObject18setServerGuiObjectEv
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this)
#[doc(alias = "RBX::GuiObject::setServerGuiObject(void)")]
pub fn stub_0x529284() -> ! {
    todo!("0x529284 RBX::GuiObject::setServerGuiObject(void)")
}

// 0x529314 — __ZN3RBX9GuiObject13getWindowRectEPNS_9GuiBase2dE
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, RBX::GuiBase2d *)
#[doc(alias = "RBX::GuiObject::getWindowRect(RBX::GuiBase2d *)")]
pub fn stub_0x529314() -> ! {
    todo!("0x529314 RBX::GuiObject::getWindowRect(RBX::GuiBase2d *)")
}

// 0x5293fc — __ZN3RBX9GuiObject12handleResizeERKN3G3D6Rect2DEb
// type: int __fastcall(RBX::GuiBase2d *)
#[doc(alias = "RBX::GuiObject::handleResize(G3D::Rect2D const&,bool)")]
pub fn stub_0x5293fc() -> ! {
    todo!("0x5293fc RBX::GuiObject::handleResize(G3D::Rect2D const&,bool)")
}

// 0x529438 — __ZN3RBX9GuiObject10handleDragEN3G3D7Vector2E
#[doc(alias = "RBX::GuiObject::handleDrag(G3D::Vector2)")]
pub fn stub_0x529438() -> ! {
    todo!("0x529438 RBX::GuiObject::handleDrag(G3D::Vector2)")
}

// 0x5294c8 — __ZN3RBX9GuiObject34recalculateAbsoluteSizeAndPositionERKN3G3D6Rect2DE
#[doc(alias = "RBX::GuiObject::recalculateAbsoluteSizeAndPosition(G3D::Rect2D const&)")]
pub fn stub_0x5294c8() -> ! {
    todo!("0x5294c8 RBX::GuiObject::recalculateAbsoluteSizeAndPosition(G3D::Rect2D const&)")
}

// 0x529650 — __ZN3RBX9GuiObject11forceResizeEv
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this)
#[doc(alias = "RBX::GuiObject::forceResize(void)")]
pub fn stub_0x529650() -> ! {
    todo!("0x529650 RBX::GuiObject::forceResize(void)")
}

// 0x5296a4 — __ZN3RBX9GuiObject14checkForResizeEv
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this)
#[doc(alias = "RBX::GuiObject::checkForResize(void)")]
pub fn stub_0x5296a4() -> ! {
    todo!("0x5296a4 RBX::GuiObject::checkForResize(void)")
}

// 0x5296f8 — __ZN3RBX9GuiObject21firstAncestorClippingEv
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this)
#[doc(alias = "RBX::GuiObject::firstAncestorClipping(void)")]
pub fn stub_0x5296f8() -> ! {
    todo!("0x5296f8 RBX::GuiObject::firstAncestorClipping(void)")
}

// 0x52973c — __ZN3RBX9GuiObject14getClippedRectEv
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this)
#[doc(alias = "RBX::GuiObject::getClippedRect(void)")]
pub fn stub_0x52973c() -> ! {
    todo!("0x52973c RBX::GuiObject::getClippedRect(void)")
}

// 0x529798 — __ZN3RBX9GuiObject17onPropertyChangedERKNS_10Reflection18PropertyDescriptorE
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, const RBX::Reflection::PropertyDescriptor *)
#[doc(alias = "RBX::GuiObject::onPropertyChanged(RBX::Reflection::PropertyDescriptor const&)")]
pub fn stub_0x529798() -> ! {
    todo!("0x529798 RBX::GuiObject::onPropertyChanged(RBX::Reflection::PropertyDescriptor const&)")
}

// 0x5297e8 — __ZN3RBX9GuiObject17onAncestorChangedERKNS_15AncestorChangedE
#[doc(alias = "RBX::GuiObject::onAncestorChanged(RBX::AncestorChanged const&)")]
pub fn stub_0x5297e8() -> ! {
    todo!("0x5297e8 RBX::GuiObject::onAncestorChanged(RBX::AncestorChanged const&)")
}

// 0x52981c — __ZNK3RBX9GuiObject12askSetParentEPKNS_8InstanceE
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, const RBX::Instance *)
#[doc(alias = "RBX::GuiObject::askSetParent(RBX::Instance const*)const")]
pub fn stub_0x52981c() -> ! {
    todo!("0x52981c RBX::GuiObject::askSetParent(RBX::Instance const*)const")
}

// 0x52986c — __ZNK3RBX9GuiObject25getRenderBackgroundColor4Ev
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this)
#[doc(alias = "RBX::GuiObject::getRenderBackgroundColor4(void)const")]
pub fn stub_0x52986c() -> ! {
    todo!("0x52986c RBX::GuiObject::getRenderBackgroundColor4(void)const")
}

// 0x5298b0 — __ZN3RBX9GuiObject14legacyRender2dEPNS_5AdornERKN3G3D6Rect2DE
// type: int __fastcall(_DWORD, _DWORD, _DWORD)
#[doc(alias = "RBX::GuiObject::legacyRender2d(RBX::Adorn *,G3D::Rect2D const&)")]
pub fn stub_0x5298b0() -> ! {
    todo!("0x5298b0 RBX::GuiObject::legacyRender2d(RBX::Adorn *,G3D::Rect2D const&)")
}

// 0x5298dc — __ZN3RBX9GuiObject8render2dEPNS_5AdornE
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, RBX::Adorn *)
#[doc(alias = "RBX::GuiObject::render2d(RBX::Adorn *)")]
pub fn stub_0x5298dc() -> ! {
    todo!("0x5298dc RBX::GuiObject::render2d(RBX::Adorn *)")
}

// 0x529948 — __ZN3RBX9GuiObject12render2dImplEPNS_5AdornERKN3G3D6Color4E
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, RBX::Adorn *, const G3D::Color4 *)
#[doc(alias = "RBX::GuiObject::render2dImpl(RBX::Adorn *,G3D::Color4 const&)")]
pub fn stub_0x529948() -> ! {
    todo!("0x529948 RBX::GuiObject::render2dImpl(RBX::Adorn *,G3D::Color4 const&)")
}

// 0x529960 — __ZThn96_N3RBX9GuiObject8render2dEPNS_5AdornE
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, RBX::Adorn *)
#[doc(alias = "`non-virtual thunk to'RBX::GuiObject::render2d(RBX::Adorn *)")]
pub fn stub_0x529960() -> ! {
    todo!("0x529960 `non-virtual thunk to'RBX::GuiObject::render2d(RBX::Adorn *)")
}

// 0x529968 — __ZN3RBX9GuiObject12render2dImplEPNS_5AdornERKN3G3D6Color4ERNS3_6Rect2DE
#[doc(alias = "RBX::GuiObject::render2dImpl(RBX::Adorn *,G3D::Color4 const&,G3D::Rect2D &)")]
pub fn stub_0x529968() -> ! {
    todo!("0x529968 RBX::GuiObject::render2dImpl(RBX::Adorn *,G3D::Color4 const&,G3D::Rect2D &)")
}

// 0x529a50 — __ZN3RBX9GuiObject18renderSelectionBoxEPNS_5AdornE
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, RBX::Adorn *)
#[doc(alias = "RBX::GuiObject::renderSelectionBox(RBX::Adorn *)")]
pub fn stub_0x529a50() -> ! {
    todo!("0x529a50 RBX::GuiObject::renderSelectionBox(RBX::Adorn *)")
}

// 0x529b14 — __ZN3RBX9GuiObject12Scale9Rect2DERKN3G3D6Rect2DEff
#[doc(alias = "RBX::GuiObject::Scale9Rect2D(G3D::Rect2D const&,float,float)")]
pub fn stub_0x529b14() -> ! {
    todo!("0x529b14 RBX::GuiObject::Scale9Rect2D(G3D::Rect2D const&,float,float)")
}

// 0x529bd4 — __ZN3RBX9GuiObject18render2dScale9ImplEPNS_5AdornERKNS_9TextureIdERKN3G3D12Vector2int16ERKNS6_7Vector2ERNS_12GuiDrawImageERNS6_6Rect2DEPS0_
// type: int __fastcall(int, int, int, int, int, RBX::GuiDrawImage *, int, int)
#[doc(alias = "RBX::GuiObject::render2dScale9Impl(RBX::Adorn *,RBX::TextureId const&,G3D::Vector2int16 const&,G3D::Vector2 const&,RBX::GuiDrawImage &,G3D::Rect2D &,RBX::GuiObject*)")]
pub fn stub_0x529bd4() -> ! {
    todo!("0x529bd4 RBX::GuiObject::render2dScale9Impl(RBX::Adorn *,RBX::TextureId const&,G3D::Vector2int16 const&,G3D::Vector2 const&,RBX::GuiDrawImage &,G3D::Rect2D &,RBX::GuiObject*)")
}

// 0x529eac — __ZN3RBX9GuiObject16render2dTextImplEPNS_5AdornERKN3G3D6Color4ERKSsNS_11TextService4FontENS9_8FontSizeES6_S6_bbNS9_10XAlignmentENS9_10YAlignmentE
// type: int __fastcall(_DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD)
#[doc(alias = "RBX::GuiObject::render2dTextImpl(RBX::Adorn *,G3D::Color4 const&,std::string const&,RBX::TextService::Font,RBX::TextService::FontSize,G3D::Color4 const&,G3D::Color4 const&,bool,bool,RBX::TextService::XAlignment,RBX::TextService::YAlignment)")]
pub fn stub_0x529eac() -> ! {
    todo!("0x529eac RBX::GuiObject::render2dTextImpl(RBX::Adorn *,G3D::Color4 const&,std::string const&,RBX::TextService::Font,RBX::TextService::FontSize,G3D::Color4 const&,G3D::Color4 const&,bool,bool,RBX::TextService::XAlignment,RBX::TextService::YAlignment)")
}

// 0x529f10 — __ZN3RBX9GuiObject16render2dTextImplEPNS_5AdornERKN3G3D6Rect2DERKSsNS_11TextService4FontENS9_8FontSizeERKNS3_6Color4ESE_bbNS9_10XAlignmentENS9_10YAlignmentE
// type: int __fastcall(int, int, int, int, int, int, int, int, int, G3D::Vector2 *, int, int, int)
#[doc(alias = "RBX::GuiObject::render2dTextImpl(RBX::Adorn *,G3D::Rect2D const&,std::string const&,RBX::TextService::Font,RBX::TextService::FontSize,G3D::Color4 const&,G3D::Color4 const&,bool,bool,RBX::TextService::XAlignment,RBX::TextService::YAlignment)")]
pub fn stub_0x529f10() -> ! {
    todo!("0x529f10 RBX::GuiObject::render2dTextImpl(RBX::Adorn *,G3D::Rect2D const&,std::string const&,RBX::TextService::Font,RBX::TextService::FontSize,G3D::Color4 const&,G3D::Color4 const&,bool,bool,RBX::TextService::XAlignment,RBX::TextService::YAlignment)")
}

// 0x52a12c — __ZN3RBX9GuiObject17getScaledFontSizeERKN3G3D6Rect2DERKSsNS_11TextService4FontEbf
// type: int __fastcall(int, int, int, int, int, float, struct _Unwind_Exception *lpuexcpt, int, int, int, int, int, int, boost::detail::sp_counted_base *, int, int, int, int)
#[doc(alias = "RBX::GuiObject::getScaledFontSize(G3D::Rect2D const&,std::string const&,RBX::TextService::Font,bool,float)")]
pub fn stub_0x52a12c() -> ! {
    todo!("0x52a12c RBX::GuiObject::getScaledFontSize(G3D::Rect2D const&,std::string const&,RBX::TextService::Font,bool,float)")
}

// 0x52a2f0 — __ZN3RBX9GuiObject7processERKNS_8GuiEventE
#[doc(alias = "RBX::GuiObject::process(RBX::GuiEvent const&)")]
pub fn stub_0x52a2f0() -> ! {
    todo!("0x52a2f0 RBX::GuiObject::process(RBX::GuiEvent const&)")
}

// 0x52a328 — __ZThn92_N3RBX9GuiObject7processERKNS_8GuiEventE
#[doc(alias = "`non-virtual thunk to'RBX::GuiObject::process(RBX::GuiEvent const&)")]
pub fn stub_0x52a328() -> ! {
    todo!("0x52a328 `non-virtual thunk to'RBX::GuiObject::process(RBX::GuiEvent const&)")
}

// 0x52a334 — __ZN3RBX9GuiObject11mouseIsOverERKN3G3D7Vector2E
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this, const G3D::Vector2 *)
#[doc(alias = "RBX::GuiObject::mouseIsOver(G3D::Vector2 const&)")]
pub fn stub_0x52a334() -> ! {
    todo!("0x52a334 RBX::GuiObject::mouseIsOver(G3D::Vector2 const&)")
}

// 0x52a414 — __ZN3RBX9GuiObject15processKeyEventERKNS_8GuiEventE
#[doc(alias = "RBX::GuiObject::processKeyEvent(RBX::GuiEvent const&)")]
pub fn stub_0x52a414() -> ! {
    todo!("0x52a414 RBX::GuiObject::processKeyEvent(RBX::GuiEvent const&)")
}

// 0x52a420 — __ZN3RBX9GuiObject17processMouseEventERKNS_8GuiEventE
// type: int __fastcall(int, RBX::GuiObject *this)
#[doc(alias = "RBX::GuiObject::processMouseEvent(RBX::GuiEvent const&)")]
pub fn stub_0x52a420() -> ! {
    todo!("0x52a420 RBX::GuiObject::processMouseEvent(RBX::GuiEvent const&)")
}

// 0x52a654 — __ZN3RBX9GuiObject18isCurrentlyVisibleEv
// type: _DWORD __fastcall(RBX::GuiObject *__hidden this)
#[doc(alias = "RBX::GuiObject::isCurrentlyVisible(void)")]
pub fn stub_0x52a654() -> ! {
    todo!("0x52a654 RBX::GuiObject::isCurrentlyVisible(void)")
}

// 0x52a6e0 — __ZN3RBX9GuiButton7setVerbESs
#[doc(alias = "RBX::GuiButton::setVerb(std::string)")]
pub fn stub_0x52a6e0() -> ! {
    todo!("0x52a6e0 RBX::GuiButton::setVerb(std::string)")
}

// 0x52a724 — __ZN3RBX9GuiButton18setAutoButtonColorEb
// type: _DWORD __fastcall(RBX::GuiButton *__hidden this, bool)
#[doc(alias = "RBX::GuiButton::setAutoButtonColor(bool)")]
pub fn stub_0x52a724() -> ! {
    todo!("0x52a724 RBX::GuiButton::setAutoButtonColor(bool)")
}

// 0x52a744 — __ZN3RBX9GuiButton11setSelectedEb
// type: _DWORD __fastcall(RBX::GuiButton *__hidden this, bool)
#[doc(alias = "RBX::GuiButton::setSelected(bool)")]
pub fn stub_0x52a744() -> ! {
    todo!("0x52a744 RBX::GuiButton::setSelected(bool)")
}

// 0x52a764 — __ZN3RBX9GuiButton8setModalEb
// type: _DWORD __fastcall(RBX::GuiButton *__hidden this, bool)
#[doc(alias = "RBX::GuiButton::setModal(bool)")]
pub fn stub_0x52a764() -> ! {
    todo!("0x52a764 RBX::GuiButton::setModal(bool)")
}

// 0x52a784 — __ZN3RBX9GuiButton8setStyleENS0_5StyleE
// type: _DWORD __fastcall(RBX::GuiButton *__hidden this, Style)
#[doc(alias = "RBX::GuiButton::setStyle(RBX::GuiButton::Style)")]
pub fn stub_0x52a784() -> ! {
    todo!("0x52a784 RBX::GuiButton::setStyle(RBX::GuiButton::Style)")
}

// 0x52a9b8 — __ZN3RBX10Reflection8EnumDescINS_9GuiButton5StyleEEC1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiButton::Style>::EnumDesc(void)")]
pub fn stub_0x52a9b8() -> ! {
    todo!("0x52a9b8 RBX::Reflection::EnumDesc<RBX::GuiButton::Style>::EnumDesc(void)")
}

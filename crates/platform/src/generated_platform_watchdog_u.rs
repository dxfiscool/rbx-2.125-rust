//! platform — generated_platform_watchdog_u — 100 stubs EA-sorted asc fallback gap filler
//! Source: ida/export.json (85545 funcs) EA-sorted asc next 100 not yet stubbed in rbx_platform (fallback gap filler after watchdog_t 0x387874..0x38c368, gap 0x38c37c..0x3906d0, rbx_core::SharedPtr not boost — iOS/RobloxView fallback)
//! Batch: 100 stubs | range 0x38c37c..0x3906d0 | rbx_core::SharedPtr not boost
//! Distinct stub_ 36548/85545 -> 36648/85545 | platform 36548->36648 | uncovered 48998->48898

#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]

use rbx_core::SharedPtr;

const _: () = {
    let _ = core::marker::PhantomData::<SharedPtr<u8>>;
};

// 0x38c37c — __ZNK3RBX7UIEvent11isCtrlEventEv — RBX::UIEvent::isCtrlEvent(void)const
// type: int __fastcall(RBX::UIEvent *this)
#[doc(alias = "RBX::UIEvent::isCtrlEvent(void)const")]
#[doc(alias = "__ZNK3RBX7UIEvent11isCtrlEventEv")]
pub fn stub_38c37c() -> ! {
    todo!("0x38c37c __ZNK3RBX7UIEvent11isCtrlEventEv")
}

// 0x38c390 — __ZNK3RBX7UIEvent19isCarriageReturnKeyEv — RBX::UIEvent::isCarriageReturnKey(void)const
// type: bool __fastcall(RBX::UIEvent *this)
#[doc(alias = "RBX::UIEvent::isCarriageReturnKey(void)const")]
#[doc(alias = "__ZNK3RBX7UIEvent19isCarriageReturnKeyEv")]
pub fn stub_38c390() -> ! {
    todo!("0x38c390 __ZNK3RBX7UIEvent19isCarriageReturnKeyEv")
}

// 0x38c3ac — __ZNK3RBX7UIEvent11isDeleteKeyEv — RBX::UIEvent::isDeleteKey(void)const
// type: bool __fastcall(RBX::UIEvent *this)
#[doc(alias = "RBX::UIEvent::isDeleteKey(void)const")]
#[doc(alias = "__ZNK3RBX7UIEvent11isDeleteKeyEv")]
pub fn stub_38c3ac() -> ! {
    todo!("0x38c3ac __ZNK3RBX7UIEvent11isDeleteKeyEv")
}

// 0x38c3b8 — __ZNK3RBX7UIEvent14isBackspaceKeyEv — RBX::UIEvent::isBackspaceKey(void)const
// type: bool __fastcall(RBX::UIEvent *this)
#[doc(alias = "RBX::UIEvent::isBackspaceKey(void)const")]
#[doc(alias = "__ZNK3RBX7UIEvent14isBackspaceKeyEv")]
pub fn stub_38c3b8() -> ! {
    todo!("0x38c3b8 __ZNK3RBX7UIEvent14isBackspaceKeyEv")
}

// 0x38c3c4 — __ZNK3RBX7UIEvent10isClearKeyEv — RBX::UIEvent::isClearKey(void)const
// type: bool __fastcall(RBX::UIEvent *this)
#[doc(alias = "RBX::UIEvent::isClearKey(void)const")]
#[doc(alias = "__ZNK3RBX7UIEvent10isClearKeyEv")]
pub fn stub_38c3c4() -> ! {
    todo!("0x38c3c4 __ZNK3RBX7UIEvent10isClearKeyEv")
}

// 0x38c3d0 — __ZNK3RBX7UIEvent11isEscapeKeyEv — RBX::UIEvent::isEscapeKey(void)const
// type: bool __fastcall(RBX::UIEvent *this)
#[doc(alias = "RBX::UIEvent::isEscapeKey(void)const")]
#[doc(alias = "__ZNK3RBX7UIEvent11isEscapeKeyEv")]
pub fn stub_38c3d0() -> ! {
    todo!("0x38c3d0 __ZNK3RBX7UIEvent11isEscapeKeyEv")
}

// 0x38c3dc — __ZNK3RBX7UIEvent14isLeftArrowKeyEv — RBX::UIEvent::isLeftArrowKey(void)const
// type: bool __fastcall(RBX::UIEvent *this)
#[doc(alias = "RBX::UIEvent::isLeftArrowKey(void)const")]
#[doc(alias = "__ZNK3RBX7UIEvent14isLeftArrowKeyEv")]
pub fn stub_38c3dc() -> ! {
    todo!("0x38c3dc __ZNK3RBX7UIEvent14isLeftArrowKeyEv")
}

// 0x38c3ec — __ZNK3RBX7UIEvent15isRightArrowKeyEv — RBX::UIEvent::isRightArrowKey(void)const
// type: bool __fastcall(RBX::UIEvent *this)
#[doc(alias = "RBX::UIEvent::isRightArrowKey(void)const")]
#[doc(alias = "__ZNK3RBX7UIEvent15isRightArrowKeyEv")]
pub fn stub_38c3ec() -> ! {
    todo!("0x38c3ec __ZNK3RBX7UIEvent15isRightArrowKeyEv")
}

// 0x38c3fc — __GLOBAL__I_a_148 — `global constructor keyed to'_a_148
// type: int()
#[doc(alias = "global constructor keyed to_a_148")]
#[doc(alias = "__GLOBAL__I_a_148")]
pub fn stub_38c3fc() -> ! {
    todo!("0x38c3fc __GLOBAL__I_a_148")
}

// 0x38c434 — __ZN3RBX5Units20kmsAccelerationToRbxERKN3G3D7Vector3E — RBX::Units::kmsAccelerationToRbx(G3D::Vector3 const&)
// type: _DWORD *__fastcall(_DWORD *this, const Vector3 *)
#[doc(alias = "RBX::Units::kmsAccelerationToRbx(G3D::Vector3 const&)")]
#[doc(alias = "__ZN3RBX5Units20kmsAccelerationToRbxERKN3G3D7Vector3E")]
pub fn stub_38c434() -> ! {
    todo!("0x38c434 __ZN3RBX5Units20kmsAccelerationToRbxERKN3G3D7Vector3E")
}

// 0x38c464 — __ZN3RBX5Units13kmsForceToRbxEf — RBX::Units::kmsForceToRbx(float)
// type: unsigned __int32 __fastcall(RBX::Units *this, float)
#[doc(alias = "RBX::Units::kmsForceToRbx(float)")]
#[doc(alias = "__ZN3RBX5Units13kmsForceToRbxEf")]
pub fn stub_38c464() -> ! {
    todo!("0x38c464 __ZN3RBX5Units13kmsForceToRbxEf")
}

// 0x38c478 — __GLOBAL__I_a_149 — `global constructor keyed to'_a_149
// type: int()
#[doc(alias = "global constructor keyed to_a_149")]
#[doc(alias = "__GLOBAL__I_a_149")]
pub fn stub_38c478() -> ! {
    todo!("0x38c478 __GLOBAL__I_a_149")
}

// 0x38c4b0 — __ZN3RBX13UserInputBaseC2Ev — RBX::UserInputBase::UserInputBase(void)
// type: int __fastcall(RBX::UserInputBase *this)
#[doc(alias = "RBX::UserInputBase::UserInputBase(void)")]
#[doc(alias = "__ZN3RBX13UserInputBaseC2Ev")]
pub fn stub_38c4b0() -> ! {
    todo!("0x38c4b0 __ZN3RBX13UserInputBaseC2Ev")
}

// 0x38c5d4 — __ZNK3RBX13UserInputBase10getNavKeysERNS_7NavKeysEb — RBX::UserInputBase::getNavKeys(RBX::NavKeys &,bool)const
// type: int __fastcall(int result, _BYTE *, int)
#[doc(alias = "RBX::UserInputBase::getNavKeys(RBX::NavKeys &,bool)const")]
#[doc(alias = "__ZNK3RBX13UserInputBase10getNavKeysERNS_7NavKeysEb")]
pub fn stub_38c5d4() -> ! {
    todo!("0x38c5d4 __ZNK3RBX13UserInputBase10getNavKeysERNS_7NavKeysEb")
}

// 0x38c6b4 — __ZN3RBX13UserInputBase13getGameCursorEPNS_5AdornE — RBX::UserInputBase::getGameCursor(RBX::Adorn *)
// type: void __fastcall(RBX::UserInputBase *this, const shared_count *, int)
#[doc(alias = "RBX::UserInputBase::getGameCursor(RBX::Adorn *)")]
#[doc(alias = "__ZN3RBX13UserInputBase13getGameCursorEPNS_5AdornE")]
pub fn stub_38c6b4() -> ! {
    todo!("0x38c6b4 __ZN3RBX13UserInputBase13getGameCursorEPNS_5AdornE")
}

// 0x38c928 — __ZN3RBX13UserInputBase11setCursorIdEPNS_5AdornERKNS_9TextureIdE — RBX::UserInputBase::setCursorId(RBX::Adorn *,RBX::TextureId const&)
// type: int __fastcall(RBX::UserInputBase *this, RBX::Adorn *, const RBX::TextureId *)
#[doc(alias = "RBX::UserInputBase::setCursorId(RBX::Adorn *,RBX::TextureId const&)")]
#[doc(alias = "__ZN3RBX13UserInputBase11setCursorIdEPNS_5AdornERKNS_9TextureIdE")]
pub fn stub_38c928() -> ! {
    todo!("0x38c928 __ZN3RBX13UserInputBase11setCursorIdEPNS_5AdornERKNS_9TextureIdE")
}

// 0x38c974 — __ZN3RBX13UserInputBase16renderGameCursorEPNS_5AdornE — RBX::UserInputBase::renderGameCursor(RBX::Adorn *)
// type: void __fastcall(RBX::UserInputBase *this, RBX::Adorn *)
#[doc(alias = "RBX::UserInputBase::renderGameCursor(RBX::Adorn *)")]
#[doc(alias = "__ZN3RBX13UserInputBase16renderGameCursorEPNS_5AdornE")]
pub fn stub_38c974() -> ! {
    todo!("0x38c974 __ZN3RBX13UserInputBase16renderGameCursorEPNS_5AdornE")
}

// 0x38cb9c — __ZN3RBX13UserInputBase10removeJobsEv — RBX::UserInputBase::removeJobs(void)
// type: void __fastcall(RBX::UserInputBase *this)
#[doc(alias = "RBX::UserInputBase::removeJobs(void)")]
#[doc(alias = "__ZN3RBX13UserInputBase10removeJobsEv")]
pub fn stub_38cb9c() -> ! {
    todo!("0x38cb9c __ZN3RBX13UserInputBase10removeJobsEv")
}

// 0x38cba0 — __GLOBAL__I_a_150 — `global constructor keyed to'_a_150
// type: 
#[doc(alias = "global constructor keyed to_a_150")]
#[doc(alias = "__GLOBAL__I_a_150")]
pub fn stub_38cba0() -> ! {
    todo!("0x38cba0 __GLOBAL__I_a_150")
}

// 0x38cc68 — __ZN3RBX5rot13ESs — RBX::rot13(std::string)
// type: void __fastcall(std::string *, std::string *)
#[doc(alias = "RBX::rot13(std::string)")]
#[doc(alias = "__ZN3RBX5rot13ESs")]
pub fn stub_38cc68() -> ! {
    todo!("0x38cc68 __ZN3RBX5rot13ESs")
}

// 0x38ce48 — __ZN3RBX15StringConverterIbE15convertToStringERKb — RBX::StringConverter<bool>::convertToString(bool const&)
// type: int __fastcall(int, _BYTE *)
#[doc(alias = "RBX::StringConverter<bool>::convertToString(bool const&)")]
#[doc(alias = "__ZN3RBX15StringConverterIbE15convertToStringERKb")]
pub fn stub_38ce48() -> ! {
    todo!("0x38ce48 __ZN3RBX15StringConverterIbE15convertToStringERKb")
}

// 0x38ce78 — __ZN3RBX15StringConverterIbE14convertToValueERKSsRb — RBX::StringConverter<bool>::convertToValue(std::string const&,bool &)
// type: int __fastcall(std::string *, _BYTE *)
#[doc(alias = "RBX::StringConverter<bool>::convertToValue(std::string const&,bool &)")]
#[doc(alias = "__ZN3RBX15StringConverterIbE14convertToValueERKSsRb")]
pub fn stub_38ce78() -> ! {
    todo!("0x38ce78 __ZN3RBX15StringConverterIbE14convertToValueERKSsRb")
}

// 0x38cf10 — __ZN3RBX15StringConverterIiE15convertToStringERKi — RBX::StringConverter<int>::convertToString(int const&)
// type: int __fastcall(int, _DWORD *)
#[doc(alias = "RBX::StringConverter<int>::convertToString(int const&)")]
#[doc(alias = "__ZN3RBX15StringConverterIiE15convertToStringERKi")]
pub fn stub_38cf10() -> ! {
    todo!("0x38cf10 __ZN3RBX15StringConverterIiE15convertToStringERKi")
}

// 0x38cf58 — __ZN3RBX15StringConverterIlE15convertToStringERKl — RBX::StringConverter<long>::convertToString(long const&)
// type: int __fastcall(int, _DWORD *)
#[doc(alias = "RBX::StringConverter<long>::convertToString(long const&)")]
#[doc(alias = "__ZN3RBX15StringConverterIlE15convertToStringERKl")]
pub fn stub_38cf58() -> ! {
    todo!("0x38cf58 __ZN3RBX15StringConverterIlE15convertToStringERKl")
}

// 0x38cfa0 — __ZN3RBX15StringConverterIiE14convertToValueERKSsRi — RBX::StringConverter<int>::convertToValue(std::string const&,int &)
// type: int __fastcall(const char **, int *)
#[doc(alias = "RBX::StringConverter<int>::convertToValue(std::string const&,int &)")]
#[doc(alias = "__ZN3RBX15StringConverterIiE14convertToValueERKSsRi")]
pub fn stub_38cfa0() -> ! {
    todo!("0x38cfa0 __ZN3RBX15StringConverterIiE14convertToValueERKSsRi")
}

// 0x38cff0 — __ZN3RBX15StringConverterIjE15convertToStringERKj — RBX::StringConverter<unsigned int>::convertToString(unsigned int const&)
// type: int __fastcall(int, _DWORD *)
#[doc(alias = "RBX::StringConverter<unsigned int>::convertToString(unsigned int const&)")]
#[doc(alias = "__ZN3RBX15StringConverterIjE15convertToStringERKj")]
pub fn stub_38cff0() -> ! {
    todo!("0x38cff0 __ZN3RBX15StringConverterIjE15convertToStringERKj")
}

// 0x38d038 — __ZN3RBX15StringConverterIjE14convertToValueERKSsRj — RBX::StringConverter<unsigned int>::convertToValue(std::string const&,unsigned int &)
// type: int __fastcall(int *, _DWORD *)
#[doc(alias = "RBX::StringConverter<unsigned int>::convertToValue(std::string const&,unsigned int &)")]
#[doc(alias = "__ZN3RBX15StringConverterIjE14convertToValueERKSsRj")]
pub fn stub_38d038() -> ! {
    todo!("0x38d038 __ZN3RBX15StringConverterIjE14convertToValueERKSsRj")
}

// 0x38d14c — __ZN3RBX15StringConverterIlE14convertToValueERKSsRl — RBX::StringConverter<long>::convertToValue(std::string const&,long &)
// type: int __fastcall(int *, _DWORD *)
#[doc(alias = "RBX::StringConverter<long>::convertToValue(std::string const&,long &)")]
#[doc(alias = "__ZN3RBX15StringConverterIlE14convertToValueERKSsRl")]
pub fn stub_38d14c() -> ! {
    todo!("0x38d14c __ZN3RBX15StringConverterIlE14convertToValueERKSsRl")
}

// 0x38d260 — __ZN3RBX15StringConverterIdE14convertToValueERKSsRd — RBX::StringConverter<double>::convertToValue(std::string const&,double &)
// type: int __fastcall(std::string *, double *)
#[doc(alias = "RBX::StringConverter<double>::convertToValue(std::string const&,double &)")]
#[doc(alias = "__ZN3RBX15StringConverterIdE14convertToValueERKSsRd")]
pub fn stub_38d260() -> ! {
    todo!("0x38d260 __ZN3RBX15StringConverterIdE14convertToValueERKSsRd")
}

// 0x38d2e0 — __ZN3RBX15StringConverterIdE15convertToStringERKd — RBX::StringConverter<double>::convertToString(double const&)
// type: int __fastcall(int, double *)
#[doc(alias = "RBX::StringConverter<double>::convertToString(double const&)")]
#[doc(alias = "__ZN3RBX15StringConverterIdE15convertToStringERKd")]
pub fn stub_38d2e0() -> ! {
    todo!("0x38d2e0 __ZN3RBX15StringConverterIdE15convertToStringERKd")
}

// 0x38d440 — __ZN3RBX15StringConverterIfE14convertToValueERKSsRf — RBX::StringConverter<float>::convertToValue(std::string const&,float &)
// type: int __fastcall(std::string *this, float *)
#[doc(alias = "RBX::StringConverter<float>::convertToValue(std::string const&,float &)")]
#[doc(alias = "__ZN3RBX15StringConverterIfE14convertToValueERKSsRf")]
pub fn stub_38d440() -> ! {
    todo!("0x38d440 __ZN3RBX15StringConverterIfE14convertToValueERKSsRf")
}

// 0x38d4c4 — __ZN3RBX15StringConverterIfE15convertToStringERKf — RBX::StringConverter<float>::convertToString(float const&)
// type: int __fastcall(int, float *)
#[doc(alias = "RBX::StringConverter<float>::convertToString(float const&)")]
#[doc(alias = "__ZN3RBX15StringConverterIfE15convertToStringERKf")]
pub fn stub_38d4c4() -> ! {
    todo!("0x38d4c4 __ZN3RBX15StringConverterIfE15convertToStringERKf")
}

// 0x38da58 — __GLOBAL__I_a_151 — `global constructor keyed to'_a_151
// type: 
#[doc(alias = "global constructor keyed to_a_151")]
#[doc(alias = "__GLOBAL__I_a_151")]
pub fn stub_38da58() -> ! {
    todo!("0x38da58 __GLOBAL__I_a_151")
}

// 0x38db20 — __ZN3RBX12Accoutrement18setAttachmentPointERKN3G3D15CoordinateFrameE — RBX::Accoutrement::setAttachmentPoint(G3D::CoordinateFrame const&)
// type: int __fastcall(RBX::Accoutrement *this, const G3D::CoordinateFrame *)
#[doc(alias = "RBX::Accoutrement::setAttachmentPoint(G3D::CoordinateFrame const&)")]
#[doc(alias = "__ZN3RBX12Accoutrement18setAttachmentPointERKN3G3D15CoordinateFrameE")]
pub fn stub_38db20() -> ! {
    todo!("0x38db20 __ZN3RBX12Accoutrement18setAttachmentPointERKN3G3D15CoordinateFrameE")
}

// 0x38dc30 — __ZNK3RBX12Accoutrement16getAttachmentPosEv — RBX::Accoutrement::getAttachmentPos(void)const
// type: int __fastcall(int this, int)
#[doc(alias = "RBX::Accoutrement::getAttachmentPos(void)const")]
#[doc(alias = "__ZNK3RBX12Accoutrement16getAttachmentPosEv")]
pub fn stub_38dc30() -> ! {
    todo!("0x38dc30 __ZNK3RBX12Accoutrement16getAttachmentPosEv")
}

// 0x38dc40 — __ZN3RBX12Accoutrement16setAttachmentPosERKN3G3D7Vector3E — RBX::Accoutrement::setAttachmentPos(G3D::Vector3 const&)
// type: int __fastcall(RBX::Accoutrement *this, const G3D::Vector3 *)
#[doc(alias = "RBX::Accoutrement::setAttachmentPos(G3D::Vector3 const&)")]
#[doc(alias = "__ZN3RBX12Accoutrement16setAttachmentPosERKN3G3D7Vector3E")]
pub fn stub_38dc40() -> ! {
    todo!("0x38dc40 __ZN3RBX12Accoutrement16setAttachmentPosERKN3G3D7Vector3E")
}

// 0x38dc70 — __ZNK3RBX12Accoutrement20getAttachmentForwardEv — RBX::Accoutrement::getAttachmentForward(void)const
// type: int __fastcall(RBX::Accoutrement *this, int)
#[doc(alias = "RBX::Accoutrement::getAttachmentForward(void)const")]
#[doc(alias = "__ZNK3RBX12Accoutrement20getAttachmentForwardEv")]
pub fn stub_38dc70() -> ! {
    todo!("0x38dc70 __ZNK3RBX12Accoutrement20getAttachmentForwardEv")
}

// 0x38dcb0 — __ZN3RBX12Accoutrement20setAttachmentForwardERKN3G3D7Vector3E — RBX::Accoutrement::setAttachmentForward(G3D::Vector3 const&)
// type: int __fastcall(RBX::Accoutrement *this, const G3D::Vector3 *)
#[doc(alias = "RBX::Accoutrement::setAttachmentForward(G3D::Vector3 const&)")]
#[doc(alias = "__ZN3RBX12Accoutrement20setAttachmentForwardERKN3G3D7Vector3E")]
pub fn stub_38dcb0() -> ! {
    todo!("0x38dcb0 __ZN3RBX12Accoutrement20setAttachmentForwardERKN3G3D7Vector3E")
}

// 0x38ddfc — __ZNK3RBX12Accoutrement15getAttachmentUpEv — RBX::Accoutrement::getAttachmentUp(void)const
// type: int __fastcall(RBX::Accoutrement *this, int)
#[doc(alias = "RBX::Accoutrement::getAttachmentUp(void)const")]
#[doc(alias = "__ZNK3RBX12Accoutrement15getAttachmentUpEv")]
pub fn stub_38ddfc() -> ! {
    todo!("0x38ddfc __ZNK3RBX12Accoutrement15getAttachmentUpEv")
}

// 0x38de0c — __ZN3RBX12Accoutrement15setAttachmentUpERKN3G3D7Vector3E — RBX::Accoutrement::setAttachmentUp(G3D::Vector3 const&)
// type: int __fastcall(RBX::Accoutrement *this, const G3D::Vector3 *)
#[doc(alias = "RBX::Accoutrement::setAttachmentUp(G3D::Vector3 const&)")]
#[doc(alias = "__ZN3RBX12Accoutrement15setAttachmentUpERKN3G3D7Vector3E")]
pub fn stub_38de0c() -> ! {
    todo!("0x38de0c __ZN3RBX12Accoutrement15setAttachmentUpERKN3G3D7Vector3E")
}

// 0x38df30 — __ZNK3RBX12Accoutrement18getAttachmentRightEv — RBX::Accoutrement::getAttachmentRight(void)const
// type: int __fastcall(RBX::Accoutrement *this, int)
#[doc(alias = "RBX::Accoutrement::getAttachmentRight(void)const")]
#[doc(alias = "__ZNK3RBX12Accoutrement18getAttachmentRightEv")]
pub fn stub_38df30() -> ! {
    todo!("0x38df30 __ZNK3RBX12Accoutrement18getAttachmentRightEv")
}

// 0x38df40 — __ZN3RBX12Accoutrement18setAttachmentRightERKN3G3D7Vector3E — RBX::Accoutrement::setAttachmentRight(G3D::Vector3 const&)
// type: int __fastcall(RBX::Accoutrement *this, const G3D::Vector3 *)
#[doc(alias = "RBX::Accoutrement::setAttachmentRight(G3D::Vector3 const&)")]
#[doc(alias = "__ZN3RBX12Accoutrement18setAttachmentRightERKN3G3D7Vector3E")]
pub fn stub_38df40() -> ! {
    todo!("0x38df40 __ZN3RBX12Accoutrement18setAttachmentRightERKN3G3D7Vector3E")
}

// 0x38e064 — __ZN3RBX12Accoutrement27setBackendAccoutrementStateEi — RBX::Accoutrement::setBackendAccoutrementState(int)
// type: RBX::Instance *__fastcall(RBX::Instance *this, int)
#[doc(alias = "RBX::Accoutrement::setBackendAccoutrementState(int)")]
#[doc(alias = "__ZN3RBX12Accoutrement27setBackendAccoutrementStateEi")]
pub fn stub_38e064() -> ! {
    todo!("0x38e064 __ZN3RBX12Accoutrement27setBackendAccoutrementStateEi")
}

// 0x38e084 — __ZN3RBX12AccoutrementC1Ev — RBX::Accoutrement::Accoutrement(void)
// type: RBX::Instance *__fastcall(RBX::Accoutrement *this)
#[doc(alias = "RBX::Accoutrement::Accoutrement(void)")]
#[doc(alias = "__ZN3RBX12AccoutrementC1Ev")]
pub fn stub_38e084() -> ! {
    todo!("0x38e084 __ZN3RBX12AccoutrementC1Ev")
}

// 0x38e4b4 — __ZN3RBX12AccoutrementC2Ev — RBX::Accoutrement::Accoutrement(void)
// type: RBX::Instance *__fastcall(RBX::Accoutrement *this, int)
#[doc(alias = "RBX::Accoutrement::Accoutrement(void)")]
#[doc(alias = "__ZN3RBX12AccoutrementC2Ev")]
pub fn stub_38e4b4() -> ! {
    todo!("0x38e4b4 __ZN3RBX12AccoutrementC2Ev")
}

// 0x38e90c — __ZN3RBX12AccoutrementD0Ev — RBX::Accoutrement::~Accoutrement()
// type: void __fastcall(RBX::Accoutrement *__hidden this)
#[doc(alias = "RBX::Accoutrement::~Accoutrement()")]
#[doc(alias = "__ZN3RBX12AccoutrementD0Ev")]
pub fn stub_38e90c() -> ! {
    todo!("0x38e90c __ZN3RBX12AccoutrementD0Ev")
}

// 0x38e9b8 — __ZN3RBX12AccoutrementD1Ev — RBX::Accoutrement::~Accoutrement()
// type: void __fastcall(RBX::Accoutrement *__hidden this)
#[doc(alias = "RBX::Accoutrement::~Accoutrement()")]
#[doc(alias = "__ZN3RBX12AccoutrementD1Ev")]
pub fn stub_38e9b8() -> ! {
    todo!("0x38e9b8 __ZN3RBX12AccoutrementD1Ev")
}

// 0x38e9c8 — __ZThn32_N3RBX12AccoutrementD0Ev — `non-virtual thunk to'RBX::Accoutrement::~Accoutrement()
// type: void __fastcall(RBX::Accoutrement *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Accoutrement::~Accoutrement()")]
#[doc(alias = "__ZThn32_N3RBX12AccoutrementD0Ev")]
pub fn stub_38e9c8() -> ! {
    todo!("0x38e9c8 __ZThn32_N3RBX12AccoutrementD0Ev")
}

// 0x38e9d0 — __ZThn36_N3RBX12AccoutrementD0Ev — `non-virtual thunk to'RBX::Accoutrement::~Accoutrement()
// type: void __fastcall(RBX::Accoutrement *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Accoutrement::~Accoutrement()")]
#[doc(alias = "__ZThn36_N3RBX12AccoutrementD0Ev")]
pub fn stub_38e9d0() -> ! {
    todo!("0x38e9d0 __ZThn36_N3RBX12AccoutrementD0Ev")
}

// 0x38e9d8 — __ZThn92_N3RBX12AccoutrementD0Ev — `non-virtual thunk to'RBX::Accoutrement::~Accoutrement()
// type: void __fastcall(RBX::Accoutrement *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Accoutrement::~Accoutrement()")]
#[doc(alias = "__ZThn92_N3RBX12AccoutrementD0Ev")]
pub fn stub_38e9d8() -> ! {
    todo!("0x38e9d8 __ZThn92_N3RBX12AccoutrementD0Ev")
}

// 0x38e9e0 — __ZThn128_N3RBX12AccoutrementD0Ev — `non-virtual thunk to'RBX::Accoutrement::~Accoutrement()
// type: void __fastcall(RBX::Accoutrement *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Accoutrement::~Accoutrement()")]
#[doc(alias = "__ZThn128_N3RBX12AccoutrementD0Ev")]
pub fn stub_38e9e0() -> ! {
    todo!("0x38e9e0 __ZThn128_N3RBX12AccoutrementD0Ev")
}

// 0x38e9e8 — __ZN3RBX12AccoutrementD2Ev — RBX::Accoutrement::~Accoutrement()
// type: void __fastcall(RBX::Accoutrement *this, int *, int)
#[doc(alias = "RBX::Accoutrement::~Accoutrement()")]
#[doc(alias = "__ZN3RBX12AccoutrementD2Ev")]
pub fn stub_38e9e8() -> ! {
    todo!("0x38e9e8 __ZN3RBX12AccoutrementD2Ev")
}

// 0x38ef1c — __ZThn32_N3RBX12AccoutrementD1Ev — `non-virtual thunk to'RBX::Accoutrement::~Accoutrement()
// type: void __fastcall(RBX::Accoutrement *this, int, int)
#[doc(alias = "non-virtual thunk toRBX::Accoutrement::~Accoutrement()")]
#[doc(alias = "__ZThn32_N3RBX12AccoutrementD1Ev")]
pub fn stub_38ef1c() -> ! {
    todo!("0x38ef1c __ZThn32_N3RBX12AccoutrementD1Ev")
}

// 0x38ef2c — __ZThn36_N3RBX12AccoutrementD1Ev — `non-virtual thunk to'RBX::Accoutrement::~Accoutrement()
// type: void __fastcall(RBX::Accoutrement *this, int, int)
#[doc(alias = "non-virtual thunk toRBX::Accoutrement::~Accoutrement()")]
#[doc(alias = "__ZThn36_N3RBX12AccoutrementD1Ev")]
pub fn stub_38ef2c() -> ! {
    todo!("0x38ef2c __ZThn36_N3RBX12AccoutrementD1Ev")
}

// 0x38ef3c — __ZThn92_N3RBX12AccoutrementD1Ev — `non-virtual thunk to'RBX::Accoutrement::~Accoutrement()
// type: void __fastcall(RBX::Accoutrement *this, int, int)
#[doc(alias = "non-virtual thunk toRBX::Accoutrement::~Accoutrement()")]
#[doc(alias = "__ZThn92_N3RBX12AccoutrementD1Ev")]
pub fn stub_38ef3c() -> ! {
    todo!("0x38ef3c __ZThn92_N3RBX12AccoutrementD1Ev")
}

// 0x38ef4c — __ZThn128_N3RBX12AccoutrementD1Ev — `non-virtual thunk to'RBX::Accoutrement::~Accoutrement()
// type: void __fastcall(RBX::Accoutrement *this, int, int)
#[doc(alias = "non-virtual thunk toRBX::Accoutrement::~Accoutrement()")]
#[doc(alias = "__ZThn128_N3RBX12AccoutrementD1Ev")]
pub fn stub_38ef4c() -> ! {
    todo!("0x38ef4c __ZThn128_N3RBX12AccoutrementD1Ev")
}

// 0x38ef5c — __ZN3RBX12Accoutrement12onCameraNearEf — RBX::Accoutrement::onCameraNear(float)
// type: unsigned int __fastcall(RBX::Accoutrement *this, float)
#[doc(alias = "RBX::Accoutrement::onCameraNear(float)")]
#[doc(alias = "__ZN3RBX12Accoutrement12onCameraNearEf")]
pub fn stub_38ef5c() -> ! {
    todo!("0x38ef5c __ZN3RBX12Accoutrement12onCameraNearEf")
}

// 0x38ef98 — __ZThn128_N3RBX12Accoutrement12onCameraNearEf — `non-virtual thunk to'RBX::Accoutrement::onCameraNear(float)
// type: unsigned int __fastcall(RBX::Accoutrement *this, float)
#[doc(alias = "non-virtual thunk toRBX::Accoutrement::onCameraNear(float)")]
#[doc(alias = "__ZThn128_N3RBX12Accoutrement12onCameraNearEf")]
pub fn stub_38ef98() -> ! {
    todo!("0x38ef98 __ZThn128_N3RBX12Accoutrement12onCameraNearEf")
}

// 0x38efa0 — __ZN3RBX12Accoutrement14render3dSelectEPNS_5AdornENS_11SelectStateE — RBX::Accoutrement::render3dSelect(RBX::Adorn *,RBX::SelectState)
// type: unsigned int __fastcall(RBX::Instance *, int, int)
#[doc(alias = "RBX::Accoutrement::render3dSelect(RBX::Adorn *,RBX::SelectState)")]
#[doc(alias = "__ZN3RBX12Accoutrement14render3dSelectEPNS_5AdornENS_11SelectStateE")]
pub fn stub_38efa0() -> ! {
    todo!("0x38efa0 __ZN3RBX12Accoutrement14render3dSelectEPNS_5AdornENS_11SelectStateE")
}

// 0x38f014 — __ZThn104_N3RBX12Accoutrement14render3dSelectEPNS_5AdornENS_11SelectStateE — `non-virtual thunk to'RBX::Accoutrement::render3dSelect(RBX::Adorn *,RBX::SelectState)
// type: unsigned int __fastcall(int, int, int)
#[doc(alias = "non-virtual thunk toRBX::Accoutrement::render3dSelect(RBX::Adorn *,RBX::SelectState)")]
#[doc(alias = "__ZThn104_N3RBX12Accoutrement14render3dSelectEPNS_5AdornENS_11SelectStateE")]
pub fn stub_38f014() -> ! {
    todo!("0x38f014 __ZThn104_N3RBX12Accoutrement14render3dSelectEPNS_5AdornENS_11SelectStateE")
}

// 0x38f01c — __ZN3RBX12Accoutrement7dropAllEPNS_13ModelInstanceE — RBX::Accoutrement::dropAll(RBX::ModelInstance *)
// type: int __fastcall(RBX::Accoutrement *this, RBX::ModelInstance *, RBX::Accoutrement *)
#[doc(alias = "RBX::Accoutrement::dropAll(RBX::ModelInstance *)")]
#[doc(alias = "__ZN3RBX12Accoutrement7dropAllEPNS_13ModelInstanceE")]
pub fn stub_38f01c() -> ! {
    todo!("0x38f01c __ZN3RBX12Accoutrement7dropAllEPNS_13ModelInstanceE")
}

// 0x38f024 — __ZN3RBX12Accoutrement13dropAllOthersEPNS_13ModelInstanceEPS0_ — RBX::Accoutrement::dropAllOthers(RBX::ModelInstance *,RBX::Accoutrement*)
// type: RBX::Instance *__fastcall(RBX::Accoutrement *this, RBX::ModelInstance *, RBX::Accoutrement *)
#[doc(alias = "RBX::Accoutrement::dropAllOthers(RBX::ModelInstance *,RBX::Accoutrement*)")]
#[doc(alias = "__ZN3RBX12Accoutrement13dropAllOthersEPNS_13ModelInstanceEPS0_")]
pub fn stub_38f024() -> ! {
    todo!("0x38f024 __ZN3RBX12Accoutrement13dropAllOthersEPNS_13ModelInstanceEPS0_")
}

// 0x38f054 — __ZNK3RBX12Accoutrement14getHandleConstEv — RBX::Accoutrement::getHandleConst(void)const
// type: char *__fastcall(RBX::Accoutrement *this)
#[doc(alias = "RBX::Accoutrement::getHandleConst(void)const")]
#[doc(alias = "__ZNK3RBX12Accoutrement14getHandleConstEv")]
pub fn stub_38f054() -> ! {
    todo!("0x38f054 __ZNK3RBX12Accoutrement14getHandleConstEv")
}

// 0x38f1c4 — __ZN3RBX12Accoutrement11getLocationEv — RBX::Accoutrement::getLocation(void)
// type: int __fastcall(RBX::Accoutrement *this, RBX::Accoutrement *)
#[doc(alias = "RBX::Accoutrement::getLocation(void)")]
#[doc(alias = "__ZN3RBX12Accoutrement11getLocationEv")]
pub fn stub_38f1c4() -> ! {
    todo!("0x38f1c4 __ZN3RBX12Accoutrement11getLocationEv")
}

// 0x38f1f8 — __ZTv0_n12_N3RBX12Accoutrement11getLocationEv — `virtual thunk to'RBX::Accoutrement::getLocation(void)
// type: int __fastcall(RBX::Accoutrement *this, _DWORD *)
#[doc(alias = "virtual thunk toRBX::Accoutrement::getLocation(void)")]
#[doc(alias = "__ZTv0_n12_N3RBX12Accoutrement11getLocationEv")]
pub fn stub_38f1f8() -> ! {
    todo!("0x38f1f8 __ZTv0_n12_N3RBX12Accoutrement11getLocationEv")
}

// 0x38f20c — __ZN3RBX12Accoutrement17connectTouchEventEv — RBX::Accoutrement::connectTouchEvent(void)
// type: void __fastcall(RBX::Accoutrement *this)
#[doc(alias = "RBX::Accoutrement::connectTouchEvent(void)")]
#[doc(alias = "__ZN3RBX12Accoutrement17connectTouchEventEv")]
pub fn stub_38f20c() -> ! {
    todo!("0x38f20c __ZN3RBX12Accoutrement17connectTouchEventEv")
}

// 0x38f3ec — __ZN3RBX12Accoutrement21onEvent_HandleTouchedEN5boost10shared_ptrINS_8InstanceEEE — RBX::Accoutrement::onEvent_HandleTouched(rbx_core::SharedPtr<RBX::Instance>)
// type: RBX::Accoutrement *__fastcall(RBX::Network::Players *, RBX::Accoutrement **, bool)
// was: RBX::Accoutrement::onEvent_HandleTouched(boost::shared_ptr<RBX::Instance>) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "RBX::Accoutrement::onEvent_HandleTouched(rbx_core::SharedPtr<RBX::Instance>)")]
#[doc(alias = "__ZN3RBX12Accoutrement21onEvent_HandleTouchedEN5boost10shared_ptrINS_8InstanceEEE")]
pub fn stub_38f3ec() -> ! {
    todo!("0x38f3ec __ZN3RBX12Accoutrement21onEvent_HandleTouchedEN5boost10shared_ptrINS_8InstanceEEE")
}

// 0x38f47c — __ZN3RBX12Accoutrement19rebuildBackendStateEv — RBX::Accoutrement::rebuildBackendState(void)
// type: int __fastcall(RBX::Accoutrement *this)
#[doc(alias = "RBX::Accoutrement::rebuildBackendState(void)")]
#[doc(alias = "__ZN3RBX12Accoutrement19rebuildBackendStateEv")]
pub fn stub_38f47c() -> ! {
    todo!("0x38f47c __ZN3RBX12Accoutrement19rebuildBackendStateEv")
}

// 0x38f4f4 — __ZN3RBX12Accoutrement19computeDesiredStateEv — RBX::Accoutrement::computeDesiredState(void)
// type: int __fastcall(RBX::Instance **this, int, bool)
#[doc(alias = "RBX::Accoutrement::computeDesiredState(void)")]
#[doc(alias = "__ZN3RBX12Accoutrement19computeDesiredStateEv")]
pub fn stub_38f4f4() -> ! {
    todo!("0x38f4f4 __ZN3RBX12Accoutrement19computeDesiredStateEv")
}

// 0x38f578 — __ZN3RBX12Accoutrement15setDesiredStateENS0_17AccoutrementStateEPKNS_15ServiceProviderE — RBX::Accoutrement::setDesiredState(RBX::Accoutrement::AccoutrementState,RBX::ServiceProvider const*)
// type: int __fastcall(RBX::Accoutrement *this, int, RBX::Network::Players *)
#[doc(alias = "RBX::Accoutrement::setDesiredState(RBX::Accoutrement::AccoutrementState,RBX::ServiceProvider const*)")]
#[doc(alias = "__ZN3RBX12Accoutrement15setDesiredStateENS0_17AccoutrementStateEPKNS_15ServiceProviderE")]
pub fn stub_38f578() -> ! {
    todo!("0x38f578 __ZN3RBX12Accoutrement15setDesiredStateENS0_17AccoutrementStateEPKNS_15ServiceProviderE")
}

// 0x38f6f0 — __ZN3RBX12Accoutrement19computeDesiredStateEPNS_8InstanceE — RBX::Accoutrement::computeDesiredState(RBX::Instance *)
// type: int __fastcall(RBX::Accoutrement *this, RBX::Instance *)
#[doc(alias = "RBX::Accoutrement::computeDesiredState(RBX::Instance *)")]
#[doc(alias = "__ZN3RBX12Accoutrement19computeDesiredStateEPNS_8InstanceE")]
pub fn stub_38f6f0() -> ! {
    todo!("0x38f6f0 __ZN3RBX12Accoutrement19computeDesiredStateEPNS_8InstanceE")
}

// 0x38f714 — __ZN3RBX12Accoutrement13upTo_EquippedEv — RBX::Accoutrement::upTo_Equipped(void)
// type: void __fastcall(RBX::Accoutrement *this)
#[doc(alias = "RBX::Accoutrement::upTo_Equipped(void)")]
#[doc(alias = "__ZN3RBX12Accoutrement13upTo_EquippedEv")]
pub fn stub_38f714() -> ! {
    todo!("0x38f714 __ZN3RBX12Accoutrement13upTo_EquippedEv")
}

// 0x38f92c — __ZN3RBX12Accoutrement16upTo_InCharacterEv — RBX::Accoutrement::upTo_InCharacter(void)
// type: void __fastcall(RBX::Humanoid **this)
#[doc(alias = "RBX::Accoutrement::upTo_InCharacter(void)")]
#[doc(alias = "__ZN3RBX12Accoutrement16upTo_InCharacterEv")]
pub fn stub_38f92c() -> ! {
    todo!("0x38f92c __ZN3RBX12Accoutrement16upTo_InCharacterEv")
}

// 0x38fb1c — __ZN3RBX12Accoutrement16upTo_InWorkspaceEv — RBX::Accoutrement::upTo_InWorkspace(void)
// type: int __fastcall(RBX::Accoutrement *this, const RBX::Instance *)
#[doc(alias = "RBX::Accoutrement::upTo_InWorkspace(void)")]
#[doc(alias = "__ZN3RBX12Accoutrement16upTo_InWorkspaceEv")]
pub fn stub_38fb1c() -> ! {
    todo!("0x38fb1c __ZN3RBX12Accoutrement16upTo_InWorkspaceEv")
}

// 0x38fbcc — __ZN3RBX12Accoutrement17downFrom_EquippedEv — RBX::Accoutrement::downFrom_Equipped(void)
// type: void __fastcall(RBX::Accoutrement *this)
#[doc(alias = "RBX::Accoutrement::downFrom_Equipped(void)")]
#[doc(alias = "__ZN3RBX12Accoutrement17downFrom_EquippedEv")]
pub fn stub_38fbcc() -> ! {
    todo!("0x38fbcc __ZN3RBX12Accoutrement17downFrom_EquippedEv")
}

// 0x38fd24 — __ZN3RBX12Accoutrement18downFrom_HasHandleEv — RBX::Accoutrement::downFrom_HasHandle(void)
// type: void __fastcall(RBX::Accoutrement *this)
#[doc(alias = "RBX::Accoutrement::downFrom_HasHandle(void)")]
#[doc(alias = "__ZN3RBX12Accoutrement18downFrom_HasHandleEv")]
pub fn stub_38fd24() -> ! {
    todo!("0x38fd24 __ZN3RBX12Accoutrement18downFrom_HasHandleEv")
}

// 0x38fd60 — __ZN3RBX12Accoutrement20onEvent_AddedBackendEN5boost10shared_ptrINS_8InstanceEEE — RBX::Accoutrement::onEvent_AddedBackend(rbx_core::SharedPtr<RBX::Instance>)
// type: int __fastcall(RBX::ServiceProvider *, const RBX::Instance *)
// was: RBX::Accoutrement::onEvent_AddedBackend(boost::shared_ptr<RBX::Instance>) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "RBX::Accoutrement::onEvent_AddedBackend(rbx_core::SharedPtr<RBX::Instance>)")]
#[doc(alias = "__ZN3RBX12Accoutrement20onEvent_AddedBackendEN5boost10shared_ptrINS_8InstanceEEE")]
pub fn stub_38fd60() -> ! {
    todo!("0x38fd60 __ZN3RBX12Accoutrement20onEvent_AddedBackendEN5boost10shared_ptrINS_8InstanceEEE")
}

// 0x38fe18 — __ZN3RBX12Accoutrement22onEvent_RemovedBackendEN5boost10shared_ptrINS_8InstanceEEE — RBX::Accoutrement::onEvent_RemovedBackend(rbx_core::SharedPtr<RBX::Instance>)
// type: int __fastcall(RBX::ServiceProvider *, const RBX::Instance *, int)
// was: RBX::Accoutrement::onEvent_RemovedBackend(boost::shared_ptr<RBX::Instance>) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "RBX::Accoutrement::onEvent_RemovedBackend(rbx_core::SharedPtr<RBX::Instance>)")]
#[doc(alias = "__ZN3RBX12Accoutrement22onEvent_RemovedBackendEN5boost10shared_ptrINS_8InstanceEEE")]
pub fn stub_38fe18() -> ! {
    todo!("0x38fe18 __ZN3RBX12Accoutrement22onEvent_RemovedBackendEN5boost10shared_ptrINS_8InstanceEEE")
}

// 0x38ff34 — __ZN3RBX12Accoutrement12onChildAddedEPNS_8InstanceE — RBX::Accoutrement::onChildAdded(RBX::Instance *)
// type: int __fastcall(RBX::Accoutrement *this, RBX::Instance *)
#[doc(alias = "RBX::Accoutrement::onChildAdded(RBX::Instance *)")]
#[doc(alias = "__ZN3RBX12Accoutrement12onChildAddedEPNS_8InstanceE")]
pub fn stub_38ff34() -> ! {
    todo!("0x38ff34 __ZN3RBX12Accoutrement12onChildAddedEPNS_8InstanceE")
}

// 0x38ff5c — __ZN3RBX12Accoutrement14onChildRemovedEPNS_8InstanceE — RBX::Accoutrement::onChildRemoved(RBX::Instance *)
// type: int __fastcall(RBX::Accoutrement *this, RBX::Instance *)
#[doc(alias = "RBX::Accoutrement::onChildRemoved(RBX::Instance *)")]
#[doc(alias = "__ZN3RBX12Accoutrement14onChildRemovedEPNS_8InstanceE")]
pub fn stub_38ff5c() -> ! {
    todo!("0x38ff5c __ZN3RBX12Accoutrement14onChildRemovedEPNS_8InstanceE")
}

// 0x38ff84 — __ZN3RBX12Accoutrement17onAncestorChangedERKNS_15AncestorChangedE — RBX::Accoutrement::onAncestorChanged(RBX::AncestorChanged const&)
// type: 
#[doc(alias = "RBX::Accoutrement::onAncestorChanged(RBX::AncestorChanged const&)")]
#[doc(alias = "__ZN3RBX12Accoutrement17onAncestorChangedERKNS_15AncestorChangedE")]
pub fn stub_38ff84() -> ! {
    todo!("0x38ff84 __ZN3RBX12Accoutrement17onAncestorChangedERKNS_15AncestorChangedE")
}

// 0x38fff0 — __ZN3RBX3HatC1Ev — RBX::Hat::Hat(void)
// type: RBX::Accoutrement *__fastcall(RBX::Hat *this)
#[doc(alias = "RBX::Hat::Hat(void)")]
#[doc(alias = "__ZN3RBX3HatC1Ev")]
pub fn stub_38fff0() -> ! {
    todo!("0x38fff0 __ZN3RBX3HatC1Ev")
}

// 0x3901bc — __ZNK3RBX12Accoutrement18getAttachmentPointEv — RBX::Accoutrement::getAttachmentPoint(void)const
// type: char *__fastcall(RBX::Accoutrement *this)
#[doc(alias = "RBX::Accoutrement::getAttachmentPoint(void)const")]
#[doc(alias = "__ZNK3RBX12Accoutrement18getAttachmentPointEv")]
pub fn stub_3901bc() -> ! {
    todo!("0x3901bc __ZNK3RBX12Accoutrement18getAttachmentPointEv")
}

// 0x3901c0 — __ZN3RBX10Reflection14PropDescriptorINS_12AccoutrementEN3G3D15CoordinateFrameEED1Ev — RBX::Reflection::PropDescriptor<RBX::Accoutrement,G3D::CoordinateFrame>::~PropDescriptor()
// type: _DWORD *__fastcall(_DWORD *)
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::Accoutrement,G3D::CoordinateFrame>::~PropDescriptor()")]
#[doc(alias = "__ZN3RBX10Reflection14PropDescriptorINS_12AccoutrementEN3G3D15CoordinateFrameEED1Ev")]
pub fn stub_3901c0() -> ! {
    todo!("0x3901c0 __ZN3RBX10Reflection14PropDescriptorINS_12AccoutrementEN3G3D15CoordinateFrameEED1Ev")
}

// 0x3901e4 — __ZN3RBX10Reflection14PropDescriptorINS_12AccoutrementEN3G3D7Vector3EED1Ev — RBX::Reflection::PropDescriptor<RBX::Accoutrement,G3D::Vector3>::~PropDescriptor()
// type: _DWORD *__fastcall(_DWORD *)
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::Accoutrement,G3D::Vector3>::~PropDescriptor()")]
#[doc(alias = "__ZN3RBX10Reflection14PropDescriptorINS_12AccoutrementEN3G3D7Vector3EED1Ev")]
pub fn stub_3901e4() -> ! {
    todo!("0x3901e4 __ZN3RBX10Reflection14PropDescriptorINS_12AccoutrementEN3G3D7Vector3EED1Ev")
}

// 0x390208 — __ZNK3RBX12Accoutrement27getBackendAccoutrementStateEv — RBX::Accoutrement::getBackendAccoutrementState(void)const
// type: int __fastcall(RBX::Accoutrement *this)
#[doc(alias = "RBX::Accoutrement::getBackendAccoutrementState(void)const")]
#[doc(alias = "__ZNK3RBX12Accoutrement27getBackendAccoutrementStateEv")]
pub fn stub_390208() -> ! {
    todo!("0x390208 __ZNK3RBX12Accoutrement27getBackendAccoutrementStateEv")
}

// 0x390210 — __ZN3RBX10Reflection14PropDescriptorINS_12AccoutrementEiED1Ev — RBX::Reflection::PropDescriptor<RBX::Accoutrement,int>::~PropDescriptor()
// type: _DWORD *__fastcall(_DWORD *)
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::Accoutrement,int>::~PropDescriptor()")]
#[doc(alias = "__ZN3RBX10Reflection14PropDescriptorINS_12AccoutrementEiED1Ev")]
pub fn stub_390210() -> ! {
    todo!("0x390210 __ZN3RBX10Reflection14PropDescriptorINS_12AccoutrementEiED1Ev")
}

// 0x390234 — __ZN3RBX8Instance15queryTypedChildINS_13CameraSubjectEEEPT_i — RBX::CameraSubject * RBX::Instance::queryTypedChild<RBX::CameraSubject>(int)
// type: void *__fastcall(int, int)
#[doc(alias = "RBX::CameraSubject * RBX::Instance::queryTypedChild<RBX::CameraSubject>(int)")]
#[doc(alias = "__ZN3RBX8Instance15queryTypedChildINS_13CameraSubjectEEEPT_i")]
pub fn stub_390234() -> ! {
    todo!("0x390234 __ZN3RBX8Instance15queryTypedChildINS_13CameraSubjectEEEPT_i")
}

// 0x390270 — __ZN3RBX12PartInstance13TouchedSignal7connectIN5boost3_bi6bind_tIvNS3_4_mfi3mf1IvNS_12AccoutrementENS3_10shared_ptrINS_8InstanceEEEEENS4_5list2INS4_5valueIPS8_EENS3_3argILi1EEEEEEEEEN3rbx7signals10connectionET_ — rbx::signals::connection RBX::PartInstance::TouchedSignal::connect<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::Accoutrement,rbx_core::SharedPtr<RBX::Instance>>,boost::_bi::list2<boost::_bi::value<RBX::Accoutrement*>,boost::arg<1>>>>(boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::Accoutrement,rbx_core::SharedPtr<RBX::Instance>>,boost::_bi::list2<boost::_bi::value<RBX::Accoutrement*>,boost::arg<1>>>)
// type: void __fastcall(rbx::signals::connection *, int, int, const void *, int)
// was: rbx::signals::connection RBX::PartInstance::TouchedSignal::connect<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::Accoutrement,boost::shared_ptr<RBX::Instance>>,boost::_bi::list2<boost::_bi::value<RBX::Accoutrement*>,boost::arg<1>>>>(boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::Accoutrement,boost::shared_ptr<RBX::Instance>>,boost::_bi::list2<boost::_bi::value<RBX::Accoutrement*>,boost::arg<1>>>) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "rbx::signals::connection RBX::PartInstance::TouchedSignal::connect<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::Accoutrement,rbx_core::SharedPtr<RBX::Instance>>,boost::_bi::list2<boost::_bi::value<RBX::Accoutrement*>,boost::arg<1>>>>(boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::Accoutrement,rbx_core::SharedPtr<RBX::Instance>>,boost::_bi::list2<boost::_bi::value<RBX::Accoutrement*>,boost::arg<1>>>)")]
#[doc(alias = "__ZN3RBX12PartInstance13TouchedSignal7connectIN5boost3_bi6bind_tIvNS3_4_mfi3mf1IvNS_12AccoutrementENS3_10shared_ptrINS_8InstanceEEEEENS4_5list2INS4_5valueIPS8_EENS3_3argILi1EEEEEEEEEN3rbx7signals10connectionET_")]
pub fn stub_390270() -> ! {
    todo!("0x390270 __ZN3RBX12PartInstance13TouchedSignal7connectIN5boost3_bi6bind_tIvNS3_4_mfi3mf1IvNS_12AccoutrementENS3_10shared_ptrINS_8InstanceEEEEENS4_5list2INS4_5valueIPS8_EENS3_3argILi1EEEEEEEEEN3rbx7signals10connectionET_")
}

// 0x3903f0 — __ZN3rbx7signals6signalIFvN5boost10shared_ptrIN3RBX8InstanceEEEEE7connectINS2_3_bi6bind_tIvNS2_4_mfi3mf1IvNS4_12AccoutrementES6_EENSA_5list2INSA_5valueIPSE_EENS2_3argILi1EEEEEEEEENS0_10connectionERKT_ — rbx::signals::connection rbx::signals::signal<void ()(rbx_core::SharedPtr<RBX::Instance>)>::connect<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::Accoutrement,rbx_core::SharedPtr<RBX::Instance>>,boost::_bi::list2<boost::_bi::value<RBX::Accoutrement*>,boost::arg<1>>>>(boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::Accoutrement,rbx_core::SharedPtr<RBX::Instance>>,boost::_bi::list2<boost::_bi::value<RBX::Accoutrement*>,boost::arg<1>>> const&)
// type: int __fastcall(int *, int, __int64 *)
// was: rbx::signals::connection rbx::signals::signal<void ()(boost::shared_ptr<RBX::Instance>)>::connect<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::Accoutrement,boost::shared_ptr<RBX::Instance>>,boost::_bi::list2<boost::_bi::value<RBX::Accoutrement*>,boost::arg<1>>>>(boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::Accoutrement,boost::shared_ptr<RBX::Instance>>,boost::_bi::list2<boost::_bi::value<RBX::Accoutrement*>,boost::arg<1>>> const&) (boost::shared_ptr -> rbx_core::SharedPtr)
#[doc(alias = "rbx::signals::connection rbx::signals::signal<void ()(rbx_core::SharedPtr<RBX::Instance>)>::connect<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::Accoutrement,rbx_core::SharedPtr<RBX::Instance>>,boost::_bi::list2<boost::_bi::value<RBX::Accoutrement*>,boost::arg<1>>>>(boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::Accoutrement,rbx_core::SharedPtr<RBX::Instance>>,boost::_bi::list2<boost::_bi::value<RBX::Accoutrement*>,boost::arg<1>>> const&)")]
#[doc(alias = "__ZN3rbx7signals6signalIFvN5boost10shared_ptrIN3RBX8InstanceEEEEE7connectINS2_3_bi6bind_tIvNS2_4_mfi3mf1IvNS4_12AccoutrementES6_EENSA_5list2INSA_5valueIPSE_EENS2_3argILi1EEEEEEEEENS0_10connectionERKT_")]
pub fn stub_3903f0() -> ! {
    todo!("0x3903f0 __ZN3rbx7signals6signalIFvN5boost10shared_ptrIN3RBX8InstanceEEEEE7connectINS2_3_bi6bind_tIvNS2_4_mfi3mf1IvNS4_12AccoutrementES6_EENSA_5list2INSA_5valueIPSE_EENS2_3argILi1EEEEEEEEENS0_10connectionERKT_")
}

// 0x390464 — __ZN3RBX18DescribedCreatableINS_3HatENS_12AccoutrementELZNS_4sHatEELNS_10Reflection15ClassDescriptor13FunctionalityE27ELNS_8Security11PermissionsE0EEC2Ev — __ZN3RBX18DescribedCreatableINS_3HatENS_12AccoutrementELZNS_4sHatEELNS_10Reflection15ClassDescriptor13FunctionalityE27ELNS_8Security11PermissionsE0EEC2Ev
// type: __guard *__fastcall(int *, _DWORD *)
#[doc(alias = "__ZN3RBX18DescribedCreatableINS_3HatENS_12AccoutrementELZNS_4sHatEELNS_10Reflection15ClassDescriptor13FunctionalityE27ELNS_8Security11PermissionsE0EEC2Ev")]
#[doc(alias = "__ZN3RBX18DescribedCreatableINS_3HatENS_12AccoutrementELZNS_4sHatEELNS_10Reflection15ClassDescriptor13FunctionalityE27ELNS_8Security11PermissionsE0EEC2Ev")]
pub fn stub_390464() -> ! {
    todo!("0x390464 __ZN3RBX18DescribedCreatableINS_3HatENS_12AccoutrementELZNS_4sHatEELNS_10Reflection15ClassDescriptor13FunctionalityE27ELNS_8Security11PermissionsE0EEC2Ev")
}

// 0x390654 — __ZNK3RBX12Accoutrement11askAddChildEPKNS_8InstanceE — RBX::Accoutrement::askAddChild(RBX::Instance const*)const
// type: int __fastcall(RBX::Accoutrement *this, const RBX::Instance *)
#[doc(alias = "RBX::Accoutrement::askAddChild(RBX::Instance const*)const")]
#[doc(alias = "__ZNK3RBX12Accoutrement11askAddChildEPKNS_8InstanceE")]
pub fn stub_390654() -> ! {
    todo!("0x390654 __ZNK3RBX12Accoutrement11askAddChildEPKNS_8InstanceE")
}

// 0x390658 — __ZNK3RBX12Accoutrement12askSetParentEPKNS_8InstanceE — RBX::Accoutrement::askSetParent(RBX::Instance const*)const
// type: int __fastcall(RBX::Accoutrement *this, const RBX::Instance *)
#[doc(alias = "RBX::Accoutrement::askSetParent(RBX::Instance const*)const")]
#[doc(alias = "__ZNK3RBX12Accoutrement12askSetParentEPKNS_8InstanceE")]
pub fn stub_390658() -> ! {
    todo!("0x390658 __ZNK3RBX12Accoutrement12askSetParentEPKNS_8InstanceE")
}

// 0x39065c — __ZNK3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E12getClassNameEv — __ZNK3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E12getClassNameEv
// type: int()
#[doc(alias = "__ZNK3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E12getClassNameEv")]
#[doc(alias = "__ZNK3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E12getClassNameEv")]
pub fn stub_39065c() -> ! {
    todo!("0x39065c __ZNK3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E12getClassNameEv")
}

// 0x39066c — __ZN3RBX12Accoutrement17getRenderLocationEv — RBX::Accoutrement::getRenderLocation(void)
// type: int __fastcall(RBX::Accoutrement *this, int)
#[doc(alias = "RBX::Accoutrement::getRenderLocation(void)")]
#[doc(alias = "__ZN3RBX12Accoutrement17getRenderLocationEv")]
pub fn stub_39066c() -> ! {
    todo!("0x39066c __ZN3RBX12Accoutrement17getRenderLocationEv")
}

// 0x39067c — __ZN3RBX12Accoutrement13getRenderSizeEv — RBX::Accoutrement::getRenderSize(void)
// type: int __fastcall(RBX::Accoutrement *this, RBX::Accoutrement *)
#[doc(alias = "RBX::Accoutrement::getRenderSize(void)")]
#[doc(alias = "__ZN3RBX12Accoutrement13getRenderSizeEv")]
pub fn stub_39067c() -> ! {
    todo!("0x39067c __ZN3RBX12Accoutrement13getRenderSizeEv")
}

// 0x3906a4 — __ZThn32_NK3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E12getClassNameEv — __ZThn32_NK3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E12getClassNameEv
// type: int()
#[doc(alias = "__ZThn32_NK3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E12getClassNameEv")]
#[doc(alias = "__ZThn32_NK3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E12getClassNameEv")]
pub fn stub_3906a4() -> ! {
    todo!("0x3906a4 __ZThn32_NK3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E12getClassNameEv")
}

// 0x3906b4 — __ZThn128_N3RBX12Accoutrement17getRenderLocationEv — `non-virtual thunk to'RBX::Accoutrement::getRenderLocation(void)
// type: int __fastcall(RBX::Accoutrement *this, int)
#[doc(alias = "non-virtual thunk toRBX::Accoutrement::getRenderLocation(void)")]
#[doc(alias = "__ZThn128_N3RBX12Accoutrement17getRenderLocationEv")]
pub fn stub_3906b4() -> ! {
    todo!("0x3906b4 __ZThn128_N3RBX12Accoutrement17getRenderLocationEv")
}

// 0x3906c4 — __ZThn128_N3RBX12Accoutrement13getRenderSizeEv — `non-virtual thunk to'RBX::Accoutrement::getRenderSize(void)
// type: int __fastcall(RBX::Accoutrement *this)
#[doc(alias = "non-virtual thunk toRBX::Accoutrement::getRenderSize(void)")]
#[doc(alias = "__ZThn128_N3RBX12Accoutrement13getRenderSizeEv")]
pub fn stub_3906c4() -> ! {
    todo!("0x3906c4 __ZThn128_N3RBX12Accoutrement13getRenderSizeEv")
}

// 0x3906d0 — __ZN3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E7CreatorD1Ev — __ZN3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E7CreatorD1Ev
// type: int()
#[doc(alias = "__ZN3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E7CreatorD1Ev")]
#[doc(alias = "__ZN3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E7CreatorD1Ev")]
pub fn stub_3906d0() -> ! {
    todo!("0x3906d0 __ZN3RBX14FactoryProductINS_12AccoutrementENS_8InstanceELZNS_13sAccoutrementEES2_E7CreatorD1Ev")
}

// Auto-generated skeletons for rbx-datamodel — from ida/export.json
// Filter: RBX::Instance|DataModel|Workspace namespace (0 remaining) — gap filler EA-sorted asc distinct not yet in crates/datamodel/src
// Source: ida/export.json (85545 funcs, base 0x4000)
// Batch: 120 stubs | range 0x4b98bc..0x4bdcd0 | gap filler EA-sorted ascending next 120 after 0x4b98bb (shard dm_wd_watchdog11)
// SharedPtr = rbx_core::SharedPtr (Arc), not boost::shared_ptr; ` and ' stripped from alias
// Shard: dm_wd_watchdog11 EA-sorted ascending continuation after datamodel_wd_watchdog10 (global gap filler)

#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]

use rbx_core::SharedPtr;
const _SHARED_PTR: Option<SharedPtr<u8>> = None;

// 0x4b98bc — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_5Voxel9CellBlockEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_5Voxel9CellBlockEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4b98bc() -> ! {
    todo!("0x4b98bc __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_5Voxel9CellBlockEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")
}

// 0x4b98e4 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_5Voxel12CellMaterialEEEE13initSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Voxel::CellMaterial> const>::initSingleton(void)")]
pub fn stub_0x4b98e4() -> ! {
    todo!("0x4b98e4 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_5Voxel12CellMaterialEEEE13initSingletonEv")
}

// 0x4b98e8 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_5Voxel12CellMaterialEEEE14doGetSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Voxel::CellMaterial> const>::doGetSingleton(void)")]
pub fn stub_0x4b98e8() -> ! {
    todo!("0x4b98e8 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_5Voxel12CellMaterialEEEE14doGetSingletonEv")
}

// 0x4b99d8 — __ZN3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEED1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Voxel::CellMaterial>::~EnumDesc()")]
pub fn stub_0x4b99d8() -> ! {
    todo!("0x4b99d8 __ZN3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEED1Ev")
}

// 0x4b99dc — __ZN3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Voxel::CellMaterial>::~EnumDesc()")]
pub fn stub_0x4b99dc() -> ! {
    todo!("0x4b99dc __ZN3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEED2Ev")
}

// 0x4b9bb0 — __ZN3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEED0Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Voxel::CellMaterial>::~EnumDesc()")]
pub fn stub_0x4b9bb0() -> ! {
    todo!("0x4b9bb0 __ZN3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEED0Ev")
}

// 0x4b9c50 — __ZNK3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEE6lookupEPKc
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Voxel::CellMaterial>::lookup(char const*)const")]
pub fn stub_0x4b9c50() -> ! {
    todo!("0x4b9c50 __ZNK3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEE6lookupEPKc")
}

// 0x4b9c80 — __ZNK3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEE6lookupERKNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Voxel::CellMaterial>::lookup(RBX::Reflection::Variant const&)const")]
pub fn stub_0x4b9c80() -> ! {
    todo!("0x4b9c80 __ZNK3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEE6lookupERKNS0_7VariantE")
}

// 0x4b9ca0 — __ZNK3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEE14convertToValueEmRNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Voxel::CellMaterial>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
pub fn stub_0x4b9ca0() -> ! {
    todo!("0x4b9ca0 __ZNK3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEE14convertToValueEmRNS0_7VariantE")
}

// 0x4b9cfc — __ZNK3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Voxel::CellMaterial>::convertToString(unsigned long,std::string &)const")]
pub fn stub_0x4b9cfc() -> ! {
    todo!("0x4b9cfc __ZNK3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEE15convertToStringEmRSs")
}

// 0x4b9e40 — __ZNK3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEE15convertToStringERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Voxel::CellMaterial>::convertToString(RBX::Voxel::CellMaterial const&)const")]
pub fn stub_0x4b9e40() -> ! {
    todo!("0x4b9e40 __ZNK3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEE15convertToStringERKS3_")
}

// 0x4b9fe0 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_5Voxel12CellMaterialEEERS3_RKT_
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Voxel::CellMaterial>(RBX::Voxel::CellMaterial const&)")]
pub fn stub_0x4b9fe0() -> ! {
    todo!("0x4b9fe0 __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_5Voxel12CellMaterialEEERS3_RKT_")
}

// 0x4ba030 — __ZN3rbx14implementation12typed_holderIN3RBX5Voxel12CellMaterialEE9singletonEv
#[doc(alias = "rbx::implementation::typed_holder<RBX::Voxel::CellMaterial>::singleton(void)")]
pub fn stub_0x4ba030() -> ! {
    todo!("0x4ba030 __ZN3rbx14implementation12typed_holderIN3RBX5Voxel12CellMaterialEE9singletonEv")
}

// 0x4ba09c — __ZN3rbx14implementation12typed_holderIN3RBX5Voxel12CellMaterialEE14construct_funcEPKcPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::Voxel::CellMaterial>::construct_func(char const*,char *)")]
pub fn stub_0x4ba09c() -> ! {
    todo!("0x4ba09c __ZN3rbx14implementation12typed_holderIN3RBX5Voxel12CellMaterialEE14construct_funcEPKcPc")
}

// 0x4ba0a8 — __ZN3rbx14implementation12typed_holderIN3RBX5Voxel12CellMaterialEE13destruct_funcEPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::Voxel::CellMaterial>::destruct_func(char *)")]
pub fn stub_0x4ba0a8() -> ! {
    todo!("0x4ba0a8 __ZN3rbx14implementation12typed_holderIN3RBX5Voxel12CellMaterialEE13destruct_funcEPc")
}

// 0x4ba0ac — __ZNK3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEE13convertToItemERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Voxel::CellMaterial>::convertToItem(RBX::Voxel::CellMaterial const&)const")]
pub fn stub_0x4ba0ac() -> ! {
    todo!("0x4ba0ac __ZNK3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEE13convertToItemERKS3_")
}

// 0x4ba178 — __ZN3rbx8any_castIRKN3RBX5Voxel12CellMaterialENS1_7Region3EEET_RNS_13placement_anyIT0_EE
#[doc(alias = "RBX::Voxel::CellMaterial const& rbx::any_cast<RBX::Voxel::CellMaterial const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
pub fn stub_0x4ba178() -> ! {
    todo!("0x4ba178 __ZN3rbx8any_castIRKN3RBX5Voxel12CellMaterialENS1_7Region3EEET_RNS_13placement_anyIT0_EE")
}

// 0x4ba268 — __ZNK3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEE14convertToValueERKNS_4NameERS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Voxel::CellMaterial>::convertToValue(RBX::Name const&,RBX::Voxel::CellMaterial&)const")]
pub fn stub_0x4ba268() -> ! {
    todo!("0x4ba268 __ZNK3RBX10Reflection8EnumDescINS_5Voxel12CellMaterialEE14convertToValueERKNS_4NameERS3_")
}

// 0x4ba2e4 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_5Voxel12CellMaterialEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_5Voxel12CellMaterialEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4ba2e4() -> ! {
    todo!("0x4ba2e4 __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_5Voxel12CellMaterialEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")
}

// 0x4ba30c — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_10DialogRoot10DialogToneEEEE13initSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogTone> const>::initSingleton(void)")]
pub fn stub_0x4ba30c() -> ! {
    todo!("0x4ba30c __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_10DialogRoot10DialogToneEEEE13initSingletonEv")
}

// 0x4ba310 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_10DialogRoot10DialogToneEEEE14doGetSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogTone> const>::doGetSingleton(void)")]
pub fn stub_0x4ba310() -> ! {
    todo!("0x4ba310 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_10DialogRoot10DialogToneEEEE14doGetSingletonEv")
}

// 0x4ba400 — __ZN3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEED1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogTone>::~EnumDesc()")]
pub fn stub_0x4ba400() -> ! {
    todo!("0x4ba400 __ZN3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEED1Ev")
}

// 0x4ba404 — __ZN3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogTone>::~EnumDesc()")]
pub fn stub_0x4ba404() -> ! {
    todo!("0x4ba404 __ZN3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEED2Ev")
}

// 0x4ba5d8 — __ZN3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEED0Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogTone>::~EnumDesc()")]
pub fn stub_0x4ba5d8() -> ! {
    todo!("0x4ba5d8 __ZN3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEED0Ev")
}

// 0x4ba678 — __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEE6lookupEPKc
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogTone>::lookup(char const*)const")]
pub fn stub_0x4ba678() -> ! {
    todo!("0x4ba678 __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEE6lookupEPKc")
}

// 0x4ba6a8 — __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEE6lookupERKNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogTone>::lookup(RBX::Reflection::Variant const&)const")]
pub fn stub_0x4ba6a8() -> ! {
    todo!("0x4ba6a8 __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEE6lookupERKNS0_7VariantE")
}

// 0x4ba6c8 — __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEE14convertToValueEmRNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogTone>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
pub fn stub_0x4ba6c8() -> ! {
    todo!("0x4ba6c8 __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEE14convertToValueEmRNS0_7VariantE")
}

// 0x4ba724 — __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogTone>::convertToString(unsigned long,std::string &)const")]
pub fn stub_0x4ba724() -> ! {
    todo!("0x4ba724 __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEE15convertToStringEmRSs")
}

// 0x4ba868 — __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEE15convertToStringERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogTone>::convertToString(RBX::DialogRoot::DialogTone const&)const")]
pub fn stub_0x4ba868() -> ! {
    todo!("0x4ba868 __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEE15convertToStringERKS3_")
}

// 0x4baa08 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_10DialogRoot10DialogToneEEERS3_RKT_
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::DialogRoot::DialogTone>(RBX::DialogRoot::DialogTone const&)")]
pub fn stub_0x4baa08() -> ! {
    todo!("0x4baa08 __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_10DialogRoot10DialogToneEEERS3_RKT_")
}

// 0x4baa58 — __ZN3rbx14implementation12typed_holderIN3RBX10DialogRoot10DialogToneEE9singletonEv
#[doc(alias = "rbx::implementation::typed_holder<RBX::DialogRoot::DialogTone>::singleton(void)")]
pub fn stub_0x4baa58() -> ! {
    todo!("0x4baa58 __ZN3rbx14implementation12typed_holderIN3RBX10DialogRoot10DialogToneEE9singletonEv")
}

// 0x4baac4 — __ZN3rbx14implementation12typed_holderIN3RBX10DialogRoot10DialogToneEE14construct_funcEPKcPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::DialogRoot::DialogTone>::construct_func(char const*,char *)")]
pub fn stub_0x4baac4() -> ! {
    todo!("0x4baac4 __ZN3rbx14implementation12typed_holderIN3RBX10DialogRoot10DialogToneEE14construct_funcEPKcPc")
}

// 0x4baad0 — __ZN3rbx14implementation12typed_holderIN3RBX10DialogRoot10DialogToneEE13destruct_funcEPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::DialogRoot::DialogTone>::destruct_func(char *)")]
pub fn stub_0x4baad0() -> ! {
    todo!("0x4baad0 __ZN3rbx14implementation12typed_holderIN3RBX10DialogRoot10DialogToneEE13destruct_funcEPc")
}

// 0x4baad4 — __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEE13convertToItemERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogTone>::convertToItem(RBX::DialogRoot::DialogTone const&)const")]
pub fn stub_0x4baad4() -> ! {
    todo!("0x4baad4 __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEE13convertToItemERKS3_")
}

// 0x4baba0 — __ZN3rbx8any_castIRKN3RBX10DialogRoot10DialogToneENS1_7Region3EEET_RNS_13placement_anyIT0_EE
#[doc(alias = "RBX::DialogRoot::DialogTone const& rbx::any_cast<RBX::DialogRoot::DialogTone const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
pub fn stub_0x4baba0() -> ! {
    todo!("0x4baba0 __ZN3rbx8any_castIRKN3RBX10DialogRoot10DialogToneENS1_7Region3EEET_RNS_13placement_anyIT0_EE")
}

// 0x4bac90 — __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEE14convertToValueERKNS_4NameERS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogTone>::convertToValue(RBX::Name const&,RBX::DialogRoot::DialogTone&)const")]
pub fn stub_0x4bac90() -> ! {
    todo!("0x4bac90 __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot10DialogToneEE14convertToValueERKNS_4NameERS3_")
}

// 0x4bad0c — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_10DialogRoot10DialogToneEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_10DialogRoot10DialogToneEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4bad0c() -> ! {
    todo!("0x4bad0c __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_10DialogRoot10DialogToneEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")
}

// 0x4bad34 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_10DialogRoot13DialogPurposeEEEE13initSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogPurpose> const>::initSingleton(void)")]
pub fn stub_0x4bad34() -> ! {
    todo!("0x4bad34 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_10DialogRoot13DialogPurposeEEEE13initSingletonEv")
}

// 0x4bad38 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_10DialogRoot13DialogPurposeEEEE14doGetSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogPurpose> const>::doGetSingleton(void)")]
pub fn stub_0x4bad38() -> ! {
    todo!("0x4bad38 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_10DialogRoot13DialogPurposeEEEE14doGetSingletonEv")
}

// 0x4bae28 — __ZN3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEED1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogPurpose>::~EnumDesc()")]
pub fn stub_0x4bae28() -> ! {
    todo!("0x4bae28 __ZN3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEED1Ev")
}

// 0x4bae2c — __ZN3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogPurpose>::~EnumDesc()")]
pub fn stub_0x4bae2c() -> ! {
    todo!("0x4bae2c __ZN3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEED2Ev")
}

// 0x4bb000 — __ZN3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEED0Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogPurpose>::~EnumDesc()")]
pub fn stub_0x4bb000() -> ! {
    todo!("0x4bb000 __ZN3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEED0Ev")
}

// 0x4bb0a0 — __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEE6lookupEPKc
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogPurpose>::lookup(char const*)const")]
pub fn stub_0x4bb0a0() -> ! {
    todo!("0x4bb0a0 __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEE6lookupEPKc")
}

// 0x4bb0d0 — __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEE6lookupERKNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogPurpose>::lookup(RBX::Reflection::Variant const&)const")]
pub fn stub_0x4bb0d0() -> ! {
    todo!("0x4bb0d0 __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEE6lookupERKNS0_7VariantE")
}

// 0x4bb0f0 — __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEE14convertToValueEmRNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogPurpose>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
pub fn stub_0x4bb0f0() -> ! {
    todo!("0x4bb0f0 __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEE14convertToValueEmRNS0_7VariantE")
}

// 0x4bb14c — __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogPurpose>::convertToString(unsigned long,std::string &)const")]
pub fn stub_0x4bb14c() -> ! {
    todo!("0x4bb14c __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEE15convertToStringEmRSs")
}

// 0x4bb290 — __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEE15convertToStringERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogPurpose>::convertToString(RBX::DialogRoot::DialogPurpose const&)const")]
pub fn stub_0x4bb290() -> ! {
    todo!("0x4bb290 __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEE15convertToStringERKS3_")
}

// 0x4bb430 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_10DialogRoot13DialogPurposeEEERS3_RKT_
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::DialogRoot::DialogPurpose>(RBX::DialogRoot::DialogPurpose const&)")]
pub fn stub_0x4bb430() -> ! {
    todo!("0x4bb430 __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_10DialogRoot13DialogPurposeEEERS3_RKT_")
}

// 0x4bb480 — __ZN3rbx14implementation12typed_holderIN3RBX10DialogRoot13DialogPurposeEE9singletonEv
#[doc(alias = "rbx::implementation::typed_holder<RBX::DialogRoot::DialogPurpose>::singleton(void)")]
pub fn stub_0x4bb480() -> ! {
    todo!("0x4bb480 __ZN3rbx14implementation12typed_holderIN3RBX10DialogRoot13DialogPurposeEE9singletonEv")
}

// 0x4bb4ec — __ZN3rbx14implementation12typed_holderIN3RBX10DialogRoot13DialogPurposeEE14construct_funcEPKcPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::DialogRoot::DialogPurpose>::construct_func(char const*,char *)")]
pub fn stub_0x4bb4ec() -> ! {
    todo!("0x4bb4ec __ZN3rbx14implementation12typed_holderIN3RBX10DialogRoot13DialogPurposeEE14construct_funcEPKcPc")
}

// 0x4bb4f8 — __ZN3rbx14implementation12typed_holderIN3RBX10DialogRoot13DialogPurposeEE13destruct_funcEPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::DialogRoot::DialogPurpose>::destruct_func(char *)")]
pub fn stub_0x4bb4f8() -> ! {
    todo!("0x4bb4f8 __ZN3rbx14implementation12typed_holderIN3RBX10DialogRoot13DialogPurposeEE13destruct_funcEPc")
}

// 0x4bb4fc — __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEE13convertToItemERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogPurpose>::convertToItem(RBX::DialogRoot::DialogPurpose const&)const")]
pub fn stub_0x4bb4fc() -> ! {
    todo!("0x4bb4fc __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEE13convertToItemERKS3_")
}

// 0x4bb5c8 — __ZN3rbx8any_castIRKN3RBX10DialogRoot13DialogPurposeENS1_7Region3EEET_RNS_13placement_anyIT0_EE
#[doc(alias = "RBX::DialogRoot::DialogPurpose const& rbx::any_cast<RBX::DialogRoot::DialogPurpose const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
pub fn stub_0x4bb5c8() -> ! {
    todo!("0x4bb5c8 __ZN3rbx8any_castIRKN3RBX10DialogRoot13DialogPurposeENS1_7Region3EEET_RNS_13placement_anyIT0_EE")
}

// 0x4bb6b8 — __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEE14convertToValueERKNS_4NameERS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DialogRoot::DialogPurpose>::convertToValue(RBX::Name const&,RBX::DialogRoot::DialogPurpose&)const")]
pub fn stub_0x4bb6b8() -> ! {
    todo!("0x4bb6b8 __ZNK3RBX10Reflection8EnumDescINS_10DialogRoot13DialogPurposeEE14convertToValueERKNS_4NameERS3_")
}

// 0x4bb734 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_10DialogRoot13DialogPurposeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_10DialogRoot13DialogPurposeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4bb734() -> ! {
    todo!("0x4bb734 __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_10DialogRoot13DialogPurposeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")
}

// 0x4bb75c — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiButton5StyleEEEE13initSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::GuiButton::Style> const>::initSingleton(void)")]
pub fn stub_0x4bb75c() -> ! {
    todo!("0x4bb75c __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiButton5StyleEEEE13initSingletonEv")
}

// 0x4bb760 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiButton5StyleEEEE14doGetSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::GuiButton::Style> const>::doGetSingleton(void)")]
pub fn stub_0x4bb760() -> ! {
    todo!("0x4bb760 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiButton5StyleEEEE14doGetSingletonEv")
}

// 0x4bb850 — __ZN3RBX10Reflection8EnumDescINS_9GuiButton5StyleEED1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiButton::Style>::~EnumDesc()")]
pub fn stub_0x4bb850() -> ! {
    todo!("0x4bb850 __ZN3RBX10Reflection8EnumDescINS_9GuiButton5StyleEED1Ev")
}

// 0x4bb854 — __ZN3RBX10Reflection8EnumDescINS_9GuiButton5StyleEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiButton::Style>::~EnumDesc()")]
pub fn stub_0x4bb854() -> ! {
    todo!("0x4bb854 __ZN3RBX10Reflection8EnumDescINS_9GuiButton5StyleEED2Ev")
}

// 0x4bba28 — __ZN3RBX10Reflection8EnumDescINS_9GuiButton5StyleEED0Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiButton::Style>::~EnumDesc()")]
pub fn stub_0x4bba28() -> ! {
    todo!("0x4bba28 __ZN3RBX10Reflection8EnumDescINS_9GuiButton5StyleEED0Ev")
}

// 0x4bbac8 — __ZNK3RBX10Reflection8EnumDescINS_9GuiButton5StyleEE6lookupEPKc
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiButton::Style>::lookup(char const*)const")]
pub fn stub_0x4bbac8() -> ! {
    todo!("0x4bbac8 __ZNK3RBX10Reflection8EnumDescINS_9GuiButton5StyleEE6lookupEPKc")
}

// 0x4bbaf8 — __ZNK3RBX10Reflection8EnumDescINS_9GuiButton5StyleEE6lookupERKNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiButton::Style>::lookup(RBX::Reflection::Variant const&)const")]
pub fn stub_0x4bbaf8() -> ! {
    todo!("0x4bbaf8 __ZNK3RBX10Reflection8EnumDescINS_9GuiButton5StyleEE6lookupERKNS0_7VariantE")
}

// 0x4bbb18 — __ZNK3RBX10Reflection8EnumDescINS_9GuiButton5StyleEE14convertToValueEmRNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiButton::Style>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
pub fn stub_0x4bbb18() -> ! {
    todo!("0x4bbb18 __ZNK3RBX10Reflection8EnumDescINS_9GuiButton5StyleEE14convertToValueEmRNS0_7VariantE")
}

// 0x4bbb74 — __ZNK3RBX10Reflection8EnumDescINS_9GuiButton5StyleEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiButton::Style>::convertToString(unsigned long,std::string &)const")]
pub fn stub_0x4bbb74() -> ! {
    todo!("0x4bbb74 __ZNK3RBX10Reflection8EnumDescINS_9GuiButton5StyleEE15convertToStringEmRSs")
}

// 0x4bbcb8 — __ZNK3RBX10Reflection8EnumDescINS_9GuiButton5StyleEE15convertToStringERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiButton::Style>::convertToString(RBX::GuiButton::Style const&)const")]
pub fn stub_0x4bbcb8() -> ! {
    todo!("0x4bbcb8 __ZNK3RBX10Reflection8EnumDescINS_9GuiButton5StyleEE15convertToStringERKS3_")
}

// 0x4bbe58 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_9GuiButton5StyleEEERS3_RKT_
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::GuiButton::Style>(RBX::GuiButton::Style const&)")]
pub fn stub_0x4bbe58() -> ! {
    todo!("0x4bbe58 __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_9GuiButton5StyleEEERS3_RKT_")
}

// 0x4bbea8 — __ZN3rbx14implementation12typed_holderIN3RBX9GuiButton5StyleEE9singletonEv
#[doc(alias = "rbx::implementation::typed_holder<RBX::GuiButton::Style>::singleton(void)")]
pub fn stub_0x4bbea8() -> ! {
    todo!("0x4bbea8 __ZN3rbx14implementation12typed_holderIN3RBX9GuiButton5StyleEE9singletonEv")
}

// 0x4bbf14 — __ZN3rbx14implementation12typed_holderIN3RBX9GuiButton5StyleEE14construct_funcEPKcPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::GuiButton::Style>::construct_func(char const*,char *)")]
pub fn stub_0x4bbf14() -> ! {
    todo!("0x4bbf14 __ZN3rbx14implementation12typed_holderIN3RBX9GuiButton5StyleEE14construct_funcEPKcPc")
}

// 0x4bbf20 — __ZN3rbx14implementation12typed_holderIN3RBX9GuiButton5StyleEE13destruct_funcEPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::GuiButton::Style>::destruct_func(char *)")]
pub fn stub_0x4bbf20() -> ! {
    todo!("0x4bbf20 __ZN3rbx14implementation12typed_holderIN3RBX9GuiButton5StyleEE13destruct_funcEPc")
}

// 0x4bbf24 — __ZNK3RBX10Reflection8EnumDescINS_9GuiButton5StyleEE13convertToItemERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiButton::Style>::convertToItem(RBX::GuiButton::Style const&)const")]
pub fn stub_0x4bbf24() -> ! {
    todo!("0x4bbf24 __ZNK3RBX10Reflection8EnumDescINS_9GuiButton5StyleEE13convertToItemERKS3_")
}

// 0x4bbff0 — __ZN3rbx8any_castIRKN3RBX9GuiButton5StyleENS1_7Region3EEET_RNS_13placement_anyIT0_EE
#[doc(alias = "RBX::GuiButton::Style const& rbx::any_cast<RBX::GuiButton::Style const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
pub fn stub_0x4bbff0() -> ! {
    todo!("0x4bbff0 __ZN3rbx8any_castIRKN3RBX9GuiButton5StyleENS1_7Region3EEET_RNS_13placement_anyIT0_EE")
}

// 0x4bc0e0 — __ZNK3RBX10Reflection8EnumDescINS_9GuiButton5StyleEE14convertToValueERKNS_4NameERS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiButton::Style>::convertToValue(RBX::Name const&,RBX::GuiButton::Style&)const")]
pub fn stub_0x4bc0e0() -> ! {
    todo!("0x4bc0e0 __ZNK3RBX10Reflection8EnumDescINS_9GuiButton5StyleEE14convertToValueERKNS_4NameERS3_")
}

// 0x4bc15c — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_9GuiButton5StyleEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_9GuiButton5StyleEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4bc15c() -> ! {
    todo!("0x4bc15c __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_9GuiButton5StyleEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")
}

// 0x4bc184 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_5Frame5StyleEEEE13initSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Frame::Style> const>::initSingleton(void)")]
pub fn stub_0x4bc184() -> ! {
    todo!("0x4bc184 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_5Frame5StyleEEEE13initSingletonEv")
}

// 0x4bc188 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_5Frame5StyleEEEE14doGetSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Frame::Style> const>::doGetSingleton(void)")]
pub fn stub_0x4bc188() -> ! {
    todo!("0x4bc188 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_5Frame5StyleEEEE14doGetSingletonEv")
}

// 0x4bc278 — __ZN3RBX10Reflection8EnumDescINS_5Frame5StyleEED1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Frame::Style>::~EnumDesc()")]
pub fn stub_0x4bc278() -> ! {
    todo!("0x4bc278 __ZN3RBX10Reflection8EnumDescINS_5Frame5StyleEED1Ev")
}

// 0x4bc27c — __ZN3RBX10Reflection8EnumDescINS_5Frame5StyleEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Frame::Style>::~EnumDesc()")]
pub fn stub_0x4bc27c() -> ! {
    todo!("0x4bc27c __ZN3RBX10Reflection8EnumDescINS_5Frame5StyleEED2Ev")
}

// 0x4bc450 — __ZN3RBX10Reflection8EnumDescINS_5Frame5StyleEED0Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Frame::Style>::~EnumDesc()")]
pub fn stub_0x4bc450() -> ! {
    todo!("0x4bc450 __ZN3RBX10Reflection8EnumDescINS_5Frame5StyleEED0Ev")
}

// 0x4bc4f0 — __ZNK3RBX10Reflection8EnumDescINS_5Frame5StyleEE6lookupEPKc
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Frame::Style>::lookup(char const*)const")]
pub fn stub_0x4bc4f0() -> ! {
    todo!("0x4bc4f0 __ZNK3RBX10Reflection8EnumDescINS_5Frame5StyleEE6lookupEPKc")
}

// 0x4bc520 — __ZNK3RBX10Reflection8EnumDescINS_5Frame5StyleEE6lookupERKNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Frame::Style>::lookup(RBX::Reflection::Variant const&)const")]
pub fn stub_0x4bc520() -> ! {
    todo!("0x4bc520 __ZNK3RBX10Reflection8EnumDescINS_5Frame5StyleEE6lookupERKNS0_7VariantE")
}

// 0x4bc540 — __ZNK3RBX10Reflection8EnumDescINS_5Frame5StyleEE14convertToValueEmRNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Frame::Style>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
pub fn stub_0x4bc540() -> ! {
    todo!("0x4bc540 __ZNK3RBX10Reflection8EnumDescINS_5Frame5StyleEE14convertToValueEmRNS0_7VariantE")
}

// 0x4bc59c — __ZNK3RBX10Reflection8EnumDescINS_5Frame5StyleEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Frame::Style>::convertToString(unsigned long,std::string &)const")]
pub fn stub_0x4bc59c() -> ! {
    todo!("0x4bc59c __ZNK3RBX10Reflection8EnumDescINS_5Frame5StyleEE15convertToStringEmRSs")
}

// 0x4bc6e0 — __ZNK3RBX10Reflection8EnumDescINS_5Frame5StyleEE15convertToStringERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Frame::Style>::convertToString(RBX::Frame::Style const&)const")]
pub fn stub_0x4bc6e0() -> ! {
    todo!("0x4bc6e0 __ZNK3RBX10Reflection8EnumDescINS_5Frame5StyleEE15convertToStringERKS3_")
}

// 0x4bc880 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_5Frame5StyleEEERS3_RKT_
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Frame::Style>(RBX::Frame::Style const&)")]
pub fn stub_0x4bc880() -> ! {
    todo!("0x4bc880 __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_5Frame5StyleEEERS3_RKT_")
}

// 0x4bc8d0 — __ZN3rbx14implementation12typed_holderIN3RBX5Frame5StyleEE9singletonEv
#[doc(alias = "rbx::implementation::typed_holder<RBX::Frame::Style>::singleton(void)")]
pub fn stub_0x4bc8d0() -> ! {
    todo!("0x4bc8d0 __ZN3rbx14implementation12typed_holderIN3RBX5Frame5StyleEE9singletonEv")
}

// 0x4bc93c — __ZN3rbx14implementation12typed_holderIN3RBX5Frame5StyleEE14construct_funcEPKcPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::Frame::Style>::construct_func(char const*,char *)")]
pub fn stub_0x4bc93c() -> ! {
    todo!("0x4bc93c __ZN3rbx14implementation12typed_holderIN3RBX5Frame5StyleEE14construct_funcEPKcPc")
}

// 0x4bc948 — __ZN3rbx14implementation12typed_holderIN3RBX5Frame5StyleEE13destruct_funcEPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::Frame::Style>::destruct_func(char *)")]
pub fn stub_0x4bc948() -> ! {
    todo!("0x4bc948 __ZN3rbx14implementation12typed_holderIN3RBX5Frame5StyleEE13destruct_funcEPc")
}

// 0x4bc94c — __ZNK3RBX10Reflection8EnumDescINS_5Frame5StyleEE13convertToItemERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Frame::Style>::convertToItem(RBX::Frame::Style const&)const")]
pub fn stub_0x4bc94c() -> ! {
    todo!("0x4bc94c __ZNK3RBX10Reflection8EnumDescINS_5Frame5StyleEE13convertToItemERKS3_")
}

// 0x4bca18 — __ZN3rbx8any_castIRKN3RBX5Frame5StyleENS1_7Region3EEET_RNS_13placement_anyIT0_EE
#[doc(alias = "RBX::Frame::Style const& rbx::any_cast<RBX::Frame::Style const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
pub fn stub_0x4bca18() -> ! {
    todo!("0x4bca18 __ZN3rbx8any_castIRKN3RBX5Frame5StyleENS1_7Region3EEET_RNS_13placement_anyIT0_EE")
}

// 0x4bcb08 — __ZNK3RBX10Reflection8EnumDescINS_5Frame5StyleEE14convertToValueERKNS_4NameERS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Frame::Style>::convertToValue(RBX::Name const&,RBX::Frame::Style&)const")]
pub fn stub_0x4bcb08() -> ! {
    todo!("0x4bcb08 __ZNK3RBX10Reflection8EnumDescINS_5Frame5StyleEE14convertToValueERKNS_4NameERS3_")
}

// 0x4bcb84 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_5Frame5StyleEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_5Frame5StyleEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4bcb84() -> ! {
    todo!("0x4bcb84 __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_5Frame5StyleEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")
}

// 0x4bcbac — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17GameBasicSettings20RenderQualitySettingEEEE13initSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::GameBasicSettings::RenderQualitySetting> const>::initSingleton(void)")]
pub fn stub_0x4bcbac() -> ! {
    todo!("0x4bcbac __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17GameBasicSettings20RenderQualitySettingEEEE13initSingletonEv")
}

// 0x4bcbb0 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17GameBasicSettings20RenderQualitySettingEEEE14doGetSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::GameBasicSettings::RenderQualitySetting> const>::doGetSingleton(void)")]
pub fn stub_0x4bcbb0() -> ! {
    todo!("0x4bcbb0 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17GameBasicSettings20RenderQualitySettingEEEE14doGetSingletonEv")
}

// 0x4bcca0 — __ZN3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEED1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::RenderQualitySetting>::~EnumDesc()")]
pub fn stub_0x4bcca0() -> ! {
    todo!("0x4bcca0 __ZN3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEED1Ev")
}

// 0x4bcca4 — __ZN3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::RenderQualitySetting>::~EnumDesc()")]
pub fn stub_0x4bcca4() -> ! {
    todo!("0x4bcca4 __ZN3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEED2Ev")
}

// 0x4bce78 — __ZN3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEED0Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::RenderQualitySetting>::~EnumDesc()")]
pub fn stub_0x4bce78() -> ! {
    todo!("0x4bce78 __ZN3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEED0Ev")
}

// 0x4bcf18 — __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEE6lookupEPKc
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::RenderQualitySetting>::lookup(char const*)const")]
pub fn stub_0x4bcf18() -> ! {
    todo!("0x4bcf18 __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEE6lookupEPKc")
}

// 0x4bcf48 — __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEE6lookupERKNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::RenderQualitySetting>::lookup(RBX::Reflection::Variant const&)const")]
pub fn stub_0x4bcf48() -> ! {
    todo!("0x4bcf48 __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEE6lookupERKNS0_7VariantE")
}

// 0x4bcf68 — __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEE14convertToValueEmRNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::RenderQualitySetting>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
pub fn stub_0x4bcf68() -> ! {
    todo!("0x4bcf68 __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEE14convertToValueEmRNS0_7VariantE")
}

// 0x4bcfc4 — __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::RenderQualitySetting>::convertToString(unsigned long,std::string &)const")]
pub fn stub_0x4bcfc4() -> ! {
    todo!("0x4bcfc4 __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEE15convertToStringEmRSs")
}

// 0x4bd108 — __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEE15convertToStringERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::RenderQualitySetting>::convertToString(RBX::GameBasicSettings::RenderQualitySetting const&)const")]
pub fn stub_0x4bd108() -> ! {
    todo!("0x4bd108 __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEE15convertToStringERKS3_")
}

// 0x4bd2a8 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_17GameBasicSettings20RenderQualitySettingEEERS3_RKT_
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::GameBasicSettings::RenderQualitySetting>(RBX::GameBasicSettings::RenderQualitySetting const&)")]
pub fn stub_0x4bd2a8() -> ! {
    todo!("0x4bd2a8 __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_17GameBasicSettings20RenderQualitySettingEEERS3_RKT_")
}

// 0x4bd2f8 — __ZN3rbx14implementation12typed_holderIN3RBX17GameBasicSettings20RenderQualitySettingEE9singletonEv
#[doc(alias = "rbx::implementation::typed_holder<RBX::GameBasicSettings::RenderQualitySetting>::singleton(void)")]
pub fn stub_0x4bd2f8() -> ! {
    todo!("0x4bd2f8 __ZN3rbx14implementation12typed_holderIN3RBX17GameBasicSettings20RenderQualitySettingEE9singletonEv")
}

// 0x4bd364 — __ZN3rbx14implementation12typed_holderIN3RBX17GameBasicSettings20RenderQualitySettingEE14construct_funcEPKcPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::GameBasicSettings::RenderQualitySetting>::construct_func(char const*,char *)")]
pub fn stub_0x4bd364() -> ! {
    todo!("0x4bd364 __ZN3rbx14implementation12typed_holderIN3RBX17GameBasicSettings20RenderQualitySettingEE14construct_funcEPKcPc")
}

// 0x4bd370 — __ZN3rbx14implementation12typed_holderIN3RBX17GameBasicSettings20RenderQualitySettingEE13destruct_funcEPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::GameBasicSettings::RenderQualitySetting>::destruct_func(char *)")]
pub fn stub_0x4bd370() -> ! {
    todo!("0x4bd370 __ZN3rbx14implementation12typed_holderIN3RBX17GameBasicSettings20RenderQualitySettingEE13destruct_funcEPc")
}

// 0x4bd374 — __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEE13convertToItemERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::RenderQualitySetting>::convertToItem(RBX::GameBasicSettings::RenderQualitySetting const&)const")]
pub fn stub_0x4bd374() -> ! {
    todo!("0x4bd374 __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEE13convertToItemERKS3_")
}

// 0x4bd440 — __ZN3rbx8any_castIRKN3RBX17GameBasicSettings20RenderQualitySettingENS1_7Region3EEET_RNS_13placement_anyIT0_EE
#[doc(alias = "RBX::GameBasicSettings::RenderQualitySetting const& rbx::any_cast<RBX::GameBasicSettings::RenderQualitySetting const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
pub fn stub_0x4bd440() -> ! {
    todo!("0x4bd440 __ZN3rbx8any_castIRKN3RBX17GameBasicSettings20RenderQualitySettingENS1_7Region3EEET_RNS_13placement_anyIT0_EE")
}

// 0x4bd530 — __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEE14convertToValueERKNS_4NameERS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::RenderQualitySetting>::convertToValue(RBX::Name const&,RBX::GameBasicSettings::RenderQualitySetting&)const")]
pub fn stub_0x4bd530() -> ! {
    todo!("0x4bd530 __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings20RenderQualitySettingEE14convertToValueERKNS_4NameERS3_")
}

// 0x4bd5ac — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_17GameBasicSettings20RenderQualitySettingEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_17GameBasicSettings20RenderQualitySettingEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4bd5ac() -> ! {
    todo!("0x4bd5ac __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_17GameBasicSettings20RenderQualitySettingEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")
}

// 0x4bd5d4 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17GameBasicSettings11ControlModeEEEE13initSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::GameBasicSettings::ControlMode> const>::initSingleton(void)")]
pub fn stub_0x4bd5d4() -> ! {
    todo!("0x4bd5d4 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17GameBasicSettings11ControlModeEEEE13initSingletonEv")
}

// 0x4bd5d8 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17GameBasicSettings11ControlModeEEEE14doGetSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::GameBasicSettings::ControlMode> const>::doGetSingleton(void)")]
pub fn stub_0x4bd5d8() -> ! {
    todo!("0x4bd5d8 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17GameBasicSettings11ControlModeEEEE14doGetSingletonEv")
}

// 0x4bd6c8 — __ZN3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEED1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::ControlMode>::~EnumDesc()")]
pub fn stub_0x4bd6c8() -> ! {
    todo!("0x4bd6c8 __ZN3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEED1Ev")
}

// 0x4bd6cc — __ZN3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::ControlMode>::~EnumDesc()")]
pub fn stub_0x4bd6cc() -> ! {
    todo!("0x4bd6cc __ZN3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEED2Ev")
}

// 0x4bd8a0 — __ZN3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEED0Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::ControlMode>::~EnumDesc()")]
pub fn stub_0x4bd8a0() -> ! {
    todo!("0x4bd8a0 __ZN3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEED0Ev")
}

// 0x4bd940 — __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEE6lookupEPKc
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::ControlMode>::lookup(char const*)const")]
pub fn stub_0x4bd940() -> ! {
    todo!("0x4bd940 __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEE6lookupEPKc")
}

// 0x4bd970 — __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEE6lookupERKNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::ControlMode>::lookup(RBX::Reflection::Variant const&)const")]
pub fn stub_0x4bd970() -> ! {
    todo!("0x4bd970 __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEE6lookupERKNS0_7VariantE")
}

// 0x4bd990 — __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEE14convertToValueEmRNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::ControlMode>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
pub fn stub_0x4bd990() -> ! {
    todo!("0x4bd990 __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEE14convertToValueEmRNS0_7VariantE")
}

// 0x4bd9ec — __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::ControlMode>::convertToString(unsigned long,std::string &)const")]
pub fn stub_0x4bd9ec() -> ! {
    todo!("0x4bd9ec __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEE15convertToStringEmRSs")
}

// 0x4bdb30 — __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEE15convertToStringERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GameBasicSettings::ControlMode>::convertToString(RBX::GameBasicSettings::ControlMode const&)const")]
pub fn stub_0x4bdb30() -> ! {
    todo!("0x4bdb30 __ZNK3RBX10Reflection8EnumDescINS_17GameBasicSettings11ControlModeEE15convertToStringERKS3_")
}

// 0x4bdcd0 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_17GameBasicSettings11ControlModeEEERS3_RKT_
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::GameBasicSettings::ControlMode>(RBX::GameBasicSettings::ControlMode const&)")]
pub fn stub_0x4bdcd0() -> ! {
    todo!("0x4bdcd0 __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_17GameBasicSettings11ControlModeEEERS3_RKT_")
}

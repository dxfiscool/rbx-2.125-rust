// Auto-generated skeletons for rbx-datamodel — from ida/export.json
// Filter: global gap filler EA-sorted asc distinct not yet in crates/datamodel/src
// Source: ida/export.json (85545 funcs, base 0x4000)
// Batch: 100 stubs | range 0x4caf98..0x4ce958 EA-sorted ascending next uncovered after 0x4caf94 (watchdog14)
// SharedPtr = rbx_core::SharedPtr (Arc), not boost::shared_ptr;  and ' stripped from alias, boost::shared_ptr -> rbx_core::SharedPtr
// Shard: dm_wd_watchdog14b EA-sorted ascending next uncovered distinct not yet in crates/datamodel/src after generated_dm_wd_watchdog14 (0x4c4358..0x4caf94)

#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]

use rbx_core::SharedPtr;
const _SHARED_PTR: Option<SharedPtr<u8>> = None;

// 0x4caf98 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescIN3G3D7Vector34AxisEEEE14doGetSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<G3D::Vector3::Axis> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescIN3G3D7Vector34AxisEEEE14doGetSingletonEv")]
pub fn stub_0x4caf98() -> ! {
    todo!("0x4caf98 RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<G3D::Vector3::Axis> const>::doGetSingleton(void)")
}

// 0x4cb088 — __ZN3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEED1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEED1Ev")]
pub fn stub_0x4cb088() -> ! {
    todo!("0x4cb088 RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::~EnumDesc()")
}

// 0x4cb08c — __ZN3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEED2Ev")]
pub fn stub_0x4cb08c() -> ! {
    todo!("0x4cb08c RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::~EnumDesc()")
}

// 0x4cb260 — __ZN3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEED0Ev
#[doc(alias = "RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEED0Ev")]
pub fn stub_0x4cb260() -> ! {
    todo!("0x4cb260 RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::~EnumDesc()")
}

// 0x4cb300 — __ZNK3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEE6lookupEPKc
#[doc(alias = "RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEE6lookupEPKc")]
pub fn stub_0x4cb300() -> ! {
    todo!("0x4cb300 RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::lookup(char const*)const")
}

// 0x4cb330 — __ZNK3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEE6lookupERKNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEE6lookupERKNS0_7VariantE")]
pub fn stub_0x4cb330() -> ! {
    todo!("0x4cb330 RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::lookup(RBX::Reflection::Variant const&)const")
}

// 0x4cb350 — __ZNK3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEE14convertToValueEmRNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_0x4cb350() -> ! {
    todo!("0x4cb350 RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")
}

// 0x4cb3ac — __ZNK3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEE15convertToStringEmRSs")]
pub fn stub_0x4cb3ac() -> ! {
    todo!("0x4cb3ac RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::convertToString(unsigned long,std::string &)const")
}

// 0x4cb4f0 — __ZNK3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEE15convertToStringERKS4_
#[doc(alias = "RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::convertToString(G3D::Vector3::Axis const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEE15convertToStringERKS4_")]
pub fn stub_0x4cb4f0() -> ! {
    todo!("0x4cb4f0 RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::convertToString(G3D::Vector3::Axis const&)const")
}

// 0x4cb690 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSIN3G3D7Vector34AxisEEERS3_RKT_
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<G3D::Vector3::Axis>(G3D::Vector3::Axis const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSIN3G3D7Vector34AxisEEERS3_RKT_")]
pub fn stub_0x4cb690() -> ! {
    todo!("0x4cb690 rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<G3D::Vector3::Axis>(G3D::Vector3::Axis const&)")
}

// 0x4cb6e0 — __ZN3rbx14implementation12typed_holderIN3G3D7Vector34AxisEE9singletonEv
#[doc(alias = "rbx::implementation::typed_holder<G3D::Vector3::Axis>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3G3D7Vector34AxisEE9singletonEv")]
pub fn stub_0x4cb6e0() -> ! {
    todo!("0x4cb6e0 rbx::implementation::typed_holder<G3D::Vector3::Axis>::singleton(void)")
}

// 0x4cb74c — __ZN3rbx14implementation12typed_holderIN3G3D7Vector34AxisEE14construct_funcEPKcPc
#[doc(alias = "rbx::implementation::typed_holder<G3D::Vector3::Axis>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3G3D7Vector34AxisEE14construct_funcEPKcPc")]
pub fn stub_0x4cb74c() -> ! {
    todo!("0x4cb74c rbx::implementation::typed_holder<G3D::Vector3::Axis>::construct_func(char const*,char *)")
}

// 0x4cb758 — __ZN3rbx14implementation12typed_holderIN3G3D7Vector34AxisEE13destruct_funcEPc
#[doc(alias = "rbx::implementation::typed_holder<G3D::Vector3::Axis>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3G3D7Vector34AxisEE13destruct_funcEPc")]
pub fn stub_0x4cb758() -> ! {
    todo!("0x4cb758 rbx::implementation::typed_holder<G3D::Vector3::Axis>::destruct_func(char *)")
}

// 0x4cb75c — __ZNK3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEE13convertToItemERKS4_
#[doc(alias = "RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::convertToItem(G3D::Vector3::Axis const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEE13convertToItemERKS4_")]
pub fn stub_0x4cb75c() -> ! {
    todo!("0x4cb75c RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::convertToItem(G3D::Vector3::Axis const&)const")
}

// 0x4cb828 — __ZN3rbx8any_castIRKN3G3D7Vector34AxisEN3RBX7Region3EEET_RNS_13placement_anyIT0_EE
#[doc(alias = "G3D::Vector3::Axis const& rbx::any_cast<G3D::Vector3::Axis const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3G3D7Vector34AxisEN3RBX7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_0x4cb828() -> ! {
    todo!("0x4cb828 G3D::Vector3::Axis const& rbx::any_cast<G3D::Vector3::Axis const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")
}

// 0x4cb918 — __ZNK3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEE14convertToValueERKNS_4NameERS4_
#[doc(alias = "RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::convertToValue(RBX::Name const&,G3D::Vector3::Axis&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescIN3G3D7Vector34AxisEE14convertToValueERKNS_4NameERS4_")]
pub fn stub_0x4cb918() -> ! {
    todo!("0x4cb918 RBX::Reflection::EnumDesc<G3D::Vector3::Axis>::convertToValue(RBX::Name const&,G3D::Vector3::Axis&)const")
}

// 0x4cb994 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_N3G3D7Vector34AxisEESt10_Select1stIS9_ESt4lessIS3_ESaIS9_EE8_M_eraseEPSt13_Rb_tree_nodeIS9_E
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,G3D::Vector3::Axis>,std::_Select1st<std::pair<RBX::Name const* const,G3D::Vector3::Axis>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,G3D::Vector3::Axis>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,G3D::Vector3::Axis>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_N3G3D7Vector34AxisEESt10_Select1stIS9_ESt4lessIS3_ESaIS9_EE8_M_eraseEPSt13_Rb_tree_nodeIS9_E")]
pub fn stub_0x4cb994() -> ! {
    todo!("0x4cb994 std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,G3D::Vector3::Axis>,std::_Select1st<std::pair<RBX::Name const* const,G3D::Vector3::Axis>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,G3D::Vector3::Axis>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,G3D::Vector3::Axis>> *)")
}

// 0x4cb9bc — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_6Legacy17SurfaceConstraintEEEE13initSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_6Legacy17SurfaceConstraintEEEE13initSingletonEv")]
pub fn stub_0x4cb9bc() -> ! {
    todo!("0x4cb9bc RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint> const>::initSingleton(void)")
}

// 0x4cb9c0 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_6Legacy17SurfaceConstraintEEEE14doGetSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_6Legacy17SurfaceConstraintEEEE14doGetSingletonEv")]
pub fn stub_0x4cb9c0() -> ! {
    todo!("0x4cb9c0 RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint> const>::doGetSingleton(void)")
}

// 0x4cbab0 — __ZN3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEED1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEED1Ev")]
pub fn stub_0x4cbab0() -> ! {
    todo!("0x4cbab0 RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::~EnumDesc()")
}

// 0x4cbab4 — __ZN3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEED2Ev")]
pub fn stub_0x4cbab4() -> ! {
    todo!("0x4cbab4 RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::~EnumDesc()")
}

// 0x4cbc88 — __ZN3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEED0Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEED0Ev")]
pub fn stub_0x4cbc88() -> ! {
    todo!("0x4cbc88 RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::~EnumDesc()")
}

// 0x4cbd28 — __ZNK3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEE6lookupEPKc
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEE6lookupEPKc")]
pub fn stub_0x4cbd28() -> ! {
    todo!("0x4cbd28 RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::lookup(char const*)const")
}

// 0x4cbd58 — __ZNK3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEE6lookupERKNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEE6lookupERKNS0_7VariantE")]
pub fn stub_0x4cbd58() -> ! {
    todo!("0x4cbd58 RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::lookup(RBX::Reflection::Variant const&)const")
}

// 0x4cbd78 — __ZNK3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEE14convertToValueEmRNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_0x4cbd78() -> ! {
    todo!("0x4cbd78 RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")
}

// 0x4cbdd4 — __ZNK3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEE15convertToStringEmRSs")]
pub fn stub_0x4cbdd4() -> ! {
    todo!("0x4cbdd4 RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::convertToString(unsigned long,std::string &)const")
}

// 0x4cbf18 — __ZNK3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEE15convertToStringERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::convertToString(RBX::Legacy::SurfaceConstraint const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEE15convertToStringERKS3_")]
pub fn stub_0x4cbf18() -> ! {
    todo!("0x4cbf18 RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::convertToString(RBX::Legacy::SurfaceConstraint const&)const")
}

// 0x4cc0b8 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_6Legacy17SurfaceConstraintEEERS3_RKT_
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Legacy::SurfaceConstraint>(RBX::Legacy::SurfaceConstraint const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_6Legacy17SurfaceConstraintEEERS3_RKT_")]
pub fn stub_0x4cc0b8() -> ! {
    todo!("0x4cc0b8 rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Legacy::SurfaceConstraint>(RBX::Legacy::SurfaceConstraint const&)")
}

// 0x4cc108 — __ZN3rbx14implementation12typed_holderIN3RBX6Legacy17SurfaceConstraintEE9singletonEv
#[doc(alias = "rbx::implementation::typed_holder<RBX::Legacy::SurfaceConstraint>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX6Legacy17SurfaceConstraintEE9singletonEv")]
pub fn stub_0x4cc108() -> ! {
    todo!("0x4cc108 rbx::implementation::typed_holder<RBX::Legacy::SurfaceConstraint>::singleton(void)")
}

// 0x4cc174 — __ZN3rbx14implementation12typed_holderIN3RBX6Legacy17SurfaceConstraintEE14construct_funcEPKcPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::Legacy::SurfaceConstraint>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX6Legacy17SurfaceConstraintEE14construct_funcEPKcPc")]
pub fn stub_0x4cc174() -> ! {
    todo!("0x4cc174 rbx::implementation::typed_holder<RBX::Legacy::SurfaceConstraint>::construct_func(char const*,char *)")
}

// 0x4cc180 — __ZN3rbx14implementation12typed_holderIN3RBX6Legacy17SurfaceConstraintEE13destruct_funcEPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::Legacy::SurfaceConstraint>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX6Legacy17SurfaceConstraintEE13destruct_funcEPc")]
pub fn stub_0x4cc180() -> ! {
    todo!("0x4cc180 rbx::implementation::typed_holder<RBX::Legacy::SurfaceConstraint>::destruct_func(char *)")
}

// 0x4cc184 — __ZNK3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEE13convertToItemERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::convertToItem(RBX::Legacy::SurfaceConstraint const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEE13convertToItemERKS3_")]
pub fn stub_0x4cc184() -> ! {
    todo!("0x4cc184 RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::convertToItem(RBX::Legacy::SurfaceConstraint const&)const")
}

// 0x4cc250 — __ZN3rbx8any_castIRKN3RBX6Legacy17SurfaceConstraintENS1_7Region3EEET_RNS_13placement_anyIT0_EE
#[doc(alias = "RBX::Legacy::SurfaceConstraint const& rbx::any_cast<RBX::Legacy::SurfaceConstraint const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX6Legacy17SurfaceConstraintENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_0x4cc250() -> ! {
    todo!("0x4cc250 RBX::Legacy::SurfaceConstraint const& rbx::any_cast<RBX::Legacy::SurfaceConstraint const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")
}

// 0x4cc340 — __ZNK3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEE14convertToValueERKNS_4NameERS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::convertToValue(RBX::Name const&,RBX::Legacy::SurfaceConstraint&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_6Legacy17SurfaceConstraintEE14convertToValueERKNS_4NameERS3_")]
pub fn stub_0x4cc340() -> ! {
    todo!("0x4cc340 RBX::Reflection::EnumDesc<RBX::Legacy::SurfaceConstraint>::convertToValue(RBX::Name const&,RBX::Legacy::SurfaceConstraint&)const")
}

// 0x4cc3bc — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_6Legacy17SurfaceConstraintEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::Legacy::SurfaceConstraint>,std::_Select1st<std::pair<RBX::Name const* const,RBX::Legacy::SurfaceConstraint>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::Legacy::SurfaceConstraint>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::Legacy::SurfaceConstraint>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_6Legacy17SurfaceConstraintEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4cc3bc() -> ! {
    todo!("0x4cc3bc std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::Legacy::SurfaceConstraint>,std::_Select1st<std::pair<RBX::Name const* const,RBX::Legacy::SurfaceConstraint>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::Legacy::SurfaceConstraint>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::Legacy::SurfaceConstraint>> *)")
}

// 0x4cc3e4 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17KeywordFilterTypeEEEE13initSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::KeywordFilterType> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17KeywordFilterTypeEEEE13initSingletonEv")]
pub fn stub_0x4cc3e4() -> ! {
    todo!("0x4cc3e4 RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::KeywordFilterType> const>::initSingleton(void)")
}

// 0x4cc3e8 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17KeywordFilterTypeEEEE14doGetSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::KeywordFilterType> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_17KeywordFilterTypeEEEE14doGetSingletonEv")]
pub fn stub_0x4cc3e8() -> ! {
    todo!("0x4cc3e8 RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::KeywordFilterType> const>::doGetSingleton(void)")
}

// 0x4cc4d8 — __ZN3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEED1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEED1Ev")]
pub fn stub_0x4cc4d8() -> ! {
    todo!("0x4cc4d8 RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::~EnumDesc()")
}

// 0x4cc4dc — __ZN3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEED2Ev")]
pub fn stub_0x4cc4dc() -> ! {
    todo!("0x4cc4dc RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::~EnumDesc()")
}

// 0x4cc6b0 — __ZN3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEED0Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEED0Ev")]
pub fn stub_0x4cc6b0() -> ! {
    todo!("0x4cc6b0 RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::~EnumDesc()")
}

// 0x4cc750 — __ZNK3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE6lookupEPKc
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE6lookupEPKc")]
pub fn stub_0x4cc750() -> ! {
    todo!("0x4cc750 RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::lookup(char const*)const")
}

// 0x4cc780 — __ZNK3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE6lookupERKNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE6lookupERKNS0_7VariantE")]
pub fn stub_0x4cc780() -> ! {
    todo!("0x4cc780 RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::lookup(RBX::Reflection::Variant const&)const")
}

// 0x4cc7a0 — __ZNK3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE14convertToValueEmRNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_0x4cc7a0() -> ! {
    todo!("0x4cc7a0 RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")
}

// 0x4cc7fc — __ZNK3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE15convertToStringEmRSs")]
pub fn stub_0x4cc7fc() -> ! {
    todo!("0x4cc7fc RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::convertToString(unsigned long,std::string &)const")
}

// 0x4cc940 — __ZNK3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE15convertToStringERKS2_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::convertToString(RBX::KeywordFilterType const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE15convertToStringERKS2_")]
pub fn stub_0x4cc940() -> ! {
    todo!("0x4cc940 RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::convertToString(RBX::KeywordFilterType const&)const")
}

// 0x4ccae0 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_17KeywordFilterTypeEEERS3_RKT_
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::KeywordFilterType>(RBX::KeywordFilterType const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_17KeywordFilterTypeEEERS3_RKT_")]
pub fn stub_0x4ccae0() -> ! {
    todo!("0x4ccae0 rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::KeywordFilterType>(RBX::KeywordFilterType const&)")
}

// 0x4ccb30 — __ZN3rbx14implementation12typed_holderIN3RBX17KeywordFilterTypeEE9singletonEv
#[doc(alias = "rbx::implementation::typed_holder<RBX::KeywordFilterType>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX17KeywordFilterTypeEE9singletonEv")]
pub fn stub_0x4ccb30() -> ! {
    todo!("0x4ccb30 rbx::implementation::typed_holder<RBX::KeywordFilterType>::singleton(void)")
}

// 0x4ccb9c — __ZN3rbx14implementation12typed_holderIN3RBX17KeywordFilterTypeEE14construct_funcEPKcPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::KeywordFilterType>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX17KeywordFilterTypeEE14construct_funcEPKcPc")]
pub fn stub_0x4ccb9c() -> ! {
    todo!("0x4ccb9c rbx::implementation::typed_holder<RBX::KeywordFilterType>::construct_func(char const*,char *)")
}

// 0x4ccba8 — __ZN3rbx14implementation12typed_holderIN3RBX17KeywordFilterTypeEE13destruct_funcEPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::KeywordFilterType>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX17KeywordFilterTypeEE13destruct_funcEPc")]
pub fn stub_0x4ccba8() -> ! {
    todo!("0x4ccba8 rbx::implementation::typed_holder<RBX::KeywordFilterType>::destruct_func(char *)")
}

// 0x4ccbac — __ZNK3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE13convertToItemERKS2_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::convertToItem(RBX::KeywordFilterType const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE13convertToItemERKS2_")]
pub fn stub_0x4ccbac() -> ! {
    todo!("0x4ccbac RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::convertToItem(RBX::KeywordFilterType const&)const")
}

// 0x4ccc78 — __ZN3rbx8any_castIRKN3RBX17KeywordFilterTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE
#[doc(alias = "RBX::KeywordFilterType const& rbx::any_cast<RBX::KeywordFilterType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX17KeywordFilterTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_0x4ccc78() -> ! {
    todo!("0x4ccc78 RBX::KeywordFilterType const& rbx::any_cast<RBX::KeywordFilterType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")
}

// 0x4ccd68 — __ZNK3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE14convertToValueERKNS_4NameERS2_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::convertToValue(RBX::Name const&,RBX::KeywordFilterType&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE14convertToValueERKNS_4NameERS2_")]
pub fn stub_0x4ccd68() -> ! {
    todo!("0x4ccd68 RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::convertToValue(RBX::Name const&,RBX::KeywordFilterType&)const")
}

// 0x4ccde4 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_17KeywordFilterTypeEESt10_Select1stIS7_ESt4lessIS3_ESaIS7_EE8_M_eraseEPSt13_Rb_tree_nodeIS7_E
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::KeywordFilterType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::KeywordFilterType>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_17KeywordFilterTypeEESt10_Select1stIS7_ESt4lessIS3_ESaIS7_EE8_M_eraseEPSt13_Rb_tree_nodeIS7_E")]
pub fn stub_0x4ccde4() -> ! {
    todo!("0x4ccde4 std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::KeywordFilterType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::KeywordFilterType>> *)")
}

// 0x4cce0c — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_5Joint9JointTypeEEEE13initSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Joint::JointType> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_5Joint9JointTypeEEEE13initSingletonEv")]
pub fn stub_0x4cce0c() -> ! {
    todo!("0x4cce0c RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Joint::JointType> const>::initSingleton(void)")
}

// 0x4cce10 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_5Joint9JointTypeEEEE14doGetSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Joint::JointType> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_5Joint9JointTypeEEEE14doGetSingletonEv")]
pub fn stub_0x4cce10() -> ! {
    todo!("0x4cce10 RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Joint::JointType> const>::doGetSingleton(void)")
}

// 0x4ccf00 — __ZN3RBX10Reflection8EnumDescINS_5Joint9JointTypeEED1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Joint::JointType>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_5Joint9JointTypeEED1Ev")]
pub fn stub_0x4ccf00() -> ! {
    todo!("0x4ccf00 RBX::Reflection::EnumDesc<RBX::Joint::JointType>::~EnumDesc()")
}

// 0x4ccf04 — __ZN3RBX10Reflection8EnumDescINS_5Joint9JointTypeEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Joint::JointType>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_5Joint9JointTypeEED2Ev")]
pub fn stub_0x4ccf04() -> ! {
    todo!("0x4ccf04 RBX::Reflection::EnumDesc<RBX::Joint::JointType>::~EnumDesc()")
}

// 0x4cd0d8 — __ZN3RBX10Reflection8EnumDescINS_5Joint9JointTypeEED0Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Joint::JointType>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_5Joint9JointTypeEED0Ev")]
pub fn stub_0x4cd0d8() -> ! {
    todo!("0x4cd0d8 RBX::Reflection::EnumDesc<RBX::Joint::JointType>::~EnumDesc()")
}

// 0x4cd178 — __ZNK3RBX10Reflection8EnumDescINS_5Joint9JointTypeEE6lookupEPKc
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Joint::JointType>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_5Joint9JointTypeEE6lookupEPKc")]
pub fn stub_0x4cd178() -> ! {
    todo!("0x4cd178 RBX::Reflection::EnumDesc<RBX::Joint::JointType>::lookup(char const*)const")
}

// 0x4cd1a8 — __ZNK3RBX10Reflection8EnumDescINS_5Joint9JointTypeEE6lookupERKNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Joint::JointType>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_5Joint9JointTypeEE6lookupERKNS0_7VariantE")]
pub fn stub_0x4cd1a8() -> ! {
    todo!("0x4cd1a8 RBX::Reflection::EnumDesc<RBX::Joint::JointType>::lookup(RBX::Reflection::Variant const&)const")
}

// 0x4cd1c8 — __ZNK3RBX10Reflection8EnumDescINS_5Joint9JointTypeEE14convertToValueEmRNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Joint::JointType>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_5Joint9JointTypeEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_0x4cd1c8() -> ! {
    todo!("0x4cd1c8 RBX::Reflection::EnumDesc<RBX::Joint::JointType>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")
}

// 0x4cd224 — __ZNK3RBX10Reflection8EnumDescINS_5Joint9JointTypeEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Joint::JointType>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_5Joint9JointTypeEE15convertToStringEmRSs")]
pub fn stub_0x4cd224() -> ! {
    todo!("0x4cd224 RBX::Reflection::EnumDesc<RBX::Joint::JointType>::convertToString(unsigned long,std::string &)const")
}

// 0x4cd368 — __ZNK3RBX10Reflection8EnumDescINS_5Joint9JointTypeEE15convertToStringERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Joint::JointType>::convertToString(RBX::Joint::JointType const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_5Joint9JointTypeEE15convertToStringERKS3_")]
pub fn stub_0x4cd368() -> ! {
    todo!("0x4cd368 RBX::Reflection::EnumDesc<RBX::Joint::JointType>::convertToString(RBX::Joint::JointType const&)const")
}

// 0x4cd508 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_5Joint9JointTypeEEERS3_RKT_
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Joint::JointType>(RBX::Joint::JointType const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_5Joint9JointTypeEEERS3_RKT_")]
pub fn stub_0x4cd508() -> ! {
    todo!("0x4cd508 rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Joint::JointType>(RBX::Joint::JointType const&)")
}

// 0x4cd558 — __ZN3rbx14implementation12typed_holderIN3RBX5Joint9JointTypeEE9singletonEv
#[doc(alias = "rbx::implementation::typed_holder<RBX::Joint::JointType>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX5Joint9JointTypeEE9singletonEv")]
pub fn stub_0x4cd558() -> ! {
    todo!("0x4cd558 rbx::implementation::typed_holder<RBX::Joint::JointType>::singleton(void)")
}

// 0x4cd5c4 — __ZN3rbx14implementation12typed_holderIN3RBX5Joint9JointTypeEE14construct_funcEPKcPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::Joint::JointType>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX5Joint9JointTypeEE14construct_funcEPKcPc")]
pub fn stub_0x4cd5c4() -> ! {
    todo!("0x4cd5c4 rbx::implementation::typed_holder<RBX::Joint::JointType>::construct_func(char const*,char *)")
}

// 0x4cd5d0 — __ZN3rbx14implementation12typed_holderIN3RBX5Joint9JointTypeEE13destruct_funcEPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::Joint::JointType>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX5Joint9JointTypeEE13destruct_funcEPc")]
pub fn stub_0x4cd5d0() -> ! {
    todo!("0x4cd5d0 rbx::implementation::typed_holder<RBX::Joint::JointType>::destruct_func(char *)")
}

// 0x4cd5d4 — __ZNK3RBX10Reflection8EnumDescINS_5Joint9JointTypeEE13convertToItemERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Joint::JointType>::convertToItem(RBX::Joint::JointType const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_5Joint9JointTypeEE13convertToItemERKS3_")]
pub fn stub_0x4cd5d4() -> ! {
    todo!("0x4cd5d4 RBX::Reflection::EnumDesc<RBX::Joint::JointType>::convertToItem(RBX::Joint::JointType const&)const")
}

// 0x4cd6a0 — __ZN3rbx8any_castIRKN3RBX5Joint9JointTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE
#[doc(alias = "RBX::Joint::JointType const& rbx::any_cast<RBX::Joint::JointType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX5Joint9JointTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_0x4cd6a0() -> ! {
    todo!("0x4cd6a0 RBX::Joint::JointType const& rbx::any_cast<RBX::Joint::JointType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")
}

// 0x4cd790 — __ZNK3RBX10Reflection8EnumDescINS_5Joint9JointTypeEE14convertToValueERKNS_4NameERS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Joint::JointType>::convertToValue(RBX::Name const&,RBX::Joint::JointType&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_5Joint9JointTypeEE14convertToValueERKNS_4NameERS3_")]
pub fn stub_0x4cd790() -> ! {
    todo!("0x4cd790 RBX::Reflection::EnumDesc<RBX::Joint::JointType>::convertToValue(RBX::Name const&,RBX::Joint::JointType&)const")
}

// 0x4cd80c — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_5Joint9JointTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::Joint::JointType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::Joint::JointType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::Joint::JointType>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::Joint::JointType>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_5Joint9JointTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4cd80c() -> ! {
    todo!("0x4cd80c std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::Joint::JointType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::Joint::JointType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::Joint::JointType>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::Joint::JointType>> *)")
}

// 0x4cd834 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_7Feature9TopBottomEEEE13initSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Feature::TopBottom> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_7Feature9TopBottomEEEE13initSingletonEv")]
pub fn stub_0x4cd834() -> ! {
    todo!("0x4cd834 RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Feature::TopBottom> const>::initSingleton(void)")
}

// 0x4cd838 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_7Feature9TopBottomEEEE14doGetSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Feature::TopBottom> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_7Feature9TopBottomEEEE14doGetSingletonEv")]
pub fn stub_0x4cd838() -> ! {
    todo!("0x4cd838 RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Feature::TopBottom> const>::doGetSingleton(void)")
}

// 0x4cd928 — __ZN3RBX10Reflection8EnumDescINS_7Feature9TopBottomEED1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_7Feature9TopBottomEED1Ev")]
pub fn stub_0x4cd928() -> ! {
    todo!("0x4cd928 RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::~EnumDesc()")
}

// 0x4cd92c — __ZN3RBX10Reflection8EnumDescINS_7Feature9TopBottomEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_7Feature9TopBottomEED2Ev")]
pub fn stub_0x4cd92c() -> ! {
    todo!("0x4cd92c RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::~EnumDesc()")
}

// 0x4cdb00 — __ZN3RBX10Reflection8EnumDescINS_7Feature9TopBottomEED0Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_7Feature9TopBottomEED0Ev")]
pub fn stub_0x4cdb00() -> ! {
    todo!("0x4cdb00 RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::~EnumDesc()")
}

// 0x4cdba0 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9TopBottomEE6lookupEPKc
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9TopBottomEE6lookupEPKc")]
pub fn stub_0x4cdba0() -> ! {
    todo!("0x4cdba0 RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::lookup(char const*)const")
}

// 0x4cdbd0 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9TopBottomEE6lookupERKNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9TopBottomEE6lookupERKNS0_7VariantE")]
pub fn stub_0x4cdbd0() -> ! {
    todo!("0x4cdbd0 RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::lookup(RBX::Reflection::Variant const&)const")
}

// 0x4cdbf0 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9TopBottomEE14convertToValueEmRNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9TopBottomEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_0x4cdbf0() -> ! {
    todo!("0x4cdbf0 RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")
}

// 0x4cdc4c — __ZNK3RBX10Reflection8EnumDescINS_7Feature9TopBottomEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9TopBottomEE15convertToStringEmRSs")]
pub fn stub_0x4cdc4c() -> ! {
    todo!("0x4cdc4c RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::convertToString(unsigned long,std::string &)const")
}

// 0x4cdd90 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9TopBottomEE15convertToStringERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::convertToString(RBX::Feature::TopBottom const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9TopBottomEE15convertToStringERKS3_")]
pub fn stub_0x4cdd90() -> ! {
    todo!("0x4cdd90 RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::convertToString(RBX::Feature::TopBottom const&)const")
}

// 0x4cdf30 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_7Feature9TopBottomEEERS3_RKT_
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Feature::TopBottom>(RBX::Feature::TopBottom const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_7Feature9TopBottomEEERS3_RKT_")]
pub fn stub_0x4cdf30() -> ! {
    todo!("0x4cdf30 rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Feature::TopBottom>(RBX::Feature::TopBottom const&)")
}

// 0x4cdf80 — __ZN3rbx14implementation12typed_holderIN3RBX7Feature9TopBottomEE9singletonEv
#[doc(alias = "rbx::implementation::typed_holder<RBX::Feature::TopBottom>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX7Feature9TopBottomEE9singletonEv")]
pub fn stub_0x4cdf80() -> ! {
    todo!("0x4cdf80 rbx::implementation::typed_holder<RBX::Feature::TopBottom>::singleton(void)")
}

// 0x4cdfec — __ZN3rbx14implementation12typed_holderIN3RBX7Feature9TopBottomEE14construct_funcEPKcPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::Feature::TopBottom>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX7Feature9TopBottomEE14construct_funcEPKcPc")]
pub fn stub_0x4cdfec() -> ! {
    todo!("0x4cdfec rbx::implementation::typed_holder<RBX::Feature::TopBottom>::construct_func(char const*,char *)")
}

// 0x4cdff8 — __ZN3rbx14implementation12typed_holderIN3RBX7Feature9TopBottomEE13destruct_funcEPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::Feature::TopBottom>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX7Feature9TopBottomEE13destruct_funcEPc")]
pub fn stub_0x4cdff8() -> ! {
    todo!("0x4cdff8 rbx::implementation::typed_holder<RBX::Feature::TopBottom>::destruct_func(char *)")
}

// 0x4cdffc — __ZNK3RBX10Reflection8EnumDescINS_7Feature9TopBottomEE13convertToItemERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::convertToItem(RBX::Feature::TopBottom const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9TopBottomEE13convertToItemERKS3_")]
pub fn stub_0x4cdffc() -> ! {
    todo!("0x4cdffc RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::convertToItem(RBX::Feature::TopBottom const&)const")
}

// 0x4ce0c8 — __ZN3rbx8any_castIRKN3RBX7Feature9TopBottomENS1_7Region3EEET_RNS_13placement_anyIT0_EE
#[doc(alias = "RBX::Feature::TopBottom const& rbx::any_cast<RBX::Feature::TopBottom const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX7Feature9TopBottomENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_0x4ce0c8() -> ! {
    todo!("0x4ce0c8 RBX::Feature::TopBottom const& rbx::any_cast<RBX::Feature::TopBottom const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")
}

// 0x4ce1b8 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9TopBottomEE14convertToValueERKNS_4NameERS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::convertToValue(RBX::Name const&,RBX::Feature::TopBottom&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9TopBottomEE14convertToValueERKNS_4NameERS3_")]
pub fn stub_0x4ce1b8() -> ! {
    todo!("0x4ce1b8 RBX::Reflection::EnumDesc<RBX::Feature::TopBottom>::convertToValue(RBX::Name const&,RBX::Feature::TopBottom&)const")
}

// 0x4ce234 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_7Feature9TopBottomEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::Feature::TopBottom>,std::_Select1st<std::pair<RBX::Name const* const,RBX::Feature::TopBottom>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::Feature::TopBottom>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::Feature::TopBottom>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_7Feature9TopBottomEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_0x4ce234() -> ! {
    todo!("0x4ce234 std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::Feature::TopBottom>,std::_Select1st<std::pair<RBX::Name const* const,RBX::Feature::TopBottom>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::Feature::TopBottom>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::Feature::TopBottom>> *)")
}

// 0x4ce25c — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_7Feature9LeftRightEEEE13initSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Feature::LeftRight> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_7Feature9LeftRightEEEE13initSingletonEv")]
pub fn stub_0x4ce25c() -> ! {
    todo!("0x4ce25c RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Feature::LeftRight> const>::initSingleton(void)")
}

// 0x4ce260 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_7Feature9LeftRightEEEE14doGetSingletonEv
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Feature::LeftRight> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_7Feature9LeftRightEEEE14doGetSingletonEv")]
pub fn stub_0x4ce260() -> ! {
    todo!("0x4ce260 RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Feature::LeftRight> const>::doGetSingleton(void)")
}

// 0x4ce350 — __ZN3RBX10Reflection8EnumDescINS_7Feature9LeftRightEED1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_7Feature9LeftRightEED1Ev")]
pub fn stub_0x4ce350() -> ! {
    todo!("0x4ce350 RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::~EnumDesc()")
}

// 0x4ce354 — __ZN3RBX10Reflection8EnumDescINS_7Feature9LeftRightEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_7Feature9LeftRightEED2Ev")]
pub fn stub_0x4ce354() -> ! {
    todo!("0x4ce354 RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::~EnumDesc()")
}

// 0x4ce528 — __ZN3RBX10Reflection8EnumDescINS_7Feature9LeftRightEED0Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_7Feature9LeftRightEED0Ev")]
pub fn stub_0x4ce528() -> ! {
    todo!("0x4ce528 RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::~EnumDesc()")
}

// 0x4ce5c8 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE6lookupEPKc
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE6lookupEPKc")]
pub fn stub_0x4ce5c8() -> ! {
    todo!("0x4ce5c8 RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::lookup(char const*)const")
}

// 0x4ce5f8 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE6lookupERKNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE6lookupERKNS0_7VariantE")]
pub fn stub_0x4ce5f8() -> ! {
    todo!("0x4ce5f8 RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::lookup(RBX::Reflection::Variant const&)const")
}

// 0x4ce618 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE14convertToValueEmRNS0_7VariantE
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_0x4ce618() -> ! {
    todo!("0x4ce618 RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")
}

// 0x4ce674 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE15convertToStringEmRSs")]
pub fn stub_0x4ce674() -> ! {
    todo!("0x4ce674 RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToString(unsigned long,std::string &)const")
}

// 0x4ce7b8 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE15convertToStringERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToString(RBX::Feature::LeftRight const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE15convertToStringERKS3_")]
pub fn stub_0x4ce7b8() -> ! {
    todo!("0x4ce7b8 RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToString(RBX::Feature::LeftRight const&)const")
}

// 0x4ce958 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_7Feature9LeftRightEEERS3_RKT_
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Feature::LeftRight>(RBX::Feature::LeftRight const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_7Feature9LeftRightEEERS3_RKT_")]
pub fn stub_0x4ce958() -> ! {
    todo!("0x4ce958 rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Feature::LeftRight>(RBX::Feature::LeftRight const&)")
}

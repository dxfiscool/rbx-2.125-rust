// Auto-generated skeletons for rbx-datamodel — from ida/export.json
// Filter: EA-sorted asc next 100 uncovered not yet in crates/datamodel/src (distinct EA, higher range gaps)
// Source: ida/export.json (85545 funcs, base 0x4000)
// Batch: 100 stubs | range 0xb79f0c..0xb99f90 | EA-sorted asc gap filler distinct from watchdog22 (higher range 0xb79f0c+)
// Shard: generated_dm_wd_watchdog23 EA-sorted asc next 100 uncovered not yet in crates/datamodel/src (higher range gap filler >0xb79f0c)
// SharedPtr = rbx_core::SharedPtr (Arc), not boost::shared_ptr; ` and single-quote stripped from alias where needed

#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]

use rbx_core::SharedPtr;
const _SHARED_PTR: Option<SharedPtr<u8>> = None;

// 0xb79f0c — __ZN3RBXL15getSurfaceFrameERKN3G3D7Vector3Ej
// type: void __fastcall(RBX *this, const G3D::Vector3 *, unsigned int)
#[doc(alias = "RBX::getSurfaceFrame(G3D::Vector3 const&,unsigned int)")]
#[doc(alias = "__ZN3RBXL15getSurfaceFrameERKN3G3D7Vector3Ej")]
pub fn stub_0xb79f0c() -> ! {
    todo!("0xb79f0c RBX::getSurfaceFrame(G3D::Vector3 const&,unsigned int)")
}

// 0xb7e648 — __ZN3RBX17GeometryGeneratorC1Ev
// type: RBX::GeometryGenerator *__fastcall(RBX::GeometryGenerator *this)
#[doc(alias = "RBX::GeometryGenerator::GeometryGenerator(void)")]
#[doc(alias = "__ZN3RBX17GeometryGeneratorC1Ev")]
pub fn stub_0xb7e648() -> ! {
    todo!("0xb7e648 RBX::GeometryGenerator::GeometryGenerator(void)")
}

// 0xb7e680 — __ZN3RBX17GeometryGeneratorC1EPNS0_6VertexEPtj
// type: int __fastcall(int, int, int, int)
#[doc(alias = "RBX::GeometryGenerator::GeometryGenerator(RBX::GeometryGenerator::Vertex *,unsigned short *,unsigned int)")]
#[doc(alias = "__ZN3RBX17GeometryGeneratorC1EPNS0_6VertexEPtj")]
pub fn stub_0xb7e680() -> ! {
    todo!("0xb7e680 RBX::GeometryGenerator::GeometryGenerator(RBX::GeometryGenerator::Vertex *,unsigned short *,unsigned int)")
}

// 0xb7e720 — __ZN3RBX17GeometryGenerator11resetBoundsEv
// type: int __fastcall(RBX::GeometryGenerator *this)
#[doc(alias = "RBX::GeometryGenerator::resetBounds(void)")]
#[doc(alias = "__ZN3RBX17GeometryGenerator11resetBoundsEv")]
pub fn stub_0xb7e720() -> ! {
    todo!("0xb7e720 RBX::GeometryGenerator::resetBounds(void)")
}

// 0xb7e7c8 — __ZN5boost10shared_ptrIN3RBX12FileMeshDataEED1Ev
// type: int __fastcall(int)
#[doc(alias = "boost::shared_ptr<RBX::FileMeshData>::~shared_ptr()")]
#[doc(alias = "__ZN5boost10shared_ptrIN3RBX12FileMeshDataEED1Ev")]
pub fn stub_0xb7e7c8() -> ! {
    todo!("0xb7e7c8 boost::shared_ptr<RBX::FileMeshData>::~shared_ptr()")
}

// 0xb7e7dc — __ZN3RBX16TrussQuadBuilder4emitERKN3G3D7Vector3ES4_fS4_f
// type: int __fastcall(RBX::TrussQuadBuilder *this, const Vector3 *, const Vector3 *, float32_t, const Vector3 *, float32_t)
#[doc(alias = "RBX::TrussQuadBuilder::emit(G3D::Vector3 const&,G3D::Vector3 const&,float,G3D::Vector3 const&,float)")]
#[doc(alias = "__ZN3RBX16TrussQuadBuilder4emitERKN3G3D7Vector3ES4_fS4_f")]
pub fn stub_0xb7e7dc() -> ! {
    todo!("0xb7e7dc RBX::TrussQuadBuilder::emit(G3D::Vector3 const&,G3D::Vector3 const&,float,G3D::Vector3 const&,float)")
}

// 0xb8562c — __ZN3RBX18getDecalUVVerticalILb1EEEN3G3D7Vector2EPNS_5DecalERKNS1_7Vector3Eb
// type: void __fastcall(float *, int, int, int)
#[doc(alias = "G3D::Vector2 RBX::getDecalUVVertical<true>(RBX::Decal *,G3D::Vector3 const&,bool)")]
#[doc(alias = "__ZN3RBX18getDecalUVVerticalILb1EEEN3G3D7Vector2EPNS_5DecalERKNS1_7Vector3Eb")]
pub fn stub_0xb8562c() -> ! {
    todo!("0xb8562c G3D::Vector2 RBX::getDecalUVVertical<true>(RBX::Decal *,G3D::Vector3 const&,bool)")
}

// 0xb8583c — __ZN3RBX18getDecalUVVerticalILb0EEEN3G3D7Vector2EPNS_5DecalERKNS1_7Vector3Eb
// type: void __fastcall(float *, int, int, int)
#[doc(alias = "G3D::Vector2 RBX::getDecalUVVertical<false>(RBX::Decal *,G3D::Vector3 const&,bool)")]
#[doc(alias = "__ZN3RBX18getDecalUVVerticalILb0EEEN3G3D7Vector2EPNS_5DecalERKNS1_7Vector3Eb")]
pub fn stub_0xb8583c() -> ! {
    todo!("0xb8583c G3D::Vector2 RBX::getDecalUVVertical<false>(RBX::Decal *,G3D::Vector3 const&,bool)")
}

// 0xb85a4c — __ZNSt6vectorIN3RBX9ContentIdESaIS1_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS1_S3_EERKS1_
// type: void __fastcall(struct _Unwind_Exception **, std::string *, const std::string *)
#[doc(alias = "std::vector<RBX::ContentId,std::allocator<RBX::ContentId>>::_M_insert_aux(__gnu_cxx::__normal_iterator<RBX::ContentId*,std::vector<RBX::ContentId,std::allocator<RBX::ContentId>>>,RBX::ContentId const&)")]
#[doc(alias = "__ZNSt6vectorIN3RBX9ContentIdESaIS1_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS1_S3_EERKS1_")]
pub fn stub_0xb85a4c() -> ! {
    todo!("0xb85a4c std::vector<RBX::ContentId,std::allocator<RBX::ContentId>>::_M_insert_aux(__gnu_cxx::__normal_iterator<RBX::ContentId*,std::vector<RBX::ContentId,std::allocator<RBX::ContentId>>>,RBX::ContentId const&)")
}

// 0xb86124 — __GLOBAL__I_a_569
// type: void()
#[doc(alias = "global constructor keyed to_a_569")]
#[doc(alias = "__GLOBAL__I_a_569")]
pub fn stub_0xb86124() -> ! {
    todo!("0xb86124 global constructor keyed to_a_569")
}

// 0xb86c08 — __ZN3RBX17MaterialGeneratorC1EPN4Ogre12VisualEngineE
// type: _DWORD *__fastcall(_DWORD *result, int)
#[doc(alias = "RBX::MaterialGenerator::MaterialGenerator(Ogre::VisualEngine *)")]
#[doc(alias = "__ZN3RBX17MaterialGeneratorC1EPN4Ogre12VisualEngineE")]
pub fn stub_0xb86c08() -> ! {
    todo!("0xb86c08 RBX::MaterialGenerator::MaterialGenerator(Ogre::VisualEngine *)")
}

// 0xb86c9c — __ZN3RBX17MaterialGenerator18createBaseMaterialEj
// type: void __fastcall(RBX::MaterialGenerator *this, unsigned int, unsigned int)
#[doc(alias = "RBX::MaterialGenerator::createBaseMaterial(unsigned int)")]
#[doc(alias = "__ZN3RBX17MaterialGenerator18createBaseMaterialEj")]
pub fn stub_0xb86c9c() -> ! {
    todo!("0xb86c9c RBX::MaterialGenerator::createBaseMaterial(unsigned int)")
}

// 0xb87024 — __ZN12_GLOBAL__N_115getMaterialNameEj
// type: void __fastcall(_anonymous_namespace_ *this, char)
#[doc(alias = "anonymous namespace::getMaterialName(unsigned int)")]
#[doc(alias = "__ZN12_GLOBAL__N_115getMaterialNameEj")]
pub fn stub_0xb87024() -> ! {
    todo!("0xb87024 anonymous namespace::getMaterialName(unsigned int)")
}

// 0xb871c0 — __ZN3RBX17MaterialGenerator20createRenderMaterialEji
// type: void __fastcall(RBX::MaterialGenerator *this, unsigned int, unsigned int, int)
#[doc(alias = "RBX::MaterialGenerator::createRenderMaterial(unsigned int,int)")]
#[doc(alias = "__ZN3RBX17MaterialGenerator20createRenderMaterialEji")]
pub fn stub_0xb871c0() -> ! {
    todo!("0xb871c0 RBX::MaterialGenerator::createRenderMaterial(unsigned int,int)")
}

// 0xb8754c — __ZN12_GLOBAL__N_115getMaterialNameEjib
// type: void __fastcall(_anonymous_namespace_ *this, char, unsigned int, int)
#[doc(alias = "anonymous namespace::getMaterialName(unsigned int,int,bool)")]
#[doc(alias = "__ZN12_GLOBAL__N_115getMaterialNameEjib")]
pub fn stub_0xb8754c() -> ! {
    todo!("0xb8754c anonymous namespace::getMaterialName(unsigned int,int,bool)")
}

// 0xb87738 — __ZN3RBX17MaterialGenerator22createTexturedMaterialERKN4Ogre10TexturePtrERKSsj
// type: void __fastcall(RBX::MaterialGenerator *this, const Ogre::TexturePtr *, const std::string *, const std::string *, unsigned int)
#[doc(alias = "RBX::MaterialGenerator::createTexturedMaterial(Ogre::TexturePtr const&,std::string const&,unsigned int)")]
#[doc(alias = "__ZN3RBX17MaterialGenerator22createTexturedMaterialERKN4Ogre10TexturePtrERKSsj")]
pub fn stub_0xb87738() -> ! {
    todo!("0xb87738 RBX::MaterialGenerator::createTexturedMaterial(Ogre::TexturePtr const&,std::string const&,unsigned int)")
}

// 0xb8aa38 — __ZN3RBX17MaterialGenerator22createMaterialForDecalEPNS_5DecalEjPNS_11AsyncResultE
// type: Ogre::NedPoolingImpl *__fastcall(_DWORD *, const Ogre::TexturePtr *, int, unsigned int, _DWORD *)
#[doc(alias = "RBX::MaterialGenerator::createMaterialForDecal(RBX::Decal *,unsigned int,RBX::AsyncResult *)")]
#[doc(alias = "__ZN3RBX17MaterialGenerator22createMaterialForDecalEPNS_5DecalEjPNS_11AsyncResultE")]
pub fn stub_0xb8aa38() -> ! {
    todo!("0xb8aa38 RBX::MaterialGenerator::createMaterialForDecal(RBX::Decal *,unsigned int,RBX::AsyncResult *)")
}

// 0xb8ab5c — __ZN12_GLOBAL__N_129createHumanoidTextureCompositEPN4Ogre12VisualEngineERKN3RBX18HumanoidIdentifierERA4_KNS_16AccoutrementMeshEPNS3_13CharacterMeshE
// type: void __fastcall(int, int, int, int *, int)
#[doc(alias = "anonymous namespace::createHumanoidTextureComposit(Ogre::VisualEngine *,RBX::HumanoidIdentifier const&,anonymous namespace::AccoutrementMesh const(&)[4],RBX::CharacterMesh *)")]
#[doc(alias = "__ZN12_GLOBAL__N_129createHumanoidTextureCompositEPN4Ogre12VisualEngineERKN3RBX18HumanoidIdentifierERA4_KNS_16AccoutrementMeshEPNS3_13CharacterMeshE")]
pub fn stub_0xb8ab5c() -> ! {
    todo!("0xb8ab5c anonymous namespace::createHumanoidTextureComposit(Ogre::VisualEngine *,RBX::HumanoidIdentifier const&,anonymous namespace::AccoutrementMesh const(&)[4],RBX::CharacterMesh *)")
}

// 0xb8bed8 — __ZN12_GLOBAL__N_129TextureCompositingDescription3addERKN3RBX6MeshIdERKNS1_10BrickColorE
// type: int __fastcall(std::string *, int, struct _Unwind_Exception *lpuexcpt)
#[doc(alias = "anonymous namespace::TextureCompositingDescription::add(RBX::MeshId const&,RBX::BrickColor const&)")]
#[doc(alias = "__ZN12_GLOBAL__N_129TextureCompositingDescription3addERKN3RBX6MeshIdERKNS1_10BrickColorE")]
pub fn stub_0xb8bed8() -> ! {
    todo!("0xb8bed8 anonymous namespace::TextureCompositingDescription::add(RBX::MeshId const&,RBX::BrickColor const&)")
}

// 0xb8c314 — __ZN12_GLOBAL__N_129TextureCompositingDescription3addERKN3RBX6MeshIdERKNS1_9ContentIdENS1_22TextureCompositorLayer12CompositModeE
// type: int __fastcall(std::string *, const std::string *, const std::string *, struct _Unwind_Exception *lpuexcpt)
#[doc(alias = "anonymous namespace::TextureCompositingDescription::add(RBX::MeshId const&,RBX::ContentId const&,RBX::TextureCompositorLayer::CompositMode)")]
#[doc(alias = "__ZN12_GLOBAL__N_129TextureCompositingDescription3addERKN3RBX6MeshIdERKNS1_9ContentIdENS1_22TextureCompositorLayer12CompositModeE")]
pub fn stub_0xb8c314() -> ! {
    todo!("0xb8c314 anonymous namespace::TextureCompositingDescription::add(RBX::MeshId const&,RBX::ContentId const&,RBX::TextureCompositorLayer::CompositMode)")
}

// 0xb8c644 — __ZSt16__introsort_loopIPN12_GLOBAL__N_116AccoutrementMeshEiNS0_28AccoutrementMeshIdComparatorEEvT_S4_T0_T1_
// type: int __fastcall(int result, __int64 *, int)
#[doc(alias = "void std::__introsort_loop<anonymous namespace::AccoutrementMesh *,int,anonymous namespace::AccoutrementMeshIdComparator>(anonymous namespace::AccoutrementMesh *,anonymous namespace::AccoutrementMesh *,int,anonymous namespace::AccoutrementMeshIdComparator)")]
#[doc(alias = "__ZSt16__introsort_loopIPN12_GLOBAL__N_116AccoutrementMeshEiNS0_28AccoutrementMeshIdComparatorEEvT_S4_T0_T1_")]
pub fn stub_0xb8c644() -> ! {
    todo!("0xb8c644 void std::__introsort_loop<anonymous namespace::AccoutrementMesh *,int,anonymous namespace::AccoutrementMeshIdComparator>(anonymous namespace::AccoutrementMesh *,anonymous namespace::AccoutrementMesh *,int,anonymous namespace::AccoutrementMeshIdComparator)")
}

// 0xb8c808 — __ZSt13__adjust_heapIPN12_GLOBAL__N_116AccoutrementMeshEiS1_NS0_28AccoutrementMeshIdComparatorEEvT_T0_S5_T1_T2_
// type: int __fastcall(int, int, int, int, int, int)
#[doc(alias = "void std::__adjust_heap<anonymous namespace::AccoutrementMesh *,int,anonymous namespace::AccoutrementMesh,anonymous namespace::AccoutrementMeshIdComparator>(anonymous namespace::AccoutrementMesh *,int,int,anonymous namespace::AccoutrementMesh,anonymous namespace::AccoutrementMeshIdComparator)")]
#[doc(alias = "__ZSt13__adjust_heapIPN12_GLOBAL__N_116AccoutrementMeshEiS1_NS0_28AccoutrementMeshIdComparatorEEvT_T0_S5_T1_T2_")]
pub fn stub_0xb8c808() -> ! {
    todo!("0xb8c808 void std::__adjust_heap<anonymous namespace::AccoutrementMesh *,int,anonymous namespace::AccoutrementMesh,anonymous namespace::AccoutrementMeshIdComparator>(anonymous namespace::AccoutrementMesh *,int,int,anonymous namespace::AccoutrementMesh,anonymous namespace::AccoutrementMeshIdComparator)")
}

// 0xb8caa0 — __ZNSt4pairIN4Ogre10TexturePtrEN5boost10shared_ptrIN3RBX17TextureCompositor3JobEEEED1Ev
// type: int __fastcall(int)
#[doc(alias = "std::pair<Ogre::TexturePtr,boost::shared_ptr<RBX::TextureCompositor::Job>>::~pair()")]
#[doc(alias = "__ZNSt4pairIN4Ogre10TexturePtrEN5boost10shared_ptrIN3RBX17TextureCompositor3JobEEEED1Ev")]
pub fn stub_0xb8caa0() -> ! {
    todo!("0xb8caa0 std::pair<Ogre::TexturePtr,boost::shared_ptr<RBX::TextureCompositor::Job>>::~pair()")
}

// 0xb8cc38 — __ZNSt6vectorIN3RBX22TextureCompositorLayerESaIS1_EE9push_backERKS1_
// type: void __fastcall(int, _DWORD *, int, int, char, int, int, int, struct _Unwind_Exception *lpuexcpt)
#[doc(alias = "std::vector<RBX::TextureCompositorLayer,std::allocator<RBX::TextureCompositorLayer>>::push_back(RBX::TextureCompositorLayer const&)")]
#[doc(alias = "__ZNSt6vectorIN3RBX22TextureCompositorLayerESaIS1_EE9push_backERKS1_")]
pub fn stub_0xb8cc38() -> ! {
    todo!("0xb8cc38 std::vector<RBX::TextureCompositorLayer,std::allocator<RBX::TextureCompositorLayer>>::push_back(RBX::TextureCompositorLayer const&)")
}

// 0xb8cd88 — __ZNSt6vectorIN3RBX22TextureCompositorLayerESaIS1_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS1_S3_EERKS1_
// type: void __fastcall(int, char *, _DWORD *)
#[doc(alias = "std::vector<RBX::TextureCompositorLayer,std::allocator<RBX::TextureCompositorLayer>>::_M_insert_aux(__gnu_cxx::__normal_iterator<RBX::TextureCompositorLayer*,std::vector<RBX::TextureCompositorLayer,std::allocator<RBX::TextureCompositorLayer>>>,RBX::TextureCompositorLayer const&)")]
#[doc(alias = "__ZNSt6vectorIN3RBX22TextureCompositorLayerESaIS1_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS1_S3_EERKS1_")]
pub fn stub_0xb8cd88() -> ! {
    todo!("0xb8cd88 std::vector<RBX::TextureCompositorLayer,std::allocator<RBX::TextureCompositorLayer>>::_M_insert_aux(__gnu_cxx::__normal_iterator<RBX::TextureCompositorLayer*,std::vector<RBX::TextureCompositorLayer,std::allocator<RBX::TextureCompositorLayer>>>,RBX::TextureCompositorLayer const&)")
}

// 0xb8d3ac — __ZN3RBX22TextureCompositorLayerC2ERKNS_6MeshIdERKN3G3D6Color3E
// type: std::string *__fastcall(std::string *, const std::string *, _DWORD *, int, char, int, int, int, struct _Unwind_Exception *lpuexcpt)
#[doc(alias = "RBX::TextureCompositorLayer::TextureCompositorLayer(RBX::MeshId const&,G3D::Color3 const&)")]
#[doc(alias = "__ZN3RBX22TextureCompositorLayerC2ERKNS_6MeshIdERKN3G3D6Color3E")]
pub fn stub_0xb8d3ac() -> ! {
    todo!("0xb8d3ac RBX::TextureCompositorLayer::TextureCompositorLayer(RBX::MeshId const&,G3D::Color3 const&)")
}

// 0xb8d500 — __ZNSt6vectorIN3RBX22TextureCompositorLayerESaIS1_EE7reserveEm
// type: unsigned int __fastcall(int *, unsigned int)
#[doc(alias = "std::vector<RBX::TextureCompositorLayer,std::allocator<RBX::TextureCompositorLayer>>::reserve(unsigned long)")]
#[doc(alias = "__ZNSt6vectorIN3RBX22TextureCompositorLayerESaIS1_EE7reserveEm")]
pub fn stub_0xb8d500() -> ! {
    todo!("0xb8d500 std::vector<RBX::TextureCompositorLayer,std::allocator<RBX::TextureCompositorLayer>>::reserve(unsigned long)")
}

// 0xb8d608 — __ZNSt6vectorIN3RBX22TextureCompositorLayerESaIS1_EE20_M_allocate_and_copyIPS1_EES5_mT_S6_
// type: void *__fastcall(int, unsigned int, int, int)
#[doc(alias = "RBX::TextureCompositorLayer* std::vector<RBX::TextureCompositorLayer,std::allocator<RBX::TextureCompositorLayer>>::_M_allocate_and_copy<RBX::TextureCompositorLayer*>(unsigned long,RBX::TextureCompositorLayer*,RBX::TextureCompositorLayer*)")]
#[doc(alias = "__ZNSt6vectorIN3RBX22TextureCompositorLayerESaIS1_EE20_M_allocate_and_copyIPS1_EES5_mT_S6_")]
pub fn stub_0xb8d608() -> ! {
    todo!("0xb8d608 RBX::TextureCompositorLayer* std::vector<RBX::TextureCompositorLayer,std::allocator<RBX::TextureCompositorLayer>>::_M_allocate_and_copy<RBX::TextureCompositorLayer*>(unsigned long,RBX::TextureCompositorLayer*,RBX::TextureCompositorLayer*)")
}

// 0xb8d6f4 — __ZNSt4pairIN4Ogre10TexturePtrEN5boost10shared_ptrIN3RBX17TextureCompositor3JobEEEEC2ERKS1_RKS7_
// type: _DWORD *__fastcall(_DWORD *, _DWORD *, _DWORD *)
#[doc(alias = "std::pair<Ogre::TexturePtr,boost::shared_ptr<RBX::TextureCompositor::Job>>::pair(Ogre::TexturePtr const&,boost::shared_ptr<RBX::TextureCompositor::Job> const&)")]
#[doc(alias = "__ZNSt4pairIN4Ogre10TexturePtrEN5boost10shared_ptrIN3RBX17TextureCompositor3JobEEEEC2ERKS1_RKS7_")]
pub fn stub_0xb8d6f4() -> ! {
    todo!("0xb8d6f4 std::pair<Ogre::TexturePtr,boost::shared_ptr<RBX::TextureCompositor::Job>>::pair(Ogre::TexturePtr const&,boost::shared_ptr<RBX::TextureCompositor::Job> const&)")
}

// 0xb8d8a4 — __GLOBAL__I_a_570
#[doc(alias = "global constructor keyed to_a_570")]
#[doc(alias = "__GLOBAL__I_a_570")]
pub fn stub_0xb8d8a4() -> ! {
    todo!("0xb8d8a4 global constructor keyed to_a_570")
}

// 0xb8edc8 — __ZN3RBX11AdornRbxGfx16getFreeSubEntityEv
// type: Ogre::RbxSubEntity *__fastcall(RBX::AdornRbxGfx *this)
#[doc(alias = "RBX::AdornRbxGfx::getFreeSubEntity(void)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx16getFreeSubEntityEv")]
pub fn stub_0xb8edc8() -> ! {
    todo!("0xb8edc8 RBX::AdornRbxGfx::getFreeSubEntity(void)")
}

// 0xb8ef38 — __ZN3RBX11AdornRbxGfx8findMeshESs
// type: void __fastcall(_DWORD *, _DWORD *, std::string *)
#[doc(alias = "RBX::AdornRbxGfx::findMesh(std::string)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx8findMeshESs")]
pub fn stub_0xb8ef38() -> ! {
    todo!("0xb8ef38 RBX::AdornRbxGfx::findMesh(std::string)")
}

// 0xb8f1b8 — __ZN3RBX11AdornRbxGfx12registerMeshERSsRN4Ogre7MeshPtrE
// type: void __fastcall(RBX::AdornRbxGfx *this, std::string *, Ogre::MeshPtr *)
#[doc(alias = "RBX::AdornRbxGfx::registerMesh(std::string &,Ogre::MeshPtr &)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx12registerMeshERSsRN4Ogre7MeshPtrE")]
pub fn stub_0xb8f1b8() -> ! {
    todo!("0xb8f1b8 RBX::AdornRbxGfx::registerMesh(std::string &,Ogre::MeshPtr &)")
}

// 0xb8f338 — __ZNK3RBX11AdornRbxGfx11getViewportEv
// type: int __fastcall(RBX::AdornRbxGfx *this, int)
#[doc(alias = "RBX::AdornRbxGfx::getViewport(void)const")]
#[doc(alias = "__ZNK3RBX11AdornRbxGfx11getViewportEv")]
pub fn stub_0xb8f338() -> ! {
    todo!("0xb8f338 RBX::AdornRbxGfx::getViewport(void)const")
}

// 0xb8f3f8 — __ZNK3RBX11AdornRbxGfx9getCameraEv
// type: int __fastcall(RBX::AdornRbxGfx *this)
#[doc(alias = "RBX::AdornRbxGfx::getCamera(void)const")]
#[doc(alias = "__ZNK3RBX11AdornRbxGfx9getCameraEv")]
pub fn stub_0xb8f3f8() -> ! {
    todo!("0xb8f3f8 RBX::AdornRbxGfx::getCamera(void)const")
}

// 0xb8f414 — __ZN3RBX11AdornRbxGfx10setTextureEiRKN5boost10shared_ptrINS_16TextureProxyBaseEEE
// type: void __fastcall(int, int, _DWORD *)
#[doc(alias = "RBX::AdornRbxGfx::setTexture(int,boost::shared_ptr<RBX::TextureProxyBase> const&)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx10setTextureEiRKN5boost10shared_ptrINS_16TextureProxyBaseEEE")]
pub fn stub_0xb8f414() -> ! {
    todo!("0xb8f414 RBX::AdornRbxGfx::setTexture(int,boost::shared_ptr<RBX::TextureProxyBase> const&)")
}

// 0xb8f560 — __ZNK3RBX11AdornRbxGfx14getTextureSizeERKN5boost10shared_ptrINS_16TextureProxyBaseEEE
// type: int __fastcall(int, int, int)
#[doc(alias = "RBX::AdornRbxGfx::getTextureSize(boost::shared_ptr<RBX::TextureProxyBase> const&)const")]
#[doc(alias = "__ZNK3RBX11AdornRbxGfx14getTextureSizeERKN5boost10shared_ptrINS_16TextureProxyBaseEEE")]
pub fn stub_0xb8f560() -> ! {
    todo!("0xb8f560 RBX::AdornRbxGfx::getTextureSize(boost::shared_ptr<RBX::TextureProxyBase> const&)const")
}

// 0xb8f588 — __ZNK3RBX11AdornRbxGfx6rect2dERKN3G3D6Rect2DES4_RKNS1_6Color4E
// type: int __fastcall(G3D::Vector2 *, int, int, int)
#[doc(alias = "RBX::AdornRbxGfx::rect2d(G3D::Rect2D const&,G3D::Rect2D const&,G3D::Color4 const&)const")]
#[doc(alias = "__ZNK3RBX11AdornRbxGfx6rect2dERKN3G3D6Rect2DES4_RKNS1_6Color4E")]
pub fn stub_0xb8f588() -> ! {
    todo!("0xb8f588 RBX::AdornRbxGfx::rect2d(G3D::Rect2D const&,G3D::Rect2D const&,G3D::Color4 const&)const")
}

// 0xb8f5c0 — __ZNK3RBX11AdornRbxGfx6rect2dERKN3G3D6Rect2DES4_RKNS1_7Vector2ES7_RKNS1_6Color4E
// type: int __fastcall(int, float *, float *, __int32 *, __int32 *, float *)
#[doc(alias = "RBX::AdornRbxGfx::rect2d(G3D::Rect2D const&,G3D::Rect2D const&,G3D::Vector2 const&,G3D::Vector2 const&,G3D::Color4 const&)const")]
#[doc(alias = "__ZNK3RBX11AdornRbxGfx6rect2dERKN3G3D6Rect2DES4_RKNS1_7Vector2ES7_RKNS1_6Color4E")]
pub fn stub_0xb8f5c0() -> ! {
    todo!("0xb8f5c0 RBX::AdornRbxGfx::rect2d(G3D::Rect2D const&,G3D::Rect2D const&,G3D::Vector2 const&,G3D::Vector2 const&,G3D::Color4 const&)const")
}

// 0xb8f8b8 — __ZNK3RBX11AdornRbxGfx6rect2dERKN3G3D6Rect2DERKNS1_6Color4E
// type: int __fastcall(G3D::Vector2 *, int, int)
#[doc(alias = "RBX::AdornRbxGfx::rect2d(G3D::Rect2D const&,G3D::Color4 const&)const")]
#[doc(alias = "__ZNK3RBX11AdornRbxGfx6rect2dERKN3G3D6Rect2DERKNS1_6Color4E")]
pub fn stub_0xb8f8b8() -> ! {
    todo!("0xb8f8b8 RBX::AdornRbxGfx::rect2d(G3D::Rect2D const&,G3D::Color4 const&)const")
}

// 0xb8f8ec — __ZNK3RBX11AdornRbxGfx6rect2dERKN3G3D6Rect2DERKNS1_7Vector2ES7_RKNS1_6Color4E
// type: int __fastcall(int, float *, __int32 *, __int32 *, float *)
#[doc(alias = "RBX::AdornRbxGfx::rect2d(G3D::Rect2D const&,G3D::Vector2 const&,G3D::Vector2 const&,G3D::Color4 const&)const")]
#[doc(alias = "__ZNK3RBX11AdornRbxGfx6rect2dERKN3G3D6Rect2DERKNS1_7Vector2ES7_RKNS1_6Color4E")]
pub fn stub_0xb8f8ec() -> ! {
    todo!("0xb8f8ec RBX::AdornRbxGfx::rect2d(G3D::Rect2D const&,G3D::Vector2 const&,G3D::Vector2 const&,G3D::Color4 const&)const")
}

// 0xb8f90c — __ZNK3RBX11AdornRbxGfx13outlineRect2dERKN3G3D6Rect2DEfRKNS1_6Color4E
// type: int __fastcall(int, float *, __int32, int)
#[doc(alias = "RBX::AdornRbxGfx::outlineRect2d(G3D::Rect2D const&,float,G3D::Color4 const&)const")]
#[doc(alias = "__ZNK3RBX11AdornRbxGfx13outlineRect2dERKN3G3D6Rect2DEfRKNS1_6Color4E")]
pub fn stub_0xb8f90c() -> ! {
    todo!("0xb8f90c RBX::AdornRbxGfx::outlineRect2d(G3D::Rect2D const&,float,G3D::Color4 const&)const")
}

// 0xb8fab0 — __ZNK3RBX11AdornRbxGfx13outlineRect2dERKN3G3D6Rect2DEfRKNS1_6Color4ES4_
// type: unsigned __int64 *__fastcall(unsigned __int64 *, float *, __int32, int, float *)
#[doc(alias = "RBX::AdornRbxGfx::outlineRect2d(G3D::Rect2D const&,float,G3D::Color4 const&,G3D::Rect2D const&)const")]
#[doc(alias = "__ZNK3RBX11AdornRbxGfx13outlineRect2dERKN3G3D6Rect2DEfRKNS1_6Color4ES4_")]
pub fn stub_0xb8fab0() -> ! {
    todo!("0xb8fab0 RBX::AdornRbxGfx::outlineRect2d(G3D::Rect2D const&,float,G3D::Color4 const&,G3D::Rect2D const&)const")
}

// 0xb8fc54 — __ZNK3RBX11AdornRbxGfx24renderClippedOutlineRectERKN3G3D6Rect2DES4_RKNS1_6Color4E
// type: unsigned __int64 *__fastcall(unsigned __int64 *result, float *, float *, int)
#[doc(alias = "RBX::AdornRbxGfx::renderClippedOutlineRect(G3D::Rect2D const&,G3D::Rect2D const&,G3D::Color4 const&)const")]
#[doc(alias = "__ZNK3RBX11AdornRbxGfx24renderClippedOutlineRectERKN3G3D6Rect2DES4_RKNS1_6Color4E")]
pub fn stub_0xb8fc54() -> ! {
    todo!("0xb8fc54 RBX::AdornRbxGfx::renderClippedOutlineRect(G3D::Rect2D const&,G3D::Rect2D const&,G3D::Color4 const&)const")
}

// 0xb8fd78 — __ZNK3RBX11AdornRbxGfx6line2dERKN3G3D7Vector2ES4_RKNS1_6Color4E
// type: int __fastcall(RBX::AdornRbxGfx *this, const G3D::Vector2 *, const G3D::Vector2 *, const G3D::Color4 *)
#[doc(alias = "RBX::AdornRbxGfx::line2d(G3D::Vector2 const&,G3D::Vector2 const&,G3D::Color4 const&)const")]
#[doc(alias = "__ZNK3RBX11AdornRbxGfx6line2dERKN3G3D7Vector2ES4_RKNS1_6Color4E")]
pub fn stub_0xb8fd78() -> ! {
    todo!("0xb8fd78 RBX::AdornRbxGfx::line2d(G3D::Vector2 const&,G3D::Vector2 const&,G3D::Color4 const&)const")
}

// 0xb8fe00 — __ZN3RBX11AdornRbxGfx14drawFont2DImplEPNS_5AdornERKSsRKN3G3D7Vector2EfRKNS5_6Color4ESB_NS_4Text4FontENSC_6XAlignENSC_6YAlignES8_RKNS5_6Rect2DE
// type: void __fastcall(_DWORD *, int, int, int, float *, float, int, int, int, int, int, int, int, int, struct _Unwind_Exception *lpuexcpt, int, int, int, int, char, int, int, int, char, int, int, int, Ogre::NedPoolingImpl *, Ogre::NedPoolingImpl *, int, int, int, int, int)
#[doc(alias = "RBX::AdornRbxGfx::drawFont2DImpl(RBX::Adorn *,std::string const&,G3D::Vector2 const&,float,G3D::Color4 const&,G3D::Color4 const&,RBX::Text::Font,RBX::Text::XAlign,RBX::Text::YAlign,G3D::Vector2 const&,G3D::Rect2D const&)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx14drawFont2DImplEPNS_5AdornERKSsRKN3G3D7Vector2EfRKNS5_6Color4ESB_NS_4Text4FontENSC_6XAlignENSC_6YAlignES8_RKNS5_6Rect2DE")]
pub fn stub_0xb8fe00() -> ! {
    todo!("0xb8fe00 RBX::AdornRbxGfx::drawFont2DImpl(RBX::Adorn *,std::string const&,G3D::Vector2 const&,float,G3D::Color4 const&,G3D::Color4 const&,RBX::Text::Font,RBX::Text::XAlign,RBX::Text::YAlign,G3D::Vector2 const&,G3D::Rect2D const&)")
}

// 0xb900fc — __ZNK3RBX11AdornRbxGfx17get2DStringBoundsERKSsfNS_4Text4FontERKN3G3D7Vector2E
// type: int __fastcall(_DWORD *, int, int, int, int, int)
#[doc(alias = "RBX::AdornRbxGfx::get2DStringBounds(std::string const&,float,RBX::Text::Font,G3D::Vector2 const&)const")]
#[doc(alias = "__ZNK3RBX11AdornRbxGfx17get2DStringBoundsERKSsfNS_4Text4FontERKN3G3D7Vector2E")]
pub fn stub_0xb900fc() -> ! {
    todo!("0xb900fc RBX::AdornRbxGfx::get2DStringBounds(std::string const&,float,RBX::Text::Font,G3D::Vector2 const&)const")
}

// 0xb90148 — __ZN3RBX11AdornRbxGfx18createTextureProxyERKNS_9ContentIdERbb
// type: void __fastcall(RBX::AdornRbxGfx *this, const RBX::ContentId *, std::string *, _BYTE *, int)
#[doc(alias = "RBX::AdornRbxGfx::createTextureProxy(RBX::ContentId const&,bool &,bool)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx18createTextureProxyERKNS_9ContentIdERbb")]
pub fn stub_0xb90148() -> ! {
    todo!("0xb90148 RBX::AdornRbxGfx::createTextureProxy(RBX::ContentId const&,bool &,bool)")
}

// 0xb90cc4 — __ZN3RBX11AdornRbxGfx22setObjectToWorldMatrixERKN3G3D15CoordinateFrameE
// type: int __fastcall(int result, __int64 *)
#[doc(alias = "RBX::AdornRbxGfx::setObjectToWorldMatrix(G3D::CoordinateFrame const&)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx22setObjectToWorldMatrixERKN3G3D15CoordinateFrameE")]
pub fn stub_0xb90cc4() -> ! {
    todo!("0xb90cc4 RBX::AdornRbxGfx::setObjectToWorldMatrix(G3D::CoordinateFrame const&)")
}

// 0xb90cfc — __ZN3RBX11AdornRbxGfx11setMaterialERKN3G3D6Color4Eii
// type: int __fastcall(RBX::AdornRbxGfx *this, const G3D::Color4 *, int, int)
#[doc(alias = "RBX::AdornRbxGfx::setMaterial(G3D::Color4 const&,int,int)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx11setMaterialERKN3G3D6Color4Eii")]
pub fn stub_0xb90cfc() -> ! {
    todo!("0xb90cfc RBX::AdornRbxGfx::setMaterial(G3D::Color4 const&,int,int)")
}

// 0xb91764 — __ZN3RBX11AdornRbxGfx16finishRenderPassEv
// type: int __fastcall(RBX::AdornRbxGfx *this)
#[doc(alias = "RBX::AdornRbxGfx::finishRenderPass(void)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx16finishRenderPassEv")]
pub fn stub_0xb91764() -> ! {
    todo!("0xb91764 RBX::AdornRbxGfx::finishRenderPass(void)")
}

// 0xb917fc — __ZN3RBX11AdornRbxGfx3boxERKN3G3D5AABoxERKNS1_6Color4E
// type: int __fastcall(int, __int32 *, const G3D::Color4 *)
#[doc(alias = "RBX::AdornRbxGfx::box(G3D::AABox const&,G3D::Color4 const&)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx3boxERKN3G3D5AABoxERKNS1_6Color4E")]
pub fn stub_0xb917fc() -> ! {
    todo!("0xb917fc RBX::AdornRbxGfx::box(G3D::AABox const&,G3D::Color4 const&)")
}

// 0xb926cc — __ZN3RBX11AdornRbxGfx14cylinderAlongXEffRKN3G3D6Color4Eb
// type: void __fastcall(RBX::AdornRbxGfx *this, float32_t, float32_t, const G3D::Color4 *, bool)
#[doc(alias = "RBX::AdornRbxGfx::cylinderAlongX(float,float,G3D::Color4 const&,bool)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx14cylinderAlongXEffRKN3G3D6Color4Eb")]
pub fn stub_0xb926cc() -> ! {
    todo!("0xb926cc RBX::AdornRbxGfx::cylinderAlongX(float,float,G3D::Color4 const&,bool)")
}

// 0xb93104 — __ZN3RBX11AdornRbxGfx6sphereERKN3G3D6SphereERKNS1_6Color4E
// type: void __fastcall(Ogre::RbxEntity **this, const G3D::Sphere *, const G3D::Color4 *)
#[doc(alias = "RBX::AdornRbxGfx::sphere(G3D::Sphere const&,G3D::Color4 const&)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx6sphereERKN3G3D6SphereERKNS1_6Color4E")]
pub fn stub_0xb93104() -> ! {
    todo!("0xb93104 RBX::AdornRbxGfx::sphere(G3D::Sphere const&,G3D::Color4 const&)")
}

// 0xb93a08 — __ZN3RBX11AdornRbxGfx11lineSegmentERKN3G3D11LineSegmentERKNS1_6Color4Ef
// type: void()
#[doc(alias = "RBX::AdornRbxGfx::lineSegment(G3D::LineSegment const&,G3D::Color4 const&,float)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx11lineSegmentERKN3G3D11LineSegmentERKNS1_6Color4Ef")]
pub fn stub_0xb93a08() -> ! {
    todo!("0xb93a08 RBX::AdornRbxGfx::lineSegment(G3D::LineSegment const&,G3D::Color4 const&,float)")
}

// 0xb93a0c — __ZN3RBX11AdornRbxGfx6line3dERKN3G3D7Vector3ES4_RKNS1_6Color4E
// type: int __fastcall(RBX::AdornRbxGfx *this, const G3D::Vector3 *, const G3D::Vector3 *, const G3D::Color4 *)
#[doc(alias = "RBX::AdornRbxGfx::line3d(G3D::Vector3 const&,G3D::Vector3 const&,G3D::Color4 const&)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx6line3dERKN3G3D7Vector3ES4_RKNS1_6Color4E")]
pub fn stub_0xb93a0c() -> ! {
    todo!("0xb93a0c RBX::AdornRbxGfx::line3d(G3D::Vector3 const&,G3D::Vector3 const&,G3D::Color4 const&)")
}

// 0xb93b60 — __ZN3RBX11AdornRbxGfx4quadERKN3G3D7Vector3ES4_S4_S4_RKNS1_6Color4ERKNS1_7Vector2ESA_ib
// type: void __fastcall(RBX::AdornRbxGfx *this, const G3D::Vector3 *, const G3D::Vector3 *, const G3D::Vector3 *, const G3D::Vector3 *, const G3D::Color4 *, const G3D::Vector2 *, const G3D::Vector2 *, int, bool)
#[doc(alias = "RBX::AdornRbxGfx::quad(G3D::Vector3 const&,G3D::Vector3 const&,G3D::Vector3 const&,G3D::Vector3 const&,G3D::Color4 const&,G3D::Vector2 const&,G3D::Vector2 const&,int,bool)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx4quadERKN3G3D7Vector3ES4_S4_S4_RKNS1_6Color4ERKNS1_7Vector2ESA_ib")]
pub fn stub_0xb93b60() -> ! {
    todo!("0xb93b60 RBX::AdornRbxGfx::quad(G3D::Vector3 const&,G3D::Vector3 const&,G3D::Vector3 const&,G3D::Vector3 const&,G3D::Color4 const&,G3D::Vector2 const&,G3D::Vector2 const&,int,bool)")
}

// 0xb94190 — __ZNK3RBX11AdornRbxGfx15convexPolygon2dEPKN3G3D7Vector2EiRKNS1_6Color4Ei
// type: int __fastcall(RBX::AdornRbxGfx *this, const G3D::Vector2 *, int, const G3D::Color4 *, int)
#[doc(alias = "RBX::AdornRbxGfx::convexPolygon2d(G3D::Vector2 const*,int,G3D::Color4 const&,int)const")]
#[doc(alias = "__ZNK3RBX11AdornRbxGfx15convexPolygon2dEPKN3G3D7Vector2EiRKNS1_6Color4Ei")]
pub fn stub_0xb94190() -> ! {
    todo!("0xb94190 RBX::AdornRbxGfx::convexPolygon2d(G3D::Vector2 const*,int,G3D::Color4 const&,int)const")
}

// 0xb94328 — __ZN3RBX11AdornRbxGfx13convexPolygonEPKN3G3D7Vector3EiRKNS1_6Color4Ei
// type: int __fastcall(RBX::AdornRbxGfx *this, const G3D::Vector3 *, int, const G3D::Color4 *, int)
#[doc(alias = "RBX::AdornRbxGfx::convexPolygon(G3D::Vector3 const*,int,G3D::Color4 const&,int)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx13convexPolygonEPKN3G3D7Vector3EiRKNS1_6Color4Ei")]
pub fn stub_0xb94328() -> ! {
    todo!("0xb94328 RBX::AdornRbxGfx::convexPolygon(G3D::Vector3 const*,int,G3D::Color4 const&,int)")
}

// 0xb94474 — __ZN3RBX11AdornRbxGfx9extrusionEPNS_13I3DLinearFuncEiS2_iRKN3G3D6Color4Ebb
// type: void __fastcall(int, int, int, int, int, G3D::Color4 *, char, char)
#[doc(alias = "RBX::AdornRbxGfx::extrusion(RBX::I3DLinearFunc *,int,RBX::I3DLinearFunc *,int,G3D::Color4 const&,bool,bool)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx9extrusionEPNS_13I3DLinearFuncEiS2_iRKN3G3D6Color4Ebb")]
pub fn stub_0xb94474() -> ! {
    todo!("0xb94474 RBX::AdornRbxGfx::extrusion(RBX::I3DLinearFunc *,int,RBX::I3DLinearFunc *,int,G3D::Color4 const&,bool,bool)")
}

// 0xb94f98 — __ZN3RBX11AdornRbxGfx4axesERKN3G3D6Color4ES4_S4_f
// type: void()
#[doc(alias = "RBX::AdornRbxGfx::axes(G3D::Color4 const&,G3D::Color4 const&,G3D::Color4 const&,float)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx4axesERKN3G3D6Color4ES4_S4_f")]
pub fn stub_0xb94f98() -> ! {
    todo!("0xb94f98 RBX::AdornRbxGfx::axes(G3D::Color4 const&,G3D::Color4 const&,G3D::Color4 const&,float)")
}

// 0xb94f9c — __ZN3RBX11AdornRbxGfx3rayERKNS_6RbxRayERKN3G3D6Color4E
// type: void __fastcall(Ogre::RbxEntity **this, const RBX::RbxRay *, const G3D::Color4 *)
#[doc(alias = "RBX::AdornRbxGfx::ray(RBX::RbxRay const&,G3D::Color4 const&)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx3rayERKNS_6RbxRayERKN3G3D6Color4E")]
pub fn stub_0xb94f9c() -> ! {
    todo!("0xb94f9c RBX::AdornRbxGfx::ray(RBX::RbxRay const&,G3D::Color4 const&)")
}

// 0xb95acc — __ZN3RBX11AdornRbxGfxD0Ev
// type: void __fastcall(RBX::AdornRbxGfx *__hidden this)
#[doc(alias = "RBX::AdornRbxGfx::~AdornRbxGfx()")]
#[doc(alias = "__ZN3RBX11AdornRbxGfxD0Ev")]
pub fn stub_0xb95acc() -> ! {
    todo!("0xb95acc RBX::AdornRbxGfx::~AdornRbxGfx()")
}

// 0xb95b6c — __ZN3RBX11AdornRbxGfxD1Ev
// type: void __fastcall(RBX::AdornRbxGfx *__hidden this)
#[doc(alias = "RBX::AdornRbxGfx::~AdornRbxGfx()")]
#[doc(alias = "__ZN3RBX11AdornRbxGfxD1Ev")]
pub fn stub_0xb95b6c() -> ! {
    todo!("0xb95b6c RBX::AdornRbxGfx::~AdornRbxGfx()")
}

// 0xb95b70 — __ZN3RBX11AdornRbxGfxD2Ev
// type: void __fastcall(RBX::AdornRbxGfx *__hidden this)
#[doc(alias = "RBX::AdornRbxGfx::~AdornRbxGfx()")]
#[doc(alias = "__ZN3RBX11AdornRbxGfxD2Ev")]
pub fn stub_0xb95b70() -> ! {
    todo!("0xb95b70 RBX::AdornRbxGfx::~AdornRbxGfx()")
}

// 0xb96238 — __ZN3RBX11AdornRbxGfx10destroyAllEv
// type: int __fastcall(RBX::AdornRbxGfx *this)
#[doc(alias = "RBX::AdornRbxGfx::destroyAll(void)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx10destroyAllEv")]
pub fn stub_0xb96238() -> ! {
    todo!("0xb96238 RBX::AdornRbxGfx::destroyAll(void)")
}

// 0xb9699c — __ZN3RBX11AdornRbxGfx9explosionERKN3G3D6SphereE
// type: int __fastcall(RBX::AdornRbxGfx *this, const G3D::Sphere *)
#[doc(alias = "RBX::AdornRbxGfx::explosion(G3D::Sphere const&)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx9explosionERKN3G3D6SphereE")]
pub fn stub_0xb9699c() -> ! {
    todo!("0xb9699c RBX::AdornRbxGfx::explosion(G3D::Sphere const&)")
}

// 0xb9834c — __ZN3RBX11AdornRbxGfx13preSubmitPassEv
// type: int __fastcall(RBX::AdornRbxGfx *this)
#[doc(alias = "RBX::AdornRbxGfx::preSubmitPass(void)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx13preSubmitPassEv")]
pub fn stub_0xb9834c() -> ! {
    todo!("0xb9834c RBX::AdornRbxGfx::preSubmitPass(void)")
}

// 0xb98388 — __ZN3RBX11AdornRbxGfx14postSubmitPassEv
// type: int __fastcall(RBX::AdornRbxGfx *this)
#[doc(alias = "RBX::AdornRbxGfx::postSubmitPass(void)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx14postSubmitPassEv")]
pub fn stub_0xb98388() -> ! {
    todo!("0xb98388 RBX::AdornRbxGfx::postSubmitPass(void)")
}

// 0xb983c8 — __ZN3RBX11AdornRbxGfx17prepareRenderPassEv
// type: RBX::AdornRbxGfx *__fastcall(RBX::AdornRbxGfx *this)
#[doc(alias = "RBX::AdornRbxGfx::prepareRenderPass(void)")]
#[doc(alias = "__ZN3RBX11AdornRbxGfx17prepareRenderPassEv")]
pub fn stub_0xb983c8() -> ! {
    todo!("0xb983c8 RBX::AdornRbxGfx::prepareRenderPass(void)")
}

// 0xb985fc — __ZNK3RBX11AdornRbxGfx13getRenderCapsEv
// type: int __fastcall(RBX::AdornRbxGfx *this)
#[doc(alias = "RBX::AdornRbxGfx::getRenderCaps(void)const")]
#[doc(alias = "__ZNK3RBX11AdornRbxGfx13getRenderCapsEv")]
pub fn stub_0xb985fc() -> ! {
    todo!("0xb985fc RBX::AdornRbxGfx::getRenderCaps(void)const")
}

// 0xb98608 — __ZN4Ogre7MeshPtrD1Ev
// type: void __fastcall(Ogre::MeshPtr *__hidden this)
#[doc(alias = "Ogre::MeshPtr::~MeshPtr()")]
#[doc(alias = "__ZN4Ogre7MeshPtrD1Ev")]
pub fn stub_0xb98608() -> ! {
    todo!("0xb98608 Ogre::MeshPtr::~MeshPtr()")
}

// 0xb98660 — __ZN3RBX5AdornD2Ev
// type: void __fastcall(RBX::Adorn *__hidden this)
#[doc(alias = "RBX::Adorn::~Adorn()")]
#[doc(alias = "__ZN3RBX5AdornD2Ev")]
pub fn stub_0xb98660() -> ! {
    todo!("0xb98660 RBX::Adorn::~Adorn()")
}

// 0xb98874 — __ZNSt3mapISsN4Ogre7MeshPtrESt4lessISsESaISt4pairIKSsS1_EEEixERS5_
// type: uint32_t *__fastcall(int, const void **)
#[doc(alias = "std::map<std::string,Ogre::MeshPtr,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::operator[](std::string const&)")]
#[doc(alias = "__ZNSt3mapISsN4Ogre7MeshPtrESt4lessISsESaISt4pairIKSsS1_EEEixERS5_")]
pub fn stub_0xb98874() -> ! {
    todo!("0xb98874 std::map<std::string,Ogre::MeshPtr,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::operator[](std::string const&)")
}

// 0xb98bb0 — __ZN4Ogre9SharedPtrINS_10DataStreamEED1Ev
// type: int __fastcall(int)
#[doc(alias = "Ogre::SharedPtr<Ogre::DataStream>::~SharedPtr()")]
#[doc(alias = "__ZN4Ogre9SharedPtrINS_10DataStreamEED1Ev")]
pub fn stub_0xb98bb0() -> ! {
    todo!("0xb98bb0 Ogre::SharedPtr<Ogre::DataStream>::~SharedPtr()")
}

// 0xb98c04 — __ZNSt3mapIN3RBX12Vector3int32EPN4Ogre9SceneNodeESt4lessIS1_ESaISt4pairIKS1_S4_EEEixERS8_
// type: _Rb_tree_node_base **__fastcall(int, int *)
#[doc(alias = "std::map<RBX::Vector3int32,Ogre::SceneNode *,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::operator[](RBX::Vector3int32 const&)")]
#[doc(alias = "__ZNSt3mapIN3RBX12Vector3int32EPN4Ogre9SceneNodeESt4lessIS1_ESaISt4pairIKS1_S4_EEEixERS8_")]
pub fn stub_0xb98c04() -> ! {
    todo!("0xb98c04 std::map<RBX::Vector3int32,Ogre::SceneNode *,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::operator[](RBX::Vector3int32 const&)")
}

// 0xb98c94 — __ZNSt6vectorIPN4Ogre9SceneNodeESaIS2_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS2_S4_EERKS2_
// type: void *__fastcall(int, char *__src, _DWORD *)
#[doc(alias = "std::vector<Ogre::SceneNode *,std::allocator<Ogre::SceneNode *>>::_M_insert_aux(__gnu_cxx::__normal_iterator<Ogre::SceneNode **,std::vector<Ogre::SceneNode *,std::allocator<Ogre::SceneNode *>>>,Ogre::SceneNode * const&)")]
#[doc(alias = "__ZNSt6vectorIPN4Ogre9SceneNodeESaIS2_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS2_S4_EERKS2_")]
pub fn stub_0xb98c94() -> ! {
    todo!("0xb98c94 std::vector<Ogre::SceneNode *,std::allocator<Ogre::SceneNode *>>::_M_insert_aux(__gnu_cxx::__normal_iterator<Ogre::SceneNode **,std::vector<Ogre::SceneNode *,std::allocator<Ogre::SceneNode *>>>,Ogre::SceneNode * const&)")
}

// 0xb98d8c — __ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS7_ERKS7_
// type: _Rb_tree_node_base *__fastcall(int, _Rb_tree_node_base *, int *)
#[doc(alias = "std::_Rb_tree<RBX::Vector3int32,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>,std::_Select1st<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *> const&)")]
#[doc(alias = "__ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS7_ERKS7_")]
pub fn stub_0xb98d8c() -> ! {
    todo!("0xb98d8c std::_Rb_tree<RBX::Vector3int32,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>,std::_Select1st<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *> const&)")
}

// 0xb990d4 — __ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE16_M_insert_uniqueERKS7_
// type: int __fastcall(int, _DWORD *, int *)
#[doc(alias = "std::_Rb_tree<RBX::Vector3int32,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>,std::_Select1st<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::_M_insert_unique(std::pair<RBX::Vector3int32 const,Ogre::SceneNode *> const&)")]
#[doc(alias = "__ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE16_M_insert_uniqueERKS7_")]
pub fn stub_0xb990d4() -> ! {
    todo!("0xb990d4 std::_Rb_tree<RBX::Vector3int32,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>,std::_Select1st<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::_M_insert_unique(std::pair<RBX::Vector3int32 const,Ogre::SceneNode *> const&)")
}

// 0xb991d4 — __ZNSt6vectorISt17_Rb_tree_iteratorISt4pairIKN3RBX12Vector3int32EPN4Ogre9SceneNodeEEESaIS9_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS9_SB_EERKS9_
// type: _DWORD *__fastcall(int, char *, _DWORD *)
#[doc(alias = "std::vector<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::allocator<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>>::_M_insert_aux(__gnu_cxx::__normal_iterator<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>*,std::vector<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::allocator<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>>>,std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>> const&)")]
#[doc(alias = "__ZNSt6vectorISt17_Rb_tree_iteratorISt4pairIKN3RBX12Vector3int32EPN4Ogre9SceneNodeEEESaIS9_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS9_SB_EERKS9_")]
pub fn stub_0xb991d4() -> ! {
    todo!("0xb991d4 std::vector<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::allocator<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>>::_M_insert_aux(__gnu_cxx::__normal_iterator<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>*,std::vector<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::allocator<std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>>>,std::_Rb_tree_iterator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>> const&)")
}

// 0xb992d8 — __ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE8_M_eraseEPSt13_Rb_tree_nodeIS7_E
// type: void __fastcall(int, _DWORD *)
#[doc(alias = "std::_Rb_tree<RBX::Vector3int32,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>,std::_Select1st<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIN3RBX12Vector3int32ESt4pairIKS1_PN4Ogre9SceneNodeEESt10_Select1stIS7_ESt4lessIS1_ESaIS7_EE8_M_eraseEPSt13_Rb_tree_nodeIS7_E")]
pub fn stub_0xb992d8() -> ! {
    todo!("0xb992d8 std::_Rb_tree<RBX::Vector3int32,std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>,std::_Select1st<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>,std::less<RBX::Vector3int32>,std::allocator<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Vector3int32 const,Ogre::SceneNode *>> *)")
}

// 0xb99300 — __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE8_M_eraseEPSt13_Rb_tree_nodeIS4_E
// type: void __fastcall(int, _DWORD *)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_erase(std::_Rb_tree_node<std::pair<std::string const,Ogre::MeshPtr>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE8_M_eraseEPSt13_Rb_tree_nodeIS4_E")]
pub fn stub_0xb99300() -> ! {
    todo!("0xb99300 std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_erase(std::_Rb_tree_node<std::pair<std::string const,Ogre::MeshPtr>> *)")
}

// 0xb99330 — __ZN9__gnu_cxx13new_allocatorISt4pairIKSsN4Ogre7MeshPtrEEE7destroyEPS5_
// type: void __fastcall(int, int)
#[doc(alias = "__gnu_cxx::new_allocator<std::pair<std::string const,Ogre::MeshPtr>>::destroy(std::pair<std::string const,Ogre::MeshPtr>*)")]
#[doc(alias = "__ZN9__gnu_cxx13new_allocatorISt4pairIKSsN4Ogre7MeshPtrEEE7destroyEPS5_")]
pub fn stub_0xb99330() -> ! {
    todo!("0xb99330 __gnu_cxx::new_allocator<std::pair<std::string const,Ogre::MeshPtr>>::destroy(std::pair<std::string const,Ogre::MeshPtr>*)")
}

// 0xb994b4 — __ZNK5boost23enable_shared_from_thisIN3RBX16TextureProxyBaseEE22_internal_accept_ownerIS2_NS1_15RbxTextureProxyEEEvPKNS_10shared_ptrIT_EEPT0_
// type: void __fastcall(_DWORD *, int, int)
#[doc(alias = "void boost::enable_shared_from_this<RBX::TextureProxyBase>::_internal_accept_owner<RBX::TextureProxyBase,RBX::RbxTextureProxy>(boost::shared_ptr<RBX::TextureProxyBase> const*,RBX::RbxTextureProxy *)const")]
#[doc(alias = "__ZNK5boost23enable_shared_from_thisIN3RBX16TextureProxyBaseEE22_internal_accept_ownerIS2_NS1_15RbxTextureProxyEEEvPKNS_10shared_ptrIT_EEPT0_")]
pub fn stub_0xb994b4() -> ! {
    todo!("0xb994b4 void boost::enable_shared_from_this<RBX::TextureProxyBase>::_internal_accept_owner<RBX::TextureProxyBase,RBX::RbxTextureProxy>(boost::shared_ptr<RBX::TextureProxyBase> const*,RBX::RbxTextureProxy *)const")
}

// 0xb9960c — __ZN5boost6detail17sp_counted_impl_pIN3RBX15RbxTextureProxyEED1Ev
// type: void()
#[doc(alias = "boost::detail::sp_counted_impl_p<RBX::RbxTextureProxy>::~sp_counted_impl_p()")]
#[doc(alias = "__ZN5boost6detail17sp_counted_impl_pIN3RBX15RbxTextureProxyEED1Ev")]
pub fn stub_0xb9960c() -> ! {
    todo!("0xb9960c boost::detail::sp_counted_impl_p<RBX::RbxTextureProxy>::~sp_counted_impl_p()")
}

// 0xb99610 — __ZN5boost6detail17sp_counted_impl_pIN3RBX15RbxTextureProxyEED0Ev
// type: int __fastcall(int)
#[doc(alias = "boost::detail::sp_counted_impl_p<RBX::RbxTextureProxy>::~sp_counted_impl_p()")]
#[doc(alias = "__ZN5boost6detail17sp_counted_impl_pIN3RBX15RbxTextureProxyEED0Ev")]
pub fn stub_0xb99610() -> ! {
    todo!("0xb99610 boost::detail::sp_counted_impl_p<RBX::RbxTextureProxy>::~sp_counted_impl_p()")
}

// 0xb99614 — __ZN5boost6detail17sp_counted_impl_pIN3RBX15RbxTextureProxyEE7disposeEv
// type: int __fastcall(int)
#[doc(alias = "boost::detail::sp_counted_impl_p<RBX::RbxTextureProxy>::dispose(void)")]
#[doc(alias = "__ZN5boost6detail17sp_counted_impl_pIN3RBX15RbxTextureProxyEE7disposeEv")]
pub fn stub_0xb99614() -> ! {
    todo!("0xb99614 boost::detail::sp_counted_impl_p<RBX::RbxTextureProxy>::dispose(void)")
}

// 0xb99624 — __ZN5boost6detail17sp_counted_impl_pIN3RBX15RbxTextureProxyEE11get_deleterERKSt9type_info
// type: int()
#[doc(alias = "boost::detail::sp_counted_impl_p<RBX::RbxTextureProxy>::get_deleter(std::type_info const&)")]
#[doc(alias = "__ZN5boost6detail17sp_counted_impl_pIN3RBX15RbxTextureProxyEE11get_deleterERKSt9type_info")]
pub fn stub_0xb99624() -> ! {
    todo!("0xb99624 boost::detail::sp_counted_impl_p<RBX::RbxTextureProxy>::get_deleter(std::type_info const&)")
}

// 0xb99628 — __ZN5boost6detail17sp_counted_impl_pIN3RBX15RbxTextureProxyEE19get_untyped_deleterEv
// type: int()
#[doc(alias = "boost::detail::sp_counted_impl_p<RBX::RbxTextureProxy>::get_untyped_deleter(void)")]
#[doc(alias = "__ZN5boost6detail17sp_counted_impl_pIN3RBX15RbxTextureProxyEE19get_untyped_deleterEv")]
pub fn stub_0xb99628() -> ! {
    todo!("0xb99628 boost::detail::sp_counted_impl_p<RBX::RbxTextureProxy>::get_untyped_deleter(void)")
}

// 0xb99630 — __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS4_ERKS4_
// type: _Rb_tree_node_base *__fastcall(int, _Rb_tree_node_base *, const void **)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<std::string const,Ogre::MeshPtr>>,std::pair<std::string const,Ogre::MeshPtr> const&)")]
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS4_ERKS4_")]
pub fn stub_0xb99630() -> ! {
    todo!("0xb99630 std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<std::string const,Ogre::MeshPtr>>,std::pair<std::string const,Ogre::MeshPtr> const&)")
}

// 0xb99978 — __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE9_M_insertEPSt18_Rb_tree_node_baseSC_RKS4_
// type: _Rb_tree_node_base *__fastcall(int, int, _Rb_tree_node_base *, const void **)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<std::string const,Ogre::MeshPtr> const&)")]
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE9_M_insertEPSt18_Rb_tree_node_baseSC_RKS4_")]
pub fn stub_0xb99978() -> ! {
    todo!("0xb99978 std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<std::string const,Ogre::MeshPtr> const&)")
}

// 0xb999ec — __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE16_M_insert_uniqueERKS4_
// type: int __fastcall(int, int, const void **)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_insert_unique(std::pair<std::string const,Ogre::MeshPtr> const&)")]
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE16_M_insert_uniqueERKS4_")]
pub fn stub_0xb999ec() -> ! {
    todo!("0xb999ec std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_insert_unique(std::pair<std::string const,Ogre::MeshPtr> const&)")
}

// 0xb99ad0 — __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE14_M_create_nodeERKS4_
// type: _DWORD *__fastcall(int, const std::string *, int, int, void *, int)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_create_node(std::pair<std::string const,Ogre::MeshPtr> const&)")]
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE14_M_create_nodeERKS4_")]
pub fn stub_0xb99ad0() -> ! {
    todo!("0xb99ad0 std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::_M_create_node(std::pair<std::string const,Ogre::MeshPtr> const&)")
}

// 0xb99be8 — __ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE4findERS1_
// type: _DWORD *__fastcall(int, const void **)
#[doc(alias = "std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::find(std::string const&)")]
#[doc(alias = "__ZNSt8_Rb_treeISsSt4pairIKSsN4Ogre7MeshPtrEESt10_Select1stIS4_ESt4lessISsESaIS4_EE4findERS1_")]
pub fn stub_0xb99be8() -> ! {
    todo!("0xb99be8 std::_Rb_tree<std::string,std::pair<std::string const,Ogre::MeshPtr>,std::_Select1st<std::pair<std::string const,Ogre::MeshPtr>>,std::less<std::string>,std::allocator<std::pair<std::string const,Ogre::MeshPtr>>>::find(std::string const&)")
}

// 0xb99c8c — __ZNSt6vectorIPN4Ogre12RbxSubEntityESaIS2_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS2_S4_EERKS2_
// type: void *__fastcall(int, char *__src, _DWORD *)
#[doc(alias = "std::vector<Ogre::RbxSubEntity *,std::allocator<Ogre::RbxSubEntity *>>::_M_insert_aux(__gnu_cxx::__normal_iterator<Ogre::RbxSubEntity **,std::vector<Ogre::RbxSubEntity *,std::allocator<Ogre::RbxSubEntity *>>>,Ogre::RbxSubEntity * const&)")]
#[doc(alias = "__ZNSt6vectorIPN4Ogre12RbxSubEntityESaIS2_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS2_S4_EERKS2_")]
pub fn stub_0xb99c8c() -> ! {
    todo!("0xb99c8c std::vector<Ogre::RbxSubEntity *,std::allocator<Ogre::RbxSubEntity *>>::_M_insert_aux(__gnu_cxx::__normal_iterator<Ogre::RbxSubEntity **,std::vector<Ogre::RbxSubEntity *,std::allocator<Ogre::RbxSubEntity *>>>,Ogre::RbxSubEntity * const&)")
}

// 0xb99d88 — __ZN4Ogre9SharedPtrINS_20GpuProgramParametersEED0Ev
// type: void __fastcall(_DWORD *)
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuProgramParameters>::~SharedPtr()")]
#[doc(alias = "__ZN4Ogre9SharedPtrINS_20GpuProgramParametersEED0Ev")]
pub fn stub_0xb99d88() -> ! {
    todo!("0xb99d88 Ogre::SharedPtr<Ogre::GpuProgramParameters>::~SharedPtr()")
}

// 0xb99e48 — __ZN4Ogre9SharedPtrINS_20GpuProgramParametersEE7destroyEv
// type: void __fastcall(int)
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuProgramParameters>::destroy(void)")]
#[doc(alias = "__ZN4Ogre9SharedPtrINS_20GpuProgramParametersEE7destroyEv")]
pub fn stub_0xb99e48() -> ! {
    todo!("0xb99e48 Ogre::SharedPtr<Ogre::GpuProgramParameters>::destroy(void)")
}

// 0xb99f40 — __ZN4Ogre9SharedPtrINS_20GpuProgramParametersEE4swapERS2_
// type: _DWORD *__fastcall(_DWORD *result, _DWORD *)
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuProgramParameters>::swap(Ogre::SharedPtr<Ogre::GpuProgramParameters>&)")]
#[doc(alias = "__ZN4Ogre9SharedPtrINS_20GpuProgramParametersEE4swapERS2_")]
pub fn stub_0xb99f40() -> ! {
    todo!("0xb99f40 Ogre::SharedPtr<Ogre::GpuProgramParameters>::swap(Ogre::SharedPtr<Ogre::GpuProgramParameters>&)")
}

// 0xb99f60 — __ZN4Ogre9SharedPtrINS_17GpuNamedConstantsEED1Ev
// type: _DWORD *__fastcall(_DWORD *)
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuNamedConstants>::~SharedPtr()")]
#[doc(alias = "__ZN4Ogre9SharedPtrINS_17GpuNamedConstantsEED1Ev")]
pub fn stub_0xb99f60() -> ! {
    todo!("0xb99f60 Ogre::SharedPtr<Ogre::GpuNamedConstants>::~SharedPtr()")
}

// 0xb99f90 — __ZN4Ogre9SharedPtrINS_22GpuLogicalBufferStructEED0Ev
// type: void __fastcall(_DWORD *)
#[doc(alias = "Ogre::SharedPtr<Ogre::GpuLogicalBufferStruct>::~SharedPtr()")]
#[doc(alias = "__ZN4Ogre9SharedPtrINS_22GpuLogicalBufferStructEED0Ev")]
pub fn stub_0xb99f90() -> ! {
    todo!("0xb99f90 Ogre::SharedPtr<Ogre::GpuLogicalBufferStruct>::~SharedPtr()")
}

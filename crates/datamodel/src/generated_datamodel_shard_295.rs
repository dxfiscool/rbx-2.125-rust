...
// Filter: demangled contains RBX:: + Instance|DataModel|Workspace|Part|Model (13497 total, 0 remaining) — global filler EA-sorted asc next 100 uncovered per-datamodel after 0x57f9c0
// Source: ida/export.json (85545 funcs, base 0x4000)
// Batch: 100 stubs | range 0x57f9c0..0x58bdd0 | EA-sorted asc distinct not yet in crates/datamodel/src (rbx_core::SharedPtr not boost)
...
pub fn stub_0x58ad14() -> ! {
    // BLOCKED: STL `std::pair` infra for the unmodeled `Callback` map (no native anywhere)
...
pub fn stub_0x58ae00() -> ! {
    // BLOCKED: STL `_Rb_tree` infra for the unmodeled `Callback` map (no native anywhere)

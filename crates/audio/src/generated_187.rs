//! audio generated_187 — 100 stubs EA-sorted asc global gap filler not yet in audio (FMOD|Sound|Audio 2541/2541 complete, gap filler)
//! Source: ida/export.json (85545 funcs) EA-sorted asc next 100 not in audio | rbx_core::SharedPtr not boost
//! Range 0x322850..0x326e58 | existing 23622 -> 23722 distinct
//! Batch: 100 stubs | // 0xADDR — mangled + #[doc(alias = "demangled")] + todo!("0xADDR mangled")

#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]

use rbx_core::SharedPtr;

const _: () = {
    let _ = core::marker::PhantomData::<SharedPtr<u8>>;
};


// 0x322850 — __ZN3RBX4Guid20generateStandardGUIDERSs
// demangled: RBX::Guid::generateStandardGUID(std::string &)
// type: _DWORD __fastcall(RBX::Guid *__hidden this, std::string *)
#[doc(alias = "RBX::Guid::generateStandardGUID(std::string &)")]
#[doc(alias = "__ZN3RBX4Guid20generateStandardGUIDERSs")]
pub fn stub_322850() -> ! {
    todo!("0x322850 RBX::Guid::generateStandardGUID(std::string &)")
}

// 0x32298c — __ZN3RBX4Guid15generateRBXGUIDERSs
// demangled: RBX::Guid::generateRBXGUID(std::string &)
// type: _DWORD __fastcall(struct _Unwind_Exception *lpuexcpt, std::string *)
#[doc(alias = "RBX::Guid::generateRBXGUID(std::string &)")]
#[doc(alias = "__ZN3RBX4Guid15generateRBXGUIDERSs")]
pub fn stub_32298c() -> ! {
    todo!("0x32298c RBX::Guid::generateRBXGUID(std::string &)")
}

// 0x322b04 — __ZN3RBX4Guid6assignENS0_4DataE
// demangled: RBX::Guid::assign(RBX::Guid::Data)
// type: Data *__fastcall(Data *this, Data)
#[doc(alias = "RBX::Guid::assign(RBX::Guid::Data)")]
#[doc(alias = "__ZN3RBX4Guid6assignENS0_4DataE")]
pub fn stub_322b04() -> ! {
    todo!("0x322b04 RBX::Guid::assign(RBX::Guid::Data)")
}

// 0x322b10 — __ZNK3RBX4Guid4DataltERKS1_
// demangled: RBX::Guid::Data::operator<(RBX::Guid::Data const&)const
// type: int __fastcall(_DWORD, _DWORD)
#[doc(alias = "RBX::Guid::Data::operator<(RBX::Guid::Data const&)const")]
#[doc(alias = "__ZNK3RBX4Guid4DataltERKS1_")]
pub fn stub_322b10() -> ! {
    todo!("0x322b10 RBX::Guid::Data::operator<(RBX::Guid::Data const&)const")
}

// 0x322b38 — __ZN3RBX4Guid7compareEPKS0_S2_
// demangled: RBX::Guid::compare(RBX::Guid const*,RBX::Guid const*)
// type: _DWORD __fastcall(RBX::Guid *__hidden this, const RBX::Guid *, const RBX::Guid *)
#[doc(alias = "RBX::Guid::compare(RBX::Guid const*,RBX::Guid const*)")]
#[doc(alias = "__ZN3RBX4Guid7compareEPKS0_S2_")]
pub fn stub_322b38() -> ! {
    todo!("0x322b38 RBX::Guid::compare(RBX::Guid const*,RBX::Guid const*)")
}

// 0x322b78 — __ZN3RBX4Guid7compareEPKS0_S2_S2_S2_
// demangled: RBX::Guid::compare(RBX::Guid const*,RBX::Guid const*,RBX::Guid const*,RBX::Guid const*)
// type: _DWORD __fastcall(RBX::Guid *__hidden this, const RBX::Guid *, const RBX::Guid *, const RBX::Guid *, const RBX::Guid *)
#[doc(alias = "RBX::Guid::compare(RBX::Guid const*,RBX::Guid const*,RBX::Guid const*,RBX::Guid const*)")]
#[doc(alias = "__ZN3RBX4Guid7compareEPKS0_S2_S2_S2_")]
pub fn stub_322b78() -> ! {
    todo!("0x322b78 RBX::Guid::compare(RBX::Guid const*,RBX::Guid const*,RBX::Guid const*,RBX::Guid const*)")
}

// 0x322bdc — __ZNK3RBX4Guid4Data14readableStringEi
// demangled: RBX::Guid::Data::readableString(int)const
// type: _DWORD __fastcall(RBX::Guid::Data *__hidden this, int)
#[doc(alias = "RBX::Guid::Data::readableString(int)const")]
#[doc(alias = "__ZNK3RBX4Guid4Data14readableStringEi")]
pub fn stub_322bdc() -> ! {
    todo!("0x322bdc RBX::Guid::Data::readableString(int)const")
}

// 0x322e00 — __GLOBAL__I_a_117
// demangled: global constructor keyed to_a_117
#[doc(alias = "global constructor keyed to_a_117")]
#[doc(alias = "__GLOBAL__I_a_117")]
pub fn stub_322e00() -> ! {
    todo!("0x322e00 global constructor keyed to_a_117")
}

// 0x322ec8 — __ZN3RBX14InstanceHandleC1EPNS_10Reflection13DescribedBaseE
// demangled: RBX::InstanceHandle::InstanceHandle(RBX::Reflection::DescribedBase *)
// type: _DWORD __fastcall(RBX::InstanceHandle *__hidden this, RBX::Reflection::DescribedBase *)
#[doc(alias = "RBX::InstanceHandle::InstanceHandle(RBX::Reflection::DescribedBase *)")]
#[doc(alias = "__ZN3RBX14InstanceHandleC1EPNS_10Reflection13DescribedBaseE")]
pub fn stub_322ec8() -> ! {
    todo!("0x322ec8 RBX::InstanceHandle::InstanceHandle(RBX::Reflection::DescribedBase *)")
}

// 0x322ed8 — __ZNK3RBX14InstanceHandle12operatorLessERKS0_
// demangled: RBX::InstanceHandle::operatorLess(RBX::InstanceHandle const&)const
// type: int __fastcall(_DWORD, _DWORD)
#[doc(alias = "RBX::InstanceHandle::operatorLess(RBX::InstanceHandle const&)const")]
#[doc(alias = "__ZNK3RBX14InstanceHandle12operatorLessERKS0_")]
pub fn stub_322ed8() -> ! {
    todo!("0x322ed8 RBX::InstanceHandle::operatorLess(RBX::InstanceHandle const&)const")
}

// 0x322ee8 — __ZNK3RBX14InstanceHandle5emptyEv
// demangled: RBX::InstanceHandle::empty(void)const
// type: _DWORD __fastcall(RBX::InstanceHandle *__hidden this)
#[doc(alias = "RBX::InstanceHandle::empty(void)const")]
#[doc(alias = "__ZNK3RBX14InstanceHandle5emptyEv")]
pub fn stub_322ee8() -> ! {
    todo!("0x322ee8 RBX::InstanceHandle::empty(void)const")
}

// 0x322ef4 — __ZN3RBX14InstanceHandle6linkToEN5boost10shared_ptrINS_10Reflection13DescribedBaseEEE
// demangled: RBX::InstanceHandle::linkTo(boost::shared_ptr<RBX::Reflection::DescribedBase>)
// type: int __fastcall(_DWORD, _DWORD)
#[doc(alias = "RBX::InstanceHandle::linkTo(rbx_core::SharedPtr<RBX::Reflection::DescribedBase>)")]
#[doc(alias = "__ZN3RBX14InstanceHandle6linkToEN5boost10shared_ptrINS_10Reflection13DescribedBaseEEE")]
pub fn stub_322ef4() -> ! {
    todo!("0x322ef4 RBX::InstanceHandle::linkTo(boost::shared_ptr<RBX::Reflection::DescribedBase>)")
}

// 0x322ef8 — __GLOBAL__I_a_118
// demangled: global constructor keyed to_a_118
#[doc(alias = "global constructor keyed to_a_118")]
#[doc(alias = "__GLOBAL__I_a_118")]
pub fn stub_322ef8() -> ! {
    todo!("0x322ef8 global constructor keyed to_a_118")
}

// 0x323028 — __ZN3RBX4Hash4hashERKSs
// demangled: RBX::Hash::hash(std::string const&)
// type: int __fastcall(unsigned __int8 **)
#[doc(alias = "RBX::Hash::hash(std::string const&)")]
#[doc(alias = "__ZN3RBX4Hash4hashERKSs")]
pub fn stub_323028() -> ! {
    todo!("0x323028 RBX::Hash::hash(std::string const&)")
}

// 0x32305c — __ZN3RBX17HeartbeatInstance34onServiceProviderHeartbeatInstanceEPNS_15ServiceProviderES2_
// demangled: RBX::HeartbeatInstance::onServiceProviderHeartbeatInstance(RBX::ServiceProvider *,RBX::ServiceProvider *)
// type: _DWORD __fastcall(RBX::HeartbeatInstance *__hidden this, RBX::ServiceProvider *, RBX::ServiceProvider *)
#[doc(alias = "RBX::HeartbeatInstance::onServiceProviderHeartbeatInstance(RBX::ServiceProvider *,RBX::ServiceProvider *)")]
#[doc(alias = "__ZN3RBX17HeartbeatInstance34onServiceProviderHeartbeatInstanceEPNS_15ServiceProviderES2_")]
pub fn stub_32305c() -> ! {
    todo!("0x32305c RBX::HeartbeatInstance::onServiceProviderHeartbeatInstance(RBX::ServiceProvider *,RBX::ServiceProvider *)")
}

// 0x323238 — __ZN3rbx7signals6signalIFvRKN3RBX9HeartbeatEEE7connectIN5boost3_bi6bind_tIvNS9_4_mfi3mf1IvNS2_17HeartbeatInstanceES5_EENSA_5list2INSA_5valueIPSE_EENS9_3argILi1EEEEEEEEENS0_10connectionERKT_
// demangled: rbx::signals::connection rbx::signals::signal<void ()(RBX::Heartbeat const&)>::connect<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>>(boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>> const&)
#[doc(alias = "rbx::signals::connection rbx::signals::signal<void ()(RBX::Heartbeat const&)>::connect<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>>(boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>> const&)")]
#[doc(alias = "__ZN3rbx7signals6signalIFvRKN3RBX9HeartbeatEEE7connectIN5boost3_bi6bind_tIvNS9_4_mfi3mf1IvNS2_17HeartbeatInstanceES5_EENSA_5list2INSA_5valueIPSE_EENS9_3argILi1EEEEEEEEENS0_10connectionERKT_")]
pub fn stub_323238() -> ! {
    todo!("0x323238 rbx::signals::connection rbx::signals::signal<void ()(RBX::Heartbeat const&)>::connect<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>>(boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>> const&)")
}

// 0x3232ac — __ZN3rbx7signals6signalIFvRKN3RBX9HeartbeatEEE13callable_slotIN5boost3_bi6bind_tIvNS9_4_mfi3mf1IvNS2_17HeartbeatInstanceES5_EENSA_5list2INSA_5valueIPSE_EENS9_3argILi1EEEEEEEED1Ev
// demangled: rbx::signals::signal<void ()(RBX::Heartbeat const&)>::callable_slot<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>>::~callable_slot()
#[doc(alias = "rbx::signals::signal<void ()(RBX::Heartbeat const&)>::callable_slot<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>>::~callable_slot()")]
#[doc(alias = "__ZN3rbx7signals6signalIFvRKN3RBX9HeartbeatEEE13callable_slotIN5boost3_bi6bind_tIvNS9_4_mfi3mf1IvNS2_17HeartbeatInstanceES5_EENSA_5list2INSA_5valueIPSE_EENS9_3argILi1EEEEEEEED1Ev")]
pub fn stub_3232ac() -> ! {
    todo!("0x3232ac rbx::signals::signal<void ()(RBX::Heartbeat const&)>::callable_slot<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>>::~callable_slot()")
}

// 0x3232d8 — __ZN3rbx7signals6signalIFvRKN3RBX9HeartbeatEEE13callable_slotIN5boost3_bi6bind_tIvNS9_4_mfi3mf1IvNS2_17HeartbeatInstanceES5_EENSA_5list2INSA_5valueIPSE_EENS9_3argILi1EEEEEEEED0Ev
// demangled: rbx::signals::signal<void ()(RBX::Heartbeat const&)>::callable_slot<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>>::~callable_slot()
#[doc(alias = "rbx::signals::signal<void ()(RBX::Heartbeat const&)>::callable_slot<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>>::~callable_slot()")]
#[doc(alias = "__ZN3rbx7signals6signalIFvRKN3RBX9HeartbeatEEE13callable_slotIN5boost3_bi6bind_tIvNS9_4_mfi3mf1IvNS2_17HeartbeatInstanceES5_EENSA_5list2INSA_5valueIPSE_EENS9_3argILi1EEEEEEEED0Ev")]
pub fn stub_3232d8() -> ! {
    todo!("0x3232d8 rbx::signals::signal<void ()(RBX::Heartbeat const&)>::callable_slot<boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>>::~callable_slot()")
}

// 0x3233ac — __ZN3rbx8callableINS_7signals6signalIFvRKN3RBX9HeartbeatEEE4slotEN5boost3_bi6bind_tIvNSA_4_mfi3mf1IvNS3_17HeartbeatInstanceES6_EENSB_5list2INSB_5valueIPSF_EENSA_3argILi1EEEEEEELi1ES7_E4callES6_
// demangled: rbx::callable<rbx::signals::signal<void ()(RBX::Heartbeat const&)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>,1,void ()(RBX::Heartbeat const&)>::call(RBX::Heartbeat const&)
#[doc(alias = "rbx::callable<rbx::signals::signal<void ()(RBX::Heartbeat const&)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>,1,void ()(RBX::Heartbeat const&)>::call(RBX::Heartbeat const&)")]
#[doc(alias = "__ZN3rbx8callableINS_7signals6signalIFvRKN3RBX9HeartbeatEEE4slotEN5boost3_bi6bind_tIvNSA_4_mfi3mf1IvNS3_17HeartbeatInstanceES6_EENSB_5list2INSB_5valueIPSF_EENSA_3argILi1EEEEEEELi1ES7_E4callES6_")]
pub fn stub_3233ac() -> ! {
    todo!("0x3233ac rbx::callable<rbx::signals::signal<void ()(RBX::Heartbeat const&)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>,1,void ()(RBX::Heartbeat const&)>::call(RBX::Heartbeat const&)")
}

// 0x3233b4 — __ZThn4_N3rbx8callableINS_7signals6signalIFvRKN3RBX9HeartbeatEEE4slotEN5boost3_bi6bind_tIvNSA_4_mfi3mf1IvNS3_17HeartbeatInstanceES6_EENSB_5list2INSB_5valueIPSF_EENSA_3argILi1EEEEEEELi1ES7_E4callES6_
// demangled: non-virtual thunk to rbx::callable<rbx::signals::signal<void ()(RBX::Heartbeat const&)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>,1,void ()(RBX::Heartbeat const&)>::call(RBX::Heartbeat const&)
#[doc(alias = "non-virtual thunk to rbx::callable<rbx::signals::signal<void ()(RBX::Heartbeat const&)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>,1,void ()(RBX::Heartbeat const&)>::call(RBX::Heartbeat const&)")]
#[doc(alias = "__ZThn4_N3rbx8callableINS_7signals6signalIFvRKN3RBX9HeartbeatEEE4slotEN5boost3_bi6bind_tIvNSA_4_mfi3mf1IvNS3_17HeartbeatInstanceES6_EENSB_5list2INSB_5valueIPSF_EENSA_3argILi1EEEEEEELi1ES7_E4callES6_")]
pub fn stub_3233b4() -> ! {
    todo!("0x3233b4 non-virtual thunk to rbx::callable<rbx::signals::signal<void ()(RBX::Heartbeat const&)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>,1,void ()(RBX::Heartbeat const&)>::call(RBX::Heartbeat const&)")
}

// 0x3233bc — __ZN5boost3_bi6bind_tIvNS_4_mfi3mf1IvN3RBX17HeartbeatInstanceERKNS4_9HeartbeatEEENS0_5list2INS0_5valueIPS5_EENS_3argILi1EEEEEEclIS6_EEvRKT_
// demangled: void boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>::operator()<RBX::Heartbeat>(RBX::Heartbeat const&)
#[doc(alias = "void boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>::operator()<RBX::Heartbeat>(RBX::Heartbeat const&)")]
#[doc(alias = "__ZN5boost3_bi6bind_tIvNS_4_mfi3mf1IvN3RBX17HeartbeatInstanceERKNS4_9HeartbeatEEENS0_5list2INS0_5valueIPS5_EENS_3argILi1EEEEEEclIS6_EEvRKT_")]
pub fn stub_3233bc() -> ! {
    todo!("0x3233bc void boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>::operator()<RBX::Heartbeat>(RBX::Heartbeat const&)")
}

// 0x3233d4 — __ZN3rbx8callableINS_7signals6signalIFvRKN3RBX9HeartbeatEEE4slotEN5boost3_bi6bind_tIvNSA_4_mfi3mf1IvNS3_17HeartbeatInstanceES6_EENSB_5list2INSB_5valueIPSF_EENSA_3argILi1EEEEEEELi1ES7_ED1Ev
// demangled: rbx::callable<rbx::signals::signal<void ()(RBX::Heartbeat const&)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>,1,void ()(RBX::Heartbeat const&)>::~callable()
#[doc(alias = "rbx::callable<rbx::signals::signal<void ()(RBX::Heartbeat const&)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>,1,void ()(RBX::Heartbeat const&)>::~callable()")]
#[doc(alias = "__ZN3rbx8callableINS_7signals6signalIFvRKN3RBX9HeartbeatEEE4slotEN5boost3_bi6bind_tIvNSA_4_mfi3mf1IvNS3_17HeartbeatInstanceES6_EENSB_5list2INSB_5valueIPSF_EENSA_3argILi1EEEEEEELi1ES7_ED1Ev")]
pub fn stub_3233d4() -> ! {
    todo!("0x3233d4 rbx::callable<rbx::signals::signal<void ()(RBX::Heartbeat const&)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>,1,void ()(RBX::Heartbeat const&)>::~callable()")
}

// 0x323400 — __ZN3rbx8callableINS_7signals6signalIFvRKN3RBX9HeartbeatEEE4slotEN5boost3_bi6bind_tIvNSA_4_mfi3mf1IvNS3_17HeartbeatInstanceES6_EENSB_5list2INSB_5valueIPSF_EENSA_3argILi1EEEEEEELi1ES7_ED0Ev
// demangled: rbx::callable<rbx::signals::signal<void ()(RBX::Heartbeat const&)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>,1,void ()(RBX::Heartbeat const&)>::~callable()
#[doc(alias = "rbx::callable<rbx::signals::signal<void ()(RBX::Heartbeat const&)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>,1,void ()(RBX::Heartbeat const&)>::~callable()")]
#[doc(alias = "__ZN3rbx8callableINS_7signals6signalIFvRKN3RBX9HeartbeatEEE4slotEN5boost3_bi6bind_tIvNSA_4_mfi3mf1IvNS3_17HeartbeatInstanceES6_EENSB_5list2INSB_5valueIPSF_EENSA_3argILi1EEEEEEELi1ES7_ED0Ev")]
pub fn stub_323400() -> ! {
    todo!("0x323400 rbx::callable<rbx::signals::signal<void ()(RBX::Heartbeat const&)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf1<void,RBX::HeartbeatInstance,RBX::Heartbeat const&>,boost::_bi::list2<boost::_bi::value<RBX::HeartbeatInstance*>,boost::arg<1>>>,1,void ()(RBX::Heartbeat const&)>::~callable()")
}

// 0x3234d4 — __GLOBAL__I_a_119
// demangled: global constructor keyed to_a_119
#[doc(alias = "global constructor keyed to_a_119")]
#[doc(alias = "__GLOBAL__I_a_119")]
pub fn stub_3234d4() -> ! {
    todo!("0x3234d4 global constructor keyed to_a_119")
}

// 0x32366c — __ZN3RBX13HandleHitTest18hitTestHandleWorldERKNS_7ExtentsENS_10HandleTypeERKNS_6RbxRayERN3G3D7Vector3ERNS_8NormalIdEi
// demangled: RBX::HandleHitTest::hitTestHandleWorld(RBX::Extents const&,RBX::HandleType,RBX::RbxRay const&,G3D::Vector3 &,RBX::NormalId &,int)
#[doc(alias = "RBX::HandleHitTest::hitTestHandleWorld(RBX::Extents const&,RBX::HandleType,RBX::RbxRay const&,G3D::Vector3 &,RBX::NormalId &,int)")]
#[doc(alias = "__ZN3RBX13HandleHitTest18hitTestHandleWorldERKNS_7ExtentsENS_10HandleTypeERKNS_6RbxRayERN3G3D7Vector3ERNS_8NormalIdEi")]
pub fn stub_32366c() -> ! {
    todo!("0x32366c RBX::HandleHitTest::hitTestHandleWorld(RBX::Extents const&,RBX::HandleType,RBX::RbxRay const&,G3D::Vector3 &,RBX::NormalId &,int)")
}

// 0x323768 — __ZN3RBX13HandleHitTest18hitTestHandleLocalERKNS_7ExtentsERKN3G3D15CoordinateFrameENS_10HandleTypeERKNS_6RbxRayERNS4_7Vector3ERNS_8NormalIdEi
// demangled: RBX::HandleHitTest::hitTestHandleLocal(RBX::Extents const&,G3D::CoordinateFrame const&,RBX::HandleType,RBX::RbxRay const&,G3D::Vector3 &,RBX::NormalId &,int)
// type: int __fastcall(int, __int32 *, int, int, float *, int *, int)
#[doc(alias = "RBX::HandleHitTest::hitTestHandleLocal(RBX::Extents const&,G3D::CoordinateFrame const&,RBX::HandleType,RBX::RbxRay const&,G3D::Vector3 &,RBX::NormalId &,int)")]
#[doc(alias = "__ZN3RBX13HandleHitTest18hitTestHandleLocalERKNS_7ExtentsERKN3G3D15CoordinateFrameENS_10HandleTypeERKNS_6RbxRayERNS4_7Vector3ERNS_8NormalIdEi")]
pub fn stub_323768() -> ! {
    todo!("0x323768 RBX::HandleHitTest::hitTestHandleLocal(RBX::Extents const&,G3D::CoordinateFrame const&,RBX::HandleType,RBX::RbxRay const&,G3D::Vector3 &,RBX::NormalId &,int)")
}

// 0x3238e8 — __ZN3RBX13HandleHitTest22hitTestMoveHandleWorldERKNS_7ExtentsERKNS_6RbxRayERN3G3D7Vector3ERNS_8NormalIdEi
// demangled: RBX::HandleHitTest::hitTestMoveHandleWorld(RBX::Extents const&,RBX::RbxRay const&,G3D::Vector3 &,RBX::NormalId &,int)
#[doc(alias = "RBX::HandleHitTest::hitTestMoveHandleWorld(RBX::Extents const&,RBX::RbxRay const&,G3D::Vector3 &,RBX::NormalId &,int)")]
#[doc(alias = "__ZN3RBX13HandleHitTest22hitTestMoveHandleWorldERKNS_7ExtentsERKNS_6RbxRayERN3G3D7Vector3ERNS_8NormalIdEi")]
pub fn stub_3238e8() -> ! {
    todo!("0x3238e8 RBX::HandleHitTest::hitTestMoveHandleWorld(RBX::Extents const&,RBX::RbxRay const&,G3D::Vector3 &,RBX::NormalId &,int)")
}

// 0x323b78 — __GLOBAL__I_a_120
// demangled: global constructor keyed to_a_120
#[doc(alias = "global constructor keyed to_a_120")]
#[doc(alias = "__GLOBAL__I_a_120")]
pub fn stub_323b78() -> ! {
    todo!("0x323b78 global constructor keyed to_a_120")
}

// 0x323de8 — __ZN3RBX8IndexBoxC1ERKN3G3D7Vector3ES4_
// demangled: RBX::IndexBox::IndexBox(G3D::Vector3 const&,G3D::Vector3 const&)
// type: _DWORD __fastcall(RBX::IndexBox *__hidden this, const G3D::Vector3 *, const G3D::Vector3 *)
#[doc(alias = "RBX::IndexBox::IndexBox(G3D::Vector3 const&,G3D::Vector3 const&)")]
#[doc(alias = "__ZN3RBX8IndexBoxC1ERKN3G3D7Vector3ES4_")]
pub fn stub_323de8() -> ! {
    todo!("0x323de8 RBX::IndexBox::IndexBox(G3D::Vector3 const&,G3D::Vector3 const&)")
}

// 0x323dec — __ZN3RBX8IndexBoxC2ERKN3G3D7Vector3ES4_
// demangled: RBX::IndexBox::IndexBox(G3D::Vector3 const&,G3D::Vector3 const&)
// type: _DWORD __fastcall(RBX::IndexBox *__hidden this, const G3D::Vector3 *, Vector3 *)
#[doc(alias = "RBX::IndexBox::IndexBox(G3D::Vector3 const&,G3D::Vector3 const&)")]
#[doc(alias = "__ZN3RBX8IndexBoxC2ERKN3G3D7Vector3ES4_")]
pub fn stub_323dec() -> ! {
    todo!("0x323dec RBX::IndexBox::IndexBox(G3D::Vector3 const&,G3D::Vector3 const&)")
}

// 0x323efc — __ZNK3RBX8IndexBox14getFaceCornersEiRN3G3D7Vector3ES3_S3_S3_
// demangled: RBX::IndexBox::getFaceCorners(int,G3D::Vector3 &,G3D::Vector3 &,G3D::Vector3 &,G3D::Vector3 &)const
// type: _DWORD __fastcall(RBX::IndexBox *__hidden this, int, G3D::Vector3 *, G3D::Vector3 *, G3D::Vector3 *, G3D::Vector3 *)
#[doc(alias = "RBX::IndexBox::getFaceCorners(int,G3D::Vector3 &,G3D::Vector3 &,G3D::Vector3 &,G3D::Vector3 &)const")]
#[doc(alias = "__ZNK3RBX8IndexBox14getFaceCornersEiRN3G3D7Vector3ES3_S3_S3_")]
pub fn stub_323efc() -> ! {
    todo!("0x323efc RBX::IndexBox::getFaceCorners(int,G3D::Vector3 &,G3D::Vector3 &,G3D::Vector3 &,G3D::Vector3 &)const")
}

// 0x323fd8 — __ZN3RBX8IndexBoxD1Ev
// demangled: RBX::IndexBox::~IndexBox()
// type: void __fastcall(RBX::IndexBox *__hidden this)
#[doc(alias = "RBX::IndexBox::~IndexBox()")]
#[doc(alias = "__ZN3RBX8IndexBoxD1Ev")]
pub fn stub_323fd8() -> ! {
    todo!("0x323fd8 RBX::IndexBox::~IndexBox()")
}

// 0x323fdc — __ZN3RBX8IndexBoxD0Ev
// demangled: RBX::IndexBox::~IndexBox()
// type: void __fastcall(RBX::IndexBox *__hidden this)
#[doc(alias = "RBX::IndexBox::~IndexBox()")]
#[doc(alias = "__ZN3RBX8IndexBoxD0Ev")]
pub fn stub_323fdc() -> ! {
    todo!("0x323fdc RBX::IndexBox::~IndexBox()")
}

// 0x323fe0 — __GLOBAL__I_a_121
// demangled: global constructor keyed to_a_121
#[doc(alias = "global constructor keyed to_a_121")]
#[doc(alias = "__GLOBAL__I_a_121")]
pub fn stub_323fe0() -> ! {
    todo!("0x323fe0 global constructor keyed to_a_121")
}

// 0x324018 — __ZN3RBX11IndexedMeshC2Ev
// demangled: RBX::IndexedMesh::IndexedMesh(void)
// type: _DWORD __fastcall(RBX::IndexedMesh *__hidden this)
#[doc(alias = "RBX::IndexedMesh::IndexedMesh(void)")]
#[doc(alias = "__ZN3RBX11IndexedMeshC2Ev")]
pub fn stub_324018() -> ! {
    todo!("0x324018 RBX::IndexedMesh::IndexedMesh(void)")
}

// 0x32403c — __ZN3RBX11IndexedMeshD0Ev
// demangled: RBX::IndexedMesh::~IndexedMesh()
// type: void __fastcall(RBX::IndexedMesh *__hidden this)
#[doc(alias = "RBX::IndexedMesh::~IndexedMesh()")]
#[doc(alias = "__ZN3RBX11IndexedMeshD0Ev")]
pub fn stub_32403c() -> ! {
    todo!("0x32403c RBX::IndexedMesh::~IndexedMesh()")
}

// 0x3240dc — __ZN3RBX11IndexedMeshD1Ev
// demangled: RBX::IndexedMesh::~IndexedMesh()
// type: void __fastcall(RBX::IndexedMesh *__hidden this)
#[doc(alias = "RBX::IndexedMesh::~IndexedMesh()")]
#[doc(alias = "__ZN3RBX11IndexedMeshD1Ev")]
pub fn stub_3240dc() -> ! {
    todo!("0x3240dc RBX::IndexedMesh::~IndexedMesh()")
}

// 0x3240e0 — __ZN3RBX11IndexedMeshD2Ev
// demangled: RBX::IndexedMesh::~IndexedMesh()
// type: void __fastcall(RBX::IndexedMesh *__hidden this)
#[doc(alias = "RBX::IndexedMesh::~IndexedMesh()")]
#[doc(alias = "__ZN3RBX11IndexedMeshD2Ev")]
pub fn stub_3240e0() -> ! {
    todo!("0x3240e0 RBX::IndexedMesh::~IndexedMesh()")
}

// 0x324354 — __ZN3RBX11IndexedMesh16setComputedUpperEPS0_
// demangled: RBX::IndexedMesh::setComputedUpper(RBX::IndexedMesh*)
// type: _DWORD __fastcall(RBX::IndexedMesh *__hidden this, RBX::IndexedMesh *)
#[doc(alias = "RBX::IndexedMesh::setComputedUpper(RBX::IndexedMesh*)")]
#[doc(alias = "__ZN3RBX11IndexedMesh16setComputedUpperEPS0_")]
pub fn stub_324354() -> ! {
    todo!("0x324354 RBX::IndexedMesh::setComputedUpper(RBX::IndexedMesh*)")
}

// 0x32438c — __ZN3RBX11IndexedMesh8setUpperEPS0_
// demangled: RBX::IndexedMesh::setUpper(RBX::IndexedMesh*)
// type: _DWORD __fastcall(RBX::IndexedMesh *__hidden this, RBX::IndexedMesh *)
#[doc(alias = "RBX::IndexedMesh::setUpper(RBX::IndexedMesh*)")]
#[doc(alias = "__ZN3RBX11IndexedMesh8setUpperEPS0_")]
pub fn stub_32438c() -> ! {
    todo!("0x32438c RBX::IndexedMesh::setUpper(RBX::IndexedMesh*)")
}

// 0x3243d4 — __ZN3RBX11IndexedMesh8setLowerEPS0_
// demangled: RBX::IndexedMesh::setLower(RBX::IndexedMesh*)
// type: _DWORD __fastcall(RBX::IndexedMesh *__hidden this, RBX::IndexedMesh *)
#[doc(alias = "RBX::IndexedMesh::setLower(RBX::IndexedMesh*)")]
#[doc(alias = "__ZN3RBX11IndexedMesh8setLowerEPS0_")]
pub fn stub_3243d4() -> ! {
    todo!("0x3243d4 RBX::IndexedMesh::setLower(RBX::IndexedMesh*)")
}

// 0x3244b8 — __ZN3RBX11IndexedMesh16getComputedUpperEv
// demangled: RBX::IndexedMesh::getComputedUpper(void)
// type: _DWORD __fastcall(RBX::IndexedMesh *__hidden this)
#[doc(alias = "RBX::IndexedMesh::getComputedUpper(void)")]
#[doc(alias = "__ZN3RBX11IndexedMesh16getComputedUpperEv")]
pub fn stub_3244b8() -> ! {
    todo!("0x3244b8 RBX::IndexedMesh::getComputedUpper(void)")
}

// 0x3244bc — __ZN3RBX11IndexedMesh20getIndexedMeshParentEv
// demangled: RBX::IndexedMesh::getIndexedMeshParent(void)
// type: _DWORD __fastcall(RBX::IndexedMesh *__hidden this)
#[doc(alias = "RBX::IndexedMesh::getIndexedMeshParent(void)")]
#[doc(alias = "__ZN3RBX11IndexedMesh20getIndexedMeshParentEv")]
pub fn stub_3244bc() -> ! {
    todo!("0x3244bc RBX::IndexedMesh::getIndexedMeshParent(void)")
}

// 0x3244c0 — __ZN3RBX11IndexedMesh14attachChildrenEPS0_
// demangled: RBX::IndexedMesh::attachChildren(RBX::IndexedMesh*)
// type: _DWORD __fastcall(RBX::IndexedMesh *__hidden this, RBX::IndexedMesh *)
#[doc(alias = "RBX::IndexedMesh::attachChildren(RBX::IndexedMesh*)")]
#[doc(alias = "__ZN3RBX11IndexedMesh14attachChildrenEPS0_")]
pub fn stub_3244c0() -> ! {
    todo!("0x3244c0 RBX::IndexedMesh::attachChildren(RBX::IndexedMesh*)")
}

// 0x3244fc — __ZN3RBX11IndexedMesh15onParentChangedEPNS_11IndexedTreeE
// demangled: RBX::IndexedMesh::onParentChanged(RBX::IndexedTree *)
// type: _DWORD __fastcall(RBX::IndexedMesh *__hidden this, RBX::IndexedTree *)
#[doc(alias = "RBX::IndexedMesh::onParentChanged(RBX::IndexedTree *)")]
#[doc(alias = "__ZN3RBX11IndexedMesh15onParentChangedEPNS_11IndexedTreeE")]
pub fn stub_3244fc() -> ! {
    todo!("0x3244fc RBX::IndexedMesh::onParentChanged(RBX::IndexedTree *)")
}

// 0x324550 — __ZN3RBX11IndexedMesh14severeChildrenEPS0_
// demangled: RBX::IndexedMesh::severeChildren(RBX::IndexedMesh*)
// type: _DWORD __fastcall(RBX::IndexedMesh *__hidden this, RBX::IndexedMesh *)
#[doc(alias = "RBX::IndexedMesh::severeChildren(RBX::IndexedMesh*)")]
#[doc(alias = "__ZN3RBX11IndexedMesh14severeChildrenEPS0_")]
pub fn stub_324550() -> ! {
    todo!("0x324550 RBX::IndexedMesh::severeChildren(RBX::IndexedMesh*)")
}

// 0x32458c — __ZNK3RBX11IndexedMesh25getConstIndexedMeshParentEv
// demangled: RBX::IndexedMesh::getConstIndexedMeshParent(void)const
// type: _DWORD __fastcall(RBX::IndexedMesh *__hidden this)
#[doc(alias = "RBX::IndexedMesh::getConstIndexedMeshParent(void)const")]
#[doc(alias = "__ZNK3RBX11IndexedMesh25getConstIndexedMeshParentEv")]
pub fn stub_32458c() -> ! {
    todo!("0x32458c RBX::IndexedMesh::getConstIndexedMeshParent(void)const")
}

// 0x3245f8 — __ZNK3RBX11IndexedMesh21getConstComputedUpperEv
// demangled: RBX::IndexedMesh::getConstComputedUpper(void)const
// type: _DWORD __fastcall(RBX::IndexedMesh *__hidden this)
#[doc(alias = "RBX::IndexedMesh::getConstComputedUpper(void)const")]
#[doc(alias = "__ZNK3RBX11IndexedMesh21getConstComputedUpperEv")]
pub fn stub_3245f8() -> ! {
    todo!("0x3245f8 RBX::IndexedMesh::getConstComputedUpper(void)const")
}

// 0x3245fc — __ZN3RBX11IndexedMesh11isUpperRootEPKS0_
// demangled: RBX::IndexedMesh::isUpperRoot(RBX::IndexedMesh const*)
// type: int __fastcall(RBX::IndexedMesh *this, const RBX::IndexedMesh *, int)
#[doc(alias = "RBX::IndexedMesh::isUpperRoot(RBX::IndexedMesh const*)")]
#[doc(alias = "__ZN3RBX11IndexedMesh11isUpperRootEPKS0_")]
pub fn stub_3245fc() -> ! {
    todo!("0x3245fc RBX::IndexedMesh::isUpperRoot(RBX::IndexedMesh const*)")
}

// 0x324684 — __ZN3RBX11IndexedTree13getTypedChildINS_11IndexedMeshEEEPT_i
// demangled: RBX::IndexedMesh * RBX::IndexedTree::getTypedChild<RBX::IndexedMesh>(int)
#[doc(alias = "RBX::IndexedMesh * RBX::IndexedTree::getTypedChild<RBX::IndexedMesh>(int)")]
#[doc(alias = "__ZN3RBX11IndexedTree13getTypedChildINS_11IndexedMeshEEEPT_i")]
pub fn stub_324684() -> ! {
    todo!("0x324684 RBX::IndexedMesh * RBX::IndexedTree::getTypedChild<RBX::IndexedMesh>(int)")
}

// 0x3246ec — __ZN3RBX11IndexedMesh13lowersChangedEv
// demangled: RBX::IndexedMesh::lowersChanged(void)
// type: _DWORD __fastcall(RBX::IndexedMesh *__hidden this)
#[doc(alias = "RBX::IndexedMesh::lowersChanged(void)")]
#[doc(alias = "__ZN3RBX11IndexedMesh13lowersChangedEv")]
pub fn stub_3246ec() -> ! {
    todo!("0x3246ec RBX::IndexedMesh::lowersChanged(void)")
}

// 0x324710 — __GLOBAL__I_a_122
// demangled: global constructor keyed to_a_122
#[doc(alias = "global constructor keyed to_a_122")]
#[doc(alias = "__GLOBAL__I_a_122")]
pub fn stub_324710() -> ! {
    todo!("0x324710 global constructor keyed to_a_122")
}

// 0x3247d8 — __ZN3RBX11IndexedTreeC2Ev
// demangled: RBX::IndexedTree::IndexedTree(void)
// type: _DWORD __fastcall(RBX::IndexedTree *__hidden this)
#[doc(alias = "RBX::IndexedTree::IndexedTree(void)")]
#[doc(alias = "__ZN3RBX11IndexedTreeC2Ev")]
pub fn stub_3247d8() -> ! {
    todo!("0x3247d8 RBX::IndexedTree::IndexedTree(void)")
}

// 0x324800 — __ZN3RBX11IndexedTreeD0Ev
// demangled: RBX::IndexedTree::~IndexedTree()
// type: void __fastcall(RBX::IndexedTree *__hidden this)
#[doc(alias = "RBX::IndexedTree::~IndexedTree()")]
#[doc(alias = "__ZN3RBX11IndexedTreeD0Ev")]
pub fn stub_324800() -> ! {
    todo!("0x324800 RBX::IndexedTree::~IndexedTree()")
}

// 0x3248a0 — __ZN3RBX11IndexedTreeD1Ev
// demangled: RBX::IndexedTree::~IndexedTree()
// type: void __fastcall(RBX::IndexedTree *__hidden this)
#[doc(alias = "RBX::IndexedTree::~IndexedTree()")]
#[doc(alias = "__ZN3RBX11IndexedTreeD1Ev")]
pub fn stub_3248a0() -> ! {
    todo!("0x3248a0 RBX::IndexedTree::~IndexedTree()")
}

// 0x3248a4 — __ZN3RBX11IndexedTreeD2Ev
// demangled: RBX::IndexedTree::~IndexedTree()
// type: void __fastcall(RBX::IndexedTree *__hidden this)
#[doc(alias = "RBX::IndexedTree::~IndexedTree()")]
#[doc(alias = "__ZN3RBX11IndexedTreeD2Ev")]
pub fn stub_3248a4() -> ! {
    todo!("0x3248a4 RBX::IndexedTree::~IndexedTree()")
}

// 0x324a74 — __ZN3RBX11IndexedTree20setIndexedTreeParentEPS0_
// demangled: RBX::IndexedTree::setIndexedTreeParent(RBX::IndexedTree*)
// type: _DWORD __fastcall(RBX::IndexedTree *__hidden this, RBX::IndexedTree *)
#[doc(alias = "RBX::IndexedTree::setIndexedTreeParent(RBX::IndexedTree*)")]
#[doc(alias = "__ZN3RBX11IndexedTree20setIndexedTreeParentEPS0_")]
pub fn stub_324a74() -> ! {
    todo!("0x324a74 RBX::IndexedTree::setIndexedTreeParent(RBX::IndexedTree*)")
}

// 0x324c14 — __ZN3RBX10IndexArrayINS_11IndexedTreeEXadL_ZNS1_8getIndexEvEEE10fastRemoveEPS1_
// demangled: RBX::IndexArray<RBX::IndexedTree,&RBX::IndexedTree::getIndex>::fastRemove(RBX::IndexedTree*)
#[doc(alias = "RBX::IndexArray<RBX::IndexedTree,&RBX::IndexedTree::getIndex>::fastRemove(RBX::IndexedTree*)")]
#[doc(alias = "__ZN3RBX10IndexArrayINS_11IndexedTreeEXadL_ZNS1_8getIndexEvEEE10fastRemoveEPS1_")]
pub fn stub_324c14() -> ! {
    todo!("0x324c14 RBX::IndexArray<RBX::IndexedTree,&RBX::IndexedTree::getIndex>::fastRemove(RBX::IndexedTree*)")
}

// 0x324ce8 — __ZN3RBX11IndexedTree15onParentChangedEPS0_
// demangled: RBX::IndexedTree::onParentChanged(RBX::IndexedTree*)
#[doc(alias = "RBX::IndexedTree::onParentChanged(RBX::IndexedTree*)")]
#[doc(alias = "__ZN3RBX11IndexedTree15onParentChangedEPS0_")]
pub fn stub_324ce8() -> ! {
    todo!("0x324ce8 RBX::IndexedTree::onParentChanged(RBX::IndexedTree*)")
}

// 0x324cec — __ZN3G3D5ArrayIPN3RBX11IndexedTreeELi10ELm32EE6appendERKS3_
// demangled: G3D::Array<RBX::IndexedTree *,10,32ul>::append(RBX::IndexedTree * const&)
// type: int __fastcall(unsigned int *, _DWORD *)
#[doc(alias = "G3D::Array<RBX::IndexedTree *,10,32ul>::append(RBX::IndexedTree * const&)")]
#[doc(alias = "__ZN3G3D5ArrayIPN3RBX11IndexedTreeELi10ELm32EE6appendERKS3_")]
pub fn stub_324cec() -> ! {
    todo!("0x324cec G3D::Array<RBX::IndexedTree *,10,32ul>::append(RBX::IndexedTree * const&)")
}

// 0x324d48 — __ZN3G3D5ArrayIPN3RBX11IndexedTreeELi10ELm32EE6resizeEib
// demangled: G3D::Array<RBX::IndexedTree *,10,32ul>::resize(int,bool)
#[doc(alias = "G3D::Array<RBX::IndexedTree *,10,32ul>::resize(int,bool)")]
#[doc(alias = "__ZN3G3D5ArrayIPN3RBX11IndexedTreeELi10ELm32EE6resizeEib")]
pub fn stub_324d48() -> ! {
    todo!("0x324d48 G3D::Array<RBX::IndexedTree *,10,32ul>::resize(int,bool)")
}

// 0x324e00 — __ZN3G3D5ArrayIPN3RBX11IndexedTreeELi10ELm32EE7reallocEi
// demangled: G3D::Array<RBX::IndexedTree *,10,32ul>::realloc(int)
#[doc(alias = "G3D::Array<RBX::IndexedTree *,10,32ul>::realloc(int)")]
#[doc(alias = "__ZN3G3D5ArrayIPN3RBX11IndexedTreeELi10ELm32EE7reallocEi")]
pub fn stub_324e00() -> ! {
    todo!("0x324e00 G3D::Array<RBX::IndexedTree *,10,32ul>::realloc(int)")
}

// 0x324fe8 — __ZN3G3D5ArrayIPN3RBX11IndexedTreeELi10ELm32EED2Ev
// demangled: G3D::Array<RBX::IndexedTree *,10,32ul>::~Array()
#[doc(alias = "G3D::Array<RBX::IndexedTree *,10,32ul>::~Array()")]
#[doc(alias = "__ZN3G3D5ArrayIPN3RBX11IndexedTreeELi10ELm32EED2Ev")]
pub fn stub_324fe8() -> ! {
    todo!("0x324fe8 G3D::Array<RBX::IndexedTree *,10,32ul>::~Array()")
}

// 0x3250bc — __ZN3G3D5ArrayIPN3RBX11IndexedTreeELi10ELm32EEC2Ev
// demangled: G3D::Array<RBX::IndexedTree *,10,32ul>::Array(void)
#[doc(alias = "G3D::Array<RBX::IndexedTree *,10,32ul>::Array(void)")]
#[doc(alias = "__ZN3G3D5ArrayIPN3RBX11IndexedTreeELi10ELm32EEC2Ev")]
pub fn stub_3250bc() -> ! {
    todo!("0x3250bc G3D::Array<RBX::IndexedTree *,10,32ul>::Array(void)")
}

// 0x3251ac — __GLOBAL__I_a_123
// demangled: global constructor keyed to_a_123
#[doc(alias = "global constructor keyed to_a_123")]
#[doc(alias = "__GLOBAL__I_a_123")]
pub fn stub_3251ac() -> ! {
    todo!("0x3251ac global constructor keyed to_a_123")
}

// 0x325278 — __ZN3RBX18InterpolatedCFrame12clearHistoryEv
// demangled: RBX::InterpolatedCFrame::clearHistory(void)
// type: _DWORD __fastcall(RBX::InterpolatedCFrame *__hidden this)
#[doc(alias = "RBX::InterpolatedCFrame::clearHistory(void)")]
#[doc(alias = "__ZN3RBX18InterpolatedCFrame12clearHistoryEv")]
pub fn stub_325278() -> ! {
    todo!("0x325278 RBX::InterpolatedCFrame::clearHistory(void)")
}

// 0x3252f8 — __ZN3RBX18InterpolatedCFrame8setValueEPNS_12PartInstanceERKN3G3D15CoordinateFrameERKNS_10RemoteTimeE
// demangled: RBX::InterpolatedCFrame::setValue(RBX::PartInstance *,G3D::CoordinateFrame const&,RBX::RemoteTime const&)
#[doc(alias = "RBX::InterpolatedCFrame::setValue(RBX::PartInstance *,G3D::CoordinateFrame const&,RBX::RemoteTime const&)")]
#[doc(alias = "__ZN3RBX18InterpolatedCFrame8setValueEPNS_12PartInstanceERKN3G3D15CoordinateFrameERKNS_10RemoteTimeE")]
pub fn stub_3252f8() -> ! {
    todo!("0x3252f8 RBX::InterpolatedCFrame::setValue(RBX::PartInstance *,G3D::CoordinateFrame const&,RBX::RemoteTime const&)")
}

// 0x325538 — __ZN3RBX18InterpolatedCFrame11interpolateERKNS_4TimeES3_RKj
// demangled: RBX::InterpolatedCFrame::interpolate(RBX::Time const&,RBX::Time const&,unsigned int const&)
// type: _DWORD __fastcall(RBX::InterpolatedCFrame *__hidden this, const RBX::Time *, const RBX::Time *, const unsigned int *)
#[doc(alias = "RBX::InterpolatedCFrame::interpolate(RBX::Time const&,RBX::Time const&,unsigned int const&)")]
#[doc(alias = "__ZN3RBX18InterpolatedCFrame11interpolateERKNS_4TimeES3_RKj")]
pub fn stub_325538() -> ! {
    todo!("0x325538 RBX::InterpolatedCFrame::interpolate(RBX::Time const&,RBX::Time const&,unsigned int const&)")
}

// 0x3258e4 — __ZN3RBX18InterpolatedCFrame23computeSampleTargetTimeERKNS_4TimeE
// demangled: RBX::InterpolatedCFrame::computeSampleTargetTime(RBX::Time const&)
// type: _DWORD __fastcall(RBX::InterpolatedCFrame *__hidden this, const Time *)
#[doc(alias = "RBX::InterpolatedCFrame::computeSampleTargetTime(RBX::Time const&)")]
#[doc(alias = "__ZN3RBX18InterpolatedCFrame23computeSampleTargetTimeERKNS_4TimeE")]
pub fn stub_3258e4() -> ! {
    todo!("0x3258e4 RBX::InterpolatedCFrame::computeSampleTargetTime(RBX::Time const&)")
}

// 0x325998 — __ZN3RBX18InterpolatedCFrame12computeValueEPNS_12PartInstanceE
// demangled: RBX::InterpolatedCFrame::computeValue(RBX::PartInstance *)
// type: _DWORD __fastcall(RBX::InterpolatedCFrame *__hidden this, RBX::PartInstance *)
#[doc(alias = "RBX::InterpolatedCFrame::computeValue(RBX::PartInstance *)")]
#[doc(alias = "__ZN3RBX18InterpolatedCFrame12computeValueEPNS_12PartInstanceE")]
pub fn stub_325998() -> ! {
    todo!("0x325998 RBX::InterpolatedCFrame::computeValue(RBX::PartInstance *)")
}

// 0x325b08 — __ZN3RBX18InterpolatedCFrame16setRenderedFrameERKN3G3D15CoordinateFrameE
// demangled: RBX::InterpolatedCFrame::setRenderedFrame(G3D::CoordinateFrame const&)
// type: void __fastcall(RBX::InterpolatedCFrame *this, const G3D::CoordinateFrame *)
#[doc(alias = "RBX::InterpolatedCFrame::setRenderedFrame(G3D::CoordinateFrame const&)")]
#[doc(alias = "__ZN3RBX18InterpolatedCFrame16setRenderedFrameERKN3G3D15CoordinateFrameE")]
pub fn stub_325b08() -> ! {
    todo!("0x325b08 RBX::InterpolatedCFrame::setRenderedFrame(G3D::CoordinateFrame const&)")
}

// 0x325b4c — __ZN3RBX18InterpolatedCFrame16setRenderedFrameERKN3G3D15CoordinateFrameERKNS_10RemoteTimeE
// demangled: RBX::InterpolatedCFrame::setRenderedFrame(G3D::CoordinateFrame const&,RBX::RemoteTime const&)
#[doc(alias = "RBX::InterpolatedCFrame::setRenderedFrame(G3D::CoordinateFrame const&,RBX::RemoteTime const&)")]
#[doc(alias = "__ZN3RBX18InterpolatedCFrame16setRenderedFrameERKN3G3D15CoordinateFrameERKNS_10RemoteTimeE")]
pub fn stub_325b4c() -> ! {
    todo!("0x325b4c RBX::InterpolatedCFrame::setRenderedFrame(G3D::CoordinateFrame const&,RBX::RemoteTime const&)")
}

// 0x325b98 — __ZNK3RBX18InterpolatedCFrame17getSampleIntervalEv
// demangled: RBX::InterpolatedCFrame::getSampleInterval(void)const
// type: _DWORD __fastcall(RBX::InterpolatedCFrame *__hidden this)
#[doc(alias = "RBX::InterpolatedCFrame::getSampleInterval(void)const")]
#[doc(alias = "__ZNK3RBX18InterpolatedCFrame17getSampleIntervalEv")]
pub fn stub_325b98() -> ! {
    todo!("0x325b98 RBX::InterpolatedCFrame::getSampleInterval(void)const")
}

// 0x325bb4 — __ZN5boost31circular_buffer_space_optimizedIN3RBX18InterpolatedCFrame9FrameInfoESaIS3_EE5clearEv
// demangled: boost::circular_buffer_space_optimized<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::clear(void)
#[doc(alias = "boost::circular_buffer_space_optimized<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::clear(void)")]
#[doc(alias = "__ZN5boost31circular_buffer_space_optimizedIN3RBX18InterpolatedCFrame9FrameInfoESaIS3_EE5clearEv")]
pub fn stub_325bb4() -> ! {
    todo!("0x325bb4 boost::circular_buffer_space_optimized<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::clear(void)")
}

// 0x325be0 — __ZN5boost31circular_buffer_space_optimizedIN3RBX18InterpolatedCFrame9FrameInfoESaIS3_EE18check_low_capacityEm
// demangled: boost::circular_buffer_space_optimized<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::check_low_capacity(unsigned long)
#[doc(alias = "boost::circular_buffer_space_optimized<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::check_low_capacity(unsigned long)")]
#[doc(alias = "__ZN5boost31circular_buffer_space_optimizedIN3RBX18InterpolatedCFrame9FrameInfoESaIS3_EE18check_low_capacityEm")]
pub fn stub_325be0() -> ! {
    todo!("0x325be0 boost::circular_buffer_space_optimized<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::check_low_capacity(unsigned long)")
}

// 0x325c38 — __ZN5boost15circular_bufferIN3RBX18InterpolatedCFrame9FrameInfoESaIS3_EE9push_backERKS3_
// demangled: boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::push_back(RBX::InterpolatedCFrame::FrameInfo const&)
// type: int __fastcall(int, G3D::Matrix3 *)
#[doc(alias = "boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::push_back(RBX::InterpolatedCFrame::FrameInfo const&)")]
#[doc(alias = "__ZN5boost15circular_bufferIN3RBX18InterpolatedCFrame9FrameInfoESaIS3_EE9push_backERKS3_")]
pub fn stub_325c38() -> ! {
    todo!("0x325c38 boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::push_back(RBX::InterpolatedCFrame::FrameInfo const&)")
}

// 0x325ce4 — __ZN5boost15circular_bufferIN3RBX18InterpolatedCFrame9FrameInfoESaIS3_EE12set_capacityEm
// demangled: boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::set_capacity(unsigned long)
#[doc(alias = "boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::set_capacity(unsigned long)")]
#[doc(alias = "__ZN5boost15circular_bufferIN3RBX18InterpolatedCFrame9FrameInfoESaIS3_EE12set_capacityEm")]
pub fn stub_325ce4() -> ! {
    todo!("0x325ce4 boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::set_capacity(unsigned long)")
}

// 0x325e8c — __ZN5boost10cb_details8iteratorINS_15circular_bufferIN3RBX18InterpolatedCFrame9FrameInfoESaIS5_EEENS0_15nonconst_traitsIS6_EEEpLEi
// demangled: boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>::operator+=(int)
#[doc(alias = "boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>::operator+=(int)")]
#[doc(alias = "__ZN5boost10cb_details8iteratorINS_15circular_bufferIN3RBX18InterpolatedCFrame9FrameInfoESaIS5_EEENS0_15nonconst_traitsIS6_EEEpLEi")]
pub fn stub_325e8c() -> ! {
    todo!("0x325e8c boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>::operator+=(int)")
}

// 0x325ed4 — __ZN5boost10cb_details8iteratorINS_15circular_bufferIN3RBX18InterpolatedCFrame9FrameInfoESaIS5_EEENS0_15nonconst_traitsIS6_EEEmIEi
// demangled: boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>::operator-=(int)
#[doc(alias = "boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>::operator-=(int)")]
#[doc(alias = "__ZN5boost10cb_details8iteratorINS_15circular_bufferIN3RBX18InterpolatedCFrame9FrameInfoESaIS5_EEENS0_15nonconst_traitsIS6_EEEmIEi")]
pub fn stub_325ed4() -> ! {
    todo!("0x325ed4 boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>::operator-=(int)")
}

// 0x325f14 — __ZN5boost31circular_buffer_space_optimizedIN3RBX18InterpolatedCFrame9FrameInfoESaIS3_EE5eraseENS_10cb_details8iteratorINS_15circular_bufferIS3_S4_EENS6_15nonconst_traitsIS4_EEEESC_
// demangled: boost::circular_buffer_space_optimized<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::erase(boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>,boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>)
#[doc(alias = "boost::circular_buffer_space_optimized<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::erase(boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>,boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>)")]
#[doc(alias = "__ZN5boost31circular_buffer_space_optimizedIN3RBX18InterpolatedCFrame9FrameInfoESaIS3_EE5eraseENS_10cb_details8iteratorINS_15circular_bufferIS3_S4_EENS6_15nonconst_traitsIS4_EEEESC_")]
pub fn stub_325f14() -> ! {
    todo!("0x325f14 boost::circular_buffer_space_optimized<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::erase(boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>,boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>)")
}

// 0x325f94 — __ZN5boost15circular_bufferIN3RBX18InterpolatedCFrame9FrameInfoESaIS3_EE5eraseENS_10cb_details8iteratorIS5_NS6_15nonconst_traitsIS4_EEEESA_
// demangled: boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::erase(boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>,boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>)
#[doc(alias = "boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::erase(boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>,boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>)")]
#[doc(alias = "__ZN5boost15circular_bufferIN3RBX18InterpolatedCFrame9FrameInfoESaIS3_EE5eraseENS_10cb_details8iteratorIS5_NS6_15nonconst_traitsIS4_EEEESA_")]
pub fn stub_325f94() -> ! {
    todo!("0x325f94 boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::erase(boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>,boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>)")
}

// 0x32607c — __ZN5boost31circular_buffer_space_optimizedIN3RBX18InterpolatedCFrame9FrameInfoESaIS3_EE19check_high_capacityEv
// demangled: boost::circular_buffer_space_optimized<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::check_high_capacity(void)
#[doc(alias = "boost::circular_buffer_space_optimized<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::check_high_capacity(void)")]
#[doc(alias = "__ZN5boost31circular_buffer_space_optimizedIN3RBX18InterpolatedCFrame9FrameInfoESaIS3_EE19check_high_capacityEv")]
pub fn stub_32607c() -> ! {
    todo!("0x32607c boost::circular_buffer_space_optimized<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::check_high_capacity(void)")
}

// 0x3260d8 — __ZNK5boost10cb_details8iteratorINS_15circular_bufferIN3RBX18InterpolatedCFrame9FrameInfoESaIS5_EEENS0_15nonconst_traitsIS6_EEE17linearize_pointerIS9_EENT_7pointerERKNS1_IS7_SC_EE
// demangled: boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::pointer boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>::linearize_pointer<boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>(boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::pointer> const&)const
// type: int __fastcall(_DWORD **, int)
#[doc(alias = "boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::pointer boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>::linearize_pointer<boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>(boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::pointer> const&)const")]
#[doc(alias = "__ZNK5boost10cb_details8iteratorINS_15circular_bufferIN3RBX18InterpolatedCFrame9FrameInfoESaIS5_EEENS0_15nonconst_traitsIS6_EEE17linearize_pointerIS9_EENT_7pointerERKNS1_IS7_SC_EE")]
pub fn stub_3260d8() -> ! {
    todo!("0x3260d8 boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::pointer boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>::linearize_pointer<boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>>(boost::cb_details::iterator<boost::circular_buffer<RBX::InterpolatedCFrame::FrameInfo,std::allocator<RBX::InterpolatedCFrame::FrameInfo>>,boost::cb_details::nonconst_traits<std::allocator<RBX::InterpolatedCFrame::FrameInfo>>::pointer> const&)const")
}

// 0x326108 — __GLOBAL__I_a_124
// demangled: global constructor keyed to_a_124
#[doc(alias = "global constructor keyed to_a_124")]
#[doc(alias = "__GLOBAL__I_a_124")]
pub fn stub_326108() -> ! {
    todo!("0x326108 global constructor keyed to_a_124")
}

// 0x326378 — __ZN3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEEC1Ev
// demangled: RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::EnumDesc(void)
// type: int(void)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::EnumDesc(void)")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEEC1Ev")]
pub fn stub_326378() -> ! {
    todo!("0x326378 RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::EnumDesc(void)")
}

// 0x32637c — __ZN3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEEC2Ev
// demangled: RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::EnumDesc(void)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::EnumDesc(void)")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEEC2Ev")]
pub fn stub_32637c() -> ! {
    todo!("0x32637c RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::EnumDesc(void)")
}

// 0x32653c — __ZN3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE7addPairES2_PKc
// demangled: RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::addPair(RBX::KeywordFilterType,char const*)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::addPair(RBX::KeywordFilterType,char const*)")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_17KeywordFilterTypeEE7addPairES2_PKc")]
pub fn stub_32653c() -> ! {
    todo!("0x32653c RBX::Reflection::EnumDesc<RBX::KeywordFilterType>::addPair(RBX::KeywordFilterType,char const*)")
}

// 0x32689c — __ZNSt6vectorIN3RBX17KeywordFilterTypeESaIS1_EE6resizeEmS1_
// demangled: std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::resize(unsigned long,RBX::KeywordFilterType)
#[doc(alias = "std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::resize(unsigned long,RBX::KeywordFilterType)")]
#[doc(alias = "__ZNSt6vectorIN3RBX17KeywordFilterTypeESaIS1_EE6resizeEmS1_")]
pub fn stub_32689c() -> ! {
    todo!("0x32689c std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::resize(unsigned long,RBX::KeywordFilterType)")
}

// 0x3268d0 — __ZNSt6vectorIN3RBX17KeywordFilterTypeESaIS1_EE9push_backERKS1_
// demangled: std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::push_back(RBX::KeywordFilterType const&)
#[doc(alias = "std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::push_back(RBX::KeywordFilterType const&)")]
#[doc(alias = "__ZNSt6vectorIN3RBX17KeywordFilterTypeESaIS1_EE9push_backERKS1_")]
pub fn stub_3268d0() -> ! {
    todo!("0x3268d0 std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::push_back(RBX::KeywordFilterType const&)")
}

// 0x3268f8 — __ZNSt3mapIPKN3RBX4NameENS0_17KeywordFilterTypeESt4lessIS3_ESaISt4pairIKS3_S4_EEEixERS8_
// demangled: std::map<RBX::Name const*,RBX::KeywordFilterType,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>>::operator[](RBX::Name const* const&)
#[doc(alias = "std::map<RBX::Name const*,RBX::KeywordFilterType,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>>::operator[](RBX::Name const* const&)")]
#[doc(alias = "__ZNSt3mapIPKN3RBX4NameENS0_17KeywordFilterTypeESt4lessIS3_ESaISt4pairIKS3_S4_EEEixERS8_")]
pub fn stub_3268f8() -> ! {
    todo!("0x3268f8 std::map<RBX::Name const*,RBX::KeywordFilterType,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>>::operator[](RBX::Name const* const&)")
}

// 0x326950 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_17KeywordFilterTypeEESt10_Select1stIS7_ESt4lessIS3_ESaIS7_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS7_ERKS7_
// demangled: std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::KeywordFilterType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>,std::pair<RBX::Name const* const,RBX::KeywordFilterType> const&)
// type: int __fastcall(int, _Rb_tree_node_base *)
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::KeywordFilterType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>,std::pair<RBX::Name const* const,RBX::KeywordFilterType> const&)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_17KeywordFilterTypeEESt10_Select1stIS7_ESt4lessIS3_ESaIS7_EE16_M_insert_uniqueESt17_Rb_tree_iteratorIS7_ERKS7_")]
pub fn stub_326950() -> ! {
    todo!("0x326950 std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::KeywordFilterType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>>::_M_insert_unique(std::_Rb_tree_iterator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>,std::pair<RBX::Name const* const,RBX::KeywordFilterType> const&)")
}

// 0x326a04 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_17KeywordFilterTypeEESt10_Select1stIS7_ESt4lessIS3_ESaIS7_EE9_M_insertEPSt18_Rb_tree_node_baseSF_RKS7_
// demangled: std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::KeywordFilterType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<RBX::Name const* const,RBX::KeywordFilterType> const&)
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::KeywordFilterType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<RBX::Name const* const,RBX::KeywordFilterType> const&)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_17KeywordFilterTypeEESt10_Select1stIS7_ESt4lessIS3_ESaIS7_EE9_M_insertEPSt18_Rb_tree_node_baseSF_RKS7_")]
pub fn stub_326a04() -> ! {
    todo!("0x326a04 std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::KeywordFilterType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>>::_M_insert(std::_Rb_tree_node_base *,std::_Rb_tree_node_base *,std::pair<RBX::Name const* const,RBX::KeywordFilterType> const&)")
}

// 0x326a5c — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_17KeywordFilterTypeEESt10_Select1stIS7_ESt4lessIS3_ESaIS7_EE16_M_insert_uniqueERKS7_
// demangled: std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::KeywordFilterType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>>::_M_insert_unique(std::pair<RBX::Name const* const,RBX::KeywordFilterType> const&)
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::KeywordFilterType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>>::_M_insert_unique(std::pair<RBX::Name const* const,RBX::KeywordFilterType> const&)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_17KeywordFilterTypeEESt10_Select1stIS7_ESt4lessIS3_ESaIS7_EE16_M_insert_uniqueERKS7_")]
pub fn stub_326a5c() -> ! {
    todo!("0x326a5c std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::KeywordFilterType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::KeywordFilterType>>>::_M_insert_unique(std::pair<RBX::Name const* const,RBX::KeywordFilterType> const&)")
}

// 0x326ac4 — __ZNSt6vectorIN3RBX17KeywordFilterTypeESaIS1_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS1_S3_EERKS1_
// demangled: std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::_M_insert_aux(__gnu_cxx::__normal_iterator<RBX::KeywordFilterType*,std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>>,RBX::KeywordFilterType const&)
// type: int __fastcall(int, char *, _DWORD *)
#[doc(alias = "std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::_M_insert_aux(__gnu_cxx::__normal_iterator<RBX::KeywordFilterType*,std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>>,RBX::KeywordFilterType const&)")]
#[doc(alias = "__ZNSt6vectorIN3RBX17KeywordFilterTypeESaIS1_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS1_S3_EERKS1_")]
pub fn stub_326ac4() -> ! {
    todo!("0x326ac4 std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::_M_insert_aux(__gnu_cxx::__normal_iterator<RBX::KeywordFilterType*,std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>>,RBX::KeywordFilterType const&)")
}

// 0x326ba8 — __ZNSt12_Vector_baseIN3RBX17KeywordFilterTypeESaIS1_EE11_M_allocateEm
// demangled: std::_Vector_base<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::_M_allocate(unsigned long)
#[doc(alias = "std::_Vector_base<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::_M_allocate(unsigned long)")]
#[doc(alias = "__ZNSt12_Vector_baseIN3RBX17KeywordFilterTypeESaIS1_EE11_M_allocateEm")]
pub fn stub_326ba8() -> ! {
    todo!("0x326ba8 std::_Vector_base<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::_M_allocate(unsigned long)")
}

// 0x326bc0 — __ZNSt15__copy_backwardILb0ESt26random_access_iterator_tagE8__copy_bIPN3RBX17KeywordFilterTypeES5_EET0_T_S7_S6_
// demangled: RBX::KeywordFilterType * std::__copy_backward<false,std::random_access_iterator_tag>::__copy_b<RBX::KeywordFilterType *,RBX::KeywordFilterType *>(RBX::KeywordFilterType *,RBX::KeywordFilterType *,RBX::KeywordFilterType *)
#[doc(alias = "RBX::KeywordFilterType * std::__copy_backward<false,std::random_access_iterator_tag>::__copy_b<RBX::KeywordFilterType *,RBX::KeywordFilterType *>(RBX::KeywordFilterType *,RBX::KeywordFilterType *,RBX::KeywordFilterType *)")]
#[doc(alias = "__ZNSt15__copy_backwardILb0ESt26random_access_iterator_tagE8__copy_bIPN3RBX17KeywordFilterTypeES5_EET0_T_S7_S6_")]
pub fn stub_326bc0() -> ! {
    todo!("0x326bc0 RBX::KeywordFilterType * std::__copy_backward<false,std::random_access_iterator_tag>::__copy_b<RBX::KeywordFilterType *,RBX::KeywordFilterType *>(RBX::KeywordFilterType *,RBX::KeywordFilterType *,RBX::KeywordFilterType *)")
}

// 0x326bfc — __ZNSt6vectorIN3RBX17KeywordFilterTypeESaIS1_EE14_M_fill_insertEN9__gnu_cxx17__normal_iteratorIPS1_S3_EEmRKS1_
// demangled: std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::_M_fill_insert(__gnu_cxx::__normal_iterator<RBX::KeywordFilterType*,std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>>,unsigned long,RBX::KeywordFilterType const&)
#[doc(alias = "std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::_M_fill_insert(__gnu_cxx::__normal_iterator<RBX::KeywordFilterType*,std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>>,unsigned long,RBX::KeywordFilterType const&)")]
#[doc(alias = "__ZNSt6vectorIN3RBX17KeywordFilterTypeESaIS1_EE14_M_fill_insertEN9__gnu_cxx17__normal_iteratorIPS1_S3_EEmRKS1_")]
pub fn stub_326bfc() -> ! {
    todo!("0x326bfc std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>::_M_fill_insert(__gnu_cxx::__normal_iterator<RBX::KeywordFilterType*,std::vector<RBX::KeywordFilterType,std::allocator<RBX::KeywordFilterType>>>,unsigned long,RBX::KeywordFilterType const&)")
}

// 0x326d8c — __GLOBAL__I_a_125
// demangled: global constructor keyed to_a_125
#[doc(alias = "global constructor keyed to_a_125")]
#[doc(alias = "__GLOBAL__I_a_125")]
pub fn stub_326d8c() -> ! {
    todo!("0x326d8c global constructor keyed to_a_125")
}

// 0x326e54 — __ZN3RBX18LegacyContentTableC1Ev
// demangled: RBX::LegacyContentTable::LegacyContentTable(void)
// type: _DWORD __fastcall(RBX::LegacyContentTable *__hidden this)
#[doc(alias = "RBX::LegacyContentTable::LegacyContentTable(void)")]
#[doc(alias = "__ZN3RBX18LegacyContentTableC1Ev")]
pub fn stub_326e54() -> ! {
    todo!("0x326e54 RBX::LegacyContentTable::LegacyContentTable(void)")
}

// 0x326e58 — __ZN3RBX18LegacyContentTableC2Ev
// demangled: RBX::LegacyContentTable::LegacyContentTable(void)
// type: _DWORD __fastcall(RBX::LegacyContentTable *__hidden this)
#[doc(alias = "RBX::LegacyContentTable::LegacyContentTable(void)")]
#[doc(alias = "__ZN3RBX18LegacyContentTableC2Ev")]
pub fn stub_326e58() -> ! {
    todo!("0x326e58 RBX::LegacyContentTable::LegacyContentTable(void)")
}

//! audio generated_audio_wdcron_E — 120 stubs EA-sorted asc gap filler not yet in audio (FMOD|Soundscape exhausted, global gap filler distinct per crate)
//! Source: ida/export.json (85545 funcs) EA-sorted asc next 120 not in audio after 0x688b07
//! Range 0x688b08..0x68ced0 | existing 37842 -> 37962 distinct
//! Batch: 120 stubs | // 0xADDR — mangled + #[doc(alias = "demangled")] + todo!("0xADDR mangled")


#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]
use rbx_core::SharedPtr;
const _: () = { let _ = core::marker::PhantomData::<SharedPtr<u8>>; };


// 0x688b08 — __ZN3RBX16ToolMouseCommandC2EPNS_9WorkspaceEPNS_4ToolE
// demangled: RBX::ToolMouseCommand::ToolMouseCommand(RBX::Workspace *,RBX::Tool *)
// type: _DWORD __fastcall(RBX::ToolMouseCommand *__hidden this, RBX::Workspace *, RBX::Tool *)
#[doc(alias = "RBX::ToolMouseCommand::ToolMouseCommand(RBX::Workspace *,RBX::Tool *)")]
#[doc(alias = "__ZN3RBX16ToolMouseCommandC2EPNS_9WorkspaceEPNS_4ToolE")]
pub fn stub_688b08() -> ! {
    todo!("0x688b08 __ZN3RBX16ToolMouseCommandC2EPNS_9WorkspaceEPNS_4ToolE")
}

// 0x688cdc — __ZN3RBX16ToolMouseCommand22onEvent_ToolUnequippedEv
// demangled: RBX::ToolMouseCommand::onEvent_ToolUnequipped(void)
// type: _DWORD __fastcall(RBX::ToolMouseCommand *__hidden this)
#[doc(alias = "RBX::ToolMouseCommand::onEvent_ToolUnequipped(void)")]
#[doc(alias = "__ZN3RBX16ToolMouseCommand22onEvent_ToolUnequippedEv")]
pub fn stub_688cdc() -> ! {
    todo!("0x688cdc __ZN3RBX16ToolMouseCommand22onEvent_ToolUnequippedEv")
}

// 0x688ce4 — __ZN3RBX16ToolMouseCommand17updateTargetPointERKNS_7UIEventEb
// demangled: RBX::ToolMouseCommand::updateTargetPoint(RBX::UIEvent const&,bool)
// type: _DWORD __fastcall(RBX::ToolMouseCommand *__hidden this, const RBX::UIEvent *, bool)
#[doc(alias = "RBX::ToolMouseCommand::updateTargetPoint(RBX::UIEvent const&,bool)")]
#[doc(alias = "__ZN3RBX16ToolMouseCommand17updateTargetPointERKNS_7UIEventEb")]
pub fn stub_688ce4() -> ! {
    todo!("0x688ce4 __ZN3RBX16ToolMouseCommand17updateTargetPointERKNS_7UIEventEb")
}

// 0x688d94 — __ZN3RBX16ToolMouseCommand16onRightMouseDownERKNS_7UIEventE
// demangled: RBX::ToolMouseCommand::onRightMouseDown(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::ToolMouseCommand *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::ToolMouseCommand::onRightMouseDown(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX16ToolMouseCommand16onRightMouseDownERKNS_7UIEventE")]
pub fn stub_688d94() -> ! {
    todo!("0x688d94 __ZN3RBX16ToolMouseCommand16onRightMouseDownERKNS_7UIEventE")
}

// 0x688da0 — __ZN3RBX16ToolMouseCommand11onMouseDownERKNS_7UIEventE
// demangled: RBX::ToolMouseCommand::onMouseDown(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::ToolMouseCommand *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::ToolMouseCommand::onMouseDown(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX16ToolMouseCommand11onMouseDownERKNS_7UIEventE")]
pub fn stub_688da0() -> ! {
    todo!("0x688da0 __ZN3RBX16ToolMouseCommand11onMouseDownERKNS_7UIEventE")
}

// 0x688f6c — __ZN3RBX16ToolMouseCommand12tryClickableERKNS_7UIEventEN5boost10shared_ptrINS_12PartInstanceEEE
// demangled: RBX::ToolMouseCommand::tryClickable(RBX::UIEvent const&,boost::shared_ptr<RBX::PartInstance>)
// type: int __fastcall(RBX::Network::Players **, RBX::Instance *, int, int, int, boost::detail::sp_counted_base *, int, int, int, int)
#[doc(alias = "RBX::ToolMouseCommand::tryClickable(RBX::UIEvent const&,boost::shared_ptr<RBX::PartInstance>)")]
#[doc(alias = "__ZN3RBX16ToolMouseCommand12tryClickableERKNS_7UIEventEN5boost10shared_ptrINS_12PartInstanceEEE")]
pub fn stub_688f6c() -> ! {
    todo!("0x688f6c __ZN3RBX16ToolMouseCommand12tryClickableERKNS_7UIEventEN5boost10shared_ptrINS_12PartInstanceEEE")
}

// 0x689080 — __ZN3RBX16ToolMouseCommand12onMouseHoverERKNS_7UIEventE
// demangled: RBX::ToolMouseCommand::onMouseHover(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::ToolMouseCommand *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::ToolMouseCommand::onMouseHover(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX16ToolMouseCommand12onMouseHoverERKNS_7UIEventE")]
pub fn stub_689080() -> ! {
    todo!("0x689080 __ZN3RBX16ToolMouseCommand12onMouseHoverERKNS_7UIEventE")
}

// 0x689198 — __ZN3RBX16ToolMouseCommand11onMouseIdleERKNS_7UIEventE
// demangled: RBX::ToolMouseCommand::onMouseIdle(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::ToolMouseCommand *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::ToolMouseCommand::onMouseIdle(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX16ToolMouseCommand11onMouseIdleERKNS_7UIEventE")]
pub fn stub_689198() -> ! {
    todo!("0x689198 __ZN3RBX16ToolMouseCommand11onMouseIdleERKNS_7UIEventE")
}

// 0x6891b4 — __ZN3RBX16ToolMouseCommand14onRightMouseUpERKNS_7UIEventE
// demangled: RBX::ToolMouseCommand::onRightMouseUp(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::ToolMouseCommand *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::ToolMouseCommand::onRightMouseUp(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX16ToolMouseCommand14onRightMouseUpERKNS_7UIEventE")]
pub fn stub_6891b4() -> ! {
    todo!("0x6891b4 __ZN3RBX16ToolMouseCommand14onRightMouseUpERKNS_7UIEventE")
}

// 0x6891c0 — __ZN3RBX16ToolMouseCommand9onMouseUpERKNS_7UIEventE
// demangled: RBX::ToolMouseCommand::onMouseUp(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::ToolMouseCommand *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::ToolMouseCommand::onMouseUp(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX16ToolMouseCommand9onMouseUpERKNS_7UIEventE")]
pub fn stub_6891c0() -> ! {
    todo!("0x6891c0 __ZN3RBX16ToolMouseCommand9onMouseUpERKNS_7UIEventE")
}

// 0x6891ec — __ZN3rbx7signals6signalIFvvEE7connectIN5boost3_bi6bind_tIvNS5_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS6_5list1INS6_5valueIPSB_EEEEEEEENS0_10connectionERKT_
// demangled: rbx::signals::connection rbx::signals::signal<void ()(void)>::connect<boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>>(boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>> const&)
#[doc(alias = "rbx::signals::connection rbx::signals::signal<void ()(void)>::connect<boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>>(boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>> const&)")]
#[doc(alias = "__ZN3rbx7signals6signalIFvvEE7connectIN5boost3_bi6bind_tIvNS5_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS6_5list1INS6_5valueIPSB_EEEEEEEENS0_10connectionERKT_")]
pub fn stub_6891ec() -> ! {
    todo!("0x6891ec __ZN3rbx7signals6signalIFvvEE7connectIN5boost3_bi6bind_tIvNS5_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS6_5list1INS6_5valueIPSB_EEEEEEEENS0_10connectionERKT_")
}

// 0x689260 — __ZNK3RBX5NamedINS_18ScriptMouseCommandELZNS_17sToolMouseCommandEEE7getNameEv
// demangled: __ZNK3RBX5NamedINS_18ScriptMouseCommandELZNS_17sToolMouseCommandEEE7getNameEv
#[doc(alias = "__ZNK3RBX5NamedINS_18ScriptMouseCommandELZNS_17sToolMouseCommandEEE7getNameEv")]
pub fn stub_689260() -> ! {
    todo!("0x689260 __ZNK3RBX5NamedINS_18ScriptMouseCommandELZNS_17sToolMouseCommandEEE7getNameEv")
}

// 0x689288 — __ZN3RBX16ToolMouseCommandD1Ev
// demangled: RBX::ToolMouseCommand::~ToolMouseCommand()
// type: void __fastcall(RBX::ToolMouseCommand *__hidden this)
#[doc(alias = "RBX::ToolMouseCommand::~ToolMouseCommand()")]
#[doc(alias = "__ZN3RBX16ToolMouseCommandD1Ev")]
pub fn stub_689288() -> ! {
    todo!("0x689288 __ZN3RBX16ToolMouseCommandD1Ev")
}

// 0x68928c — __ZN3RBX16ToolMouseCommandD0Ev
// demangled: RBX::ToolMouseCommand::~ToolMouseCommand()
// type: void __fastcall(RBX::ToolMouseCommand *__hidden this)
#[doc(alias = "RBX::ToolMouseCommand::~ToolMouseCommand()")]
#[doc(alias = "__ZN3RBX16ToolMouseCommandD0Ev")]
pub fn stub_68928c() -> ! {
    todo!("0x68928c __ZN3RBX16ToolMouseCommandD0Ev")
}

// 0x68932c — __ZThn36_N3RBX16ToolMouseCommandD1Ev
// demangled: `non-virtual thunk to'RBX::ToolMouseCommand::~ToolMouseCommand()
// type: void __fastcall(RBX::ToolMouseCommand *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::ToolMouseCommand::~ToolMouseCommand()")]
#[doc(alias = "__ZThn36_N3RBX16ToolMouseCommandD1Ev")]
pub fn stub_68932c() -> ! {
    todo!("0x68932c __ZThn36_N3RBX16ToolMouseCommandD1Ev")
}

// 0x689334 — __ZThn36_N3RBX16ToolMouseCommandD0Ev
// demangled: `non-virtual thunk to'RBX::ToolMouseCommand::~ToolMouseCommand()
// type: void __fastcall(RBX::ToolMouseCommand *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::ToolMouseCommand::~ToolMouseCommand()")]
#[doc(alias = "__ZThn36_N3RBX16ToolMouseCommandD0Ev")]
pub fn stub_689334() -> ! {
    todo!("0x689334 __ZThn36_N3RBX16ToolMouseCommandD0Ev")
}

// 0x689420 — __ZN3rbx7signals6signalIFvvEE13callable_slotIN5boost3_bi6bind_tIvNS5_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS6_5list1INS6_5valueIPSB_EEEEEEED1Ev
// demangled: rbx::signals::signal<void ()(void)>::callable_slot<boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>>::~callable_slot()
#[doc(alias = "rbx::signals::signal<void ()(void)>::callable_slot<boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>>::~callable_slot()")]
#[doc(alias = "__ZN3rbx7signals6signalIFvvEE13callable_slotIN5boost3_bi6bind_tIvNS5_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS6_5list1INS6_5valueIPSB_EEEEEEED1Ev")]
pub fn stub_689420() -> ! {
    todo!("0x689420 __ZN3rbx7signals6signalIFvvEE13callable_slotIN5boost3_bi6bind_tIvNS5_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS6_5list1INS6_5valueIPSB_EEEEEEED1Ev")
}

// 0x68944c — __ZN3rbx7signals6signalIFvvEE13callable_slotIN5boost3_bi6bind_tIvNS5_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS6_5list1INS6_5valueIPSB_EEEEEEED0Ev
// demangled: rbx::signals::signal<void ()(void)>::callable_slot<boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>>::~callable_slot()
// type: void __fastcall(_DWORD *)
#[doc(alias = "rbx::signals::signal<void ()(void)>::callable_slot<boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>>::~callable_slot()")]
#[doc(alias = "__ZN3rbx7signals6signalIFvvEE13callable_slotIN5boost3_bi6bind_tIvNS5_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS6_5list1INS6_5valueIPSB_EEEEEEED0Ev")]
pub fn stub_68944c() -> ! {
    todo!("0x68944c __ZN3rbx7signals6signalIFvvEE13callable_slotIN5boost3_bi6bind_tIvNS5_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS6_5list1INS6_5valueIPSB_EEEEEEED0Ev")
}

// 0x689520 — __ZN3rbx8callableINS_7signals6signalIFvvEE4slotEN5boost3_bi6bind_tIvNS6_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS7_5list1INS7_5valueIPSC_EEEEEELi0ES3_E4callEv
// demangled: rbx::callable<rbx::signals::signal<void ()(void)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>,0,void ()(void)>::call(void)
// type: int()
#[doc(alias = "rbx::callable<rbx::signals::signal<void ()(void)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>,0,void ()(void)>::call(void)")]
#[doc(alias = "__ZN3rbx8callableINS_7signals6signalIFvvEE4slotEN5boost3_bi6bind_tIvNS6_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS7_5list1INS7_5valueIPSC_EEEEEELi0ES3_E4callEv")]
pub fn stub_689520() -> ! {
    todo!("0x689520 __ZN3rbx8callableINS_7signals6signalIFvvEE4slotEN5boost3_bi6bind_tIvNS6_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS7_5list1INS7_5valueIPSC_EEEEEELi0ES3_E4callEv")
}

// 0x689528 — __ZThn4_N3rbx8callableINS_7signals6signalIFvvEE4slotEN5boost3_bi6bind_tIvNS6_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS7_5list1INS7_5valueIPSC_EEEEEELi0ES3_E4callEv
// demangled: `non-virtual thunk to'rbx::callable<rbx::signals::signal<void ()(void)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>,0,void ()(void)>::call(void)
#[doc(alias = "non-virtual thunk torbx::callable<rbx::signals::signal<void ()(void)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>,0,void ()(void)>::call(void)")]
#[doc(alias = "__ZThn4_N3rbx8callableINS_7signals6signalIFvvEE4slotEN5boost3_bi6bind_tIvNS6_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS7_5list1INS7_5valueIPSC_EEEEEELi0ES3_E4callEv")]
pub fn stub_689528() -> ! {
    todo!("0x689528 __ZThn4_N3rbx8callableINS_7signals6signalIFvvEE4slotEN5boost3_bi6bind_tIvNS6_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS7_5list1INS7_5valueIPSC_EEEEEELi0ES3_E4callEv")
}

// 0x689530 — __ZN5boost3_bi6bind_tIvNS_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS0_5list1INS0_5valueIPS5_EEEEEclEv
// demangled: boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>::operator()(void)
#[doc(alias = "boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>::operator()(void)")]
#[doc(alias = "__ZN5boost3_bi6bind_tIvNS_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS0_5list1INS0_5valueIPS5_EEEEEclEv")]
pub fn stub_689530() -> ! {
    todo!("0x689530 __ZN5boost3_bi6bind_tIvNS_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS0_5list1INS0_5valueIPS5_EEEEEclEv")
}

// 0x689548 — __ZN3rbx8callableINS_7signals6signalIFvvEE4slotEN5boost3_bi6bind_tIvNS6_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS7_5list1INS7_5valueIPSC_EEEEEELi0ES3_ED1Ev
// demangled: rbx::callable<rbx::signals::signal<void ()(void)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>,0,void ()(void)>::~callable()
#[doc(alias = "rbx::callable<rbx::signals::signal<void ()(void)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>,0,void ()(void)>::~callable()")]
#[doc(alias = "__ZN3rbx8callableINS_7signals6signalIFvvEE4slotEN5boost3_bi6bind_tIvNS6_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS7_5list1INS7_5valueIPSC_EEEEEELi0ES3_ED1Ev")]
pub fn stub_689548() -> ! {
    todo!("0x689548 __ZN3rbx8callableINS_7signals6signalIFvvEE4slotEN5boost3_bi6bind_tIvNS6_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS7_5list1INS7_5valueIPSC_EEEEEELi0ES3_ED1Ev")
}

// 0x689574 — __ZN3rbx8callableINS_7signals6signalIFvvEE4slotEN5boost3_bi6bind_tIvNS6_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS7_5list1INS7_5valueIPSC_EEEEEELi0ES3_ED0Ev
// demangled: rbx::callable<rbx::signals::signal<void ()(void)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>,0,void ()(void)>::~callable()
#[doc(alias = "rbx::callable<rbx::signals::signal<void ()(void)>::slot,boost::_bi::bind_t<void,boost::_mfi::mf0<void,RBX::ToolMouseCommand>,boost::_bi::list1<boost::_bi::value<RBX::ToolMouseCommand*>>>,0,void ()(void)>::~callable()")]
#[doc(alias = "__ZN3rbx8callableINS_7signals6signalIFvvEE4slotEN5boost3_bi6bind_tIvNS6_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS7_5list1INS7_5valueIPSC_EEEEEELi0ES3_ED0Ev")]
pub fn stub_689574() -> ! {
    todo!("0x689574 __ZN3rbx8callableINS_7signals6signalIFvvEE4slotEN5boost3_bi6bind_tIvNS6_4_mfi3mf0IvN3RBX16ToolMouseCommandEEENS7_5list1INS7_5valueIPSC_EEEEEELi0ES3_ED0Ev")
}

// 0x689648 — __ZN3RBX16ToolMouseCommandD2Ev
// demangled: RBX::ToolMouseCommand::~ToolMouseCommand()
// type: void __fastcall(RBX::ToolMouseCommand *__hidden this)
#[doc(alias = "RBX::ToolMouseCommand::~ToolMouseCommand()")]
#[doc(alias = "__ZN3RBX16ToolMouseCommandD2Ev")]
pub fn stub_689648() -> ! {
    todo!("0x689648 __ZN3RBX16ToolMouseCommandD2Ev")
}

// 0x689a5c — __ZN3RBX9ModelToolC2EPNS_9WorkspaceE
// demangled: RBX::ModelTool::ModelTool(RBX::Workspace *)
// type: _DWORD __fastcall(RBX::ModelTool *__hidden this, RBX::Workspace *)
#[doc(alias = "RBX::ModelTool::ModelTool(RBX::Workspace *)")]
#[doc(alias = "__ZN3RBX9ModelToolC2EPNS_9WorkspaceE")]
pub fn stub_689a5c() -> ! {
    todo!("0x689a5c __ZN3RBX9ModelToolC2EPNS_9WorkspaceE")
}

// 0x689b64 — __ZN3RBX9ModelToolD0Ev
// demangled: RBX::ModelTool::~ModelTool()
// type: void __fastcall(RBX::ModelTool *__hidden this)
#[doc(alias = "RBX::ModelTool::~ModelTool()")]
#[doc(alias = "__ZN3RBX9ModelToolD0Ev")]
pub fn stub_689b64() -> ! {
    todo!("0x689b64 __ZN3RBX9ModelToolD0Ev")
}

// 0x689c04 — __ZN3RBX9ModelToolD1Ev
// demangled: RBX::ModelTool::~ModelTool()
// type: void __fastcall(RBX::ModelTool *__hidden this)
#[doc(alias = "RBX::ModelTool::~ModelTool()")]
#[doc(alias = "__ZN3RBX9ModelToolD1Ev")]
pub fn stub_689c04() -> ! {
    todo!("0x689c04 __ZN3RBX9ModelToolD1Ev")
}

// 0x689c08 — __ZThn36_N3RBX9ModelToolD0Ev
// demangled: `non-virtual thunk to'RBX::ModelTool::~ModelTool()
// type: void __fastcall(RBX::ModelTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::ModelTool::~ModelTool()")]
#[doc(alias = "__ZThn36_N3RBX9ModelToolD0Ev")]
pub fn stub_689c08() -> ! {
    todo!("0x689c08 __ZThn36_N3RBX9ModelToolD0Ev")
}

// 0x689c10 — __ZN3RBX9ModelToolD2Ev
// demangled: RBX::ModelTool::~ModelTool()
// type: void __fastcall(RBX::ModelTool *__hidden this)
#[doc(alias = "RBX::ModelTool::~ModelTool()")]
#[doc(alias = "__ZN3RBX9ModelToolD2Ev")]
pub fn stub_689c10() -> ! {
    todo!("0x689c10 __ZN3RBX9ModelToolD2Ev")
}

// 0x689d2c — __ZThn36_N3RBX9ModelToolD1Ev
// demangled: `non-virtual thunk to'RBX::ModelTool::~ModelTool()
// type: void __fastcall(RBX::ModelTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::ModelTool::~ModelTool()")]
#[doc(alias = "__ZThn36_N3RBX9ModelToolD1Ev")]
pub fn stub_689d2c() -> ! {
    todo!("0x689d2c __ZThn36_N3RBX9ModelToolD1Ev")
}

// 0x689d34 — __ZN3RBX9ModelTool12onMouseHoverERKNS_7UIEventE
// demangled: RBX::ModelTool::onMouseHover(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::ModelTool *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::ModelTool::onMouseHover(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX9ModelTool12onMouseHoverERKNS_7UIEventE")]
pub fn stub_689d34() -> ! {
    todo!("0x689d34 __ZN3RBX9ModelTool12onMouseHoverERKNS_7UIEventE")
}

// 0x689e28 — __ZN3RBX9ModelTool13render3dAdornEPNS_5AdornE
// demangled: RBX::ModelTool::render3dAdorn(RBX::Adorn *)
// type: _DWORD __fastcall(RBX::ModelTool *__hidden this, RBX::Adorn *)
#[doc(alias = "RBX::ModelTool::render3dAdorn(RBX::Adorn *)")]
#[doc(alias = "__ZN3RBX9ModelTool13render3dAdornEPNS_5AdornE")]
pub fn stub_689e28() -> ! {
    todo!("0x689e28 __ZN3RBX9ModelTool13render3dAdornEPNS_5AdornE")
}

// 0x689e6c — __ZThn4_N3RBX9ModelTool13render3dAdornEPNS_5AdornE
// demangled: `non-virtual thunk to'RBX::ModelTool::render3dAdorn(RBX::Adorn *)
// type: int __fastcall(RBX::ModelTool *this, RBX::Adorn *)
#[doc(alias = "non-virtual thunk toRBX::ModelTool::render3dAdorn(RBX::Adorn *)")]
#[doc(alias = "__ZThn4_N3RBX9ModelTool13render3dAdornEPNS_5AdornE")]
pub fn stub_689e6c() -> ! {
    todo!("0x689e6c __ZThn4_N3RBX9ModelTool13render3dAdornEPNS_5AdornE")
}

// 0x689e74 — __ZN3RBX17anchorAllChildrenEN5boost10shared_ptrINS_8InstanceEEERKb
// demangled: RBX::anchorAllChildren(boost::shared_ptr<RBX::Instance>,bool const&)
// type: int __fastcall(int, int, int, int, boost::detail::sp_counted_base *, int, int, int, int, int)
#[doc(alias = "RBX::anchorAllChildren(boost::shared_ptr<RBX::Instance>,bool const&)")]
#[doc(alias = "__ZN3RBX17anchorAllChildrenEN5boost10shared_ptrINS_8InstanceEEERKb")]
pub fn stub_689e74() -> ! {
    todo!("0x689e74 __ZN3RBX17anchorAllChildrenEN5boost10shared_ptrINS_8InstanceEEERKb")
}

// 0x689f48 — __ZN3RBX10AnchorTool12onMouseHoverERKNS_7UIEventE
// demangled: RBX::AnchorTool::onMouseHover(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::AnchorTool *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::AnchorTool::onMouseHover(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX10AnchorTool12onMouseHoverERKNS_7UIEventE")]
pub fn stub_689f48() -> ! {
    todo!("0x689f48 __ZN3RBX10AnchorTool12onMouseHoverERKNS_7UIEventE")
}

// 0x68a040 — __ZNK3RBX10AnchorTool13getCursorNameEv
// demangled: RBX::AnchorTool::getCursorName(void)const
// type: _DWORD __fastcall(RBX::AnchorTool *__hidden this)
#[doc(alias = "RBX::AnchorTool::getCursorName(void)const")]
#[doc(alias = "__ZNK3RBX10AnchorTool13getCursorNameEv")]
pub fn stub_68a040() -> ! {
    todo!("0x68a040 __ZNK3RBX10AnchorTool13getCursorNameEv")
}

// 0x68a120 — __ZN3RBX10AnchorTool11onMouseDownERKNS_7UIEventE
// demangled: RBX::AnchorTool::onMouseDown(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::AnchorTool *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::AnchorTool::onMouseDown(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX10AnchorTool11onMouseDownERKNS_7UIEventE")]
pub fn stub_68a120() -> ! {
    todo!("0x68a120 __ZN3RBX10AnchorTool11onMouseDownERKNS_7UIEventE")
}

// 0x68a298 — __ZNK3RBX8LockTool13getCursorNameEv
// demangled: RBX::LockTool::getCursorName(void)const
// type: _DWORD __fastcall(RBX::LockTool *__hidden this)
#[doc(alias = "RBX::LockTool::getCursorName(void)const")]
#[doc(alias = "__ZNK3RBX8LockTool13getCursorNameEv")]
pub fn stub_68a298() -> ! {
    todo!("0x68a298 __ZNK3RBX8LockTool13getCursorNameEv")
}

// 0x68a380 — __ZN3RBX8LockTool11onMouseDownERKNS_7UIEventE
// demangled: RBX::LockTool::onMouseDown(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::LockTool *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::LockTool::onMouseDown(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX8LockTool11onMouseDownERKNS_7UIEventE")]
pub fn stub_68a380() -> ! {
    todo!("0x68a380 __ZN3RBX8LockTool11onMouseDownERKNS_7UIEventE")
}

// 0x68a490 — __ZN3RBXL17HasUnAnchoredNodeEPNS_8InstanceE
// demangled: RBX::HasUnAnchoredNode(RBX::Instance *)
// type: _DWORD __fastcall(RBX *__hidden this, RBX::Instance *)
#[doc(alias = "RBX::HasUnAnchoredNode(RBX::Instance *)")]
#[doc(alias = "__ZN3RBXL17HasUnAnchoredNodeEPNS_8InstanceE")]
pub fn stub_68a490() -> ! {
    todo!("0x68a490 __ZN3RBXL17HasUnAnchoredNodeEPNS_8InstanceE")
}

// 0x68a5c8 — __ZN3RBX10AnchorNodeclEN5boost10shared_ptrINS_8InstanceEEE
// demangled: RBX::AnchorNode::operator()(boost::shared_ptr<RBX::Instance>)
#[doc(alias = "RBX::AnchorNode::operator()(boost::shared_ptr<RBX::Instance>)")]
#[doc(alias = "__ZN3RBX10AnchorNodeclEN5boost10shared_ptrINS_8InstanceEEE")]
pub fn stub_68a5c8() -> ! {
    todo!("0x68a5c8 __ZN3RBX10AnchorNodeclEN5boost10shared_ptrINS_8InstanceEEE")
}

// 0x68a61c — __ZN3RBX10AnchorToolD1Ev
// demangled: RBX::AnchorTool::~AnchorTool()
// type: void __fastcall(RBX::AnchorTool *__hidden this)
#[doc(alias = "RBX::AnchorTool::~AnchorTool()")]
#[doc(alias = "__ZN3RBX10AnchorToolD1Ev")]
pub fn stub_68a61c() -> ! {
    todo!("0x68a61c __ZN3RBX10AnchorToolD1Ev")
}

// 0x68a620 — __ZN3RBX10AnchorToolD0Ev
// demangled: RBX::AnchorTool::~AnchorTool()
// type: void __fastcall(RBX::AnchorTool *__hidden this)
#[doc(alias = "RBX::AnchorTool::~AnchorTool()")]
#[doc(alias = "__ZN3RBX10AnchorToolD0Ev")]
pub fn stub_68a620() -> ! {
    todo!("0x68a620 __ZN3RBX10AnchorToolD0Ev")
}

// 0x68a6c0 — __ZThn36_N3RBX10AnchorToolD1Ev
// demangled: `non-virtual thunk to'RBX::AnchorTool::~AnchorTool()
// type: void __fastcall(RBX::AnchorTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::AnchorTool::~AnchorTool()")]
#[doc(alias = "__ZThn36_N3RBX10AnchorToolD1Ev")]
pub fn stub_68a6c0() -> ! {
    todo!("0x68a6c0 __ZThn36_N3RBX10AnchorToolD1Ev")
}

// 0x68a6c8 — __ZThn36_N3RBX10AnchorToolD0Ev
// demangled: `non-virtual thunk to'RBX::AnchorTool::~AnchorTool()
// type: void __fastcall(RBX::AnchorTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::AnchorTool::~AnchorTool()")]
#[doc(alias = "__ZThn36_N3RBX10AnchorToolD0Ev")]
pub fn stub_68a6c8() -> ! {
    todo!("0x68a6c8 __ZThn36_N3RBX10AnchorToolD0Ev")
}

// 0x68a76c — __ZN3RBX8LockToolD1Ev
// demangled: RBX::LockTool::~LockTool()
// type: void __fastcall(RBX::LockTool *__hidden this)
#[doc(alias = "RBX::LockTool::~LockTool()")]
#[doc(alias = "__ZN3RBX8LockToolD1Ev")]
pub fn stub_68a76c() -> ! {
    todo!("0x68a76c __ZN3RBX8LockToolD1Ev")
}

// 0x68a770 — __ZN3RBX8LockToolD0Ev
// demangled: RBX::LockTool::~LockTool()
// type: void __fastcall(RBX::LockTool *__hidden this)
#[doc(alias = "RBX::LockTool::~LockTool()")]
#[doc(alias = "__ZN3RBX8LockToolD0Ev")]
pub fn stub_68a770() -> ! {
    todo!("0x68a770 __ZN3RBX8LockToolD0Ev")
}

// 0x68a810 — __ZThn36_N3RBX8LockToolD1Ev
// demangled: `non-virtual thunk to'RBX::LockTool::~LockTool()
// type: void __fastcall(RBX::LockTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::LockTool::~LockTool()")]
#[doc(alias = "__ZThn36_N3RBX8LockToolD1Ev")]
pub fn stub_68a810() -> ! {
    todo!("0x68a810 __ZThn36_N3RBX8LockToolD1Ev")
}

// 0x68a818 — __ZThn36_N3RBX8LockToolD0Ev
// demangled: `non-virtual thunk to'RBX::LockTool::~LockTool()
// type: void __fastcall(RBX::LockTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::LockTool::~LockTool()")]
#[doc(alias = "__ZThn36_N3RBX8LockToolD0Ev")]
pub fn stub_68a818() -> ! {
    todo!("0x68a818 __ZThn36_N3RBX8LockToolD0Ev")
}

// 0x68a8bc — __ZNK3RBX8Instance13visitChildrenINS_10AnchorNodeEEEvRKT_
// demangled: void RBX::Instance::visitChildren<RBX::AnchorNode>(RBX::AnchorNode const&)const
// type: int __fastcall(int, int, int, int, int, struct _Unwind_Exception *lpuexcpt, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, int, int, int)
#[doc(alias = "void RBX::Instance::visitChildren<RBX::AnchorNode>(RBX::AnchorNode const&)const")]
#[doc(alias = "__ZNK3RBX8Instance13visitChildrenINS_10AnchorNodeEEEvRKT_")]
pub fn stub_68a8bc() -> ! {
    todo!("0x68a8bc __ZNK3RBX8Instance13visitChildrenINS_10AnchorNodeEEEvRKT_")
}

// 0x68ac9c — __ZN3RBX8PartToolC2EPNS_9WorkspaceE
// demangled: RBX::PartTool::PartTool(RBX::Workspace *)
// type: _DWORD __fastcall(RBX::PartTool *__hidden this, RBX::Workspace *)
#[doc(alias = "RBX::PartTool::PartTool(RBX::Workspace *)")]
#[doc(alias = "__ZN3RBX8PartToolC2EPNS_9WorkspaceE")]
pub fn stub_68ac9c() -> ! {
    todo!("0x68ac9c __ZN3RBX8PartToolC2EPNS_9WorkspaceE")
}

// 0x68ada4 — __ZN3RBX8PartToolD0Ev
// demangled: RBX::PartTool::~PartTool()
// type: void __fastcall(RBX::PartTool *__hidden this)
#[doc(alias = "RBX::PartTool::~PartTool()")]
#[doc(alias = "__ZN3RBX8PartToolD0Ev")]
pub fn stub_68ada4() -> ! {
    todo!("0x68ada4 __ZN3RBX8PartToolD0Ev")
}

// 0x68ae44 — __ZN3RBX8PartToolD1Ev
// demangled: RBX::PartTool::~PartTool()
// type: void __fastcall(RBX::PartTool *__hidden this)
#[doc(alias = "RBX::PartTool::~PartTool()")]
#[doc(alias = "__ZN3RBX8PartToolD1Ev")]
pub fn stub_68ae44() -> ! {
    todo!("0x68ae44 __ZN3RBX8PartToolD1Ev")
}

// 0x68ae48 — __ZThn36_N3RBX8PartToolD0Ev
// demangled: `non-virtual thunk to'RBX::PartTool::~PartTool()
// type: void __fastcall(RBX::PartTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::PartTool::~PartTool()")]
#[doc(alias = "__ZThn36_N3RBX8PartToolD0Ev")]
pub fn stub_68ae48() -> ! {
    todo!("0x68ae48 __ZThn36_N3RBX8PartToolD0Ev")
}

// 0x68ae50 — __ZN3RBX8PartToolD2Ev
// demangled: RBX::PartTool::~PartTool()
// type: void __fastcall(RBX::PartTool *__hidden this)
#[doc(alias = "RBX::PartTool::~PartTool()")]
#[doc(alias = "__ZN3RBX8PartToolD2Ev")]
pub fn stub_68ae50() -> ! {
    todo!("0x68ae50 __ZN3RBX8PartToolD2Ev")
}

// 0x68af6c — __ZThn36_N3RBX8PartToolD1Ev
// demangled: `non-virtual thunk to'RBX::PartTool::~PartTool()
// type: void __fastcall(RBX::PartTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::PartTool::~PartTool()")]
#[doc(alias = "__ZThn36_N3RBX8PartToolD1Ev")]
pub fn stub_68af6c() -> ! {
    todo!("0x68af6c __ZThn36_N3RBX8PartToolD1Ev")
}

// 0x68af74 — __ZN3RBX8PartTool12onMouseHoverERKNS_7UIEventE
// demangled: RBX::PartTool::onMouseHover(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::PartTool *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::PartTool::onMouseHover(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX8PartTool12onMouseHoverERKNS_7UIEventE")]
pub fn stub_68af74() -> ! {
    todo!("0x68af74 __ZN3RBX8PartTool12onMouseHoverERKNS_7UIEventE")
}

// 0x68b04c — __ZN3RBX8PartTool13render3dAdornEPNS_5AdornE
// demangled: RBX::PartTool::render3dAdorn(RBX::Adorn *)
// type: _DWORD __fastcall(RBX::PartTool *__hidden this, RBX::Adorn *)
#[doc(alias = "RBX::PartTool::render3dAdorn(RBX::Adorn *)")]
#[doc(alias = "__ZN3RBX8PartTool13render3dAdornEPNS_5AdornE")]
pub fn stub_68b04c() -> ! {
    todo!("0x68b04c __ZN3RBX8PartTool13render3dAdornEPNS_5AdornE")
}

// 0x68b060 — __ZThn4_N3RBX8PartTool13render3dAdornEPNS_5AdornE
// demangled: `non-virtual thunk to'RBX::PartTool::render3dAdorn(RBX::Adorn *)
// type: _DWORD __fastcall(RBX::PartTool *__hidden this, RBX::Adorn *)
#[doc(alias = "non-virtual thunk toRBX::PartTool::render3dAdorn(RBX::Adorn *)")]
#[doc(alias = "__ZThn4_N3RBX8PartTool13render3dAdornEPNS_5AdornE")]
pub fn stub_68b060() -> ! {
    todo!("0x68b060 __ZThn4_N3RBX8PartTool13render3dAdornEPNS_5AdornE")
}

// 0x68b074 — __ZN3RBX13FillToolColorC2Ev
// demangled: RBX::FillToolColor::FillToolColor(void)
// type: _DWORD __fastcall(RBX::FillToolColor *__hidden this)
#[doc(alias = "RBX::FillToolColor::FillToolColor(void)")]
#[doc(alias = "__ZN3RBX13FillToolColorC2Ev")]
pub fn stub_68b074() -> ! {
    todo!("0x68b074 __ZN3RBX13FillToolColorC2Ev")
}

// 0x68b150 — __ZN3RBX8FillTool11onMouseDownERKNS_7UIEventE
// demangled: RBX::FillTool::onMouseDown(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::FillTool *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::FillTool::onMouseDown(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX8FillTool11onMouseDownERKNS_7UIEventE")]
pub fn stub_68b150() -> ! {
    todo!("0x68b150 __ZN3RBX8FillTool11onMouseDownERKNS_7UIEventE")
}

// 0x68b1b8 — __ZN3RBX12MaterialTool11onMouseDownERKNS_7UIEventE
// demangled: RBX::MaterialTool::onMouseDown(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::MaterialTool *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::MaterialTool::onMouseDown(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX12MaterialTool11onMouseDownERKNS_7UIEventE")]
pub fn stub_68b1b8() -> ! {
    todo!("0x68b1b8 __ZN3RBX12MaterialTool11onMouseDownERKNS_7UIEventE")
}

// 0x68b220 — __ZN3RBX11DropperTool11onMouseDownERKNS_7UIEventE
// demangled: RBX::DropperTool::onMouseDown(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::DropperTool *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::DropperTool::onMouseDown(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX11DropperTool11onMouseDownERKNS_7UIEventE")]
pub fn stub_68b220() -> ! {
    todo!("0x68b220 __ZN3RBX11DropperTool11onMouseDownERKNS_7UIEventE")
}

// 0x68b34c — __ZN3RBX13FillToolColorD1Ev
// demangled: RBX::FillToolColor::~FillToolColor()
// type: void __fastcall(RBX::FillToolColor *__hidden this)
#[doc(alias = "RBX::FillToolColor::~FillToolColor()")]
#[doc(alias = "__ZN3RBX13FillToolColorD1Ev")]
pub fn stub_68b34c() -> ! {
    todo!("0x68b34c __ZN3RBX13FillToolColorD1Ev")
}

// 0x68b418 — __ZN3RBX8FillToolD1Ev
// demangled: RBX::FillTool::~FillTool()
// type: void __fastcall(RBX::FillTool *__hidden this)
#[doc(alias = "RBX::FillTool::~FillTool()")]
#[doc(alias = "__ZN3RBX8FillToolD1Ev")]
pub fn stub_68b418() -> ! {
    todo!("0x68b418 __ZN3RBX8FillToolD1Ev")
}

// 0x68b41c — __ZN3RBX8FillToolD0Ev
// demangled: RBX::FillTool::~FillTool()
// type: void __fastcall(RBX::FillTool *__hidden this)
#[doc(alias = "RBX::FillTool::~FillTool()")]
#[doc(alias = "__ZN3RBX8FillToolD0Ev")]
pub fn stub_68b41c() -> ! {
    todo!("0x68b41c __ZN3RBX8FillToolD0Ev")
}

// 0x68b4bc — __ZThn36_N3RBX8FillToolD1Ev
// demangled: `non-virtual thunk to'RBX::FillTool::~FillTool()
// type: void __fastcall(RBX::FillTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::FillTool::~FillTool()")]
#[doc(alias = "__ZThn36_N3RBX8FillToolD1Ev")]
pub fn stub_68b4bc() -> ! {
    todo!("0x68b4bc __ZThn36_N3RBX8FillToolD1Ev")
}

// 0x68b4c4 — __ZThn36_N3RBX8FillToolD0Ev
// demangled: `non-virtual thunk to'RBX::FillTool::~FillTool()
// type: void __fastcall(RBX::FillTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::FillTool::~FillTool()")]
#[doc(alias = "__ZThn36_N3RBX8FillToolD0Ev")]
pub fn stub_68b4c4() -> ! {
    todo!("0x68b4c4 __ZThn36_N3RBX8FillToolD0Ev")
}

// 0x68b568 — __ZN3RBX11DropperToolD1Ev
// demangled: RBX::DropperTool::~DropperTool()
// type: void __fastcall(RBX::DropperTool *__hidden this)
#[doc(alias = "RBX::DropperTool::~DropperTool()")]
#[doc(alias = "__ZN3RBX11DropperToolD1Ev")]
pub fn stub_68b568() -> ! {
    todo!("0x68b568 __ZN3RBX11DropperToolD1Ev")
}

// 0x68b56c — __ZN3RBX11DropperToolD0Ev
// demangled: RBX::DropperTool::~DropperTool()
// type: void __fastcall(RBX::DropperTool *__hidden this)
#[doc(alias = "RBX::DropperTool::~DropperTool()")]
#[doc(alias = "__ZN3RBX11DropperToolD0Ev")]
pub fn stub_68b56c() -> ! {
    todo!("0x68b56c __ZN3RBX11DropperToolD0Ev")
}

// 0x68b60c — __ZThn36_N3RBX11DropperToolD1Ev
// demangled: `non-virtual thunk to'RBX::DropperTool::~DropperTool()
// type: void __fastcall(RBX::DropperTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::DropperTool::~DropperTool()")]
#[doc(alias = "__ZThn36_N3RBX11DropperToolD1Ev")]
pub fn stub_68b60c() -> ! {
    todo!("0x68b60c __ZThn36_N3RBX11DropperToolD1Ev")
}

// 0x68b614 — __ZThn36_N3RBX11DropperToolD0Ev
// demangled: `non-virtual thunk to'RBX::DropperTool::~DropperTool()
// type: void __fastcall(RBX::DropperTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::DropperTool::~DropperTool()")]
#[doc(alias = "__ZThn36_N3RBX11DropperToolD0Ev")]
pub fn stub_68b614() -> ! {
    todo!("0x68b614 __ZThn36_N3RBX11DropperToolD0Ev")
}

// 0x68b6b8 — __ZN3RBX12MaterialToolD1Ev
// demangled: RBX::MaterialTool::~MaterialTool()
// type: void __fastcall(RBX::MaterialTool *__hidden this)
#[doc(alias = "RBX::MaterialTool::~MaterialTool()")]
#[doc(alias = "__ZN3RBX12MaterialToolD1Ev")]
pub fn stub_68b6b8() -> ! {
    todo!("0x68b6b8 __ZN3RBX12MaterialToolD1Ev")
}

// 0x68b6bc — __ZN3RBX12MaterialToolD0Ev
// demangled: RBX::MaterialTool::~MaterialTool()
// type: void __fastcall(RBX::MaterialTool *__hidden this)
#[doc(alias = "RBX::MaterialTool::~MaterialTool()")]
#[doc(alias = "__ZN3RBX12MaterialToolD0Ev")]
pub fn stub_68b6bc() -> ! {
    todo!("0x68b6bc __ZN3RBX12MaterialToolD0Ev")
}

// 0x68b75c — __ZThn36_N3RBX12MaterialToolD1Ev
// demangled: `non-virtual thunk to'RBX::MaterialTool::~MaterialTool()
// type: void __fastcall(RBX::MaterialTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::MaterialTool::~MaterialTool()")]
#[doc(alias = "__ZThn36_N3RBX12MaterialToolD1Ev")]
pub fn stub_68b75c() -> ! {
    todo!("0x68b75c __ZThn36_N3RBX12MaterialToolD1Ev")
}

// 0x68b764 — __ZThn36_N3RBX12MaterialToolD0Ev
// demangled: `non-virtual thunk to'RBX::MaterialTool::~MaterialTool()
// type: void __fastcall(RBX::MaterialTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::MaterialTool::~MaterialTool()")]
#[doc(alias = "__ZThn36_N3RBX12MaterialToolD0Ev")]
pub fn stub_68b764() -> ! {
    todo!("0x68b764 __ZThn36_N3RBX12MaterialToolD0Ev")
}

// 0x68bb74 — __ZN3RBX11SurfaceToolC2EPNS_9WorkspaceE
// demangled: RBX::SurfaceTool::SurfaceTool(RBX::Workspace *)
// type: _DWORD __fastcall(RBX::SurfaceTool *__hidden this, RBX::Workspace *)
#[doc(alias = "RBX::SurfaceTool::SurfaceTool(RBX::Workspace *)")]
#[doc(alias = "__ZN3RBX11SurfaceToolC2EPNS_9WorkspaceE")]
pub fn stub_68bb74() -> ! {
    todo!("0x68bb74 __ZN3RBX11SurfaceToolC2EPNS_9WorkspaceE")
}

// 0x68bcb0 — __ZN3RBX11SurfaceToolD0Ev
// demangled: RBX::SurfaceTool::~SurfaceTool()
// type: void __fastcall(RBX::SurfaceTool *__hidden this)
#[doc(alias = "RBX::SurfaceTool::~SurfaceTool()")]
#[doc(alias = "__ZN3RBX11SurfaceToolD0Ev")]
pub fn stub_68bcb0() -> ! {
    todo!("0x68bcb0 __ZN3RBX11SurfaceToolD0Ev")
}

// 0x68bd50 — __ZN3RBX11SurfaceToolD1Ev
// demangled: RBX::SurfaceTool::~SurfaceTool()
// type: void __fastcall(RBX::SurfaceTool *__hidden this)
#[doc(alias = "RBX::SurfaceTool::~SurfaceTool()")]
#[doc(alias = "__ZN3RBX11SurfaceToolD1Ev")]
pub fn stub_68bd50() -> ! {
    todo!("0x68bd50 __ZN3RBX11SurfaceToolD1Ev")
}

// 0x68bd54 — __ZThn36_N3RBX11SurfaceToolD0Ev
// demangled: `non-virtual thunk to'RBX::SurfaceTool::~SurfaceTool()
// type: void __fastcall(RBX::SurfaceTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::SurfaceTool::~SurfaceTool()")]
#[doc(alias = "__ZThn36_N3RBX11SurfaceToolD0Ev")]
pub fn stub_68bd54() -> ! {
    todo!("0x68bd54 __ZThn36_N3RBX11SurfaceToolD0Ev")
}

// 0x68bd5c — __ZN3RBX11SurfaceToolD2Ev
// demangled: RBX::SurfaceTool::~SurfaceTool()
// type: void __fastcall(RBX::SurfaceTool *__hidden this)
#[doc(alias = "RBX::SurfaceTool::~SurfaceTool()")]
#[doc(alias = "__ZN3RBX11SurfaceToolD2Ev")]
pub fn stub_68bd5c() -> ! {
    todo!("0x68bd5c __ZN3RBX11SurfaceToolD2Ev")
}

// 0x68be88 — __ZThn36_N3RBX11SurfaceToolD1Ev
// demangled: `non-virtual thunk to'RBX::SurfaceTool::~SurfaceTool()
// type: void __fastcall(RBX::SurfaceTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::SurfaceTool::~SurfaceTool()")]
#[doc(alias = "__ZThn36_N3RBX11SurfaceToolD1Ev")]
pub fn stub_68be88() -> ! {
    todo!("0x68be88 __ZThn36_N3RBX11SurfaceToolD1Ev")
}

// 0x68be90 — __ZN3RBX11SurfaceTool12onMouseHoverERKNS_7UIEventE
// demangled: RBX::SurfaceTool::onMouseHover(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::SurfaceTool *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::SurfaceTool::onMouseHover(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX11SurfaceTool12onMouseHoverERKNS_7UIEventE")]
pub fn stub_68be90() -> ! {
    todo!("0x68be90 __ZN3RBX11SurfaceTool12onMouseHoverERKNS_7UIEventE")
}

// 0x68bf78 — __ZN3RBX11SurfaceTool11onMouseDownERKNS_7UIEventE
// demangled: RBX::SurfaceTool::onMouseDown(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::SurfaceTool *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::SurfaceTool::onMouseDown(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX11SurfaceTool11onMouseDownERKNS_7UIEventE")]
pub fn stub_68bf78() -> ! {
    todo!("0x68bf78 __ZN3RBX11SurfaceTool11onMouseDownERKNS_7UIEventE")
}

// 0x68bfc8 — __ZN3RBX11SurfaceTool13render3dAdornEPNS_5AdornE
// demangled: RBX::SurfaceTool::render3dAdorn(RBX::Adorn *)
// type: _DWORD __fastcall(RBX::SurfaceTool *__hidden this, RBX::Adorn *)
#[doc(alias = "RBX::SurfaceTool::render3dAdorn(RBX::Adorn *)")]
#[doc(alias = "__ZN3RBX11SurfaceTool13render3dAdornEPNS_5AdornE")]
pub fn stub_68bfc8() -> ! {
    todo!("0x68bfc8 __ZN3RBX11SurfaceTool13render3dAdornEPNS_5AdornE")
}

// 0x68c060 — __ZThn4_N3RBX11SurfaceTool13render3dAdornEPNS_5AdornE
// demangled: `non-virtual thunk to'RBX::SurfaceTool::render3dAdorn(RBX::Adorn *)
// type: _DWORD __fastcall(RBX::SurfaceTool *__hidden this, RBX::Adorn *)
#[doc(alias = "non-virtual thunk toRBX::SurfaceTool::render3dAdorn(RBX::Adorn *)")]
#[doc(alias = "__ZThn4_N3RBX11SurfaceTool13render3dAdornEPNS_5AdornE")]
pub fn stub_68c060() -> ! {
    todo!("0x68c060 __ZThn4_N3RBX11SurfaceTool13render3dAdornEPNS_5AdornE")
}

// 0x68c068 — __ZN3RBX9DecalTool11onMouseMoveERKNS_7UIEventE
// demangled: RBX::DecalTool::onMouseMove(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::DecalTool *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::DecalTool::onMouseMove(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX9DecalTool11onMouseMoveERKNS_7UIEventE")]
pub fn stub_68c068() -> ! {
    todo!("0x68c068 __ZN3RBX9DecalTool11onMouseMoveERKNS_7UIEventE")
}

// 0x68c1ec — __ZN3RBX9DecalTool9onKeyDownERKNS_7UIEventE
// demangled: RBX::DecalTool::onKeyDown(RBX::UIEvent const&)
// type: _DWORD __fastcall(RBX::DecalTool *__hidden this, const RBX::UIEvent *)
#[doc(alias = "RBX::DecalTool::onKeyDown(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX9DecalTool9onKeyDownERKNS_7UIEventE")]
pub fn stub_68c1ec() -> ! {
    todo!("0x68c1ec __ZN3RBX9DecalTool9onKeyDownERKNS_7UIEventE")
}

// 0x68c2d8 — __ZN3RBX9DecalTool17onCancelOperationEv
// demangled: RBX::DecalTool::onCancelOperation(void)
// type: _DWORD __fastcall(RBX::DecalTool *__hidden this)
#[doc(alias = "RBX::DecalTool::onCancelOperation(void)")]
#[doc(alias = "__ZN3RBX9DecalTool17onCancelOperationEv")]
pub fn stub_68c2d8() -> ! {
    todo!("0x68c2d8 __ZN3RBX9DecalTool17onCancelOperationEv")
}

// 0x68c300 — __ZThn88_N3RBX9DecalTool17onCancelOperationEv
// demangled: `non-virtual thunk to'RBX::DecalTool::onCancelOperation(void)
// type: _DWORD __fastcall(RBX::DecalTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::DecalTool::onCancelOperation(void)")]
#[doc(alias = "__ZThn88_N3RBX9DecalTool17onCancelOperationEv")]
pub fn stub_68c300() -> ! {
    todo!("0x68c300 __ZThn88_N3RBX9DecalTool17onCancelOperationEv")
}

// 0x68c30c — __ZN3RBX9DecalTool9onMouseUpERKNS_7UIEventE
// demangled: RBX::DecalTool::onMouseUp(RBX::UIEvent const&)
// type: RBX::ChangeHistoryService *__fastcall(RBX::DecalTool *this, const char **)
#[doc(alias = "RBX::DecalTool::onMouseUp(RBX::UIEvent const&)")]
#[doc(alias = "__ZN3RBX9DecalTool9onMouseUpERKNS_7UIEventE")]
pub fn stub_68c30c() -> ! {
    todo!("0x68c30c __ZN3RBX9DecalTool9onMouseUpERKNS_7UIEventE")
}

// 0x68c338 — __ZN3RBX8FlatTool8doActionEPNS_7SurfaceE
// demangled: RBX::FlatTool::doAction(RBX::Surface *)
// type: _DWORD __fastcall(RBX::FlatTool *__hidden this, RBX::Surface *)
#[doc(alias = "RBX::FlatTool::doAction(RBX::Surface *)")]
#[doc(alias = "__ZN3RBX8FlatTool8doActionEPNS_7SurfaceE")]
pub fn stub_68c338() -> ! {
    todo!("0x68c338 __ZN3RBX8FlatTool8doActionEPNS_7SurfaceE")
}

// 0x68c358 — __ZN3RBX8GlueTool8doActionEPNS_7SurfaceE
// demangled: RBX::GlueTool::doAction(RBX::Surface *)
// type: _DWORD __fastcall(RBX::GlueTool *__hidden this, RBX::Surface *)
#[doc(alias = "RBX::GlueTool::doAction(RBX::Surface *)")]
#[doc(alias = "__ZN3RBX8GlueTool8doActionEPNS_7SurfaceE")]
pub fn stub_68c358() -> ! {
    todo!("0x68c358 __ZN3RBX8GlueTool8doActionEPNS_7SurfaceE")
}

// 0x68c390 — __ZN3RBX8WeldTool8doActionEPNS_7SurfaceE
// demangled: RBX::WeldTool::doAction(RBX::Surface *)
// type: _DWORD __fastcall(RBX::WeldTool *__hidden this, RBX::Surface *)
#[doc(alias = "RBX::WeldTool::doAction(RBX::Surface *)")]
#[doc(alias = "__ZN3RBX8WeldTool8doActionEPNS_7SurfaceE")]
pub fn stub_68c390() -> ! {
    todo!("0x68c390 __ZN3RBX8WeldTool8doActionEPNS_7SurfaceE")
}

// 0x68c3c8 — __ZN3RBX9StudsTool8doActionEPNS_7SurfaceE
// demangled: RBX::StudsTool::doAction(RBX::Surface *)
// type: _DWORD __fastcall(RBX::StudsTool *__hidden this, RBX::Surface *)
#[doc(alias = "RBX::StudsTool::doAction(RBX::Surface *)")]
#[doc(alias = "__ZN3RBX9StudsTool8doActionEPNS_7SurfaceE")]
pub fn stub_68c3c8() -> ! {
    todo!("0x68c3c8 __ZN3RBX9StudsTool8doActionEPNS_7SurfaceE")
}

// 0x68c400 — __ZN3RBX9InletTool8doActionEPNS_7SurfaceE
// demangled: RBX::InletTool::doAction(RBX::Surface *)
// type: _DWORD __fastcall(RBX::InletTool *__hidden this, RBX::Surface *)
#[doc(alias = "RBX::InletTool::doAction(RBX::Surface *)")]
#[doc(alias = "__ZN3RBX9InletTool8doActionEPNS_7SurfaceE")]
pub fn stub_68c400() -> ! {
    todo!("0x68c400 __ZN3RBX9InletTool8doActionEPNS_7SurfaceE")
}

// 0x68c438 — __ZN3RBX13UniversalTool8doActionEPNS_7SurfaceE
// demangled: RBX::UniversalTool::doAction(RBX::Surface *)
// type: _DWORD __fastcall(RBX::UniversalTool *__hidden this, RBX::Surface *)
#[doc(alias = "RBX::UniversalTool::doAction(RBX::Surface *)")]
#[doc(alias = "__ZN3RBX13UniversalTool8doActionEPNS_7SurfaceE")]
pub fn stub_68c438() -> ! {
    todo!("0x68c438 __ZN3RBX13UniversalTool8doActionEPNS_7SurfaceE")
}

// 0x68c470 — __ZN3RBX9HingeTool8doActionEPNS_7SurfaceE
// demangled: RBX::HingeTool::doAction(RBX::Surface *)
// type: _DWORD __fastcall(RBX::HingeTool *__hidden this, RBX::Surface *)
#[doc(alias = "RBX::HingeTool::doAction(RBX::Surface *)")]
#[doc(alias = "__ZN3RBX9HingeTool8doActionEPNS_7SurfaceE")]
pub fn stub_68c470() -> ! {
    todo!("0x68c470 __ZN3RBX9HingeTool8doActionEPNS_7SurfaceE")
}

// 0x68c4ac — __ZN3RBX14RightMotorTool8doActionEPNS_7SurfaceE
// demangled: RBX::RightMotorTool::doAction(RBX::Surface *)
// type: _DWORD __fastcall(RBX::RightMotorTool *__hidden this, RBX::Surface *)
#[doc(alias = "RBX::RightMotorTool::doAction(RBX::Surface *)")]
#[doc(alias = "__ZN3RBX14RightMotorTool8doActionEPNS_7SurfaceE")]
pub fn stub_68c4ac() -> ! {
    todo!("0x68c4ac __ZN3RBX14RightMotorTool8doActionEPNS_7SurfaceE")
}

// 0x68c4f4 — __ZN3RBX13LeftMotorTool8doActionEPNS_7SurfaceE
// demangled: RBX::LeftMotorTool::doAction(RBX::Surface *)
// type: _DWORD __fastcall(RBX::LeftMotorTool *__hidden this, RBX::Surface *)
#[doc(alias = "RBX::LeftMotorTool::doAction(RBX::Surface *)")]
#[doc(alias = "__ZN3RBX13LeftMotorTool8doActionEPNS_7SurfaceE")]
pub fn stub_68c4f4() -> ! {
    todo!("0x68c4f4 __ZN3RBX13LeftMotorTool8doActionEPNS_7SurfaceE")
}

// 0x68c53c — __ZN3RBX18OscillateMotorTool8doActionEPNS_7SurfaceE
// demangled: RBX::OscillateMotorTool::doAction(RBX::Surface *)
// type: _DWORD __fastcall(RBX::OscillateMotorTool *__hidden this, RBX::Surface *)
#[doc(alias = "RBX::OscillateMotorTool::doAction(RBX::Surface *)")]
#[doc(alias = "__ZN3RBX18OscillateMotorTool8doActionEPNS_7SurfaceE")]
pub fn stub_68c53c() -> ! {
    todo!("0x68c53c __ZN3RBX18OscillateMotorTool8doActionEPNS_7SurfaceE")
}

// 0x68c584 — __ZN3RBX20SmoothNoOutlinesTool8doActionEPNS_7SurfaceE
// demangled: RBX::SmoothNoOutlinesTool::doAction(RBX::Surface *)
// type: _DWORD __fastcall(RBX::SmoothNoOutlinesTool *__hidden this, RBX::Surface *)
#[doc(alias = "RBX::SmoothNoOutlinesTool::doAction(RBX::Surface *)")]
#[doc(alias = "__ZN3RBX20SmoothNoOutlinesTool8doActionEPNS_7SurfaceE")]
pub fn stub_68c584() -> ! {
    todo!("0x68c584 __ZN3RBX20SmoothNoOutlinesTool8doActionEPNS_7SurfaceE")
}

// 0x68c5bc — __ZN3RBX11shared_fromINS_9DecalToolEEEN5boost10shared_ptrIT_EEPS4_
// demangled: boost::shared_ptr<RBX::DecalTool> RBX::shared_from<RBX::DecalTool>(RBX::DecalTool*)
#[doc(alias = "boost::shared_ptr<RBX::DecalTool> RBX::shared_from<RBX::DecalTool>(RBX::DecalTool*)")]
#[doc(alias = "__ZN3RBX11shared_fromINS_9DecalToolEEEN5boost10shared_ptrIT_EEPS4_")]
pub fn stub_68c5bc() -> ! {
    todo!("0x68c5bc __ZN3RBX11shared_fromINS_9DecalToolEEEN5boost10shared_ptrIT_EEPS4_")
}

// 0x68c724 — __ZN3RBX9DecalToolD1Ev
// demangled: RBX::DecalTool::~DecalTool()
// type: void __fastcall(RBX::DecalTool *__hidden this)
#[doc(alias = "RBX::DecalTool::~DecalTool()")]
#[doc(alias = "__ZN3RBX9DecalToolD1Ev")]
pub fn stub_68c724() -> ! {
    todo!("0x68c724 __ZN3RBX9DecalToolD1Ev")
}

// 0x68c808 — __ZN3RBX9DecalToolD0Ev
// demangled: RBX::DecalTool::~DecalTool()
// type: void __fastcall(RBX::DecalTool *__hidden this)
#[doc(alias = "RBX::DecalTool::~DecalTool()")]
#[doc(alias = "__ZN3RBX9DecalToolD0Ev")]
pub fn stub_68c808() -> ! {
    todo!("0x68c808 __ZN3RBX9DecalToolD0Ev")
}

// 0x68c900 — __ZThn36_N3RBX9DecalToolD1Ev
// demangled: `non-virtual thunk to'RBX::DecalTool::~DecalTool()
// type: void __fastcall(RBX::DecalTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::DecalTool::~DecalTool()")]
#[doc(alias = "__ZThn36_N3RBX9DecalToolD1Ev")]
pub fn stub_68c900() -> ! {
    todo!("0x68c900 __ZThn36_N3RBX9DecalToolD1Ev")
}

// 0x68c9e4 — __ZThn36_N3RBX9DecalToolD0Ev
// demangled: `non-virtual thunk to'RBX::DecalTool::~DecalTool()
// type: void __fastcall(RBX::DecalTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::DecalTool::~DecalTool()")]
#[doc(alias = "__ZThn36_N3RBX9DecalToolD0Ev")]
pub fn stub_68c9e4() -> ! {
    todo!("0x68c9e4 __ZThn36_N3RBX9DecalToolD0Ev")
}

// 0x68cae0 — __ZN3RBX8FlatToolD1Ev
// demangled: RBX::FlatTool::~FlatTool()
// type: void __fastcall(RBX::FlatTool *__hidden this)
#[doc(alias = "RBX::FlatTool::~FlatTool()")]
#[doc(alias = "__ZN3RBX8FlatToolD1Ev")]
pub fn stub_68cae0() -> ! {
    todo!("0x68cae0 __ZN3RBX8FlatToolD1Ev")
}

// 0x68cae4 — __ZN3RBX8FlatToolD0Ev
// demangled: RBX::FlatTool::~FlatTool()
// type: void __fastcall(RBX::FlatTool *__hidden this)
#[doc(alias = "RBX::FlatTool::~FlatTool()")]
#[doc(alias = "__ZN3RBX8FlatToolD0Ev")]
pub fn stub_68cae4() -> ! {
    todo!("0x68cae4 __ZN3RBX8FlatToolD0Ev")
}

// 0x68cb84 — __ZThn36_N3RBX8FlatToolD1Ev
// demangled: `non-virtual thunk to'RBX::FlatTool::~FlatTool()
// type: void __fastcall(RBX::FlatTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::FlatTool::~FlatTool()")]
#[doc(alias = "__ZThn36_N3RBX8FlatToolD1Ev")]
pub fn stub_68cb84() -> ! {
    todo!("0x68cb84 __ZThn36_N3RBX8FlatToolD1Ev")
}

// 0x68cb8c — __ZThn36_N3RBX8FlatToolD0Ev
// demangled: `non-virtual thunk to'RBX::FlatTool::~FlatTool()
// type: void __fastcall(RBX::FlatTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::FlatTool::~FlatTool()")]
#[doc(alias = "__ZThn36_N3RBX8FlatToolD0Ev")]
pub fn stub_68cb8c() -> ! {
    todo!("0x68cb8c __ZThn36_N3RBX8FlatToolD0Ev")
}

// 0x68cc30 — __ZN3RBX8GlueToolD1Ev
// demangled: RBX::GlueTool::~GlueTool()
// type: void __fastcall(RBX::GlueTool *__hidden this)
#[doc(alias = "RBX::GlueTool::~GlueTool()")]
#[doc(alias = "__ZN3RBX8GlueToolD1Ev")]
pub fn stub_68cc30() -> ! {
    todo!("0x68cc30 __ZN3RBX8GlueToolD1Ev")
}

// 0x68cc34 — __ZN3RBX8GlueToolD0Ev
// demangled: RBX::GlueTool::~GlueTool()
// type: void __fastcall(RBX::GlueTool *__hidden this)
#[doc(alias = "RBX::GlueTool::~GlueTool()")]
#[doc(alias = "__ZN3RBX8GlueToolD0Ev")]
pub fn stub_68cc34() -> ! {
    todo!("0x68cc34 __ZN3RBX8GlueToolD0Ev")
}

// 0x68ccd4 — __ZThn36_N3RBX8GlueToolD1Ev
// demangled: `non-virtual thunk to'RBX::GlueTool::~GlueTool()
// type: void __fastcall(RBX::GlueTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::GlueTool::~GlueTool()")]
#[doc(alias = "__ZThn36_N3RBX8GlueToolD1Ev")]
pub fn stub_68ccd4() -> ! {
    todo!("0x68ccd4 __ZThn36_N3RBX8GlueToolD1Ev")
}

// 0x68ccdc — __ZThn36_N3RBX8GlueToolD0Ev
// demangled: `non-virtual thunk to'RBX::GlueTool::~GlueTool()
// type: void __fastcall(RBX::GlueTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::GlueTool::~GlueTool()")]
#[doc(alias = "__ZThn36_N3RBX8GlueToolD0Ev")]
pub fn stub_68ccdc() -> ! {
    todo!("0x68ccdc __ZThn36_N3RBX8GlueToolD0Ev")
}

// 0x68cd80 — __ZN3RBX8WeldToolD1Ev
// demangled: RBX::WeldTool::~WeldTool()
// type: void __fastcall(RBX::WeldTool *__hidden this)
#[doc(alias = "RBX::WeldTool::~WeldTool()")]
#[doc(alias = "__ZN3RBX8WeldToolD1Ev")]
pub fn stub_68cd80() -> ! {
    todo!("0x68cd80 __ZN3RBX8WeldToolD1Ev")
}

// 0x68cd84 — __ZN3RBX8WeldToolD0Ev
// demangled: RBX::WeldTool::~WeldTool()
// type: void __fastcall(RBX::WeldTool *__hidden this)
#[doc(alias = "RBX::WeldTool::~WeldTool()")]
#[doc(alias = "__ZN3RBX8WeldToolD0Ev")]
pub fn stub_68cd84() -> ! {
    todo!("0x68cd84 __ZN3RBX8WeldToolD0Ev")
}

// 0x68ce24 — __ZThn36_N3RBX8WeldToolD1Ev
// demangled: `non-virtual thunk to'RBX::WeldTool::~WeldTool()
// type: void __fastcall(RBX::WeldTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::WeldTool::~WeldTool()")]
#[doc(alias = "__ZThn36_N3RBX8WeldToolD1Ev")]
pub fn stub_68ce24() -> ! {
    todo!("0x68ce24 __ZThn36_N3RBX8WeldToolD1Ev")
}

// 0x68ce2c — __ZThn36_N3RBX8WeldToolD0Ev
// demangled: `non-virtual thunk to'RBX::WeldTool::~WeldTool()
// type: void __fastcall(RBX::WeldTool *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::WeldTool::~WeldTool()")]
#[doc(alias = "__ZThn36_N3RBX8WeldToolD0Ev")]
pub fn stub_68ce2c() -> ! {
    todo!("0x68ce2c __ZThn36_N3RBX8WeldToolD0Ev")
}

// 0x68ced0 — __ZN3RBX9StudsToolD1Ev
// demangled: RBX::StudsTool::~StudsTool()
// type: void __fastcall(RBX::StudsTool *__hidden this)
#[doc(alias = "RBX::StudsTool::~StudsTool()")]
#[doc(alias = "__ZN3RBX9StudsToolD1Ev")]
pub fn stub_68ced0() -> ! {
    todo!("0x68ced0 __ZN3RBX9StudsToolD1Ev")
}

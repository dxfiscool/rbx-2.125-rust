...
/// Declared RBX::Name for RBX::Soundscape::sSoundService (IDA 0x445890:
/// guard-once static n, Name::declare(&sSoundService); host: &str).
static SOUND_SERVICE_NAME: LazyLock<String> = LazyLock::new(|| "SoundService".to_owned());

/// FactoryProduct<SoundService, Instance>::creatorPrivate (IDA 0x445768).
/// The image keeps one static Creator; LazyLock never drops (atexit equivalent).
static SOUND_SERVICE_CREATOR: LazyLock<SoundServiceCreator> = LazyLock::new(|| SoundServiceCreator);

/// Source for RBX::ServiceProvider::newIndex (IDA 0x4459f0: the SoundService
/// class index is allocated once under the doGetClassIndex guard).
static SERVICE_INDEX_SOURCE: AtomicU32 = AtomicU32::new(1);

/// RBX::ServiceProvider::doGetClassIndex<SoundService> cache (IDA 0x445978:
/// guard-once index allocated via newIndex; host: LazyLock never drops).
static SOUND_SERVICE_CLASS_INDEX: LazyLock<u32> =
    LazyLock::new(|| SERVICE_INDEX_SOURCE.fetch_add(1, Ordering::Relaxed));
...
/// Holder vtable for the SoundType placement_any tag (IDA 0x37fc3e/0x37fc52:
/// operator=<SoundType> + retype to the SoundType singleton, cf. 0x377988).
pub struct SoundTypeHolder {
    pub type_name: &'static str,
    pub construct: fn(*const SoundType, *mut SoundType) -> SoundType,
    pub destruct: fn(),
}

static SOUND_TYPE_HOLDER: LazyLock<SoundTypeHolder> = LazyLock::new(|| SoundTypeHolder {
    type_name: "N3RBX9SoundTypeE",
    construct: stub_sound_type_construct,
    destruct: stub_sound_type_destruct,
});

/// Trivial-enum holder construct (cf. the Reverb twin at 0x377a44): null dst
/// returns the source word untouched, else copies it; returns the word.
/// SAFETY: holder protocol guarantees src readable and dst writable-or-null.
fn stub_sound_type_construct(src: *const SoundType, dst: *mut SoundType) -> SoundType {
...
/// Trivial-enum holder destruct (cf. 0x377a50: BX LR, nothing to destroy).
fn stub_sound_type_destruct() {}

/// RBX::Reflection::EnumDesc<RBX::SoundType> host descriptor (IDA 0x37f4dc:
/// EnumDescriptor base "SoundType" + 15 addPair calls at 0x37f5c0..0x37f6f4).
/// The convert tables live in SOUND_TYPE_ITEMS / SOUND_TYPE_BY_NAME_SORTED;
/// the host descriptor is a unit tag over them (cf. ReverbEnumDesc).
pub struct SoundTypeEnumDesc;

static SOUND_TYPE_ENUM_DESC: LazyLock<SoundTypeEnumDesc> = LazyLock::new(|| SoundTypeEnumDesc);

/// String form of a SoundType value (cf. the Reverb twin at 0x3777e8).
fn sound_type_to_string(value: SoundType, out: &mut String) {
    ...
    if flog_asserts() {
        ...
        assert!(
            (raw as usize) < SOUND_TYPE_ITEMS.len(),
            "(size_t)value<enumToItem.size() file: include/reflection/enumconverter.h line: 263"
        );
    }
    if raw < 0 || (raw as usize) >= SOUND_TYPE_ITEMS.len() {
        out.clear();
    } else {
        out.clear();
        out.push_str(SOUND_TYPE_ITEMS[raw as usize].0);
    }
}

/// Item index of a SoundType value (cf. the Reverb twin at 0x377a54: dense
/// identity; failure returns 0, colliding with NoSound's item, as in the original).
fn sound_type_to_item(value: SoundType) -> u32 {
    let raw = value as i32;
    if flog_asserts() {
        assert!(
            raw >= 0,
    ...
    if (raw as usize) < SOUND_TYPE_ITEMS.len() {
        raw as u32
    } else {
        0
    }
...
/// RBX::Reflection::Variant holdings seen on the SoundId/SoundType convert paths.
#[derive(Clone, Default)]
pub enum Variant {
    #[default]
    Empty,
...
impl Variant {
    fn type_name(&self) -> &'static str {
        match self {
            ...
            Variant::SoundType(_) => "N3RBX9SoundTypeE",
            Variant::Text(_) => "Ss",
        }
    }
    ...
    /// Payload probe for the genericConvert<SoundType> fast path (IDA 0x37fba2).
    fn as_sound_type(&self) -> Option<&SoundType> {
        match self {
            Variant::SoundType(value) => Some(value),
            _ => None,
        }
    }
}
...
pub fn stub_37f4d8() -> &'static SoundTypeEnumDesc {
    // IDA 0x37f4d8: C1 thunk (B.W) into the C2 below.
    stub_37f4dc()
}
...
// type: int __fastcall(int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::SoundType>::EnumDesc(void)")]
pub fn stub_37f4dc() -> &'static SoundTypeEnumDesc {
    // IDA 0x37f4dc: EnumDescriptor base "SoundType" (0x37f512) plus the 15
    // addPair calls at 0x37f5c0..0x37f6f4; host tables are const, so the
    // descriptor is a unit tag over SOUND_TYPE_ITEMS (cf. ReverbEnumDesc).
    &*SOUND_TYPE_ENUM_DESC
}
...
pub fn stub_37f7c8(variant: &mut Variant) -> Result<&SoundType, SoundCastError> {
    // IDA 0x37f7c8: thunk (B.W) into genericConvert<SoundType>; host: delegate.
    stub_37fb78(variant)
}
...
pub fn stub_37f7cc(text: &str, out: &mut SoundType) -> bool {
    // IDA 0x37f7cc (disasm; Hex-Rays declined): singleton call_once
    // (0x37f7ee), doGetSingleton (0x37f7f2), Name::lookup on the text
    // (0x37f7fc), EnumDesc::convertToValue(name, out) (0x37f810); the int
    // return is the convert hit. Host: the name table decides.
    match sound_type_from_name(text) {
        Some(value) => {
            *out = value;
            true
        }
...
pub fn stub_37f818(value: SoundType, name: &str) {
    // IDA 0x37f818: Descriptor::Descriptor(name) item (0x37f884), push it
    // into the item table (0x37f8be), grow + fill the enumToItem/value maps
    // (0x37f8d4..0x37f9cc), push the value (0x37f9d8), grow + fill the name
    // tables (0x37f9ee..0x37fa90), and file the name->item map entry
    // (0x37faa4) with the log2 count word (0x37faae..0x37fac6) under the
    // value>=0 / value<=2304 asserts (0x37f8fa..0x37f986). Host tables are
    // const and already hold the ctor's 15 pairs, so only the asserts plus
    // the pair's agreement with the table survive (host: panic).
    let raw = value as i32;
    if flog_asserts() {
        assert!(
            raw >= 0,
    ...
    if (raw as usize) < SOUND_TYPE_ITEMS.len() {
        debug_assert_eq!(SOUND_TYPE_ITEMS[raw as usize].0, name);
        debug_assert_eq!(SOUND_TYPE_ITEMS[raw as usize].1, raw);
    }
}
...
pub fn stub_37fb78(variant: &mut Variant) -> Result<&SoundType, SoundCastError> {
    // IDA 0x37fb78: any_cast<SoundType> fast path (0x37fba2..0x37fbcc); on
    // miss only a std::string holding converts (else runtime_error "Unable
    // to cast %s to %s", 0x37fca0..0x37fcf8): convert via StringConverter
    // (0x37fc14), store back into the slot (0x37fc3e), retype to the
    // SoundType singleton (0x37fc48), and return the fresh payload
    // (0x37fc52). Twin of the SoundId path at 0x376ce4.
    if variant.as_sound_type().is_none() {
        // Miss path: only a std::string holding converts, else runtime_error
        // "Unable to cast %s to %s" (0x37fca0..0x37fcf8).
        let text = match &*variant {
            Variant::Text(text) => text.clone(),
            other => {
                return Err(SoundCastError::BadCast {
        ...
        // Convert via StringConverter (0x37fc14); a failed lookup throws the
        // same runtime_error, so the miss converts to a BadCast here.
        let mut value = SoundType::NoSound;
        if !stub_37f7cc(&text, &mut value) {
            return Err(SoundCastError::BadCast {
                from: "Ss",
                to: "N3RBX9SoundTypeE",
            });
        }
        // Store back into the slot (0x37fc3e) and retype (0x37fc48); the tag
        // is implied by the Variant::SoundType payload.
        *variant = Variant::SoundType(value);
    }
    // Fast-path hit (0x37fba2..0x37fbcc) or the fresh payload (0x37fc52).
    match &*variant {
        Variant::SoundType(value) => Ok(value),
        _ => unreachable!("genericConvert<SoundType> always leaves a SoundType"),
    }
}
...
pub fn stub_37fd64(slot: &PlacementAny) -> Option<&SoundType> {
    // IDA 0x37fd64: null holder -> void typeinfo (0x37fd70..0x37fd7c);
    // holder or name ("N3RBX9SoundTypeE", 0x37fd8e..0x37fdb2) mismatch ->
    // null; else payload at +1 (0x37fd98/0x37fdb4). Host: the enum tag
    // subsumes the holder checks (twin of the SoundId check at 0x37bc94).
    match slot {
        PlacementAny::SoundType(value) => Some(value),
        _ => None,
    }
}
...
// type: char ****__fastcall(char ****)
#[doc(alias = "RBX::SoundType & rbx::any_cast<RBX::SoundType &,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
pub fn stub_37fdbc(slot: &PlacementAny) -> Result<&SoundType, SoundCastError> {
    // IDA 0x37fdbc: throwing any_cast<SoundType&> — mismatch throws
    // bad_placement_any_cast via boost::throw_exception (0x37fe72..0x37fe7a);
    // else payload at +1 (0x37fe62). Host: thiserror (twin of 0x37bcec).
    match slot {
        PlacementAny::SoundType(value) => Ok(value),
        _ => Err(SoundCastError::BadPlacementAnyCast),
    }
}
...
pub fn stub_37feac(vec: &mut Vec<SoundType>, len: usize, value: SoundType) {
    // IDA 0x37feac: finish <= len -> _M_fill_insert (0x37fed4); else shrink
    // the finish word in place (0x37feca, relocating nothing). Host: resize.
...
pub fn stub_37fee0(vec: &mut Vec<SoundType>, value: &SoundType) {
    // IDA 0x37fee0: finish == end-of-storage -> _M_insert_aux (0x37ff02);
    // else store at finish (0x37fef4) and bump it (0x37fef8). Host: push.
    vec.push(*value);
}
...
pub fn stub_37ff08<'a>(map: &'a mut HashMap<String, SoundType>, key: &str) -> &'a mut SoundType {
    // IDA 0x37ff08: RB-tree lower_bound on the Name* key (0x37ff20..0x37ff30);
    // on a miss _M_insert_unique a default SoundType (0x37ff54), then return
    // the value slot (node+16, 0x37ff5c). Host: entry API; the default is
    // value 0 (NoSound). Twin of the SoundId-map index at 0x3772c0.
    map.entry(key.to_owned()).or_insert(SoundType::NoSound)
}
...
pub fn stub_37ff60(map: &mut HashMap<String, SoundType>, key: &str, value: SoundType) -> bool {
    // IDA 0x37ff60: _M_insert_unique with a position hint — the hint only
    // seeds the lower_bound walk; a duplicate key still inserts nothing and
    // returns the existing iterator (0x380002). Host: the hint is meaningless
    // for HashMap, delegate to the unique insert (cf. 0x3788dc).
    stub_38006c(map, key, value)
}
...
pub fn stub_380014(map: &mut HashMap<String, SoundType>, key: &str, value: SoundType) -> bool {
    // IDA 0x380014: _M_insert(parent, node, value) — allocates the node
    // (0x380044), copies the pair (0x38004a), links + rebalances
    // (0x380056), bumps the size (0x380060); the caller established the
    // miss. Host: the miss is rechecked by the unique insert (cf. 0x3789c4).
    stub_38006c(map, key, value)
}
...
pub fn stub_38006c(map: &mut HashMap<String, SoundType>, key: &str, value: SoundType) -> bool {
    // IDA 0x38006c: _M_insert_unique(value) — lower_bound on the key
    // (0x380080..0x380098); on a hit return the existing node with false
    // (0x3800c8..0x3800cc), else _M_insert the node (0x3800b6) and return
    // true (0x3800c4). Host: HashMap insert reports vacant (true) vs
    // occupied (false); the occupied slot keeps its value (cf. 0x378a14).
    use std::collections::hash_map::Entry;
...
pub fn stub_3800d4(vec: &mut Vec<SoundType>, index: usize, value: &SoundType) {
    // IDA 0x3800d4: spare capacity -> shift the tail up by one (copy_backward
    // at 0x380100) and store (0x3800ec..0x380104); else reallocate at
    // 2*size+1 (0x38010a..0x38012a, length_error at 0x3fffffff), copy both
    // halves around the new element (0x380132..0x38017e), and swap the
    // buffer (0x380184..0x380194). Host: single insert covers both.
...
pub fn stub_3801b8(count: usize) -> Vec<SoundType> {
    // IDA 0x3801b8: count >= 0x40000000 -> __throw_bad_alloc (0x3801c2);
    // else operator new(4 * count) (0x3801c4). Host: reserve the storage;
    // the caller fills it (elements are Copy, so no uninit exposure).
    let mut vec = Vec::new();
...
pub fn stub_3801d0(vec: &mut Vec<SoundType>, first: usize, last: usize, dest_end: usize) {
    // IDA 0x3801d0: byte count a2-a1 (0x3801d0); nonzero -> word copy from
    // the end down (0x3801f6..0x380202), returning the shifted result end
    // (0x380204). Host: copy_within over the same half-open range.
    vec.copy_within(first..last, dest_end - (last - first));
}
...
pub fn stub_38020c(vec: &mut Vec<SoundType>, index: usize, count: usize, value: &SoundType) {
    // IDA 0x38020c: zero count -> no-op; spare capacity -> shift-or-fill in
    // place (0x3802f2..0x38037a, copy_backward at 0x38031e); else reallocate
    // at size+max(size,count) (0x38023a..0x38026c, length_error past
    // 0x3fffffff), fill the gap (0x380298..0x3802a8), and swap the buffer
    // (0x3802d6..0x3802ea). Host: splice with a repeat fill.
...
pub fn stub_434d00(provider: &mut ServiceProvider) -> SharedPtr<SoundService> {
    // IDA 0x434d00: find<SoundService> (0x434d26); on a hit return it
    // (0x434e94). On a miss Creatable::create the service (0x434d5e),
    // setAndLockParent into this provider (0x434d6e), file it in the
    // class-index slot (0x434d8e..0x434da8), then the FLog::Asserts-gated
    // serviceMap registration check (0x434dc4..0x434e6c, Service.h:105).
    // Host: the job/parent/factory wiring has no image, so a miss builds
    // the default service and files it in the provider slot; the class
    // name is the "SoundService" word, which is registered by construction.
    if let Some(service) = provider.find_sound_service() {
        return service;
    }
    let service = SharedPtr::new(SoundService::default());
    provider.sound_service = Some(SharedPtr::clone(&service));
    let _ = stub_445978();
    service
}
...
pub fn stub_44558c(provider: &ServiceProvider) -> Option<SharedPtr<SoundService>> {
    // IDA 0x44558c: call_once the class-index init (0x4455c4), read the
    // cached service slot (0x4455ea..0x44563c); a live entry returns it
    // (0x445640), else resize the slot vector (0x445624) and
    // findServiceByClassName through the creator's class name
    // (0x445648..0x445660), filing a hit back into the slot (0x44567a).
    // Host: the slot is the provider's sound_service field (twin of the
    // scoped find at 0x377154).
    let _ = stub_445974();
    provider.find_sound_service()
}
...
pub fn stub_445700() -> bool {
    // IDA 0x445700: FLog::Asserts-gated "!className().empty()" ReleaseAssert
    // over the creator's class name (0x445714..0x445764, object.h:292),
    // then return 0 (0x445764). The name is "SoundService", never empty.
    let _ = stub_4457dc();
    false
}
...
pub fn stub_445768() -> &'static SoundServiceCreator {
    // IDA 0x445768: FLog::Asserts-gated Creator::wasConstructed()
    // ReleaseAssert (0x445778..0x4457ca, object.h:282), return creatorPrivate
    // (0x4457da). Host: the stateless creator (twin of 0x3787a0).
    &*SOUND_SERVICE_CREATOR
}
...
pub fn stub_4457dc() -> &'static str {
    // IDA 0x4457dc: FLog::Asserts-gated wasConstructed() ReleaseAssert
    // (0x4457ec..0x44583c, object.h:236), then Name::declare + doDeclare
    // returning the sSoundService name (twin of the Sound path at 0x377efc).
    stub_445890()
}
...
pub fn stub_445848() -> &'static str {
    // IDA 0x445848 (disasm; Hex-Rays declined): null sSoundService ->
    // getNullName (0x445882..0x445886); else call_once callDoDeclare
    // (0x44585e..0x445876) and tail-call doDeclare (0x44587e). Host: the
    // name is always declared (twin of the Sound path at 0x37847c).
    stub_445890()
}
...
pub fn stub_44588c() -> &'static str {
    // IDA 0x44588c: single B.W into doDeclare (call_once target); host: delegate.
    stub_445890()
}
...
pub fn stub_445890() -> &'static str {
    // IDA 0x445890: guard-once static n (cxa_guard_acquire/release,
    // 0x4458ec..0x445918); Name::declare(&sSoundService) (0x445914) stored
    // into n, return n (0x445946). Host: LazyLock init (twin of 0x37847c).
    LazyLock::force(&SOUND_SERVICE_NAME);
    SOUND_SERVICE_NAME.as_str()
}
...
pub fn stub_445974() -> u32 {
    // IDA 0x445974: thunk (B.W) into doGetClassIndex; host: delegate.
    stub_445978()
}
...
pub fn stub_445978() -> u32 {
    // IDA 0x445978: guard-once index (cxa_guard_acquire/release,
    // 0x4459d4..0x4459f4); newIndex (0x4459f0) stored into index, return
    // index (0x445a22). Host: LazyLock init over the index source.
    *SOUND_SERVICE_CLASS_INDEX
}
...
pub fn stub_452950() -> SharedPtr<SoundService> {
    // IDA 0x452950: operator new(0xE0) (0x452984), SoundService ctor
    // (0x4529a8), shared_ptr<SoundService> with Creatable<Instance>::Deleter
    // (0x4529b6). Host: Arc construction adopts the deleter/owner wiring
    // (twin of the provider create at 0x434d00).
    SharedPtr::new(SoundService::default())
}
...
pub fn stub_452a00(service: SharedPtr<SoundService>) -> SharedPtr<SoundService> {
    // IDA 0x452a00: ptr word copy (0x452a20), shared_count ctor with the
    // Creatable deleter (0x452a28), _internal_accept_owner into the
    // DescribedBase weak slot at +40 on non-null (0x452a56..0x452a66).
    // Host: Arc move adopts owners (carrier no-op, cf. stub_452ac8).
    service
}
...
pub fn stub_452ce8() -> &'static SoundServiceCreator {
    // IDA 0x452ce8: Creator C2 — vtable install (0x452d12),
    // Name::declare<sSoundService> (0x452d14), then lower_bound walk and
    // unique insert into AbstractFactoryProduct<Instance>::getCreators()
    // keyed by name under FLog::Asserts-gated duplicate/isConstructed
    // ReleaseAsserts. Host: force the name + creator registration
    // (twin of the SoundChannel path at 0x37855c).
    let _ = stub_445890();
    stub_445768()
}
...
pub fn stub_452f10(service: &SharedPtr<SoundService>) -> SharedPtr<SoundService> {
    // IDA 0x452f10: shared_count copy (0x452f22), ptr word adopt (0x452f2c),
    // old count release (0x452f38..0x452f3a). Derived-to-base coercion has no
    // host image; the Arc clone carries the ownership (cf. stub_452ac8).
    SharedPtr::clone(service)
}
...
pub fn stub_4ab168() -> &'static SoundTypeEnumDesc {
    // IDA 0x4ab168 (disasm; Hex-Rays declined): once-init over flag_ptr /
    // initSingleton (0x4ab16a..0x4ab182), then return the EnumDesc<SoundType>
    // singleton. Host: delegate to doGetSingleton (twin of 0x378e84).
    stub_4c2178()
}
...
pub fn stub_4c2174() -> &'static SoundTypeEnumDesc {
    // IDA 0x4c2174: thunk (B.W) into doGetSingleton; host: delegate.
    stub_4c2178()
}
...
pub fn stub_4c2178() -> &'static SoundTypeEnumDesc {
    // IDA 0x4c2178: guard-once static s (cxa_guard_acquire/release),
    // EnumDesc C2 in place (0x4c21ee), atexit dtor (0x4c220c, host: LazyLock
    // never drops), return s. Host: the unit descriptor tag (twin of 0x378e88).
    &*SOUND_TYPE_ENUM_DESC
}
...
pub fn stub_4c24e0(name: &str, value_out: &mut SoundType) -> u32 {
    // IDA 0x4c24e0: Name::lookup (0x4c24ec, host: the &str itself), then
    // convertToValue(Name) (0x4c24fa); on success convertToItem (0x4c2506),
    // else 0 (0x4c24fc/0x4c250c). Twin of the Reverb path at 0x3775f8.
    if stub_4c2af8(name, value_out) {
        stub_4c293c(*value_out)
    } else {
        0
    }
}
...
pub fn stub_4c2510(any: &PlacementAny) -> Result<u32, SoundCastError> {
    // IDA 0x4c2510: any_cast<const SoundType&> (0x4c2522, throws bad_cast on
    // mismatch) then convertToItem (0x4c252c). Twin of 0x377628.
    let value = stub_4c2a08(any)?;
    Ok(stub_4c293c(*value))
}
...
pub fn stub_4c2530(index: u32, out: &mut SoundType) -> bool {
    // IDA 0x4c2530 (disasm; Hex-Rays declined): count = [this,#0x28],
    // table = [this,#0x90]; HI (index < count) -> out = table[index],
    // return 1 (0x4c253c..0x4c254a), else return 0. Host table is dense
    // identity (twin of the Reverb path at 0x377648).
    if (index as usize) < SOUND_TYPE_ITEMS.len() {
        if let Some(value) = SoundType::from_i32(SOUND_TYPE_ITEMS[index as usize].1) {
            *out = value;
            return true;
        }
    }
...
pub fn stub_4c258c(index: u32, out: &mut String) -> bool {
    // IDA 0x4c258c: if (*(this+40) > index): item = *(this+144)[index],
    // convertToString(item) into a temp, string::assign, destroy the temp,
    // return 1; else return 0. Twin of the Reverb path at 0x3776a4.
    if (index as usize) < SOUND_TYPE_ITEMS.len() {
        stub_4c26d0(SOUND_TYPE_ITEMS[index as usize].1, out);
        true
    } else {
        false
    }
...
pub fn stub_4c26d0(value: i32, out: &mut String) {
    // IDA 0x4c26d0: FLog::Asserts-gated ReleaseAsserts "value>=0"
    // (enumconverter.h:262) and "(size_t)value<enumToItem.size()" (:263,
    // host: panic); then value <= -1 -> "" (cf. 0x37792a), out of range -> ""
    // (cf. 0x377942), else names[value]. Twin of 0x3777e8.
    if flog_asserts() {
        assert!(
            value >= 0,
            "value>=0 file: include/reflection/enumconverter.h line: 262"
        ...
        assert!(
            (value as usize) < SOUND_TYPE_ITEMS.len(),
            "(size_t)value<enumToItem.size() file: include/reflection/enumconverter.h line: 263"
        );
    }
    match SoundType::from_i32(value) {
        Some(typed) => sound_type_to_string(typed, out),
        None => out.clear(),
    }
}
...
pub fn stub_4c2870<'a>(slot: &'a mut PlacementAny, value: SoundType) -> &'a mut PlacementAny {
    // IDA 0x4c2870: singleton() names the holder (0x4c287c); same holder ->
    // word copy (0x4c28a8); else destroy old payload (0x4c2894..0x4c28a0,
    // host: enum drop) then store + retag (0x4c28b2). Trivial enum: no-op
    // dtor. Twin of the Reverb path at 0x377988.
    match &mut *slot {
        PlacementAny::SoundType(current) => {
            *current = value;
        }
        other => {
            *other = PlacementAny::SoundType(value);
        }
    }
    slot
}
...
pub fn stub_4c28c0() -> &'static SoundTypeHolder {
    // IDA 0x4c28c0: cxa_guard_acquire/release around s
    // (0x4c28da..0x4c2916); s = {typeinfo, destruct_func} + construct_func
    // word. Host: LazyLock never drops (atexit equivalent, twin of 0x3779d8).
    &*SOUND_TYPE_HOLDER
}
...
pub fn stub_4c292c(src: *const SoundType, dst: *mut SoundType) -> SoundType {
    // IDA 0x4c292c: null dst -> return src word untouched (0x4c292e);
    // else *dst = loaded word (0x4c2930/0x4c2932, trivial 4-byte enum copy).
    // Twin of the Reverb path at 0x377a44; host: delegate.
    stub_sound_type_construct(src, dst)
}
...
pub fn stub_4c2938() {
    // IDA 0x4c2938: BX LR — trivial enum, nothing to destroy (twin of 0x377a50).
    stub_sound_type_destruct()
}
...
pub fn stub_4c293c(value: SoundType) -> u32 {
    // IDA 0x4c293c: same assert pair as 0x4c26d0 at enumconverter.h:273/274
    // (host: panic); then value < 0 -> 0, out of range -> 0, else
    // enumToItem[value] (dense identity). Twin of 0x377a54.
    // NOTE: failure returns 0, which collides with NoSound's item — as in the original.
    let raw = value as i32;
    if flog_asserts() {
        assert!(
            raw >= 0,
...
pub fn stub_4c2a08(slot: &PlacementAny) -> Result<&SoundType, SoundCastError> {
    // IDA 0x4c2a08: null holder -> void typeinfo; holder or name
    // ("N3RBX9SoundTypeE") mismatch -> throw bad_placement_any_cast; else
    // payload at +1. Host: the enum tag subsumes both checks (twin of 0x377b20).
    match slot {
        PlacementAny::SoundType(value) => Ok(value),
        _ => Err(SoundCastError::BadPlacementAnyCast),
    }
...
pub fn stub_4c2af8(name: &str, out: &mut SoundType) -> bool {
    // IDA 0x4c2af8: two RB-tree lower_bound walks (item map, name map) with
    // exact-key rechecks; success stores the value word and returns 1, else
    // 0. Host: binary search over the name-sorted table (twin of 0x377c10).
    match SOUND_TYPE_BY_NAME_SORTED.binary_search_by(|probe| probe.0.cmp(name)) {
        Ok(found) => {
            if let Some(value) = SoundType::from_i32(SOUND_TYPE_BY_NAME_SORTED[found].1) {
                *out = value;
                return true;
            }
            false
...
/// RBX::Soundscape::SoundService class descriptor (IDA 0x710c78:
/// guard-once describedClassDescriptor, base Instance, name "SoundService",
/// atexit dtor; host: LazyLock never drops). Twin of STOCK_SOUND_DESCRIPTOR.
static SOUND_SERVICE_DESCRIPTOR: LazyLock<ClassDescriptor> = LazyLock::new(|| ClassDescriptor {
    name: "SoundService",
    base: Some(&*SOUND_INSTANCE_DESCRIPTOR),
});
...
pub fn stub_710c78() -> &'static ClassDescriptor {
    // IDA 0x710c78: guard-once describedClassDescriptor (0x710cd4..0x710d40);
    // base = Instance classDescriptor, name "SoundService" (0x710d1c),
    // atexit dtor (host: LazyLock never drops). Twin of 0x378f78.
    &*SOUND_SERVICE_DESCRIPTOR
}
...
pub fn stub_7aac50(provider: &mut ServiceProvider, _instance: &Instance) -> SharedPtr<SoundService> {
    // IDA 0x7aac50: findServiceProvider(scope, instance) (0x7aac54); on a hit
    // tail-call create<SoundService> on the found provider (0x7aac64), which
    // itself finds-or-creates the service (0x434d00). Host: the instance
    // scope check folds into the provider slot; host: delegate.
    stub_434d00(provider)
}

/// RBX::Soundscape::CollisionSound — one loaded impact sample (IDA 0x7f937c:
/// SoundId at +0, FMOD sound adopted on a Sound::get hit, 3D min/max from
/// the id payload, has_sound flag at +8).
#[derive(Clone, Default)]
...
/// RBX::Soundscape::CollisionSoundType — material-pair key plus a rand()%3
/// variant (IDA 0x7f9bb8: (matA & 0xFF0F) | ((matB & 0xFF0F) << 16) with the
/// smaller material word low, variant added). Material ids in LoadSounds
/// order: 1 plastic, 2 wood, 6 ice, 4 metal, 5 grass, 3 stone.
#[derive(Clone, Copy, PartialEq, Eq, Hash, Debug)]
pub struct CollisionSoundType(pub u32);

impl CollisionSoundType {
    /// Canonical pair key with the variant folded in (IDA 0x7f9be6..0x7f9c2c).
    pub fn with_variant(mat_a: u32, mat_b: u32, variant: u32) -> CollisionSoundType {
        let (lo, hi) = if mat_a > mat_b {
            (mat_b, mat_a)
        } else {
            (mat_a, mat_b)
        };
        CollisionSoundType(((lo & 0xFF0F) | ((hi & 0xFF0F) << 16)).wrapping_add(variant))
    }
}

/// RBX::Soundscape::CollisionSoundManager — FMOD system/group words plus the
/// type -> sound map at +8 (IDA 0x7f9bb8/0x7fca48; std::map -> HashMap, null
/// shared_ptr -> None).
#[derive(Clone, Default)]
pub struct CollisionSoundManager {
    pub fmod_system: u32,
...
/// plastic/wood/ice/metal/grass/stone order, 3 variants each.
pub const COLLISION_SOUND_TABLE: &[(u32, &str)] = &[
    (0x01000100, "sounds/plasticplastic.ogg"),
    (0x01000101, "sounds/plasticplastic2.ogg"),
    (0x01000102, "sounds/plasticplastic3.ogg"),
    (0x02000100, "sounds/woodplastic.ogg"),
    (0x02000101, "sounds/woodplastic2.ogg"),
    (0x02000102, "sounds/woodplastic3.ogg"),
    (0x06000100, "sounds/plasticice.ogg"),
    (0x06000101, "sounds/plasticice2.ogg"),
    (0x06000102, "sounds/plasticice3.ogg"),
    (0x04000100, "sounds/plasticmetal.ogg"),
    (0x04000101, "sounds/plasticmetal2.ogg"),
    (0x04000102, "sounds/plasticmetal3.ogg"),
    (0x05000100, "sounds/plasticgrass.ogg"),
    (0x05000101, "sounds/plasticgrass2.ogg"),
    (0x05000102, "sounds/plasticgrass3.ogg"),
    (0x03000100, "sounds/plasticstone.ogg"),
    (0x03000101, "sounds/plasticstone2.ogg"),
    (0x03000102, "sounds/plasticstone3.ogg"),
    (0x02000200, "sounds/woodwood.ogg"),
    (0x02000201, "sounds/woodwood2.ogg"),
    (0x02000202, "sounds/woodwood3.ogg"),
    (0x06000200, "sounds/woodice.ogg"),
    (0x06000201, "sounds/woodice2.ogg"),
    (0x06000202, "sounds/woodice3.ogg"),
    (0x04000200, "sounds/woodmetal.ogg"),
    (0x04000201, "sounds/woodmetal2.ogg"),
    (0x04000202, "sounds/woodmetal3.ogg"),
    (0x05000200, "sounds/woodgrass.ogg"),
    (0x05000201, "sounds/woodgrass2.ogg"),
    (0x05000202, "sounds/woodgrass3.ogg"),
    (0x03000200, "sounds/woodstone.ogg"),
    (0x03000201, "sounds/woodstone2.ogg"),
    (0x03000202, "sounds/woodstone3.ogg"),
    (0x06000600, "sounds/ice.ogg"),
    (0x06000601, "sounds/ice2.ogg"),
    (0x06000602, "sounds/ice3.ogg"),
    (0x04000600, "sounds/icemetal.ogg"),
    (0x04000601, "sounds/icemetal2.ogg"),
    (0x04000602, "sounds/icemetal3.ogg"),
    (0x05000600, "sounds/icegrass.ogg"),
    (0x05000601, "sounds/icegrass2.ogg"),
    (0x05000602, "sounds/icegrass3.ogg"),
    (0x03000600, "sounds/icestone.ogg"),
    (0x03000601, "sounds/icestone2.ogg"),
    (0x03000602, "sounds/icestone3.ogg"),
    (0x04000400, "sounds/metal.ogg"),
    (0x04000401, "sounds/metal2.ogg"),
    (0x04000402, "sounds/metal3.ogg"),
    (0x05000400, "sounds/metalgrass.ogg"),
    (0x05000401, "sounds/metalgrass2.ogg"),
    (0x05000402, "sounds/metalgrass3.ogg"),
    (0x03000400, "sounds/metalstone.ogg"),
    (0x03000401, "sounds/metalstone2.ogg"),
    (0x03000402, "sounds/metalstone3.ogg"),
    (0x05000500, "sounds/grass.ogg"),
    (0x05000501, "sounds/grass2.ogg"),
    (0x05000502, "sounds/grass3.ogg"),
    (0x03000500, "sounds/grassstone.ogg"),
    (0x03000501, "sounds/grassstone2.ogg"),
    (0x03000502, "sounds/grassstone3.ogg"),
    (0x03000300, "sounds/stone.ogg"),
    (0x03000301, "sounds/stone2.ogg"),
    (0x03000302, "sounds/stone3.ogg"),
];
...
pub fn stub_7f937c(sound_id: SoundId) -> CollisionSound {
    // IDA 0x7f937c: SoundId copies (0x7f93f2/0x7f941c), shared_ptr<Sound>
    // reset over the fresh id (0x7f943a); on a non-empty asset Sound::get
    // (slot 5, 0x7f9454..0x7f945e) hits, the FMOD sound takes setSoundGroup
    // (0x7f947a) + set3DMinMaxDistance(min/max from the id payload at
    // +0x30/+0x34, 0x7f948a) and has_sound = 1 (0x7f946c), else has_sound = 0
    // (0x7f9494) with StandardOut printfs on the failure paths
    // (0x7f94b6/0x7f95da). Host: the Sound::get hit is the non-empty asset;
    // the FMOD words and the 3D distances live on the FMOD side.
    CollisionSound {
        has_sound: !sound_id.asset.is_empty(),
        sound_id,
...
pub fn stub_7f9bb8(
    ...
    // IDA 0x7f9bb8: PartInstance::fromPrimitive (0x7f9bd0); the fw material
    // words fold into the canonical pair key (0x7f9bd6..0x7f9c0a, smaller
    // word low); variant = rand() % 3 via the 0x55555556 SMMUL idiom
    // (0x7f9c0e..0x7f9c2c) added to the key; exact RB-tree match over the
    // manager map at +8 (0x7f9c34..0x7f9c5a, miss -> return 0x7f9cd2); then
    // getLinearVelocity (0x7f9c5e), getPartSizeXml (0x7f9c70), the
    // IsPartPlayer gate (0x7f9c86, 0x7fcf00), the [prim+0xAC] > 1 touch
    // cooldown gate (0x7f9c8c..0x7f9c92), the -120.0 volume floor (0x7f9c9c),
    // and CollisionSound::Play(system, group, pos, vel, volume) (0x7f9cce).
    // Host: the part/physics words arrive as parameters and the FMOD volume
    // math stays with the Play call; the map hit is returned for it.
    if is_part_player || touch_cooldown > 1 {
        return None;
    }
    let key = CollisionSoundType::with_variant(mat_a, mat_b, seed % 3);
    manager.sounds.get(&key.0).and_then(|slot| slot.clone())
}
...
pub fn stub_7f9e70(manager: &mut CollisionSoundManager) {
    // IDA 0x7f9e70: 63 LoadSound(type, path, instance) calls over the
    // upper-triangular material matrix (0x7f9ee4..0x7face8). Host: replay
    // COLLISION_SOUND_TABLE in image order.
    for &(key, path) in COLLISION_SOUND_TABLE {
        stub_7fca48(manager, key, path);
    }
}
...
pub fn stub_7fca48(manager: &mut CollisionSoundManager, key: u32, asset: &str) {
    // IDA 0x7fca48: ContentId::fromAssets(path) (0x7fca70, host: the asset
    // text), SoundId::SoundId(contentId) (0x7fcaa6), new CollisionSound
    // (0x7fcaf0), shared_ptr adopt (0x7fcb02), map::operator[] + assign
    // (0x7fcb22/0x7fcb2c, host: the entry API, cf. stub_7fd084).
    let sound = stub_7f937c(SoundId {
        asset: asset.to_owned(),
        extra: 0,
    });
    *stub_7fd084(manager, key) = Some(SharedPtr::new(sound));
}
...
pub fn stub_7fd084(
    manager: &mut CollisionSoundManager,
    key: u32,
) -> &mut Option<SharedPtr<CollisionSound>> {
    // IDA 0x7fd084: map::operator[] — lower_bound plus insert-on-miss of a
    // default (null) shared_ptr, returning the slot. Host: entry API over
    // the sounds map (None is the null shared_ptr).
    manager.sounds.entry(key).or_default()
}

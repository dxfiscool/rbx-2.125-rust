...
pub fn stub_065d400(part: &PartSurfaceData) -> SurfaceVariant {
    // IDA 0x65d400 (`SurfaceEnumPropDescriptor<3, SurfaceType>::
    // getVariant`): reads the inner value, tags it with the
    // `SurfaceType` singleton and placement-moves it in. Same as
    // the face-2 twin at 0x659ea4.
    SurfaceVariant::SurfaceType(part.faces[3].surface_type)
}
...
pub fn stub_065d428(part: &mut PartSurfaceData, variant: &SurfaceVariant) {
    // IDA 0x65d428 (`SurfaceEnumPropDescriptor<3, SurfaceType>::
    // setVariant`): any-cast-or-convert then inner set, same as the
    // face-2 twin at 0x659ecc (decompile same length 2118). Host:
    // the convert-or-throw into the face-3 slot.
    part.faces[3].surface_type = stub_0658e24(variant);
}
...
pub fn stub_065d580(first: &PartSurfaceData, second: &mut PartSurfaceData) {
    // IDA 0x65d580 (`SurfaceEnumPropDescriptor<3, SurfaceType>::
    // copyValue`): inner `getValue` + inner `setValue`, same as the
    // face-2 twin at 0x65a024. Host: copy the face-3 type slot.
    second.faces[3].surface_type = first.faces[3].surface_type;
}
...
pub fn stub_065d5a4() -> bool {
    // IDA 0x65d5a4 (`SurfaceEnumPropDescriptor<3, SurfaceType>::
    // hasStringValue`): returns 1 — always stringable.
    true
}
...
pub fn stub_065d5a8(part: &PartSurfaceData) -> String {
    // IDA 0x65d5a8 (`SurfaceEnumPropDescriptor<3, SurfaceType>::
    // getStringValue`): singleton once + inner `getValue` +
    // `convertToString`, same as the face-2 twin at 0x65a04c
    // (decompile same length 1431). Host: the grounded item name.
    surface_type_name(part.faces[3].surface_type).to_owned()
}
...
pub fn stub_065d5f8(part: &mut PartSurfaceData, name: &str) -> bool {
    // IDA 0x65d5f8 (`SurfaceEnumPropDescriptor<3, SurfaceType>::
    // setStringValue(string)`): lookup + convert + conditional
    // inner set, same as the face-2 twin at 0x65a09c. Host: table
    // position decides.
    match SURFACE_TYPE_ITEMS.iter().position(|(n, _)| *n == name) {
        Some(index) => {
            part.faces[3].surface_type = SURFACE_TYPE_ITEMS[index].1;
            true
        }
        None => false,
    }
...
pub fn stub_065d65c(part: &PartSurfaceData, out: &mut XmlIntSlot) -> i32 {
    // IDA 0x65d65c (`SurfaceEnumPropDescriptor<3, SurfaceType>::
    // writeValue`): inner `getValue`, `clearValue`, tag `5` at +16,
    // value at +20, returns 5 — same as the face-2 twin at 0x65a100.
    out.value_type = 0;
    out.value_type = 5;
    out.int_value = part.faces[3].surface_type as i32;
    5
}
...
pub fn stub_065d67c(part: &mut PartSurfaceData, xml: &XmlReadValue) {
    // IDA 0x65d67c (`SurfaceEnumPropDescriptor<3, SurfaceType>::
    // readValue`): xsi:nil early-out, string pair with fallthrough,
    // raw int set, else `ReleaseAssert(false)` (Surface.cpp line
    ...
    match xml {
        XmlReadValue::Nil => {}
        XmlReadValue::Int(value) => {
            part.faces[3].surface_type = *value as u32;
        }
        XmlReadValue::Text(text) => {
            if stub_065d5f8(part, text) {
                return;
            }
            if flog_asserts() {
                panic!("false file: Client/App/v8datamodel/Surface.cpp line: 313 (IDA 0x65d67c)");
            }
        }
        XmlReadValue::Other => {
            if flog_asserts() {
...
pub fn stub_065d8d4(part: &PartSurfaceData) -> i32 {
    // IDA 0x65d8d4 (`SurfaceEnumPropDescriptor<3, SurfaceType>::
    // getIndexValue`, disasm singleton once + inner `getValue` +
    // tail-call `EnumDesc::convertToIndex`): the item index of the
    // live value (host: stub_065a544). Same as the face-2 twin at
    // 0x65a378.
    stub_065a544(part.faces[3].surface_type as i32)
}
...
pub fn stub_065d91c(part: &mut PartSurfaceData, index: u32) -> bool {
    // IDA 0x65d91c (`SurfaceEnumPropDescriptor<3, SurfaceType>::
    // setIndexValue`): `count > index` gates storing the indexed
    // item's value, same as the face-2 twin at 0x65a3c0. Host:
    // table read decides.
    match SURFACE_TYPE_ITEMS.get(index as usize) {
        Some((_, value)) => {
            part.faces[3].surface_type = *value;
            true
        }
        None => false,
    }
...
pub fn stub_065d978(part: &PartSurfaceData) -> u32 {
    // IDA 0x65d978 (`SurfaceEnumPropDescriptor<3, SurfaceType>::
    // getEnumValue`): inner `getValue` through the +44 `GetSet`
    // (host: stub_065daa8). Host: the face-3 slot.
    part.faces[3].surface_type
}
...
pub fn stub_065d980(part: &mut PartSurfaceData, value: u32) -> bool {
    // IDA 0x65d980 (`SurfaceEnumPropDescriptor<3, SurfaceType>::
    // setEnumValue`): `find_if` membership + conditional inner set,
...
pub fn stub_065d9f4(part: &PartSurfaceData) -> Option<u32> {
    // IDA 0x65d9f4 (`SurfaceEnumPropDescriptor<3, SurfaceType>::
    // getEnumItem`, disasm singleton once + inner `getValue` +
    // `convertToItem`): the table entry for the live value. Same
    // as the face-2 twin at 0x65a498.
    SURFACE_TYPE_ITEMS
        .iter()
        .position(|(_, v)| *v == part.faces[3].surface_type)
        .map(|i| i as u32)
}
...
pub fn stub_065da44(part: &mut PartSurfaceData, name: &str) -> bool {
    // IDA 0x65da44 (`SurfaceEnumPropDescriptor<3, SurfaceType>::
    // setStringValue(Name)`): convert + conditional inner set, same
    // as the face-2 twin at 0x65a4e8. Host: the string twin in
    // this file.
    stub_065d5f8(part, name)
}
...
pub fn stub_065daa0() -> bool {
    // IDA 0x65daa0 (`SurfaceGetSet<3, SurfaceType>::isReadOnly`):
    // returns 0 — always readable.
    false
}
...
pub fn stub_065daa4() -> bool {
    // IDA 0x65daa4 (`SurfaceGetSet<3, SurfaceType>::isWriteOnly`):
    // returns 0 — always writable.
    false
}
...
pub fn stub_065daa8(part: &PartSurfaceData) -> u32 {
    // IDA 0x65daa8 (`SurfaceGetSet<3, SurfaceType>::getValue`): the
    // member-pointer resolve tail-calling the getter with the face
    // baked in (`, 3)`, verified in the decompile). Same shape as
...
pub fn stub_065dac8(part: &mut PartSurfaceData, value: u32) {
    // IDA 0x65dac8 (`SurfaceGetSet<3, SurfaceType>::setValue`): the
    // member-pointer resolve tail-calling the setter with
    // `(instance, 3, value)`. Same shape as the face-2 twin at
    // 0x65a5dc.
    part.faces[3].surface_type = value;
...
pub fn stub_065daec(
    ...
    // IDA 0x65daec (`SurfacePropDescriptor<4, float>` ctor): same
    // `classDescriptor` + impl + base-init shape as the face-2 twin
    // at 0x65937c (decompile same length 2306), with the face baked
    // in as 4. Host: the cutover with face 4.
    FaceFloatProp::new(4, slot, name, category, functionality, permissions)
}
...
pub fn stub_065dc2c() -> bool {
    // IDA 0x65dc2c (`SurfaceGetSet<4, float>::isReadOnly`):
    // returns 0 — always readable.
    false
}
...
pub fn stub_065dc30() -> bool {
    // IDA 0x65dc30 (`SurfaceGetSet<4, float>::isWriteOnly`):
    // returns 0 — always writable.
    false
}
...
pub fn stub_065dc34(part: &PartSurfaceData, slot: FaceFloatSlot) -> f32 {
    // IDA 0x65dc34 (`SurfaceGetSet<4, float>::getValue`): the
    // member-pointer resolve tail-calling the getter with the face
    // baked in (`, 4)`, verified in the decompile). Same shape as
    // the face-2 twin at 0x6594c4.
    part.faces[4].float_slot(slot)
}
...
pub fn stub_065dc54(part: &mut PartSurfaceData, slot: FaceFloatSlot, value: f32) {
    // IDA 0x65dc54 (`SurfaceGetSet<4, float>::setValue`): the
    // member-pointer resolve tail-calling the setter with
    // `(instance, 4, value)`. Same shape as the face-2 twin at
    // 0x6594e4.
    part.faces[4].set_float_slot(slot, value);
}
...
pub fn stub_065dc78(name: &str, category: &str, functionality: u32) -> FaceInputProp {
    // IDA 0x65dc78 (`SurfaceEnumPropDescriptor<4, InputType>`
    // ctor): same `classDescriptor` + `EnumDesc` singleton +
    // base-init + impl shape as the face-2 twin at 0x659508
    // (decompile same length 2675), with the face baked in as 4.
    // Host: the cutover with face 4.
    FaceInputProp::new(4, name, category, functionality)
}
...
pub fn stub_065dd50() -> bool {
    // IDA 0x65dd50 (`SurfaceEnumPropDescriptor<4, InputType>::
    // isReadOnly`): delegates to the inner face-4 `GetSet` at +44 —
    // always readable (same shape as the face-2 twin at 0x6595e0).
    false
}
...
pub fn stub_065dd60() -> bool {
    // IDA 0x65dd60 (`SurfaceEnumPropDescriptor<4, InputType>::
    // isWriteOnly`): delegates to the inner face-4 `GetSet` at +44 —
    // always writable (same shape as the face-2 twin at 0x6595f0).
    false
}
...
pub fn stub_065dd70(first: &PartSurfaceData, second: &PartSurfaceData) -> bool {
    // IDA 0x65dd70 (`SurfaceEnumPropDescriptor<4, InputType>::
    // equalValues`): reads the inner value for both instances via
    // the +44 `GetSet` and compares. Host: compare the face-4
    // input slots.
    first.faces[4].surface_input == second.faces[4].surface_input
}
...
pub fn stub_065dd98(part: &PartSurfaceData) -> SurfaceVariant {
    // IDA 0x65dd98 (`SurfaceEnumPropDescriptor<4, InputType>::
    // getVariant`): reads the inner value, tags it with the
    // `InputType` singleton and placement-moves it in. Same as the
    // face-2 twin at 0x659628.
    SurfaceVariant::SurfaceInput(part.faces[4].surface_input)
}
...
pub fn stub_065ddc0(part: &mut PartSurfaceData, variant: &SurfaceVariant) {
    // IDA 0x65ddc0 (`SurfaceEnumPropDescriptor<4, InputType>::
    // setVariant`): any-cast-or-convert then inner set, same as the
    // face-2 twin at 0x659650 (decompile same length 2201). Host:
    // the convert-or-throw into the face-4 slot.
    part.faces[4].surface_input = stub_0658f8c(variant);
}
...
pub fn stub_065df18(first: &PartSurfaceData, second: &mut PartSurfaceData) {
    // IDA 0x65df18 (`SurfaceEnumPropDescriptor<4, InputType>::
    // copyValue`): inner `getValue` + inner `setValue`, same as the
    // face-2 twin at 0x6597a8. Host: copy the face-4 slot.
    second.faces[4].surface_input = first.faces[4].surface_input;
}
...
pub fn stub_065df3c() -> bool {
    // IDA 0x65df3c (`SurfaceEnumPropDescriptor<4, InputType>::
    // hasStringValue`): returns 1 — always stringable.
    true
}
...
pub fn stub_065df40(part: &PartSurfaceData) -> String {
    // IDA 0x65df40 (`SurfaceEnumPropDescriptor<4, InputType>::
    // getStringValue`): singleton once + inner `getValue` +
    // `convertToString`, same as the face-2 twin at 0x6597d0.
    // Host: the grounded item name.
    input_type_name(part.faces[4].surface_input).to_owned()
}
...
pub fn stub_065df90(part: &mut PartSurfaceData, name: &str) -> bool {
    // IDA 0x65df90 (`SurfaceEnumPropDescriptor<4, InputType>::
    // setStringValue(string)`): lookup + convert + conditional
    // inner set, same as the face-2 twin at 0x659820. Host: table
    // position decides.
    match INPUT_TYPE_ITEMS.iter().position(|(n, _)| *n == name) {
        Some(index) => {
            part.faces[4].surface_input = INPUT_TYPE_ITEMS[index].1;
            true
        }
        None => false,
    }
...
pub fn stub_065dff4(part: &PartSurfaceData, out: &mut XmlIntSlot) -> i32 {
    // IDA 0x65dff4 (`SurfaceEnumPropDescriptor<4, InputType>::
    // writeValue`): inner `getValue`, `clearValue`, tag `5` at +16,
    // value at +20, returns 5 — same as the face-2 twin at 0x659884.
    out.value_type = 0;
    out.value_type = 5;
    out.int_value = part.faces[4].surface_input as i32;
    5
}
...
pub fn stub_065e014(part: &mut PartSurfaceData, xml: &XmlReadValue) {
    // IDA 0x65e014 (`SurfaceEnumPropDescriptor<4, InputType>::
    // readValue`): xsi:nil early-out, string pair with fallthrough,
    // raw int set, else `ReleaseAssert(false)` (Surface.cpp line
    // 313) — same as the face-2 twin at 0x6598a4 (decompile same
    // length 5208).
    match xml {
        XmlReadValue::Nil => {}
        XmlReadValue::Int(value) => {
            part.faces[4].surface_input = *value as u32;
        }
        XmlReadValue::Text(text) => {
            if stub_065df90(part, text) {
                return;
            }
            if flog_asserts() {
                panic!("false file: Client/App/v8datamodel/Surface.cpp line: 313 (IDA 0x65e014)");
            }
        }
        XmlReadValue::Other => {
            if flog_asserts() {
...
pub fn stub_065e26c(part: &PartSurfaceData) -> i32 {
    // IDA 0x65e26c (`SurfaceEnumPropDescriptor<4, InputType>::
    // getIndexValue`, disasm singleton once + inner `getValue` +
    // tail-call `EnumDesc::convertToIndex`): the item index of the
    // live value (host: stub_0659cc8). Same as the face-2 twin at
    // 0x659afc.
    stub_0659cc8(part.faces[4].surface_input as i32)
}
...
pub fn stub_065e2b4(part: &mut PartSurfaceData, index: u32) -> bool {
    // IDA 0x65e2b4 (`SurfaceEnumPropDescriptor<4, InputType>::
    // setIndexValue`): `count > index` gates storing the indexed
    // item's value, same as the face-2 twin at 0x659b44. Host:
    // table read decides.
    match INPUT_TYPE_ITEMS.get(index as usize) {
        Some((_, value)) => {
            part.faces[4].surface_input = *value;
            true
        }
        None => false,
    }
...
pub fn stub_065e310(part: &PartSurfaceData) -> u32 {
    // IDA 0x65e310 (`SurfaceEnumPropDescriptor<4, InputType>::
    // getEnumValue`): inner `getValue` through the +44 `GetSet`
    // (host: stub_065e440). Host: the face-4 slot.
    part.faces[4].surface_input
}
...
pub fn stub_065e318(part: &mut PartSurfaceData, value: u32) -> bool {
    // IDA 0x65e318 (`SurfaceEnumPropDescriptor<4, InputType>::
    // setEnumValue`): `find_if` membership + conditional inner set,
    // same as the face-2 twin at 0x659ba8. Host: table membership
    // decides.
    if INPUT_TYPE_ITEMS.iter().any(|(_, v)| *v == value) {
...
pub fn stub_065e38c(part: &PartSurfaceData) -> Option<u32> {
    // IDA 0x65e38c (`SurfaceEnumPropDescriptor<4, InputType>::
    // getEnumItem`, disasm singleton once + inner `getValue` +
    // `convertToItem`): the table entry for the live value. Same
    // as the face-2 twin at 0x659c1c.
    INPUT_TYPE_ITEMS
        .iter()
        .position(|(_, v)| *v == part.faces[4].surface_input)
        .map(|i| i as u32)
}
...
pub fn stub_065e3dc(part: &mut PartSurfaceData, name: &str) -> bool {
    // IDA 0x65e3dc (`SurfaceEnumPropDescriptor<4, InputType>::
    // setStringValue(Name)`): convert + conditional inner set, same
    // as the face-2 twin at 0x659c6c. Host: the string twin in
    // this file.
    stub_065df90(part, name)
}
...
pub fn stub_065e438() -> bool {
    // IDA 0x65e438 (`SurfaceGetSet<4, InputType>::isReadOnly`):
    // returns 0 — always readable.
    false
}
...
pub fn stub_065e43c() -> bool {
    // IDA 0x65e43c (`SurfaceGetSet<4, InputType>::isWriteOnly`):
    // returns 0 — always writable.
    false
}
...
pub fn stub_065e440(part: &PartSurfaceData) -> u32 {
    // IDA 0x65e440 (`SurfaceGetSet<4, InputType>::getValue`): the
    // member-pointer resolve tail-calling the getter with the face
    // baked in (`, 4)`, verified in the decompile). Same shape as
    // the face-2 twin at 0x659d40.
    part.faces[4].surface_input
...
pub fn stub_065e460(part: &mut PartSurfaceData, value: u32) {
    // IDA 0x65e460 (`SurfaceGetSet<4, InputType>::setValue`): the
    // member-pointer resolve tail-calling the setter with
    // `(instance, 4, value)`. Same shape as the face-2 twin at
    // 0x659d60.
    part.faces[4].surface_input = value;
...
pub fn stub_065e484(name: &str, category: &str, functionality: u32) -> FaceTypeProp {
    // IDA 0x65e484 (`SurfaceEnumPropDescriptor<4, SurfaceType>`
    // ctor): same `classDescriptor` + `EnumDesc` singleton + impl
    // shape as the face-2 twin at 0x659d84 (decompile same length
    // 2547), with the face baked in as 4. Host: the cutover with
    // face 4.
    FaceTypeProp::new(4, name, category, functionality)
}
...
pub fn stub_065e55c() -> bool {
    // IDA 0x65e55c (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // isReadOnly`): delegates to the inner face-4 `GetSet` at +44 —
    // always readable (same shape as the face-2 twin at 0x659e5c).
    false
}
...
pub fn stub_065e56c() -> bool {
    // IDA 0x65e56c (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // isWriteOnly`): delegates to the inner face-4 `GetSet` at +44 —
    // always writable (same shape as the face-2 twin at 0x659e6c).
    false
}
...
pub fn stub_065e57c(first: &PartSurfaceData, second: &PartSurfaceData) -> bool {
    // IDA 0x65e57c (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // equalValues`): reads the inner value for both instances via
    // the +44 `GetSet` and compares. Host: compare the face-4 type
    // slots.
    first.faces[4].surface_type == second.faces[4].surface_type
}
...
pub fn stub_065e5a4(part: &PartSurfaceData) -> SurfaceVariant {
    // IDA 0x65e5a4 (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // getVariant`): reads the inner value, tags it with the
    // `SurfaceType` singleton and placement-moves it in. Same as
    // the face-2 twin at 0x659ea4.
    SurfaceVariant::SurfaceType(part.faces[4].surface_type)
}
...
pub fn stub_065e5cc(part: &mut PartSurfaceData, variant: &SurfaceVariant) {
    // IDA 0x65e5cc (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // setVariant`): any-cast-or-convert then inner set, same as the
    // face-2 twin at 0x659ecc (decompile same length 2118). Host:
    // the convert-or-throw into the face-4 slot.
    part.faces[4].surface_type = stub_0658e24(variant);
}
...
pub fn stub_065e724(first: &PartSurfaceData, second: &mut PartSurfaceData) {
    // IDA 0x65e724 (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // copyValue`): inner `getValue` + inner `setValue`, same as the
    // face-2 twin at 0x65a024. Host: copy the face-4 type slot.
    second.faces[4].surface_type = first.faces[4].surface_type;
}
...
pub fn stub_065e748() -> bool {
    // IDA 0x65e748 (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // hasStringValue`): returns 1 — always stringable.
    true
}
...
pub fn stub_065e74c(part: &PartSurfaceData) -> String {
    // IDA 0x65e74c (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // getStringValue`): singleton once + inner `getValue` +
    // `convertToString`, same as the face-2 twin at 0x65a04c
    // (decompile same length 1431). Host: the grounded item name.
    surface_type_name(part.faces[4].surface_type).to_owned()
}
...
pub fn stub_065e79c(part: &mut PartSurfaceData, name: &str) -> bool {
    // IDA 0x65e79c (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // setStringValue(string)`): lookup + convert + conditional
    // inner set, same as the face-2 twin at 0x65a09c. Host: table
    // position decides.
    match SURFACE_TYPE_ITEMS.iter().position(|(n, _)| *n == name) {
        Some(index) => {
            part.faces[4].surface_type = SURFACE_TYPE_ITEMS[index].1;
            true
        }
        None => false,
    }
...
pub fn stub_065e800(part: &PartSurfaceData, out: &mut XmlIntSlot) -> i32 {
    // IDA 0x65e800 (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // writeValue`): inner `getValue`, `clearValue`, tag `5` at +16,
    // value at +20, returns 5 — same as the face-2 twin at 0x65a100.
    out.value_type = 0;
    out.value_type = 5;
    out.int_value = part.faces[4].surface_type as i32;
    5
}
...
pub fn stub_065e820(part: &mut PartSurfaceData, xml: &XmlReadValue) {
    // IDA 0x65e820 (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // readValue`): xsi:nil early-out, string pair with fallthrough,
    // raw int set, else `ReleaseAssert(false)` (Surface.cpp line
    // 313) — same as the face-2 twin at 0x65a120 (decompile length
    // 5111 vs 5112, same shape).
    match xml {
        XmlReadValue::Nil => {}
        XmlReadValue::Int(value) => {
            part.faces[4].surface_type = *value as u32;
        }
        XmlReadValue::Text(text) => {
            if stub_065e79c(part, text) {
                return;
            }
            if flog_asserts() {
                panic!("false file: Client/App/v8datamodel/Surface.cpp line: 313 (IDA 0x65e820)");
            }
        }
        XmlReadValue::Other => {
            if flog_asserts() {
...
pub fn stub_065ea78(part: &PartSurfaceData) -> i32 {
    // IDA 0x65ea78 (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // getIndexValue`, disasm singleton once + inner `getValue` +
    // tail-call `EnumDesc::convertToIndex`): the item index of the
    // live value (host: stub_065a544). Same as the face-2 twin at
    // 0x65a378.
    stub_065a544(part.faces[4].surface_type as i32)
}
...
pub fn stub_065eac0(part: &mut PartSurfaceData, index: u32) -> bool {
    // IDA 0x65eac0 (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // setIndexValue`): `count > index` gates storing the indexed
    // item's value, same as the face-2 twin at 0x65a3c0. Host:
    // table read decides.
    match SURFACE_TYPE_ITEMS.get(index as usize) {
        Some((_, value)) => {
            part.faces[4].surface_type = *value;
            true
        }
        None => false,
    }
...
pub fn stub_065eb1c(part: &PartSurfaceData) -> u32 {
    // IDA 0x65eb1c (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // getEnumValue`): inner `getValue` through the +44 `GetSet`
    // (host: stub_065ec4c). Host: the face-4 slot.
    part.faces[4].surface_type
}
...
pub fn stub_065eb24(part: &mut PartSurfaceData, value: u32) -> bool {
    // IDA 0x65eb24 (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // setEnumValue`): `find_if` membership + conditional inner set,
    // same as the face-2 twin at 0x65a424. Host: table membership
    // decides.
    if SURFACE_TYPE_ITEMS.iter().any(|(_, v)| *v == value) {
...
pub fn stub_065eb98(part: &PartSurfaceData) -> Option<u32> {
    // IDA 0x65eb98 (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // getEnumItem`, disasm singleton once + inner `getValue` +
    // `convertToItem`): the table entry for the live value. Same
    // as the face-2 twin at 0x65a498.
    SURFACE_TYPE_ITEMS
        .iter()
        .position(|(_, v)| *v == part.faces[4].surface_type)
        .map(|i| i as u32)
}
...
pub fn stub_065ebe8(part: &mut PartSurfaceData, name: &str) -> bool {
    // IDA 0x65ebe8 (`SurfaceEnumPropDescriptor<4, SurfaceType>::
    // setStringValue(Name)`): convert + conditional inner set, same
    // as the face-2 twin at 0x65a4e8. Host: the string twin in
    // this file.
    stub_065e79c(part, name)
}
...
pub fn stub_065ec44() -> bool {
    // IDA 0x65ec44 (`SurfaceGetSet<4, SurfaceType>::isReadOnly`):
    // returns 0 — always readable.
    false
}
...
pub fn stub_065ec48() -> bool {
    // IDA 0x65ec48 (`SurfaceGetSet<4, SurfaceType>::isWriteOnly`):
    // returns 0 — always writable.
    false
}
...
pub fn stub_065ec4c(part: &PartSurfaceData) -> u32 {
    // IDA 0x65ec4c (`SurfaceGetSet<4, SurfaceType>::getValue`): the
    // member-pointer resolve tail-calling the getter with the face
    // baked in (`, 4)`, verified in the decompile). Same shape as
    // the face-2 twin at 0x65a5bc.
    part.faces[4].surface_type
...
pub fn stub_065ec6c(part: &mut PartSurfaceData, value: u32) {
    // IDA 0x65ec6c (`SurfaceGetSet<4, SurfaceType>::setValue`): the
    // member-pointer resolve tail-calling the setter with
    // `(instance, 4, value)`. Same shape as the face-2 twin at
    // 0x65a5dc.
    part.faces[4].surface_type = value;
...
pub fn stub_065ec90(
    ...
    // IDA 0x65ec90 (`SurfacePropDescriptor<1, float>` ctor): same
    // `classDescriptor` + impl + base-init shape as the face-2 twin
    // at 0x65937c (decompile same length 2306), with the face baked
    // in as 1. Host: the cutover with face 1.
    FaceFloatProp::new(1, slot, name, category, functionality, permissions)
}
...
pub fn stub_065edd0() -> bool {
    // IDA 0x65edd0 (`SurfaceGetSet<1, float>::isReadOnly`):
    // returns 0 — always readable.
    false
}
...
pub fn stub_065edd4() -> bool {
    // IDA 0x65edd4 (`SurfaceGetSet<1, float>::isWriteOnly`):
    // returns 0 — always writable.
    false
}

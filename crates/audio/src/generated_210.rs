//! audio generated_210 — 100 stubs EA-sorted asc gap filler not yet in audio (FMOD|Sound|Audio 2541/2541 complete, gap filler)
//! Source: ida/export.json (85545 funcs) EA-sorted asc next 100 not in audio | rbx_core::SharedPtr not boost
//! Range 0x4ce618..0x4d1ec4 | existing 28213 -> 28313 distinct
//! Batch: 100 stubs | // 0xADDR — mangled + #[doc(alias = "demangled")] + todo!("0xADDR mangled")

#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]

use rbx_core::SharedPtr;

const _: () = {
    let _ = core::marker::PhantomData::<SharedPtr<u8>>;
};

// 0x4ce618 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE14convertToValueEmRNS0_7VariantE
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToValue(unsigned long,RBX::Reflection::Variant &)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_4ce618() -> ! {
    todo!("0x4ce618 __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE14convertToValueEmRNS0_7VariantE")
}

// 0x4ce674 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE15convertToStringEmRSs
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToString(unsigned long,std::string &)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE15convertToStringEmRSs")]
pub fn stub_4ce674() -> ! {
    todo!("0x4ce674 __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE15convertToStringEmRSs")
}

// 0x4ce7b8 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE15convertToStringERKS3_
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToString(RBX::Feature::LeftRight const&)const
// type: int __fastcall(int, int, int, int, struct _Unwind_Exception *lpuexcpt, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToString(RBX::Feature::LeftRight const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE15convertToStringERKS3_")]
pub fn stub_4ce7b8() -> ! {
    todo!("0x4ce7b8 __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE15convertToStringERKS3_")
}

// 0x4ce958 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_7Feature9LeftRightEEERS3_RKT_
// demangled: rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Feature::LeftRight>(RBX::Feature::LeftRight const&)
// type: 
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Feature::LeftRight>(RBX::Feature::LeftRight const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_7Feature9LeftRightEEERS3_RKT_")]
pub fn stub_4ce958() -> ! {
    todo!("0x4ce958 __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_7Feature9LeftRightEEERS3_RKT_")
}

// 0x4ce9a8 — __ZN3rbx14implementation12typed_holderIN3RBX7Feature9LeftRightEE9singletonEv
// demangled: rbx::implementation::typed_holder<RBX::Feature::LeftRight>::singleton(void)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::Feature::LeftRight>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX7Feature9LeftRightEE9singletonEv")]
pub fn stub_4ce9a8() -> ! {
    todo!("0x4ce9a8 __ZN3rbx14implementation12typed_holderIN3RBX7Feature9LeftRightEE9singletonEv")
}

// 0x4cea14 — __ZN3rbx14implementation12typed_holderIN3RBX7Feature9LeftRightEE14construct_funcEPKcPc
// demangled: rbx::implementation::typed_holder<RBX::Feature::LeftRight>::construct_func(char const*,char *)
// type: _DWORD *__fastcall(_DWORD *result, _DWORD *)
#[doc(alias = "rbx::implementation::typed_holder<RBX::Feature::LeftRight>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX7Feature9LeftRightEE14construct_funcEPKcPc")]
pub fn stub_4cea14() -> ! {
    todo!("0x4cea14 __ZN3rbx14implementation12typed_holderIN3RBX7Feature9LeftRightEE14construct_funcEPKcPc")
}

// 0x4cea20 — __ZN3rbx14implementation12typed_holderIN3RBX7Feature9LeftRightEE13destruct_funcEPc
// demangled: rbx::implementation::typed_holder<RBX::Feature::LeftRight>::destruct_func(char *)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::Feature::LeftRight>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX7Feature9LeftRightEE13destruct_funcEPc")]
pub fn stub_4cea20() -> ! {
    todo!("0x4cea20 __ZN3rbx14implementation12typed_holderIN3RBX7Feature9LeftRightEE13destruct_funcEPc")
}

// 0x4cea24 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE13convertToItemERKS3_
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToItem(RBX::Feature::LeftRight const&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToItem(RBX::Feature::LeftRight const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE13convertToItemERKS3_")]
pub fn stub_4cea24() -> ! {
    todo!("0x4cea24 __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE13convertToItemERKS3_")
}

// 0x4ceaf0 — __ZN3rbx8any_castIRKN3RBX7Feature9LeftRightENS1_7Region3EEET_RNS_13placement_anyIT0_EE
// demangled: RBX::Feature::LeftRight const& rbx::any_cast<RBX::Feature::LeftRight const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)
// type: 
#[doc(alias = "RBX::Feature::LeftRight const& rbx::any_cast<RBX::Feature::LeftRight const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX7Feature9LeftRightENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_4ceaf0() -> ! {
    todo!("0x4ceaf0 __ZN3rbx8any_castIRKN3RBX7Feature9LeftRightENS1_7Region3EEET_RNS_13placement_anyIT0_EE")
}

// 0x4cebe0 — __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE14convertToValueERKNS_4NameERS3_
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToValue(RBX::Name const&,RBX::Feature::LeftRight&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::LeftRight>::convertToValue(RBX::Name const&,RBX::Feature::LeftRight&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE14convertToValueERKNS_4NameERS3_")]
pub fn stub_4cebe0() -> ! {
    todo!("0x4cebe0 __ZNK3RBX10Reflection8EnumDescINS_7Feature9LeftRightEE14convertToValueERKNS_4NameERS3_")
}

// 0x4cec5c — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_7Feature9LeftRightEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
// demangled: std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::Feature::LeftRight>,std::_Select1st<std::pair<RBX::Name const* const,RBX::Feature::LeftRight>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::Feature::LeftRight>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::Feature::LeftRight>> *)
// type: 
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::Feature::LeftRight>,std::_Select1st<std::pair<RBX::Name const* const,RBX::Feature::LeftRight>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::Feature::LeftRight>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::Feature::LeftRight>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_7Feature9LeftRightEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_4cec5c() {
    // IDA 0x4cec5c: shared_ptr ctor/op= (addref new, release old; derived-to-base coercion). Arc move — carrier no-op.
}

// 0x4cec84 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_7Feature5InOutEEEE13initSingletonEv
// demangled: RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Feature::InOut> const>::initSingleton(void)
// type: 
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Feature::InOut> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_7Feature5InOutEEEE13initSingletonEv")]
pub fn stub_4cec84() -> ! {
    todo!("0x4cec84 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_7Feature5InOutEEEE13initSingletonEv")
}

// 0x4cec88 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_7Feature5InOutEEEE14doGetSingletonEv
// demangled: RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Feature::InOut> const>::doGetSingleton(void)
// type: 
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Feature::InOut> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_7Feature5InOutEEEE14doGetSingletonEv")]
pub fn stub_4cec88() -> ! {
    todo!("0x4cec88 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_7Feature5InOutEEEE14doGetSingletonEv")
}

// 0x4ced78 — __ZN3RBX10Reflection8EnumDescINS_7Feature5InOutEED1Ev
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::InOut>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::InOut>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_7Feature5InOutEED1Ev")]
pub fn stub_4ced78() {
    // IDA 0x4ced78: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4ced7c — __ZN3RBX10Reflection8EnumDescINS_7Feature5InOutEED2Ev
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::InOut>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::InOut>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_7Feature5InOutEED2Ev")]
pub fn stub_4ced7c() {
    // IDA 0x4ced7c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4cef50 — __ZN3RBX10Reflection8EnumDescINS_7Feature5InOutEED0Ev
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::InOut>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::InOut>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_7Feature5InOutEED0Ev")]
pub fn stub_4cef50() {
    // IDA 0x4cef50: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4ceff0 — __ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE6lookupEPKc
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::InOut>::lookup(char const*)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::InOut>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE6lookupEPKc")]
pub fn stub_4ceff0() -> ! {
    todo!("0x4ceff0 __ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE6lookupEPKc")
}

// 0x4cf020 — __ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE6lookupERKNS0_7VariantE
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::InOut>::lookup(RBX::Reflection::Variant const&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::InOut>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE6lookupERKNS0_7VariantE")]
pub fn stub_4cf020() -> ! {
    todo!("0x4cf020 __ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE6lookupERKNS0_7VariantE")
}

// 0x4cf040 — __ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE14convertToValueEmRNS0_7VariantE
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::InOut>::convertToValue(unsigned long,RBX::Reflection::Variant &)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::InOut>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_4cf040() -> ! {
    todo!("0x4cf040 __ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE14convertToValueEmRNS0_7VariantE")
}

// 0x4cf09c — __ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE15convertToStringEmRSs
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::InOut>::convertToString(unsigned long,std::string &)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::InOut>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE15convertToStringEmRSs")]
pub fn stub_4cf09c() -> ! {
    todo!("0x4cf09c __ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE15convertToStringEmRSs")
}

// 0x4cf1e0 — __ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE15convertToStringERKS3_
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::InOut>::convertToString(RBX::Feature::InOut const&)const
// type: int __fastcall(int, int, int, int, struct _Unwind_Exception *lpuexcpt, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::InOut>::convertToString(RBX::Feature::InOut const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE15convertToStringERKS3_")]
pub fn stub_4cf1e0() -> ! {
    todo!("0x4cf1e0 __ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE15convertToStringERKS3_")
}

// 0x4cf380 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_7Feature5InOutEEERS3_RKT_
// demangled: rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Feature::InOut>(RBX::Feature::InOut const&)
// type: 
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Feature::InOut>(RBX::Feature::InOut const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_7Feature5InOutEEERS3_RKT_")]
pub fn stub_4cf380() -> ! {
    todo!("0x4cf380 __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_7Feature5InOutEEERS3_RKT_")
}

// 0x4cf3d0 — __ZN3rbx14implementation12typed_holderIN3RBX7Feature5InOutEE9singletonEv
// demangled: rbx::implementation::typed_holder<RBX::Feature::InOut>::singleton(void)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::Feature::InOut>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX7Feature5InOutEE9singletonEv")]
pub fn stub_4cf3d0() -> ! {
    todo!("0x4cf3d0 __ZN3rbx14implementation12typed_holderIN3RBX7Feature5InOutEE9singletonEv")
}

// 0x4cf43c — __ZN3rbx14implementation12typed_holderIN3RBX7Feature5InOutEE14construct_funcEPKcPc
// demangled: rbx::implementation::typed_holder<RBX::Feature::InOut>::construct_func(char const*,char *)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::Feature::InOut>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX7Feature5InOutEE14construct_funcEPKcPc")]
pub fn stub_4cf43c() -> ! {
    todo!("0x4cf43c __ZN3rbx14implementation12typed_holderIN3RBX7Feature5InOutEE14construct_funcEPKcPc")
}

// 0x4cf448 — __ZN3rbx14implementation12typed_holderIN3RBX7Feature5InOutEE13destruct_funcEPc
// demangled: rbx::implementation::typed_holder<RBX::Feature::InOut>::destruct_func(char *)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::Feature::InOut>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX7Feature5InOutEE13destruct_funcEPc")]
pub fn stub_4cf448() -> ! {
    todo!("0x4cf448 __ZN3rbx14implementation12typed_holderIN3RBX7Feature5InOutEE13destruct_funcEPc")
}

// 0x4cf44c — __ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE13convertToItemERKS3_
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::InOut>::convertToItem(RBX::Feature::InOut const&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::InOut>::convertToItem(RBX::Feature::InOut const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE13convertToItemERKS3_")]
pub fn stub_4cf44c() -> ! {
    todo!("0x4cf44c __ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE13convertToItemERKS3_")
}

// 0x4cf518 — __ZN3rbx8any_castIRKN3RBX7Feature5InOutENS1_7Region3EEET_RNS_13placement_anyIT0_EE
// demangled: RBX::Feature::InOut const& rbx::any_cast<RBX::Feature::InOut const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)
// type: 
#[doc(alias = "RBX::Feature::InOut const& rbx::any_cast<RBX::Feature::InOut const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX7Feature5InOutENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_4cf518() -> ! {
    todo!("0x4cf518 __ZN3rbx8any_castIRKN3RBX7Feature5InOutENS1_7Region3EEET_RNS_13placement_anyIT0_EE")
}

// 0x4cf608 — __ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE14convertToValueERKNS_4NameERS3_
// demangled: RBX::Reflection::EnumDesc<RBX::Feature::InOut>::convertToValue(RBX::Name const&,RBX::Feature::InOut&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::Feature::InOut>::convertToValue(RBX::Name const&,RBX::Feature::InOut&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE14convertToValueERKNS_4NameERS3_")]
pub fn stub_4cf608() -> ! {
    todo!("0x4cf608 __ZNK3RBX10Reflection8EnumDescINS_7Feature5InOutEE14convertToValueERKNS_4NameERS3_")
}

// 0x4cf684 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_7Feature5InOutEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
// demangled: std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::Feature::InOut>,std::_Select1st<std::pair<RBX::Name const* const,RBX::Feature::InOut>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::Feature::InOut>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::Feature::InOut>> *)
// type: 
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::Feature::InOut>,std::_Select1st<std::pair<RBX::Name const* const,RBX::Feature::InOut>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::Feature::InOut>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::Feature::InOut>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_7Feature5InOutEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_4cf684() {
    // IDA 0x4cf684: shared_ptr ctor/op= (addref new, release old; derived-to-base coercion). Arc move — carrier no-op.
}

// 0x4cf6ac — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9EThrottle13EThrottleTypeEEEE13initSingletonEv
// demangled: RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::EThrottle::EThrottleType> const>::initSingleton(void)
// type: 
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::EThrottle::EThrottleType> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9EThrottle13EThrottleTypeEEEE13initSingletonEv")]
pub fn stub_4cf6ac() -> ! {
    todo!("0x4cf6ac __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9EThrottle13EThrottleTypeEEEE13initSingletonEv")
}

// 0x4cf6b0 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9EThrottle13EThrottleTypeEEEE14doGetSingletonEv
// demangled: RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::EThrottle::EThrottleType> const>::doGetSingleton(void)
// type: 
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::EThrottle::EThrottleType> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9EThrottle13EThrottleTypeEEEE14doGetSingletonEv")]
pub fn stub_4cf6b0() -> ! {
    todo!("0x4cf6b0 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9EThrottle13EThrottleTypeEEEE14doGetSingletonEv")
}

// 0x4cf7a0 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_13DataModelMesh7LODTypeEEEE13initSingletonEv
// demangled: RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType> const>::initSingleton(void)
// type: 
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_13DataModelMesh7LODTypeEEEE13initSingletonEv")]
pub fn stub_4cf7a0() -> ! {
    todo!("0x4cf7a0 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_13DataModelMesh7LODTypeEEEE13initSingletonEv")
}

// 0x4cf7a4 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_13DataModelMesh7LODTypeEEEE14doGetSingletonEv
// demangled: RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType> const>::doGetSingleton(void)
// type: 
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_13DataModelMesh7LODTypeEEEE14doGetSingletonEv")]
pub fn stub_4cf7a4() -> ! {
    todo!("0x4cf7a4 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_13DataModelMesh7LODTypeEEEE14doGetSingletonEv")
}

// 0x4cf894 — __ZN3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEED1Ev
// demangled: RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEED1Ev")]
pub fn stub_4cf894() {
    // IDA 0x4cf894: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4cf898 — __ZN3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEED2Ev
// demangled: RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEED2Ev")]
pub fn stub_4cf898() {
    // IDA 0x4cf898: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4cfa6c — __ZN3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEED0Ev
// demangled: RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEED0Ev")]
pub fn stub_4cfa6c() {
    // IDA 0x4cfa6c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4cfb0c — __ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE6lookupEPKc
// demangled: RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::lookup(char const*)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE6lookupEPKc")]
pub fn stub_4cfb0c() -> ! {
    todo!("0x4cfb0c __ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE6lookupEPKc")
}

// 0x4cfb3c — __ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE6lookupERKNS0_7VariantE
// demangled: RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::lookup(RBX::Reflection::Variant const&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE6lookupERKNS0_7VariantE")]
pub fn stub_4cfb3c() -> ! {
    todo!("0x4cfb3c __ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE6lookupERKNS0_7VariantE")
}

// 0x4cfb5c — __ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE14convertToValueEmRNS0_7VariantE
// demangled: RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::convertToValue(unsigned long,RBX::Reflection::Variant &)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_4cfb5c() -> ! {
    todo!("0x4cfb5c __ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE14convertToValueEmRNS0_7VariantE")
}

// 0x4cfbb8 — __ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE15convertToStringEmRSs
// demangled: RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::convertToString(unsigned long,std::string &)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE15convertToStringEmRSs")]
pub fn stub_4cfbb8() -> ! {
    todo!("0x4cfbb8 __ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE15convertToStringEmRSs")
}

// 0x4cfcfc — __ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE15convertToStringERKS3_
// demangled: RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::convertToString(RBX::DataModelMesh::LODType const&)const
// type: int __fastcall(int, int, int, int, struct _Unwind_Exception *lpuexcpt, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::convertToString(RBX::DataModelMesh::LODType const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE15convertToStringERKS3_")]
pub fn stub_4cfcfc() -> ! {
    todo!("0x4cfcfc __ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE15convertToStringERKS3_")
}

// 0x4cfe9c — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_13DataModelMesh7LODTypeEEERS3_RKT_
// demangled: rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::DataModelMesh::LODType>(RBX::DataModelMesh::LODType const&)
// type: 
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::DataModelMesh::LODType>(RBX::DataModelMesh::LODType const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_13DataModelMesh7LODTypeEEERS3_RKT_")]
pub fn stub_4cfe9c() -> ! {
    todo!("0x4cfe9c __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_13DataModelMesh7LODTypeEEERS3_RKT_")
}

// 0x4cfeec — __ZN3rbx14implementation12typed_holderIN3RBX13DataModelMesh7LODTypeEE9singletonEv
// demangled: rbx::implementation::typed_holder<RBX::DataModelMesh::LODType>::singleton(void)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::DataModelMesh::LODType>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX13DataModelMesh7LODTypeEE9singletonEv")]
pub fn stub_4cfeec() -> ! {
    todo!("0x4cfeec __ZN3rbx14implementation12typed_holderIN3RBX13DataModelMesh7LODTypeEE9singletonEv")
}

// 0x4cff58 — __ZN3rbx14implementation12typed_holderIN3RBX13DataModelMesh7LODTypeEE14construct_funcEPKcPc
// demangled: rbx::implementation::typed_holder<RBX::DataModelMesh::LODType>::construct_func(char const*,char *)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::DataModelMesh::LODType>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX13DataModelMesh7LODTypeEE14construct_funcEPKcPc")]
pub fn stub_4cff58() -> ! {
    todo!("0x4cff58 __ZN3rbx14implementation12typed_holderIN3RBX13DataModelMesh7LODTypeEE14construct_funcEPKcPc")
}

// 0x4cff64 — __ZN3rbx14implementation12typed_holderIN3RBX13DataModelMesh7LODTypeEE13destruct_funcEPc
// demangled: rbx::implementation::typed_holder<RBX::DataModelMesh::LODType>::destruct_func(char *)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::DataModelMesh::LODType>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX13DataModelMesh7LODTypeEE13destruct_funcEPc")]
pub fn stub_4cff64() -> ! {
    todo!("0x4cff64 __ZN3rbx14implementation12typed_holderIN3RBX13DataModelMesh7LODTypeEE13destruct_funcEPc")
}

// 0x4cff68 — __ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE13convertToItemERKS3_
// demangled: RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::convertToItem(RBX::DataModelMesh::LODType const&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::convertToItem(RBX::DataModelMesh::LODType const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE13convertToItemERKS3_")]
pub fn stub_4cff68() -> ! {
    todo!("0x4cff68 __ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE13convertToItemERKS3_")
}

// 0x4d0034 — __ZN3rbx8any_castIRKN3RBX13DataModelMesh7LODTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE
// demangled: RBX::DataModelMesh::LODType const& rbx::any_cast<RBX::DataModelMesh::LODType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)
// type: 
#[doc(alias = "RBX::DataModelMesh::LODType const& rbx::any_cast<RBX::DataModelMesh::LODType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX13DataModelMesh7LODTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_4d0034() -> ! {
    todo!("0x4d0034 __ZN3rbx8any_castIRKN3RBX13DataModelMesh7LODTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE")
}

// 0x4d0124 — __ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE14convertToValueERKNS_4NameERS3_
// demangled: RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::convertToValue(RBX::Name const&,RBX::DataModelMesh::LODType&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::DataModelMesh::LODType>::convertToValue(RBX::Name const&,RBX::DataModelMesh::LODType&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE14convertToValueERKNS_4NameERS3_")]
pub fn stub_4d0124() -> ! {
    todo!("0x4d0124 __ZNK3RBX10Reflection8EnumDescINS_13DataModelMesh7LODTypeEE14convertToValueERKNS_4NameERS3_")
}

// 0x4d01a0 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_13DataModelMesh7LODTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
// demangled: std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::DataModelMesh::LODType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::DataModelMesh::LODType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::DataModelMesh::LODType>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::DataModelMesh::LODType>> *)
// type: 
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::DataModelMesh::LODType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::DataModelMesh::LODType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::DataModelMesh::LODType>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::DataModelMesh::LODType>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_13DataModelMesh7LODTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_4d01a0() {
    // IDA 0x4d01a0: shared_ptr ctor/op= (addref new, release old; derived-to-base coercion). Arc move — carrier no-op.
}

// 0x4d01c8 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_16LegacyController9InputTypeEEEE13initSingletonEv
// demangled: RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::LegacyController::InputType> const>::initSingleton(void)
// type: 
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::LegacyController::InputType> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_16LegacyController9InputTypeEEEE13initSingletonEv")]
pub fn stub_4d01c8() -> ! {
    todo!("0x4d01c8 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_16LegacyController9InputTypeEEEE13initSingletonEv")
}

// 0x4d01cc — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_16LegacyController9InputTypeEEEE14doGetSingletonEv
// demangled: RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::LegacyController::InputType> const>::doGetSingleton(void)
// type: 
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::LegacyController::InputType> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_16LegacyController9InputTypeEEEE14doGetSingletonEv")]
pub fn stub_4d01cc() -> ! {
    todo!("0x4d01cc __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_16LegacyController9InputTypeEEEE14doGetSingletonEv")
}

// 0x4d02bc — __ZN3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEED1Ev
// demangled: RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEED1Ev")]
pub fn stub_4d02bc() {
    // IDA 0x4d02bc: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4d02c0 — __ZN3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEED2Ev
// demangled: RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEED2Ev")]
pub fn stub_4d02c0() {
    // IDA 0x4d02c0: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4d0494 — __ZN3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEED0Ev
// demangled: RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEED0Ev")]
pub fn stub_4d0494() {
    // IDA 0x4d0494: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4d0534 — __ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE6lookupEPKc
// demangled: RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::lookup(char const*)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE6lookupEPKc")]
pub fn stub_4d0534() -> ! {
    todo!("0x4d0534 __ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE6lookupEPKc")
}

// 0x4d0564 — __ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE6lookupERKNS0_7VariantE
// demangled: RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::lookup(RBX::Reflection::Variant const&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE6lookupERKNS0_7VariantE")]
pub fn stub_4d0564() -> ! {
    todo!("0x4d0564 __ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE6lookupERKNS0_7VariantE")
}

// 0x4d0584 — __ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE14convertToValueEmRNS0_7VariantE
// demangled: RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::convertToValue(unsigned long,RBX::Reflection::Variant &)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_4d0584() -> ! {
    todo!("0x4d0584 __ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE14convertToValueEmRNS0_7VariantE")
}

// 0x4d05e0 — __ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE15convertToStringEmRSs
// demangled: RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::convertToString(unsigned long,std::string &)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE15convertToStringEmRSs")]
pub fn stub_4d05e0() -> ! {
    todo!("0x4d05e0 __ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE15convertToStringEmRSs")
}

// 0x4d0724 — __ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE15convertToStringERKS3_
// demangled: RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::convertToString(RBX::LegacyController::InputType const&)const
// type: int __fastcall(int, int, int, int, struct _Unwind_Exception *lpuexcpt, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::convertToString(RBX::LegacyController::InputType const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE15convertToStringERKS3_")]
pub fn stub_4d0724() -> ! {
    todo!("0x4d0724 __ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE15convertToStringERKS3_")
}

// 0x4d08c4 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_16LegacyController9InputTypeEEERS3_RKT_
// demangled: rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::LegacyController::InputType>(RBX::LegacyController::InputType const&)
// type: 
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::LegacyController::InputType>(RBX::LegacyController::InputType const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_16LegacyController9InputTypeEEERS3_RKT_")]
pub fn stub_4d08c4() -> ! {
    todo!("0x4d08c4 __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_16LegacyController9InputTypeEEERS3_RKT_")
}

// 0x4d0914 — __ZN3rbx14implementation12typed_holderIN3RBX16LegacyController9InputTypeEE9singletonEv
// demangled: rbx::implementation::typed_holder<RBX::LegacyController::InputType>::singleton(void)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::LegacyController::InputType>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX16LegacyController9InputTypeEE9singletonEv")]
pub fn stub_4d0914() -> ! {
    todo!("0x4d0914 __ZN3rbx14implementation12typed_holderIN3RBX16LegacyController9InputTypeEE9singletonEv")
}

// 0x4d0980 — __ZN3rbx14implementation12typed_holderIN3RBX16LegacyController9InputTypeEE14construct_funcEPKcPc
// demangled: rbx::implementation::typed_holder<RBX::LegacyController::InputType>::construct_func(char const*,char *)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::LegacyController::InputType>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX16LegacyController9InputTypeEE14construct_funcEPKcPc")]
pub fn stub_4d0980() -> ! {
    todo!("0x4d0980 __ZN3rbx14implementation12typed_holderIN3RBX16LegacyController9InputTypeEE14construct_funcEPKcPc")
}

// 0x4d098c — __ZN3rbx14implementation12typed_holderIN3RBX16LegacyController9InputTypeEE13destruct_funcEPc
// demangled: rbx::implementation::typed_holder<RBX::LegacyController::InputType>::destruct_func(char *)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::LegacyController::InputType>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX16LegacyController9InputTypeEE13destruct_funcEPc")]
pub fn stub_4d098c() -> ! {
    todo!("0x4d098c __ZN3rbx14implementation12typed_holderIN3RBX16LegacyController9InputTypeEE13destruct_funcEPc")
}

// 0x4d0990 — __ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE13convertToItemERKS3_
// demangled: RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::convertToItem(RBX::LegacyController::InputType const&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::convertToItem(RBX::LegacyController::InputType const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE13convertToItemERKS3_")]
pub fn stub_4d0990() -> ! {
    todo!("0x4d0990 __ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE13convertToItemERKS3_")
}

// 0x4d0a5c — __ZN3rbx8any_castIRKN3RBX16LegacyController9InputTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE
// demangled: RBX::LegacyController::InputType const& rbx::any_cast<RBX::LegacyController::InputType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)
// type: 
#[doc(alias = "RBX::LegacyController::InputType const& rbx::any_cast<RBX::LegacyController::InputType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX16LegacyController9InputTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_4d0a5c() -> ! {
    todo!("0x4d0a5c __ZN3rbx8any_castIRKN3RBX16LegacyController9InputTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE")
}

// 0x4d0b4c — __ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE14convertToValueERKNS_4NameERS3_
// demangled: RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::convertToValue(RBX::Name const&,RBX::LegacyController::InputType&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::LegacyController::InputType>::convertToValue(RBX::Name const&,RBX::LegacyController::InputType&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE14convertToValueERKNS_4NameERS3_")]
pub fn stub_4d0b4c() -> ! {
    todo!("0x4d0b4c __ZNK3RBX10Reflection8EnumDescINS_16LegacyController9InputTypeEE14convertToValueERKNS_4NameERS3_")
}

// 0x4d0bc8 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_16LegacyController9InputTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
// demangled: std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::LegacyController::InputType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::LegacyController::InputType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::LegacyController::InputType>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::LegacyController::InputType>> *)
// type: 
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::LegacyController::InputType>,std::_Select1st<std::pair<RBX::Name const* const,RBX::LegacyController::InputType>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::LegacyController::InputType>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::LegacyController::InputType>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_16LegacyController9InputTypeEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_4d0bc8() {
    // IDA 0x4d0bc8: shared_ptr ctor/op= (addref new, release old; derived-to-base coercion). Arc move — carrier no-op.
}

// 0x4d0bf0 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_6Camera10CameraModeEEEE13initSingletonEv
// demangled: RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Camera::CameraMode> const>::initSingleton(void)
// type: 
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Camera::CameraMode> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_6Camera10CameraModeEEEE13initSingletonEv")]
pub fn stub_4d0bf0() -> ! {
    todo!("0x4d0bf0 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_6Camera10CameraModeEEEE13initSingletonEv")
}

// 0x4d0bf4 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_6Camera10CameraModeEEEE14doGetSingletonEv
// demangled: RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Camera::CameraMode> const>::doGetSingleton(void)
// type: 
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::Camera::CameraMode> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_6Camera10CameraModeEEEE14doGetSingletonEv")]
pub fn stub_4d0bf4() -> ! {
    todo!("0x4d0bf4 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_6Camera10CameraModeEEEE14doGetSingletonEv")
}

// 0x4d0ce4 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiObject20TweenEasingDirectionEEEE13initSingletonEv
// demangled: RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection> const>::initSingleton(void)
// type: 
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiObject20TweenEasingDirectionEEEE13initSingletonEv")]
pub fn stub_4d0ce4() -> ! {
    todo!("0x4d0ce4 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiObject20TweenEasingDirectionEEEE13initSingletonEv")
}

// 0x4d0ce8 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiObject20TweenEasingDirectionEEEE14doGetSingletonEv
// demangled: RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection> const>::doGetSingleton(void)
// type: 
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiObject20TweenEasingDirectionEEEE14doGetSingletonEv")]
pub fn stub_4d0ce8() -> ! {
    todo!("0x4d0ce8 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiObject20TweenEasingDirectionEEEE14doGetSingletonEv")
}

// 0x4d0dd8 — __ZN3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEED1Ev
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEED1Ev")]
pub fn stub_4d0dd8() {
    // IDA 0x4d0dd8: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4d0ddc — __ZN3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEED2Ev
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEED2Ev")]
pub fn stub_4d0ddc() {
    // IDA 0x4d0ddc: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4d0fb0 — __ZN3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEED0Ev
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEED0Ev")]
pub fn stub_4d0fb0() {
    // IDA 0x4d0fb0: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4d1050 — __ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE6lookupEPKc
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::lookup(char const*)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE6lookupEPKc")]
pub fn stub_4d1050() -> ! {
    todo!("0x4d1050 __ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE6lookupEPKc")
}

// 0x4d1080 — __ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE6lookupERKNS0_7VariantE
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::lookup(RBX::Reflection::Variant const&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE6lookupERKNS0_7VariantE")]
pub fn stub_4d1080() -> ! {
    todo!("0x4d1080 __ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE6lookupERKNS0_7VariantE")
}

// 0x4d10a0 — __ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE14convertToValueEmRNS0_7VariantE
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::convertToValue(unsigned long,RBX::Reflection::Variant &)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_4d10a0() -> ! {
    todo!("0x4d10a0 __ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE14convertToValueEmRNS0_7VariantE")
}

// 0x4d10fc — __ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE15convertToStringEmRSs
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::convertToString(unsigned long,std::string &)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE15convertToStringEmRSs")]
pub fn stub_4d10fc() -> ! {
    todo!("0x4d10fc __ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE15convertToStringEmRSs")
}

// 0x4d1240 — __ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE15convertToStringERKS3_
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::convertToString(RBX::GuiObject::TweenEasingDirection const&)const
// type: int __fastcall(int, int, int, int, struct _Unwind_Exception *lpuexcpt, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::convertToString(RBX::GuiObject::TweenEasingDirection const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE15convertToStringERKS3_")]
pub fn stub_4d1240() -> ! {
    todo!("0x4d1240 __ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE15convertToStringERKS3_")
}

// 0x4d13e0 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_9GuiObject20TweenEasingDirectionEEERS3_RKT_
// demangled: rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::GuiObject::TweenEasingDirection>(RBX::GuiObject::TweenEasingDirection const&)
// type: 
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::GuiObject::TweenEasingDirection>(RBX::GuiObject::TweenEasingDirection const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_9GuiObject20TweenEasingDirectionEEERS3_RKT_")]
pub fn stub_4d13e0() -> ! {
    todo!("0x4d13e0 __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_9GuiObject20TweenEasingDirectionEEERS3_RKT_")
}

// 0x4d1430 — __ZN3rbx14implementation12typed_holderIN3RBX9GuiObject20TweenEasingDirectionEE9singletonEv
// demangled: rbx::implementation::typed_holder<RBX::GuiObject::TweenEasingDirection>::singleton(void)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::GuiObject::TweenEasingDirection>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX9GuiObject20TweenEasingDirectionEE9singletonEv")]
pub fn stub_4d1430() -> ! {
    todo!("0x4d1430 __ZN3rbx14implementation12typed_holderIN3RBX9GuiObject20TweenEasingDirectionEE9singletonEv")
}

// 0x4d149c — __ZN3rbx14implementation12typed_holderIN3RBX9GuiObject20TweenEasingDirectionEE14construct_funcEPKcPc
// demangled: rbx::implementation::typed_holder<RBX::GuiObject::TweenEasingDirection>::construct_func(char const*,char *)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::GuiObject::TweenEasingDirection>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX9GuiObject20TweenEasingDirectionEE14construct_funcEPKcPc")]
pub fn stub_4d149c() -> ! {
    todo!("0x4d149c __ZN3rbx14implementation12typed_holderIN3RBX9GuiObject20TweenEasingDirectionEE14construct_funcEPKcPc")
}

// 0x4d14a8 — __ZN3rbx14implementation12typed_holderIN3RBX9GuiObject20TweenEasingDirectionEE13destruct_funcEPc
// demangled: rbx::implementation::typed_holder<RBX::GuiObject::TweenEasingDirection>::destruct_func(char *)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::GuiObject::TweenEasingDirection>::destruct_func(char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX9GuiObject20TweenEasingDirectionEE13destruct_funcEPc")]
pub fn stub_4d14a8() -> ! {
    todo!("0x4d14a8 __ZN3rbx14implementation12typed_holderIN3RBX9GuiObject20TweenEasingDirectionEE13destruct_funcEPc")
}

// 0x4d14ac — __ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE13convertToItemERKS3_
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::convertToItem(RBX::GuiObject::TweenEasingDirection const&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::convertToItem(RBX::GuiObject::TweenEasingDirection const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE13convertToItemERKS3_")]
pub fn stub_4d14ac() -> ! {
    todo!("0x4d14ac __ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE13convertToItemERKS3_")
}

// 0x4d1578 — __ZN3rbx8any_castIRKN3RBX9GuiObject20TweenEasingDirectionENS1_7Region3EEET_RNS_13placement_anyIT0_EE
// demangled: RBX::GuiObject::TweenEasingDirection const& rbx::any_cast<RBX::GuiObject::TweenEasingDirection const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)
// type: 
#[doc(alias = "RBX::GuiObject::TweenEasingDirection const& rbx::any_cast<RBX::GuiObject::TweenEasingDirection const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
#[doc(alias = "__ZN3rbx8any_castIRKN3RBX9GuiObject20TweenEasingDirectionENS1_7Region3EEET_RNS_13placement_anyIT0_EE")]
pub fn stub_4d1578() -> ! {
    todo!("0x4d1578 __ZN3rbx8any_castIRKN3RBX9GuiObject20TweenEasingDirectionENS1_7Region3EEET_RNS_13placement_anyIT0_EE")
}

// 0x4d1668 — __ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE14convertToValueERKNS_4NameERS3_
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::convertToValue(RBX::Name const&,RBX::GuiObject::TweenEasingDirection&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenEasingDirection>::convertToValue(RBX::Name const&,RBX::GuiObject::TweenEasingDirection&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE14convertToValueERKNS_4NameERS3_")]
pub fn stub_4d1668() -> ! {
    todo!("0x4d1668 __ZNK3RBX10Reflection8EnumDescINS_9GuiObject20TweenEasingDirectionEE14convertToValueERKNS_4NameERS3_")
}

// 0x4d16e4 — __ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_9GuiObject20TweenEasingDirectionEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E
// demangled: std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::GuiObject::TweenEasingDirection>,std::_Select1st<std::pair<RBX::Name const* const,RBX::GuiObject::TweenEasingDirection>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::GuiObject::TweenEasingDirection>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::GuiObject::TweenEasingDirection>> *)
// type: 
#[doc(alias = "std::_Rb_tree<RBX::Name const*,std::pair<RBX::Name const* const,RBX::GuiObject::TweenEasingDirection>,std::_Select1st<std::pair<RBX::Name const* const,RBX::GuiObject::TweenEasingDirection>>,std::less<RBX::Name const*>,std::allocator<std::pair<RBX::Name const* const,RBX::GuiObject::TweenEasingDirection>>>::_M_erase(std::_Rb_tree_node<std::pair<RBX::Name const* const,RBX::GuiObject::TweenEasingDirection>> *)")]
#[doc(alias = "__ZNSt8_Rb_treeIPKN3RBX4NameESt4pairIKS3_NS0_9GuiObject20TweenEasingDirectionEESt10_Select1stIS8_ESt4lessIS3_ESaIS8_EE8_M_eraseEPSt13_Rb_tree_nodeIS8_E")]
pub fn stub_4d16e4() {
    // IDA 0x4d16e4: shared_ptr ctor/op= (addref new, release old; derived-to-base coercion). Arc move — carrier no-op.
}

// 0x4d170c — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiObject11TweenStatusEEEE13initSingletonEv
// demangled: RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus> const>::initSingleton(void)
// type: 
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus> const>::initSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiObject11TweenStatusEEEE13initSingletonEv")]
pub fn stub_4d170c() -> ! {
    todo!("0x4d170c __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiObject11TweenStatusEEEE13initSingletonEv")
}

// 0x4d1710 — __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiObject11TweenStatusEEEE14doGetSingletonEv
// demangled: RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus> const>::doGetSingleton(void)
// type: 
#[doc(alias = "RBX::Reflection::Singleton<RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus> const>::doGetSingleton(void)")]
#[doc(alias = "__ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiObject11TweenStatusEEEE14doGetSingletonEv")]
pub fn stub_4d1710() -> ! {
    todo!("0x4d1710 __ZN3RBX10Reflection9SingletonIKNS0_8EnumDescINS_9GuiObject11TweenStatusEEEE14doGetSingletonEv")
}

// 0x4d1800 — __ZN3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEED1Ev
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEED1Ev")]
pub fn stub_4d1800() {
    // IDA 0x4d1800: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4d1804 — __ZN3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEED2Ev
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEED2Ev")]
pub fn stub_4d1804() {
    // IDA 0x4d1804: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4d19d8 — __ZN3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEED0Ev
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::~EnumDesc()
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::~EnumDesc()")]
#[doc(alias = "__ZN3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEED0Ev")]
pub fn stub_4d19d8() {
    // IDA 0x4d19d8: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4d1a78 — __ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE6lookupEPKc
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::lookup(char const*)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::lookup(char const*)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE6lookupEPKc")]
pub fn stub_4d1a78() -> ! {
    todo!("0x4d1a78 __ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE6lookupEPKc")
}

// 0x4d1aa8 — __ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE6lookupERKNS0_7VariantE
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::lookup(RBX::Reflection::Variant const&)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::lookup(RBX::Reflection::Variant const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE6lookupERKNS0_7VariantE")]
pub fn stub_4d1aa8() -> ! {
    todo!("0x4d1aa8 __ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE6lookupERKNS0_7VariantE")
}

// 0x4d1ac8 — __ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE14convertToValueEmRNS0_7VariantE
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::convertToValue(unsigned long,RBX::Reflection::Variant &)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::convertToValue(unsigned long,RBX::Reflection::Variant &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE14convertToValueEmRNS0_7VariantE")]
pub fn stub_4d1ac8() -> ! {
    todo!("0x4d1ac8 __ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE14convertToValueEmRNS0_7VariantE")
}

// 0x4d1b24 — __ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE15convertToStringEmRSs
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::convertToString(unsigned long,std::string &)const
// type: 
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::convertToString(unsigned long,std::string &)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE15convertToStringEmRSs")]
pub fn stub_4d1b24() -> ! {
    todo!("0x4d1b24 __ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE15convertToStringEmRSs")
}

// 0x4d1c68 — __ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE15convertToStringERKS3_
// demangled: RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::convertToString(RBX::GuiObject::TweenStatus const&)const
// type: int __fastcall(int, int, int, int, struct _Unwind_Exception *lpuexcpt, int)
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::GuiObject::TweenStatus>::convertToString(RBX::GuiObject::TweenStatus const&)const")]
#[doc(alias = "__ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE15convertToStringERKS3_")]
pub fn stub_4d1c68() -> ! {
    todo!("0x4d1c68 __ZNK3RBX10Reflection8EnumDescINS_9GuiObject11TweenStatusEE15convertToStringERKS3_")
}

// 0x4d1e08 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_9GuiObject11TweenStatusEEERS3_RKT_
// demangled: rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::GuiObject::TweenStatus>(RBX::GuiObject::TweenStatus const&)
// type: 
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::GuiObject::TweenStatus>(RBX::GuiObject::TweenStatus const&)")]
#[doc(alias = "__ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_9GuiObject11TweenStatusEEERS3_RKT_")]
pub fn stub_4d1e08() -> ! {
    todo!("0x4d1e08 __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_9GuiObject11TweenStatusEEERS3_RKT_")
}

// 0x4d1e58 — __ZN3rbx14implementation12typed_holderIN3RBX9GuiObject11TweenStatusEE9singletonEv
// demangled: rbx::implementation::typed_holder<RBX::GuiObject::TweenStatus>::singleton(void)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::GuiObject::TweenStatus>::singleton(void)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX9GuiObject11TweenStatusEE9singletonEv")]
pub fn stub_4d1e58() -> ! {
    todo!("0x4d1e58 __ZN3rbx14implementation12typed_holderIN3RBX9GuiObject11TweenStatusEE9singletonEv")
}

// 0x4d1ec4 — __ZN3rbx14implementation12typed_holderIN3RBX9GuiObject11TweenStatusEE14construct_funcEPKcPc
// demangled: rbx::implementation::typed_holder<RBX::GuiObject::TweenStatus>::construct_func(char const*,char *)
// type: 
#[doc(alias = "rbx::implementation::typed_holder<RBX::GuiObject::TweenStatus>::construct_func(char const*,char *)")]
#[doc(alias = "__ZN3rbx14implementation12typed_holderIN3RBX9GuiObject11TweenStatusEE14construct_funcEPKcPc")]
pub fn stub_4d1ec4() -> ! {
    todo!("0x4d1ec4 __ZN3rbx14implementation12typed_holderIN3RBX9GuiObject11TweenStatusEE14construct_funcEPKcPc")
}


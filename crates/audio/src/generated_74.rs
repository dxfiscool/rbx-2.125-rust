//! audio generated_74 — next 100 stubs EA-sorted, from ida/export.json
//! Filter: FMOD|Sound|Audio exhausted (2541 distinct) — filler workspace EA-sorted asc, skip existing, rbx_core::SharedPtr not boost
//! Batch: 100 stubs | skeleton batch | range 0x6cb000..0x6d1310 EA-sorted asc filler after 0x6cae14, skip existing, rbx_core::SharedPtr not boost
//! Generated: 2026-09-01

#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]

use rbx_core::SharedPtr;

// Ensure SharedPtr is seen as used — mirrors boost::shared_ptr<T> -> rbx_core::SharedPtr<T>
const _: () = {
    let _ = core::marker::PhantomData::<SharedPtr<u8>>;
};

// 0x6cb000 — __ZN3RBX9Workspace10makeJointsEN5boost10shared_ptrIKSt6vectorINS2_INS_8InstanceEEESaIS5_EEEE
#[doc(alias = "RBX::Workspace::makeJoints(rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> const>)")]
pub fn stub_6cb000() -> ! {
    todo!("0x6cb000 RBX::Workspace::makeJoints(boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>> const>)")
}

// 0x6cb014 — __ZN3RBX9Workspace11breakJointsEN5boost10shared_ptrIKSt6vectorINS2_INS_8InstanceEEESaIS5_EEEE
#[doc(alias = "RBX::Workspace::breakJoints(rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> const>)")]
pub fn stub_6cb014() -> ! {
    todo!("0x6cb014 RBX::Workspace::breakJoints(boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>> const>)")
}

// 0x6cb028 — __ZN3RBX9Workspace18findPartsInRegion3ENS_7Region3EN5boost10shared_ptrINS_8InstanceEEEi
// type: int __fastcall(int, int, int, int, int, int, struct _Unwind_Exception *lpuexcpt, int, boost::detail::sp_counted_base *, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, boost::detail::sp_counted_base *, int, int, int, int)
#[doc(alias = "RBX::Workspace::findPartsInRegion3(RBX::Region3,rbx_core::SharedPtr<RBX::Instance>,int)")]
pub fn stub_6cb028() -> ! {
    todo!("0x6cb028 RBX::Workspace::findPartsInRegion3(RBX::Region3,boost::shared_ptr<RBX::Instance>,int)")
}

// 0x6cb188 — __ZN3RBX9Workspace14isRegion3EmptyENS_7Region3EN5boost10shared_ptrINS_8InstanceEEE
// type: int __fastcall(int, int, int, int, boost::detail::sp_counted_base *, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, boost::detail::sp_counted_base *, int, int, int, int)
#[doc(alias = "RBX::Workspace::isRegion3Empty(RBX::Region3,rbx_core::SharedPtr<RBX::Instance>)")]
pub fn stub_6cb188() -> ! {
    todo!("0x6cb188 RBX::Workspace::isRegion3Empty(RBX::Region3,boost::shared_ptr<RBX::Instance>)")
}

// 0x6cb2e8 — __ZN3RBX9Workspace18findPartsInRegion3ENS_7Region3EN5boost10shared_ptrIKSt6vectorINS3_INS_8InstanceEEESaIS6_EEEEi
// type: int __fastcall(int, int, int, int, int, struct _Unwind_Exception *lpuexcpt, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, char, int, int, int, int, boost::detail::sp_counted_base *, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int)
#[doc(alias = "RBX::Workspace::findPartsInRegion3(RBX::Region3,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> const>,int)")]
pub fn stub_6cb2e8() -> ! {
    todo!("0x6cb2e8 RBX::Workspace::findPartsInRegion3(RBX::Region3,boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>> const>,int)")
}

// 0x6cb58c — __ZN3RBX9Workspace14isRegion3EmptyENS_7Region3EN5boost10shared_ptrIKSt6vectorINS3_INS_8InstanceEEESaIS6_EEEE
// type: int __fastcall(boost::detail::sp_counted_base *, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, boost::detail::sp_counted_base *, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, boost::detail::sp_counted_base *, int, int, int, int)
#[doc(alias = "RBX::Workspace::isRegion3Empty(RBX::Region3,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> const>)")]
pub fn stub_6cb58c() -> ! {
    todo!("0x6cb58c RBX::Workspace::isRegion3Empty(RBX::Region3,boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>> const>)")
}

// 0x6cb718 — __ZNK3RBX9Workspace10getTerrainEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::getTerrain(void)const")]
pub fn stub_6cb718() -> ! {
    todo!("0x6cb718 RBX::Workspace::getTerrain(void)const")
}

// 0x6cb720 — __ZN3RBX9Workspace13zoomToExtentsEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::zoomToExtents(void)")]
pub fn stub_6cb720() -> ! {
    todo!("0x6cb720 RBX::Workspace::zoomToExtents(void)")
}

// 0x6cb73c — __ZNK3RBX9Workspace25getCurrentCameraDangerousEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::getCurrentCameraDangerous(void)const")]
pub fn stub_6cb73c() -> ! {
    todo!("0x6cb73c RBX::Workspace::getCurrentCameraDangerous(void)const")
}

// 0x6cb744 — __ZN3RBX9Workspace16setCurrentCameraEPNS_6CameraE
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, RBX::Camera *)
#[doc(alias = "RBX::Workspace::setCurrentCamera(RBX::Camera *)")]
pub fn stub_6cb744() -> ! {
    todo!("0x6cb744 RBX::Workspace::setCurrentCamera(RBX::Camera *)")
}

// 0x6cb8c4 — __ZN3RBX9Workspace17getRealPhysicsFPSEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::getRealPhysicsFPS(void)")]
pub fn stub_6cb8c4() -> ! {
    todo!("0x6cb8c4 RBX::Workspace::getRealPhysicsFPS(void)")
}

// 0x6cb8f8 — __ZN3RBX9Workspace20getPhysicsThrottlingEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::getPhysicsThrottling(void)")]
pub fn stub_6cb8f8() -> ! {
    todo!("0x6cb8f8 RBX::Workspace::getPhysicsThrottling(void)")
}

// 0x6cb920 — __ZN3RBX9Workspace16getNumAwakePartsEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::getNumAwakeParts(void)")]
pub fn stub_6cb920() -> ! {
    todo!("0x6cb920 RBX::Workspace::getNumAwakeParts(void)")
}

// 0x6cb974 — __ZN3RBX9WorkspaceC1EPNS_10IDataStateE
// type: int __fastcall(_DWORD, _DWORD)
#[doc(alias = "RBX::Workspace::Workspace(RBX::IDataState *)")]
pub fn stub_6cb974() -> ! {
    todo!("0x6cb974 RBX::Workspace::Workspace(RBX::IDataState *)")
}

// 0x6cc0b4 — __ZN3RBX9WorkspaceD0Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::~Workspace()")]
pub fn stub_6cc0b4() {
    // IDA 0x6cc0b4: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc160 — __ZN3RBX9WorkspaceD1Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::~Workspace()")]
pub fn stub_6cc160() {
    // IDA 0x6cc160: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc170 — __ZThn32_N3RBX9WorkspaceD0Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Workspace::~Workspace()")]
pub fn stub_6cc170() {
    // IDA 0x6cc170: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc178 — __ZThn36_N3RBX9WorkspaceD0Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Workspace::~Workspace()")]
pub fn stub_6cc178() {
    // IDA 0x6cc178: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc180 — __ZThn120_N3RBX9WorkspaceD0Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Workspace::~Workspace()")]
pub fn stub_6cc180() {
    // IDA 0x6cc180: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc188 — __ZThn280_N3RBX9WorkspaceD0Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Workspace::~Workspace()")]
pub fn stub_6cc188() {
    // IDA 0x6cc188: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc190 — __ZThn324_N3RBX9WorkspaceD0Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Workspace::~Workspace()")]
pub fn stub_6cc190() {
    // IDA 0x6cc190: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc198 — __ZThn356_N3RBX9WorkspaceD0Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Workspace::~Workspace()")]
pub fn stub_6cc198() {
    // IDA 0x6cc198: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc1a0 — __ZN3RBX9WorkspaceD2Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::~Workspace()")]
pub fn stub_6cc1a0() {
    // IDA 0x6cc1a0: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc71c — __ZThn32_N3RBX9WorkspaceD1Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Workspace::~Workspace()")]
pub fn stub_6cc71c() {
    // IDA 0x6cc71c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc72c — __ZThn36_N3RBX9WorkspaceD1Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Workspace::~Workspace()")]
pub fn stub_6cc72c() {
    // IDA 0x6cc72c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc73c — __ZThn120_N3RBX9WorkspaceD1Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Workspace::~Workspace()")]
pub fn stub_6cc73c() {
    // IDA 0x6cc73c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc74c — __ZThn280_N3RBX9WorkspaceD1Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Workspace::~Workspace()")]
pub fn stub_6cc74c() {
    // IDA 0x6cc74c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc760 — __ZThn324_N3RBX9WorkspaceD1Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Workspace::~Workspace()")]
pub fn stub_6cc760() {
    // IDA 0x6cc760: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc774 — __ZThn356_N3RBX9WorkspaceD1Ev
// type: void __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Workspace::~Workspace()")]
pub fn stub_6cc774() {
    // IDA 0x6cc774: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cc788 — __ZN3RBX9Workspace23computeExtentsWorldFastEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::computeExtentsWorldFast(void)")]
pub fn stub_6cc788() -> ! {
    todo!("0x6cc788 RBX::Workspace::computeExtentsWorldFast(void)")
}

// 0x6cc804 — __ZN3RBX9Workspace11onHeartbeatERKNS_9HeartbeatE
// type: int __fastcall(int, int, int, int, int, boost::detail::sp_counted_base *, int, int, int, int)
#[doc(alias = "RBX::Workspace::onHeartbeat(RBX::Heartbeat const&)")]
pub fn stub_6cc804() -> ! {
    todo!("0x6cc804 RBX::Workspace::onHeartbeat(RBX::Heartbeat const&)")
}

// 0x6ccaa8 — __ZN3RBX9Workspace15replenishCameraEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::replenishCamera(void)")]
pub fn stub_6ccaa8() -> ! {
    todo!("0x6ccaa8 RBX::Workspace::replenishCamera(void)")
}

// 0x6ccc18 — __ZNK3RBX9Workspace11askAddChildEPKNS_8InstanceE
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, const RBX::Instance *lpsrc)
#[doc(alias = "RBX::Workspace::askAddChild(RBX::Instance const*)const")]
pub fn stub_6ccc18() -> ! {
    todo!("0x6ccc18 RBX::Workspace::askAddChild(RBX::Instance const*)const")
}

// 0x6ccc50 — __ZN3RBX9Workspace20onDescendantRemovingERKN5boost10shared_ptrINS_8InstanceEEE
#[doc(alias = "RBX::Workspace::onDescendantRemoving(rbx_core::SharedPtr<RBX::Instance> const&)")]
pub fn stub_6ccc50() -> ! {
    todo!("0x6ccc50 RBX::Workspace::onDescendantRemoving(boost::shared_ptr<RBX::Instance> const&)")
}

// 0x6ccc98 — __ZN3RBX9Workspace17onDescendantAddedEPNS_8InstanceE
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, RBX::Instance *)
#[doc(alias = "RBX::Workspace::onDescendantAdded(RBX::Instance *)")]
pub fn stub_6ccc98() -> ! {
    todo!("0x6ccc98 RBX::Workspace::onDescendantAdded(RBX::Instance *)")
}

// 0x6ccda0 — __ZN3RBX9Workspace14startDecalDragEPNS_5DecalE
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, RBX::Decal *)
#[doc(alias = "RBX::Workspace::startDecalDrag(RBX::Decal *)")]
pub fn stub_6ccda0() -> ! {
    todo!("0x6ccda0 RBX::Workspace::startDecalDrag(RBX::Decal *)")
}

// 0x6ccf30 — __ZN3RBX9Workspace15setMouseCommandEN5boost10shared_ptrINS_12MouseCommandEEE
// type: int __fastcall(int, int, int, int, int, int, int, struct _Unwind_Exception *lpuexcpt, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, int, int, int)
#[doc(alias = "RBX::Workspace::setMouseCommand(rbx_core::SharedPtr<RBX::MouseCommand>)")]
pub fn stub_6ccf30() -> ! {
    todo!("0x6ccf30 RBX::Workspace::setMouseCommand(boost::shared_ptr<RBX::MouseCommand>)")
}

// 0x6cd45c — __ZN3RBX9Workspace9getCameraEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::getCamera(void)")]
pub fn stub_6cd45c() -> ! {
    todo!("0x6cd45c RBX::Workspace::getCamera(void)")
}

// 0x6cd464 — __ZThn280_N3RBX9Workspace9getCameraEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Workspace::getCamera(void)")]
pub fn stub_6cd464() {
    // IDA 0x6cd464: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cd478 — __ZNK3RBX9Workspace14getConstCameraEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::getConstCamera(void)const")]
pub fn stub_6cd478() -> ! {
    todo!("0x6cd478 RBX::Workspace::getConstCamera(void)const")
}

// 0x6cd488 — __ZThn280_NK3RBX9Workspace14getConstCameraEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "non-virtual thunk toRBX::Workspace::getConstCamera(void)const")]
pub fn stub_6cd488() {
    // IDA 0x6cd488: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cd498 — __ZN3RBX19destroyIfNotCurrentEN5boost10shared_ptrINS_8InstanceEEEPKNS_6CameraE
#[doc(alias = "RBX::destroyIfNotCurrent(rbx_core::SharedPtr<RBX::Instance>,RBX::Camera const*)")]
pub fn stub_6cd498() -> ! {
    todo!("0x6cd498 RBX::destroyIfNotCurrent(boost::shared_ptr<RBX::Instance>,RBX::Camera const*)")
}

// 0x6cd540 — __ZN3RBX9Workspace10setTerrainEPNS_8InstanceE
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, RBX::Instance *)
#[doc(alias = "RBX::Workspace::setTerrain(RBX::Instance *)")]
pub fn stub_6cd540() -> ! {
    todo!("0x6cd540 RBX::Workspace::setTerrain(RBX::Instance *)")
}

// 0x6cd688 — __ZN3RBX9Workspace13createTerrainEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::createTerrain(void)")]
pub fn stub_6cd688() -> ! {
    todo!("0x6cd688 RBX::Workspace::createTerrain(void)")
}

// 0x6cd86c — __ZN3RBX9Workspace12clearTerrainEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::clearTerrain(void)")]
pub fn stub_6cd86c() -> ! {
    todo!("0x6cd86c RBX::Workspace::clearTerrain(void)")
}

// 0x6cd8e0 — __ZN3RBX9Workspace27selectAllTopLevelRenderableEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::selectAllTopLevelRenderable(void)")]
pub fn stub_6cd8e0() -> ! {
    todo!("0x6cd8e0 RBX::Workspace::selectAllTopLevelRenderable(void)")
}

// 0x6cda3c — __ZN3RBX9Workspace11insertItemsEP10XmlElementRSt6vectorIN5boost10shared_ptrINS_8InstanceEEESaIS7_EENS_10InsertModeENS_10PromptModeEb
#[doc(alias = "RBX::Workspace::insertItems(XmlElement *,std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> &,RBX::InsertMode,RBX::PromptMode,bool)")]
pub fn stub_6cda3c() -> ! {
    todo!("0x6cda3c RBX::Workspace::insertItems(XmlElement *,std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>> &,RBX::InsertMode,RBX::PromptMode,bool)")
}

// 0x6cdae8 — __ZN3RBX9Workspace13insertContentENS_9ContentIdERSt6vectorIN5boost10shared_ptrINS_8InstanceEEESaIS6_EENS_10InsertModeENS_10PromptModeE
// type: int __fastcall(int, int, int, struct _Unwind_Exception *, int)
#[doc(alias = "RBX::Workspace::insertContent(RBX::ContentId,std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> &,RBX::InsertMode,RBX::PromptMode)")]
pub fn stub_6cdae8() -> ! {
    todo!("0x6cdae8 RBX::Workspace::insertContent(RBX::ContentId,std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>> &,RBX::InsertMode,RBX::PromptMode)")
}

// 0x6cdd98 — __ZN3RBX9Workspace11joinAllHackEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::joinAllHack(void)")]
pub fn stub_6cdd98() -> ! {
    todo!("0x6cdd98 RBX::Workspace::joinAllHack(void)")
}

// 0x6cdda0 — __ZN3RBXL7wrapperILb1EEEvN5boost10shared_ptrINS_8InstanceEEE
#[doc(alias = "void RBX::wrapper<true>(rbx_core::SharedPtr<RBX::Instance>)")]
pub fn stub_6cdda0() -> ! {
    todo!("0x6cdda0 void RBX::wrapper<true>(boost::shared_ptr<RBX::Instance>)")
}

// 0x6cddf8 — __ZN3RBXL7wrapperILb0EEEvN5boost10shared_ptrINS_8InstanceEEE
#[doc(alias = "void RBX::wrapper<false>(rbx_core::SharedPtr<RBX::Instance>)")]
pub fn stub_6cddf8() -> ! {
    todo!("0x6cddf8 void RBX::wrapper<false>(boost::shared_ptr<RBX::Instance>)")
}

// 0x6cde50 — __ZN3RBX9Workspace5startEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::start(void)")]
pub fn stub_6cde50() -> ! {
    todo!("0x6cde50 RBX::Workspace::start(void)")
}

// 0x6ce0b8 — __ZN3RBX9Workspace8assembleEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::assemble(void)")]
pub fn stub_6ce0b8() -> ! {
    todo!("0x6ce0b8 RBX::Workspace::assemble(void)")
}

// 0x6ce128 — __ZN3RBX9Workspace4stopEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::stop(void)")]
pub fn stub_6ce128() -> ! {
    todo!("0x6ce128 RBX::Workspace::stop(void)")
}

// 0x6ce398 — __ZN3RBX9Workspace25updateDistributedGameTimeEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::updateDistributedGameTime(void)")]
pub fn stub_6ce398() -> ! {
    todo!("0x6ce398 RBX::Workspace::updateDistributedGameTime(void)")
}

// 0x6ce3e8 — __ZN3RBX9Workspace5resetEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::reset(void)")]
pub fn stub_6ce3e8() -> ! {
    todo!("0x6ce3e8 RBX::Workspace::reset(void)")
}

// 0x6ce400 — __ZN3RBX9Workspace12detachParentEPNS_8InstanceE
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, RBX::Instance *)
#[doc(alias = "RBX::Workspace::detachParent(RBX::Instance *)")]
pub fn stub_6ce400() -> ! {
    todo!("0x6ce400 RBX::Workspace::detachParent(RBX::Instance *)")
}

// 0x6ce4d4 — __ZN3RBX9Workspace18clearEmptiedModelsERN5boost10shared_ptrINS_8InstanceEEE
#[doc(alias = "RBX::Workspace::clearEmptiedModels(rbx_core::SharedPtr<RBX::Instance> &)")]
pub fn stub_6ce4d4() -> ! {
    todo!("0x6ce4d4 RBX::Workspace::clearEmptiedModels(boost::shared_ptr<RBX::Instance> &)")
}

// 0x6ce5b8 — __ZN3RBX9Workspace17handleFallenPartsEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::handleFallenParts(void)")]
pub fn stub_6ce5b8() -> ! {
    todo!("0x6ce5b8 RBX::Workspace::handleFallenParts(void)")
}

// 0x6ce8dc — __ZN3RBX9Workspace11physicsStepEbfi
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, bool, float, int)
#[doc(alias = "RBX::Workspace::physicsStep(bool,float,int)")]
pub fn stub_6ce8dc() -> ! {
    todo!("0x6ce8dc RBX::Workspace::physicsStep(bool,float,int)")
}

// 0x6ceda4 — __ZN3RBX9Workspace22setDefaultMouseCommandEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::setDefaultMouseCommand(void)")]
pub fn stub_6ceda4() -> ! {
    todo!("0x6ceda4 RBX::Workspace::setDefaultMouseCommand(void)")
}

// 0x6cee80 — __ZN3RBX9Workspace19setNullMouseCommandEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::setNullMouseCommand(void)")]
pub fn stub_6cee80() -> ! {
    todo!("0x6cee80 RBX::Workspace::setNullMouseCommand(void)")
}

// 0x6cefa4 — __ZN3RBX9Workspace8render2dEPNS_5AdornE
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, RBX::Adorn *)
#[doc(alias = "RBX::Workspace::render2d(RBX::Adorn *)")]
pub fn stub_6cefa4() -> ! {
    todo!("0x6cefa4 RBX::Workspace::render2d(RBX::Adorn *)")
}

// 0x6cefc8 — __ZThn96_N3RBX9Workspace8render2dEPNS_5AdornE
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, RBX::Adorn *)
#[doc(alias = "non-virtual thunk toRBX::Workspace::render2d(RBX::Adorn *)")]
pub fn stub_6cefc8() {
    // IDA 0x6cefc8: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6ceff0 — __ZN3RBX9Workspace9getCursorEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::getCursor(void)")]
pub fn stub_6ceff0() -> ! {
    todo!("0x6ceff0 RBX::Workspace::getCursor(void)")
}

// 0x6cf11c — __ZN3RBX9Workspace13render3dAdornEPNS_5AdornE
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, RBX::Adorn *)
#[doc(alias = "RBX::Workspace::render3dAdorn(RBX::Adorn *)")]
pub fn stub_6cf11c() -> ! {
    todo!("0x6cf11c RBX::Workspace::render3dAdorn(RBX::Adorn *)")
}

// 0x6cf3f4 — __ZN3RBXL9DrawAdornEN5boost10shared_ptrINS_8InstanceEEEPNS_5AdornENS_11SelectStateEPNS_9WorkspaceE
#[doc(alias = "RBX::DrawAdorn(rbx_core::SharedPtr<RBX::Instance>,RBX::Adorn *,RBX::SelectState,RBX::Workspace *)")]
pub fn stub_6cf3f4() -> ! {
    todo!("0x6cf3f4 RBX::DrawAdorn(boost::shared_ptr<RBX::Instance>,RBX::Adorn *,RBX::SelectState,RBX::Workspace *)")
}

// 0x6cf454 — __ZThn96_N3RBX9Workspace13render3dAdornEPNS_5AdornE
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, RBX::Adorn *)
#[doc(alias = "non-virtual thunk toRBX::Workspace::render3dAdorn(RBX::Adorn *)")]
pub fn stub_6cf454() {
    // IDA 0x6cf454: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cf45c — __ZN3RBX9Workspace19append3dSortedAdornERSt6vectorIPNS_10IAdornableESaIS3_EE
// type: int __fastcall(_DWORD, _DWORD)
#[doc(alias = "RBX::Workspace::append3dSortedAdorn(std::vector<RBX::IAdornable *,std::allocator<RBX::IAdornable *>> &)")]
pub fn stub_6cf45c() -> ! {
    todo!("0x6cf45c RBX::Workspace::append3dSortedAdorn(std::vector<RBX::IAdornable *,std::allocator<RBX::IAdornable *>> &)")
}

// 0x6cf47c — __ZN3RBX9Workspace18hasModalGuiObjectsEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::hasModalGuiObjects(void)")]
pub fn stub_6cf47c() -> ! {
    todo!("0x6cf47c RBX::Workspace::hasModalGuiObjects(void)")
}

// 0x6cf570 — __ZN3RBX9Workspace24requestFirstPersonCameraEbbi
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, bool, bool, int)
#[doc(alias = "RBX::Workspace::requestFirstPersonCamera(bool,bool,int)")]
pub fn stub_6cf570() -> ! {
    todo!("0x6cf570 RBX::Workspace::requestFirstPersonCamera(bool,bool,int)")
}

// 0x6cf618 — __ZN3RBX9Workspace16setRightMousePanEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::setRightMousePan(void)")]
pub fn stub_6cf618() -> ! {
    todo!("0x6cf618 RBX::Workspace::setRightMousePan(void)")
}

// 0x6cf648 — __ZN3RBX9Workspace19cancelRightMousePanEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::cancelRightMousePan(void)")]
pub fn stub_6cf648() -> ! {
    todo!("0x6cf648 RBX::Workspace::cancelRightMousePan(void)")
}

// 0x6cf66c — __ZN3RBX9Workspace7processERKNS_8GuiEventE
// type: int __fastcall(struct _Unwind_Exception *lpuexcpt, int, int, int, int, struct _Unwind_Exception *, int, int, int, int, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, int, int, boost::detail::sp_counted_base *, int, int, int, boost::detail::sp_counted_base *, int, boost::detail::sp_counted_base *, int, int, int, int)
#[doc(alias = "RBX::Workspace::process(RBX::GuiEvent const&)")]
pub fn stub_6cf66c() -> ! {
    todo!("0x6cf66c RBX::Workspace::process(RBX::GuiEvent const&)")
}

// 0x6cfe54 — __ZThn320_N3RBX9Workspace7processERKNS_8GuiEventE
#[doc(alias = "non-virtual thunk toRBX::Workspace::process(RBX::GuiEvent const&)")]
pub fn stub_6cfe54() {
    // IDA 0x6cfe54: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6cfe64 — __ZN3RBX9Workspace17onServiceProviderEPNS_15ServiceProviderES2_
// type: void __fastcall(RBX::Workspace *this, RBX::ServiceProvider *, RBX::ServiceProvider *, int)
#[doc(alias = "RBX::Workspace::onServiceProvider(RBX::ServiceProvider *,RBX::ServiceProvider *)")]
pub fn stub_6cfe64() -> ! {
    todo!("0x6cfe64 RBX::Workspace::onServiceProvider(RBX::ServiceProvider *,RBX::ServiceProvider *)")
}

// 0x6d0138 — __ZN3RBX9Workspace15scriptShouldRunEPNS_10BaseScriptE
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, RBX::BaseScript *)
#[doc(alias = "RBX::Workspace::scriptShouldRun(RBX::BaseScript *)")]
pub fn stub_6d0138() -> ! {
    todo!("0x6d0138 RBX::Workspace::scriptShouldRun(RBX::BaseScript *)")
}

// 0x6d02e4 — __ZThn388_N3RBX9Workspace15scriptShouldRunEPNS_10BaseScriptE
// type: int __fastcall(RBX::Workspace *this, RBX::BaseScript *)
#[doc(alias = "non-virtual thunk toRBX::Workspace::scriptShouldRun(RBX::BaseScript *)")]
pub fn stub_6d02e4() {
    // IDA 0x6d02e4: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d02f4 — __ZN3RBX10hash_valueERKNS_9TouchPairE
// type: int __fastcall(_DWORD)
#[doc(alias = "RBX::hash_value(RBX::TouchPair const&)")]
pub fn stub_6d02f4() -> ! {
    todo!("0x6d02f4 RBX::hash_value(RBX::TouchPair const&)")
}

// 0x6d0328 — __ZNK3RBX9Workspace22getDistributedGameTimeEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::getDistributedGameTime(void)const")]
pub fn stub_6d0328() -> ! {
    todo!("0x6d0328 RBX::Workspace::getDistributedGameTime(void)const")
}

// 0x6d0334 — __ZN3RBX10Reflection14PropDescriptorINS_9WorkspaceEdED1Ev
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::Workspace,double>::~PropDescriptor()")]
pub fn stub_6d0334() {
    // IDA 0x6d0334: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d0358 — __ZNK3RBX9Workspace26getNetworkStreamingEnabledEv
// type: _DWORD __fastcall(RBX::Workspace *__hidden this)
#[doc(alias = "RBX::Workspace::getNetworkStreamingEnabled(void)const")]
pub fn stub_6d0358() -> ! {
    todo!("0x6d0358 RBX::Workspace::getNetworkStreamingEnabled(void)const")
}

// 0x6d0360 — __ZN3RBX9Workspace26setNetworkStreamingEnabledEb
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, bool)
#[doc(alias = "RBX::Workspace::setNetworkStreamingEnabled(bool)")]
pub fn stub_6d0360() -> ! {
    todo!("0x6d0360 RBX::Workspace::setNetworkStreamingEnabled(bool)")
}

// 0x6d0368 — __ZN3RBX10Reflection14PropDescriptorINS_9WorkspaceEbED1Ev
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::Workspace,bool>::~PropDescriptor()")]
pub fn stub_6d0368() {
    // IDA 0x6d0368: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d038c — __ZN3RBX10Reflection13BoundFuncDescINS_9WorkspaceEFN5boost10shared_ptrIKSt6vectorINS4_INS_8InstanceEEESaIS7_EEEENS_9ContentIdEELi1EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::Workspace,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> const> ()(RBX::ContentId),1>::~BoundFuncDesc()")]
pub fn stub_6d038c() {
    // IDA 0x6d038c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d03cc — __ZN3RBX10Reflection13BoundFuncDescINS_9WorkspaceEFvN5boost10shared_ptrIKSt6vectorINS4_INS_8InstanceEEESaIS7_EEEEELi1EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::Workspace,void ()(rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> const>),1>::~BoundFuncDesc()")]
pub fn stub_6d03cc() {
    // IDA 0x6d03cc: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d04c0 — __ZN3RBX10Reflection13BoundFuncDescINS_9WorkspaceEFN5boost10shared_ptrIKSt6vectorINS4_INS_8InstanceEEESaIS7_EEEENS_7Region3ES7_iELi3EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::Workspace,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> const> ()(RBX::Region3,rbx_core::SharedPtr<RBX::Instance>,int),3>::~BoundFuncDesc()")]
pub fn stub_6d04c0() {
    // IDA 0x6d04c0: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d05d8 — __ZN3RBX10Reflection13BoundFuncDescINS_9WorkspaceEFbNS_7Region3EN5boost10shared_ptrINS_8InstanceEEEELi2EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::Workspace,bool ()(RBX::Region3,rbx_core::SharedPtr<RBX::Instance>),2>::~BoundFuncDesc()")]
pub fn stub_6d05d8() {
    // IDA 0x6d05d8: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d06e8 — __ZN3RBX9Workspace9getRayHitINS_8InstanceEEEN5boost10shared_ptrIKNS_10Reflection5TupleEEENS_6RbxRayENS4_IT_EEb
#[doc(alias = "rbx_core::SharedPtr<RBX::Reflection::Tuple const> RBX::Workspace::getRayHit<RBX::Instance>(RBX::RbxRay,rbx_core::SharedPtr<RBX::Instance>,bool)")]
pub fn stub_6d06e8() -> ! {
    todo!("0x6d06e8 boost::shared_ptr<RBX::Reflection::Tuple const> RBX::Workspace::getRayHit<RBX::Instance>(RBX::RbxRay,boost::shared_ptr<RBX::Instance>,bool)")
}

// 0x6d0a5c — __ZN3RBX10Reflection13BoundFuncDescINS_9WorkspaceEFN5boost10shared_ptrIKNS0_5TupleEEENS_6RbxRayENS4_INS_8InstanceEEEbELi3EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::Workspace,rbx_core::SharedPtr<RBX::Reflection::Tuple const> ()(RBX::RbxRay,rbx_core::SharedPtr<RBX::Instance>,bool),3>::~BoundFuncDesc()")]
pub fn stub_6d0a5c() {
    // IDA 0x6d0a5c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d0b88 — __ZN3RBX10Reflection13BoundFuncDescINS_9WorkspaceEFN5boost10shared_ptrIKSt6vectorINS4_INS_8InstanceEEESaIS7_EEEENS_7Region3ESB_iELi3EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::Workspace,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> const> ()(RBX::Region3,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> const>,int),3>::~BoundFuncDesc()")]
pub fn stub_6d0b88() {
    // IDA 0x6d0b88: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d0ca0 — __ZN3RBX10Reflection13BoundFuncDescINS_9WorkspaceEFbNS_7Region3EN5boost10shared_ptrIKSt6vectorINS5_INS_8InstanceEEESaIS8_EEEEELi2EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::Workspace,bool ()(RBX::Region3,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> const>),2>::~BoundFuncDesc()")]
pub fn stub_6d0ca0() {
    // IDA 0x6d0ca0: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d0db0 — __ZN3RBX9Workspace9getRayHitIKSt6vectorIN5boost10shared_ptrINS_8InstanceEEESaIS6_EEEENS4_IKNS_10Reflection5TupleEEENS_6RbxRayENS4_IT_EEb
#[doc(alias = "rbx_core::SharedPtr<RBX::Reflection::Tuple const> RBX::Workspace::getRayHit<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> const>(RBX::RbxRay,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> const>,bool)")]
pub fn stub_6d0db0() -> ! {
    todo!("0x6d0db0 boost::shared_ptr<RBX::Reflection::Tuple const> RBX::Workspace::getRayHit<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>> const>(RBX::RbxRay,boost::shared_ptr<std::vector<boost::shared_ptr<RBX::Instance>,std::allocator<boost::shared_ptr<RBX::Instance>>> const>,bool)")
}

// 0x6d1124 — __ZN3RBX10Reflection13BoundFuncDescINS_9WorkspaceEFN5boost10shared_ptrIKNS0_5TupleEEENS_6RbxRayENS4_IKSt6vectorINS4_INS_8InstanceEEESaISB_EEEEbELi3EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::Workspace,rbx_core::SharedPtr<RBX::Reflection::Tuple const> ()(RBX::RbxRay,rbx_core::SharedPtr<std::vector<rbx_core::SharedPtr<RBX::Instance>,std::allocator<rbx_core::SharedPtr<RBX::Instance>>> const>,bool),3>::~BoundFuncDesc()")]
pub fn stub_6d1124() {
    // IDA 0x6d1124: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d1250 — __ZN3RBX10Reflection17RefPropDescriptorINS_9WorkspaceENS_8InstanceEED1Ev
#[doc(alias = "RBX::Reflection::RefPropDescriptor<RBX::Workspace,RBX::Instance>::~RefPropDescriptor()")]
pub fn stub_6d1250() {
    // IDA 0x6d1250: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d127c — __ZN3RBX9Workspace9doNothingEb
// type: _DWORD __fastcall(RBX::Workspace *__hidden this, bool)
#[doc(alias = "RBX::Workspace::doNothing(bool)")]
pub fn stub_6d127c() -> ! {
    todo!("0x6d127c RBX::Workspace::doNothing(bool)")
}

// 0x6d1280 — __ZN3RBX10Reflection13BoundFuncDescINS_9WorkspaceEFvbELi1EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::Workspace,void ()(bool),1>::~BoundFuncDesc()")]
pub fn stub_6d1280() {
    // IDA 0x6d1280: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d12c0 — __ZN3RBX10Reflection13BoundFuncDescINS_9WorkspaceEFvvELi0EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::Workspace,void ()(void),0>::~BoundFuncDesc()")]
pub fn stub_6d12c0() {
    // IDA 0x6d12c0: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d12e4 — __ZN3RBX10Reflection17RefPropDescriptorINS_9WorkspaceENS_6CameraEED1Ev
#[doc(alias = "RBX::Reflection::RefPropDescriptor<RBX::Workspace,RBX::Camera>::~RefPropDescriptor()")]
pub fn stub_6d12e4() {
    // IDA 0x6d12e4: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x6d1310 — __ZN3RBX10Reflection13BoundFuncDescINS_9WorkspaceEFdvELi0EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::Workspace,double ()(void),0>::~BoundFuncDesc()")]
pub fn stub_6d1310() {
    // IDA 0x6d1310: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

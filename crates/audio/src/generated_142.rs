//! audio generated_142 — next 150 stubs EA-sorted, from ida/export.json
//! Filter: FMOD|Sound|Audio exhausted (2541 distinct) — filler EA-sorted asc, skip existing, rbx_core::SharedPtr not boost
//! Batch: 150 stubs | skeleton batch | range 0x3ca848..0x479d28 EA-sorted asc filler after 0x253cc, skip existing, rbx_core::SharedPtr not boost
//! Generated: 2026-09-01

#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]

use rbx_core::SharedPtr;

// Ensure SharedPtr is seen as used — mirrors boost::shared_ptr<T> -> rbx_core::SharedPtr<T>
const _: () = {
    let _ = core::marker::PhantomData::<SharedPtr<u8>>;
};

// 0x3ca848 — __ZN3rbx8any_castIRKN3RBX6Camera13CameraPanModeENS1_7Region3EEET_RNS_13placement_anyIT0_EE
#[doc(alias = "RBX::Camera::CameraPanMode const& rbx::any_cast<RBX::Camera::CameraPanMode const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
pub fn stub_3ca848() -> ! {
    todo!("0x3ca848 RBX::Camera::CameraPanMode const& rbx::any_cast<RBX::Camera::CameraPanMode const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")
}

// 0x3cad28 — __ZN3rbx13placement_anyIN3RBX7Region3EEaSINS1_6Camera10CameraModeEEERS3_RKT_
#[doc(alias = "rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Camera::CameraMode>(RBX::Camera::CameraMode const&)")]
pub fn stub_3cad28() -> ! {
    todo!("0x3cad28 rbx::placement_any<RBX::Region3>& rbx::placement_any<RBX::Region3>::operator=<RBX::Camera::CameraMode>(RBX::Camera::CameraMode const&)")
}

// 0x3cad78 — __ZN3rbx14implementation12typed_holderIN3RBX6Camera10CameraModeEE9singletonEv
#[doc(alias = "rbx::implementation::typed_holder<RBX::Camera::CameraMode>::singleton(void)")]
pub fn stub_3cad78() -> ! {
    todo!("0x3cad78 rbx::implementation::typed_holder<RBX::Camera::CameraMode>::singleton(void)")
}

// 0x3cade4 — __ZN3rbx14implementation12typed_holderIN3RBX6Camera10CameraModeEE14construct_funcEPKcPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::Camera::CameraMode>::construct_func(char const*,char *)")]
pub fn stub_3cade4() -> ! {
    todo!("0x3cade4 rbx::implementation::typed_holder<RBX::Camera::CameraMode>::construct_func(char const*,char *)")
}

// 0x3cadf0 — __ZN3rbx14implementation12typed_holderIN3RBX6Camera10CameraModeEE13destruct_funcEPc
#[doc(alias = "rbx::implementation::typed_holder<RBX::Camera::CameraMode>::destruct_func(char *)")]
pub fn stub_3cadf0() -> ! {
    todo!("0x3cadf0 rbx::implementation::typed_holder<RBX::Camera::CameraMode>::destruct_func(char *)")
}

// 0x3cb538 — __ZN3rbx8any_castIRKN3RBX6Camera10CameraTypeENS1_7Region3EEET_RNS_13placement_anyIT0_EE
#[doc(alias = "RBX::Camera::CameraType const& rbx::any_cast<RBX::Camera::CameraType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")]
pub fn stub_3cb538() -> ! {
    todo!("0x3cb538 RBX::Camera::CameraType const& rbx::any_cast<RBX::Camera::CameraType const&,RBX::Region3>(rbx::placement_any<RBX::Region3> &)")
}

// 0x3d7f90 — __ZN3RBX20ChangeHistoryService4Item14recordPropertyEPKNS_10Reflection18PropertyDescriptorE
#[doc(alias = "RBX::ChangeHistoryService::Item::recordProperty(RBX::Reflection::PropertyDescriptor const*)")]
pub fn stub_3d7f90() -> ! {
    todo!("0x3d7f90 RBX::ChangeHistoryService::Item::recordProperty(RBX::Reflection::PropertyDescriptor const*)")
}

// 0x3d8310 — __ZN3RBX10Reflection8EnumDescINS_20ChangeHistoryService19RuntimeUndoBehaviorEED1Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::ChangeHistoryService::RuntimeUndoBehavior>::~EnumDesc()")]
pub fn stub_3d8310() {
    // IDA 0x3d8310: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x3d8318 — __ZNK3RBX10Reflection8EnumDescINS_20ChangeHistoryService19RuntimeUndoBehaviorEE15convertToStringEmRSs
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::ChangeHistoryService::RuntimeUndoBehavior>::convertToString(unsigned long,std::string &)const")]
pub fn stub_3d8318() -> ! {
    todo!("0x3d8318 RBX::Reflection::EnumDesc<RBX::ChangeHistoryService::RuntimeUndoBehavior>::convertToString(unsigned long,std::string &)const")
}

// 0x3d8470 — __ZNK3RBX10Reflection8EnumDescINS_20ChangeHistoryService19RuntimeUndoBehaviorEE13convertToItemERKS3_
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::ChangeHistoryService::RuntimeUndoBehavior>::convertToItem(RBX::ChangeHistoryService::RuntimeUndoBehavior const&)const")]
pub fn stub_3d8470() -> ! {
    todo!("0x3d8470 RBX::Reflection::EnumDesc<RBX::ChangeHistoryService::RuntimeUndoBehavior>::convertToItem(RBX::ChangeHistoryService::RuntimeUndoBehavior const&)const")
}

// 0x3d8540 — __ZN3RBX10Reflection8EnumDescINS_20ChangeHistoryService19RuntimeUndoBehaviorEED2Ev
#[doc(alias = "RBX::Reflection::EnumDesc<RBX::ChangeHistoryService::RuntimeUndoBehavior>::~EnumDesc()")]
pub fn stub_3d8540() {
    // IDA 0x3d8540: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x403198 — __ZNK3RBX13ServiceClientINS_9SelectionEE13createServiceEv
#[doc(alias = "RBX::ServiceClient<RBX::Selection>::createService(void)const")]
pub fn stub_403198() -> ! {
    todo!("0x403198 RBX::ServiceClient<RBX::Selection>::createService(void)const")
}

// 0x404314 — __ZN3RBX9TToolVerbINS_9CloneToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::CloneTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404314() {
    // IDA 0x404314: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404318 — __ZN3RBX9TToolVerbINS_8GrabToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::GrabTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404318() {
    // IDA 0x404318: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40431c — __ZN3RBX9TToolVerbINS_8GameToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::GameTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_40431c() {
    // IDA 0x40431c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404320 — __ZN3RBX9TToolVerbINS_8NullToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::NullTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404320() {
    // IDA 0x404320: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404324 — __ZN3RBX9TToolVerbINS_11DropperToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::DropperTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404324() {
    // IDA 0x404324: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404328 — __ZN3RBX9TToolVerbINS_12MaterialToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::MaterialTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404328() {
    // IDA 0x404328: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40432c — __ZN3RBX9TToolVerbINS_8FillToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::FillTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_40432c() {
    // IDA 0x40432c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404330 — __ZN3RBX9TToolVerbINS_8LockToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::LockTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404330() {
    // IDA 0x404330: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404334 — __ZN3RBX9TToolVerbINS_10AnchorToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::AnchorTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404334() {
    // IDA 0x404334: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404338 — __ZN3RBX9TToolVerbINS_20SmoothNoOutlinesToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::SmoothNoOutlinesTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404338() {
    // IDA 0x404338: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40433c — __ZN3RBX9TToolVerbINS_18OscillateMotorToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::OscillateMotorTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_40433c() {
    // IDA 0x40433c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404340 — __ZN3RBX9TToolVerbINS_13LeftMotorToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::LeftMotorTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404340() {
    // IDA 0x404340: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404344 — __ZN3RBX9TToolVerbINS_14RightMotorToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::RightMotorTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404344() {
    // IDA 0x404344: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404348 — __ZN3RBX9TToolVerbINS_9HingeToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::HingeTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404348() {
    // IDA 0x404348: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40434c — __ZN3RBX9TToolVerbINS_13UniversalToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::UniversalTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_40434c() {
    // IDA 0x40434c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404350 — __ZN3RBX9TToolVerbINS_9InletToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::InletTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404350() {
    // IDA 0x404350: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404354 — __ZN3RBX9TToolVerbINS_9StudsToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::StudsTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404354() {
    // IDA 0x404354: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404358 — __ZN3RBX9TToolVerbINS_8WeldToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::WeldTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404358() {
    // IDA 0x404358: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40435c — __ZN3RBX9TToolVerbINS_8GlueToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::GlueTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_40435c() {
    // IDA 0x40435c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404360 — __ZN3RBX9TToolVerbINS_8FlatToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::FlatTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404360() {
    // IDA 0x404360: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404364 — __ZN3RBX9TToolVerbINS_18MoveResizeJoinToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::MoveResizeJoinTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404364() {
    // IDA 0x404364: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404368 — __ZN3RBX9TToolVerbINS_12AdvArrowToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::AdvArrowTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404368() {
    // IDA 0x404368: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40436c — __ZN3RBX9TToolVerbINS_13AdvRotateToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::AdvRotateTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_40436c() {
    // IDA 0x40436c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404370 — __ZN3RBX9TToolVerbINS_11AdvMoveToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::AdvMoveTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404370() {
    // IDA 0x404370: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404374 — __ZN3RBX9TToolVerbINS_14AxisRotateToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::AxisRotateTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404374() {
    // IDA 0x404374: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404378 — __ZN3RBX15MoveUpBrickVerbD1Ev
#[doc(alias = "RBX::MoveUpBrickVerb::~MoveUpBrickVerb()")]
pub fn stub_404378() {
    // IDA 0x404378: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40437c — __ZN3RBX15MoveUpPlateVerbD1Ev
#[doc(alias = "RBX::MoveUpPlateVerb::~MoveUpPlateVerb()")]
pub fn stub_40437c() {
    // IDA 0x40437c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404380 — __ZN3RBX14CanCollideVerbD1Ev
#[doc(alias = "RBX::CanCollideVerb::~CanCollideVerb()")]
pub fn stub_404380() {
    // IDA 0x404380: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404384 — __ZN3RBX15TranslucentVerbD1Ev
#[doc(alias = "RBX::TranslucentVerb::~TranslucentVerb()")]
pub fn stub_404384() {
    // IDA 0x404384: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404388 — __ZN3RBX10AnchorVerbD1Ev
#[doc(alias = "RBX::AnchorVerb::~AnchorVerb()")]
pub fn stub_404388() {
    // IDA 0x404388: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40438c — __ZN3RBX19DeleteSelectionVerbD1Ev
#[doc(alias = "RBX::DeleteSelectionVerb::~DeleteSelectionVerb()")]
pub fn stub_40438c() {
    // IDA 0x40438c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404390 — __ZN3RBX23PlayDeleteSelectionVerbD1Ev
#[doc(alias = "RBX::PlayDeleteSelectionVerb::~PlayDeleteSelectionVerb()")]
pub fn stub_404390() {
    // IDA 0x404390: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404518 — __ZN3RBX9TToolVerbINS_10HammerToolENS_12RunStateVerbEED1Ev
#[doc(alias = "RBX::TToolVerb<RBX::HammerTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404518() {
    // IDA 0x404518: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40451c — __ZN3RBX9TToolVerbINS_10HammerToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::HammerTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_40451c() {
    // IDA 0x40451c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4045bc — __ZNK3RBX9TToolVerbINS_10HammerToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::HammerTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_4045bc() -> ! {
    todo!("0x4045bc RBX::TToolVerb<RBX::HammerTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x4045f4 — __ZN3RBX9TToolVerbINS_10HammerToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::HammerTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_4045f4() -> ! {
    todo!("0x4045f4 RBX::TToolVerb<RBX::HammerTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x404708 — __ZN3RBX9TToolVerbINS_10HammerToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::HammerTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_404708() -> ! {
    todo!("0x404708 RBX::TToolVerb<RBX::HammerTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x404e10 — __ZN3RBX9TToolVerbINS_9CloneToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::CloneTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_404e10() {
    // IDA 0x404e10: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x404eb0 — __ZNK3RBX9TToolVerbINS_9CloneToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::CloneTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_404eb0() -> ! {
    todo!("0x404eb0 RBX::TToolVerb<RBX::CloneTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x404ee8 — __ZN3RBX9TToolVerbINS_9CloneToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::CloneTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_404ee8() -> ! {
    todo!("0x404ee8 RBX::TToolVerb<RBX::CloneTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x404ffc — __ZN3RBX9TToolVerbINS_9CloneToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::CloneTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_404ffc() -> ! {
    todo!("0x404ffc RBX::TToolVerb<RBX::CloneTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x405704 — __ZN3RBX9TToolVerbINS_8GrabToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::GrabTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_405704() {
    // IDA 0x405704: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4057a4 — __ZNK3RBX9TToolVerbINS_8GrabToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::GrabTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_4057a4() -> ! {
    todo!("0x4057a4 RBX::TToolVerb<RBX::GrabTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x4057dc — __ZN3RBX9TToolVerbINS_8GrabToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::GrabTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_4057dc() -> ! {
    todo!("0x4057dc RBX::TToolVerb<RBX::GrabTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x4058f0 — __ZN3RBX9TToolVerbINS_8GrabToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::GrabTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_4058f0() -> ! {
    todo!("0x4058f0 RBX::TToolVerb<RBX::GrabTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x405ff8 — __ZN3RBX9TToolVerbINS_8GameToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::GameTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_405ff8() {
    // IDA 0x405ff8: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x406098 — __ZNK3RBX9TToolVerbINS_8GameToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::GameTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_406098() -> ! {
    todo!("0x406098 RBX::TToolVerb<RBX::GameTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x4060d0 — __ZN3RBX9TToolVerbINS_8GameToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::GameTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_4060d0() -> ! {
    todo!("0x4060d0 RBX::TToolVerb<RBX::GameTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x4061e4 — __ZN3RBX9TToolVerbINS_8GameToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::GameTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_4061e4() -> ! {
    todo!("0x4061e4 RBX::TToolVerb<RBX::GameTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x4068ec — __ZN3RBX9TToolVerbINS_8NullToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::NullTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_4068ec() {
    // IDA 0x4068ec: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40698c — __ZNK3RBX9TToolVerbINS_8NullToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::NullTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_40698c() -> ! {
    todo!("0x40698c RBX::TToolVerb<RBX::NullTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x4069c4 — __ZN3RBX9TToolVerbINS_8NullToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::NullTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_4069c4() -> ! {
    todo!("0x4069c4 RBX::TToolVerb<RBX::NullTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x406ad8 — __ZN3RBX9TToolVerbINS_8NullToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::NullTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_406ad8() -> ! {
    todo!("0x406ad8 RBX::TToolVerb<RBX::NullTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x4071e0 — __ZN3RBX9TToolVerbINS_11DropperToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::DropperTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_4071e0() {
    // IDA 0x4071e0: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x407280 — __ZNK3RBX9TToolVerbINS_11DropperToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::DropperTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_407280() -> ! {
    todo!("0x407280 RBX::TToolVerb<RBX::DropperTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x4072b8 — __ZN3RBX9TToolVerbINS_11DropperToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::DropperTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_4072b8() -> ! {
    todo!("0x4072b8 RBX::TToolVerb<RBX::DropperTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x4073cc — __ZN3RBX9TToolVerbINS_11DropperToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::DropperTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_4073cc() -> ! {
    todo!("0x4073cc RBX::TToolVerb<RBX::DropperTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x407574 — __ZNK3RBX11DropperTool13getCursorNameEv
#[doc(alias = "RBX::DropperTool::getCursorName(void)const")]
pub fn stub_407574() -> ! {
    todo!("0x407574 RBX::DropperTool::getCursorName(void)const")
}

// 0x407b18 — __ZN3RBX9TToolVerbINS_12MaterialToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::MaterialTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_407b18() {
    // IDA 0x407b18: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x407bb8 — __ZNK3RBX9TToolVerbINS_12MaterialToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::MaterialTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_407bb8() -> ! {
    todo!("0x407bb8 RBX::TToolVerb<RBX::MaterialTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x407bf0 — __ZN3RBX9TToolVerbINS_12MaterialToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::MaterialTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_407bf0() -> ! {
    todo!("0x407bf0 RBX::TToolVerb<RBX::MaterialTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x407d04 — __ZN3RBX9TToolVerbINS_12MaterialToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::MaterialTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_407d04() -> ! {
    todo!("0x407d04 RBX::TToolVerb<RBX::MaterialTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x407eac — __ZNK3RBX12MaterialTool8isStickyEv
#[doc(alias = "RBX::MaterialTool::isSticky(void)const")]
pub fn stub_407eac() -> ! {
    todo!("0x407eac RBX::MaterialTool::isSticky(void)const")
}

// 0x407f74 — __ZNK3RBX12MaterialTool13getCursorNameEv
#[doc(alias = "RBX::MaterialTool::getCursorName(void)const")]
pub fn stub_407f74() -> ! {
    todo!("0x407f74 RBX::MaterialTool::getCursorName(void)const")
}

// 0x408518 — __ZN3RBX9TToolVerbINS_8FillToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::FillTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_408518() {
    // IDA 0x408518: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x4085b8 — __ZNK3RBX9TToolVerbINS_8FillToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::FillTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_4085b8() -> ! {
    todo!("0x4085b8 RBX::TToolVerb<RBX::FillTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x4085f0 — __ZN3RBX9TToolVerbINS_8FillToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::FillTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_4085f0() -> ! {
    todo!("0x4085f0 RBX::TToolVerb<RBX::FillTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x408704 — __ZN3RBX9TToolVerbINS_8FillToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::FillTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_408704() -> ! {
    todo!("0x408704 RBX::TToolVerb<RBX::FillTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x4088ac — __ZNK3RBX8FillTool8isStickyEv
#[doc(alias = "RBX::FillTool::isSticky(void)const")]
pub fn stub_4088ac() -> ! {
    todo!("0x4088ac RBX::FillTool::isSticky(void)const")
}

// 0x408974 — __ZNK3RBX8FillTool13getCursorNameEv
#[doc(alias = "RBX::FillTool::getCursorName(void)const")]
pub fn stub_408974() -> ! {
    todo!("0x408974 RBX::FillTool::getCursorName(void)const")
}

// 0x408f18 — __ZN3RBX9TToolVerbINS_8LockToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::LockTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_408f18() {
    // IDA 0x408f18: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x408fb8 — __ZNK3RBX9TToolVerbINS_8LockToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::LockTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_408fb8() -> ! {
    todo!("0x408fb8 RBX::TToolVerb<RBX::LockTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x408ff0 — __ZN3RBX9TToolVerbINS_8LockToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::LockTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_408ff0() -> ! {
    todo!("0x408ff0 RBX::TToolVerb<RBX::LockTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x409104 — __ZN3RBX9TToolVerbINS_8LockToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::LockTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_409104() -> ! {
    todo!("0x409104 RBX::TToolVerb<RBX::LockTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x4092ac — __ZN3RBX8LockTool9onMouseUpERKNS_7UIEventE
#[doc(alias = "RBX::LockTool::onMouseUp(RBX::UIEvent const&)")]
pub fn stub_4092ac() -> ! {
    todo!("0x4092ac RBX::LockTool::onMouseUp(RBX::UIEvent const&)")
}

// 0x409a64 — __ZN3RBX9TToolVerbINS_10AnchorToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::AnchorTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_409a64() {
    // IDA 0x409a64: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x409b04 — __ZNK3RBX9TToolVerbINS_10AnchorToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::AnchorTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_409b04() -> ! {
    todo!("0x409b04 RBX::TToolVerb<RBX::AnchorTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x409b3c — __ZN3RBX9TToolVerbINS_10AnchorToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::AnchorTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_409b3c() -> ! {
    todo!("0x409b3c RBX::TToolVerb<RBX::AnchorTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x409c50 — __ZN3RBX9TToolVerbINS_10AnchorToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::AnchorTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_409c50() -> ! {
    todo!("0x409c50 RBX::TToolVerb<RBX::AnchorTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x409e00 — __ZN3RBX10AnchorTool9onMouseUpERKNS_7UIEventE
#[doc(alias = "RBX::AnchorTool::onMouseUp(RBX::UIEvent const&)")]
pub fn stub_409e00() -> ! {
    todo!("0x409e00 RBX::AnchorTool::onMouseUp(RBX::UIEvent const&)")
}

// 0x40a5b8 — __ZN3RBX9TToolVerbINS_20SmoothNoOutlinesToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::SmoothNoOutlinesTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_40a5b8() {
    // IDA 0x40a5b8: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40a658 — __ZNK3RBX9TToolVerbINS_20SmoothNoOutlinesToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::SmoothNoOutlinesTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_40a658() -> ! {
    todo!("0x40a658 RBX::TToolVerb<RBX::SmoothNoOutlinesTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x40a690 — __ZN3RBX9TToolVerbINS_20SmoothNoOutlinesToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::SmoothNoOutlinesTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_40a690() -> ! {
    todo!("0x40a690 RBX::TToolVerb<RBX::SmoothNoOutlinesTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x40a7a4 — __ZN3RBX9TToolVerbINS_20SmoothNoOutlinesToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::SmoothNoOutlinesTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_40a7a4() -> ! {
    todo!("0x40a7a4 RBX::TToolVerb<RBX::SmoothNoOutlinesTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x40a94c — __ZNK3RBX20SmoothNoOutlinesTool8isStickyEv
#[doc(alias = "RBX::SmoothNoOutlinesTool::isSticky(void)const")]
pub fn stub_40a94c() -> ! {
    todo!("0x40a94c RBX::SmoothNoOutlinesTool::isSticky(void)const")
}

// 0x40aa14 — __ZNK3RBX20SmoothNoOutlinesTool13getCursorNameEv
#[doc(alias = "RBX::SmoothNoOutlinesTool::getCursorName(void)const")]
pub fn stub_40aa14() -> ! {
    todo!("0x40aa14 RBX::SmoothNoOutlinesTool::getCursorName(void)const")
}

// 0x40afb8 — __ZN3RBX9TToolVerbINS_18OscillateMotorToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::OscillateMotorTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_40afb8() {
    // IDA 0x40afb8: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40b058 — __ZNK3RBX9TToolVerbINS_18OscillateMotorToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::OscillateMotorTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_40b058() -> ! {
    todo!("0x40b058 RBX::TToolVerb<RBX::OscillateMotorTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x40b090 — __ZN3RBX9TToolVerbINS_18OscillateMotorToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::OscillateMotorTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_40b090() -> ! {
    todo!("0x40b090 RBX::TToolVerb<RBX::OscillateMotorTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x40b1a4 — __ZN3RBX9TToolVerbINS_18OscillateMotorToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::OscillateMotorTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_40b1a4() -> ! {
    todo!("0x40b1a4 RBX::TToolVerb<RBX::OscillateMotorTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x40b34c — __ZNK3RBX18OscillateMotorTool13getCursorNameEv
#[doc(alias = "RBX::OscillateMotorTool::getCursorName(void)const")]
pub fn stub_40b34c() -> ! {
    todo!("0x40b34c RBX::OscillateMotorTool::getCursorName(void)const")
}

// 0x40b8f0 — __ZN3RBX9TToolVerbINS_13LeftMotorToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::LeftMotorTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_40b8f0() {
    // IDA 0x40b8f0: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40b990 — __ZNK3RBX9TToolVerbINS_13LeftMotorToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::LeftMotorTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_40b990() -> ! {
    todo!("0x40b990 RBX::TToolVerb<RBX::LeftMotorTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x40b9c8 — __ZN3RBX9TToolVerbINS_13LeftMotorToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::LeftMotorTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_40b9c8() -> ! {
    todo!("0x40b9c8 RBX::TToolVerb<RBX::LeftMotorTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x40badc — __ZN3RBX9TToolVerbINS_13LeftMotorToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::LeftMotorTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_40badc() -> ! {
    todo!("0x40badc RBX::TToolVerb<RBX::LeftMotorTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x40bc84 — __ZNK3RBX13LeftMotorTool13getCursorNameEv
#[doc(alias = "RBX::LeftMotorTool::getCursorName(void)const")]
pub fn stub_40bc84() -> ! {
    todo!("0x40bc84 RBX::LeftMotorTool::getCursorName(void)const")
}

// 0x40c228 — __ZN3RBX9TToolVerbINS_14RightMotorToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::RightMotorTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_40c228() {
    // IDA 0x40c228: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40c2c8 — __ZNK3RBX9TToolVerbINS_14RightMotorToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::RightMotorTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_40c2c8() -> ! {
    todo!("0x40c2c8 RBX::TToolVerb<RBX::RightMotorTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x40c300 — __ZN3RBX9TToolVerbINS_14RightMotorToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::RightMotorTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_40c300() -> ! {
    todo!("0x40c300 RBX::TToolVerb<RBX::RightMotorTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x40c414 — __ZN3RBX9TToolVerbINS_14RightMotorToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::RightMotorTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_40c414() -> ! {
    todo!("0x40c414 RBX::TToolVerb<RBX::RightMotorTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x40c5bc — __ZNK3RBX14RightMotorTool8isStickyEv
#[doc(alias = "RBX::RightMotorTool::isSticky(void)const")]
pub fn stub_40c5bc() -> ! {
    todo!("0x40c5bc RBX::RightMotorTool::isSticky(void)const")
}

// 0x40c684 — __ZNK3RBX14RightMotorTool13getCursorNameEv
#[doc(alias = "RBX::RightMotorTool::getCursorName(void)const")]
pub fn stub_40c684() -> ! {
    todo!("0x40c684 RBX::RightMotorTool::getCursorName(void)const")
}

// 0x40cc28 — __ZN3RBX9TToolVerbINS_9HingeToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::HingeTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_40cc28() {
    // IDA 0x40cc28: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40ccc8 — __ZNK3RBX9TToolVerbINS_9HingeToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::HingeTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_40ccc8() -> ! {
    todo!("0x40ccc8 RBX::TToolVerb<RBX::HingeTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x40cd00 — __ZN3RBX9TToolVerbINS_9HingeToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::HingeTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_40cd00() -> ! {
    todo!("0x40cd00 RBX::TToolVerb<RBX::HingeTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x40ce14 — __ZN3RBX9TToolVerbINS_9HingeToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::HingeTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_40ce14() -> ! {
    todo!("0x40ce14 RBX::TToolVerb<RBX::HingeTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x40cfbc — __ZNK3RBX9HingeTool8isStickyEv
#[doc(alias = "RBX::HingeTool::isSticky(void)const")]
pub fn stub_40cfbc() -> ! {
    todo!("0x40cfbc RBX::HingeTool::isSticky(void)const")
}

// 0x40d084 — __ZNK3RBX9HingeTool13getCursorNameEv
#[doc(alias = "RBX::HingeTool::getCursorName(void)const")]
pub fn stub_40d084() -> ! {
    todo!("0x40d084 RBX::HingeTool::getCursorName(void)const")
}

// 0x40d628 — __ZN3RBX9TToolVerbINS_13UniversalToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::UniversalTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_40d628() {
    // IDA 0x40d628: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40d6c8 — __ZNK3RBX9TToolVerbINS_13UniversalToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::UniversalTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_40d6c8() -> ! {
    todo!("0x40d6c8 RBX::TToolVerb<RBX::UniversalTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x40d700 — __ZN3RBX9TToolVerbINS_13UniversalToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::UniversalTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_40d700() -> ! {
    todo!("0x40d700 RBX::TToolVerb<RBX::UniversalTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x40d814 — __ZN3RBX9TToolVerbINS_13UniversalToolENS_12RunStateVerbEE15newMouseCommandEv
#[doc(alias = "RBX::TToolVerb<RBX::UniversalTool,RBX::RunStateVerb>::newMouseCommand(void)")]
pub fn stub_40d814() -> ! {
    todo!("0x40d814 RBX::TToolVerb<RBX::UniversalTool,RBX::RunStateVerb>::newMouseCommand(void)")
}

// 0x40d9bc — __ZNK3RBX13UniversalTool8isStickyEv
#[doc(alias = "RBX::UniversalTool::isSticky(void)const")]
pub fn stub_40d9bc() -> ! {
    todo!("0x40d9bc RBX::UniversalTool::isSticky(void)const")
}

// 0x40da84 — __ZNK3RBX13UniversalTool13getCursorNameEv
#[doc(alias = "RBX::UniversalTool::getCursorName(void)const")]
pub fn stub_40da84() -> ! {
    todo!("0x40da84 RBX::UniversalTool::getCursorName(void)const")
}

// 0x40e028 — __ZN3RBX9TToolVerbINS_9InletToolENS_12RunStateVerbEED0Ev
#[doc(alias = "RBX::TToolVerb<RBX::InletTool,RBX::RunStateVerb>::~TToolVerb()")]
pub fn stub_40e028() {
    // IDA 0x40e028: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x40e0c8 — __ZNK3RBX9TToolVerbINS_9InletToolENS_12RunStateVerbEE9isCheckedEv
#[doc(alias = "RBX::TToolVerb<RBX::InletTool,RBX::RunStateVerb>::isChecked(void)const")]
pub fn stub_40e0c8() -> ! {
    todo!("0x40e0c8 RBX::TToolVerb<RBX::InletTool,RBX::RunStateVerb>::isChecked(void)const")
}

// 0x40e100 — __ZN3RBX9TToolVerbINS_9InletToolENS_12RunStateVerbEE4doItEPNS_10IDataStateE
#[doc(alias = "RBX::TToolVerb<RBX::InletTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")]
pub fn stub_40e100() -> ! {
    todo!("0x40e100 RBX::TToolVerb<RBX::InletTool,RBX::RunStateVerb>::doIt(RBX::IDataState *)")
}

// 0x4608c8 — __ZNK3RBX10Reflection21RefPropertyDescriptor11getDataSizeEPKNS0_13DescribedBaseE
#[doc(alias = "RBX::Reflection::RefPropertyDescriptor::getDataSize(RBX::Reflection::DescribedBase const*)const")]
pub fn stub_4608c8() -> ! {
    todo!("0x4608c8 RBX::Reflection::RefPropertyDescriptor::getDataSize(RBX::Reflection::DescribedBase const*)const")
}

// 0x4608d0 — __ZNK3RBX10Reflection21RefPropertyDescriptor14getStringValueEPKNS0_13DescribedBaseE
#[doc(alias = "RBX::Reflection::RefPropertyDescriptor::getStringValue(RBX::Reflection::DescribedBase const*)const")]
pub fn stub_4608d0() -> ! {
    todo!("0x4608d0 RBX::Reflection::RefPropertyDescriptor::getStringValue(RBX::Reflection::DescribedBase const*)const")
}

// 0x4608f8 — __ZNK3RBX10Reflection21RefPropertyDescriptor14setStringValueEPNS0_13DescribedBaseERKSs
#[doc(alias = "RBX::Reflection::RefPropertyDescriptor::setStringValue(RBX::Reflection::DescribedBase *,std::string const&)const")]
pub fn stub_4608f8() -> ! {
    todo!("0x4608f8 RBX::Reflection::RefPropertyDescriptor::setStringValue(RBX::Reflection::DescribedBase *,std::string const&)const")
}

// 0x4633a4 — __ZN3RBX10Reflection16CallbackDescImplIFbvELi0EEC2ERNS0_15ClassDescriptorEPKcNS0_10Descriptor10AttributesENS_8Security11PermissionsE
#[doc(alias = "RBX::Reflection::CallbackDescImpl<bool ()(void),0>::CallbackDescImpl(RBX::Reflection::ClassDescriptor &,char const*,RBX::Reflection::Descriptor::Attributes,RBX::Security::Permissions)")]
pub fn stub_4633a4() -> ! {
    todo!("0x4633a4 RBX::Reflection::CallbackDescImpl<bool ()(void),0>::CallbackDescImpl(RBX::Reflection::ClassDescriptor &,char const*,RBX::Reflection::Descriptor::Attributes,RBX::Security::Permissions)")
}

// 0x4634e4 — __ZN3RBX10Reflection17BoundCallbackDescIFbvEED0Ev
#[doc(alias = "RBX::Reflection::BoundCallbackDesc<bool ()(void)>::~BoundCallbackDesc()")]
pub fn stub_4634e4() {
    // IDA 0x4634e4: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x463730 — __ZNK3RBX10Reflection12CallbackDescIFbvEE13clearCallbackEPNS0_13DescribedBaseE
#[doc(alias = "RBX::Reflection::CallbackDesc<bool ()(void)>::clearCallback(RBX::Reflection::DescribedBase *)const")]
pub fn stub_463730() -> ! {
    todo!("0x463730 RBX::Reflection::CallbackDesc<bool ()(void)>::clearCallback(RBX::Reflection::DescribedBase *)const")
}

// 0x46455c — __ZN3RBX10Reflection12CallbackDescIFbvEED1Ev
#[doc(alias = "RBX::Reflection::CallbackDesc<bool ()(void)>::~CallbackDesc()")]
pub fn stub_46455c() {
    // IDA 0x46455c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x464654 — __ZN3RBX10Reflection12CallbackDescIFbvEED0Ev
#[doc(alias = "RBX::Reflection::CallbackDesc<bool ()(void)>::~CallbackDesc()")]
pub fn stub_464654() {
    // IDA 0x464654: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x464760 — __ZN3RBX10Reflection18CallbackDescriptorD1Ev
#[doc(alias = "RBX::Reflection::CallbackDescriptor::~CallbackDescriptor()")]
pub fn stub_464760() {
    // IDA 0x464760: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x464acc — __ZN3RBX10Reflection16CallbackDescImplIFbvELi0EED1Ev
#[doc(alias = "RBX::Reflection::CallbackDescImpl<bool ()(void),0>::~CallbackDescImpl()")]
pub fn stub_464acc() {
    // IDA 0x464acc: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x464bc4 — __ZN3RBX10Reflection16CallbackDescImplIFbvELi0EED0Ev
#[doc(alias = "RBX::Reflection::CallbackDescImpl<bool ()(void),0>::~CallbackDescImpl()")]
pub fn stub_464bc4() {
    // IDA 0x464bc4: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x473fd0 — __ZNSt6vectorIPKN3RBX10Reflection14EnumDescriptor4ItemESaIS5_EE6resizeEmS5_
#[doc(alias = "std::vector<RBX::Reflection::EnumDescriptor::Item const*,std::allocator<RBX::Reflection::EnumDescriptor::Item const*>>::resize(unsigned long,RBX::Reflection::EnumDescriptor::Item const*)")]
pub fn stub_473fd0() -> ! {
    todo!("0x473fd0 std::vector<RBX::Reflection::EnumDescriptor::Item const*,std::allocator<RBX::Reflection::EnumDescriptor::Item const*>>::resize(unsigned long,RBX::Reflection::EnumDescriptor::Item const*)")
}

// 0x4741d0 — __ZNSt6vectorIPKN3RBX10Reflection14EnumDescriptor4ItemESaIS5_EE14_M_fill_insertEN9__gnu_cxx17__normal_iteratorIPS5_S7_EEmRKS5_
#[doc(alias = "std::vector<RBX::Reflection::EnumDescriptor::Item const*,std::allocator<RBX::Reflection::EnumDescriptor::Item const*>>::_M_fill_insert(__gnu_cxx::__normal_iterator<RBX::Reflection::EnumDescriptor::Item const**,std::vector<RBX::Reflection::EnumDescriptor::Item const*,std::allocator<RBX::Reflection::EnumDescriptor::Item const*>>>,unsigned long,RBX::Reflection::EnumDescriptor::Item const* const&)")]
pub fn stub_4741d0() -> ! {
    todo!("0x4741d0 std::vector<RBX::Reflection::EnumDescriptor::Item const*,std::allocator<RBX::Reflection::EnumDescriptor::Item const*>>::_M_fill_insert(__gnu_cxx::__normal_iterator<RBX::Reflection::EnumDescriptor::Item const**,std::vector<RBX::Reflection::EnumDescriptor::Item const*,std::allocator<RBX::Reflection::EnumDescriptor::Item const*>>>,unsigned long,RBX::Reflection::EnumDescriptor::Item const* const&)")
}

// 0x474338 — __ZNSt12_Vector_baseIPKN3RBX10Reflection14EnumDescriptor4ItemESaIS5_EE11_M_allocateEm
#[doc(alias = "std::_Vector_base<RBX::Reflection::EnumDescriptor::Item const*,std::allocator<RBX::Reflection::EnumDescriptor::Item const*>>::_M_allocate(unsigned long)")]
pub fn stub_474338() -> ! {
    todo!("0x474338 std::_Vector_base<RBX::Reflection::EnumDescriptor::Item const*,std::allocator<RBX::Reflection::EnumDescriptor::Item const*>>::_M_allocate(unsigned long)")
}

// 0x474b50 — __ZNSt6vectorIPKN3RBX10Reflection14EnumDescriptor4ItemESaIS5_EE13_M_insert_auxEN9__gnu_cxx17__normal_iteratorIPS5_S7_EERKS5_
#[doc(alias = "std::vector<RBX::Reflection::EnumDescriptor::Item const*,std::allocator<RBX::Reflection::EnumDescriptor::Item const*>>::_M_insert_aux(__gnu_cxx::__normal_iterator<RBX::Reflection::EnumDescriptor::Item const**,std::vector<RBX::Reflection::EnumDescriptor::Item const*,std::allocator<RBX::Reflection::EnumDescriptor::Item const*>>>,RBX::Reflection::EnumDescriptor::Item const* const&)")]
pub fn stub_474b50() -> ! {
    todo!("0x474b50 std::vector<RBX::Reflection::EnumDescriptor::Item const*,std::allocator<RBX::Reflection::EnumDescriptor::Item const*>>::_M_insert_aux(__gnu_cxx::__normal_iterator<RBX::Reflection::EnumDescriptor::Item const**,std::vector<RBX::Reflection::EnumDescriptor::Item const*,std::allocator<RBX::Reflection::EnumDescriptor::Item const*>>>,RBX::Reflection::EnumDescriptor::Item const* const&)")
}

// 0x474c30 — __ZN3RBX10Reflection14EnumDescriptor4ItemD0Ev
#[doc(alias = "RBX::Reflection::EnumDescriptor::Item::~Item()")]
pub fn stub_474c30() {
    // IDA 0x474c30: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x477a14 — __ZN3RBX10Reflection14PropDescriptorINS_13DebrisServiceEiED1Ev
#[doc(alias = "RBX::Reflection::PropDescriptor<RBX::DebrisService,int>::~PropDescriptor()")]
pub fn stub_477a14() {
    // IDA 0x477a14: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x477b50 — __ZN3RBX10Reflection13BoundFuncDescINS_13DebrisServiceEFvbELi1EED1Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::DebrisService,void ()(bool),1>::~BoundFuncDesc()")]
pub fn stub_477b50() {
    // IDA 0x477b50: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

// 0x479b80 — __ZN3RBX10Reflection13BoundFuncDescINS_13DebrisServiceEFvbELi1EEC2EMS2_FvbEPKcS8_NS_8Security11PermissionsENS0_10Descriptor10AttributesE
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::DebrisService,void ()(bool),1>::BoundFuncDesc(void (RBX::DebrisService::*)(bool),char const*,char const*,RBX::Security::Permissions,RBX::Reflection::Descriptor::Attributes)")]
pub fn stub_479b80() -> ! {
    todo!("0x479b80 RBX::Reflection::BoundFuncDesc<RBX::DebrisService,void ()(bool),1>::BoundFuncDesc(void (RBX::DebrisService::*)(bool),char const*,char const*,RBX::Security::Permissions,RBX::Reflection::Descriptor::Attributes)")
}

// 0x479cf8 — __ZN3RBX10Reflection13BoundFuncDescINS_13DebrisServiceEFvbELi1EE16declareSignatureEPKcNS0_7VariantE
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::DebrisService,void ()(bool),1>::declareSignature(char const*,RBX::Reflection::Variant)")]
pub fn stub_479cf8() -> ! {
    todo!("0x479cf8 RBX::Reflection::BoundFuncDesc<RBX::DebrisService,void ()(bool),1>::declareSignature(char const*,RBX::Reflection::Variant)")
}

// 0x479d28 — __ZN3RBX10Reflection13BoundFuncDescINS_13DebrisServiceEFvbELi1EED0Ev
#[doc(alias = "RBX::Reflection::BoundFuncDesc<RBX::DebrisService,void ()(bool),1>::~BoundFuncDesc()")]
pub fn stub_479d28() {
    // IDA 0x479d28: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

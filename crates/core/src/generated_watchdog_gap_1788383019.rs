//! core watchdog gap 1788383019 — 150 gap filler stubs EA-sorted asc pure gap filler any namespace (global dedup).
//! Source: ida/export.json (85545 funcs) global EA asc not yet in crates (global dedup stub_0x) — next 150 uncovered EA-sorted asc pure gap filler any namespace (0x71af7c..0x74e2cc, EA-sorted asc, distinct not yet in global).
//! Sanitized: boost::shared_ptr -> rbx_core::SharedPtr, boost::weak_ptr -> rbx_core::WeakPtr, boost::intrusive_ptr -> rbx_core::SharedPtr, single quotes and backticks removed from alias.

#![allow(non_camel_case_types, non_snake_case, dead_code, unused_variables, clippy::all)]
use crate::SharedPtr;
const _SHARED_PTR: Option<SharedPtr<u8>> = None;

#[doc(alias = "RBX::BallPolyContact::~BallPolyContact()")]
#[doc(alias = "__ZN3RBX15BallPolyContactD0Ev")]
// 0x71af7c — __ZN3RBX15BallPolyContactD0Ev
// type: void __fastcall(RBX::BallPolyContact *__hidden this)
pub fn stub_0x71af7c() {
    // IDA 0x71af7c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::PolyContact::numConnectors(void)const")]
#[doc(alias = "__ZNK3RBX11PolyContact13numConnectorsEv")]
// 0x71b030 — __ZNK3RBX11PolyContact13numConnectorsEv
// type: _DWORD __fastcall(RBX::PolyContact *__hidden this)
pub fn stub_0x71b030() {
    // IDA 0x71b030: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Allocator<RBX::BallPolyContact>::releaseMemory(void)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_15BallPolyContactEE13releaseMemoryEv")]
// 0x71b038 — __ZN3RBX9AllocatorINS_15BallPolyContactEE13releaseMemoryEv
// type: 
pub fn stub_0x71b038() {
    // IDA 0x71b038: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Allocator<RBX::BallPolyContact>::operator delete(void *)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_15BallPolyContactEEdlEPv")]
// 0x71b084 — __ZN3RBX9AllocatorINS_15BallPolyContactEEdlEPv
// type: void __fastcall(void *)
pub fn stub_0x71b084() {
    // IDA 0x71b084: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Block::init(void)")]
#[doc(alias = "__ZN3RBX5Block4initEv")]
// 0x71b460 — __ZN3RBX5Block4initEv
// type: _DWORD __fastcall(RBX::Block *__hidden this)
pub fn stub_0x71b460() {
    // IDA 0x71b460: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Block::buildMesh(void)")]
#[doc(alias = "__ZN3RBX5Block9buildMeshEv")]
// 0x71b4a8 — __ZN3RBX5Block9buildMeshEv
// type: _DWORD __fastcall(RBX::Block *__hidden this)
pub fn stub_0x71b4a8() {
    // IDA 0x71b4a8: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Block::getMomentHollow(float)const")]
#[doc(alias = "__ZNK3RBX5Block15getMomentHollowEf")]
// 0x71b72c — __ZNK3RBX5Block15getMomentHollowEf
// type: _DWORD __fastcall(RBX::Block *__hidden this, float)
pub fn stub_0x71b72c() {
    // IDA 0x71b72c: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Block::getVolume(void)const")]
#[doc(alias = "__ZNK3RBX5Block9getVolumeEv")]
// 0x71bb08 — __ZNK3RBX5Block9getVolumeEv
// type: _DWORD __fastcall(RBX::Block *__hidden this)
pub fn stub_0x71bb08() {
    // IDA 0x71bb08: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Block::getSurfaceCoordInBody(unsigned long)const")]
#[doc(alias = "__ZNK3RBX5Block21getSurfaceCoordInBodyEm")]
// 0x71c050 — __ZNK3RBX5Block21getSurfaceCoordInBodyEm
// type: _DWORD __fastcall(RBX::Block *__hidden this, unsigned int)
pub fn stub_0x71c050() {
    // IDA 0x71c050: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Block::~Block()")]
#[doc(alias = "__ZN3RBX5BlockD1Ev")]
// 0x71c3f0 — __ZN3RBX5BlockD1Ev
// type: void __fastcall(RBX::Block *__hidden this)
pub fn stub_0x71c3f0() {
    // IDA 0x71c3f0: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Block::~Block()")]
#[doc(alias = "__ZN3RBX5BlockD0Ev")]
// 0x71c3f4 — __ZN3RBX5BlockD0Ev
// type: void __fastcall(RBX::Block *__hidden this)
pub fn stub_0x71c3f4() {
    // IDA 0x71c3f4: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Block::getGeometryType(void)const")]
#[doc(alias = "__ZNK3RBX5Block15getGeometryTypeEv")]
// 0x71c494 — __ZNK3RBX5Block15getGeometryTypeEv
// type: _DWORD __fastcall(RBX::Block *__hidden this)
pub fn stub_0x71c494() {
    // IDA 0x71c494: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Block::getCollideType(void)const")]
#[doc(alias = "__ZNK3RBX5Block14getCollideTypeEv")]
// 0x71c498 — __ZNK3RBX5Block14getCollideTypeEv
// type: _DWORD __fastcall(RBX::Block *__hidden this)
pub fn stub_0x71c498() {
    // IDA 0x71c498: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Geometry::setGeometryParameter(std::string const&,int)")]
#[doc(alias = "__ZN3RBX8Geometry20setGeometryParameterERKSsi")]
// 0x71c49c — __ZN3RBX8Geometry20setGeometryParameterERKSsi
// type: _DWORD __fastcall(RBX::Geometry *__hidden this, const std::string *, int)
pub fn stub_0x71c49c() {
    // IDA 0x71c49c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Geometry::getGeometryParameter(std::string const&)const")]
#[doc(alias = "__ZNK3RBX8Geometry20getGeometryParameterERKSs")]
// 0x71c4f4 — __ZNK3RBX8Geometry20getGeometryParameterERKSs
// type: _DWORD __fastcall(RBX::Geometry *__hidden this, const std::string *)
pub fn stub_0x71c4f4() {
    // IDA 0x71c4f4: libstdc++ container/algorithm internals. Vec/BTreeMap/VecDeque/Iterator — monomorph artifact, no-op carrier.
}

#[doc(alias = "RBX::Poly::getRadius(void)const")]
#[doc(alias = "__ZNK3RBX4Poly9getRadiusEv")]
// 0x71c548 — __ZNK3RBX4Poly9getRadiusEv
// type: _DWORD __fastcall(RBX::Poly *__hidden this)
pub fn stub_0x71c548() {
    // IDA 0x71c548: libstdc++ container/algorithm internals. Vec/BTreeMap/VecDeque/Iterator — monomorph artifact, no-op carrier.
}

#[doc(alias = "RBX::Poly::getNumSurfaces(void)const")]
#[doc(alias = "__ZNK3RBX4Poly14getNumSurfacesEv")]
// 0x71c54c — __ZNK3RBX4Poly14getNumSurfacesEv
// type: _DWORD __fastcall(RBX::Poly *__hidden this)
pub fn stub_0x71c54c() {
    // IDA 0x71c54c: libstdc++ container/algorithm internals. Vec/BTreeMap/VecDeque/Iterator — monomorph artifact, no-op carrier.
}

#[doc(alias = "RBX::Geometry::getFaceFromLegacyNormalId(RBX::NormalId)const")]
#[doc(alias = "__ZNK3RBX8Geometry25getFaceFromLegacyNormalIdENS_8NormalIdE")]
// 0x71c564 — __ZNK3RBX8Geometry25getFaceFromLegacyNormalIdENS_8NormalIdE
// type: 
pub fn stub_0x71c564() {
    // IDA 0x71c564: libstdc++ container/algorithm internals. Vec/BTreeMap/VecDeque/Iterator — monomorph artifact, no-op carrier.
}

#[doc(alias = "RBX::Geometry::isGeometryOrthogonal(void)const")]
#[doc(alias = "__ZNK3RBX8Geometry20isGeometryOrthogonalEv")]
// 0x71c568 — __ZNK3RBX8Geometry20isGeometryOrthogonalEv
// type: _DWORD __fastcall(RBX::Geometry *__hidden this)
pub fn stub_0x71c568() {
    // IDA 0x71c568: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Block::getMoment(float)const")]
#[doc(alias = "__ZNK3RBX5Block9getMomentEf")]
// 0x71c56c — __ZNK3RBX5Block9getMomentEf
// type: _DWORD __fastcall(RBX::Block *__hidden this, float)
pub fn stub_0x71c56c() {
    // IDA 0x71c56c: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Allocator<RBX::POLY::BlockCorners>::operator delete(void *)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_4POLY12BlockCornersEEdlEPv")]
// 0x71ca14 — __ZN3RBX9AllocatorINS_4POLY12BlockCornersEEdlEPv
// type: void __fastcall(void *)
pub fn stub_0x71ca14() {
    // IDA 0x71ca14: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Allocator<RBX::POLY::BlockCorners>::operator new(unsigned long)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_4POLY12BlockCornersEEnwEm")]
// 0x71d050 — __ZN3RBX9AllocatorINS_4POLY12BlockCornersEEnwEm
// type: 
pub fn stub_0x71d050() {
    // IDA 0x71d050: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Allocator<RBX::POLY::BlockCorners>::Allocator(void)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_4POLY12BlockCornersEEC2Ev")]
// 0x71d14c — __ZN3RBX9AllocatorINS_4POLY12BlockCornersEEC2Ev
// type: 
pub fn stub_0x71d14c() {
    // IDA 0x71d14c: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Allocator<RBX::POLY::BlockCorners>::releaseMemory(void)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_4POLY12BlockCornersEE13releaseMemoryEv")]
// 0x71d1b0 — __ZN3RBX9AllocatorINS_4POLY12BlockCornersEE13releaseMemoryEv
// type: 
pub fn stub_0x71d1b0() {
    // IDA 0x71d1b0: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Allocator<RBX::POLY::BlockMesh>::operator delete(void *)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_4POLY9BlockMeshEEdlEPv")]
// 0x71da2c — __ZN3RBX9AllocatorINS_4POLY9BlockMeshEEdlEPv
// type: void __fastcall(void *)
pub fn stub_0x71da2c() {
    // IDA 0x71da2c: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "std::vector<RBX::POLY::Vertex,std::allocator<RBX::POLY::Vertex>>::~vector()")]
#[doc(alias = "__ZNSt6vectorIN3RBX4POLY6VertexESaIS2_EED2Ev")]
// 0x71da68 — __ZNSt6vectorIN3RBX4POLY6VertexESaIS2_EED2Ev
// type: 
pub fn stub_0x71da68() {
    // IDA 0x71da68: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "std::vector<RBX::POLY::Face,std::allocator<RBX::POLY::Face>>::~vector()")]
#[doc(alias = "__ZNSt6vectorIN3RBX4POLY4FaceESaIS2_EED2Ev")]
// 0x71daa0 — __ZNSt6vectorIN3RBX4POLY4FaceESaIS2_EED2Ev
// type: 
pub fn stub_0x71daa0() {
    // IDA 0x71daa0: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Allocator<RBX::POLY::BlockMesh>::operator new(unsigned long)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_4POLY9BlockMeshEEnwEm")]
// 0x71e0b0 — __ZN3RBX9AllocatorINS_4POLY9BlockMeshEEnwEm
// type: 
pub fn stub_0x71e0b0() {
    // IDA 0x71e0b0: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Allocator<RBX::POLY::BlockMesh>::Allocator(void)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_4POLY9BlockMeshEEC2Ev")]
// 0x71e230 — __ZN3RBX9AllocatorINS_4POLY9BlockMeshEEC2Ev
// type: 
pub fn stub_0x71e230() {
    // IDA 0x71e230: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Allocator<RBX::POLY::BlockMesh>::releaseMemory(void)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_4POLY9BlockMeshEE13releaseMemoryEv")]
// 0x71e294 — __ZN3RBX9AllocatorINS_4POLY9BlockMeshEE13releaseMemoryEv
// type: 
pub fn stub_0x71e294() {
    // IDA 0x71e294: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Block::~Block()")]
#[doc(alias = "__ZN3RBX5BlockD2Ev")]
// 0x71e3bc — __ZN3RBX5BlockD2Ev
// type: void __fastcall(RBX::Block *__hidden this)
pub fn stub_0x71e3bc() {
    // IDA 0x71e3bc: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::CleanStage::CleanStage(RBX::IStage *,RBX::World *)")]
#[doc(alias = "__ZN3RBX10CleanStageC1EPNS_6IStageEPNS_5WorldE")]
// 0x71e5cc — __ZN3RBX10CleanStageC1EPNS_6IStageEPNS_5WorldE
// type: _DWORD __fastcall(RBX::CleanStage *__hidden this, RBX::IStage *, RBX::World *)
pub fn stub_0x71e5cc() {
    // IDA 0x71e5cc: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::CleanStage::CleanStage(RBX::IStage *,RBX::World *)")]
#[doc(alias = "__ZN3RBX10CleanStageC2EPNS_6IStageEPNS_5WorldE")]
// 0x71e5d0 — __ZN3RBX10CleanStageC2EPNS_6IStageEPNS_5WorldE
// type: _DWORD __fastcall(RBX::CleanStage *__hidden this, RBX::IStage *, RBX::World *)
pub fn stub_0x71e5d0() {
    // IDA 0x71e5d0: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::CleanStage::onPrimitiveAdded(RBX::Primitive *)")]
#[doc(alias = "__ZN3RBX10CleanStage16onPrimitiveAddedEPNS_9PrimitiveE")]
// 0x71e6a4 — __ZN3RBX10CleanStage16onPrimitiveAddedEPNS_9PrimitiveE
// type: _DWORD __fastcall(RBX::CleanStage *__hidden this, RBX::Primitive *)
pub fn stub_0x71e6a4() {
    // IDA 0x71e6a4: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::CleanStage::onPrimitiveRemoving(RBX::Primitive *)")]
#[doc(alias = "__ZN3RBX10CleanStage19onPrimitiveRemovingEPNS_9PrimitiveE")]
// 0x71e6c0 — __ZN3RBX10CleanStage19onPrimitiveRemovingEPNS_9PrimitiveE
// type: _DWORD __fastcall(RBX::CleanStage *__hidden this, RBX::Primitive *)
pub fn stub_0x71e6c0() {
    // IDA 0x71e6c0: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::CleanStage::onJointPrimitiveNulling(RBX::Joint *,RBX::Primitive *)")]
#[doc(alias = "__ZN3RBX10CleanStage23onJointPrimitiveNullingEPNS_5JointEPNS_9PrimitiveE")]
// 0x71e6dc — __ZN3RBX10CleanStage23onJointPrimitiveNullingEPNS_5JointEPNS_9PrimitiveE
// type: _DWORD __fastcall(RBX::CleanStage *__hidden this, RBX::Joint *, RBX::Primitive *)
pub fn stub_0x71e6dc() {
    // IDA 0x71e6dc: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::CleanStage::onJointPrimitiveSet(RBX::Joint *,RBX::Primitive *)")]
#[doc(alias = "__ZN3RBX10CleanStage19onJointPrimitiveSetEPNS_5JointEPNS_9PrimitiveE")]
// 0x71e7fc — __ZN3RBX10CleanStage19onJointPrimitiveSetEPNS_5JointEPNS_9PrimitiveE
// type: _DWORD __fastcall(RBX::CleanStage *__hidden this, RBX::Joint *, RBX::Primitive *)
pub fn stub_0x71e7fc() {
    // IDA 0x71e7fc: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::CleanStage::onEdgeAdded(RBX::Edge *)")]
#[doc(alias = "__ZN3RBX10CleanStage11onEdgeAddedEPNS_4EdgeE")]
// 0x71e87c — __ZN3RBX10CleanStage11onEdgeAddedEPNS_4EdgeE
// type: _DWORD __fastcall(RBX::CleanStage *__hidden this, RBX::Edge *)
pub fn stub_0x71e87c() {
    // IDA 0x71e87c: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::CleanStage::onEdgeRemoving(RBX::Edge *)")]
#[doc(alias = "__ZN3RBX10CleanStage14onEdgeRemovingEPNS_4EdgeE")]
// 0x71e8b0 — __ZN3RBX10CleanStage14onEdgeRemovingEPNS_4EdgeE
// type: _DWORD __fastcall(RBX::CleanStage *__hidden this, RBX::Edge *)
pub fn stub_0x71e8b0() {
    // IDA 0x71e8b0: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::IPipelined::inStage(RBX::IStage *)const")]
#[doc(alias = "__ZNK3RBX10IPipelined7inStageEPNS_6IStageE")]
// 0x71e984 — __ZNK3RBX10IPipelined7inStageEPNS_6IStageE
// type: _DWORD __fastcall(RBX::IPipelined *__hidden this, RBX::IStage *)
pub fn stub_0x71e984() {
    // IDA 0x71e984: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::CleanStage::~CleanStage()")]
#[doc(alias = "__ZN3RBX10CleanStageD1Ev")]
// 0x71ea28 — __ZN3RBX10CleanStageD1Ev
// type: void __fastcall(RBX::CleanStage *__hidden this)
pub fn stub_0x71ea28() {
    // IDA 0x71ea28: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::CleanStage::~CleanStage()")]
#[doc(alias = "__ZN3RBX10CleanStageD0Ev")]
// 0x71ea4c — __ZN3RBX10CleanStageD0Ev
// type: void __fastcall(RBX::CleanStage *__hidden this)
pub fn stub_0x71ea4c() {
    // IDA 0x71ea4c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::CleanStage::getStageType(void)const")]
#[doc(alias = "__ZNK3RBX10CleanStage12getStageTypeEv")]
// 0x71eb04 — __ZNK3RBX10CleanStage12getStageTypeEv
// type: _DWORD __fastcall(RBX::CleanStage *__hidden this)
pub fn stub_0x71eb04() {
    // IDA 0x71eb04: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Clump::Clump(void)")]
#[doc(alias = "__ZN3RBX5ClumpC1Ev")]
// 0x71ebd0 — __ZN3RBX5ClumpC1Ev
// type: _DWORD __fastcall(RBX::Clump *__hidden this)
pub fn stub_0x71ebd0() {
    // IDA 0x71ebd0: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Clump::~Clump()")]
#[doc(alias = "__ZN3RBX5ClumpD0Ev")]
// 0x71ebec — __ZN3RBX5ClumpD0Ev
// type: void __fastcall(RBX::Clump *__hidden this)
pub fn stub_0x71ebec() {
    // IDA 0x71ebec: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Clump::~Clump()")]
#[doc(alias = "__ZN3RBX5ClumpD1Ev")]
// 0x71ec8c — __ZN3RBX5ClumpD1Ev
// type: void __fastcall(RBX::Clump *__hidden this)
pub fn stub_0x71ec8c() {
    // IDA 0x71ec8c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Clump::isClumpRootPrimitive(RBX::Primitive const*)")]
#[doc(alias = "__ZN3RBX5Clump20isClumpRootPrimitiveEPKNS_9PrimitiveE")]
// 0x71ec9c — __ZN3RBX5Clump20isClumpRootPrimitiveEPKNS_9PrimitiveE
// type: _DWORD __fastcall(RBX::Clump *__hidden this, const RBX::Primitive *)
pub fn stub_0x71ec9c() {
    // IDA 0x71ec9c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Clump::getPrimitiveClump(RBX::Primitive *)")]
#[doc(alias = "__ZN3RBX5Clump17getPrimitiveClumpEPNS_9PrimitiveE")]
// 0x71ecac — __ZN3RBX5Clump17getPrimitiveClumpEPNS_9PrimitiveE
// type: _DWORD __fastcall(RBX::Primitive *__hidden this)
pub fn stub_0x71ecac() {
    // IDA 0x71ecac: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Clump::getConstPrimitiveClump(RBX::Primitive const*)")]
#[doc(alias = "__ZN3RBX5Clump22getConstPrimitiveClumpEPKNS_9PrimitiveE")]
// 0x71ecb4 — __ZN3RBX5Clump22getConstPrimitiveClumpEPKNS_9PrimitiveE
// type: _DWORD __fastcall(RBX::Primitive *__hidden this)
pub fn stub_0x71ecb4() {
    // IDA 0x71ecb4: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::IndexedTree::onParentChanging(void)")]
#[doc(alias = "__ZN3RBX11IndexedTree16onParentChangingEv")]
// 0x71f024 — __ZN3RBX11IndexedTree16onParentChangingEv
// type: _DWORD __fastcall(RBX::IndexedTree *__hidden this)
pub fn stub_0x71f024() {
    // IDA 0x71f024: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::IndexedTree::onChildAdding(RBX::IndexedTree*)")]
#[doc(alias = "__ZN3RBX11IndexedTree13onChildAddingEPS0_")]
// 0x71f028 — __ZN3RBX11IndexedTree13onChildAddingEPS0_
// type: _DWORD __fastcall(RBX::IndexedTree *__hidden this, RBX::IndexedTree *)
pub fn stub_0x71f028() {
    // IDA 0x71f028: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::IndexedTree::onChildAdded(RBX::IndexedTree*)")]
#[doc(alias = "__ZN3RBX11IndexedTree12onChildAddedEPS0_")]
// 0x71f02c — __ZN3RBX11IndexedTree12onChildAddedEPS0_
// type: _DWORD __fastcall(RBX::IndexedTree *__hidden this, RBX::IndexedTree *)
pub fn stub_0x71f02c() {
    // IDA 0x71f02c: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::IndexedTree::onChildRemoving(RBX::IndexedTree*)")]
#[doc(alias = "__ZN3RBX11IndexedTree15onChildRemovingEPS0_")]
// 0x71f030 — __ZN3RBX11IndexedTree15onChildRemovingEPS0_
// type: _DWORD __fastcall(RBX::IndexedTree *__hidden this, RBX::IndexedTree *)
pub fn stub_0x71f030() {
    // IDA 0x71f030: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::IndexedTree::onChildRemoved(RBX::IndexedTree*)")]
#[doc(alias = "__ZN3RBX11IndexedTree14onChildRemovedEPS0_")]
// 0x71f034 — __ZN3RBX11IndexedTree14onChildRemovedEPS0_
// type: _DWORD __fastcall(RBX::IndexedTree *__hidden this, RBX::IndexedTree *)
pub fn stub_0x71f034() {
    // IDA 0x71f034: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::IndexedTree::onAncestorChanged(void)")]
#[doc(alias = "__ZN3RBX11IndexedTree17onAncestorChangedEv")]
// 0x71f038 — __ZN3RBX11IndexedTree17onAncestorChangedEv
// type: _DWORD __fastcall(RBX::IndexedTree *__hidden this)
pub fn stub_0x71f038() {
    // IDA 0x71f038: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::IndexedMesh::onLowersChanged(void)")]
#[doc(alias = "__ZN3RBX11IndexedMesh15onLowersChangedEv")]
// 0x71f03c — __ZN3RBX11IndexedMesh15onLowersChangedEv
// type: _DWORD __fastcall(RBX::IndexedMesh *__hidden this)
pub fn stub_0x71f03c() {
    // IDA 0x71f03c: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::pairHitRatio(void)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact12pairHitRatioEv")]
// 0x71f648 — __ZN3RBX17BlockBlockContact12pairHitRatioEv
// type: _DWORD __fastcall(RBX::BlockBlockContact *__hidden this)
pub fn stub_0x71f648() {
    // IDA 0x71f648: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::featureHitRatio(void)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact15featureHitRatioEv")]
// 0x71f684 — __ZN3RBX17BlockBlockContact15featureHitRatioEv
// type: _DWORD __fastcall(RBX::BlockBlockContact *__hidden this)
pub fn stub_0x71f684() {
    // IDA 0x71f684: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Contact::getBody(int)")]
#[doc(alias = "__ZN3RBX7Contact7getBodyEi")]
// 0x71f6c0 — __ZN3RBX7Contact7getBodyEi
// type: _DWORD __fastcall(RBX::Contact *__hidden this, int)
pub fn stub_0x71f6c0() {
    // IDA 0x71f6c0: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Contact::Contact(RBX::Primitive *,RBX::Primitive *)")]
#[doc(alias = "__ZN3RBX7ContactC2EPNS_9PrimitiveES2_")]
// 0x71f6cc — __ZN3RBX7ContactC2EPNS_9PrimitiveES2_
// type: _DWORD __fastcall(RBX::Contact *__hidden this, RBX::Primitive *, RBX::Primitive *)
pub fn stub_0x71f6cc() {
    // IDA 0x71f6cc: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::Contact::~Contact()")]
#[doc(alias = "__ZN3RBX7ContactD0Ev")]
// 0x71f6fc — __ZN3RBX7ContactD0Ev
// type: void __fastcall(RBX::Contact *__hidden this)
pub fn stub_0x71f6fc() {
    // IDA 0x71f6fc: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Contact::~Contact()")]
#[doc(alias = "__ZN3RBX7ContactD1Ev")]
// 0x71f79c — __ZN3RBX7ContactD1Ev
// type: void __fastcall(RBX::Contact *__hidden this)
pub fn stub_0x71f79c() {
    // IDA 0x71f79c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Contact::~Contact()")]
#[doc(alias = "__ZN3RBX7ContactD2Ev")]
// 0x71f7a0 — __ZN3RBX7ContactD2Ev
// type: void __fastcall(RBX::Contact *__hidden this)
pub fn stub_0x71f7a0() {
    // IDA 0x71f7a0: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Contact::primitiveMovedExternally(void)")]
#[doc(alias = "__ZN3RBX7Contact24primitiveMovedExternallyEv")]
// 0x71f890 — __ZN3RBX7Contact24primitiveMovedExternallyEv
// type: _DWORD __fastcall(RBX::Contact *__hidden this)
pub fn stub_0x71f890() {
    // IDA 0x71f890: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Contact::step(int)")]
#[doc(alias = "__ZN3RBX7Contact4stepEi")]
// 0x71f8d4 — __ZN3RBX7Contact4stepEi
// type: _DWORD __fastcall(RBX::Contact *__hidden this, int)
pub fn stub_0x71f8d4() {
    // IDA 0x71f8d4: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Contact::computeIsAdjacentUi(float)")]
#[doc(alias = "__ZN3RBX7Contact19computeIsAdjacentUiEf")]
// 0x71f9e4 — __ZN3RBX7Contact19computeIsAdjacentUiEf
// type: _DWORD __fastcall(RBX::Contact *__hidden this, float)
pub fn stub_0x71f9e4() {
    // IDA 0x71f9e4: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Contact::computeIsCollidingUi(float)")]
#[doc(alias = "__ZN3RBX7Contact20computeIsCollidingUiEf")]
// 0x71fa14 — __ZN3RBX7Contact20computeIsCollidingUiEf
// type: _DWORD __fastcall(RBX::Contact *__hidden this, float)
pub fn stub_0x71fa14() {
    // IDA 0x71fa14: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::calculateFriction(float,float)")]
#[doc(alias = "__ZN3RBX17calculateFrictionEff")]
// 0x71fa34 — __ZN3RBX17calculateFrictionEff
// type: _DWORD __fastcall(RBX *__hidden this, float, float)
pub fn stub_0x71fa34() {
    // IDA 0x71fa34: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Contact::onPrimitiveContactParametersChanged(void)")]
#[doc(alias = "__ZN3RBX7Contact35onPrimitiveContactParametersChangedEv")]
// 0x71fac4 — __ZN3RBX7Contact35onPrimitiveContactParametersChangedEv
// type: _DWORD __fastcall(RBX::Contact *__hidden this)
pub fn stub_0x71fac4() {
    // IDA 0x71fac4: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::Contact::deleteConnector(RBX::ContactConnector *)")]
#[doc(alias = "__ZN3RBX7Contact15deleteConnectorEPNS_16ContactConnectorE")]
// 0x71fbb8 — __ZN3RBX7Contact15deleteConnectorEPNS_16ContactConnectorE
// type: _DWORD __fastcall(RBX::Contact *__hidden this, RBX::ContactConnector *)
pub fn stub_0x71fbb8() {
    // IDA 0x71fbb8: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::Contact::generateDataForMovingAssemblyStage(void)")]
#[doc(alias = "__ZN3RBX7Contact34generateDataForMovingAssemblyStageEv")]
// 0x71fbdc — __ZN3RBX7Contact34generateDataForMovingAssemblyStageEv
// type: _DWORD __fastcall(RBX::Contact *__hidden this)
pub fn stub_0x71fbdc() {
    // IDA 0x71fbdc: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::BallBallContact::getConnector(int)")]
#[doc(alias = "__ZN3RBX15BallBallContact12getConnectorEi")]
// 0x71fc04 — __ZN3RBX15BallBallContact12getConnectorEi
// type: _DWORD __fastcall(RBX::BallBallContact *__hidden this, int)
pub fn stub_0x71fc04() {
    // IDA 0x71fc04: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::BallBallContact::deleteAllConnectors(void)")]
#[doc(alias = "__ZN3RBX15BallBallContact19deleteAllConnectorsEv")]
// 0x71fc08 — __ZN3RBX15BallBallContact19deleteAllConnectorsEv
// type: _DWORD __fastcall(RBX::BallBallContact *__hidden this)
pub fn stub_0x71fc08() {
    // IDA 0x71fc08: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::BallBallContact::computeIsColliding(float)")]
#[doc(alias = "__ZN3RBX15BallBallContact18computeIsCollidingEf")]
// 0x71fc24 — __ZN3RBX15BallBallContact18computeIsCollidingEf
// type: _DWORD __fastcall(RBX::BallBallContact *__hidden this, float)
pub fn stub_0x71fc24() {
    // IDA 0x71fc24: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::BallBallContact::stepContact(void)")]
#[doc(alias = "__ZN3RBX15BallBallContact11stepContactEv")]
// 0x71fcfc — __ZN3RBX15BallBallContact11stepContactEv
// type: _DWORD __fastcall(RBX::BallBallContact *__hidden this)
pub fn stub_0x71fcfc() {
    // IDA 0x71fcfc: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BallBallContact::generateDataForMovingAssemblyStage(void)")]
#[doc(alias = "__ZN3RBX15BallBallContact34generateDataForMovingAssemblyStageEv")]
// 0x71fec4 — __ZN3RBX15BallBallContact34generateDataForMovingAssemblyStageEv
// type: _DWORD __fastcall(RBX::BallBallContact *__hidden this)
pub fn stub_0x71fec4() {
    // IDA 0x71fec4: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::BallBlockContact::getConnector(int)")]
#[doc(alias = "__ZN3RBX16BallBlockContact12getConnectorEi")]
// 0x71fec8 — __ZN3RBX16BallBlockContact12getConnectorEi
// type: _DWORD __fastcall(RBX::BallBlockContact *__hidden this, int)
pub fn stub_0x71fec8() {
    // IDA 0x71fec8: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::BallBlockContact::deleteAllConnectors(void)")]
#[doc(alias = "__ZN3RBX16BallBlockContact19deleteAllConnectorsEv")]
// 0x71fecc — __ZN3RBX16BallBlockContact19deleteAllConnectorsEv
// type: _DWORD __fastcall(RBX::BallBlockContact *__hidden this)
pub fn stub_0x71fecc() {
    // IDA 0x71fecc: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::BallBlockContact::computeIsColliding(float)")]
#[doc(alias = "__ZN3RBX16BallBlockContact18computeIsCollidingEf")]
// 0x71fee8 — __ZN3RBX16BallBlockContact18computeIsCollidingEf
// type: _DWORD __fastcall(RBX::BallBlockContact *__hidden this, float)
pub fn stub_0x71fee8() {
    // IDA 0x71fee8: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::BallBlockContact::stepContact(void)")]
#[doc(alias = "__ZN3RBX16BallBlockContact11stepContactEv")]
// 0x7200f8 — __ZN3RBX16BallBlockContact11stepContactEv
// type: _DWORD __fastcall(RBX::BallBlockContact *__hidden this)
pub fn stub_0x7200f8() {
    // IDA 0x7200f8: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BallBlockContact::generateDataForMovingAssemblyStage(void)")]
#[doc(alias = "__ZN3RBX16BallBlockContact34generateDataForMovingAssemblyStageEv")]
// 0x72034c — __ZN3RBX16BallBlockContact34generateDataForMovingAssemblyStageEv
// type: _DWORD __fastcall(RBX::BallBlockContact *__hidden this)
pub fn stub_0x72034c() {
    // IDA 0x72034c: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::getConnector(int)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact12getConnectorEi")]
// 0x720354 — __ZN3RBX17BlockBlockContact12getConnectorEi
// type: _DWORD __fastcall(RBX::BlockBlockContact *__hidden this, int)
pub fn stub_0x720354() {
    // IDA 0x720354: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::deleteAllConnectorsOrig(void)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact23deleteAllConnectorsOrigEv")]
// 0x720388 — __ZN3RBX17BlockBlockContact23deleteAllConnectorsOrigEv
// type: _DWORD __fastcall(RBX::BlockBlockContact *__hidden this)
pub fn stub_0x720388() {
    // IDA 0x720388: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::deleteAllConnectorsFFlag(void)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact24deleteAllConnectorsFFlagEv")]
// 0x720414 — __ZN3RBX17BlockBlockContact24deleteAllConnectorsFFlagEv
// type: _DWORD __fastcall(RBX::BlockBlockContact *__hidden this)
pub fn stub_0x720414() {
    // IDA 0x720414: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::findGeoPairConnector(RBX::Body *,RBX::Body *,RBX::GeoPairType,int,int)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact20findGeoPairConnectorEPNS_4BodyES2_NS_11GeoPairTypeEii")]
// 0x7204b8 — __ZN3RBX17BlockBlockContact20findGeoPairConnectorEPNS_4BodyES2_NS_11GeoPairTypeEii
// type: 
pub fn stub_0x7204b8() {
    // IDA 0x7204b8: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContactData::findGeoPairConnector(RBX::Body *,RBX::Body *,RBX::GeoPairType,int,int)")]
#[doc(alias = "__ZN3RBX21BlockBlockContactData20findGeoPairConnectorEPNS_4BodyES2_NS_11GeoPairTypeEii")]
// 0x720548 — __ZN3RBX21BlockBlockContactData20findGeoPairConnectorEPNS_4BodyES2_NS_11GeoPairTypeEii
// type: int __fastcall(int, int, int, int, int, int, int, int, struct _Unwind_Exception *lpuexcpt, void *, int, int, int, int)
pub fn stub_0x720548() {
    // IDA 0x720548: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::findGeoPairConnectorFFlag(RBX::Body *,RBX::Body *,RBX::GeoPairType,int,int)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact25findGeoPairConnectorFFlagEPNS_4BodyES2_NS_11GeoPairTypeEii")]
// 0x720734 — __ZN3RBX17BlockBlockContact25findGeoPairConnectorFFlagEPNS_4BodyES2_NS_11GeoPairTypeEii
// type: 
pub fn stub_0x720734() {
    // IDA 0x720734: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContactData::findGeoPairConnectorFFlag(RBX::Body *,RBX::Body *,RBX::GeoPairType,int,int)")]
#[doc(alias = "__ZN3RBX21BlockBlockContactData25findGeoPairConnectorFFlagEPNS_4BodyES2_NS_11GeoPairTypeEii")]
// 0x7207c4 — __ZN3RBX21BlockBlockContactData25findGeoPairConnectorFFlagEPNS_4BodyES2_NS_11GeoPairTypeEii
// type: 
pub fn stub_0x7207c4() {
    // IDA 0x7207c4: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::computeIsColliding(float)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact18computeIsCollidingEf")]
// 0x720898 — __ZN3RBX17BlockBlockContact18computeIsCollidingEf
// type: _DWORD __fastcall(RBX::BlockBlockContact *__hidden this, float)
pub fn stub_0x720898() {
    // IDA 0x720898: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::computeIsColliding(float,bool &)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact18computeIsCollidingEfRb")]
// 0x7208b0 — __ZN3RBX17BlockBlockContact18computeIsCollidingEfRb
// type: _DWORD __fastcall(RBX::BlockBlockContact *__hidden this, float, bool *)
pub fn stub_0x7208b0() {
    // IDA 0x7208b0: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::stepContact(void)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact11stepContactEv")]
// 0x72090c — __ZN3RBX17BlockBlockContact11stepContactEv
// type: _DWORD __fastcall(RBX::BlockBlockContact *__hidden this)
pub fn stub_0x72090c() {
    // IDA 0x72090c: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContactData::stepContactFFlag(void)")]
#[doc(alias = "__ZN3RBX21BlockBlockContactData16stepContactFFlagEv")]
// 0x720988 — __ZN3RBX21BlockBlockContactData16stepContactFFlagEv
// type: _DWORD __fastcall(RBX::BlockBlockContactData *__hidden this)
pub fn stub_0x720988() {
    // IDA 0x720988: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContactData::stepContact(void)")]
#[doc(alias = "__ZN3RBX21BlockBlockContactData11stepContactEv")]
// 0x720a08 — __ZN3RBX21BlockBlockContactData11stepContactEv
// type: _DWORD __fastcall(RBX::BlockBlockContactData *__hidden this)
pub fn stub_0x720a08() {
    // IDA 0x720a08: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::loadGeoPairEdgeEdge(RBX::FixedArray<RBX::GeoPairConnector *,8ul> &,int,int,int,int)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact19loadGeoPairEdgeEdgeERNS_10FixedArrayIPNS_16GeoPairConnectorELm8EEEiiii")]
// 0x720aac — __ZN3RBX17BlockBlockContact19loadGeoPairEdgeEdgeERNS_10FixedArrayIPNS_16GeoPairConnectorELm8EEEiiii
// type: 
pub fn stub_0x720aac() {
    // IDA 0x720aac: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::loadGeoPairEdgeEdgeFFlag(int,int,int,int)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact24loadGeoPairEdgeEdgeFFlagEiiii")]
// 0x720bbc — __ZN3RBX17BlockBlockContact24loadGeoPairEdgeEdgeFFlagEiiii
// type: _DWORD __fastcall(RBX::BlockBlockContact *__hidden this, int, int, int, int)
pub fn stub_0x720bbc() {
    // IDA 0x720bbc: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::loadGeoPairPointPlane(RBX::FixedArray<RBX::GeoPairConnector *,8ul> &,int,int,int,RBX::NormalId,RBX::NormalId)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact21loadGeoPairPointPlaneERNS_10FixedArrayIPNS_16GeoPairConnectorELm8EEEiiiNS_8NormalIdES6_")]
// 0x720fe4 — __ZN3RBX17BlockBlockContact21loadGeoPairPointPlaneERNS_10FixedArrayIPNS_16GeoPairConnectorELm8EEEiiiNS_8NormalIdES6_
// type: 
pub fn stub_0x720fe4() {
    // IDA 0x720fe4: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::loadGeoPairPointPlaneFFlag(int,int,int,RBX::NormalId,RBX::NormalId)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact26loadGeoPairPointPlaneFFlagEiiiNS_8NormalIdES1_")]
// 0x7210c8 — __ZN3RBX17BlockBlockContact26loadGeoPairPointPlaneFFlagEiiiNS_8NormalIdES1_
// type: int __fastcall(int, int, int, int, int, int, int, struct _Unwind_Exception *lpuexcpt, int, void *, int, int, int, int)
pub fn stub_0x7210c8() {
    // IDA 0x7210c8: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::geoFeaturesOverlap(int,int,int,RBX::NormalId,RBX::NormalId)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact18geoFeaturesOverlapEiiiNS_8NormalIdES1_")]
// 0x7215a4 — __ZN3RBX17BlockBlockContact18geoFeaturesOverlapEiiiNS_8NormalIdES1_
// type: 
pub fn stub_0x7215a4() {
    // IDA 0x7215a4: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContactData::loadGeoPairEdgeEdgePlane(RBX::FixedArray<RBX::GeoPairConnector *,8ul> &,int,int,int,int)")]
#[doc(alias = "__ZN3RBX21BlockBlockContactData24loadGeoPairEdgeEdgePlaneERNS_10FixedArrayIPNS_16GeoPairConnectorELm8EEEiiii")]
// 0x721778 — __ZN3RBX21BlockBlockContactData24loadGeoPairEdgeEdgePlaneERNS_10FixedArrayIPNS_16GeoPairConnectorELm8EEEiiii
// type: int __fastcall(int, void *, int, int, int, int)
pub fn stub_0x721778() {
    // IDA 0x721778: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContactData::getBestPlaneEdge(float,bool &)")]
#[doc(alias = "__ZN3RBX21BlockBlockContactData16getBestPlaneEdgeEfRb")]
// 0x721920 — __ZN3RBX21BlockBlockContactData16getBestPlaneEdgeEfRb
// type: _DWORD __fastcall(RBX::BlockBlockContactData *__hidden this, float, bool *)
pub fn stub_0x721920() {
    // IDA 0x721920: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContactData::computePlaneContact(RBX::FixedArray<RBX::GeoPairConnector *,8ul> &)")]
#[doc(alias = "__ZN3RBX21BlockBlockContactData19computePlaneContactERNS_10FixedArrayIPNS_16GeoPairConnectorELm8EEE")]
// 0x72205c — __ZN3RBX21BlockBlockContactData19computePlaneContactERNS_10FixedArrayIPNS_16GeoPairConnectorELm8EEE
// type: 
pub fn stub_0x72205c() {
    // IDA 0x72205c: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::BlockBlockContact(RBX::Primitive *,RBX::Primitive *)")]
#[doc(alias = "__ZN3RBX17BlockBlockContactC1EPNS_9PrimitiveES2_")]
// 0x7223c0 — __ZN3RBX17BlockBlockContactC1EPNS_9PrimitiveES2_
// type: _DWORD __fastcall(RBX::BlockBlockContact *__hidden this, RBX::Primitive *, RBX::Primitive *)
pub fn stub_0x7223c0() {
    // IDA 0x7223c0: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::BlockBlockContact(RBX::Primitive *,RBX::Primitive *)")]
#[doc(alias = "__ZN3RBX17BlockBlockContactC2EPNS_9PrimitiveES2_")]
// 0x7223c4 — __ZN3RBX17BlockBlockContactC2EPNS_9PrimitiveES2_
// type: _DWORD __fastcall(RBX::BlockBlockContact *__hidden this, RBX::Primitive *, RBX::Primitive *)
pub fn stub_0x7223c4() {
    // IDA 0x7223c4: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::BlockBlockContact::~BlockBlockContact()")]
#[doc(alias = "__ZN3RBX17BlockBlockContactD0Ev")]
// 0x7224b8 — __ZN3RBX17BlockBlockContactD0Ev
// type: void __fastcall(RBX::BlockBlockContact *__hidden this)
pub fn stub_0x7224b8() {
    // IDA 0x7224b8: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::BlockBlockContact::~BlockBlockContact()")]
#[doc(alias = "__ZN3RBX17BlockBlockContactD1Ev")]
// 0x72256c — __ZN3RBX17BlockBlockContactD1Ev
// type: void __fastcall(RBX::BlockBlockContact *__hidden this)
pub fn stub_0x72256c() {
    // IDA 0x72256c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::BlockBlockContact::~BlockBlockContact()")]
#[doc(alias = "__ZN3RBX17BlockBlockContactD2Ev")]
// 0x722570 — __ZN3RBX17BlockBlockContactD2Ev
// type: void __fastcall(RBX::BlockBlockContact *__hidden this)
pub fn stub_0x722570() {
    // IDA 0x722570: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::BlockBlockContact::numConnectors(void)const")]
#[doc(alias = "__ZNK3RBX17BlockBlockContact13numConnectorsEv")]
// 0x722598 — __ZNK3RBX17BlockBlockContact13numConnectorsEv
// type: _DWORD __fastcall(RBX::BlockBlockContact *__hidden this)
pub fn stub_0x722598() {
    // IDA 0x722598: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::BlockBlockContact::generateDataForMovingAssemblyStage(void)")]
#[doc(alias = "__ZN3RBX17BlockBlockContact34generateDataForMovingAssemblyStageEv")]
// 0x7225c8 — __ZN3RBX17BlockBlockContact34generateDataForMovingAssemblyStageEv
// type: _DWORD __fastcall(RBX::BlockBlockContact *__hidden this)
pub fn stub_0x7225c8() {
    // IDA 0x7225c8: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::BlockBlockContactData::computePlaneContactFFlag(void)")]
#[doc(alias = "__ZN3RBX21BlockBlockContactData24computePlaneContactFFlagEv")]
// 0x722608 — __ZN3RBX21BlockBlockContactData24computePlaneContactFFlagEv
// type: _DWORD __fastcall(RBX::BlockBlockContactData *__hidden this)
pub fn stub_0x722608() {
    // IDA 0x722608: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::BlockBlockContactData::loadGeoPairEdgeEdgePlaneFFlag(int,int,int,int)")]
#[doc(alias = "__ZN3RBX21BlockBlockContactData29loadGeoPairEdgeEdgePlaneFFlagEiiii")]
// 0x722968 — __ZN3RBX21BlockBlockContactData29loadGeoPairEdgeEdgePlaneFFlagEiiii
// type: _DWORD __fastcall(RBX::BlockBlockContactData *__hidden this, int, int, int, int)
pub fn stub_0x722968() {
    // IDA 0x722968: simulation/instance gameplay wiring owned by the datamodel crate — carrier no-op in core.
}

#[doc(alias = "RBX::Edge::~Edge()")]
#[doc(alias = "__ZN3RBX4EdgeD2Ev")]
// 0x723508 — __ZN3RBX4EdgeD2Ev
// type: void __fastcall(RBX::Edge *__hidden this)
pub fn stub_0x723508() {
    // IDA 0x723508: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Allocator<RBX::BallBallConnector>::operator new(unsigned long)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_17BallBallConnectorEEnwEm")]
// 0x723748 — __ZN3RBX9AllocatorINS_17BallBallConnectorEEnwEm
// type: 
pub fn stub_0x723748() {
    // IDA 0x723748: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Allocator<RBX::BallBlockConnector>::operator new(unsigned long)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_18BallBlockConnectorEEnwEm")]
// 0x7237b8 — __ZN3RBX9AllocatorINS_18BallBlockConnectorEEnwEm
// type: 
pub fn stub_0x7237b8() {
    // IDA 0x7237b8: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Block::getEdgeVertex(int)const")]
#[doc(alias = "__ZNK3RBX5Block13getEdgeVertexEi")]
// 0x723828 — __ZNK3RBX5Block13getEdgeVertexEi
// type: _DWORD __fastcall(RBX::Block *__hidden this, int)
pub fn stub_0x723828() {
    // IDA 0x723828: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::FixedArray<RBX::GeoPairConnector *,8ul>::push_back(RBX::GeoPairConnector * const&)")]
#[doc(alias = "__ZN3RBX10FixedArrayIPNS_16GeoPairConnectorELm8EE9push_backERKS2_")]
// 0x7238f0 — __ZN3RBX10FixedArrayIPNS_16GeoPairConnectorELm8EE9push_backERKS2_
// type: 
pub fn stub_0x7238f0() {
    // IDA 0x7238f0: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Allocator<RBX::GeoPairConnector>::operator new(unsigned long)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_16GeoPairConnectorEEnwEm")]
// 0x723958 — __ZN3RBX9AllocatorINS_16GeoPairConnectorEEnwEm
// type: 
pub fn stub_0x723958() {
    // IDA 0x723958: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Allocator<RBX::GeoPairConnector>::operator delete(void *)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_16GeoPairConnectorEEdlEPv")]
// 0x7239c8 — __ZN3RBX9AllocatorINS_16GeoPairConnectorEEdlEPv
// type: void __fastcall(void *)
pub fn stub_0x7239c8() {
    // IDA 0x7239c8: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::ContactConnector::isIntersecting(void)")]
#[doc(alias = "__ZN3RBX16ContactConnector14isIntersectingEv")]
// 0x723a08 — __ZN3RBX16ContactConnector14isIntersectingEv
// type: _DWORD __fastcall(RBX::ContactConnector *__hidden this)
pub fn stub_0x723a08() {
    // IDA 0x723a08: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Allocator<RBX::BlockBlockContact>::Allocator(void)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_17BlockBlockContactEEC2Ev")]
// 0x723a7c — __ZN3RBX9AllocatorINS_17BlockBlockContactEEC2Ev
// type: 
pub fn stub_0x723a7c() {
    // IDA 0x723a7c: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Allocator<RBX::BlockBlockContact>::operator delete(void *)")]
#[doc(alias = "__ZN3RBX9AllocatorINS_17BlockBlockContactEEdlEPv")]
// 0x723ae0 — __ZN3RBX9AllocatorINS_17BlockBlockContactEEdlEPv
// type: void __fastcall(void *)
pub fn stub_0x723ae0() {
    // IDA 0x723ae0: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::CornerWedgePoly::isGeometryOrthogonal(void)const")]
#[doc(alias = "__ZNK3RBX15CornerWedgePoly20isGeometryOrthogonalEv")]
// 0x74d130 — __ZNK3RBX15CornerWedgePoly20isGeometryOrthogonalEv
// type: _DWORD __fastcall(RBX::CornerWedgePoly *__hidden this)
pub fn stub_0x74d130() {
    // IDA 0x74d130: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::RightAngleRampPoly::getGeometryType(void)const")]
#[doc(alias = "__ZNK3RBX18RightAngleRampPoly15getGeometryTypeEv")]
// 0x74d134 — __ZNK3RBX18RightAngleRampPoly15getGeometryTypeEv
// type: _DWORD __fastcall(RBX::RightAngleRampPoly *__hidden this)
pub fn stub_0x74d134() {
    // IDA 0x74d134: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::RightAngleRampPoly::isGeometryOrthogonal(void)const")]
#[doc(alias = "__ZNK3RBX18RightAngleRampPoly20isGeometryOrthogonalEv")]
// 0x74d138 — __ZNK3RBX18RightAngleRampPoly20isGeometryOrthogonalEv
// type: _DWORD __fastcall(RBX::RightAngleRampPoly *__hidden this)
pub fn stub_0x74d138() {
    // IDA 0x74d138: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::ParallelRampPoly::getGeometryType(void)const")]
#[doc(alias = "__ZNK3RBX16ParallelRampPoly15getGeometryTypeEv")]
// 0x74d13c — __ZNK3RBX16ParallelRampPoly15getGeometryTypeEv
// type: _DWORD __fastcall(RBX::ParallelRampPoly *__hidden this)
pub fn stub_0x74d13c() {
    // IDA 0x74d13c: physics world/stage/poly geometry type. Collision-world internals owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::ParallelRampPoly::isGeometryOrthogonal(void)const")]
#[doc(alias = "__ZNK3RBX16ParallelRampPoly20isGeometryOrthogonalEv")]
// 0x74d140 — __ZNK3RBX16ParallelRampPoly20isGeometryOrthogonalEv
// type: _DWORD __fastcall(RBX::ParallelRampPoly *__hidden this)
pub fn stub_0x74d140() {
    // IDA 0x74d140: physics world/stage/poly geometry type. Collision-world internals owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::PyramidPoly::getGeometryType(void)const")]
#[doc(alias = "__ZNK3RBX11PyramidPoly15getGeometryTypeEv")]
// 0x74d144 — __ZNK3RBX11PyramidPoly15getGeometryTypeEv
// type: int __fastcall(RBX::PyramidPoly *this)
pub fn stub_0x74d144() {
    // IDA 0x74d144: physics world/stage/poly geometry type. Collision-world internals owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::PyramidPoly::isGeometryOrthogonal(void)const")]
#[doc(alias = "__ZNK3RBX11PyramidPoly20isGeometryOrthogonalEv")]
// 0x74d148 — __ZNK3RBX11PyramidPoly20isGeometryOrthogonalEv
// type: _DWORD __fastcall(RBX::PyramidPoly *__hidden this)
pub fn stub_0x74d148() {
    // IDA 0x74d148: physics world/stage/poly geometry type. Collision-world internals owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::PrismPoly::getGeometryType(void)const")]
#[doc(alias = "__ZNK3RBX9PrismPoly15getGeometryTypeEv")]
// 0x74d14c — __ZNK3RBX9PrismPoly15getGeometryTypeEv
// type: _DWORD __fastcall(RBX::PrismPoly *__hidden this)
pub fn stub_0x74d14c() {
    // IDA 0x74d14c: physics world/stage/poly geometry type. Collision-world internals owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::PrismPoly::isGeometryOrthogonal(void)const")]
#[doc(alias = "__ZNK3RBX9PrismPoly20isGeometryOrthogonalEv")]
// 0x74d150 — __ZNK3RBX9PrismPoly20isGeometryOrthogonalEv
// type: _DWORD __fastcall(RBX::PrismPoly *__hidden this)
pub fn stub_0x74d150() {
    // IDA 0x74d150: physics world/stage/poly geometry type. Collision-world internals owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::WedgePoly::getGeometryType(void)const")]
#[doc(alias = "__ZNK3RBX9WedgePoly15getGeometryTypeEv")]
// 0x74d154 — __ZNK3RBX9WedgePoly15getGeometryTypeEv
// type: _DWORD __fastcall(RBX::WedgePoly *__hidden this)
pub fn stub_0x74d154() {
    // IDA 0x74d154: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::WedgePoly::isGeometryOrthogonal(void)const")]
#[doc(alias = "__ZNK3RBX9WedgePoly20isGeometryOrthogonalEv")]
// 0x74d158 — __ZNK3RBX9WedgePoly20isGeometryOrthogonalEv
// type: _DWORD __fastcall(RBX::WedgePoly *__hidden this)
pub fn stub_0x74d158() {
    // IDA 0x74d158: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Ball::~Ball()")]
#[doc(alias = "__ZN3RBX4BallD1Ev")]
// 0x74d15c — __ZN3RBX4BallD1Ev
// type: void __fastcall(RBX::Ball *__hidden this)
pub fn stub_0x74d15c() {
    // IDA 0x74d15c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Ball::~Ball()")]
#[doc(alias = "__ZN3RBX4BallD0Ev")]
// 0x74d160 — __ZN3RBX4BallD0Ev
// type: void __fastcall(RBX::Ball *__hidden this)
pub fn stub_0x74d160() {
    // IDA 0x74d160: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Ball::getGeometryType(void)const")]
#[doc(alias = "__ZNK3RBX4Ball15getGeometryTypeEv")]
// 0x74d164 — __ZNK3RBX4Ball15getGeometryTypeEv
// type: _DWORD __fastcall(RBX::Ball *__hidden this)
pub fn stub_0x74d164() {
    // IDA 0x74d164: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Ball::getCollideType(void)const")]
#[doc(alias = "__ZNK3RBX4Ball14getCollideTypeEv")]
// 0x74d168 — __ZNK3RBX4Ball14getCollideTypeEv
// type: _DWORD __fastcall(RBX::Ball *__hidden this)
pub fn stub_0x74d168() {
    // IDA 0x74d168: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Ball::getRadius(void)const")]
#[doc(alias = "__ZNK3RBX4Ball9getRadiusEv")]
// 0x74d16c — __ZNK3RBX4Ball9getRadiusEv
// type: _DWORD __fastcall(RBX::Ball *__hidden this)
pub fn stub_0x74d16c() {
    // IDA 0x74d16c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::Ball::getNumSurfaces(void)const")]
#[doc(alias = "__ZNK3RBX4Ball14getNumSurfacesEv")]
// 0x74d170 — __ZNK3RBX4Ball14getNumSurfacesEv
// type: _DWORD __fastcall(RBX::Ball *__hidden this)
pub fn stub_0x74d170() {
    // IDA 0x74d170: physics world/stage/poly geometry type. Collision-world internals owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::Ball::getMoment(float)const")]
#[doc(alias = "__ZNK3RBX4Ball9getMomentEf")]
// 0x74d28c — __ZNK3RBX4Ball9getMomentEf
// type: _DWORD __fastcall(RBX::Ball *__hidden this, float)
pub fn stub_0x74d28c() {
    // IDA 0x74d28c: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::EdgeList::~EdgeList()")]
#[doc(alias = "__ZN3RBX8EdgeListD2Ev")]
// 0x74d298 — __ZN3RBX8EdgeListD2Ev
// type: void __fastcall(RBX::EdgeList *__hidden this)
pub fn stub_0x74d298() {
    // IDA 0x74d298: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::EdgeList::EdgeList(RBX::Primitive *)")]
#[doc(alias = "__ZN3RBX8EdgeListC2EPNS_9PrimitiveE")]
// 0x74d420 — __ZN3RBX8EdgeListC2EPNS_9PrimitiveE
// type: _DWORD __fastcall(RBX::EdgeList *__hidden this, RBX::Primitive *)
pub fn stub_0x74d420() {
    // IDA 0x74d420: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::SpanningNode::~SpanningNode()")]
#[doc(alias = "__ZN3RBX12SpanningNodeD1Ev")]
// 0x74d4d8 — __ZN3RBX12SpanningNodeD1Ev
// type: void __fastcall(RBX::SpanningNode *__hidden this)
pub fn stub_0x74d4d8() {
    // IDA 0x74d4d8: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::SpanningNode::~SpanningNode()")]
#[doc(alias = "__ZN3RBX12SpanningNodeD0Ev")]
// 0x74d4dc — __ZN3RBX12SpanningNodeD0Ev
// type: void __fastcall(RBX::SpanningNode *__hidden this)
pub fn stub_0x74d4dc() {
    // IDA 0x74d4dc: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::PrismPoly::buildMesh(void)")]
#[doc(alias = "__ZN3RBX9PrismPoly9buildMeshEv")]
// 0x74da88 — __ZN3RBX9PrismPoly9buildMeshEv
// type: _DWORD __fastcall(RBX::PrismPoly *__hidden this)
pub fn stub_0x74da88() {
    // IDA 0x74da88: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::PrismPoly::setGeometryParameter(std::string const&,int)")]
#[doc(alias = "__ZN3RBX9PrismPoly20setGeometryParameterERKSsi")]
// 0x74db6c — __ZN3RBX9PrismPoly20setGeometryParameterERKSsi
// type: _DWORD __fastcall(RBX::PrismPoly *__hidden this, const std::string *, int)
pub fn stub_0x74db6c() {
    // IDA 0x74db6c: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::PrismPoly::getGeometryParameter(std::string const&)const")]
#[doc(alias = "__ZNK3RBX9PrismPoly20getGeometryParameterERKSs")]
// 0x74dc10 — __ZNK3RBX9PrismPoly20getGeometryParameterERKSs
// type: _DWORD __fastcall(RBX::PrismPoly *__hidden this, const std::string *)
pub fn stub_0x74dc10() {
    // IDA 0x74dc10: C++ dtor/thunk (deleting dtors adjust this, run member dtors, release). Drop glue — no-op.
}

#[doc(alias = "RBX::PrismPoly::getMoment(float)const")]
#[doc(alias = "__ZNK3RBX9PrismPoly9getMomentEf")]
// 0x74dc9c — __ZNK3RBX9PrismPoly9getMomentEf
// type: _DWORD __fastcall(RBX::PrismPoly *__hidden this, float)
pub fn stub_0x74dc9c() {
    // IDA 0x74dc9c: libstdc++ container/algorithm internals. Vec/BTreeMap/VecDeque/Iterator — monomorph artifact, no-op carrier.
}

#[doc(alias = "RBX::PrismPoly::getCofmOffset(void)const")]
#[doc(alias = "__ZNK3RBX9PrismPoly13getCofmOffsetEv")]
// 0x74ddd0 — __ZNK3RBX9PrismPoly13getCofmOffsetEv
// type: _DWORD __fastcall(RBX::PrismPoly *__hidden this)
pub fn stub_0x74ddd0() {
    // IDA 0x74ddd0: libstdc++ container/algorithm internals. Vec/BTreeMap/VecDeque/Iterator — monomorph artifact, no-op carrier.
}

#[doc(alias = "RBX::PrismPoly::getSurfaceCoordInBody(unsigned long)const")]
#[doc(alias = "__ZNK3RBX9PrismPoly21getSurfaceCoordInBodyEm")]
// 0x74de04 — __ZNK3RBX9PrismPoly21getSurfaceCoordInBodyEm
// type: _DWORD __fastcall(RBX::PrismPoly *__hidden this, unsigned int)
pub fn stub_0x74de04() {
    // IDA 0x74de04: libstdc++ container/algorithm internals. Vec/BTreeMap/VecDeque/Iterator — monomorph artifact, no-op carrier.
}

#[doc(alias = "RBX::PrismPoly::getFaceFromLegacyNormalId(RBX::NormalId)const")]
#[doc(alias = "__ZNK3RBX9PrismPoly25getFaceFromLegacyNormalIdENS_8NormalIdE")]
// 0x74e174 — __ZNK3RBX9PrismPoly25getFaceFromLegacyNormalIdENS_8NormalIdE
// type: 
pub fn stub_0x74e174() {
    // IDA 0x74e174: physics-engine collision/contact helper (Body/Primitive/Contact graph). Owned by higher crates — carrier no-op in core.
}

#[doc(alias = "RBX::GeometryPool<RBX::Vector3_2Ints,RBX::POLY::PrismMesh,RBX::Vector3_2IntsComparer>::getToken(RBX::Vector3_2Ints const&)")]
#[doc(alias = "__ZN3RBX12GeometryPoolINS_13Vector3_2IntsENS_4POLY9PrismMeshENS_21Vector3_2IntsComparerEE8getTokenERKS1_")]
// 0x74e2cc — __ZN3RBX12GeometryPoolINS_13Vector3_2IntsENS_4POLY9PrismMeshENS_21Vector3_2IntsComparerEE8getTokenERKS1_
// type: int __fastcall(int, int, int, int, struct _Unwind_Exception *lpuexcpt, void *, int, int, int, int)
pub fn stub_0x74e2cc() {
    // IDA 0x74e2cc: RBXL/G3D model-geometry helper owned by the datamodel/rendering crates — carrier no-op in core.
}


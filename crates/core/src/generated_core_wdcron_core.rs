//! core — generated_core_wdcron_core — 120 stubs EA-sorted gap filler distinct not yet in crates/core/src
//! Source: ida/export.json (85545 funcs) EA-sorted gap filler, next 120 uncovered not in crates/core/src
//! Sanitized: boost::shared_ptr -> rbx_core::SharedPtr, uses rbx_core::SharedPtr not boost

#![allow(non_snake_case, dead_code, unused_variables, unused_imports, clippy::all)]
use crate::SharedPtr;
const _SHARED_PTR: Option<SharedPtr<u8>> = None;

// 0x7fd9c — __ZN4FMOD5Codec7releaseEv
// type: int __fastcall(FMOD::Codec *this)
#[doc(alias = "__ZN4FMOD5Codec7releaseEv")]
pub fn stub_0x7fd9c() {
    // IDA 0x7fd9c: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x7fe6c — __ZN4FMOD5Codec11setPositionEijj
// type: int __fastcall(FMOD::Codec *this, int, unsigned int, unsigned int)
#[doc(alias = "__ZN4FMOD5Codec11setPositionEijj")]
pub fn stub_0x7fe6c() {
    // IDA 0x7fe6c: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x80388 — __ZN4FMOD9CodecAIFF19setPositionInternalEijj
// type: int __fastcall(FMOD::CodecAIFF *this, int, unsigned int, unsigned int)
#[doc(alias = "__ZN4FMOD9CodecAIFF19setPositionInternalEijj")]
pub fn stub_0x80388() {
    // IDA 0x80388: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x804cc — __ZN4FMOD9CodecAIFF19setPositionCallbackEP16FMOD_CODEC_STATEijj
// type: int __fastcall(FMOD::CodecAIFF *, int, unsigned int, unsigned int)
#[doc(alias = "__ZN4FMOD9CodecAIFF19setPositionCallbackEP16FMOD_CODEC_STATEijj")]
pub fn stub_0x804cc() {
    // IDA 0x804cc: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x804d8 — __ZN4FMOD9CodecAIFF12readInternalEPvjPj
// type: int __fastcall(FMOD::CodecAIFF *this, char *, unsigned int, unsigned int *)
#[doc(alias = "__ZN4FMOD9CodecAIFF12readInternalEPvjPj")]
pub fn stub_0x804d8() {
    // IDA 0x804d8: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x806e4 — __ZN4FMOD9CodecAIFF12readCallbackEP16FMOD_CODEC_STATEPvjPj
// type: int __fastcall(FMOD::CodecAIFF *, char *, unsigned int, unsigned int *)
#[doc(alias = "__ZN4FMOD9CodecAIFF12readCallbackEP16FMOD_CODEC_STATEPvjPj")]
pub fn stub_0x806e4() {
    // IDA 0x806e4: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x806f0 — __ZN4FMOD9CodecAIFF13closeInternalEv
// type: int __fastcall(FMOD::CodecAIFF *this)
#[doc(alias = "__ZN4FMOD9CodecAIFF13closeInternalEv")]
pub fn stub_0x806f0() {
    // IDA 0x806f0: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x80744 — __ZN4FMOD9CodecAIFF13closeCallbackEP16FMOD_CODEC_STATE
// type: int __fastcall(FMOD::CodecAIFF *)
#[doc(alias = "__ZN4FMOD9CodecAIFF13closeCallbackEP16FMOD_CODEC_STATE")]
pub fn stub_0x80744() {
    // IDA 0x80744: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x80750 — __ZN4FMOD23ConvertFromIeeeExtendedEPh
// type: int __fastcall(FMOD *this, unsigned __int8 *)
#[doc(alias = "__ZN4FMOD23ConvertFromIeeeExtendedEPh")]
pub fn stub_0x80750() {
    // IDA 0x80750: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x80864 — __ZN4FMOD9CodecAIFF12openInternalEjP22FMOD_CREATESOUNDEXINFO
// type: int __fastcall(int)
#[doc(alias = "__ZN4FMOD9CodecAIFF12openInternalEjP22FMOD_CREATESOUNDEXINFO")]
pub fn stub_0x80864() {
    // IDA 0x80864: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x81068 — __ZN4FMOD9CodecAIFF12openCallbackEP16FMOD_CODEC_STATEjP22FMOD_CREATESOUNDEXINFO
// type: int __fastcall(int)
#[doc(alias = "__ZN4FMOD9CodecAIFF12openCallbackEP16FMOD_CODEC_STATEjP22FMOD_CREATESOUNDEXINFO")]
pub fn stub_0x81068() {
    // IDA 0x81068: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x81074 — __ZN4FMOD9CodecAIFF16getDescriptionExEv
// type: int *__fastcall(FMOD::CodecAIFF *this)
#[doc(alias = "__ZN4FMOD9CodecAIFF16getDescriptionExEv")]
pub fn stub_0x81074() {
    // IDA 0x81074: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x8115c — __GLOBAL__I__ZN4FMOD9aiffcodecE
// type: int()
#[doc(alias = "__GLOBAL__I__ZN4FMOD9aiffcodecE")]
pub fn stub_0x8115c() {
    // IDA 0x8115c: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x81168 — __ZN4FMOD8CodecDLS19setPositionInternalEijj
// type: int __fastcall(FMOD::CodecDLS *this, int, unsigned int, unsigned int)
#[doc(alias = "__ZN4FMOD8CodecDLS19setPositionInternalEijj")]
pub fn stub_0x81168() {
    // IDA 0x81168: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x8132c — __ZN4FMOD8CodecDLS19setPositionCallbackEP16FMOD_CODEC_STATEijj
// type: int __fastcall(FMOD::CodecDLS *, int, unsigned int, unsigned int)
#[doc(alias = "__ZN4FMOD8CodecDLS19setPositionCallbackEP16FMOD_CODEC_STATEijj")]
pub fn stub_0x8132c() {
    // IDA 0x8132c: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x81338 — __ZN4FMOD8CodecDLS12readInternalEPvjPj
// type: int __fastcall(FMOD::File **this, void *, unsigned int, unsigned int *)
#[doc(alias = "__ZN4FMOD8CodecDLS12readInternalEPvjPj")]
pub fn stub_0x81338() {
    // IDA 0x81338: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x813e8 — __ZN4FMOD8CodecDLS12readCallbackEP16FMOD_CODEC_STATEPvjPj
// type: int __fastcall(FMOD::File **, void *, unsigned int, unsigned int *)
#[doc(alias = "__ZN4FMOD8CodecDLS12readCallbackEP16FMOD_CODEC_STATEPvjPj")]
pub fn stub_0x813e8() {
    // IDA 0x813e8: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x813f4 — __ZN4FMOD8CodecDLS13closeInternalEv
// type: int __fastcall(FMOD::CodecDLS *this)
#[doc(alias = "__ZN4FMOD8CodecDLS13closeInternalEv")]
pub fn stub_0x813f4() {
    // IDA 0x813f4: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x815e0 — __ZN4FMOD8CodecDLS13closeCallbackEP16FMOD_CODEC_STATE
// type: int __fastcall(FMOD::CodecDLS *)
#[doc(alias = "__ZN4FMOD8CodecDLS13closeCallbackEP16FMOD_CODEC_STATE")]
pub fn stub_0x815e0() {
    // IDA 0x815e0: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x815ec — __ZN4FMOD8CodecDLS16getDescriptionExEv
// type: int *__fastcall(FMOD::CodecDLS *this)
#[doc(alias = "__ZN4FMOD8CodecDLS16getDescriptionExEv")]
pub fn stub_0x815ec() {
    // IDA 0x815ec: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x8168c — __ZN4FMOD8CodecDLS10parseChunkEPcj
// type: int __fastcall(FMOD::File **this, char *, unsigned int)
#[doc(alias = "__ZN4FMOD8CodecDLS10parseChunkEPcj")]
pub fn stub_0x8168c() {
    // IDA 0x8168c: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x82848 — __ZN4FMOD8CodecDLS12openInternalEjP22FMOD_CREATESOUNDEXINFO
// type: int __fastcall(int)
#[doc(alias = "__ZN4FMOD8CodecDLS12openInternalEjP22FMOD_CREATESOUNDEXINFO")]
pub fn stub_0x82848() {
    // IDA 0x82848: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x82970 — __ZN4FMOD8CodecDLS12openCallbackEP16FMOD_CODEC_STATEjP22FMOD_CREATESOUNDEXINFO
// type: int __fastcall(int)
#[doc(alias = "__ZN4FMOD8CodecDLS12openCallbackEP16FMOD_CODEC_STATEjP22FMOD_CREATESOUNDEXINFO")]
pub fn stub_0x82970() {
    // IDA 0x82970: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x829c8 — __GLOBAL__I__ZN4FMOD8dlscodecE
// type: int()
#[doc(alias = "__GLOBAL__I__ZN4FMOD8dlscodecE")]
pub fn stub_0x829c8() {
    // IDA 0x829c8: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x829d4 — __ZN4FMODL24FMOD_FLAC_LengthCallbackEPK19FLAC__StreamDecoderPyPv
// type: int __fastcall(int, _DWORD *, int)
#[doc(alias = "__ZN4FMODL24FMOD_FLAC_LengthCallbackEPK19FLAC__StreamDecoderPyPv")]
pub fn stub_0x829d4() {
    // IDA 0x829d4: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x82a1c — __ZN4FMODL23FMOD_FLAC_ErrorCallbackEPK19FLAC__StreamDecoder30FLAC__StreamDecoderErrorStatusPv
// type: void()
#[doc(alias = "__ZN4FMODL23FMOD_FLAC_ErrorCallbackEPK19FLAC__StreamDecoder30FLAC__StreamDecoderErrorStatusPv")]
pub fn stub_0x82a1c() {
    // IDA 0x82a1c: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x82a20 — __ZN4FMOD9CodecFLAC19setPositionInternalEijj
// type: int __fastcall(FMOD::CodecFLAC *this, int, unsigned int, unsigned int)
#[doc(alias = "__ZN4FMOD9CodecFLAC19setPositionInternalEijj")]
pub fn stub_0x82a20() {
    // IDA 0x82a20: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x948b4 — __ZN4FMOD8CodecMOD12openInternalEjP22FMOD_CREATESOUNDEXINFO
// type: int __fastcall(int, __int16, int)
#[doc(alias = "__ZN4FMOD8CodecMOD12openInternalEjP22FMOD_CREATESOUNDEXINFO")]
pub fn stub_0x948b4() {
    // IDA 0x948b4: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x95a74 — __ZN4FMOD8CodecMOD12openCallbackEP16FMOD_CODEC_STATEjP22FMOD_CREATESOUNDEXINFO
// type: int __fastcall(int, __int16, int)
#[doc(alias = "__ZN4FMOD8CodecMOD12openCallbackEP16FMOD_CODEC_STATEjP22FMOD_CREATESOUNDEXINFO")]
pub fn stub_0x95a74() {
    // IDA 0x95a74: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x95a80 — __ZN4FMOD8CodecMOD12readInternalEPvjPj
// type: unsigned int *__fastcall(FMOD::CodecMOD *this, char *, unsigned int, unsigned int *)
#[doc(alias = "__ZN4FMOD8CodecMOD12readInternalEPvjPj")]
pub fn stub_0x95a80() {
    // IDA 0x95a80: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x95e64 — __ZN4FMOD8CodecMOD12readCallbackEP16FMOD_CODEC_STATEPvjPj
// type: unsigned int *__fastcall(FMOD::CodecMOD *, char *, unsigned int, unsigned int *)
#[doc(alias = "__ZN4FMOD8CodecMOD12readCallbackEP16FMOD_CODEC_STATEPvjPj")]
pub fn stub_0x95e64() {
    // IDA 0x95e64: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x95ebc — __GLOBAL__I__ZN4FMOD8modcodecE
// type: int()
#[doc(alias = "__GLOBAL__I__ZN4FMOD8modcodecE")]
pub fn stub_0x95ebc() {
    // IDA 0x95ebc: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x95ec8 — __ZN4FMOD9CodecMPEG13resetCallbackEP16FMOD_CODEC_STATE
// type: int __fastcall(FMOD::CodecMPEG *)
#[doc(alias = "__ZN4FMOD9CodecMPEG13resetCallbackEP16FMOD_CODEC_STATE")]
pub fn stub_0x95ec8() {
    // IDA 0x95ec8: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x95ee0 — __ZN4FMOD9CodecMPEG19soundCreateInternalEiP10FMOD_SOUND
// type: int __fastcall(int, int, FMOD::SoundI *this)
#[doc(alias = "__ZN4FMOD9CodecMPEG19soundCreateInternalEiP10FMOD_SOUND")]
pub fn stub_0x95ee0() {
    // IDA 0x95ee0: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x95fe8 — __ZN4FMOD9CodecMPEG19soundCreateCallbackEP16FMOD_CODEC_STATEiP10FMOD_SOUND
// type: int __fastcall(int, int, FMOD::SoundI *)
#[doc(alias = "__ZN4FMOD9CodecMPEG19soundCreateCallbackEP16FMOD_CODEC_STATEiP10FMOD_SOUND")]
pub fn stub_0x95fe8() {
    // IDA 0x95fe8: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x95ff4 — __ZN4FMOD9CodecMPEG13closeInternalEv
// type: int __fastcall(FMOD::CodecMPEG *this)
#[doc(alias = "__ZN4FMOD9CodecMPEG13closeInternalEv")]
pub fn stub_0x95ff4() {
    // IDA 0x95ff4: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x96114 — __ZN4FMOD9CodecMPEG13closeCallbackEP16FMOD_CODEC_STATE
// type: int __fastcall(FMOD::CodecMPEG *)
#[doc(alias = "__ZN4FMOD9CodecMPEG13closeCallbackEP16FMOD_CODEC_STATE")]
pub fn stub_0x96114() {
    // IDA 0x96114: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x96120 — __ZN4FMOD9CodecMPEG19setPositionInternalEijj
// type: int __fastcall(FMOD::File **this, int, unsigned int, unsigned int)
#[doc(alias = "__ZN4FMOD9CodecMPEG19setPositionInternalEijj")]
pub fn stub_0x96120() {
    // IDA 0x96120: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x964d8 — __ZN4FMOD9CodecMPEG19setPositionCallbackEP16FMOD_CODEC_STATEijj
// type: int __fastcall(FMOD::File **, int, unsigned int, unsigned int)
#[doc(alias = "__ZN4FMOD9CodecMPEG19setPositionCallbackEP16FMOD_CODEC_STATEijj")]
pub fn stub_0x964d8() {
    // IDA 0x964d8: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x964e4 — __ZN4FMOD9CodecMPEG16getDescriptionExEv
// type: int *__fastcall(FMOD::CodecMPEG *this)
#[doc(alias = "__ZN4FMOD9CodecMPEG16getDescriptionExEv")]
pub fn stub_0x964e4() {
    // IDA 0x964e4: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x965a4 — __ZN4FMOD9CodecMPEG12readInternalEPvjPj
// type: int __fastcall(FMOD::CodecMPEG *this, char *, unsigned int, unsigned int *)
#[doc(alias = "__ZN4FMOD9CodecMPEG12readInternalEPvjPj")]
pub fn stub_0x965a4() {
    // IDA 0x965a4: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x96854 — __ZN4FMOD9CodecMPEG12readCallbackEP16FMOD_CODEC_STATEPvjPj
// type: int __fastcall(FMOD::CodecMPEG *, char *, unsigned int, unsigned int *)
#[doc(alias = "__ZN4FMOD9CodecMPEG12readCallbackEP16FMOD_CODEC_STATEPvjPj")]
pub fn stub_0x96854() {
    // IDA 0x96854: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x96860 — __ZN4FMOD9CodecMPEG12getPCMLengthEv
// type: int __fastcall(FMOD::File **this)
#[doc(alias = "__ZN4FMOD9CodecMPEG12getPCMLengthEv")]
pub fn stub_0x96860() {
    // IDA 0x96860: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x96a24 — __ZN4FMOD9CodecMPEG10makeTablesEi
// type: int __fastcall(int this, int)
#[doc(alias = "__ZN4FMOD9CodecMPEG10makeTablesEi")]
pub fn stub_0x96a24() {
    // IDA 0x96a24: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x96c4c — __ZN4FMOD9CodecMPEG7initAllEv
// type: int __fastcall(FMOD::CodecMPEG *this, int)
#[doc(alias = "__ZN4FMOD9CodecMPEG7initAllEv")]
pub fn stub_0x96c4c() {
    // IDA 0x96c4c: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x96c9c — __ZN4FMOD9CodecMPEG12openInternalEjP22FMOD_CREATESOUNDEXINFO
// type: int __fastcall(int, __int16)
#[doc(alias = "__ZN4FMOD9CodecMPEG12openInternalEjP22FMOD_CREATESOUNDEXINFO")]
pub fn stub_0x96c9c() {
    // IDA 0x96c9c: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x97670 — __ZN4FMOD9CodecMPEG12openCallbackEP16FMOD_CODEC_STATEjP22FMOD_CREATESOUNDEXINFO
// type: int __fastcall(int, __int16)
#[doc(alias = "__ZN4FMOD9CodecMPEG12openCallbackEP16FMOD_CODEC_STATEjP22FMOD_CREATESOUNDEXINFO")]
pub fn stub_0x97670() {
    // IDA 0x97670: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x976c8 — __GLOBAL__I__ZN4FMOD9mpegcodecE
// type: int()
#[doc(alias = "__GLOBAL__I__ZN4FMOD9mpegcodecE")]
pub fn stub_0x976c8() {
    // IDA 0x976c8: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x976d4 — __ZN4FMOD9CodecMPEG7getBitsEi
// type: unsigned int __fastcall(FMOD::CodecMPEG *this, int)
#[doc(alias = "__ZN4FMOD9CodecMPEG7getBitsEi")]
pub fn stub_0x976d4() {
    // IDA 0x976d4: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x97758 — __ZN4FMOD9CodecMPEG11getBitsFastEi
// type: unsigned int __fastcall(FMOD::CodecMPEG *this, int)
#[doc(alias = "__ZN4FMOD9CodecMPEG11getBitsFastEi")]
pub fn stub_0x97758() {
    // IDA 0x97758: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x977c0 — __ZN4FMOD9CodecMPEG5dct64EPfS1_S1_
// type: __int32 *__fastcall(__int32 *this, float *, float *, float *)
#[doc(alias = "__ZN4FMOD9CodecMPEG5dct64EPfS1_S1_")]
pub fn stub_0x977c0() {
    // IDA 0x977c0: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x981d4 — __ZN4FMOD9CodecMPEG6synthCEPfiiPs
// type: int __fastcall(FMOD::CodecMPEG *this, float *, int, int, __int16 *)
#[doc(alias = "__ZN4FMOD9CodecMPEG6synthCEPfiiPs")]
pub fn stub_0x981d4() {
    // IDA 0x981d4: global static ctor/dtor key. Static init — carrier no-op.
}

// 0x9854c — __ZN4FMOD9CodecMPEG5synthEPvPfii
// type: int __fastcall(FMOD::CodecMPEG *this, __int16 *, float *, int, int)
#[doc(alias = "__ZN4FMOD9CodecMPEG5synthEPvPfii")]
pub fn stub_0x9854c() {
    // IDA 0x9854c: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x986f8 — __ZN4FMOD9CodecMPEG10resetFrameEv
// type: int __fastcall(FMOD::CodecMPEG *this)
#[doc(alias = "__ZN4FMOD9CodecMPEG10resetFrameEv")]
pub fn stub_0x986f8() {
    // IDA 0x986f8: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x987e4 — __ZN4FMOD9CodecMPEG16decodeXingHeaderEPhS1_Pj
// type: int __fastcall(FMOD::CodecMPEG *this, unsigned __int8 *, unsigned __int8 *, unsigned int *)
#[doc(alias = "__ZN4FMOD9CodecMPEG16decodeXingHeaderEPhS1_Pj")]
pub fn stub_0x987e4() {
    // IDA 0x987e4: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9891c — __ZN4FMOD9CodecMPEG12decodeHeaderEPvPiS2_S2_
// type: int __fastcall(FMOD::CodecMPEG *this, unsigned __int8 *, int *, int *, int *)
#[doc(alias = "__ZN4FMOD9CodecMPEG12decodeHeaderEPvPiS2_S2_")]
pub fn stub_0x9891c() {
    // IDA 0x9891c: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x98e9c — __ZN4FMOD9CodecMPEG11decodeFrameEPhPvPj
// type: int __fastcall(FMOD::CodecMPEG *this, unsigned __int8 *, void *, unsigned int *)
#[doc(alias = "__ZN4FMOD9CodecMPEG11decodeFrameEPhPvPj")]
pub fn stub_0x98e9c() {
    // IDA 0x98e9c: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x99024 — __ZN4FMOD9CodecMPEG10getIIStuffEv
// type: int __fastcall(FMOD::CodecMPEG *this)
#[doc(alias = "__ZN4FMOD9CodecMPEG10getIIStuffEv")]
pub fn stub_0x99024() {
    // IDA 0x99024: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x99118 — __ZN4FMOD9CodecMPEG11II_step_twoEPjPA4_A32_fPii
// type: int __fastcall(FMOD::CodecMPEG *this, unsigned int *, float (*)[4][32], int *, int)
#[doc(alias = "__ZN4FMOD9CodecMPEG11II_step_twoEPjPA4_A32_fPii")]
pub fn stub_0x99118() {
    // IDA 0x99118: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x99728 — __ZN4FMOD9CodecMPEG11II_step_oneEPjPi
// type: int __fastcall(FMOD::CodecMPEG *this, unsigned int *, unsigned int *)
#[doc(alias = "__ZN4FMOD9CodecMPEG11II_step_oneEPjPi")]
pub fn stub_0x99728() {
    // IDA 0x99728: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x99a10 — __ZN4FMOD9CodecMPEG12decodeLayer2EPvPj
// type: int __fastcall(FMOD::CodecMPEG *this, __int16 *, unsigned int *)
#[doc(alias = "__ZN4FMOD9CodecMPEG12decodeLayer2EPvPj")]
pub fn stub_0x99a10() {
    // IDA 0x99a10: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x99b08 — __ZN4FMOD9CodecMPEG10initLayer2Ev
// type: int __fastcall(FMOD::CodecMPEG *this)
#[doc(alias = "__ZN4FMOD9CodecMPEG10initLayer2Ev")]
pub fn stub_0x99b08() {
    // IDA 0x99b08: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x99d7c — __ZN4FMOD9CodecMPEG12III_i_stereoEPA32_A18_fPiPNS_9gr_info_sEiii
// type: int __fastcall(int, int, int, _DWORD *, int, int, int)
#[doc(alias = "__ZN4FMOD9CodecMPEG12III_i_stereoEPA32_A18_fPiPNS_9gr_info_sEiii")]
pub fn stub_0x99d7c() {
    // IDA 0x99d7c: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9a240 — __ZN4FMOD9CodecMPEG13III_antialiasEPA18_fPNS_9gr_info_sE
// type: int __fastcall(int, int, _DWORD *)
#[doc(alias = "__ZN4FMOD9CodecMPEG13III_antialiasEPA18_fPNS_9gr_info_sE")]
pub fn stub_0x9a240() {
    // IDA 0x9a240: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9a308 — __ZN4FMOD9CodecMPEG5dct36EPfS1_S1_S1_S1_
// type: float *__fastcall(FMOD::CodecMPEG *this, float *, float *, float *, float *, float *)
#[doc(alias = "__ZN4FMOD9CodecMPEG5dct36EPfS1_S1_S1_S1_")]
pub fn stub_0x9a308() {
    // IDA 0x9a308: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9a9e8 — __ZN4FMOD9CodecMPEG5dct12EPfS1_S1_S1_S1_
// type: __int32 *__fastcall(__int32 *this, float *, float *, float *, float *, float *)
#[doc(alias = "__ZN4FMOD9CodecMPEG5dct12EPfS1_S1_S1_S1_")]
pub fn stub_0x9a9e8() {
    // IDA 0x9a9e8: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9af14 — __ZN4FMOD9CodecMPEG10III_hybridEPA18_fPA32_fiPNS_9gr_info_sE
// type: int __fastcall(int, int, float *, int, _DWORD *)
#[doc(alias = "__ZN4FMOD9CodecMPEG10III_hybridEPA18_fPA32_fiPNS_9gr_info_sE")]
pub fn stub_0x9af14() {
    // IDA 0x9af14: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9b1f8 — __ZN4FMOD9CodecMPEG24III_dequantize_sample_msEPA32_A18_fPiPNS_9gr_info_sEii
// type: int __fastcall(FMOD::CodecMPEG *this, _DWORD *, int *, _DWORD *, int, int)
#[doc(alias = "__ZN4FMOD9CodecMPEG24III_dequantize_sample_msEPA32_A18_fPiPNS_9gr_info_sEii")]
pub fn stub_0x9b1f8() {
    // IDA 0x9b1f8: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9c668 — __ZN4FMOD9CodecMPEG21III_dequantize_sampleEPA18_fPiPNS_9gr_info_sEii
// type: int __fastcall(FMOD::CodecMPEG *, _DWORD *, int *, _DWORD *, int, int)
#[doc(alias = "__ZN4FMOD9CodecMPEG21III_dequantize_sampleEPA18_fPiPNS_9gr_info_sEii")]
pub fn stub_0x9c668() {
    // IDA 0x9c668: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9d78c — __ZN4FMOD9CodecMPEG23III_get_scale_factors_2EPiPNS_9gr_info_sEiS1_
// type: int __fastcall(FMOD::CodecMPEG *, unsigned int *, _DWORD *, int, _DWORD *)
#[doc(alias = "__ZN4FMOD9CodecMPEG23III_get_scale_factors_2EPiPNS_9gr_info_sEiS1_")]
pub fn stub_0x9d78c() {
    // IDA 0x9d78c: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9d920 — __ZN4FMOD9CodecMPEG23III_get_scale_factors_1EPiPNS_9gr_info_sES1_
// type: int __fastcall(FMOD::CodecMPEG *this, unsigned int *, int *, _DWORD *)
#[doc(alias = "__ZN4FMOD9CodecMPEG23III_get_scale_factors_1EPiPNS_9gr_info_sES1_")]
pub fn stub_0x9d920() {
    // IDA 0x9d920: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9dcbc — __ZN4FMOD9CodecMPEG19III_get_side_info_2EPNS_12III_sideinfoEiii
// type: int __fastcall(FMOD::CodecMPEG *, unsigned int *, int, int, int)
#[doc(alias = "__ZN4FMOD9CodecMPEG19III_get_side_info_2EPNS_12III_sideinfoEiii")]
pub fn stub_0x9dcbc() {
    // IDA 0x9dcbc: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9e0e0 — __ZN4FMOD9CodecMPEG19III_get_side_info_1EPNS_12III_sideinfoEiii
// type: int __fastcall(FMOD::CodecMPEG *, unsigned int *, int, int, int)
#[doc(alias = "__ZN4FMOD9CodecMPEG19III_get_side_info_1EPNS_12III_sideinfoEiii")]
pub fn stub_0x9e0e0() {
    // IDA 0x9e0e0: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9e5ac — __ZN4FMOD9CodecMPEG12decodeLayer3EPvPj
// type: int __fastcall(FMOD::CodecMPEG *this, __int16 *, unsigned int *)
#[doc(alias = "__ZN4FMOD9CodecMPEG12decodeLayer3EPvPj")]
pub fn stub_0x9e5ac() {
    // IDA 0x9e5ac: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9eb14 — __ZN4FMOD9CodecMPEG10initLayer3Ei
// type: int __fastcall(FMOD::CodecMPEG *this, int)
#[doc(alias = "__ZN4FMOD9CodecMPEG10initLayer3Ei")]
pub fn stub_0x9eb14() {
    // IDA 0x9eb14: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9fa10 — __ZN4FMOD14CodecOggVorbis17getMemoryUsedImplEPNS_13MemoryTrackerE
// type: int __fastcall(FMOD::CodecOggVorbis *this, FMOD::MemoryTracker *)
#[doc(alias = "__ZN4FMOD14CodecOggVorbis17getMemoryUsedImplEPNS_13MemoryTrackerE")]
pub fn stub_0x9fa10() {
    // IDA 0x9fa10: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9fa34 — __ZN4FMOD14CodecOggVorbis21getMemoryUsedCallbackEP16FMOD_CODEC_STATEPNS_13MemoryTrackerE
// type: int __fastcall(FMOD::CodecOggVorbis *this, FMOD::MemoryTracker *)
#[doc(alias = "__ZN4FMOD14CodecOggVorbis21getMemoryUsedCallbackEP16FMOD_CODEC_STATEPNS_13MemoryTrackerE")]
pub fn stub_0x9fa34() {
    // IDA 0x9fa34: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9fa8c — __ZN4FMOD14CodecOggVorbis18readVorbisCommentsEv
// type: int __fastcall(FMOD::CodecOggVorbis *this)
#[doc(alias = "__ZN4FMOD14CodecOggVorbis18readVorbisCommentsEv")]
pub fn stub_0x9fa8c() {
    // IDA 0x9fa8c: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9fb70 — __ZN4FMOD14CodecOggVorbis19setPositionInternalEijj
// type: int __fastcall(FMOD::CodecOggVorbis *this, int, unsigned int, unsigned int)
#[doc(alias = "__ZN4FMOD14CodecOggVorbis19setPositionInternalEijj")]
pub fn stub_0x9fb70() {
    // IDA 0x9fb70: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9fba0 — __ZN4FMOD14CodecOggVorbis19setPositionCallbackEP16FMOD_CODEC_STATEijj
// type: int __fastcall(FMOD::CodecOggVorbis *, int, unsigned int, unsigned int)
#[doc(alias = "__ZN4FMOD14CodecOggVorbis19setPositionCallbackEP16FMOD_CODEC_STATEijj")]
pub fn stub_0x9fba0() {
    // IDA 0x9fba0: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9fbac — __ZN4FMOD14CodecOggVorbis12readInternalEPvjPj
// type: int __fastcall(FMOD::CodecOggVorbis *this, void *, unsigned int, unsigned int *)
#[doc(alias = "__ZN4FMOD14CodecOggVorbis12readInternalEPvjPj")]
pub fn stub_0x9fbac() {
    // IDA 0x9fbac: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9fd24 — __ZN4FMOD14CodecOggVorbis12readCallbackEP16FMOD_CODEC_STATEPvjPj
// type: int __fastcall(FMOD::CodecOggVorbis *, void *, unsigned int, unsigned int *)
#[doc(alias = "__ZN4FMOD14CodecOggVorbis12readCallbackEP16FMOD_CODEC_STATEPvjPj")]
pub fn stub_0x9fd24() {
    // IDA 0x9fd24: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9fd30 — __ZN4FMOD14CodecOggVorbis13closeInternalEv
// type: int __fastcall(FMOD::CodecOggVorbis *this)
#[doc(alias = "__ZN4FMOD14CodecOggVorbis13closeInternalEv")]
pub fn stub_0x9fd30() {
    // IDA 0x9fd30: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9fd50 — __ZN4FMOD14CodecOggVorbis13closeCallbackEP16FMOD_CODEC_STATE
// type: int __fastcall(FMOD::CodecOggVorbis *)
#[doc(alias = "__ZN4FMOD14CodecOggVorbis13closeCallbackEP16FMOD_CODEC_STATE")]
pub fn stub_0x9fd50() {
    // IDA 0x9fd50: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9fd5c — __ZN4FMOD27FMOD_OggVorbis_SeekCallbackEPvxi
// type: int __fastcall(FMOD *this, int, __int64, int)
#[doc(alias = "__ZN4FMOD27FMOD_OggVorbis_SeekCallbackEPvxi")]
pub fn stub_0x9fd5c() {
    // IDA 0x9fd5c: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9fd80 — __ZN4FMOD14CodecOggVorbis16getDescriptionExEv
// type: int *__fastcall(FMOD::CodecOggVorbis *this)
#[doc(alias = "__ZN4FMOD14CodecOggVorbis16getDescriptionExEv")]
pub fn stub_0x9fd80() {
    // IDA 0x9fd80: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9fe30 — __ZN4FMOD27FMOD_OggVorbis_ReadCallbackEPvmmS0_
// type: unsigned int __fastcall(FMOD *this, unsigned int, unsigned int, FMOD::File *, void *)
#[doc(alias = "__ZN4FMOD27FMOD_OggVorbis_ReadCallbackEPvmmS0_")]
pub fn stub_0x9fe30() {
    // IDA 0x9fe30: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9fe7c — _FMOD_OggVorbis_Free
// type: int __fastcall(int, _DWORD *)
#[doc(alias = "_FMOD_OggVorbis_Free")]
pub fn stub_0x9fe7c() {
    // IDA 0x9fe7c: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0x9fec8 — __ZN4FMOD14CodecOggVorbis12openInternalEjP22FMOD_CREATESOUNDEXINFO
// type: int __fastcall(int)
#[doc(alias = "__ZN4FMOD14CodecOggVorbis12openInternalEjP22FMOD_CREATESOUNDEXINFO")]
pub fn stub_0x9fec8() {
    // IDA 0x9fec8: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa0448 — __ZN4FMOD14CodecOggVorbis12openCallbackEP16FMOD_CODEC_STATEjP22FMOD_CREATESOUNDEXINFO
// type: int __fastcall(int)
#[doc(alias = "__ZN4FMOD14CodecOggVorbis12openCallbackEP16FMOD_CODEC_STATEjP22FMOD_CREATESOUNDEXINFO")]
pub fn stub_0xa0448() {
    // IDA 0xa0448: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa0454 — __ZN4FMOD27FMOD_OggVorbis_TellCallbackEPv
// type: unsigned int __fastcall(FMOD *this, void *)
#[doc(alias = "__ZN4FMOD27FMOD_OggVorbis_TellCallbackEPv")]
pub fn stub_0xa0454() {
    // IDA 0xa0454: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa0474 — _FMOD_OggVorbis_ReAlloc
// type: int __fastcall(int, _DWORD *, int, int)
#[doc(alias = "_FMOD_OggVorbis_ReAlloc")]
pub fn stub_0xa0474() {
    // IDA 0xa0474: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa0500 — _FMOD_OggVorbis_Calloc
// type: int __fastcall(int, int, int)
#[doc(alias = "_FMOD_OggVorbis_Calloc")]
pub fn stub_0xa0500() {
    // IDA 0xa0500: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa0564 — _FMOD_OggVorbis_Malloc
// type: int __fastcall(int, int)
#[doc(alias = "_FMOD_OggVorbis_Malloc")]
pub fn stub_0xa0564() {
    // IDA 0xa0564: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa0614 — __GLOBAL__I_FMOD_OggVorbis_Malloc
// type: int()
#[doc(alias = "__GLOBAL__I_FMOD_OggVorbis_Malloc")]
pub fn stub_0xa0614() {
    // IDA 0xa0614: global static ctor/dtor key. Static init — carrier no-op.
}

// 0xa0620 — __ZN4FMOD13CodecPlaylist12getQuoteDataEPKcPcPi
// type: int __fastcall(FMOD::CodecPlaylist *this, const char *, char *, int *)
#[doc(alias = "__ZN4FMOD13CodecPlaylist12getQuoteDataEPKcPcPi")]
pub fn stub_0xa0620() {
    // IDA 0xa0620: global static ctor/dtor key. Static init — carrier no-op.
}

// 0xa0684 — __ZN4FMOD13CodecPlaylist13closeInternalEv
// type: int __fastcall(FMOD::CodecPlaylist *this)
#[doc(alias = "__ZN4FMOD13CodecPlaylist13closeInternalEv")]
pub fn stub_0xa0684() {
    // IDA 0xa0684: global static ctor/dtor key. Static init — carrier no-op.
}

// 0xa068c — __ZN4FMOD13CodecPlaylist13closeCallbackEP16FMOD_CODEC_STATE
// type: int __fastcall(FMOD::CodecPlaylist *)
#[doc(alias = "__ZN4FMOD13CodecPlaylist13closeCallbackEP16FMOD_CODEC_STATE")]
pub fn stub_0xa068c() {
    // IDA 0xa068c: global static ctor/dtor key. Static init — carrier no-op.
}

// 0xa0698 — __ZN4FMOD13CodecPlaylist12readCallbackEP16FMOD_CODEC_STATEPvjPj
// type: int()
#[doc(alias = "__ZN4FMOD13CodecPlaylist12readCallbackEP16FMOD_CODEC_STATEPvjPj")]
pub fn stub_0xa0698() {
    // IDA 0xa0698: global static ctor/dtor key. Static init — carrier no-op.
}

// 0xa06a0 — __ZN4FMOD13CodecPlaylist19setPositionCallbackEP16FMOD_CODEC_STATEijj
// type: int()
#[doc(alias = "__ZN4FMOD13CodecPlaylist19setPositionCallbackEP16FMOD_CODEC_STATEijj")]
pub fn stub_0xa06a0() {
    // IDA 0xa06a0: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa06a8 — __ZN4FMOD13CodecPlaylist9isNewLineEc
// type: bool __fastcall(FMOD::File **this, char)
#[doc(alias = "__ZN4FMOD13CodecPlaylist9isNewLineEc")]
pub fn stub_0xa06a8() {
    // IDA 0xa06a8: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa0704 — __ZN4FMOD13CodecPlaylist14skipWhiteSpaceEPi
// type: int __fastcall(FMOD::File **this, int *)
#[doc(alias = "__ZN4FMOD13CodecPlaylist14skipWhiteSpaceEPi")]
pub fn stub_0xa0704() {
    // IDA 0xa0704: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa0784 — __ZN4FMOD13CodecPlaylist8readLineEPciPi
// type: int __fastcall(FMOD::File **this, char *, int, int *)
#[doc(alias = "__ZN4FMOD13CodecPlaylist8readLineEPciPi")]
pub fn stub_0xa0784() {
    // IDA 0xa0784: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa0820 — __ZN4FMOD13CodecPlaylist18skipSimpleCommentsEv
// type: int __fastcall(FMOD::File **this)
#[doc(alias = "__ZN4FMOD13CodecPlaylist18skipSimpleCommentsEv")]
pub fn stub_0xa0820() {
    // IDA 0xa0820: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa08b8 — __ZN4FMOD13CodecPlaylist11getPLSTokenEPciPi
// type: int __fastcall(FMOD::File **this, char *, int, int *)
#[doc(alias = "__ZN4FMOD13CodecPlaylist11getPLSTokenEPciPi")]
pub fn stub_0xa08b8() {
    // IDA 0xa08b8: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa0a54 — __ZN4FMOD13CodecPlaylist13getNextXMLTagEPcPiS1_S2_
// type: int __fastcall(FMOD::File **this, char *, int *, char *, int *)
#[doc(alias = "__ZN4FMOD13CodecPlaylist13getNextXMLTagEPcPiS1_S2_")]
pub fn stub_0xa0a54() {
    // IDA 0xa0a54: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa0bb8 — __ZN4FMOD13CodecPlaylist10readSimpleEv
// type: int __fastcall(FMOD::File **this)
#[doc(alias = "__ZN4FMOD13CodecPlaylist10readSimpleEv")]
pub fn stub_0xa0bb8() {
    // IDA 0xa0bb8: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa0c58 — __ZN4FMOD13CodecPlaylist7readPLSEv
// type: int __fastcall(FMOD::File **this)
#[doc(alias = "__ZN4FMOD13CodecPlaylist7readPLSEv")]
pub fn stub_0xa0c58() {
    // IDA 0xa0c58: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa0edc — __ZN4FMOD13CodecPlaylist7readM3UEv
// type: int __fastcall(FMOD::File **this)
#[doc(alias = "__ZN4FMOD13CodecPlaylist7readM3UEv")]
pub fn stub_0xa0edc() {
    // IDA 0xa0edc: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa1218 — __ZN4FMOD13CodecPlaylist7readB4SEv
// type: int __fastcall(FMOD::File **this)
#[doc(alias = "__ZN4FMOD13CodecPlaylist7readB4SEv")]
pub fn stub_0xa1218() {
    // IDA 0xa1218: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa1520 — __ZN4FMOD13CodecPlaylist7readWPLEv
// type: int __fastcall(FMOD::File **this)
#[doc(alias = "__ZN4FMOD13CodecPlaylist7readWPLEv")]
pub fn stub_0xa1520() {
    // IDA 0xa1520: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa1738 — __ZN4FMOD13CodecPlaylist7readASXEv
// type: int __fastcall(FMOD::File **this)
#[doc(alias = "__ZN4FMOD13CodecPlaylist7readASXEv")]
pub fn stub_0xa1738() {
    // IDA 0xa1738: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa1aac — __ZN4FMOD13CodecPlaylist16getDescriptionExEv
// type: int *__fastcall(FMOD::CodecPlaylist *this)
#[doc(alias = "__ZN4FMOD13CodecPlaylist16getDescriptionExEv")]
pub fn stub_0xa1aac() {
    // IDA 0xa1aac: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa1b4c — __ZN4FMOD13CodecPlaylist12openInternalEjP22FMOD_CREATESOUNDEXINFO
// type: int __fastcall(int)
#[doc(alias = "__ZN4FMOD13CodecPlaylist12openInternalEjP22FMOD_CREATESOUNDEXINFO")]
pub fn stub_0xa1b4c() {
    // IDA 0xa1b4c: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa1df4 — __ZN4FMOD13CodecPlaylist12openCallbackEP16FMOD_CODEC_STATEjP22FMOD_CREATESOUNDEXINFO
// type: int __fastcall(int)
#[doc(alias = "__ZN4FMOD13CodecPlaylist12openCallbackEP16FMOD_CODEC_STATEjP22FMOD_CREATESOUNDEXINFO")]
pub fn stub_0xa1df4() {
    // IDA 0xa1df4: FMOD audio struct/helper owned by the audio crate — carrier no-op in core.
}

// 0xa1e4c — __GLOBAL__I__ZN4FMOD13playlistcodecE
// type: int()
#[doc(alias = "__GLOBAL__I__ZN4FMOD13playlistcodecE")]
pub fn stub_0xa1e4c() {
    // IDA 0xa1e4c: global static ctor/dtor key. Static init — carrier no-op.
}

// 0xa1e58 — __ZN4FMOD8CodecRaw13closeInternalEv
// type: int __fastcall(FMOD::CodecRaw *this)
#[doc(alias = "__ZN4FMOD8CodecRaw13closeInternalEv")]
pub fn stub_0xa1e58() {
    // IDA 0xa1e58: global static ctor/dtor key. Static init — carrier no-op.
}

// 0xa1e60 — __ZN4FMOD8CodecRaw16canPointInternalEv
// type: int __fastcall(FMOD::CodecRaw *this)
#[doc(alias = "__ZN4FMOD8CodecRaw16canPointInternalEv")]
pub fn stub_0xa1e60() {
    // IDA 0xa1e60: global static ctor/dtor key. Static init — carrier no-op.
}

// 0xa1e68 — __ZN4FMOD8CodecRaw13closeCallbackEP16FMOD_CODEC_STATE
// type: int __fastcall(FMOD::CodecRaw *)
#[doc(alias = "__ZN4FMOD8CodecRaw13closeCallbackEP16FMOD_CODEC_STATE")]
pub fn stub_0xa1e68() {
    // IDA 0xa1e68: global static ctor/dtor key. Static init — carrier no-op.
}

// 0xa1e74 — __ZN4FMOD8CodecRaw16canPointCallbackEP16FMOD_CODEC_STATE
// type: int __fastcall(FMOD::CodecRaw *)
#[doc(alias = "__ZN4FMOD8CodecRaw16canPointCallbackEP16FMOD_CODEC_STATE")]
pub fn stub_0xa1e74() {
    // IDA 0xa1e74: global static ctor/dtor key. Static init — carrier no-op.
}

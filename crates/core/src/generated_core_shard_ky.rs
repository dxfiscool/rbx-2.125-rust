//! core — generated_core_shard_ky — 120 core stubs EA-sorted asc global gap filler not yet in core (fallback filter).
//! Source: `ida/export.json` (85545 funcs) EA-sorted asc, 120 stubs 0x13179c..0x17afe8 (fallback excludes Reflection|Instance|DataModel|Workspace|Ogre|G3D|RakNet|Network|Replicator|Lua|Script|Yield|FMOD|Audio|Sound|EnumDesc; 21150 filtered, 21030 remaining before -> 20910 after, rbx_core::SharedPtr not boost).
//! Format: // 0xADDR — mangled + #[doc(alias = "demangled")] + todo!("0xADDR") using rbx_core::SharedPtr not boost.
//! Sanitized: boost::shared_ptr -> rbx_core::SharedPtr, boost::weak_ptr -> rbx_core::WeakPtr, boost::intrusive_ptr -> rbx_core::SharedPtr, single quotes and backticks removed.

#![allow(non_camel_case_types, non_snake_case, dead_code, unused_variables, clippy::all)]

use crate::SharedPtr;
const _SHARED_PTR: Option<SharedPtr<u8>> = None;

#[doc(alias = "_fullsize_smooth_downsample")]
// 0x13179c — _fullsize_smooth_downsample
pub fn stub_0x13179c() -> ! {
    todo!("0x13179c _fullsize_smooth_downsample")
}

#[doc(alias = "_jinit_downsampler")]
// 0x131a2c — _jinit_downsampler
pub fn stub_0x131a2c() -> ! {
    todo!("0x131a2c _jinit_downsampler")
}

#[doc(alias = "_fullsize_downsample")]
// 0x131d08 — _fullsize_downsample
// type: int __fastcall(int, int, int, int)
pub fn stub_0x131d08() -> ! {
    todo!("0x131d08 _fullsize_downsample")
}

#[doc(alias = "_jpeg_consume_input")]
// 0x131d68 — _jpeg_consume_input
pub fn stub_0x131d68() -> ! {
    todo!("0x131d68 _jpeg_consume_input")
}

#[doc(alias = "_jpeg_finish_decompress")]
// 0x132034 — _jpeg_finish_decompress
pub fn stub_0x132034() -> ! {
    todo!("0x132034 _jpeg_finish_decompress")
}

#[doc(alias = "_jpeg_read_header")]
// 0x132124 — _jpeg_read_header
pub fn stub_0x132124() -> ! {
    todo!("0x132124 _jpeg_read_header")
}

#[doc(alias = "_jpeg_destroy_decompress")]
// 0x1321b0 — _jpeg_destroy_decompress
pub fn stub_0x1321b0() -> ! {
    todo!("0x1321b0 _jpeg_destroy_decompress")
}

#[doc(alias = "_jpeg_CreateDecompress")]
// 0x1321c0 — _jpeg_CreateDecompress
// type: int __fastcall(void *__b)
pub fn stub_0x1321c0() -> ! {
    todo!("0x1321c0 _jpeg_CreateDecompress")
}

#[doc(alias = "_output_pass_setup")]
// 0x1322d4 — _output_pass_setup
pub fn stub_0x1322d4() -> ! {
    todo!("0x1322d4 _output_pass_setup")
}

#[doc(alias = "_jpeg_read_scanlines")]
// 0x1323d4 — _jpeg_read_scanlines
pub fn stub_0x1323d4() -> ! {
    todo!("0x1323d4 _jpeg_read_scanlines")
}

#[doc(alias = "_jpeg_read_raw_data")]
// 0x1324bc — _jpeg_read_raw_data
pub fn stub_0x1324bc() -> ! {
    todo!("0x1324bc _jpeg_read_raw_data")
}

#[doc(alias = "_jpeg_start_decompress")]
// 0x1325cc — _jpeg_start_decompress
pub fn stub_0x1325cc() -> ! {
    todo!("0x1325cc _jpeg_start_decompress")
}

#[doc(alias = "_get_byte")]
// 0x1326e0 — _get_byte
pub fn stub_0x1326e0() -> ! {
    todo!("0x1326e0 _get_byte")
}

#[doc(alias = "_arith_decode")]
// 0x132744 — _arith_decode
pub fn stub_0x132744() -> ! {
    todo!("0x132744 _arith_decode")
}

#[doc(alias = "_jinit_arith_decoder")]
// 0x1328a4 — _jinit_arith_decoder
pub fn stub_0x1328a4() -> ! {
    todo!("0x1328a4 _jinit_arith_decoder")
}

#[doc(alias = "_process_restart")]
// 0x1329f0 — _process_restart
pub fn stub_0x1329f0() -> ! {
    todo!("0x1329f0 _process_restart")
}

#[doc(alias = "_decode_mcu")]
// 0x132b08 — _decode_mcu
pub fn stub_0x132b08() -> ! {
    todo!("0x132b08 _decode_mcu")
}

#[doc(alias = "_decode_mcu_AC_refine")]
// 0x13307c — _decode_mcu_AC_refine
pub fn stub_0x13307c() -> ! {
    todo!("0x13307c _decode_mcu_AC_refine")
}

#[doc(alias = "_decode_mcu_DC_refine")]
// 0x1334a8 — _decode_mcu_DC_refine
pub fn stub_0x1334a8() -> ! {
    todo!("0x1334a8 _decode_mcu_DC_refine")
}

#[doc(alias = "_decode_mcu_AC_first")]
// 0x133548 — _decode_mcu_AC_first
pub fn stub_0x133548() -> ! {
    todo!("0x133548 _decode_mcu_AC_first")
}

#[doc(alias = "_decode_mcu_DC_first")]
// 0x133758 — _decode_mcu_DC_first
pub fn stub_0x133758() -> ! {
    todo!("0x133758 _decode_mcu_DC_first")
}

#[doc(alias = "_start_pass_0")]
// 0x133980 — _start_pass_0
pub fn stub_0x133980() -> ! {
    todo!("0x133980 _start_pass_0")
}

#[doc(alias = "_start_iMCU_row_0")]
// 0x133d60 — _start_iMCU_row_0
pub fn stub_0x133d60() -> ! {
    todo!("0x133d60 _start_iMCU_row_0")
}

#[doc(alias = "_start_input_pass")]
// 0x133dac — _start_input_pass
pub fn stub_0x133dac() -> ! {
    todo!("0x133dac _start_input_pass")
}

#[doc(alias = "_dummy_consume_data")]
// 0x133db8 — _dummy_consume_data
pub fn stub_0x133db8() -> ! {
    todo!("0x133db8 _dummy_consume_data")
}

#[doc(alias = "_consume_data")]
// 0x133dc0 — _consume_data
pub fn stub_0x133dc0() -> ! {
    todo!("0x133dc0 _consume_data")
}

#[doc(alias = "_decompress_data")]
// 0x133fb4 — _decompress_data
pub fn stub_0x133fb4() -> ! {
    todo!("0x133fb4 _decompress_data")
}

#[doc(alias = "_start_output_pass")]
// 0x134188 — _start_output_pass
pub fn stub_0x134188() -> ! {
    todo!("0x134188 _start_output_pass")
}

#[doc(alias = "_jinit_d_coef_controller")]
// 0x134328 — _jinit_d_coef_controller
pub fn stub_0x134328() -> ! {
    todo!("0x134328 _jinit_d_coef_controller")
}

#[doc(alias = "_decompress_onepass")]
// 0x1344e8 — _decompress_onepass
pub fn stub_0x1344e8() -> ! {
    todo!("0x1344e8 _decompress_onepass")
}

#[doc(alias = "_decompress_smooth_data")]
// 0x1348ec — _decompress_smooth_data
pub fn stub_0x1348ec() -> ! {
    todo!("0x1348ec _decompress_smooth_data")
}

#[doc(alias = "_build_ycc_rgb_table")]
// 0x135064 — _build_ycc_rgb_table
pub fn stub_0x135064() -> ! {
    todo!("0x135064 _build_ycc_rgb_table")
}

#[doc(alias = "_ycc_rgb_convert")]
// 0x135268 — _ycc_rgb_convert
pub fn stub_0x135268() -> ! {
    todo!("0x135268 _ycc_rgb_convert")
}

#[doc(alias = "_null_convert_0")]
// 0x135810 — _null_convert_0
pub fn stub_0x135810() -> ! {
    todo!("0x135810 _null_convert_0")
}

#[doc(alias = "_gray_rgb_convert")]
// 0x135980 — _gray_rgb_convert
pub fn stub_0x135980() -> ! {
    todo!("0x135980 _gray_rgb_convert")
}

#[doc(alias = "_ycck_cmyk_convert")]
// 0x135b68 — _ycck_cmyk_convert
pub fn stub_0x135b68() -> ! {
    todo!("0x135b68 _ycck_cmyk_convert")
}

#[doc(alias = "_start_pass_dcolor")]
// 0x1362a4 — _start_pass_dcolor
pub fn stub_0x1362a4() -> ! {
    todo!("0x1362a4 _start_pass_dcolor")
}

#[doc(alias = "_jinit_color_deconverter")]
// 0x1362a8 — _jinit_color_deconverter
pub fn stub_0x1362a8() -> ! {
    todo!("0x1362a8 _jinit_color_deconverter")
}

#[doc(alias = "_grayscale_convert_0")]
// 0x1364b0 — _grayscale_convert_0
// type: int __fastcall(int, int, int, int, int)
pub fn stub_0x1364b0() -> ! {
    todo!("0x1364b0 _grayscale_convert_0")
}

#[doc(alias = "_start_pass_1")]
// 0x1364ec — _start_pass_1
pub fn stub_0x1364ec() -> ! {
    todo!("0x1364ec _start_pass_1")
}

#[doc(alias = "_jinit_inverse_dct")]
// 0x136de4 — _jinit_inverse_dct
pub fn stub_0x136de4() -> ! {
    todo!("0x136de4 _jinit_inverse_dct")
}

#[doc(alias = "_jpeg_fill_bit_buffer")]
// 0x136e7c — _jpeg_fill_bit_buffer
pub fn stub_0x136e7c() -> ! {
    todo!("0x136e7c _jpeg_fill_bit_buffer")
}

#[doc(alias = "_jpeg_huff_decode")]
// 0x136fcc — _jpeg_huff_decode
pub fn stub_0x136fcc() -> ! {
    todo!("0x136fcc _jpeg_huff_decode")
}

#[doc(alias = "_process_restart_0")]
// 0x1370e4 — _process_restart_0
pub fn stub_0x1370e4() -> ! {
    todo!("0x1370e4 _process_restart_0")
}

#[doc(alias = "_decode_mcu_DC_first_0")]
// 0x137180 — _decode_mcu_DC_first_0
pub fn stub_0x137180() -> ! {
    todo!("0x137180 _decode_mcu_DC_first_0")
}

#[doc(alias = "_decode_mcu_AC_first_0")]
// 0x1373f0 — _decode_mcu_AC_first_0
pub fn stub_0x1373f0() -> ! {
    todo!("0x1373f0 _decode_mcu_AC_first_0")
}

#[doc(alias = "_decode_mcu_DC_refine_0")]
// 0x13766c — _decode_mcu_DC_refine_0
pub fn stub_0x13766c() -> ! {
    todo!("0x13766c _decode_mcu_DC_refine_0")
}

#[doc(alias = "_decode_mcu_AC_refine_0")]
// 0x137774 — _decode_mcu_AC_refine_0
pub fn stub_0x137774() -> ! {
    todo!("0x137774 _decode_mcu_AC_refine_0")
}

#[doc(alias = "_decode_mcu_0")]
// 0x1386c4 — _decode_mcu_0
pub fn stub_0x1386c4() -> ! {
    todo!("0x1386c4 _decode_mcu_0")
}

#[doc(alias = "_jinit_huff_decoder")]
// 0x138bc0 — _jinit_huff_decoder
pub fn stub_0x138bc0() -> ! {
    todo!("0x138bc0 _jinit_huff_decoder")
}

#[doc(alias = "_jpeg_make_d_derived_tbl")]
// 0x138ccc — _jpeg_make_d_derived_tbl
pub fn stub_0x138ccc() -> ! {
    todo!("0x138ccc _jpeg_make_d_derived_tbl")
}

#[doc(alias = "_start_pass_huff_decoder")]
// 0x139938 — _start_pass_huff_decoder
pub fn stub_0x139938() -> ! {
    todo!("0x139938 _start_pass_huff_decoder")
}

#[doc(alias = "_finish_input_pass")]
// 0x139d84 — _finish_input_pass
pub fn stub_0x139d84() -> ! {
    todo!("0x139d84 _finish_input_pass")
}

#[doc(alias = "_reset_input_controller")]
// 0x139d9c — _reset_input_controller
pub fn stub_0x139d9c() -> ! {
    todo!("0x139d9c _reset_input_controller")
}

#[doc(alias = "_jinit_input_controller")]
// 0x139df4 — _jinit_input_controller
pub fn stub_0x139df4() -> ! {
    todo!("0x139df4 _jinit_input_controller")
}

#[doc(alias = "_start_input_pass_0")]
// 0x139e70 — _start_input_pass_0
pub fn stub_0x139e70() -> ! {
    todo!("0x139e70 _start_input_pass_0")
}

#[doc(alias = "_consume_markers")]
// 0x13a278 — _consume_markers
pub fn stub_0x13a278() -> ! {
    todo!("0x13a278 _consume_markers")
}

#[doc(alias = "_start_pass_main_0")]
// 0x13a5a8 — _start_pass_main_0
pub fn stub_0x13a5a8() -> ! {
    todo!("0x13a5a8 _start_pass_main_0")
}

#[doc(alias = "_process_data_simple_main_0")]
// 0x13aae0 — _process_data_simple_main_0
pub fn stub_0x13aae0() -> ! {
    todo!("0x13aae0 _process_data_simple_main_0")
}

#[doc(alias = "_process_data_context_main")]
// 0x13ab84 — _process_data_context_main
pub fn stub_0x13ab84() -> ! {
    todo!("0x13ab84 _process_data_context_main")
}

#[doc(alias = "_process_data_crank_post")]
// 0x13b194 — _process_data_crank_post
pub fn stub_0x13b194() -> ! {
    todo!("0x13b194 _process_data_crank_post")
}

#[doc(alias = "_jinit_d_main_controller")]
// 0x13b1c8 — _jinit_d_main_controller
pub fn stub_0x13b1c8() -> ! {
    todo!("0x13b1c8 _jinit_d_main_controller")
}

#[doc(alias = "_get_sof")]
// 0x13b3b8 — _get_sof
pub fn stub_0x13b3b8() -> ! {
    todo!("0x13b3b8 _get_sof")
}

#[doc(alias = "_examine_app0")]
// 0x13b7a0 — _examine_app0
pub fn stub_0x13b7a0() -> ! {
    todo!("0x13b7a0 _examine_app0")
}

#[doc(alias = "_examine_app14")]
// 0x13ba30 — _examine_app14
pub fn stub_0x13ba30() -> ! {
    todo!("0x13ba30 _examine_app14")
}

#[doc(alias = "_get_interesting_appn")]
// 0x13bb18 — _get_interesting_appn
pub fn stub_0x13bb18() -> ! {
    todo!("0x13bb18 _get_interesting_appn")
}

#[doc(alias = "_save_marker")]
// 0x13be18 — _save_marker
pub fn stub_0x13be18() -> ! {
    todo!("0x13be18 _save_marker")
}

#[doc(alias = "_skip_variable")]
// 0x13c120 — _skip_variable
pub fn stub_0x13c120() -> ! {
    todo!("0x13c120 _skip_variable")
}

#[doc(alias = "_next_marker")]
// 0x13c1f8 — _next_marker
pub fn stub_0x13c1f8() -> ! {
    todo!("0x13c1f8 _next_marker")
}

#[doc(alias = "_read_restart_marker")]
// 0x13c320 — _read_restart_marker
pub fn stub_0x13c320() -> ! {
    todo!("0x13c320 _read_restart_marker")
}

#[doc(alias = "_jpeg_resync_to_restart")]
// 0x13c3d4 — _jpeg_resync_to_restart
pub fn stub_0x13c3d4() -> ! {
    todo!("0x13c3d4 _jpeg_resync_to_restart")
}

#[doc(alias = "_reset_marker_reader")]
// 0x13c544 — _reset_marker_reader
pub fn stub_0x13c544() -> ! {
    todo!("0x13c544 _reset_marker_reader")
}

#[doc(alias = "_jinit_marker_reader")]
// 0x13c56c — _jinit_marker_reader
pub fn stub_0x13c56c() -> ! {
    todo!("0x13c56c _jinit_marker_reader")
}

#[doc(alias = "_jpeg_save_markers")]
// 0x13c66c — _jpeg_save_markers
pub fn stub_0x13c66c() -> ! {
    todo!("0x13c66c _jpeg_save_markers")
}

#[doc(alias = "_read_markers")]
// 0x13c75c — _read_markers
pub fn stub_0x13c75c() -> ! {
    todo!("0x13c75c _read_markers")
}

#[doc(alias = "_use_merged_upsample")]
// 0x13e020 — _use_merged_upsample
pub fn stub_0x13e020() -> ! {
    todo!("0x13e020 _use_merged_upsample")
}

#[doc(alias = "_prepare_for_output_pass")]
// 0x13e100 — _prepare_for_output_pass
pub fn stub_0x13e100() -> ! {
    todo!("0x13e100 _prepare_for_output_pass")
}

#[doc(alias = "_finish_output_pass")]
// 0x13e2dc — _finish_output_pass
pub fn stub_0x13e2dc() -> ! {
    todo!("0x13e2dc _finish_output_pass")
}

#[doc(alias = "_jpeg_calc_output_dimensions")]
// 0x13e30c — _jpeg_calc_output_dimensions
pub fn stub_0x13e30c() -> ! {
    todo!("0x13e30c _jpeg_calc_output_dimensions")
}

#[doc(alias = "_jinit_master_decompress")]
// 0x13e90c — _jinit_master_decompress
pub fn stub_0x13e90c() -> ! {
    todo!("0x13e90c _jinit_master_decompress")
}

#[doc(alias = "_start_pass_merged_upsample")]
// 0x13ed24 — _start_pass_merged_upsample
pub fn stub_0x13ed24() -> ! {
    todo!("0x13ed24 _start_pass_merged_upsample")
}

#[doc(alias = "_merged_1v_upsample")]
// 0x13ed3c — _merged_1v_upsample
pub fn stub_0x13ed3c() -> ! {
    todo!("0x13ed3c _merged_1v_upsample")
}

#[doc(alias = "_h2v1_merged_upsample")]
// 0x13ed88 — _h2v1_merged_upsample
pub fn stub_0x13ed88() -> ! {
    todo!("0x13ed88 _h2v1_merged_upsample")
}

#[doc(alias = "_h2v2_merged_upsample")]
// 0x13f070 — _h2v2_merged_upsample
pub fn stub_0x13f070() -> ! {
    todo!("0x13f070 _h2v2_merged_upsample")
}

#[doc(alias = "_jinit_merged_upsampler")]
// 0x13f2c8 — _jinit_merged_upsampler
pub fn stub_0x13f2c8() -> ! {
    todo!("0x13f2c8 _jinit_merged_upsampler")
}

#[doc(alias = "_merged_2v_upsample")]
// 0x13f580 — _merged_2v_upsample
pub fn stub_0x13f580() -> ! {
    todo!("0x13f580 _merged_2v_upsample")
}

#[doc(alias = "_start_pass_dpost")]
// 0x13f680 — _start_pass_dpost
pub fn stub_0x13f680() -> ! {
    todo!("0x13f680 _start_pass_dpost")
}

#[doc(alias = "_post_process_1pass")]
// 0x13f798 — _post_process_1pass
pub fn stub_0x13f798() -> ! {
    todo!("0x13f798 _post_process_1pass")
}

#[doc(alias = "_post_process_prepass")]
// 0x13f838 — _post_process_prepass
pub fn stub_0x13f838() -> ! {
    todo!("0x13f838 _post_process_prepass")
}

#[doc(alias = "_post_process_2pass")]
// 0x13f934 — _post_process_2pass
pub fn stub_0x13f934() -> ! {
    todo!("0x13f934 _post_process_2pass")
}

#[doc(alias = "_jinit_d_post_controller")]
// 0x13fa24 — _jinit_d_post_controller
pub fn stub_0x13fa24() -> ! {
    todo!("0x13fa24 _jinit_d_post_controller")
}

#[doc(alias = "_start_pass_upsample")]
// 0x13fb08 — _start_pass_upsample
pub fn stub_0x13fb08() -> ! {
    todo!("0x13fb08 _start_pass_upsample")
}

#[doc(alias = "_sep_upsample")]
// 0x13fb20 — _sep_upsample
pub fn stub_0x13fb20() -> ! {
    todo!("0x13fb20 _sep_upsample")
}

#[doc(alias = "_fullsize_upsample")]
// 0x13fc50 — _fullsize_upsample
pub fn stub_0x13fc50() -> ! {
    todo!("0x13fc50 _fullsize_upsample")
}

#[doc(alias = "_noop_upsample")]
// 0x13fc58 — _noop_upsample
pub fn stub_0x13fc58() -> ! {
    todo!("0x13fc58 _noop_upsample")
}

#[doc(alias = "_h2v1_upsample")]
// 0x13fc64 — _h2v1_upsample
pub fn stub_0x13fc64() -> ! {
    todo!("0x13fc64 _h2v1_upsample")
}

#[doc(alias = "_jinit_upsampler")]
// 0x13fdd4 — _jinit_upsampler
pub fn stub_0x13fdd4() -> ! {
    todo!("0x13fdd4 _jinit_upsampler")
}

#[doc(alias = "_h2v2_upsample")]
// 0x140044 — _h2v2_upsample
pub fn stub_0x140044() -> ! {
    todo!("0x140044 _h2v2_upsample")
}

#[doc(alias = "_int_upsample")]
// 0x1401e8 — _int_upsample
pub fn stub_0x1401e8() -> ! {
    todo!("0x1401e8 _int_upsample")
}

#[doc(alias = "_emit_message")]
// 0x140350 — _emit_message
pub fn stub_0x140350() -> ! {
    todo!("0x140350 _emit_message")
}

#[doc(alias = "_reset_error_mgr")]
// 0x1403ac — _reset_error_mgr
pub fn stub_0x1403ac() -> ! {
    todo!("0x1403ac _reset_error_mgr")
}

#[doc(alias = "_jpeg_std_error")]
// 0x1403c4 — _jpeg_std_error
pub fn stub_0x1403c4() -> ! {
    todo!("0x1403c4 _jpeg_std_error")
}

#[doc(alias = "_format_message")]
// 0x140444 — _format_message
// type: int __fastcall(int, char *)
pub fn stub_0x140444() -> ! {
    todo!("0x140444 _format_message")
}

#[doc(alias = "_output_message")]
// 0x140540 — _output_message
pub fn stub_0x140540() -> ! {
    todo!("0x140540 _output_message")
}

#[doc(alias = "_error_exit")]
// 0x140588 — _error_exit
pub fn stub_0x140588() -> ! {
    todo!("0x140588 _error_exit")
}

#[doc(alias = "_jpeg_fdct_float")]
// 0x1405b0 — _jpeg_fdct_float
// type: _DWORD *__fastcall(_DWORD *result, int, int)
pub fn stub_0x1405b0() -> ! {
    todo!("0x1405b0 _jpeg_fdct_float")
}

#[doc(alias = "_jpeg_fdct_ifast")]
// 0x140d6c — _jpeg_fdct_ifast
pub fn stub_0x140d6c() -> ! {
    todo!("0x140d6c _jpeg_fdct_ifast")
}

#[doc(alias = "_jpeg_fdct_islow")]
// 0x141018 — _jpeg_fdct_islow
pub fn stub_0x141018() -> ! {
    todo!("0x141018 _jpeg_fdct_islow")
}

#[doc(alias = "_jpeg_fdct_9x9")]
// 0x14136c — _jpeg_fdct_9x9
pub fn stub_0x14136c() -> ! {
    todo!("0x14136c _jpeg_fdct_9x9")
}

#[doc(alias = "_jpeg_fdct_10x10")]
// 0x141748 — _jpeg_fdct_10x10
pub fn stub_0x141748() -> ! {
    todo!("0x141748 _jpeg_fdct_10x10")
}

#[doc(alias = "_jpeg_fdct_11x11")]
// 0x141b78 — _jpeg_fdct_11x11
pub fn stub_0x141b78() -> ! {
    todo!("0x141b78 _jpeg_fdct_11x11")
}

#[doc(alias = "_jpeg_fdct_12x12")]
// 0x14214c — _jpeg_fdct_12x12
pub fn stub_0x14214c() -> ! {
    todo!("0x14214c _jpeg_fdct_12x12")
}

#[doc(alias = "_png_write_tEXt")]
// 0x17a6d8 — _png_write_tEXt
pub fn stub_0x17a6d8() -> ! {
    todo!("0x17a6d8 _png_write_tEXt")
}

#[doc(alias = "_png_write_zTXt")]
// 0x17a7b0 — _png_write_zTXt
pub fn stub_0x17a7b0() -> ! {
    todo!("0x17a7b0 _png_write_zTXt")
}

#[doc(alias = "_png_write_hIST")]
// 0x17a8c0 — _png_write_hIST
pub fn stub_0x17a8c0() -> ! {
    todo!("0x17a8c0 _png_write_hIST")
}

#[doc(alias = "_png_write_sPLT")]
// 0x17aa58 — _png_write_sPLT
// type: int __fastcall(int, int *)
pub fn stub_0x17aa58() -> ! {
    todo!("0x17aa58 _png_write_sPLT")
}

#[doc(alias = "_png_write_iCCP")]
// 0x17aba4 — _png_write_iCCP
pub fn stub_0x17aba4() -> ! {
    todo!("0x17aba4 _png_write_iCCP")
}

#[doc(alias = "_png_write_PLTE")]
// 0x17ad6c — _png_write_PLTE
pub fn stub_0x17ad6c() -> ! {
    todo!("0x17ad6c _png_write_PLTE")
}

#[doc(alias = "_png_write_chunk")]
// 0x17afac — _png_write_chunk
// type: int __fastcall(int result, int, int, int)
pub fn stub_0x17afac() -> ! {
    todo!("0x17afac _png_write_chunk")
}

#[doc(alias = "_png_write_tIME")]
// 0x17afe8 — _png_write_tIME
pub fn stub_0x17afe8() -> ! {
    todo!("0x17afe8 _png_write_tIME")
}
